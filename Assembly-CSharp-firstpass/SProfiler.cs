...
public class SProfiler : MonoSingleton<SProfiler>
{
	...
	public class Group
	{
		...
		public void flush(double totalTime)
		{
			...
			double num2 = 0.0;
			double num3 = 0.0;
			for (int i = 0; i < this.samples.get_Count(); i++)
			{
				SProfiler.Sample sample = this.samples.get_Item(i);
		...
		{
			double num = (this.count != 0) ? (this.time / (double)this.count) : 0.0;
		...
		{
			double num = (this.count != 0) ? (this.selfTime / (double)this.count) : 0.0;
		...
		{
			string text = this.maxTime.ToString("F2");
			if (this.maxTime > 5.0)
			{
		...
		{
			string text = this.maxSelfTime.ToString("F2");
			if (this.maxSelfTime > 5.0)
			{
				str_maxSelfTime += "<color=red>";
				str_maxSelfTime += text;
		...
		{
			double num = (this.count != 0) ? (this.time / (double)this.count) : 0.0;
			double num2 = (this.groupToCompare.count != 0) ? (this.groupToCompare.time / (double)this.groupToCompare.count) : 0.0;
		...
		{
			double num = (this.count != 0) ? (this.selfTime / (double)this.count) : 0.0;
			double num2 = (this.groupToCompare.count != 0) ? (this.groupToCompare.selfTime / (double)this.groupToCompare.count) : 0.0;
			double num3 = num2 - num;
	...
	public class ProfileObj
	{
		...
		public void PostLoad()
		{
			...
			using (Dictionary<int, SProfiler.Sample>.ValueCollection.Enumerator enumerator = this.samples.get_Values().GetEnumerator())
			{
				while (enumerator.MoveNext())
				{
					SProfiler.Sample current = enumerator.get_Current();
					SProfiler.Group group = null;
					if (!this.groups.TryGetValue(current.name, ref group))
					{
		...
		public void Begin(string name)
		{
			...
			if (!this.samples.TryGetValue(identifity, ref sample))
			{
				...
				SProfiler.Group group = null;
				if (!this.groups.TryGetValue(name, ref group))
				{
					group = new SProfiler.Group();
					group.name = name;
			...
			{
				SProfiler.Sample[] array = new SProfiler.Sample[this.stacks.Length * 2];
				Array.Copy(this.stacks, array, this.stacks.Length);
				this.stacks = array;
			}
		...
		public int getIdentifity(string name)
		{
			...
			int num3 = 0;
			if (!this.idmap.TryGetValue(num, ref num3))
			{
		...
		public void Reset()
		{
			...
			while (enumerator.MoveNext())
			{
				KeyValuePair<int, SProfiler.Sample> current = enumerator.get_Current();
				SProfiler.Sample value = current.get_Value();
				value.reset();
			}
			this.totalTime = 0.0;
			this.compareMode = false;
		...
		{
			int count = obj1.groupList.get_Count();
			for (int i = 0; i < count; i++)
			{
				SProfiler.Group group = obj1.groupList.get_Item(i);
				SProfiler.Group group2 = null;
				if (group.groupToCompare == null)
				{
					if (!obj2.groups.TryGetValue(group.name, ref group2))
					{
		...
		{
			for (int i = 0; i < this.groupList.get_Count(); i++)
			{
				SProfiler.Group group = this.groupList.get_Item(i);
				group.flush(this.totalTime);
			}
			if (this.compareMode)
			{
				for (int j = 0; j < this.compareObj.groupList.get_Count(); j++)
				{
					SProfiler.Group group2 = this.compareObj.groupList.get_Item(j);
			...
			case SProfiler.SortType.AvgTime:
				this.groupList.Sort(delegate(SProfiler.Group x, SProfiler.Group y)
				{
					double num = (x.count != 0) ? (x.time / (double)x.count) : 0.0;
					double num2 = (y.count != 0) ? (y.time / (double)y.count) : 0.0;
					if (num < num2)
					{
						return le;
					}
			...
			case SProfiler.SortType.AvgTime_Diff:
				this.groupList.Sort(delegate(SProfiler.Group x, SProfiler.Group y)
				{
					double num = (x.count != 0) ? (x.time / (double)x.count) : 0.0;
					double num2 = (y.count != 0) ? (y.time / (double)y.count) : 0.0;
					double num3 = (x.groupToCompare.count != 0) ? (x.groupToCompare.time / (double)x.groupToCompare.count) : 0.0;
					double num4 = (y.groupToCompare.count != 0) ? (y.groupToCompare.time / (double)y.groupToCompare.count) : 0.0;
			...
			case SProfiler.SortType.AvgSelfTime:
				this.groupList.Sort(delegate(SProfiler.Group x, SProfiler.Group y)
				{
					double num = (x.count != 0) ? (x.selfTime / (double)x.count) : 0.0;
					double num2 = (y.count != 0) ? (y.selfTime / (double)y.count) : 0.0;
					if (num < num2)
					{
						return le;
					}
			...
			case SProfiler.SortType.AvgSelfTime_Diff:
				this.groupList.Sort(delegate(SProfiler.Group x, SProfiler.Group y)
				{
					double num = (x.count != 0) ? (x.selfTime / (double)x.count) : 0.0;
					double num2 = (y.count != 0) ? (y.selfTime / (double)y.count) : 0.0;
					double num3 = (x.groupToCompare.count != 0) ? (x.groupToCompare.selfTime / (double)x.groupToCompare.count) : 0.0;
					double num4 = (y.groupToCompare.count != 0) ? (y.groupToCompare.selfTime / (double)y.groupToCompare.count) : 0.0;
					double num5 = num - num3;
					double num6 = num2 - num4;
					if (num5 < num6)
					{
		...
		private void DrawGroupColumns()
		{
			...
			for (int i = 0; i < SProfiler.ProfileObj.column_props.Length; i++)
			{
				...
				if ((!columnProp.showInCompareMode || this.compareMode) && (!this.compareMode || !columnProp.hidden))
				{
		...
		private void DrawButtons()
		{
			...
			GUI.color = (SProfiler.paused ? Color.red : color);
			...
			num3 += num + num2;
			GUI.color = color;
			GUI.color = (SProfiler.showData ? color : Color.red);
			...
			num3 += num + num2;
			GUI.color = color;
			GUI.color = (SProfiler.disableGameInput ? Color.red : color);
			...
			{
				num3 += num + num2;
				GUI.color = (SProfiler.showColomn_Time ? color : Color.red);
				...
				num3 += num + num2;
				GUI.color = (SProfiler.showColomn_SelfTime ? color : Color.red);
				...
				num3 += num + num2;
				GUI.color = (SProfiler.showColomn_AvgTime ? color : Color.red);
				...
				num3 += num + num2;
				GUI.color = (SProfiler.showColomn_AvgSelfTime ? color : Color.red);
				...
				num3 += num + num2;
				GUI.color = (SProfiler.showColomn_MaxTime ? color : Color.red);
				...
				num3 += num + num2;
				GUI.color = (SProfiler.showColomn_MaxSelfTime ? color : Color.red);
				...
				num3 += num + num2;
				GUI.color = (SProfiler.showColomn_Count ? color : Color.red);
		...
		public void SaveProfileData()
		{
			...
			string path = string.Format("{0}/{1}.spf", DebugHelper.logRootPath, text);
		...
		private void RefreshProfileFiles()
		{
			...
			this.profileFiles = Directory.GetFiles(logRootPath, "*.spf", 0);
		}
		...
		public void DrawGroups_MaxSelfTimeOnly()
		{
			...
			string text = string.Empty;
			if (this.groupListForSort.get_Count() > 0)
			{
				...
				for (int j = 0; j < this.groupListForSort.get_Count(); j++)
				{
					SProfiler.Group group2 = this.groupListForSort.get_Item(j);
			...
			num += num3 + 10f;
			GUI.color = (SProfiler.paused ? Color.red : GUI.color);
		...
		public void SaveToFile(string path)
		{
			...
			try
			{
				...
				fileStream = new FileStream(path, 2, 2, 0);
		...
		public static SProfiler.ProfileObj LoadFromFile(string path)
		{
			...
			{
				BinaryFormatter binaryFormatter = new BinaryFormatter();
				fileStream = new FileStream(path, 3, 1);
	...
	private static void EnableEventSystem(bool enable)
	{
		...
		{
			SProfiler.gui_eventSystem = EventSystem.get_current();
		}
		if (SProfiler.gui_eventSystem != null)
		{
