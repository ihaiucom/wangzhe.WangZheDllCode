...
internal class ConsoleViewMobile : IConsoleView
{
	...
	public ConsoleViewMobile(ConsoleWindow InParent)
	{
		...
		this.States.RegisterStateByAttributes<CommandDisplayAttribute>(typeof(CommandDisplayAttribute).get_Assembly(), new object[]
		{
			this.ParentWindow,
			this
		});
	...
	public void OnConsole(int InWindowID)
	{
		...
		{
			DebugHelper.Assert(false, string.Format("ConsoleViewMobile Exception: {0} , Stack: {1}", ex.get_Message(), ex.get_StackTrace()));
		}
	}
