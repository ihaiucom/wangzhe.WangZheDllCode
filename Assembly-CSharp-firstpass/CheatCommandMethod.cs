...
public class CheatCommandMethod : CheatCommandBase
{
	...
	public CheatCommandMethod(MethodInfo InMethod, CheatCommandEntryAttribute InEntryAttr, CheatCommandEntryMethodAttribute InMethodAttr)
	{
		...
		this.Comment = ((array != null && array.Length > 0) ? array[array.Length - 1] : this.Method.get_Name());
	...
	protected void CacheArgumentDescriptions()
	{
		...
		if (parameters != null && parameters.Length > 0)
		{
			...
			{
				this.ArgumentDescs[i] = new ArgumentDescriptionAttribute(ArgumentDescriptionAttribute.EDefaultValueTag.Tag, i, parameters[i].get_ParameterType(), parameters[i].get_Name(), (parameters[i].get_DefaultValue() != null) ? parameters[i].get_DefaultValue().ToString() : string.Empty, new object[0]);
			}
		}
	}
