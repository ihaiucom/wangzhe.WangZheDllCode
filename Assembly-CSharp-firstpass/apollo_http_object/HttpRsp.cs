...
{
	public class HttpRsp : IPackable, IUnpackable, tsf4g_csharp_interface
	{
		public ResponseStatus stResponseStatus;

		public HttpHeaders stHttpHeaders;

		public ResponseContent stResponseContent;

		private uint[] has_bits_ = new uint[1];

		public HttpRsp()
		{
			this.stResponseStatus = new ResponseStatus();
			this.stHttpHeaders = new HttpHeaders();
			this.stResponseContent = new ResponseContent();
		}
		...
		{
			TdrError.ErrorType errorType = this.stResponseStatus.construct();
			...
			errorType = this.stResponseContent.construct();
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
