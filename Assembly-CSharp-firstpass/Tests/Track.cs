	...
	public class Track
	{
		...
		public float maxValue
		{
			get
			{
				...
				{
					if (this.samples.get_Item(i) > num)
					{
						num = this.samples.get_Item(i);
					}
				}
				return num;
			}
		...
		public bool hasSamples
		{
			get
			{
				return this.samples.get_Count() > 0;
			}
		}
		...
		{
			if (this.samples.get_Count() > this.maxSampling)
			{
				this.samples.RemoveRange(0, this.samples.get_Count() - this.maxSampling);
			}
		}
		...
		{
			if (reverseIndex >= 0 && this.samples.get_Count() - reverseIndex > 0)
			{
				this.samples.set_Item(this.samples.get_Count() - reverseIndex - 1, InValue);
			}
		}
		...
		private float CalcX(int InIndex)
		{
			...
			return (float)(InIndex * Screen.width) * (1f - Track.Clip * 2f) / (float)this.samples.get_Count();
		}
		...
		public void OnRender(float InMinValue, float InMaxValue)
		{
			...
			for (int i = 0; i < this.samples.get_Count(); i++)
			{
				float num2 = this.samples.get_Item(i) - this.CalcMin(InMinValue);
