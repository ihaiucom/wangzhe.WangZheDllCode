...
namespace Apollo
{
	public class ApolloHttpRequest : ApolloObject, IApolloHttpRequest
	{
		...
		private byte[] version = Encoding.get_UTF8().GetBytes("HTTP/1.1");

		private byte[] method = Encoding.get_UTF8().GetBytes("GET");
		...
		public event OnRespondHandler ResponseEvent
		{
			...
			add
			{
				this.ResponseEvent = (OnRespondHandler)Delegate.Combine(this.ResponseEvent, value);
			}
			[MethodImpl(32)]
			remove
			{
				this.ResponseEvent = (OnRespondHandler)Delegate.Remove(this.ResponseEvent, value);
			}
		}
		...
		{
			string @string = Encoding.get_UTF8().GetString(bytearray);
		...
		public ApolloResult SetHttpVersion(string version)
		{
			...
			byte[] bytes = Encoding.get_UTF8().GetBytes(version);
			this.version = bytes;
		...
		{
			return string.IsNullOrEmpty(URL) || (URL.get_Length() > 1 && (URL.Substring(0, 1) == "/" || URL.Substring(0, 1) == "?")) || (URL.get_Length() > 7 && URL.Substring(0, 7) == "http://");
		}
		...
		public ApolloResult SetURL(string URL)
		{
			...
			byte[] bytes = Encoding.get_UTF8().GetBytes(URL);
			this.URL = bytes;
			return ApolloResult.Success;
		}
		...
		public ApolloResult SetMethod(string method)
		{
			...
			byte[] bytes = Encoding.get_UTF8().GetBytes(method);
			this.method = bytes;
			return ApolloResult.Success;
		}
