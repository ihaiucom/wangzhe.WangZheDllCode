...
namespace Apollo
{
	internal class ApolloPayService : ApolloObject, IApolloPayService, IApolloServiceBase
	{
		...
		public event OnApolloPaySvrEvenHandle PayEvent
		{
			...
			add
			{
				this.PayEvent = (OnApolloPaySvrEvenHandle)Delegate.Combine(this.PayEvent, value);
			}
			[MethodImpl(32)]
			remove
			{
				this.PayEvent = (OnApolloPaySvrEvenHandle)Delegate.Remove(this.PayEvent, value);
			}
		}
			...
			if (this.actionCallbackCollection.ContainsKey(apolloAction.Action))
			{
				...
				if (apolloActionBufferBase != null)
				{
					...
					if (apolloActionDelegate != null)
					{
						...
						{
							ADebug.LogError("OnApolloPayActionProc exception:" + ex);
						}
					}
					else
					{
		...
		private static extern bool apollo_pay_Initialize([MarshalAs(42)] byte[] data, int len);

		[DllImport("apollo", CallingConvention = CallingConvention.Cdecl)]
		private static extern bool apollo_pay_Pay([MarshalAs(42)] byte[] data, int len);

		[DllImport("apollo", CallingConvention = CallingConvention.Cdecl)]
		private static extern bool apollo_pay_Pay4Mounth([MarshalAs(42)] byte[] data, int len);

		[DllImport("apollo", CallingConvention = CallingConvention.Cdecl)]
		private static extern bool apollo_pay_Dipose();

		[DllImport("apollo", CallingConvention = CallingConvention.Cdecl)]
		private static extern void apollo_pay_action([MarshalAs(42)] byte[] data, int len);
	}
}
