...
namespace Apollo
{
	internal class ApolloAccountService : ApolloObject, IApolloAccountService, IApolloServiceBase
	{
		...
		public event AccountInitializeHandle InitializeEvent
		{
			...
			add
			{
				this.InitializeEvent = (AccountInitializeHandle)Delegate.Combine(this.InitializeEvent, value);
			}
			[MethodImpl(32)]
			remove
			{
				this.InitializeEvent = (AccountInitializeHandle)Delegate.Remove(this.InitializeEvent, value);
			}
		}
		...
		public event AccountLoginHandle LoginEvent
		{
			...
			add
			{
				this.LoginEvent = (AccountLoginHandle)Delegate.Combine(this.LoginEvent, value);
			}
			[MethodImpl(32)]
			remove
			{
				this.LoginEvent = (AccountLoginHandle)Delegate.Remove(this.LoginEvent, value);
			}
		}
		...
		public event AccountLogoutHandle LogoutEvent
		{
			...
			add
			{
				this.LogoutEvent = (AccountLogoutHandle)Delegate.Combine(this.LogoutEvent, value);
			}
			[MethodImpl(32)]
			remove
			{
				this.LogoutEvent = (AccountLogoutHandle)Delegate.Remove(this.LogoutEvent, value);
			}
		}
		...
		public event RefreshAccessTokenHandler RefreshAtkEvent
		{
			...
			add
			{
				this.RefreshAtkEvent = (RefreshAccessTokenHandler)Delegate.Combine(this.RefreshAtkEvent, value);
			}
			[MethodImpl(32)]
			remove
			{
				this.RefreshAtkEvent = (RefreshAccessTokenHandler)Delegate.Remove(this.RefreshAtkEvent, value);
			}
		}
		...
		public event RealNameAuthHandler RealNameAutEvent
		{
			...
			add
			{
				this.RealNameAutEvent = (RealNameAuthHandler)Delegate.Combine(this.RealNameAutEvent, value);
			}
			[MethodImpl(32)]
			remove
			{
				this.RealNameAutEvent = (RealNameAuthHandler)Delegate.Remove(this.RealNameAutEvent, value);
			}
		}
		...
		private static extern bool apollo_account_initialize([MarshalAs(42)] byte[] data, int len);
		...
		private static extern ApolloResult apollo_account_getRecord(ulong objId, [MarshalAs(20)] StringBuilder pAccountInfo, int size);
		...
		[DllImport("apollo", CallingConvention = CallingConvention.Cdecl)]
		private static extern void apollo_account_realname_auth([MarshalAs(42)] byte[] data, int len);
	}
}
