...
namespace PigeonCoopToolkit.Effects.Trails
{
	public abstract class TrailRenderer_Base : MonoBehaviour
	{
		...
		protected virtual void LateUpdate()
		{
			...
			{
				CombineInstance[] array = new CombineInstance[current.get_Value().Count];
				for (int i = 0; i < current.get_Value().Count; i++)
				{
					array[i] = new CombineInstance
					{
						mesh = current.get_Value()[i].Mesh,
						subMeshIndex = 0,
				...
				this.DrawMesh(mesh, current.get_Key());
		...
		protected void AddPoint(PCTrailPoint newPoint, Vector3 pos)
		{
			...
			newPoint.PointNumber = ((this._activeTrail.Points.Count == 0) ? 0 : (this._activeTrail.Points[this._activeTrail.Points.Count - 1].PointNumber + 1));
			this.InitialiseNewPoint(newPoint);
			newPoint.SetDistanceFromStart((this._activeTrail.Points.Count == 0) ? 0f : (this._activeTrail.Points[this._activeTrail.Points.Count - 1].GetDistanceFromStart() + Vector3.Distance(this._activeTrail.Points[this._activeTrail.Points.Count - 1].Position, pos)));
			if (this.TrailData.UseForwardOverride)
			{
				newPoint.Forward = (this.TrailData.ForwardOverrideRelative ? this._t.TransformDirection(this.TrailData.ForwardOverride.normalized) : this.TrailData.ForwardOverride.normalized);
			}
		...
		private void GenerateMesh(PCTrail trail)
		{
			...
			for (int i = 0; i < trail.Points.Count; i++)
			{
				...
				if (pCTrailPoint.TimeActive() <= this.TrailData.Lifetime)
				{
					...
					Color color = this.TrailData.StretchColorToFit ? (this.TrailData.UsingSimpleColor ? Color.Lerp(this.TrailData.SimpleColorOverLifeStart, this.TrailData.SimpleColorOverLifeEnd, 1f - (float)num / (float)trail.activePointCount / 2f) : this.TrailData.ColorOverLife.Evaluate(1f - (float)num / (float)trail.activePointCount / 2f)) : (this.TrailData.UsingSimpleColor ? Color.Lerp(this.TrailData.SimpleColorOverLifeStart, this.TrailData.SimpleColorOverLifeEnd, num2) : this.TrailData.ColorOverLife.Evaluate(num2));
					float d = this.TrailData.StretchSizeToFit ? (this.TrailData.UsingSimpleSize ? Mathf.Lerp(this.TrailData.SimpleSizeOverLifeStart, this.TrailData.SimpleSizeOverLifeEnd, 1f - (float)num / (float)trail.activePointCount / 2f) : this.TrailData.SizeOverLife.Evaluate(1f - (float)num / (float)trail.activePointCount / 2f)) : (this.TrailData.UsingSimpleSize ? Mathf.Lerp(this.TrailData.SimpleSizeOverLifeStart, this.TrailData.SimpleSizeOverLifeEnd, num2) : this.TrailData.SizeOverLife.Evaluate(num2));
		...
		private void UpdatePoints(PCTrail line, float deltaTime)
		{
			...
			{
				line.Points[i].Update(this._noDecay ? 0f : deltaTime);
			}
		}
