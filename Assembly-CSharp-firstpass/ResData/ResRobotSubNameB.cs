...
namespace ResData
{
	public class ResRobotSubNameB : tsf4g_csharp_interface, IUnpackable
	{
		public uint dwObjId;
		...
		{
			if (cutVer == 0u || ResRobotSubNameB.CURRVERSION < cutVer)
			{
				cutVer = ResRobotSubNameB.CURRVERSION;
			}
			if (ResRobotSubNameB.BASEVERSION > cutVer)
			{
			...
			{
				if ((ulong)num > (ulong)ResRobotSubNameB.LENGTH_szName)
				{
		...
		public TdrError.ErrorType load(ref TdrReadBuf srcBuf, uint cutVer)
		{
			...
			{
				this.szName_ByteArray = new byte[ResRobotSubNameB.LENGTH_szName];
			}
