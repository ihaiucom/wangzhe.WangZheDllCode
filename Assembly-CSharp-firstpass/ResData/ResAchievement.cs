...
namespace ResData
{
	public class ResAchievement : tsf4g_csharp_interface, IUnpackable
	{
		public uint dwID;
		...
		public ResRewardInfo[] astReward;

		public byte bShowProcess;
		...
		public ResAchievement()
		{
			...
			this.astReward = new ResRewardInfo[3];
			for (int i = 0; i < 3; i++)
			{
		...
		{
			if (cutVer == 0u || ResAchievement.CURRVERSION < cutVer)
			{
				cutVer = ResAchievement.CURRVERSION;
			}
			if (ResAchievement.BASEVERSION > cutVer)
			{
			...
			errorType = srcBuf.readUInt32(ref this.dwType);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			errorType = srcBuf.readUInt32(ref this.dwClassification);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			...
			errorType = srcBuf.readUInt32(ref this.dwBgImage);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			errorType = srcBuf.readUInt8(ref this.bHideForegroundImage);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			...
			{
				if ((ulong)num > (ulong)ResAchievement.LENGTH_szName)
				{
			...
			{
				if ((ulong)num3 > (ulong)ResAchievement.LENGTH_szDesc)
				{
					return TdrError.ErrorType.TDR_ERR_STR_LEN_TOO_BIG;
				}
			...
			if (num5 > (uint)this.szTips_ByteArray.GetLength(0))
			{
				if ((ulong)num5 > (ulong)ResAchievement.LENGTH_szTips)
				{
					return TdrError.ErrorType.TDR_ERR_STR_LEN_TOO_BIG;
				}
			...
			errorType = srcBuf.readCString(ref this.szTips_ByteArray, (int)num5);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			if (this.szTips_ByteArray[(int)(num5 - 1u)] != 0)
			{
				return TdrError.ErrorType.TDR_ERR_STR_LEN_CONFLICT;
			}
			...
			errorType = srcBuf.readUInt32(ref this.dwPreAchievementID);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			errorType = srcBuf.readUInt32(ref this.dwAchieveLvl);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			errorType = srcBuf.readUInt8(ref this.bIsTriggerPopUp);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			errorType = srcBuf.readUInt8(ref this.bIsPopUpDelay);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			...
			errorType = srcBuf.readUInt32(ref this.dwDoneType);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			errorType = srcBuf.readUInt32(ref this.dwDoneCondi);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			errorType = srcBuf.readUInt32(ref this.dwDoneParm);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			for (int i = 0; i < 3; i++)
			{
				errorType = this.astReward[i].unpack(ref srcBuf, cutVer);
				if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
				{
					return errorType;
				}
			}
			errorType = srcBuf.readUInt8(ref this.bShowProcess);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
		...
		public TdrError.ErrorType load(ref TdrReadBuf srcBuf, uint cutVer)
		{
			...
			{
				this.szName_ByteArray = new byte[ResAchievement.LENGTH_szName];
			}
			...
			{
				this.szDesc_ByteArray = new byte[ResAchievement.LENGTH_szDesc];
			}
			...
			{
				this.szTips_ByteArray = new byte[ResAchievement.LENGTH_szTips];
			}
			errorType = srcBuf.readCString(ref this.szTips_ByteArray, num3);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			...
			for (int i = 0; i < 3; i++)
			{
				errorType = this.astReward[i].load(ref srcBuf, cutVer);
				if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
				{
					return errorType;
				}
