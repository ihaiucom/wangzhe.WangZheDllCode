...
namespace ResData
{
	public class ResHeroSelectTextData : tsf4g_csharp_interface, IUnpackable
	{
		public uint dwID;
		...
		{
			if (cutVer == 0u || ResHeroSelectTextData.CURRVERSION < cutVer)
			{
				cutVer = ResHeroSelectTextData.CURRVERSION;
			}
			if (ResHeroSelectTextData.BASEVERSION > cutVer)
			{
			...
			errorType = srcBuf.readUInt32(ref this.dwTag);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			...
			{
				if ((ulong)num > (ulong)ResHeroSelectTextData.LENGTH_szContent)
				{
		...
		public TdrError.ErrorType load(ref TdrReadBuf srcBuf, uint cutVer)
		{
			...
			{
				this.szContent_ByteArray = new byte[ResHeroSelectTextData.LENGTH_szContent];
			}
