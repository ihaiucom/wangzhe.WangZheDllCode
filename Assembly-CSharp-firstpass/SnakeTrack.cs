...
public static class SnakeTrack
{
	...
	public class MemoryLogger
	{
		...
		public void SetCheckPoint()
		{
			if (this._stream != null)
			{
				...
				this._checkPoint = this._stream.get_Position();
			}
		}
		...
		public void ClearCheckPoint()
		{
			if (this._checkPoint > 0L && this._stream != null)
			{
				...
				long num = this._stream.get_Position() - this._checkPoint;
				if (num > 0L)
				{
		...
		public MemoryStream RetriveStream(bool stopTraceImmediately)
		{
			...
			byte[] buffer = this._stream.GetBuffer();
			int num = (int)((this._checkPoint > 0L) ? this._checkPoint : this._stream.get_Position());
	...
	public static List<MemoryStream> RetrieveStreams(bool stopTraceImmediately)
	{
		...
		for (int i = 0; i < 3; i++)
		{
			list.Add((SnakeTrack._traceEntrys[i] != null) ? SnakeTrack._traceEntrys[i].RetriveStream(stopTraceImmediately) : null);
		}
		return list;
	}
}
