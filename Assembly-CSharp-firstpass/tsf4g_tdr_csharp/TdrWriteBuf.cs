...
namespace tsf4g_tdr_csharp
{
	public class TdrWriteBuf : TdrBufBase
	{
		...
		public TdrError.ErrorType writeInt16(short src)
		{
			...
			{
				this.beginPtr[this.position++] = unsafeBytes[i];
			}
			return TdrError.ErrorType.TDR_NO_ERROR;
		}

		private unsafe byte[] GetUnsafeBytes(short src)
		{
			int num = 2;
			for (int i = 0; i < num; i++)
			{
				this._shortBytes[i] = *(ref src + i);
			}
		...
		private unsafe byte[] GetUnsafeBytes(int src)
		{
			if (this._intBytes != null && this._intBytes.Length != 0)
			{
				int num = 4;
				for (int i = 0; i < num; i++)
				{
					this._intBytes[i] = *(ref src + i);
				}
			}
		...
		private unsafe byte[] GetUnsafeBytes(long src)
		{
			if (this._longBytes != null && this._longBytes.Length != 0)
			{
				int num = 8;
				for (int i = 0; i < num; i++)
				{
					this._longBytes[i] = *(ref src + i);
				}
			}
