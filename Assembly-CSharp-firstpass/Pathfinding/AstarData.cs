...
namespace Pathfinding
{
	...
	public class AstarData
	{
		...
		{
			AstarData.<GetUpdateableGraphs>c__Iterator0 <GetUpdateableGraphs>c__Iterator = new AstarData.<GetUpdateableGraphs>c__Iterator0();
			<GetUpdateableGraphs>c__Iterator.<>f__this = this;
			AstarData.<GetUpdateableGraphs>c__Iterator0 expr_0E = <GetUpdateableGraphs>c__Iterator;
			expr_0E.$PC = -2;
			return expr_0E;
		}
		...
		private bool getNearest(VInt3 position, List<object> objs, ref long minDist, ref TriangleMeshNode nearestNode)
		{
			...
			{
				TriangleMeshNode triangleMeshNode = objs.get_Item(i) as TriangleMeshNode;
				if (triangleMeshNode == null)
				{
					return false;
				}
		...
		private TriangleMeshNode checkObjIntersects(ref int edge, VInt3 start, VInt3 end, int gridX, int gridY)
		{
			...
			for (int i = 0; i < objs.get_Count(); i++)
			{
				...
				for (int j = 0; j < 3; j++)
				{
					...
					{
						bool inCondition;
						VInt3 vInt = Polygon.IntersectionPoint(ref array[num3], ref array[num4], ref start, ref end, out inCondition);
		...
		public TriangleMeshNode IntersectByRasterizer(VInt3 start, VInt3 end, out int edge)
		{
			...
			int num7 = (num > 0) ? (this.rasterizer.cellSize - num6) : (-num6 - 1);
			...
			while (num5 >= 0 && num11 >= 0 && num11 < num9)
			{
				...
				int num13 = Mathf.Abs((num != 0) ? IntMath.Divide(num2 * num7, num) : num2);
				int num14 = num12 % this.rasterizer.cellSize;
				int num15 = (num2 >= 0) ? (this.rasterizer.cellSize - num14) : (-num14 - 1);
		...
		public void FindGraphTypes()
		{
			...
			{
				Type type = array[i];
				for (Type baseType = type.get_BaseType(); baseType != null; baseType = baseType.get_BaseType())
				{
		...
		public Type GetGraphType(string type)
		{
			...
			{
				if (this.graphTypes[i].get_Name() == type)
				{
