...
namespace Pathfinding
{
	public class GraphUpdateObject
	{
		...
		public virtual void RevertFromBackup()
		{
			...
			int num = 0;
			for (int i = 0; i < this.changedNodes.get_Count(); i++)
			{
				this.changedNodes.get_Item(i).Penalty = this.backupData.get_Item(num);
				num++;
				this.changedNodes.get_Item(i).Flags = this.backupData.get_Item(num);
				num++;
				this.changedNodes.get_Item(i).position = this.backupPositionData.get_Item(i);
			}
