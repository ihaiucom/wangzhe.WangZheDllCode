...
public class CheatCommandRegister : Singleton<CheatCommandRegister>
{
	...
	{
		Type returnType = InMethod.get_ReturnType();
		...
		if (parameters != null && parameters.Length > 0)
		{
			for (int i = 0; i < parameters.Length; i++)
			{
				...
				if (parameterInfo.get_IsOut())
				{
					DebugHelper.Assert(false, string.Format("method command argument can't be out parameter. Method:{0}, Parameter:{1} {2}", InMethod.get_Name(), parameterInfo.get_ParameterType().get_Name(), parameterInfo.get_Name()));
					return false;
				}
				if (parameterInfo.get_ParameterType().get_IsByRef())
				{
					DebugHelper.Assert(false, string.Format("method command argument can't be ref parameter. Method:{0}, Parameter:{1} {2}", InMethod.get_Name(), parameterInfo.get_ParameterType().get_Name(), parameterInfo.get_Name()));
					return false;
				}
				IArgumentDescription description = Singleton<ArgumentDescriptionRepository>.instance.GetDescription(parameterInfo.get_ParameterType());
				DebugHelper.Assert(description != null);
				if (!description.AcceptAsMethodParameter(parameterInfo.get_ParameterType()))
				{
					DebugHelper.Assert(false, string.Format("unsupported argument type for method command. Method:{0}, {1}, {2}", InMethod.get_Name(), parameterInfo.get_ParameterType().get_Name(), parameterInfo.get_Name()));
					return false;
				}
			}
		}
