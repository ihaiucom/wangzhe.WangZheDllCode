...
{
	public class COMDT_ADDFIREND_INFO : ProtocolObject
	{
		...
		public static readonly int CLASS_ID = 259;

		public COMDT_ADDFRIEND_PVP stPvp
		{
			get
			{
				return this.dataObject as COMDT_ADDFRIEND_PVP;
			}
			set
			{
		...
		{
			if (cutVer == 0u || COMDT_ADDFIREND_INFO.CURRVERSION < cutVer)
			{
				cutVer = COMDT_ADDFIREND_INFO.CURRVERSION;
			}
			if (COMDT_ADDFIREND_INFO.BASEVERSION > cutVer)
			{
		...
		private void select_1_1(long selector)
		{
			...
			else if (!(this.dataObject is COMDT_ADDFRIEND_PVP))
			{
				if (this.dataObject != null)
				{
					this.dataObject.Release();
				}
				this.dataObject = (COMDT_ADDFRIEND_PVP)ProtocolObjectPool.Get(COMDT_ADDFRIEND_PVP.CLASS_ID);
			}
		}

		public override int GetClassID()
		{
			return COMDT_ADDFIREND_INFO.CLASS_ID;
		}
