...
{
	public class COMDT_GUILD_RECRUIT_INFO : ProtocolObject
	{
		public ulong ullSenderUid;

		public byte bSenderLevel;
		...
		public byte[] szSendName;

		public uint dwSenderHeadID;

		public byte[] szSenderHeadUrl;
		...
		public static readonly uint LENGTH_szSendName = 64u;

		public static readonly uint LENGTH_szSenderHeadUrl = 256u;

		public static readonly int CLASS_ID = 354;

		public COMDT_GUILD_RECRUIT_INFO()
		{
			this.szGuildName = new byte[32];
			this.szSendName = new byte[64];
			this.szSenderHeadUrl = new byte[256];
		}
		...
		public override TdrError.ErrorType unpack(ref TdrReadBuf srcBuf, uint cutVer)
		{
			...
			TdrError.ErrorType errorType = srcBuf.readUInt64(ref this.ullSenderUid);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			errorType = srcBuf.readUInt8(ref this.bSenderLevel);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			...
			{
				if ((ulong)num > (ulong)COMDT_GUILD_RECRUIT_INFO.LENGTH_szGuildName)
				{
					return TdrError.ErrorType.TDR_ERR_STR_LEN_TOO_BIG;
				}
			...
			if (num3 > (uint)this.szSendName.GetLength(0))
			{
				if ((ulong)num3 > (ulong)COMDT_GUILD_RECRUIT_INFO.LENGTH_szSendName)
				{
					return TdrError.ErrorType.TDR_ERR_STR_LEN_TOO_BIG;
				}
				this.szSendName = new byte[num3];
			}
			if (1u > num3)
			{
				return TdrError.ErrorType.TDR_ERR_STR_LEN_TOO_SMALL;
			}
			errorType = srcBuf.readCString(ref this.szSendName, (int)num3);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			if (this.szSendName[(int)(num3 - 1u)] != 0)
			{
				return TdrError.ErrorType.TDR_ERR_STR_LEN_CONFLICT;
			}
			int num4 = TdrTypeUtil.cstrlen(this.szSendName) + 1;
			if ((ulong)num3 != (ulong)((long)num4))
			{
				return TdrError.ErrorType.TDR_ERR_STR_LEN_CONFLICT;
			}
			errorType = srcBuf.readUInt32(ref this.dwSenderHeadID);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			...
			if (num5 > (uint)this.szSenderHeadUrl.GetLength(0))
			{
				if ((ulong)num5 > (ulong)COMDT_GUILD_RECRUIT_INFO.LENGTH_szSenderHeadUrl)
				{
					return TdrError.ErrorType.TDR_ERR_STR_LEN_TOO_BIG;
				}
				this.szSenderHeadUrl = new byte[num5];
			}
			if (1u > num5)
			{
				return TdrError.ErrorType.TDR_ERR_STR_LEN_TOO_SMALL;
			}
			errorType = srcBuf.readCString(ref this.szSenderHeadUrl, (int)num5);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			if (this.szSenderHeadUrl[(int)(num5 - 1u)] != 0)
			{
				return TdrError.ErrorType.TDR_ERR_STR_LEN_CONFLICT;
			}
			int num6 = TdrTypeUtil.cstrlen(this.szSenderHeadUrl) + 1;
		...
		{
			return COMDT_GUILD_RECRUIT_INFO.CLASS_ID;
		}

		public override void OnRelease()
		{
			this.ullSenderUid = 0uL;
			this.bSenderLevel = 0;
			this.ullGuildID = 0uL;
			this.iLogicWorldID = 0;
			this.dwSendTime = 0u;
			this.dwSenderHeadID = 0u;
			this.bLimitGrade = 0;
			this.bLimitLevel = 0;
		}
	}
