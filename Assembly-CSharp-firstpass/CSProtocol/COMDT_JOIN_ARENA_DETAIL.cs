...
{
	public class COMDT_JOIN_ARENA_DETAIL : ProtocolObject
	{
		...
		public static readonly int CLASS_ID = 527;

		public COMDT_ARENA_INFO stSucc
		{
			get
			{
				return this.dataObject as COMDT_ARENA_INFO;
			}
			set
			{
		...
		{
			if (cutVer == 0u || COMDT_JOIN_ARENA_DETAIL.CURRVERSION < cutVer)
			{
				cutVer = COMDT_JOIN_ARENA_DETAIL.CURRVERSION;
			}
			if (COMDT_JOIN_ARENA_DETAIL.BASEVERSION > cutVer)
			{
		...
		private void select_0_0(long selector)
		{
			...
			else if (!(this.dataObject is COMDT_ARENA_INFO))
			{
				if (this.dataObject != null)
				{
					this.dataObject.Release();
				}
				this.dataObject = (COMDT_ARENA_INFO)ProtocolObjectPool.Get(COMDT_ARENA_INFO.CLASS_ID);
			}
		}

		public override int GetClassID()
		{
			return COMDT_JOIN_ARENA_DETAIL.CLASS_ID;
		}
