...
{
	public class COMDT_TRANS_CONTEXT_GETPROCESS_STUDENTINFO : ProtocolObject
	{
		...
		public static readonly int CLASS_ID = 432;

		public COMDT_TRANS_CONTEXT_GETPROCESS_STUDENTINFO()
		{
			this.stMasterUniq = (COMDT_ACNT_UNIQ)ProtocolObjectPool.Get(COMDT_ACNT_UNIQ.CLASS_ID);
			this.astStudentUniq = new COMDT_ACNT_UNIQ[6];
			for (int i = 0; i < 6; i++)
			{
		...
		{
			if (cutVer == 0u || COMDT_TRANS_CONTEXT_GETPROCESS_STUDENTINFO.CURRVERSION < cutVer)
			{
				cutVer = COMDT_TRANS_CONTEXT_GETPROCESS_STUDENTINFO.CURRVERSION;
			}
			if (COMDT_TRANS_CONTEXT_GETPROCESS_STUDENTINFO.BASEVERSION > cutVer)
			{
		...
		{
			return COMDT_TRANS_CONTEXT_GETPROCESS_STUDENTINFO.CLASS_ID;
		}
