...
{
	public class SCDT_COINDRAW_DETAIL : ProtocolObject
	{
		...
		public static readonly int CLASS_ID = 987;

		public SCDT_COINDRAW_SUCC stSucc
		{
			get
			{
				return this.dataObject as SCDT_COINDRAW_SUCC;
			}
			set
			{
		...
		{
			if (cutVer == 0u || SCDT_COINDRAW_DETAIL.CURRVERSION < cutVer)
			{
				cutVer = SCDT_COINDRAW_DETAIL.CURRVERSION;
			}
			if (SCDT_COINDRAW_DETAIL.BASEVERSION > cutVer)
			{
		...
		private void select_0_0(long selector)
		{
			...
			else if (!(this.dataObject is SCDT_COINDRAW_SUCC))
			{
				if (this.dataObject != null)
				{
					this.dataObject.Release();
				}
				this.dataObject = (SCDT_COINDRAW_SUCC)ProtocolObjectPool.Get(SCDT_COINDRAW_SUCC.CLASS_ID);
			}
		}

		public override int GetClassID()
		{
			return SCDT_COINDRAW_DETAIL.CLASS_ID;
		}
