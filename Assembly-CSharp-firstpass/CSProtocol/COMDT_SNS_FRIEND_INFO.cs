...
namespace CSProtocol
{
	public class COMDT_SNS_FRIEND_INFO : ProtocolObject
	{
		...
		public static readonly int CLASS_ID = 402;

		public COMDT_SNS_FRIEND_INFO()
		{
		...
		public override TdrError.ErrorType unpack(ref TdrReadBuf srcBuf, uint cutVer)
		{
			...
			{
				if ((ulong)num > (ulong)COMDT_SNS_FRIEND_INFO.LENGTH_szNickName)
				{
					return TdrError.ErrorType.TDR_ERR_STR_LEN_TOO_BIG;
				}
			...
			if (num3 > (uint)this.szRoleName.GetLength(0))
			{
				if ((ulong)num3 > (ulong)COMDT_SNS_FRIEND_INFO.LENGTH_szRoleName)
				{
					return TdrError.ErrorType.TDR_ERR_STR_LEN_TOO_BIG;
				}
			...
			errorType = srcBuf.readCString(ref this.szRoleName, (int)num3);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			if (this.szRoleName[(int)(num3 - 1u)] != 0)
			{
				return TdrError.ErrorType.TDR_ERR_STR_LEN_CONFLICT;
			}
			...
			if (num5 > (uint)this.szHeadUrl.GetLength(0))
			{
				if ((ulong)num5 > (ulong)COMDT_SNS_FRIEND_INFO.LENGTH_szHeadUrl)
				{
					return TdrError.ErrorType.TDR_ERR_STR_LEN_TOO_BIG;
				}
			...
			errorType = srcBuf.readCString(ref this.szHeadUrl, (int)num5);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			if (this.szHeadUrl[(int)(num5 - 1u)] != 0)
			{
				return TdrError.ErrorType.TDR_ERR_STR_LEN_CONFLICT;
			}
			...
			errorType = srcBuf.readUInt32(ref this.dwLvl);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
		...
		{
			return COMDT_SNS_FRIEND_INFO.CLASS_ID;
		}
