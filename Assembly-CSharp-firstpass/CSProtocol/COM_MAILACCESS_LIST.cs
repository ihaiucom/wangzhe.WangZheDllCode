...
namespace CSProtocol
{
	public class COM_MAILACCESS_LIST : ProtocolObject
	{
		...
		public static readonly int CLASS_ID = 275;
		...
		{
			if (cutVer == 0u || COM_MAILACCESS_LIST.CURRVERSION < cutVer)
			{
				cutVer = COM_MAILACCESS_LIST.CURRVERSION;
			}
			if (COM_MAILACCESS_LIST.BASEVERSION > cutVer)
			{
				return TdrError.ErrorType.TDR_ERR_CUTVER_TOO_SMALL;
			}
			TdrError.ErrorType errorType = destBuf.writeUInt8(this.bAccessCnt);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
		...
		public override TdrError.ErrorType unpack(ref TdrReadBuf srcBuf, uint cutVer)
		{
			...
			TdrError.ErrorType errorType = srcBuf.readUInt8(ref this.bAccessCnt);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			...
			for (int i = 0; i < (int)this.bAccessCnt; i++)
			{
				errorType = this.astAccess[i].unpack(ref srcBuf, cutVer);
				if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
				{
					return errorType;
				}
		...
		{
			return COM_MAILACCESS_LIST.CLASS_ID;
		}
