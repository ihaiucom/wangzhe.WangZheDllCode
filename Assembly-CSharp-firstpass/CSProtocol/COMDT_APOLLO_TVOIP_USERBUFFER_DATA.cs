...
namespace CSProtocol
{
	public class COMDT_APOLLO_TVOIP_USERBUFFER_DATA : ProtocolObject
	{
		...
		public static readonly int CLASS_ID = 491;
		...
		{
			if (cutVer == 0u || COMDT_APOLLO_TVOIP_USERBUFFER_DATA.CURRVERSION < cutVer)
			{
				cutVer = COMDT_APOLLO_TVOIP_USERBUFFER_DATA.CURRVERSION;
			}
			if (COMDT_APOLLO_TVOIP_USERBUFFER_DATA.BASEVERSION > cutVer)
			{
		...
		private void select_1_2(long selector)
		{
			if (selector != 1L)
			{
				...
				else if (!(this.dataObject is COMDT_APOLLO_TVOIP_JOIN_ROOM))
				{
					if (this.dataObject != null)
					{
						this.dataObject.Release();
			...
			else if (!(this.dataObject is COMDT_APOLLO_TVOIP_CREATE_ROOM))
			{
				if (this.dataObject != null)
				{
					this.dataObject.Release();
		...
		{
			return COMDT_APOLLO_TVOIP_USERBUFFER_DATA.CLASS_ID;
		}
