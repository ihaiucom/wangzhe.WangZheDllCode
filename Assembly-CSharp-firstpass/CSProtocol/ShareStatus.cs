...
{
	public class ShareStatus : tsf4g_csharp_interface, IPackable, IUnpackable
	{
		public byte[] szOpenid;

		public int iShareStatus;
		...
		{
			if (cutVer == 0u || ShareStatus.CURRVERSION < cutVer)
			{
				cutVer = ShareStatus.CURRVERSION;
			}
			if (ShareStatus.BASEVERSION > cutVer)
			{
			...
			errorType = destBuf.writeInt32(this.iShareStatus);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
		...
		public TdrError.ErrorType unpack(ref TdrReadBuf srcBuf, uint cutVer)
		{
			...
			{
				if ((ulong)num > (ulong)ShareStatus.LENGTH_szOpenid)
				{
					return TdrError.ErrorType.TDR_ERR_STR_LEN_TOO_BIG;
				}
			...
			errorType = srcBuf.readInt32(ref this.iShareStatus);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
