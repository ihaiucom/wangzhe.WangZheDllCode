...
{
	public class SCPKG_CMD_NTF_FRIEND_LOGIN_STATUS : ProtocolObject
	{
		public COMDT_ACNT_UNIQ stUin;

		public byte bLoginStatus;
		...
		public static readonly int CLASS_ID = 1071;

		public SCPKG_CMD_NTF_FRIEND_LOGIN_STATUS()
		{
		...
		{
			if (cutVer == 0u || SCPKG_CMD_NTF_FRIEND_LOGIN_STATUS.CURRVERSION < cutVer)
			{
				cutVer = SCPKG_CMD_NTF_FRIEND_LOGIN_STATUS.CURRVERSION;
			}
			if (SCPKG_CMD_NTF_FRIEND_LOGIN_STATUS.BASEVERSION > cutVer)
			{
			...
			errorType = destBuf.writeUInt8(this.bLoginStatus);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			errorType = destBuf.writeUInt8(this.bFriendType);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
		...
		public override TdrError.ErrorType unpack(ref TdrReadBuf srcBuf, uint cutVer)
		{
			...
			errorType = srcBuf.readUInt8(ref this.bLoginStatus);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			errorType = srcBuf.readUInt8(ref this.bFriendType);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
		...
		{
			return SCPKG_CMD_NTF_FRIEND_LOGIN_STATUS.CLASS_ID;
		}
