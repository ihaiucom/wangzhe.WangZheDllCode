...
namespace TMPro
{
	...
	public class TextMeshPro : MonoBehaviour
	{
		...
		private void CreateMaterialInstance()
		{
			...
			Material expr_20 = material2;
			expr_20.name += " Instance";
		...
		private void ParseInputText()
		{
			...
			TextMeshPro.TextInputSources inputSource = this.m_inputSource;
			if (inputSource != TextMeshPro.TextInputSources.Text)
			{
				if (inputSource == TextMeshPro.TextInputSources.SetText)
				{
		...
		private void GenerateTextMesh()
		{
			...
			this.m_fontScale = this.m_fontSize / this.m_fontAssetArray[this.m_fontIndex].fontInfo.PointSize * (this.m_isOrthographic ? 1f : 0.1f);
			...
			while (this.m_char_buffer[num9] != 0)
			{
				...
				if (this.m_isRichText && num10 == 60 && this.ValidateHtmlTag(this.m_char_buffer, num9 + 1, out num8))
				{
					...
					{
						this.m_fontScale = this.m_currentFontSize / this.m_fontAssetArray[this.m_fontIndex].fontInfo.PointSize * (this.m_isOrthographic ? 1f : 0.1f);
				...
				else
				{
					...
					else if ((this.m_fontStyle & FontStyles.SmallCaps) == FontStyles.SmallCaps || (this.m_style & FontStyles.SmallCaps) == FontStyles.SmallCaps)
					{
						if (char.IsLower((char)num10))
						{
							this.m_fontScale = this.m_currentFontSize * 0.8f / this.m_fontAssetArray[this.m_fontIndex].fontInfo.PointSize * (this.m_isOrthographic ? 1f : 0.1f);
							num10 -= 32;
						}
						else
						{
					...
					this.m_fontAssetArray[this.m_fontIndex].characterDictionary.TryGetValue(num10, ref this.m_cached_GlyphInfo);
					if (this.m_cached_GlyphInfo == null)
					{
						if (char.IsLower((char)num10))
						{
							if (this.m_fontAssetArray[this.m_fontIndex].characterDictionary.TryGetValue(num10 - 32, ref this.m_cached_GlyphInfo))
							{
								num10 -= 32;
							}
						}
						else if (char.IsUpper((char)num10) && this.m_fontAssetArray[this.m_fontIndex].characterDictionary.TryGetValue(num10 + 32, ref this.m_cached_GlyphInfo))
						{
							num10 += 32;
						}
						if (this.m_cached_GlyphInfo == null)
					...
					{
						this.m_maxAscender = ((this.m_maxAscender > num14) ? this.m_maxAscender : num14);
					}
					...
					if (num10 != 32 && num10 != 9 && num10 != 10 && num10 != 13)
					{
						...
						if (this.m_xAdvance + this.m_cached_GlyphInfo.xAdvance * this.m_fontScale > this.m_marginWidth + 0.0001f && !this.m_textContainer.isDefaultWidth)
						{
							...
							if (this.enableWordWrapping && this.m_characterCount != this.m_firstVisibleCharacterOfLine)
							{
								...
								num21 = ((num21 < num22) ? num21 : num22);
								if (this.m_textInfo.characterInfo[this.m_firstVisibleCharacterOfLine].isVisible)
								{
									this.m_maxDescender = ((this.m_maxDescender < num21) ? this.m_maxDescender : num21);
								}
						...
						color.a = ((this.m_fontColor32.a < color.a) ? this.m_fontColor32.a : color.a);
					...
					if (num10 == 10 || this.m_characterCount == num - 1)
					{
						...
						num27 = ((num27 < num28) ? num27 : num28);
						this.m_maxDescender = ((this.m_maxDescender < num27) ? this.m_maxDescender : num27);
						this.m_textInfo.lineInfo[this.m_lineNumber].firstCharacterIndex = this.m_firstVisibleCharacterOfLine;
					...
					if (num10 != 13 && num10 != 10 && this.m_pageNumber < 16)
					{
						...
						this.m_textInfo.pageInfo[this.m_pageNumber].descender = ((num15 < this.m_textInfo.pageInfo[this.m_pageNumber].descender) ? num15 : this.m_textInfo.pageInfo[this.m_pageNumber].descender);
					...
					if (this.m_enableWordWrapping || this.m_overflowMode == TextOverflowModes.Truncate || this.m_overflowMode == TextOverflowModes.Ellipsis)
					{
						...
						else if (((flag3 || this.m_isCharacterWrappingEnabled) && this.m_characterCount < num - 1 && !this.m_fontAsset.lineBreakingInfo.leadingCharacters.ContainsKey(num10) && !this.m_fontAsset.lineBreakingInfo.followingCharacters.ContainsKey((int)this.m_VisibleCharacters.get_Item(this.m_characterCount + 1))) || flag4)
						{
							this.SaveWordWrappingState(ref this.m_SavedWordWrapState, num9, this.m_characterCount);
						}
					}
			...
			else
			{
				...
				{
					int lineNumber2 = (int)this.m_textInfo.characterInfo[j].lineNumber;
					...
					if (this.m_textInfo.characterInfo[j].isVisible)
					{
						...
						float num37 = this.m_uvLineOffset * (float)lineNumber2 % 1f + this.m_uvOffset.x;
						...
						if ((this.m_maxVisibleCharacters != -1 && j >= this.m_maxVisibleCharacters) || (this.m_maxVisibleLines != -1 && lineNumber2 >= this.m_maxVisibleLines) || (this.m_overflowMode == TextOverflowModes.Page && (int)this.m_textInfo.characterInfo[j].pageNumber != this.m_pageToDisplay))
						{
					...
					int num47 = j;
					characterInfo[num47].bottomLeft = characterInfo[num47].bottomLeft + b3;
					TMP_CharacterInfo[] characterInfo2 = this.m_textInfo.characterInfo;
					int num48 = j;
					characterInfo2[num48].topRight = characterInfo2[num48].topRight + b3;
					TMP_CharacterInfo[] characterInfo3 = this.m_textInfo.characterInfo;
					int num49 = j;
					characterInfo3[num49].topLine = characterInfo3[num49].topLine + b3.y;
					TMP_CharacterInfo[] characterInfo4 = this.m_textInfo.characterInfo;
					int num50 = j;
					characterInfo4[num50].bottomLine = characterInfo4[num50].bottomLine + b3.y;
					TMP_CharacterInfo[] characterInfo5 = this.m_textInfo.characterInfo;
					int num51 = j;
					characterInfo5[num51].baseLine = characterInfo5[num51].baseLine + b3.y;
					...
					if (lineNumber2 != num34 || j == this.m_characterCount - 1)
					{
						if (lineNumber2 != num34)
						{
					...
					{
						int num52 = (j == this.m_characterCount - 1 && char.IsLetterOrDigit(character2)) ? j : (j - 1);
						flag6 = false;
						num32++;
						TMP_LineInfo[] lineInfo2 = this.m_textInfo.lineInfo;
						int num53 = lineNumber2;
						lineInfo2[num53].wordCount = lineInfo2[num53].wordCount + 1;
						TMP_WordInfo tMP_WordInfo = default(TMP_WordInfo);
						tMP_WordInfo.firstCharacterIndex = num35;
						tMP_WordInfo.lastCharacterIndex = num52;
						tMP_WordInfo.characterCount = num52 - num35 + 1;
				...
				this.m_textInfo.wordCount = (int)((num32 != 0 && this.m_characterCount > 0) ? ((short)num32) : 1);
		...
		private void ResizeMeshBuffers(int size)
		{
			...
			Array.Resize<Vector3>(ref this.m_vertices, num);
			Array.Resize<Vector3>(ref this.m_normals, num);
			Array.Resize<Vector4>(ref this.m_tangents, num);
			Array.Resize<Color32>(ref this.m_vertColors, num);
			Array.Resize<Vector2>(ref this.m_uvs, num);
			Array.Resize<Vector2>(ref this.m_uv2s, num);
			Array.Resize<int>(ref this.m_triangles, num2);
			Array.Resize<Color>(ref this.m_tempColors, num);
			Array.Resize<Vector3>(ref this.m_tempVertices, num);
		...
		private void AdjustLineOffset(int startIndex, int endIndex, float offset)
		{
			...
			for (int i = startIndex; i <= endIndex; i++)
			{
				if (this.m_textInfo.characterInfo[i].isVisible)
				{
					...
					TMP_CharacterInfo[] characterInfo2 = this.m_textInfo.characterInfo;
					int num2 = i;
					characterInfo2[num2].topRight = characterInfo2[num2].topRight - b;
					TMP_CharacterInfo[] characterInfo3 = this.m_textInfo.characterInfo;
					int num3 = i;
					characterInfo3[num3].bottomLine = characterInfo3[num3].bottomLine - b.y;
					TMP_CharacterInfo[] characterInfo4 = this.m_textInfo.characterInfo;
					int num4 = i;
					characterInfo4[num4].topLine = characterInfo4[num4].topLine - b.y;
				...
				case 116:
					...
					case 242:
						...
						if (num6 == 1020)
						{
							...
							this.m_baselineOffset = 0f;
							this.m_fontScale = this.m_currentFontSize / this.m_fontAsset.fontInfo.PointSize * (this.m_isOrthographic ? 1f : 0.1f);
							return true;
						}
						if (num6 == 98)
						{
		...
		public void SetText(string text, float arg0, float arg1, float arg2)
		{
			...
			if (this.m_input_CharArray.Length < text.get_Length())
			{
				this.m_input_CharArray = new char[Mathf.NextPowerOfTwo(text.get_Length() + 1)];
			}
			this.old_text = text;
			this.old_arg1 = 255f;
			this.old_arg2 = 255f;
		...
		public void SetCharArray(char[] charArray)
		{
			if (charArray != null && charArray.Length != 0)
			{
				...
				this.m_inputSource = TextMeshPro.TextInputSources.SetCharArray;
				this.havePropertiesChanged = true;
				this.isInputParsingRequired = true;
			}
		}
