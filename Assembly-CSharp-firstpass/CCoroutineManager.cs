...
public class CCoroutineManager : Singleton<CCoroutineManager>
{
	...
	private DateTime m_lastEvaluateTime = DateTime.get_Now();
	...
	public override void Init()
	{
		...
		this.m_lastFrameTime = DateTime.get_Now();
		this.m_waitTillTime = DateTime.get_Now();
		this.m_frameRate = 30;
	...
	public void Update()
	{
		if (DateTime.get_Now() <= this.m_waitTillTime)
		{
			return;
		}
		this.m_lastFrameTime = DateTime.get_Now();
		while ((DateTime.get_Now() - this.m_lastFrameTime).get_TotalMilliseconds() < this.m_deltaMillSecondsPerFrame && this.m_execlusiveCoroutineList.get_Count() > 0)
		{
			LinkedListNode<CCoroutine> last = this.m_execlusiveCoroutineList.get_Last();
			IEnumerator iter = last.get_Value().iter;
			if (!iter.MoveNext())
			{
			...
			{
				object current = iter.get_Current();
				if (current == null)
				{
					this.m_execlusiveCoroutineList.Remove(last);
				}
	...
	private void ReEvaluateTimeCost()
	{
		...
		{
			this.m_lastEvaluateTime = DateTime.get_Now();
			this.m_evaluateStart = true;
			return;
		}
		DateTime now = DateTime.get_Now();
		double totalMilliseconds = (now - this.m_lastEvaluateTime).get_TotalMilliseconds();
	...
	private void ProcessWaitForSeconds(CWaitForSecond handle)
	{
		if (handle.m_interval > 0f)
		{
			...
			this.m_waitTillTime = DateTime.get_Now().AddSeconds((double)handle.m_interval);
		}
	}
