...
public class GSDKsys : MonoSingleton<GSDKsys>
{
	...
	private void OnCheckMna(CUIEvent uiEvent)
	{
		...
		this.StartKartin(DateTime.get_Now().ToString());
	}
	...
	private void SetBarColor(Image image, int bRed)
	{
		if (image)
		{
			if (bRed == 2)
			{
				image.set_color(new Color(0.882f, 0.21168f, 0.21952f));
			}
			else if (bRed == 1)
			{
				image.set_color(new Color(8.83568f, 0.70168f, 0.22736001f));
			}
			else if (bRed == 0)
			{
				image.set_color(new Color(0.08624f, 0.71736f, 0.29792f));
			}
			else
			{
				image.set_color(new Color(0.18816f, 0.2156f, 0.27832f));
			}
		}
	}
	...
	private void SetTextColor(Text text, int bRed, string str)
	{
		if (text)
		{
			if (bRed == 2)
			{
				text.set_text(string.Format("<color=#ff3638>{0}</color>", str));
			}
			else if (bRed == 1)
			{
				text.set_text(string.Format("<color=#ffb33a>{0}</color>", str));
			}
			else if (bRed == 0)
			{
				text.set_text(string.Format("<color=#16B74C>{0}</color>", str));
			}
			else
			{
				text.set_text(string.Format("<color=#303747>{0}</color>", str));
			}
		}
	}
	...
	public void UpdateUI(bool bDefault = false)
	{
			...
			if (transform3 != null)
			{
				...
				if (transform5)
				{
					if (i == 1)
					{
						if (jump_network == 4)
						{
							transform5.GetComponent<Text>().set_text("路由器延迟");
						}
						else
						{
							transform5.GetComponent<Text>().set_text("基站延迟");
						}
					}
					else if (i == 0)
					{
						transform5.GetComponent<Text>().set_text("社区宽带延迟");
					}
					else if (i == 2)
					{
						transform5.GetComponent<Text>().set_text("共享WIFI设备");
					}
				}
				if (jump_network == 4)
				{
					...
					if (i == 0)
					{
						...
						transform7.GetComponent<Text>().set_text(this.m_lastQuery.export_desc.ToString());
					...
					else if (i == 1)
					{
						...
						transform7.GetComponent<Text>().set_text(this.m_lastQuery.router_desc.ToString());
					...
					else if (i == 2)
					{
						...
						transform7.GetComponent<Text>().set_text(this.m_lastQuery.terminal_desc.ToString());
		...
		if (transform8)
		{
			...
			if (component)
			{
				if (jump_network == 4)
				{
					component.set_text("WIFI");
				}
				else if (jump_network == 3)
				{
					component.set_text("4G");
				}
				else if (jump_network == 2)
				{
					component.set_text("3G");
				}
				else if (jump_network == 1)
				{
					component.set_text("2G");
				}
				else
				{
					component.set_text("非WIFI网络");
				}
			}
		}
		...
		if (transform9)
		{
			transform9.GetComponent<Text>().set_text(this.m_lastQuery.signal_desc);
		}
		this.setStartKartinButtonEnable();
	}
