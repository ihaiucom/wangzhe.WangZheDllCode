...
namespace Assets.Scripts.GameLogic
{
	internal class TeleportTargetSelector : Singleton<TeleportTargetSelector>
	{
			...
			for (int i = 0; i < count; i++)
			{
				...
				if (poolObjHandle && !(poolObjHandle == hostPlayer.Captain))
				{
					...
					if (!handle.ActorControl.IsDeadState && handle.IsHostCamp() && handle.InCamera)
					{
						...
						if (((ulong)dwSkillTargetFilter & 1uL << (int)(handle.TheActorMeta.ActorType & (ActorTypeDef)31)) <= 0uL)
						{
		...
		private void RefreshTeleportTargetEffect()
		{
			...
			{
				KeyValuePair<PoolObjHandle<ActorRoot>, GameObject> current = enumerator.get_Current();
				if (!this.m_CanTeleportActorList.Contains(current.get_Key()))
				{
					if (current.get_Value() != null)
					{
						Singleton<CGameObjectPool>.GetInstance().RecycleGameObject(current.get_Value());
					}
			...
			for (int i = 0; i < list.get_Count(); i++)
			{
				...
				if (this.m_CachedTeleportGameObjects.ContainsKey(poolObjHandle))
				{
					this.m_CachedTeleportGameObjects.Remove(poolObjHandle);
				}
			}
			if (this.m_NeedConfirm)
			{
				...
				{
					for (int j = 0; j < this.m_CanTeleportActorList.get_Count(); j++)
					{
						PoolObjHandle<ActorRoot> poolObjHandle2 = this.m_CanTeleportActorList.get_Item(j);
			...
			{
				for (int k = 0; k < this.m_CanTeleportActorList.get_Count(); k++)
				{
					PoolObjHandle<ActorRoot> poolObjHandle3 = this.m_CanTeleportActorList.get_Item(k);
					if (!this.m_CachedTeleportGameObjects.ContainsKey(poolObjHandle3) && this.IsShowTeleportNoteEffect(poolObjHandle3))
					{
						this.ShowTeleportNoteEffect(poolObjHandle3);
					}
				}
			}
			if (this.m_NeedConfirm)
			{
				this.ShowSkillButtonFlowEffect(this.m_CanTeleportActorList.get_Count() > 0 && !this.m_bConfirmed && this.m_ClickDownStatus && this.m_TargetSkillEnable);
			}
		}
		...
		public void ShowTeleportNoteEffect(PoolObjHandle<ActorRoot> actorRoot)
		{
			if (!this.m_CachedTeleportGameObjects.ContainsKey(actorRoot) && (actorRoot.handle.TheActorMeta.ActorType == ActorTypeDef.Actor_Type_Organ || actorRoot.handle.TheActorMeta.ActorType == ActorTypeDef.Actor_Type_EYE))
			{
				...
				this.m_CachedTeleportGameObjects.set_Item(actorRoot, pooledGameObjLOD);
			}
		}
	}
}
