...
namespace Assets.Scripts.GameLogic
{
	...
	public class AreaEventTrigger : FuncRegion, ITrigger
	{
		...
		public int InActorCount
		{
			get
			{
				return this._inActors.get_Count();
			}
		}
		...
		private int GetCollidingActors()
		{
			...
			else if (this.m_collidePolygon != null)
			{
				...
				{
					num = actorsInPolygon.get_Count();
					int num2 = 0;
					while (num2 < num && num2 < this.Capacity)
					{
						this._actorTestCache[num2] = actorsInPolygon.get_Item(num2);
						num2++;
					}
				}
			}
		...
		public void DoSelfDeactivating()
		{
			...
			while (enumerator.MoveNext())
			{
				...
				int num2 = num++;
				KeyValuePair<uint, AreaEventTrigger.STriggerContext> current = enumerator.get_Current();
				actorTormvCache[num2] = current.get_Key();
			}
			for (int i = 0; i < num; i++)
			{
				uint num3 = this._actorTormvCache[i];
				PoolObjHandle<ActorRoot> actor = this._inActors.get_Item(num3).actor;
				this.DoActorLeave(ref actor);
				this._inActors.Remove(num3);
			}
			this.DeactivateSelf();
			this.ActivateNext();
		}
		...
		protected virtual DictionaryView<TriggerActionBase, RefParamOperator> DoActorEnter(ref PoolObjHandle<ActorRoot> inActor)
		{
			...
			{
				Singleton<CSoundManager>.GetInstance().PostEvent(this.EnterSound, this.bUseTriggerLocationAsSoundSource ? base.gameObject : null);
			}
		...
		protected virtual void DoActorLeave(ref PoolObjHandle<ActorRoot> inActor)
		{
			...
			{
				Singleton<CSoundManager>.GetInstance().PostEvent(this.LeaveSound, this.bUseTriggerLocationAsSoundSource ? base.gameObject : null);
			}
		}
		...
		public bool HasActorInside(Func<PoolObjHandle<ActorRoot>, bool> predicate)
		{
			...
			{
				KeyValuePair<uint, AreaEventTrigger.STriggerContext> current = enumerator.get_Current();
				PoolObjHandle<ActorRoot> actor = current.get_Value().actor;
				if (actor && predicate.Invoke(actor))
				{
					return true;
				}
			}
		...
		public bool HasActorInside(PoolObjHandle<ActorRoot> target)
		{
			...
			{
				KeyValuePair<uint, AreaEventTrigger.STriggerContext> current = enumerator.get_Current();
				if (current.get_Value().actor == target)
				{
					return true;
				}
			}
		...
		public List<PoolObjHandle<ActorRoot>> GetActors(Func<PoolObjHandle<ActorRoot>, bool> predicate)
		{
			...
			while (enumerator.MoveNext())
			{
				KeyValuePair<uint, AreaEventTrigger.STriggerContext> current = enumerator.get_Current();
				PoolObjHandle<ActorRoot> actor = current.get_Value().actor;
				if (actor && (predicate == null || predicate.Invoke(actor)))
				{
					List<PoolObjHandle<ActorRoot>> list2 = list;
					KeyValuePair<uint, AreaEventTrigger.STriggerContext> current2 = enumerator.get_Current();
					list2.Add(current2.get_Value().actor);
				}
			}
			return list;
		}
