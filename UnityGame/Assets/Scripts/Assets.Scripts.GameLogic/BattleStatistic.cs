...
namespace Assets.Scripts.GameLogic
{
	public class BattleStatistic : Singleton<BattleStatistic>
	{
		...
		private void doDestroyStat(PoolObjHandle<ActorRoot> src, PoolObjHandle<ActorRoot> atker)
		{
			...
			DestroyStat destroyStat;
			if (!dictionary.TryGetValue(src.handle.TheActorMeta.ConfigId, ref destroyStat))
			{
				destroyStat = default(DestroyStat);
				dictionary.Add(src.handle.TheActorMeta.ConfigId, destroyStat);
			}
			...
			int num = (playerCamp != actorCamp) ? 1 : 0;
			if (num == 1)
			{
			...
			this.destroyStats[(uint)src.handle.TheActorMeta.ActorType].set_Item(src.handle.TheActorMeta.ConfigId, destroyStat);
		}
		...
		public int GetCampScore(COM_PLAYERCAMP camp)
		{
			...
			while (enumerator.MoveNext())
			{
				...
				if (camp == current.get_Value().PlayerCamp)
				{
					...
					num = num2 + current2.get_Value().numKill;
				}
			}
			return num;
		}
		...
		private void OnAddExpValue(ref AddSoulExpEventParam prm)
		{
			...
			if (prm.atker && prm.atker.handle.TheActorMeta.ActorType == ActorTypeDef.Actor_Type_Hero)
			{
				...
				if (valueComponent != null && valueComponent.ObjValueStatistic != null)
				{
					...
					valueComponent.ObjValueStatistic.uiAddSoulExpIntervalMax = ((valueComponent.ObjValueStatistic.uiAddSoulExpIntervalMax > num) ? valueComponent.ObjValueStatistic.uiAddSoulExpIntervalMax : num);
				}
				List<PoolObjHandle<ActorRoot>> heroActors = Singleton<GameObjMgr>.instance.HeroActors;
				for (int i = 0; i < heroActors.get_Count(); i++)
				{
					if (heroActors.get_Item(i).handle.TheActorMeta.ActorCamp == prm.atker.handle.TheActorMeta.ActorCamp && heroActors.get_Item(i).handle.ValueComponent != null && heroActors.get_Item(i).handle.ValueComponent.ObjValueStatistic != null)
					{
						heroActors.get_Item(i).handle.ValueComponent.ObjValueStatistic.uiTeamSoulExpTotal += (uint)prm.iAddExpValue;
					}
				}
			}
		}
