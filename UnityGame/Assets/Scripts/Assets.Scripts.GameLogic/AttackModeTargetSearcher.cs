...
namespace Assets.Scripts.GameLogic
{
	public class AttackModeTargetSearcher : Singleton<AttackModeTargetSearcher>
	{
		...
		{
			if (!_actorPtr.handle.HorizonMarker.IsVisibleFor(this.curActorPtr.handle.TheActorMeta.ActorCamp) || !this.curActorPtr.handle.CanAttack(_actorPtr) || this.curActorPtr == _actorPtr || ((ulong)this.searchTypeMask & 1uL << (int)(this.curActorPtr.handle.TheActorMeta.ActorType & (ActorTypeDef)31)) > 0uL)
			{
				return;
			}
		...
		private uint GetSearchPriorityTarget(bool bIncludeMonsterNotInBattle = true)
		{
			...
			if (this.bossList.get_Count() >= 1)
			{
				return this.bossList.get_Item(0).ObjID;
			}
			if (this.monsterList.get_Count() >= 1)
			{
				return this.monsterList.get_Item(0).ObjID;
			}
			if (this.organList.get_Count() >= 1)
			{
				return this.organList.get_Item(0).ObjID;
			}
			if (this.monsterNotInBattleList.get_Count() >= 1 && bIncludeMonsterNotInBattle)
			{
				return this.monsterNotInBattleList.get_Item(0).ObjID;
			}
			return 0u;
		}
