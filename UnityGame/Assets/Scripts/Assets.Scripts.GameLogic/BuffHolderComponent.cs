...
namespace Assets.Scripts.GameLogic
{
	public class BuffHolderComponent : LogicComponent
	{
		...
		public void ClearBuff()
		{
			...
			{
				BuffSkill buffSkill2 = this.SpawnedBuffList.get_Item(j);
				if (buffSkill2 != null)
				{
					buffSkill2.Release();
				}
		...
		public int GetCoinAddRate(PoolObjHandle<ActorRoot> _target, bool bIsKiller = true)
		{
			...
			for (int i = 0; i < this.SpawnedBuffList.get_Count(); i++)
			{
				...
				if (buffSkill != null && buffSkill.FindSkillFunc(71, out skillFunc))
				{
					...
					int skillFuncParam4 = buffSkill.GetSkillFuncParam(skillFunc, 3, false);
					if ((buffSkill.GetSkillFuncParam(skillFunc, 4, false) != 0 || bIsKiller) && this.CheckTargetSubType(skillFuncParam, skillFuncParam2, _target))
					{
						bool flag = true;
						if (skillFuncParam4 > 0)
						{
		...
		private int OnConditionExtraHurt(BuffSkill _buffSkill, PoolObjHandle<ActorRoot> _attack)
		{
			...
			if (_buffSkill != null && _buffSkill.FindSkillFunc(44, out skillFunc))
			{
				...
				int num = flag ? _attack.handle.ValueComponent.actorHp : this.actor.ValueComponent.actorHp;
				int num2 = flag ? _attack.handle.ValueComponent.mActorValue[RES_FUNCEFT_TYPE.RES_FUNCEFT_MAXHP].totalValue : this.actor.ValueComponent.mActorValue[RES_FUNCEFT_TYPE.RES_FUNCEFT_MAXHP].totalValue;
		...
		public int GetExtraHurtOutputRate(PoolObjHandle<ActorRoot> _attack)
		{
			...
			for (int i = 0; i < _attack.handle.BuffHolderComp.SpawnedBuffList.get_Count(); i++)
			{
				BuffSkill buffSkill = _attack.handle.BuffHolderComp.SpawnedBuffList.get_Item(i);
		...
		private void OnDamageTriggerEffect(PoolObjHandle<ActorRoot> _attacker)
		{
			...
			for (int i = 0; i < this.actorPtr.handle.BuffHolderComp.SpawnedBuffList.get_Count(); i++)
			{
				BuffSkill buffSkill = this.actorPtr.handle.BuffHolderComp.SpawnedBuffList.get_Item(i);
		...
		private void OnDamageExtraEffect(PoolObjHandle<ActorRoot> _attack, SkillSlotType _slotType, SkillSlotType _extraEffectSlotType)
		{
			...
			for (int i = 0; i < _attack.handle.BuffHolderComp.SpawnedBuffList.get_Count(); i++)
			{
				...
				if (buffSkill != null && buffSkill.FindSkillFunc(33, out skillFunc))
				{
					...
					if (skillFuncParam3 == 0 && this.CheckTargetSubType(skillFuncParam4, skillFuncParam5, this.actorPtr) && (skillFuncParam7 == 0 || this.CheckTriggerCondtion(skillFuncParam7, skillFuncParam8, _attack, this.actorPtr)))
					{
						...
						if (flag && flag2 && (skillFuncParam6 != -2 || !buffSkill.IsNextDestroy()))
						{
							SkillUseParam skillUseParam = default(SkillUseParam);
							skillUseParam.Init();
		...
		public void OnDead(PoolObjHandle<ActorRoot> _attack)
		{
			...
			{
				BuffSkill buffSkill2 = this.SpawnedBuffList.get_Item(j);
				if (num == j && buffSkill2 != null && buffSkill2.FindSkillFunc(32, out skillFunc))
				{
					int skillFuncParam2 = buffSkill2.GetSkillFuncParam(skillFunc, 0, false);
					int skillFuncParam3 = buffSkill2.GetSkillFuncParam(skillFunc, 1, false);
					int skillFuncParam4 = buffSkill2.GetSkillFuncParam(skillFunc, 2, false);
					bool autoReset = (skillFuncParam4 & 1) == 1;
					bool bBaseRevive = buffSkill2.GetSkillFuncParam(skillFunc, 3, false) == 0;
					bool bCDReset = buffSkill2.GetSkillFuncParam(skillFunc, 4, false) == 1;
					int skillFuncParam5 = buffSkill2.GetSkillFuncParam(skillFunc, 5, false);
					int skillFuncParam6 = buffSkill2.GetSkillFuncParam(skillFunc, 6, false);
					bool bIsPassiveSkill = (skillFuncParam4 & 2) > 0;
					uint uiBuffObjId = 0u;
					if (buffSkill2.skillContext != null && buffSkill2.skillContext.Originator)
					{
						uiBuffObjId = buffSkill2.skillContext.Originator.handle.ObjID;
					}
					...
					this.RemoveBuff(buffSkill2);
				}
				if (buffSkill2 != null && buffSkill2.cfgData != null && buffSkill2.cfgData.bIsInheritByKiller == 1)
				{
					this.RemoveBuff(buffSkill2);
					if (_attack && _attack.handle.TheActorMeta.ActorType == ActorTypeDef.Actor_Type_Hero && !_attack.handle.ActorControl.IsDeadState)
					{
						SkillUseParam skillUseParam = default(SkillUseParam);
						skillUseParam.SetOriginator(_attack);
						skillUseParam.bExposing = buffSkill2.skillContext.bExposing;
						skillUseParam.uiFromId = buffSkill2.skillContext.uiFromId;
						skillUseParam.skillUseFrom = buffSkill2.skillContext.skillUseFrom;
						_attack.handle.SkillControl.SpawnBuff(_attack, ref skillUseParam, buffSkill2.SkillID, true);
					}
				}
			}
