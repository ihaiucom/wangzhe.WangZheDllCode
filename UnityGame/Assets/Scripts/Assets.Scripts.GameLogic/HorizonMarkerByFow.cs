	...
	public class HorizonMarkerByFow : HorizonMarkerBase
	{
		internal class CampMarker
		{
			...
			public void AddHideMark(HorizonConfig.HideMark hm, int count)
			{
				if (count >= 0)
				{
					byte[] hideMarks = this._hideMarks;
					hideMarks[(int)hm] = hideMarks[(int)hm] + (byte)count;
				}
				else
				{
					byte b = -(byte)count;
					...
					{
						byte[] hideMarks2 = this._hideMarks;
						hideMarks2[(int)hm] = hideMarks2[(int)hm] - b;
					}
				}
			}
			...
			{
				this._hideMarks[(int)hm] = (bSet ? 1 : 0);
			}
