...
namespace Assets.Scripts.GameLogic
{
	...
	public class ObjAgent : BTBaseAgent, IActorComponent, IPooledMonoBehaviour
	{
		...
		public void ShowRandomSignal()
		{
			...
			int num2 = (int)(1 + FrameRandom.Random(4u));
			ActorRoot handle = towerActors.get_Item(num).handle;
			...
			int num3 = handle.location.x + (int)FrameRandom.Random(10000u) - 5000;
			int y = handle.location.y;
			int num4 = handle.location.z + (int)FrameRandom.Random(6000u) - 3000;
			...
			if (signalPanel != null)
			{
				signalPanel.ExecCommand(playerId, configId, num2, num3 / 1000, y / 1000, num4 / 1000, 0, 0, 0u);
			}
		}
		...
		{
			using (Dictionary<int, ShenFuObjects>.Enumerator enumerator = Singleton<ShenFuSystem>.GetInstance()._shenFuTriggerPool.GetEnumerator())
			{
				while (enumerator.MoveNext())
				{
					KeyValuePair<int, ShenFuObjects> current = enumerator.get_Current();
					if (current.get_Value().ShenFu.activeSelf)
					{
						Vector3 position = current.get_Value().ShenFu.transform.position;
		...
		public EBTStatus IsAlive(uint objID)
		{
			...
			if (actor)
			{
				return actor.handle.ActorControl.IsDeadState ? EBTStatus.BT_FAILURE : EBTStatus.BT_SUCCESS;
			}
			return EBTStatus.BT_FAILURE;
		}
		...
		public int GetOurCampActorsCount(int srchR)
		{
			...
			{
				return num + ourCampActors.get_Count();
			}
			return num;
		}
		...
		{
			if (Singleton<BattleLogic>.GetInstance().m_dragonSpawn != null && Singleton<BattleLogic>.GetInstance().m_dragonSpawn.GetSpawnedList().get_Count() > 0)
			{
				PoolObjHandle<ActorRoot> ptr = Singleton<BattleLogic>.GetInstance().m_dragonSpawn.GetSpawnedList().get_Item(0);
				if (ptr)
				{
		...
		public EBTStatus PlayAgeAction(string actionName)
		{
			...
			{
				if (actionName == null || actionName.get_Length() <= 0)
				{
					return EBTStatus.BT_FAILURE;
				}
		...
		public EBTStatus PlayDeadAgeAction()
		{
			...
			else
			{
				...
				if (deadAgePath == null || deadAgePath.get_Length() <= 0)
				{
					return EBTStatus.BT_FAILURE;
				}
		...
		public virtual void RealMoveInMoveAttack(uint objID, int range)
		{
			...
			else
			{
				...
				VInt3 vInt2 = (vInt == VInt3.zero) ? VInt3.forward : vInt;
		...
		public EBTStatus SelectRouteBySelfCampIndex()
		{
			...
			for (int i = 0; i < waypointsList.Count; i++)
			{
				if (!(waypointsList[i] == null) && waypointsList[i].m_index == num)
				{
					this.m_wrapper.m_curWaypointsHolder = waypointsList[i];
					this.m_wrapper.m_curWaypointTarget = this.m_wrapper.m_curWaypointsHolder.startPoint;
					this.m_wrapper.m_curWaypointTargetPosition = new VInt3(this.m_wrapper.m_curWaypointTarget.transform.position);
		...
		public EBTStatus CheckSkillFilter(SkillSlotType InSlot, uint objID)
		{
			...
			PoolObjHandle<ActorRoot> actor = Singleton<GameObjMgr>.GetInstance().GetActor(objID);
			if (((ulong)dwSkillTargetFilter & 1uL << (int)(actor.handle.TheActorMeta.ActorType & (ActorTypeDef)31)) > 0uL)
			{
				return EBTStatus.BT_FAILURE;
			}
			return EBTStatus.BT_SUCCESS;
		...
		public bool CheckUseSkillPosition()
		{
			...
			if (skill != null)
			{
				...
				{
					long num3 = (long)(skill.cfgData.iMaxAttackDistance * skill.cfgData.iMaxAttackDistance);
					long num4 = (this.m_wrapper.actorLocation - curSkillUseInfo.UseVector).sqrMagnitudeLong2D - num3;
					return (double)num4 <= (double)num3 * 0.09;
				}
		...
		public virtual EBTStatus MoveToSkillTarget()
		{
			...
			if (nextSkill != null)
			{
				...
				case SkillRangeAppointType.Pos:
				{
					long num2 = (long)(nextSkill.cfgData.iMaxAttackDistance * nextSkill.cfgData.iMaxAttackDistance);
					if ((this.m_wrapper.actorLocation - curSkillUseInfo.UseVector).sqrMagnitudeLong2D > num2)
					{
		...
		public virtual EBTStatus MoveToSkillTargetWithRange(int range)
		{
			...
			if (nextSkill != null)
			{
				...
				else
				{
					...
					long num3 = nextSkill.cfgData.iMaxAttackDistance;
					num3 += (long)curSkillUseInfo.TargetActor.handle.ActorControl.GetDetectedRadius();
					num3 *= num3;
					if ((this.m_wrapper.actorLocation - curSkillUseInfo.TargetActor.handle.location).sqrMagnitudeLong2D > num3)
					{
						this.m_wrapper.RealMovePosition(curSkillUseInfo.TargetActor.handle.location, 0u);
						return EBTStatus.BT_RUNNING;
					}
		...
		public void NotifyTeamSelectRoute(int index, int range, uint centerObjId)
		{
			...
			for (int i = 0; i < waypointsList.Count; i++)
			{
				if (!(waypointsList[i] == null) && waypointsList[i].m_index == index)
				{
					long num = (long)range;
					num *= num;
					...
					{
						PoolObjHandle<ActorRoot> poolObjHandle = heroActors.get_Item(j);
		...
		{
			int count = Singleton<GameObjMgr>.instance.GameActors.get_Count();
			for (int i = 0; i < count; i++)
			{
				PoolObjHandle<ActorRoot> ptr = Singleton<GameObjMgr>.instance.GameActors.get_Item(i);
		...
		{
			List<PoolObjHandle<ActorRoot>> towerActors = Singleton<GameObjMgr>.GetInstance().TowerActors;
			for (int i = 0; i < towerActors.get_Count(); i++)
			{
				PoolObjHandle<ActorRoot> poolObjHandle = towerActors.get_Item(i);
