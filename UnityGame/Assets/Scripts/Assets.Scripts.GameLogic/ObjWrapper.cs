...
namespace Assets.Scripts.GameLogic
{
	public class ObjWrapper : LogicComponent
	{
		...
		private int[] NoAbility = new int[13];
		...
		public event ActorDeadEventHandler eventActorDead
		{
			...
			add
			{
				this.eventActorDead = (ActorDeadEventHandler)Delegate.Combine(this.eventActorDead, value);
			}
			[MethodImpl(32)]
			remove
			{
				this.eventActorDead = (ActorDeadEventHandler)Delegate.Remove(this.eventActorDead, value);
			}
		}
		...
		public event ActorEventHandler eventActorRevive
		{
			...
			add
			{
				this.eventActorRevive = (ActorEventHandler)Delegate.Combine(this.eventActorRevive, value);
			}
			[MethodImpl(32)]
			remove
			{
				this.eventActorRevive = (ActorEventHandler)Delegate.Remove(this.eventActorRevive, value);
			}
		}
		...
		public event ActorEventHandler eventActorAssist
		{
			...
			add
			{
				this.eventActorAssist = (ActorEventHandler)Delegate.Combine(this.eventActorAssist, value);
			}
			[MethodImpl(32)]
			remove
			{
				this.eventActorAssist = (ActorEventHandler)Delegate.Remove(this.eventActorAssist, value);
			}
		}
		...
		public event ActorEventHandler eventActorEnterCombat
		{
			...
			add
			{
				this.eventActorEnterCombat = (ActorEventHandler)Delegate.Combine(this.eventActorEnterCombat, value);
			}
			[MethodImpl(32)]
			remove
			{
				this.eventActorEnterCombat = (ActorEventHandler)Delegate.Remove(this.eventActorEnterCombat, value);
			}
		}
		...
		public event ActorEventHandler eventActorExitCombat
		{
			...
			add
			{
				this.eventActorExitCombat = (ActorEventHandler)Delegate.Combine(this.eventActorExitCombat, value);
			}
			[MethodImpl(32)]
			remove
			{
				this.eventActorExitCombat = (ActorEventHandler)Delegate.Remove(this.eventActorExitCombat, value);
			}
		}
		...
		public int ReviveCooldown
		{
			...
			set
			{
				this.m_reviveTick = ((value >= 0) ? value : 0);
			}
		}
		...
		public bool CanMove
		{
			get
			{
				return this.actor.isMovable && this.NoAbility[1] == 0;
			}
		}
		...
		public bool CanRotate
		{
			get
			{
				return this.actor.isRotatable && this.NoAbility[5] == 0;
			}
		}
		...
		public int GetReviveTotalTime()
		{
			...
			{
				return (this.m_reviveContext.ReviveTime > 0) ? this.m_reviveContext.ReviveTime : this.CfgReviveCD;
			}
		...
		public void SetInAttack(PoolObjHandle<ActorRoot> inAttackee, bool bExposeAttacker)
		{
			...
			{
				this.actor.HorizonMarker.SetExposeMark(true, inAttackee ? inAttackee.handle.TheActorMeta.ActorCamp : COM_PLAYERCAMP.COM_PLAYERCAMP_COUNT, false);
			}
		}
		...
		{
			PoolObjHandle<ActorRoot> poolObjHandle = this.myLastAtker ? this.myLastAtker.handle.ActorControl.GetOrignalActor() : this.myLastAtker;
			...
			if (this.m_reviveContext.bEnable)
			{
				this.m_reviveTick = ((this.m_reviveContext.ReviveTime > 0) ? this.m_reviveContext.ReviveTime : this.CfgReviveCD);
			}
			else
			{
		...
		protected void updateAffectActors()
		{
			...
			int i = 0;
			while (i < this.hurtSelfActorList.get_Count())
			{
				if (logicFrameTick > this.hurtSelfActorList.get_Item(i).get_Value() + 10000uL)
				{
			...
			i = 0;
			while (i < this.helpSelfActorList.get_Count())
			{
				if (logicFrameTick > this.helpSelfActorList.get_Item(i).get_Value() + 10000uL)
				{
		...
		public ResSkillCfgInfo GetSkillCfgData(SkillSlotType slot)
		{
			...
			return (skill != null) ? skill.cfgData : null;
		}
		...
		{
			bool noAbilityFlag = this.actor.ActorControl.GetNoAbilityFlag(ObjAbilityType.ObjAbility_DeadControl);
			if (this.IsDeadState && !noAbilityFlag)
			{
				return;
			}
			if (noAbilityFlag)
			{
				this.actor.ActorControl.SetDeadMode(ObjDeadMode.DeadState_Move);
				this.curMoveCommand = cmd;
				this.SetAutoAI(false);
			}
			else
			{
				...
				if (!this.actor.SkillControl.AbortCurUseSkill(SkillAbortType.TYPE_MOVE))
				{
					if (this.actor.TheActorMeta.ActorType == ActorTypeDef.Actor_Type_Hero)
					{
						this.actor.SkillControl.SkillUseCache.SetCacheMoveCommand(this.curMoveCommand);
					}
				}
		...
		public void BTExMoveCmd()
		{
			...
			else if (this.curMoveCommand.cmdType == 131)
			{
		...
		public void OnMoveSpdChange()
		{
			...
			if (this.actor.ValueComponent.ObjValueStatistic != null)
			{
				...
				this.actor.ValueComponent.ObjValueStatistic.iMoveSpeedMax = ((iMoveSpeedMax > num) ? iMoveSpeedMax : num);
			}
		}
		...
		public void AddHurtSelfActor(PoolObjHandle<ActorRoot> actor)
		{
			...
			ulong logicFrameTick = Singleton<FrameSynchr>.instance.LogicFrameTick;
			int i;
			for (i = 0; i < this.hurtSelfActorList.get_Count(); i++)
			{
				if (this.hurtSelfActorList.get_Item(i).get_Key() == ptr.handle.ObjID)
				{
					this.hurtSelfActorList.set_Item(i, new KeyValuePair<uint, ulong>(ptr.handle.ObjID, logicFrameTick));
					break;
				}
			}
			if (i >= this.hurtSelfActorList.get_Count())
			{
		...
		public void AddHelpSelfActor(PoolObjHandle<ActorRoot> actor)
		{
			...
			ulong logicFrameTick = Singleton<FrameSynchr>.instance.LogicFrameTick;
			int i;
			for (i = 0; i < this.helpSelfActorList.get_Count(); i++)
			{
				if (this.helpSelfActorList.get_Item(i).get_Key() == ptr.handle.ObjID)
				{
					this.helpSelfActorList.set_Item(i, new KeyValuePair<uint, ulong>(ptr.handle.ObjID, logicFrameTick));
					break;
				}
			}
			if (i >= this.helpSelfActorList.get_Count())
			{
		...
		{
			return (this.actor.shape != null) ? this.actor.shape.AvgCollisionRadius : 0;
		}
	}
}
