	...
	public class BufferMark : BaseSkill
	{
		...
		private bool CheckTargetType(uint typeMask)
		{
			...
			if (this.sourceActor)
			{
				...
				if (((ulong)typeMask & 1uL << (actorType & 31)) > 0uL)
				{
					return true;
				}
			}
		...
		public void Init(BuffHolderComponent _buffHolder, PoolObjHandle<ActorRoot> _originator, uint _markType)
		{
			...
			this.lastMaxTime = ((this.cfgData.iLastMaxTime == 0) ? 2147483647 : this.cfgData.iLastMaxTime);
		}
		...
		public void UpdateLogic(int nDelta)
		{
			if (this.immuneTime > 0)
			{
				...
				this.immuneTime = ((this.immuneTime > 0) ? this.immuneTime : 0);
			}
			if (this.triggerCDTime > 0)
			{
				this.triggerCDTime -= nDelta;
				this.triggerCDTime = ((this.triggerCDTime > 0) ? this.triggerCDTime : 0);
			}
			this.curTime += nDelta;
		...
		public void AddLayer(int _addLayer)
		{
			...
			num += _addLayer;
			num = ((num > this.cfgData.iMaxLayer) ? this.cfgData.iMaxLayer : num);
