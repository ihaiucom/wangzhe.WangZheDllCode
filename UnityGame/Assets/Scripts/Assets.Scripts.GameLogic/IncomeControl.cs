...
namespace Assets.Scripts.GameLogic
{
	public class IncomeControl
	{
		...
		public void InitExpCompensateInfo(byte bIsOpenExpCompensate, ref ResDT_ExpCompensateInfo[] expCompensateInfo)
		{
			...
			if (IncomeControl.m_isExpCompensate && expCompensateInfo != null)
			{
				...
				for (int i = 0; i < expCompensateInfo.Length; i++)
				{
					...
					if (bLevelDiff > 0)
					{
						for (int j = IncomeControl.m_compensateRateList.get_Count(); j < bLevelDiff; j++)
						{
							IncomeControl.m_compensateRateList.Add(num);
						}
		...
		{
			if (actorRoot == null || !IncomeControl.m_isExpCompensate || IncomeControl.m_compensateRateList.get_Count() == 0)
			{
				return exp;
			}
			...
			int num2 = 0;
			if (num >= 0 && num < IncomeControl.m_compensateRateList.get_Count())
			{
				num2 = IncomeControl.m_compensateRateList.get_Item(num);
			}
			else if (num >= IncomeControl.m_compensateRateList.get_Count())
			{
				num2 = IncomeControl.m_compensateRateList.get_Item(IncomeControl.m_compensateRateList.get_Count() - 1);
			}
