...
namespace AGE
{
	public class Track : PooledClassObject
	{
		...
		public override void OnRelease()
		{
			...
			{
				this.trackEvents.get_Item(i).Release();
			}
			this.trackEvents.Clear();
			this.activeEvents.Clear();
			this.waitForConditions = null;
		...
		public bool LocateEvent(int _curTime, out int _result)
		{
			...
			int num = 0;
			int num2 = this.trackEvents.get_Count() - 1;
			while (num != num2)
			{
				int num3 = (num + num2) / 2 + 1;
				if (_curTime < this.trackEvents.get_Item(num3).time)
				{
					num2 = num3 - 1;
				}
				else
			...
			int time = this.trackEvents.get_Item(0).time;
			int time2 = this.trackEvents.get_Item(count - 1).time;
			int num4;
			...
			{
				int time3 = this.trackEvents.get_Item(num4).time;
				int time4 = this.trackEvents.get_Item(num4 + 1).time;
		...
		public BaseEvent GetOffsetEvent(BaseEvent _curEvent, int _offset)
		{
			...
			if (this.Loop)
			{
				int num2 = (num + _offset) % this.trackEvents.get_Count();
				if (num2 < 0)
				{
					num2 += this.trackEvents.get_Count();
				}
				return this.trackEvents.get_Item(num2);
			}
			int num3 = num + _offset;
			if (num3 < 0 || num3 >= this.trackEvents.get_Count())
			{
				return null;
			}
			return this.trackEvents.get_Item(num3);
		}

		public BaseEvent GetEvent(int index)
		{
			if (index >= 0 && index < this.trackEvents.get_Count())
			{
				return this.trackEvents.get_Item(index);
			}
			return null;
		}
		...
		{
			return this.trackEvents.get_Count();
		}
		...
		public void Stop(Action _action)
		{
			...
			for (int i = 0; i < this.activeEvents.get_Count(); i++)
			{
				this.activeEvents.get_Item(i).Leave(this.action, this);
			}
			this.activeEvents.Clear();
			if (!this.isCondition)
			{
		...
		{
			if (this.trackEvents.get_Count() == 0)
			{
				return 0;
			}
			if (this.isDurationEvent)
			{
				return (this.trackEvents.get_Item(this.trackEvents.get_Count() - 1) as DurationEvent).End + 33;
			}
			return (this.trackEvents.get_Item(this.trackEvents.get_Count() - 1) as TickEvent).time + 33;
		}
