	...
	public class HitTriggerDurationContext
	{
		...
		public void Leave(Action _action, Track _track)
		{
			...
			for (int j = 0; j < this.RemoveSkillList.get_Count(); j++)
			{
				...
				if (buffSkill != null && !buffSkill.isFinish && buffSkill.skillContext.TargetActor)
				{
		...
		{
			int num;
			return this.CollideMaxCount > 0 && this.collideCountMap.TryGetValue(actor.ObjID, ref num) && num >= this.CollideMaxCount;
		}
		...
		private void CopyTargetList(List<PoolObjHandle<ActorRoot>> _srcList, List<PoolObjHandle<ActorRoot>> _destList, int _count)
		{
			...
			{
				_destList.Add(_srcList.get_Item(i));
			}
		}
		...
		{
			if (this.residueActorCount < triggerList.get_Count())
			{
			...
			this.CopyTargetList(triggerList, this.triggerPriority, triggerList.get_Count());
			this.residueActorCount -= triggerList.get_Count();
		...
		private void PriorityTrigger(Action _action)
		{
			...
			for (int i = 0; i < this.triggerPriority.get_Count(); i++)
			{
				PoolObjHandle<ActorRoot> poolObjHandle = this.triggerPriority.get_Item(i);
		...
		private void HitTrigger(Action _action)
		{
			...
			if (this.collidedActors != null && this.collidedActors.get_Count() > 0)
			{
				...
				{
					this.skillContext.TargetActor = this.collidedActors.get_Item(0);
				}
