	...
	public class CurvlData
	{
		...
		private void CreateCurvlPoints(bool useUnifyInterp)
		{
			...
			for (int i = 0; i < count; i++)
			{
				if (this.closeCurvl || i != 0)
				{
					...
					{
						if (this.curvePoints.get_Count() <= this.curvePointCount)
						{
							this.curvePoints.Add(pos);
						}
						else
						{
							this.curvePoints.set_Item(this.curvePointCount, pos);
						}
						...
						{
							this.curvePoints.set_Item(this.curvePointCount, pos2);
						}
						this.curvePointCount++;
					}
					else
		...
		private void CalculateCurvlPoints(Vector3 prevPoint, Vector3 curnPoint, Vector3 ctrlPoint1, Vector3 ctrlPoint2, float stepLen)
		{
			...
			while (num <= 1f)
			{
				...
				{
					this.curvePoints.set_Item(this.curvePointCount, vector);
				}
				this.curvePointCount++;
				num += stepLen;
				if (flag)
		...
		public void DrawCurvel(bool useUnifyInterp)
		{
			...
			{
				Vector3 start = this.curvePoints.get_Item(i);
				Vector3 end = this.curvePoints.get_Item(i + 1);
