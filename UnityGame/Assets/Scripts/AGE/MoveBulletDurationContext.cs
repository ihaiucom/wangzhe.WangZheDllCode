...
namespace AGE
{
	internal class MoveBulletDurationContext
	{
		...
		private void FindEnemy(Action _action, int _localTime)
		{
			...
			{
				if (this.IsValidEnemy(heroActors.get_Item(i)))
				{
					_action.refParams.AddRefParam("FindEnemyActor", heroActors.get_Item(i));
			...
			for (int j = 0; j < monsterActors.get_Count(); j++)
			{
				if (this.IsValidEnemy(monsterActors.get_Item(j)))
				{
					_action.refParams.AddRefParam("FindEnemyActor", monsterActors.get_Item(j));
					skillControl.RotateBodyBulletFindEnemyLogicFrameTick = Singleton<FrameSynchr>.GetInstance().LogicFrameTick;
					this.stopCondtion = true;
					return;
				}
