...
namespace AGE
{
	public class Action : PooledClassObject
	{
		...
		public event ActionStopDelegate onActionStop
		{
			...
			add
			{
				this.onActionStop = (ActionStopDelegate)Delegate.Combine(this.onActionStop, value);
			}
			[MethodImpl(32)]
			remove
			{
				this.onActionStop = (ActionStopDelegate)Delegate.Remove(this.onActionStop, value);
			}
		}
		...
		public override void OnUse()
		{
			...
			DebugHelper.Assert(this.actorHandles.get_Count() == 0 && this.gameObjects.Count == 0, "age gameObjects is not null");
		}
		...
		public void UpdateLogic(int nDelta)
		{
			...
			int num = (this.playSpeed.nom == this.playSpeed.den) ? nDelta : ((int)((long)nDelta * this.playSpeed.nom / this.playSpeed.den));
			num += this.time;
			this.ForceUpdate(num);
		}
		...
		{
			if (_index >= 0 && _index < this.actorHandles.get_Count())
			{
				return this.actorHandles.get_Item(_index);
			}
		...
		{
			return this.conditions.get_Item(_track);
		}

		public int GetConditionCount()
		{
			return this.conditions.get_Count();
		}

		public void SetCondition(Track _track, bool _status)
		{
			bool flag = this.conditions.get_Item(_track);
			if (flag != _status)
			{
				this.conditionChanged = true;
				this.conditions.set_Item(_track, _status);
			}
		}
						...
						using (Dictionary<string, bool>.KeyCollection.Enumerator enumerator = associatedResources.get_Keys().GetEnumerator())
						{
							while (enumerator.MoveNext())
							{
								...
								if (dictionary.ContainsKey(current))
								{
									...
									string text;
									bool flag = (dictionary2 = dictionary).get_Item(text = current);
									dictionary2.set_Item(text, flag | associatedResources.get_Item(current));
								}
								else
								{
