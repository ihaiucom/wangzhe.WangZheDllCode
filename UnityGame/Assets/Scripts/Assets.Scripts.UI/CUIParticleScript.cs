...
namespace Assets.Scripts.UI
{
	public class CUIParticleScript : CUIComponent
	{
		...
		private void ResetParticleScale()
		{
			...
			if (this.m_belongedFormScript.m_canvasScaler.get_matchWidthOrHeight() == 0f)
			{
				scale = component.rect.width / component.rect.height / (this.m_belongedFormScript.m_canvasScaler.get_referenceResolution().x / this.m_belongedFormScript.m_canvasScaler.get_referenceResolution().y);
			}
			else if (this.m_belongedFormScript.m_canvasScaler.get_matchWidthOrHeight() == 1f)
			{
			}
