...
namespace Assets.Scripts.UI
{
	public class CUITimerScript : CUIComponent
	{
		...
		[HideInInspector]
		public enUIEventID[] m_eventIDs = new enUIEventID[Enum.GetValues(typeof(enTimerEventType)).get_Length()];

		public stUIEventParams[] m_eventParams = new stUIEventParams[Enum.GetValues(typeof(enTimerEventType)).get_Length()];
		...
		private void RefreshTimeDisplay()
		{
			...
			if (this.m_timerDisplayType != enTimerDisplayType.None)
			{
				...
				case enTimerDisplayType.H_M_S:
				{
					...
					this.m_timerText.set_text(string.Format("{0:D2}:{1:D2}:{2:D2}", num2, num3, num4));
					break;
				}
				case enTimerDisplayType.M_S:
				{
					...
					this.m_timerText.set_text(string.Format("{0:D2}:{1:D2}", num5, num6));
					break;
				}
				case enTimerDisplayType.S:
					this.m_timerText.set_text(string.Format("{0:D}", num));
					break;
				case enTimerDisplayType.H_M:
				{
					...
					num -= num7 * 3600;
					int num8 = num / 60;
					this.m_timerText.set_text(string.Format("{0:D2}:{1:D2}", num7, num8));
					break;
				}
				case enTimerDisplayType.D_H_M_S:
				{
					...
					this.m_timerText.set_text(string.Format("{0}天{1:D2}:{2:D2}:{3:D2}", new object[]
					{
						num9,
						num10,
						num11,
				...
				case enTimerDisplayType.D_H_M:
				{
					...
					num -= num14 * 3600;
					int num15 = num / 60;
					this.m_timerText.set_text(string.Format("{0}天{1:D2}:{2:D2}", num13, num14, num15));
					break;
				}
				case enTimerDisplayType.D:
				{
					int num16 = num / 86400;
					this.m_timerText.set_text(string.Format("{0}天", num16));
					break;
				}
				}
			}
