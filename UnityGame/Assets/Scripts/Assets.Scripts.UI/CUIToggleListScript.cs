...
namespace Assets.Scripts.UI
{
	public class CUIToggleListScript : CUIListScript
	{
		...
		{
			return this.m_multiSelectedIdx.get_Count();
		}
		...
		private bool MultiLimitCheckStep1(int index, bool selected)
		{
			if (selected)
			{
				if (this.m_multiSelectedIdx.get_Count() >= this.m_maxSelectCount && this.m_maxSelectCount > 0)
				{
					return true;
				}
		...
		private void GetOrgToggleTextColor(Toggle tgl)
		{
			...
			if (component != null && !component.m_hasSetTextColor)
			{
				...
				{
					component.m_orgToggleTextColor = componentInChildren.get_color();
				}
		...
		{
			return this.m_isMultiSelected ? this.m_multiSelected[index] : (index == this.m_selected);
		}
	}
}
