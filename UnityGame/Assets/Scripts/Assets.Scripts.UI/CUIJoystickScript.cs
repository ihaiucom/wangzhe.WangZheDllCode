...
{
	public class CUIJoystickScript : CUIComponent, IEventSystemHandler, IPointerDownHandler, IPointerUpHandler, IBeginDragHandler, IDragHandler, IEndDragHandler
	{
		...
		private void MoveAxis(Vector2 position, bool isDown)
		{
			if (isDown || (this.m_axisCurrentScreenPosition == Vector2.zero && this.m_axisTargetScreenPosition == Vector2.zero))
			{
				...
				this.m_axisRectTransform.position = CUIUtility.ScreenToWorldPoint((this.m_belongedFormScript != null) ? this.m_belongedFormScript.GetCamera() : null, this.m_axisCurrentScreenPosition, this.m_axisRectTransform.position.z);
			}
			...
			this.m_cursorRectTransform.anchoredPosition = ((num > this.m_cursorDisplayMaxRadius) ? (vector2.normalized * this.m_cursorDisplayMaxRadius) : vector2);
			if (this.m_isAxisMoveable && num > this.m_cursorDisplayMaxRadius)
			{
				DebugHelper.Assert(this.m_belongedFormScript != null);
				this.m_axisTargetScreenPosition = this.m_axisCurrentScreenPosition + (position - CUIUtility.WorldToScreenPoint((this.m_belongedFormScript != null) ? this.m_belongedFormScript.GetCamera() : null, this.m_cursorRectTransform.position));
