...
namespace Assets.Scripts.UI
{
	public class CUIStepListScript : CUIListScript
	{
		...
		protected override void Update()
		{
			...
			else
			{
				...
				if (!(bool)field.GetValue(this.m_scrollRect))
				{
					...
					{
						float num2 = Mathf.Abs(this.m_scrollRect.get_velocity().x);
						int num3 = -1;
				...
				this.m_scrollRectLastScrollSpeed = Mathf.Abs(this.m_scrollRect.get_velocity().x);
			}
			base.DetectScroll();
		}
		...
		{
			float num = this.m_elementsRect.get_Item(index).m_center.x + this.m_contentRectTransform.anchoredPosition.x;
			return num > this.m_selectAreaMin && num < this.m_selectAreaMax;
		}

		private float GetElementScale(int index)
		{
			int num = (int)(this.m_elementsRect.get_Item(index).m_center.x + this.m_contentRectTransform.anchoredPosition.x);
		...
		private float GetContentTargetPosition(int selectedIndex)
		{
			...
			float num = this.m_stepListCenter - this.m_elementsRect.get_Item(selectedIndex).m_center.x;
			if (num > 0f)
			{
				num = 0f;
			}
