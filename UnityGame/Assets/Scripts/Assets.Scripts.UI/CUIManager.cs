	...
	public class CUIManager : Singleton<CUIManager>
	{
		...
		public CUIFormScript OpenForm(string formPath, bool useFormPool, bool useCameraRenderMode = true)
		{
			...
			if (cUIFormScript != null)
			{
				...
				cUIFormScript.Open(formPath, useCameraRenderMode ? this.m_formCamera : null, this.m_formSequence, false, formOpenOrder2);
		...
		{
			MemberInfo[] member = typeof(GraphicRaycaster).GetMember("s_SortedGraphics", 41);
			if (member != null && member.Length == 1)
			{
				MemberInfo memberInfo = member[0];
				if (memberInfo != null && memberInfo.get_MemberType() == 4)
				{
		...
		private GameObject CreateForm(string formPrefabPath, bool useFormPool)
		{
			...
			if (useFormPool)
			{
				for (int i = 0; i < this.m_pooledForms.Count; i++)
				{
					if (string.Equals(formPrefabPath, this.m_pooledForms[i].m_formPath, 5))
					{
		...
		public void OpenAwardTip(CUseable[] items, string title = null, bool playSound = false, enUIEventID eventID = enUIEventID.None, bool displayAll = false, bool forceNotGoToBag = false, string formPath = "Form_Award")
		{
			...
			if (title != null)
			{
				Utility.GetComponetInChild<Text>(cUIFormScript.gameObject, "bg/Title").set_text(title);
			}
			...
			for (int i = 0; i < num; i++)
			{
				if (!(component.GetElemenet(i) == null) && items[i] != null)
				{
					...
					gameObject.CustomSetActive(true);
					gameObject.transform.FindChild("ItemName").GetComponent<Text>().set_text(items[i].m_name);
					if (playSound)
					{
		...
		public void OpenEditForm(string title, string editContent, enUIEventID confirmEventId = enUIEventID.None)
		{
			...
			{
				Text component2 = cUIFormScript.GetWidget(1).GetComponent<Text>();
				component2.set_text(editContent);
			}
		...
		public void OpenExchangeCountSelectForm(CUseable item, int count, enUIEventID confirmId, stUIEventParams par, uint price = 0u, uint totalPrice = 0u)
		{
			...
			if (cUIFormScript != null)
			{
				...
				{
					component2.set_text(string.Format(Singleton<CTextManager>.GetInstance().GetText("ExchangeNumSelectPoint"), this._price, this._totalPirce));
					component2.gameObject.CustomSetActive(true);
				}
				else
				{
		...
		private void OnAddExchangeCount(CUIEvent uiEvent)
		{
			...
			if (component != null)
			{
				int num = int.Parse(component.get_text());
				num++;
				...
				if (this._price > 0u && this._totalPirce >= this._price)
				{
					component2.set_text(string.Format(Singleton<CTextManager>.GetInstance().GetText("ExchangeNumSelectPoint"), (long)((ulong)this._price * (ulong)((long)num)), this._totalPirce));
					component2.gameObject.CustomSetActive(true);
				}
				else
				{
		...
		private void OnMaxExchangeCount(CUIEvent uiEvent)
		{
			...
			if (component != null)
			{
				...
				if (this._price > 0u && this._totalPirce >= this._price)
				{
					component2.set_text(string.Format(Singleton<CTextManager>.GetInstance().GetText("ExchangeNumSelectPoint"), (long)((ulong)this._price * (ulong)((long)this._curMaxExchangeCount)), this._totalPirce));
					component2.gameObject.CustomSetActive(true);
				}
				else
				{
		...
		public void OpenStringSenderBox(string title, string desc, string stringPlacer, CUIManager.StringSendboxOnSend onSendCallback, string defaultString = "")
		{
			...
			if (desc != null)
			{
				...
				widget2.GetComponent<Text>().set_text(desc);
			}
			if (stringPlacer != null)
			{
				GameObject widget3 = cUIFormScript.GetWidget(3);
				widget3.GetComponent<Text>().set_text(stringPlacer);
			}
			GameObject widget4 = cUIFormScript.GetWidget(0);
			widget4.GetComponent<InputField>().set_text(defaultString);
