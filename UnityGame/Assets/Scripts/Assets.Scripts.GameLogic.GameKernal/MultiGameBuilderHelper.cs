...
namespace Assets.Scripts.GameLogic.GameKernal
{
	internal class MultiGameBuilderHelper : BaseBuilderHelper
	{
		...
		{
			base.PreLoad();
			Singleton<FrameSynchr>.GetInstance().SetSynchrConfig(Singleton<GameContextEx>.GetInstance().GameContextMobaInfo.KFrapsFreqMs, (uint)Singleton<GameContextEx>.GetInstance().GameContextMobaInfo.KFrapsLater, (uint)Singleton<GameContextEx>.GetInstance().GameContextMobaInfo.PreActFrap, Singleton<GameContextEx>.GetInstance().GameContextMobaInfo.RandomSeed);
		}
