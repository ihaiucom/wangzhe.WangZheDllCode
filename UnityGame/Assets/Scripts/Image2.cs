...
public class Image2 : Image
{
	...
	public Material baseMaterial
	{
		get
		{
			...
			{
				return (this.alphaTexLayout == ImageAlphaTexLayout.None) ? Graphic.get_defaultGraphicMaterial() : Image2.defaultMaterial;
			}
	...
	private void UpdateInternalState()
	{
		if (this.m_ShouldRecalculate)
		{
			...
			Image2.s_components.Clear();
			while (base.get_maskable() && parent != null)
			{
				parent.GetComponents(typeof(IMask), Image2.s_components);
				if (Image2.s_components.get_Count() > 0)
				{
	...
	{
		Vector4 vector = (!(base.get_overrideSprite() == null)) ? DataUtility.GetPadding(base.get_overrideSprite()) : Vector4.zero;
		Vector2 vector2 = (!(base.get_overrideSprite() == null)) ? new Vector2(base.get_overrideSprite().rect.width * sizeScaling.x, base.get_overrideSprite().rect.height * sizeScaling.y) : Vector2.zero;
		...
		if (shouldPreserveAspect && (double)vector2.sqrMagnitude > 0.0)
		{
			...
			if ((double)num3 > (double)num4)
			{
				...
				pixelAdjustedRect.y += (height - pixelAdjustedRect.height) * base.get_rectTransform().pivot.y;
			}
			else
			{
				float width = pixelAdjustedRect.width;
				pixelAdjustedRect.width = pixelAdjustedRect.height * num3;
				pixelAdjustedRect.x += (width - pixelAdjustedRect.width) * base.get_rectTransform().pivot.x;
			}
		}
	...
	private void GenerateSimpleSprite(List<UIVertex> vbo, bool preserveAspect)
	{
		...
		Vector4 vector = (!(base.get_overrideSprite() != null)) ? Vector4.zero : DataUtility.GetOuterUV(base.get_overrideSprite());
		float y = vector.y;
		float w = vector.w;
		float y2 = (this.alphaTexLayout == ImageAlphaTexLayout.Vertical) ? ((y + w) * 0.5f) : w;
		float y3 = (this.alphaTexLayout == ImageAlphaTexLayout.Vertical) ? ((y + w) * 0.5f) : y;
		float x = vector.x;
		float z = vector.z;
		float x2 = (this.alphaTexLayout == ImageAlphaTexLayout.Horizonatal) ? ((x + z) * 0.5f) : z;
		float x3 = (this.alphaTexLayout == ImageAlphaTexLayout.Horizonatal) ? ((x + z) * 0.5f) : x;
	...
	private Vector4 GetAdjustedBorders(Vector4 border, Rect rect)
	{
		for (int i = 0; i <= 1; i++)
		{
			...
			if ((double)rect.size[i] < (double)num && (double)num != 0.0)
			{
				...
				int index;
				float num3 = border[index = i];
				border[index] = num3 * num2;
				int index2;
				num3 = border[index2 = i + 2];
	...
	{
		if (!base.get_overrideSprite())
		{
			return;
		}
		Vector2 vector = Image2.s_sizeScaling[(int)this.alphaTexLayout];
		float x = base.get_overrideSprite().rect.width * vector.x / base.get_pixelsPerUnit();
		float y = base.get_overrideSprite().rect.height * vector.y / base.get_pixelsPerUnit();
		base.get_rectTransform().anchorMax = base.get_rectTransform().anchorMin;
		base.get_rectTransform().sizeDelta = new Vector2(x, y);
		this.SetAllDirty();
	}
