...
namespace Assets.Scripts.GameSystem
{
	internal class CBuySkillPt : CBuyPanelBase
	{
		...
		private void RefreshUI()
		{
			...
			if (!base.bTimesOut)
			{
				this.m_content.set_text(Singleton<CTextManager>.GetInstance().GetText("BuyAct_NoSkillPoint", new string[]
				{
			...
			{
				this.m_content.set_text(Singleton<CTextManager>.GetInstance().GetText("BuyAct_NoMore", new string[]
				{
					this.CurBuyTime.ToString(),
		...
		protected override uint GetRequireDianquan(int times)
		{
			...
			for (int i = 0; i < times; i++)
			{
				...
				if ((ulong)(dwCoinPrice + num) > (ulong)-1)
				{
					return 4294967295u;
				}
				num += dwCoinPrice;
