	...
	internal class CLobbySystem : Singleton<CLobbySystem>
	{
		...
		public void Play_UnLock_Animation(RES_SPECIALFUNCUNLOCK_TYPE type)
		{
			...
			case RES_SPECIALFUNCUNLOCK_TYPE.RES_SPECIALFUNCUNLOCKTYPE_UNION:
				text = "SocialBtn";
				goto IL_A5;
			case RES_SPECIALFUNCUNLOCK_TYPE.RES_SPECIALFUNCUNLOCKTYPE_SYMBOLSHOP:
			case RES_SPECIALFUNCUNLOCK_TYPE.RES_SPECIALFUNCUNLOCKTYPE_PVPCOINSHOP:
			case RES_SPECIALFUNCUNLOCK_TYPE.RES_SPECIALFUNCUNLOCKTYPE_GUILDSHOP:
			case RES_SPECIALFUNCUNLOCK_TYPE.RES_SPECIALFUNCUNLOCKTYPE_PVPRANK:
				IL_39:
				if (type == RES_SPECIALFUNCUNLOCK_TYPE.RES_SPECIALFUNCUNLOCKTYPE_SYMBOL)
				{
					text = "SymbolBtn";
					goto IL_A5;
				}
				if (type != RES_SPECIALFUNCUNLOCK_TYPE.RES_SPECIALFUNCUNLOCKTYPE_ACHIEVEMENT)
				{
					goto IL_A5;
				}
				text = "AchievementBtn";
			...
			if (!string.IsNullOrEmpty(text))
			{
				...
				CUIAnimatorScript component = Singleton<CUIManager>.instance.OpenForm(CLobbySystem.LOBBY_FUN_UNLOCK_PATH, false, true).GetComponent<CUIAnimatorScript>();
		...
		private void RefreshRankList()
		{
			...
			else if (this.m_rankingType == RankingSystem.RankingSubView.All)
			{
				...
				if (rankList != null)
				{
					int dwItemNum = (int)rankList.dwItemNum;
					componetInChild.SetElementAmount(dwItemNum);
					for (int j = 0; j < dwItemNum; j++)
					{
		...
		private void OnUpdateRankingFriendElement(GameObject go, int index)
		{
			...
			int num = (index > this.myRankingNo) ? -1 : 0;
		...
		private void InitForm(CUIFormScript form)
		{
			...
			if (component)
			{
				...
				if (transform8)
				{
					transform8.gameObject.CustomSetActive(!component.get_interactable());
			...
			if (component2)
			{
				...
				component2.set_interactable(!flag);
			...
			else
			{
				...
				if (GameDataMgr.globalInfoDatabin.GetDataByKey(204u).dwConfValue > 0u)
				{
					...
					component3.set_text(Singleton<CTextManager>.GetInstance().GetText("CrossPlat_Plat_Channel_Open_Lobby_Msg"));
				}
				else
				{
					widget.CustomSetActive(!CLobbySystem.IsPlatChannelOpen);
					component3.set_text(Singleton<CTextManager>.GetInstance().GetText("CrossPlat_Plat_Channel_Not_Open_Lobby_Msg"));
				}
			}
		}
		...
		private void OnPlayerLvlChange()
		{
			if (this.m_LobbyForm)
			{
				...
				if (component)
				{
					...
					if (transform2)
					{
						transform2.gameObject.CustomSetActive(!component.get_interactable());
		...
		private void UpdateGameCenterState(CUIFormScript form)
		{
			...
			CRoleInfo masterRoleInfo = Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo();
			COM_PRIVILEGE_TYPE privilegeType = (masterRoleInfo == null) ? COM_PRIVILEGE_TYPE.COM_PRIVILEGE_TYPE_NONE : masterRoleInfo.m_privilegeType;
		...
		public void CheckWifi()
		{
			...
			num = ((num > 100) ? ((num - 100) * 7 / 10 + 100) : num);
			...
			if (this.m_wifiIconCheckTicks == -1 || this.m_wifiIconCheckTicks >= this.m_wifiIconCheckMaxTicks)
			{
				...
				{
					CUICommonSystem.PlayAnimator(this.m_wifiIcon, CLobbySystem.s_wifiStateName[(int)((uint)((UIntPtr)num2))]);
				}
				else if (netWorkType == enNetWorkType.enNet)
				{
					CUICommonSystem.PlayAnimator(this.m_wifiIcon, CLobbySystem.s_netStateName[(int)((uint)((UIntPtr)num2))]);
				}
