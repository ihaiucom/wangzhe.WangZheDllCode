...
namespace Assets.Scripts.GameSystem
{
	public abstract class Activity
	{
		...
		public event Activity.ActivityEvent OnTimeStateChange
		{
			...
			add
			{
				this.OnTimeStateChange = (Activity.ActivityEvent)Delegate.Combine(this.OnTimeStateChange, value);
			}
			[MethodImpl(32)]
			remove
			{
				this.OnTimeStateChange = (Activity.ActivityEvent)Delegate.Remove(this.OnTimeStateChange, value);
			}
		}
		...
		public event Activity.ActivityEvent OnMaskStateChange
		{
			...
			add
			{
				this.OnMaskStateChange = (Activity.ActivityEvent)Delegate.Combine(this.OnMaskStateChange, value);
			}
			[MethodImpl(32)]
			remove
			{
				this.OnMaskStateChange = (Activity.ActivityEvent)Delegate.Remove(this.OnMaskStateChange, value);
			}
		}
		...
		public virtual bool IsImageTitle
		{
			...
			{
				string text = Utility.UTF8Convert(this._config.szTitle);
				return text.get_Length() > 2 && text.IndexOf("i:", 0, 2) == 0;
			}
		}
		...
		public virtual string Tips
		{
			get
			{
				...
				text = text2.Replace(text3, current.ToString());
		...
		public virtual RES_WEAL_COLORBAR_TYPE FlagType
		{
			get
			{
				...
				if (rES_WEAL_COLORBAR_TYPE == RES_WEAL_COLORBAR_TYPE.RES_WEAL_COLORBAR_TYPE_NEW && this.StartTime > 0L)
				{
					...
					if ((dateTime - Utility.ToUtcTime2Local(this.StartTime)).get_TotalDays() > 2.0)
					{
		...
		public string UID
		{
			get
			{
				...
				{
					"sgame_",
					(accountInfo != null) ? accountInfo.OpenId : "0",
					"_activity_",
					this.Key
				});
			}
		...
		public string PeriodText
		{
			get
			{
				...
				return (text != text2) ? (text + " ~ " + text2) : text;
			}
		}
		...
		public bool Visited
		{
			...
			set
			{
				...
				{
					this._visited = value;
					PlayerPrefs.SetInt(this.UID, this._visited ? 1 : 0);
					PlayerPrefs.Save();
		...
		{
			return (uint)((uint)type << 28 | (COM_WEAL_TYPE)(268435455u & id));
		}
