...
namespace Assets.Scripts.GameSystem
{
	public class ScoreBoard
	{
		...
		public void Init(GameObject obj, GameObject objTime)
		{
			...
			this._lastTimeSec = 0;
			Utility.GetComponetInChild<Text>(this._root, "Kill").set_text("0");
			Utility.GetComponetInChild<Text>(this._root, "Death").set_text("0");
			Utility.GetComponetInChild<Text>(this._root, "Assist").set_text("0");
		}
		...
		public void Update()
		{
			...
			if (num != this._lastTimeSec)
			{
				if (Singleton<WinLoseByStarSys>.instance.CurLevelTimeDuration <= 0u)
				{
					...
					this._txtTime.set_text(string.Format("{0:D2}:{1:D2}", num2, num3));
				}
				else
				{
					...
					this._txtTime.set_text(string.Format("{0:D2}:{1:D2}", num6, num7));
				}
		...
		private void validateScore(COM_PLAYERCAMP campType, int inCampScore, int inHeadPts)
		{
			...
			if (campType == Singleton<GamePlayerCenter>.instance.GetHostPlayer().PlayerCamp)
			{
				...
				{
					this._leftScore.set_text(string.Format(Singleton<CTextManager>.GetInstance().GetText("ScoreBoard_FireHole_1"), inHeadPts, this.levelContext.m_headPtsUpperLimit));
				}
				else
				{
					this._leftScore.set_text(string.Format(Singleton<CTextManager>.GetInstance().GetText("ScoreBoard_Normal_1"), inHeadPts));
				}
			}
			else if (campType != COM_PLAYERCAMP.COM_PLAYERCAMP_MID && campType != COM_PLAYERCAMP.COM_PLAYERCAMP_COUNT)
			{
				if (this.levelContext.m_headPtsUpperLimit > 0)
				{
					this._rightScore.set_text(string.Format(Singleton<CTextManager>.GetInstance().GetText("ScoreBoard_FireHole_2"), inHeadPts, this.levelContext.m_headPtsUpperLimit));
				}
				else
				{
					this._rightScore.set_text(string.Format(Singleton<CTextManager>.GetInstance().GetText("ScoreBoard_Normal_2"), inHeadPts));
				}
			}
		}
