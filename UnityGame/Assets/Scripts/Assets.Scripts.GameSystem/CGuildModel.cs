...
namespace Assets.Scripts.GameSystem
{
	public class CGuildModel : Singleton<CGuildModel>
	{
		...
		{
			List<stApplicantInfo> list = new List<stApplicantInfo>(this.m_ApplicantDic.get_Count());
			Dictionary<ulong, stApplicantInfo>.Enumerator enumerator = this.m_ApplicantDic.GetEnumerator();
			while (enumerator.MoveNext())
			{
				List<stApplicantInfo> list2 = list;
				KeyValuePair<ulong, stApplicantInfo> current = enumerator.get_Current();
				list2.Add(current.get_Value());
			}
			return list;
		}
		...
		{
			return (this.m_ApplicantDic == null) ? 0 : this.m_ApplicantDic.get_Count();
		}
		...
		{
			int count = applicants.get_Count();
			for (int i = 0; i < count; i++)
			{
				this.AddApplicant(applicants.get_Item(i));
			}
		}
		...
		{
			for (int i = 0; i < infoList.get_Count(); i++)
			{
				this.AddRecommendInfo(infoList.get_Item(i));
			}
		}
		...
		{
			List<stRecommendInfo> list = new List<stRecommendInfo>(this.m_recommendInfoDic.get_Count());
			Dictionary<ulong, stRecommendInfo>.Enumerator enumerator = this.m_recommendInfoDic.GetEnumerator();
			while (enumerator.MoveNext())
			{
				List<stRecommendInfo> list2 = list;
				KeyValuePair<ulong, stRecommendInfo> current = enumerator.get_Current();
				list2.Add(current.get_Value());
			}
			return list;
		}
		...
		{
			return this.m_recommendTimeInfoDic.ContainsKey(uid) ? this.m_recommendTimeInfoDic.get_Item(uid) : -1;
		}

		public int GetInviteTimeInfoByUid(ulong uid)
		{
			return this.m_inviteTimeInfoDic.ContainsKey(uid) ? this.m_inviteTimeInfoDic.get_Item(uid) : -1;
		}

		public int GetRecommendInfoCount()
		{
			return (this.m_recommendInfoDic == null) ? 0 : this.m_recommendInfoDic.get_Count();
		}
		...
		public stRecommendInfo GetRecommendInfoByUid(ulong uulUid)
		{
			...
			if (this.m_recommendInfoDic.ContainsKey(uulUid))
			{
				this.m_recommendInfoDic.TryGetValue(uulUid, ref result);
				return result;
			}
			return result;
		}
