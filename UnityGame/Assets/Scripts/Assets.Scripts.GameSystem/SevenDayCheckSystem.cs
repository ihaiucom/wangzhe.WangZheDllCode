	...
	public class SevenDayCheckSystem : Singleton<SevenDayCheckSystem>
	{
		...
		private void UpdateCheckView()
		{
			if (this._curActivity != null)
			{
				...
				transform4.gameObject.GetComponent<Text>().set_text(Singleton<CTextManager>.instance.GetText("SevenCheckIn_Title"));
				Text component = this._form.gameObject.transform.FindChild("Panel/Desc").gameObject.GetComponent<Text>();
				component.set_text(Singleton<CTextManager>.instance.GetText("SevenCheckIn_Desc"));
				if (Singleton<ApolloHelper>.instance.IsWXGameCenter)
				{
					...
					component2.set_text(Singleton<CTextManager>.instance.GetText("SevenCheckIn_WeChatPrivilege_Desc"));
				}
				else
				{
		...
		private void SetItem(CUseable usable, GameObject uiNode, bool received, bool ready, uint vipLv)
		{
			...
			if (transform2 != null)
			{
				...
				{
					component3.set_text(usable.m_stackCount.ToString());
				}
				else
				{
					component3.set_text(usable.m_stackCount / 10000 + "万");
				}
				...
				if (usable.m_type == COM_ITEM_TYPE.COM_OBJTYPE_ITEMSYMBOL)
				{
					...
					{
						component3.set_text(usable.GetSalableCount().ToString());
					}
				}
			}
			if (received)
