	...
	public class MinimapSys
	{
		...
		public void Init(CUIFormScript formObj, SLevelContext levelContext)
		{
			...
			if (levelContext.IsMobaMode())
			{
				...
				rectTransform.anchoredPosition = new Vector2((float)this.mmSpt3D.textureWidth * 0.5f, -(float)this.mmSpt3D.textureHeight * 0.5f);
				...
				rectTransform2.anchoredPosition = new Vector2((float)this.bmSpt3D.textureWidth * 0.5f, -(float)this.bmSpt3D.textureHeight * 0.5f);
		...
		public void ChangeBigMapSide(bool bLeft)
		{
			...
			if (this.bmBGMap_3dui != null && this.bmRoot != null && this.bmUGUIRoot != null)
			{
				...
				this.m_bmFinalScreenPos.x = (bLeft ? ((float)component.textureWidth * 0.5f * Sprite3D.Ratio()) : ((float)Screen.width - (float)component.textureWidth * 0.5f * Sprite3D.Ratio()));
				...
				if (this.ownerForm != null)
				{
					...
					float x = bLeft ? ((float)component.textureWidth * 0.5f) : (this.ownerForm.ChangeScreenValueToForm((float)Screen.width) - (float)component.textureWidth * 0.5f);
					rectTransform.anchoredPosition = new Vector2(x, -(float)component.textureHeight * 0.5f);
				}
				if (this.m_dragonIcon != null)
				{
		...
		{
			int num = bMiniMap ? levelContext.m_mapWidth : levelContext.m_bigMapWidth;
			int num2 = bMiniMap ? levelContext.m_mapHeight : levelContext.m_bigMapHeight;
			...
			if (null != sprite)
			{
				float x = bMiniMap ? levelContext.m_mapFowScale : levelContext.m_bigMapFowScale;
				float y = levelContext.m_isCameraFlip ? 0f : 1f;
		...
		public void EnableShowFow(MinimapSys.EMapType type, bool bShowFow)
		{
			...
			Sprite3D sprite3D = flag ? this.mmSpt3D : this.bmSpt3D;
			if (sprite3D == null)
			{
				return;
			}
			SLevelContext curLvelContext = Singleton<BattleLogic>.GetInstance().GetCurLvelContext();
			float x = flag ? curLvelContext.m_mapFowScale : curLvelContext.m_bigMapFowScale;
			if (!bShowFow)
			{
				x = 0f;
			}
			...
			{
				sprite3D.atlas.material.SetVector("_FowParams", new Vector4(x, curLvelContext.m_isCameraFlip ? 0f : 1f, 1f, 1f));
			}
		}
		...
		private void OnEvent_EyePerish(ref EyePerishWarnParam prm)
		{
			...
			if (this.CurMapType() == MinimapSys.EMapType.Big || this.CurMapType() == MinimapSys.EMapType.Skill)
			{
				...
				UIParticleInfo uIParticleInfo = Singleton<CUIParticleSystem>.instance.AddParticle(MiniMapEffectModule.EyePerishEft, (float)(prm.LeftTime / 1000 + 2), sreenLoc, default(Quaternion?));
			...
			else if (this.CurMapType() == MinimapSys.EMapType.Mini)
			{
				...
				UIParticleInfo uIParticleInfo2 = Singleton<CUIParticleSystem>.instance.AddParticle(MiniMapEffectModule.EyePerishEft, (float)(prm.LeftTime / 1000 + 2), sreenLoc2, default(Quaternion?));
		...
		private void OnBuildingAttacked(ref DefaultGameEventParam evtParam)
		{
			...
			if (organType == 1 || organType == 4 || organType == 2)
			{
				...
				{
					Sprite3D sprite3D = smallTower_Spt3D;
					float fillAmount = single;
					bigTower_Spt3D.fillAmount = fillAmount;
					sprite3D.fillAmount = fillAmount;
				}
			}
		}
		...
		private void send_signal(CUIEvent uiEvent, GameObject img, MinimapSys.ElementType elementType = MinimapSys.ElementType.None, int signal_id = 0)
		{
			...
			SignalPanel signalPanel = (Singleton<CBattleSystem>.GetInstance().FightForm != null) ? Singleton<CBattleSystem>.instance.FightForm.GetSignalPanel() : null;
			if (signalPanel != null)
			{
				if (eMapType == MinimapSys.EMapType.Skill)
				{
		...
		public static void Send_Position_Signal(CUIEvent uiEvent, GameObject img, byte signal_id, MinimapSys.ElementType type, bool bBigMap = true)
		{
			...
			Vector2 vector = bBigMap ? theMinimapSys.GetBMFianlScreenPos() : theMinimapSys.GetMMFianlScreenPos();
