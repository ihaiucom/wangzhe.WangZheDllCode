...
namespace Assets.Scripts.GameSystem
{
	public class SignalPanel
	{
		...
		public void OnClickMiniMap(CUIEvent uiEvent)
		{
			...
			ActorRoot actorRoot = (hostPlayer == null) ? null : hostPlayer.Captain.handle;
			...
			zero.x = (uiEvent.m_pointerEventData.get_position().x - this.m_miniMapScreenPosition.x) * Singleton<CBattleSystem>.GetInstance().UI_world_Factor_Small.x;
			zero.y = ((actorRoot != null) ? ((Vector3)actorRoot.location).y : 0.15f);
			zero.z = (uiEvent.m_pointerEventData.get_position().y - this.m_miniMapScreenPosition.y) * Singleton<CBattleSystem>.GetInstance().UI_world_Factor_Small.y;
			VInt vInt;
		...
		public void ExecCommand_SignalMiniMap_Position(uint senderPlayerID, byte signalID, ref VInt3 worldPos, byte elementType)
		{
			...
			if (signalID < 200)
			{
				this.ExecCommand(senderPlayerID, (uint)configId, (int)signalID, worldPos.x, worldPos.y, worldPos.z, flag ? 1 : 0, elementType, 0u);
			}
			else
			{
		...
		public void ExecCommand_SignalMiniMap_Target(uint senderPlayerID, byte signalID, byte type, uint targetObjID)
		{
			...
			this.ExecCommand(actor, senderPlayerID, (uint)configId2, (int)signalID, flag ? 1 : 0, type, 0u, (uint)configId);
		}
		...
		public void ExecCommand(uint senderPlayerID, uint heroID, int signalID, int worldPositionX, int worldPositionY, int worldPositionZ, byte bAlice = 0, byte elementType = 0, uint targetHeroID = 0u)
		{
			...
			senderPlayerID = ((elementType != 0) ? 0u : senderPlayerID);
		...
		private void PlaySignalTipsSound(byte elementType, byte bAlice, uint targetHeroID)
		{
			...
			{
			case 1:
				text = ((bAlice == 1) ? "Play_notice_map_2" : "Play_notice_map_1");
				break;
			case 2:
				text = ((bAlice == 1) ? "Play_sys_bobao_jihe_7" : "Play_sys_bobao_jihe_6");
				break;
			case 3:
			{
				...
				text = ((bAlice == 1) ? "Play_Call_Guard" : "Play_Call_Attack");
				break;
			}
			case 4:
