...
namespace Assets.Scripts.GameSystem
{
	...
	internal class CUILoadingSystem : Singleton<CUILoadingSystem>
	{
		...
		public static void OnMultiGameLoadProcess(CSPkg msg)
		{
			...
			if (memberItem != null)
			{
				...
				if (gameObject)
				{
					...
					component.set_text(string.Format("{0}%", msg.stPkgData.stMultGameLoadProcessRsp.wProcess));
				}
			}
		}
		...
		private static Player GetPlayer(COM_PLAYERCAMP Camp, int Pos)
		{
			...
			while (enumerator.MoveNext())
			{
				if (Pos == enumerator.get_Current().CampPos + 1)
				{
		...
		private static uint GetPlayerId(COM_PLAYERCAMP Camp, int Pos)
		{
			...
			while (enumerator.MoveNext())
			{
				if (enumerator.get_Current().PlayerCamp == Camp && Pos == enumerator.get_Current().CampPos + 1)
				{
					return enumerator.get_Current().PlayerId;
				}
			}
			return 0u;
		}
