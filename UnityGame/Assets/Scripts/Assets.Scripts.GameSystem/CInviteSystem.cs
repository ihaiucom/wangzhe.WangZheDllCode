...
namespace Assets.Scripts.GameSystem
{
	...
	internal class CInviteSystem : Singleton<CInviteSystem>
	{
		...
		private void OnInvite_SendInviteFriend(CUIEvent uiEvent)
		{
			...
			COMDT_FRIEND_INFO cOMDT_FRIEND_INFO = (info == null) ? info2 : info;
			...
			if (srcWidget != null)
			{
				...
				if (parent != null)
				{
					...
					if (transform)
					{
						...
						component.set_text(Singleton<CTextManager>.GetInstance().GetText("Guild_Has_Invited"));
					}
				}
			}
		}
			...
			if (form)
			{
				...
				for (int i = 0; i < component.m_elementAmount; i++)
				{
					...
					if (elemenet != null)
					{
						...
						if (component2.get_text() == username)
						{
							return elemenet;
						}
					}
		...
		private void OnInviteErrCodeNtf(COM_INVITE_JOIN_TYPE inviteType, byte errorCode, string userName)
		{
			if ((inviteType == COM_INVITE_JOIN_TYPE.COM_INVITE_JOIN_ROOM && errorCode == 14) || (inviteType == COM_INVITE_JOIN_TYPE.COM_INVITE_JOIN_TEAM && errorCode == 6))
			{
				...
				if (listItem != null)
				{
					...
					component.set_text(string.Format("<color=#ff0000>{0}</color>", Singleton<CTextManager>.instance.GetText("Invite_Friend_Tips_2")));
				}
		...
		{
			return CInviteSystem.IsGuildMatchInvite() ? Singleton<CGuildMatchSystem>.GetInstance().GetGuildMemberInviteList() : this.m_allGuildMemberList;
		}
