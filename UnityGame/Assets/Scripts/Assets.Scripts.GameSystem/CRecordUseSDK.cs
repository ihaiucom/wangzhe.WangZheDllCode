	...
	public class CRecordUseSDK : Singleton<CRecordUseSDK>
	{
			...
			if (Singleton<GamePlayerCenter>.instance != null && Singleton<GamePlayerCenter>.instance.GetHostPlayer() != null && prm.atker != Singleton<GamePlayerCenter>.instance.GetHostPlayer().Captain)
			{
				if (prm.src && prm.src.handle.ActorControl != null)
				{
					...
					while (enumerator.MoveNext())
					{
						...
						if (current.get_Key() == Singleton<GamePlayerCenter>.instance.GetHostPlayer().Captain.handle.ObjID)
						{
				...
				if (prm.atker && prm.atker.handle.ActorControl != null)
				{
					...
					while (enumerator2.MoveNext())
					{
						...
						if (current2.get_Key() == Singleton<GamePlayerCenter>.instance.GetHostPlayer().Captain.handle.ObjID)
						{
							this.UpdateRecordEvent(Singleton<GamePlayerCenter>.instance.GetHostPlayer().Captain, CRecordUseSDK.RECORD_EVENT_PRIORITY.RECORD_EVENT_TYPE_ASSIST);
							return;
						}
		...
		private void ChooseTopEvent()
		{
			...
			for (CRecordUseSDK.RECORD_EVENT_PRIORITY rECORD_EVENT_PRIORITY = CRecordUseSDK.RECORD_EVENT_PRIORITY.RECORD_EVENT_TYPE_PENTAKILL; rECORD_EVENT_PRIORITY > CRecordUseSDK.RECORD_EVENT_PRIORITY.RECORD_EVENT_TYPE_DOUBLEKILL; rECORD_EVENT_PRIORITY--)
			{
				if (this.m_stRecordInfo != null && this.m_stRecordInfo.TryGetValue(rECORD_EVENT_PRIORITY, ref sortedList2) && sortedList2 != null)
				{
					...
					while (enumerator.MoveNext())
					{
						...
						{
							KeyValuePair<long, long> current2 = enumerator.get_Current();
							num3 = current2.get_Key() - (long)((ulong)this.m_uiEventStartTimeInterval);
						}
						long num4 = num3;
						KeyValuePair<long, long> current3 = enumerator.get_Current();
						long num5 = current3.get_Value() + (long)((ulong)this.m_uiEventEndTimeInterval);
						num5 = ((num5 > this.m_lGameEndTime) ? this.m_lGameEndTime : num5);
						num += (int)(num5 - num4);
						...
						if (!sortedList.ContainsKey(num4))
						{
							sortedList.Add(num4, new CRecordUseSDK.RECORD_INFO(rECORD_EVENT_PRIORITY, num5));
						}
					}
				}
				if (flag)
					...
					if (this.m_stRecordInfo != null && this.m_stRecordInfo.TryGetValue(rECORD_EVENT_PRIORITY2, ref sortedList2) && sortedList2 != null)
					{
						...
						while (enumerator2.MoveNext())
						{
							...
							else
							{
								...
								num6 = current5.get_Key() - (long)((ulong)this.m_uiEventStartTimeInterval);
							}
							long num7 = num6;
							KeyValuePair<long, long> current6 = enumerator2.get_Current();
							long num8 = current6.get_Value() + (long)((ulong)this.m_uiEventEndTimeInterval) - (long)((ulong)this.m_uiOnceDoubleEventTimeIntervalReduce);
							num8 = ((num8 > this.m_lGameEndTime) ? this.m_lGameEndTime : num8);
							if (!flag2)
							{
								num2++;
								if ((long)num2 > (long)((ulong)this.m_uiEventNumMax))
								...
								{
									sortedList.Add(num7, new CRecordUseSDK.RECORD_INFO(rECORD_EVENT_PRIORITY2, num8));
								}
							}
							else
							{
								int assistCountWithTime = this.GetAssistCountWithTime((float)num7, (float)num8);
								this.InsertAssistInfo(ref sortedList3, assistCountWithTime, num7, num8);
							}
						}
				...
				if (!flag && this.m_stRecordInfo != null && this.m_stRecordInfo.TryGetValue(CRecordUseSDK.RECORD_EVENT_PRIORITY.RECORD_EVENT_TYPE_ASSIST, ref sortedList2) && sortedList2 != null)
				{
					...
					while (enumerator4.MoveNext())
					{
						...
						else
						{
							...
							num9 = current10.get_Key() - (long)((ulong)this.m_uiEventStartTimeInterval);
						}
						long num10 = num9;
						KeyValuePair<long, long> current11 = enumerator4.get_Current();
						long num11 = current11.get_Value() + (long)((ulong)this.m_uiEventEndTimeInterval) - (long)((ulong)this.m_uiOnceDoubleEventTimeIntervalReduce);
						num11 = ((num11 > this.m_lGameEndTime) ? this.m_lGameEndTime : num11);
						num2++;
						if ((long)num2 > (long)((ulong)this.m_uiEventNumMax))
						{
							break;
						...
						if (!sortedList.ContainsKey(num10))
						{
							sortedList.Add(num10, new CRecordUseSDK.RECORD_INFO(CRecordUseSDK.RECORD_EVENT_PRIORITY.RECORD_EVENT_TYPE_ASSIST, num11));
						}
					}
				}
			}
			...
			while (enumerator5.MoveNext())
			{
				...
				long num14 = current12.get_Key();
				KeyValuePair<long, CRecordUseSDK.RECORD_INFO> current13 = enumerator5.get_Current();
				long lEndTime = current13.get_Value().lEndTime;
				if (list.get_Count() > 0 && num12 > num14)
				{
					list.RemoveAt(list.get_Count() - 1);
					num -= (int)(num12 - num13);
					num12 = (num12 + num14) / 2L;
					num14 = num12;
					num += (int)(num12 - num13);
					list.Add(new TimeStamp(num13, num12));
				}
				num13 = num14;
				num12 = lEndTime;
				list.Add(new TimeStamp(num14, lEndTime));
			}
		...
		private void SetExtraInfos()
		{
			...
			if (this.m_hostActor)
			{
				...
				if (!string.IsNullOrEmpty(this.m_strHostHeroName))
				{
					...
					string text = this.m_strHostHeroName.Substring(num + 1, this.m_strHostHeroName.get_Length() - num - 2);
					this.m_extraInfos.Add("HN", text);
				}
				...
				this.m_extraInfos.Add("MVP", (this.m_bIsMvp ? 1 : 0).ToString());
			}
		...
		private string GetVideoName()
		{
			...
			{
				int num = this.m_strHostHeroName.IndexOf('(');
				string text2 = this.m_strHostHeroName.Substring(num + 1, this.m_strHostHeroName.get_Length() - num - 2);
				text += text2;
			}
		...
		{
			this.m_enmCheckPermissionRes = (bRes ? CRecordUseSDK.CHECK_PERMISSION_STUTAS.CHECK_PERMISSION_STUTAS_TYPE_PERMISSIONOK : CRecordUseSDK.CHECK_PERMISSION_STUTAS.CHECK_PERMISSION_STUTAS_TYPE_NOPERMISSION);
