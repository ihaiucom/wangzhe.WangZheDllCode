	...
	public class CVersionUpdateSystem : MonoSingleton<CVersionUpdateSystem>
	{
		...
		private void StartDownloadApp()
		{
			...
			if (CVersionUpdateSystem.s_platform == enPlatform.Android)
			{
				...
				this.m_reportAppDownloadStartTime = DateTime.get_Now();
		...
		{
			CVersionUpdateSystem.<CheckFirstExtractResource>c__Iterator24 <CheckFirstExtractResource>c__Iterator = new CVersionUpdateSystem.<CheckFirstExtractResource>c__Iterator24();
			<CheckFirstExtractResource>c__Iterator.<>f__this = this;
			return <CheckFirstExtractResource>c__Iterator;
		}
		...
		private void StartDownloadResource()
		{
			...
			this.m_reportResourceDownloadStartTime = DateTime.get_Now();
		...
		{
			CVersionUpdateSystem.<VersionUpdateComplete>c__Iterator25 <VersionUpdateComplete>c__Iterator = new CVersionUpdateSystem.<VersionUpdateComplete>c__Iterator25();
			<VersionUpdateComplete>c__Iterator.<>f__this = this;
			return <VersionUpdateComplete>c__Iterator;
		}
		...
		private string GetCheckAppVersionJsonConfig(enPlatform platform)
		{
			...
			if (platform == enPlatform.IOS)
			{
				result = string.Format("{{\r\n                            \"m_update_type\" : 4,\r\n                            \"log_debug\" : {5},\r\n\r\n                            \"basic_version\":\r\n                            {{\r\n                                \"m_server_url_list\" : [{0}],\r\n                                \"m_app_id\" : {1},\r\n                                \"m_service_id\" : {2},\r\n                                \"m_current_version_str\" : \"{3}\",\r\n                                \"m_retry_count\" : {4},\r\n                                \"m_retry_interval_ms\" : 1000,\r\n                                \"m_connect_timeout_ms\" : 1000,\r\n                                \"m_send_timeout_ms\" : 2000,\r\n                                \"m_recv_timeout_ms\" : 3000                                \r\n                            }}\r\n                    }}", new object[]
				{
					...
					CVersionUpdateSystem.s_logDebug ? "true" : "false"
				});
			}
		...
		private string GetCheckResourceVersionJsonConfig(enPlatform platform)
		{
			...
			return string.Format("{{\r\n                        \"basic_update\":\r\n                        {{\r\n                            \"m_ifs_save_path\" : \"{4}\",\r\n                            \"m_nextaction\" : \"basic_diffupdata\"\r\n                        }},\r\n                \r\n                        \"full_diff\":\r\n\t\t\t\t        {{ \r\n\t\t\t\t\t        \"m_ifs_save_path\":\"{4}\",\r\n\t\t\t\t\t        \"m_file_extract_path\":\"{5}\"\r\n\t\t\t\t        }},\r\n                \r\n                        \"m_update_type\" : 5,\r\n                        \"log_debug\" : {8},\r\n                        {7}\r\n\r\n                        \"basic_version\":\r\n                        {{\r\n                            \"m_server_url_list\" : [{0}],\r\n                            \"m_app_id\" : {1},\r\n                            \"m_service_id\" : {2},\r\n                            \"m_current_version_str\" : \"{3}\",\r\n                            \"m_retry_count\" : {6},\r\n\t\t                    \"m_retry_interval_ms\" : 1000,\r\n\t\t                    \"m_connect_timeout_ms\" : 1000,\r\n\t\t                    \"m_send_timeout_ms\" : 2000,\r\n\t\t                    \"m_recv_timeout_ms\" : 3000\r\n                        }}\r\n                    }}\r\n                ", new object[]
			{
				...
				CVersionUpdateSystem.s_ifsExtractPath,
				this.GetIIPSServerAmount() + 2,
				this.m_useCurseResourceDownloadSize ? "\"need_down_size\" : true," : string.Empty,
				CVersionUpdateSystem.s_logDebug ? "true" : "false"
			});
		}
		...
		public byte OnGetNewVersionInfo(IIPSMobileVersionCallBack.VERSIONINFO newVersionInfo)
		{
			...
			else if (this.IsInUpdateResourceStage())
			{
				...
				if (CVersionUpdateSystem.I2B((int)newVersionInfo.isAppUpdating))
				{
					if (this.m_resourceCheckCompatibility && !this.CheckResourceCompatibility(this.m_appIsDelayToInstall ? this.m_downloadAppVersion : CVersion.GetAppVersion(), this.m_downloadResourceVersion))
					{
		...
		{
			this.UpdateUIProgress(enVersionUpdateFormWidget.Slider_SingleProgress, enVersionUpdateFormWidget.Text_SinglePercent, (totalSize == 0uL) ? 0f : (nowSize / totalSize));
			...
			case IIPSMobileVersionCallBack.VERSIONSTAGE.VS_FullUpdate_Extract:
				goto IL_6A;
			case IIPSMobileVersionCallBack.VERSIONSTAGE.VS_DiffUpdata:
			case IIPSMobileVersionCallBack.VERSIONSTAGE.VS_FullUpdate_GetFileList:
			case IIPSMobileVersionCallBack.VERSIONSTAGE.VS_FullUpdate_GetMetaFile:
			case IIPSMobileVersionCallBack.VERSIONSTAGE.VS_FullUpdate_CompareMetaFile:
			case IIPSMobileVersionCallBack.VERSIONSTAGE.VS_DownApkConfig:
				IL_55:
				if (curVersionStage == IIPSMobileVersionCallBack.VERSIONSTAGE.VS_SourceDownload)
				{
					goto IL_80;
				}
				if (curVersionStage != IIPSMobileVersionCallBack.VERSIONSTAGE.VS_SourceExtract)
				{
					return;
				}
				goto IL_6A;
		...
		{
			if (CVersionUpdateSystem.s_platform == enPlatform.Android && !this.m_previousKeyVersionLoaded && this.m_versionUpdateState >= enVersionUpdateState.StartCheckAppVersion && this.m_versionUpdateState <= enVersionUpdateState.CheckAppVersion)
			{
		...
		public void OnDownloadYYBProgressChanged(string msg)
		{
			...
			this.UpdateUIProgress(enVersionUpdateFormWidget.Slider_SingleProgress, enVersionUpdateFormWidget.Text_SinglePercent, (float)num / (float)((num2 == 0) ? num : num2));
		...
		private string GetParamValueInContent(string titleContent, string param)
		{
			...
			for (int i = 0; i < array.Length; i++)
			{
				string text;
				string result;
				this.GetParamPair(array[i], out text, out result);
				if (string.Equals(param, text))
				{
					return result;
				}
			}
		...
		private string GetErrorResult(uint errorCode)
		{
			...
			{
			case 1:
				key = (this.IsUseWifi() ? "IIPS_Error_Result_NetworkError" : "IIPS_Error_Result_NetworkError_NoWifi");
				break;
			case 2:
			...
			case 5:
				...
				break;
			case 6:
				key = (this.IsUseWifi() ? "IIPS_Error_Result_NoSupportUpdate" : "IIPS_Error_Result_NoSupportUpdate_NoWifi");
				break;
			case 7:
				key = "IIPS_Error_Result_NotSure";
				break;
			case 8:
				key = (this.IsUseWifi() ? "IIPS_Error_Result_ApkException" : "IIPS_Error_Result_ApkException_NoWifi");
				break;
			}
		...
		{
			GameObject gameObject = (!(this.m_versionUpdateFormScript != null)) ? null : this.m_versionUpdateFormScript.GetWidget(5);
			if (gameObject != null)
			{
				gameObject.CustomSetActive(false);
			}
			GameObject gameObject2 = (!(this.m_versionUpdateFormScript != null)) ? null : this.m_versionUpdateFormScript.GetWidget(17);
			if (gameObject2 != null)
			{
		...
		{
			GameObject gameObject = (!(this.m_versionUpdateFormScript != null)) ? null : this.m_versionUpdateFormScript.GetWidget(7);
			if (gameObject != null)
			{
				gameObject.CustomSetActive(false);
			}
		...
		private void SetUpdateNotice(string noticeContent)
		{
			...
			if (widget != null)
			{
				...
				if (component != null)
				{
					component.set_text(noticeContent);
				}
			}
		}
		...
		private void SetConfirmPanelButton(enVersionUpdateFormWidget widgetIndex, bool active, string textKey, enUIEventID eventID)
		{
			...
			if (!string.IsNullOrEmpty(textKey))
			{
				...
				if (transform != null)
				{
					...
					if (component != null)
					{
						component.set_text(Singleton<CTextManager>.GetInstance().GetText(textKey));
					}
				}
			}
		...
		private void SendVersionUpdateReportEvent(DateTime startTime, int versionType, string currentVersion, string updateVersion, bool isSuccessful, uint errorCode, uint totalDownloadSize, uint downloadSize, string downloadUrl)
		{
			...
			try
			{
				...
				list.Add(new KeyValuePair<string, string>("errorCode", (isSuccessful ? 0 : 1).ToString()));
				...
				list.Add(new KeyValuePair<string, string>("totaltime", (DateTime.get_Now() - startTime).get_TotalMilliseconds().ToString()));
		...
		{
			return string.Format("{0}.{1}.0.0", (CVersionUpdateSystem.s_appType == enAppType.Exp) ? "0" : "1", this.m_previousKeyVersion);
		}

		private string GetPreviousVirualNodeVersion()
		{
			return string.Format("{0}.{1}.1.0", (CVersionUpdateSystem.s_appType == enAppType.Exp) ? "0" : "1", this.m_previousKeyVersion);
		}
