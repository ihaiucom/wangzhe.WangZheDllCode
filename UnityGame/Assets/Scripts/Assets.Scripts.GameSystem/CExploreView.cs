	...
	public class CExploreView
	{
		...
		public static void OnExploreListScroll(GameObject root)
		{
			...
			if (component != null)
			{
				...
				zero.x = ((contentSize.x == scrollAreaSize.x) ? 0f : (contentPosition.x / (contentSize.x - scrollAreaSize.x)));
				...
				float zAngle = (num == 0f) ? 0f : (num / (1f / (float)(CAdventureSys.CHAPTER_NUM - 1)) * 120f);
