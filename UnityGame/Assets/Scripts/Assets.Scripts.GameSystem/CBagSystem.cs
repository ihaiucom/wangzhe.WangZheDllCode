	...
	public class CBagSystem : Singleton<CBagSystem>
	{
		...
		private CUseableContainer GetContainerBySelectType(CUseableContainer allContainer)
		{
			...
			else if (this.m_selectUseableType == enItemMenuType.RecentGet)
			{
				...
				{
					CUseable useableByIndex2 = allContainer.GetUseableByIndex(j);
					if ((dateTime.get_Date() - Utility.ToUtcTime2Local((long)useableByIndex2.m_getTime).get_Date()).get_Days() <= 3 && (useableByIndex2.m_type != COM_ITEM_TYPE.COM_OBJTYPE_ITEMSYMBOL || useableByIndex2.GetSalableCount() > 0))
					{
						this.m_ContainerRecentGet.Add(useableByIndex2);
					}
				}
			...
			else if (this.m_selectUseableType == enItemMenuType.Item)
			{
				...
				{
					CUseable useableByIndex3 = allContainer.GetUseableByIndex(k);
					if (useableByIndex3.m_type == COM_ITEM_TYPE.COM_OBJTYPE_ITEMPROP)
					{
						CItem cItem = (CItem)useableByIndex3;
			...
			else if (this.m_selectUseableType == enItemMenuType.Gift)
			{
				...
				{
					CUseable useableByIndex4 = allContainer.GetUseableByIndex(l);
					if (useableByIndex4.m_type == COM_ITEM_TYPE.COM_OBJTYPE_ITEMPROP)
					{
						CItem cItem2 = (CItem)useableByIndex4;
			...
			else if (this.m_selectUseableType == enItemMenuType.ExpCard)
			{
				...
				{
					CUseable useableByIndex5 = allContainer.GetUseableByIndex(m);
					if (useableByIndex5.m_type == COM_ITEM_TYPE.COM_OBJTYPE_ITEMPROP)
					{
						CItem cItem3 = (CItem)useableByIndex5;
			...
			else if (this.m_selectUseableType == enItemMenuType.Symbol)
			{
				...
				{
					CUseable useableByIndex6 = allContainer.GetUseableByIndex(n);
					if (useableByIndex6.m_type == COM_ITEM_TYPE.COM_OBJTYPE_ITEMSYMBOL && useableByIndex6.GetSalableCount() > 0)
					{
						this.m_ContainerSymbol.Add(useableByIndex6);
					}
				}
		...
		private void RefreshSelectItem(CUIFormScript formScript, GameObject root, int selectIndex)
		{
			...
			component3.set_text(this.m_selectUseable.GetSalableCount().ToString());
			component4.set_text(this.m_selectUseable.m_description);
			component5.set_text(this.m_selectUseable.m_coinSale.ToString());
		...
		private void OnBag_OpenSaleForm(CUIEvent uiEvent)
		{
			...
			component.set_text(this.m_selectUseable.m_name);
			component2.set_text(this.m_selectUseable.GetSalableCount().ToString());
			component3.set_text(this.m_selectUseable.m_coinSale.ToString());
			component4.set_text(this.m_saleCount + "/" + this.m_selectUseable.GetSalableCount());
			component5.set_text(((long)this.m_saleCount * (long)((ulong)this.m_selectUseable.m_coinSale)).ToString());
		}
		...
		private void OnBag_SaleForm_CountDown(CUIEvent uiEvent)
		{
			...
			component2.set_text(((long)this.m_saleCount * (long)((ulong)this.m_selectUseable.m_coinSale)).ToString());
		}
		...
		private void OnBag_OpenUseForm(CUIEvent uiEvent)
		{
			if (this.m_selectUseable.m_type == COM_ITEM_TYPE.COM_OBJTYPE_ITEMPROP)
			{
				...
				if (cItem.m_itemData.bType == 1 || cItem.m_itemData.bType == 7 || cItem.m_itemData.bType == 4 || cItem.m_itemData.bType == 10 || cItem.m_itemData.bType == 11)
				{
					...
					{
						component3.set_text(this.m_useCount + "/" + cItem.m_itemData.wBatchUseCnt);
					}
					else
					{
						component3.set_text(this.m_useCount + "/" + this.m_selectUseable.GetSalableCount());
					}
				}
		...
		private void OnBag_UseForm_CountUp(CUIEvent uiEvent)
		{
			...
			component.set_text(this.m_useCount + "/" + num);
		}
		...
		private void OnTips_ItemSourceInfoOpen(CUIEvent uiEvent)
		{
			...
			component2.set_text(CUIUtility.StringReplace(iconUseable.m_description, new string[]
			{
		...
		private CUseableContainer GetAutoSaleUseableContainer(ref uint salePrice)
		{
			...
			for (int i = 0; i < curUseableCount; i++)
			{
				...
				if (useableByIndex.m_type == COM_ITEM_TYPE.COM_OBJTYPE_ITEMPROP && useableByIndex.m_isSale > 0 && cUseableContainer.GetCurUseableCount() < 5)
				{
					...
					{
						DateTime dateTime2 = Utility.ULongToDateTime(ullInvalidTime);
			...
			if (list != null)
			{
				...
				for (int j = 0; j < num; j++)
				{
					...
					if (bType == 1)
					{
						...
						component2.set_text(dataByKey.iChapterId + "-" + dataByKey.bLevelNo);
						if (component5 != null)
						{
		...
		public static ListView<IHeroData> GetExpCardHeroList()
		{
			...
			{
				listView.Add(CHeroDataFactory.CreateHeroData(list.get_Item(k)));
			}
			return listView;
		}
		...
		public static void ReciveItemAdd(CSPkg msg)
		{
			...
			for (int i = 0; i < (int)stAddList.wItemCnt; i++)
			{
				...
				if (cUseable != null && cUseable != null && cUseable.m_stackCount >= cUseable.m_stackMax && (cUseable.m_type != COM_ITEM_TYPE.COM_OBJTYPE_ITEMSYMBOL || !((CSymbolItem)cUseable).IsGuildSymbol()))
				{
