...
namespace Assets.Scripts.GameSystem
{
	...
	public class CHeroOverviewSystem : Singleton<CHeroOverviewSystem>
	{
		...
		private void OnHeroSortTypeSelect(CUIEvent uiEvent)
		{
			...
			if (widget2 != null)
			{
				widget2.GetComponent<Text>().set_text(CMallSortHelper.CreateHeroViewSorter().GetSortTypeName(this.m_heroSortType));
			}
		}
		...
		private void OnHeroOwnFlagChange(CUIEvent uiEvent)
		{
			...
			if (widget != null)
			{
				...
				PlayerPrefs.SetInt("Hero_Overview_Hero_Own_Flag_Key", this.m_ownFlag ? 1 : 0);
				PlayerPrefs.Save();
			}
			this.RefreshHeroOwnFlag();
			this.ResetHeroListData(true);
