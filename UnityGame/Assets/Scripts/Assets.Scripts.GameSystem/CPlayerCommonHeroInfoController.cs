...
namespace Assets.Scripts.GameSystem
{
	public class CPlayerCommonHeroInfoController : Singleton<CPlayerCommonHeroInfoController>
	{
		...
		public void UpdateUI()
		{
			...
			componetInChild4.set_text(CPlayerCommonHeroInfoController.sSubTitles[selectedIndex]);
		...
		private void UpdateSortMenu()
		{
			...
			{
				this.m_lastIsUp = ((this.m_lastIsUp == 1) ? 0 : 1);
			}
		...
		private void OnCommonHeroItemEnable(CUIEvent uiEvent)
		{
			...
			componetInChild.set_text(heroData.heroName);
			Text componetInChild2 = Utility.GetComponetInChild<Text>(gameObject, "TotalCount");
			componetInChild2.set_text(Singleton<CTextManager>.instance.GetText("Player_Info_PVP_Total_Count", new string[]
			{
			...
			componetInChild3.set_text(Singleton<CTextManager>.instance.GetText("Player_Info_PVP_Win_Rate", new string[]
			{
			...
			num4 = ((num4 == 0u) ? 1u : num4);
			Utility.GetComponetInChild<Text>(gameObject, "AverKDA").set_text(Singleton<CTextManager>.instance.GetText("Player_Info_PVP_AverKDA", new string[]
			{
				(num / (ulong)num4 / 100uL).ToString("0.0")
			}));
			Utility.GetComponetInChild<Text>(gameObject, "AverHurt").set_text(Singleton<CTextManager>.instance.GetText("Player_Info_PVP_AverHurt", new string[]
			{
				(num2 / (ulong)num4).ToString("d")
			}));
			Utility.GetComponetInChild<Text>(gameObject, "AverTakenHurt").set_text(Singleton<CTextManager>.instance.GetText("Player_Info_PVP_AverTakenHurt", new string[]
			{
		...
		public void OnCommonHeroListEnable(CUIEvent uiEvent)
		{
			...
			if (srcWidgetIndexInBelongedList >= 0 && srcWidgetIndexInBelongedList < this.m_commonHeroList.Count)
			{
				...
				componetInChild2.set_text((dataByKey == null) ? string.Empty : dataByKey.szName);
				GameObject widget = form.GetWidget(0);
				...
				if (cOMDT_MOST_USED_HERO_INFO.stStatisticDetail.dwNum > 0u)
				{
					...
					if (gameType == 0)
					{
						componetInChild3.set_text(b.ToString());
						componetInChild4.set_text(CPlayerProfile.Divide(dwGameWinNum, b).ToString("P0"));
					}
					else
					{
						componetInChild3.set_text(b2.ToString());
						componetInChild4.set_text(CPlayerProfile.Divide(num, b2).ToString("P0"));
					}
					componetInChild5.set_text(string.Format("{0}/{1}/{2}", CPlayerProfile.Divide(num3, b2).ToString("F1"), CPlayerProfile.Divide(num4, b2).ToString("F1"), CPlayerProfile.Divide(num5, b2).ToString("F1")));
					componetInChild6.set_text(CPlayerProfile.Divide(num6, b2).ToString("F0"));
					componetInChild7.set_text(CPlayerProfile.Divide(num7, b2).ToString("F0"));
					componetInChild8.set_text(CPlayerProfile.Divide(num8, b2).ToString("F0"));
				}
				else
				{
					componetInChild3.set_text("0");
					componetInChild4.set_text("0");
					componetInChild5.set_text("0.0/0.0/0.0");
					componetInChild6.set_text("0");
					componetInChild7.set_text("0");
					componetInChild8.set_text("0");
				}
			}
		}
		...
		private int CommonHeroSortOrder(COMDT_MOST_USED_HERO_INFO left, COMDT_MOST_USED_HERO_INFO right)
		{
			...
			return (int)(((this.m_lastIsUp == 1) ? 4294967295u : 1u) * (dataByKey2.dwShowSortId - dataByKey.dwShowSortId));
		}

		private int CommonHeroSortUsedCnt(COMDT_MOST_USED_HERO_INFO left, COMDT_MOST_USED_HERO_INFO right)
		{
			return (int)(((this.m_lastIsUp == 1) ? 4294967295u : 1u) * (right.dwGameWinNum + right.dwGameLoseNum - left.dwGameWinNum - left.dwGameLoseNum));
		}

		private int CommonHeroSortWins(COMDT_MOST_USED_HERO_INFO left, COMDT_MOST_USED_HERO_INFO right)
		{
			return (int)(((this.m_lastIsUp == 1) ? 4294967295u : 1u) * (1000u * right.dwGameWinNum / (right.dwGameWinNum + right.dwGameLoseNum) - 1000u * left.dwGameWinNum / (left.dwGameWinNum + left.dwGameLoseNum)));
		}

		private int CommonHeroSortKda(COMDT_MOST_USED_HERO_INFO left, COMDT_MOST_USED_HERO_INFO right)
		{
			float num = (left.stStatisticDetail.dwNum > 0u) ? (CPlayerProfile.Divide(left.stStatisticDetail.astTypeDetail[0].ullKDAPct, left.stStatisticDetail.astTypeDetail[0].dwWinNum + left.stStatisticDetail.astTypeDetail[0].dwLoseNum) * 100f) : 0f;
			float num2 = (right.stStatisticDetail.dwNum > 0u) ? (CPlayerProfile.Divide(right.stStatisticDetail.astTypeDetail[0].ullKDAPct, right.stStatisticDetail.astTypeDetail[0].dwWinNum + right.stStatisticDetail.astTypeDetail[0].dwLoseNum) * 100f) : 0f;
		...
		{
			float num = (left.stStatisticDetail.dwNum > 0u) ? (CPlayerProfile.Divide(left.stStatisticDetail.astTypeDetail[0].dwGPM, left.stStatisticDetail.astTypeDetail[0].dwWinNum + left.stStatisticDetail.astTypeDetail[0].dwLoseNum) * 100f) : 0f;
			float num2 = (right.stStatisticDetail.dwNum > 0u) ? (CPlayerProfile.Divide(right.stStatisticDetail.astTypeDetail[0].dwGPM, right.stStatisticDetail.astTypeDetail[0].dwWinNum + right.stStatisticDetail.astTypeDetail[0].dwLoseNum) * 100f) : 0f;
			return ((this.m_lastIsUp == 1) ? -1 : 1) * ((int)num2 - (int)num);
		}
		...
		{
			float num = (left.stStatisticDetail.dwNum > 0u) ? (CPlayerProfile.Divide(left.stStatisticDetail.astTypeDetail[0].ullTotalBeHurt, left.stStatisticDetail.astTypeDetail[0].dwWinNum + left.stStatisticDetail.astTypeDetail[0].dwLoseNum) * 100f) : 0f;
			float num2 = (right.stStatisticDetail.dwNum > 0u) ? (CPlayerProfile.Divide(right.stStatisticDetail.astTypeDetail[0].ullTotalBeHurt, right.stStatisticDetail.astTypeDetail[0].dwWinNum + right.stStatisticDetail.astTypeDetail[0].dwLoseNum) * 100f) : 0f;
			return ((this.m_lastIsUp == 1) ? -1 : 1) * ((int)num2 - (int)num);
		}
