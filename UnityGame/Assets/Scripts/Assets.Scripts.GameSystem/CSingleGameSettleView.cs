...
namespace Assets.Scripts.GameSystem
{
	internal class CSingleGameSettleView
	{
		...
		private static void SetPlayerStat(CUIFormScript formScript, GameObject item, PlayerKDA playerKDA, HeroKDA kda)
		{
			...
			componetInChild.set_color((playerKDA.PlayerCamp == COM_PLAYERCAMP.COM_PLAYERCAMP_1) ? new Color(0.031f, 0.961f, 0f, 1f) : new Color(0.545f, 0f, 0f, 1f));
			...
			item.transform.Find("Txt_KDA").gameObject.GetComponent<Text>().set_text(string.Format("{0} / {1}", text, text2));
			item.transform.Find("Txt_Hurt").gameObject.GetComponent<Text>().set_text(kda.hurtToEnemy.ToString());
			item.transform.Find("Txt_HurtTaken").gameObject.GetComponent<Text>().set_text(kda.hurtTakenByEnemy.ToString());
			item.transform.Find("Txt_Heal").gameObject.GetComponent<Text>().set_text(kda.hurtToHero.ToString());
