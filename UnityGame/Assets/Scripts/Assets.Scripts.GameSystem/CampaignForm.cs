		...
		public class ActivityMenuItem
		{
			...
			public void Validate()
			{
				...
				if (flagType != RES_WEAL_COLORBAR_TYPE.RES_WEAL_COLORBAR_TYPE_NULL)
				{
					...
					this.flagText.set_text(Singleton<CTextManager>.GetInstance().GetText(text));
				}
				else
				{
		...
		private void onInitTimer(int seq)
		{
			...
			case 2:
			{
				...
				for (int k = 0; k < num; k++)
				{
					...
					if (componetInChild2)
					{
						componetInChild2.set_text(MonoSingleton<PandroaSys>.GetInstance().GetPandoraTabName(PandroaSys.PandoraModuleType.action, k));
					}
				...
				if (componetInChild3)
				{
					componetInChild3.set_text("游戏公告");
				}
