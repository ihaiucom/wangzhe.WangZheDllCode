	...
	public class CStarEvalPanel
	{
		...
		public void reset()
		{
			...
			while (enumerator.MoveNext())
			{
				...
				{
					this.conditionTexts[enumerator.Current.index].set_color(Color.green);
				}
				else
				{
					this.conditionTexts[enumerator.Current.index].set_color(Color.white);
				}
			}
		...
		public void OnEvaluationChange(IStarEvaluation InStarEvaluation, IStarCondition InStarCondition)
		{
			...
			if (transform)
			{
				...
				{
					transform.Find(string.Format("Condition{0}", InStarEvaluation.index + 1)).GetComponent<Text>().set_color(Color.green);
				}
				else
				{
					transform.Find(string.Format("Condition{0}", InStarEvaluation.index + 1)).GetComponent<Text>().set_color(Color.white);
				}
			}
		}
		...
		public void openTaskPanel(CUIEvent uiEvent)
		{
			...
			eventParams.tag = (this.bPanelOpen ? 5 : 6);
			...
			this.taskPanel.GetComponent<Animator>().Play(this.bPanelOpen ? "Form_Battle_EvalPlanel_out" : "Form_Battle_EvalPlanel_in");
			this.arrowIcon.GetComponent<Animator>().Play(this.bPanelOpen ? "Form_Battle_EvalPlanel_out2" : "Form_Battle_EvalPlanel_in2");
