...
namespace Assets.Scripts.GameSystem
{
	...
	public class VoiceInteractionEncounterAllies : VoiceInteraction
	{
		...
		private bool CheckActor(ref PoolObjHandle<ActorRoot> InTestActor, ref PoolObjHandle<ActorRoot> HostActor)
		{
			...
			{
				PoolObjHandle<ActorRoot> current = enumerator.get_Current();
				if (!(current == InTestActor) && current && current.handle.IsSelfCamp(InTestActor) && current.handle.HorizonMarker != null && base.ValidateTriggerActor(ref current) && current.handle.HorizonMarker.IsVisibleFor(HostActor.handle.TheActorMeta.ActorCamp) && this.CheckTriggerDistance(ref InTestActor, ref current))
				{
