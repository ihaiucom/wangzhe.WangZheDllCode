...
namespace Assets.Scripts.GameSystem
{
	public class ShareSys : MonoSingleton<ShareSys>
	{
		...
		{
			RectTransform rectTransform = (displayeRect != null) ? displayeRect.GetComponent<RectTransform>() : new RectTransform();
		...
		{
			CUI3DImageScript cUI3DImageScript = (rawImage != null) ? rawImage.GetComponent<CUI3DImageScript>() : null;
			string objectName = CUICommonSystem.GetHeroPrefabPath(heroId, (int)skinId, true).ObjectName;
			GameObject gameObject = (cUI3DImageScript != null) ? cUI3DImageScript.AddGameObject(objectName, false, false) : null;
		...
		private void LoadShareSkinImage(ShareSys.CLoadReq loadReq, Image imageObj)
		{
			...
			string text = CFileManager.CombinePath(cachePath, path);
			if (File.Exists(text))
			{
		...
		private bool SharePicIsExist(string url, string tagPath, int type)
		{
			...
			string path = MonoSingleton<IDIPSys>.GetInstance().ToMD5(cDNSharePicUrl);
			string text = CFileManager.CombinePath(tagPath, path);
		...
		public void ShowNewSkinShare(uint heroId, uint skinId, bool bInitAnima = true, COM_REWARDS_TYPE rewardType = COM_REWARDS_TYPE.COM_REWARDS_TYPE_HERO, bool interactableTransition = false)
		{
			...
			if (masterRoleInfo != null)
			{
				...
				component3.set_text(masterRoleInfo.GetHeroSkinCount(false).ToString());
			}
		}
		...
		public void ShowNewHeroShare(uint heroId, uint skinId, bool bInitAnima = true, COM_REWARDS_TYPE rewardType = COM_REWARDS_TYPE.COM_REWARDS_TYPE_HERO, bool interactableTransition = false)
		{
			...
			{
				Text component2 = cUIFormScript.GetWidget(1).GetComponent<Text>();
				component2.set_text(masterRoleInfo.GetHaveHeroCount(false).ToString());
		...
		private void UpdateTimelineBtn()
		{
			if (this.m_TimelineBtn != null)
			{
				...
				if (this.m_bShowTimeline && gameObject != null)
				{
					...
					gameObject.GetComponent<Button>().set_interactable(false);
					gameObject.GetComponent<Image>().set_color(new Color(gameObject.GetComponent<Image>().get_color().r, gameObject.GetComponent<Image>().get_color().g, gameObject.GetComponent<Image>().get_color().b, 0.37f));
			...
			base.StartCoroutine(this.Capture(screenShotRect, delegate(string filename)
			{
				...
				if (Application.platform == RuntimePlatform.Android)
				{
					try
					{
						...
						using (FileStream fileStream = new FileStream(this.m_sharePic, 3, 1))
						{
		...
		private string GenHashCodeFromString(string sKey)
		{
			...
			text2 += text;
			ulong num2 = 0uL;
			for (int i = 0; i < sKey.get_Length(); i++)
			{
				char c = sKey.get_Chars(i);
		...
		public void OnShareCallBack()
		{
			...
			if (!this.isRegisterd)
			{
				apolloSnsService.onShareEvent += delegate(ApolloShareResult shareResponseInfo)
				{
					...
					{
						if (!(shareResponseInfo.extInfo == ShareSys.SNS_SHARE_SEND_HEART) && !(shareResponseInfo.extInfo == ShareSys.SNS_SHARE_RECALL_FRIEND))
						{
		...
		private void Share(string buttonType, string sharePathPic)
		{
			...
			using (FileStream fileStream = new FileStream(sharePathPic, 3, 1))
			{
				byte[] array = new byte[fileStream.get_Length()];
				int num = Convert.ToInt32(fileStream.get_Length());
				fileStream.Read(array, 0, num);
		...
		private bool CheckEnterShareTeamLimit(ref string paramDevicePlatStr, ref string paramPlatformStr)
		{
			...
			int num = -1;
			if (!int.TryParse(paramPlatformStr, ref num) || num != (int)ApolloConfig.platform)
			{
		...
		public bool UnpackDownloadWatch(string data)
		{
			...
			int num = 0;
			if (!int.TryParse(array[4], ref num))
			{
				Singleton<CUIManager>.GetInstance().OpenTips(Singleton<CTextManager>.GetInstance().GetText("Download_ParamWrong"), false, 1.5f, null, new object[0]);
				return false;
			}
		...
		public bool UnpackGuildMatchInvite(string data)
		{
			...
			if (ulong.TryParse(array[2], ref num) && num != masterRoleInfo.m_baseGuildInfo.uulUid)
			{
		...
		public bool UnpackLaunchWatch(string data)
		{
			...
			if (int.TryParse(array[2], ref friendType) && uint.TryParse(array[3], ref cOMDT_ACNT_UNIQ.dwLogicWorldId) && ulong.TryParse(array[4], ref cOMDT_ACNT_UNIQ.ullUid))
			{
		...
		private bool UnPackSNSDataStr(string data)
		{
			...
			if (text3 != null)
			{
				if (ShareSys.<>f__switch$map1 == null)
				{
					Dictionary<string, int> dictionary = new Dictionary<string, int>(6);
					dictionary.Add("ShareRoom", 0);
					dictionary.Add("ShareTeam", 1);
					dictionary.Add("OpenForm", 2);
					dictionary.Add("watch", 3);
					dictionary.Add("DownloadWatch", 4);
					dictionary.Add("GuildMatchInvite", 5);
					ShareSys.<>f__switch$map1 = dictionary;
				}
				int num;
				if (ShareSys.<>f__switch$map1.TryGetValue(text3, ref num))
				{
					switch (num)
					{
					case 0:
		...
		public bool UnpackTeamData(string data)
		{
			...
			try
			{
				if (ulong.TryParse(array[1], ref this.m_ShareTeam.stInviterInfo.ullUid) && uint.TryParse(array[2], ref this.m_ShareTeam.stInviteDetail.stInviteJoinTeam.dwTeamId) && uint.TryParse(array[3], ref this.m_ShareTeam.stInviteDetail.stInviteJoinTeam.dwTeamSeq) && int.TryParse(array[4], ref this.m_ShareTeam.stInviteDetail.stInviteJoinTeam.iTeamEntity) && ulong.TryParse(array[5], ref this.m_ShareTeam.stInviteDetail.stInviteJoinTeam.ullTeamFeature) && byte.TryParse(array[6], ref this.m_ShareTeam.stInviteDetail.stInviteJoinTeam.bShowGradeOfInviter) && byte.TryParse(array[7], ref this.m_ShareTeam.stInviteDetail.stInviteJoinTeam.stTeamInfo.bGameMode) && byte.TryParse(array[8], ref this.m_ShareTeam.stInviteDetail.stInviteJoinTeam.stTeamInfo.bPkAI) && byte.TryParse(array[9], ref this.m_ShareTeam.stInviteDetail.stInviteJoinTeam.stTeamInfo.bMapType) && uint.TryParse(array[10], ref this.m_ShareTeam.stInviteDetail.stInviteJoinTeam.stTeamInfo.dwMapId) && byte.TryParse(array[11], ref this.m_ShareTeam.stInviteDetail.stInviteJoinTeam.stTeamInfo.bAILevel) && byte.TryParse(array[14], ref this.m_ShareTeam.stInviteDetail.stInviteJoinTeam.stTeamInfo.bMaxTeamNum) && byte.TryParse(array[15], ref this.m_ShareTeam.stInviteDetail.stInviteJoinTeam.stTeamInfo.bShowGradeOfRank))
				{
		...
		public bool UnPackQQGameTeamData(string data)
		{
			...
			if (cOM_ROOM_TYPE == COM_ROOM_TYPE.COM_ROOM_TYPE_MATCH)
			{
				...
				this.m_ShareTeam.iInviteType = 2;
				this.m_ShareTeam.stInviteDetail.stInviteJoinTeam = new COMDT_INVITE_TEAM_DETAIL();
				if (ulong.TryParse(array[4], ref this.m_ShareTeam.stInviterInfo.ullUid) && uint.TryParse(array[5], ref this.m_ShareTeam.stInviteDetail.stInviteJoinTeam.dwTeamId) && uint.TryParse(array[6], ref this.m_ShareTeam.stInviteDetail.stInviteJoinTeam.dwTeamSeq) && int.TryParse(array[7], ref this.m_ShareTeam.stInviteDetail.stInviteJoinTeam.iTeamEntity) && ulong.TryParse(array[8], ref this.m_ShareTeam.stInviteDetail.stInviteJoinTeam.ullTeamFeature) && byte.TryParse(array[9], ref this.m_ShareTeam.stInviteDetail.stInviteJoinTeam.bShowGradeOfInviter) && byte.TryParse(array[10], ref this.m_ShareTeam.stInviteDetail.stInviteJoinTeam.stTeamInfo.bGameMode) && byte.TryParse(array[11], ref this.m_ShareTeam.stInviteDetail.stInviteJoinTeam.stTeamInfo.bPkAI) && byte.TryParse(array[12], ref this.m_ShareTeam.stInviteDetail.stInviteJoinTeam.stTeamInfo.bAILevel) && byte.TryParse(array[13], ref this.m_ShareTeam.stInviteDetail.stInviteJoinTeam.stTeamInfo.bMaxTeamNum) && byte.TryParse(array[14], ref this.m_ShareTeam.stInviteDetail.stInviteJoinTeam.stTeamInfo.bShowGradeOfRank))
				{
					this.m_ShareTeam.stInviteDetail.stInviteJoinTeam.bGradeOfInviter = CLadderSystem.GetGradeDataByShowGrade((int)this.m_ShareTeam.stInviteDetail.stInviteJoinTeam.bShowGradeOfInviter).bLogicGrade;
					this.m_ShareTeam.stInviteDetail.stInviteJoinTeam.stTeamInfo.bGradeOfRank = CLadderSystem.GetGradeDataByShowGrade((int)this.m_ShareTeam.stInviteDetail.stInviteJoinTeam.stTeamInfo.bShowGradeOfRank).bLogicGrade;
		...
		private void SendQQGameTeamCreateMsg(string roomInfoStr)
		{
			...
			else if (num == 8)
			{
				if (Singleton<LobbyLogic>.instance.isLogin)
				{
					...
					if (int.TryParse(array[3], ref cOMDT_QQSPROT_EXTRA.iRoomEntity) && uint.TryParse(array[4], ref cOMDT_QQSPROT_EXTRA.dwRoomID) && byte.TryParse(array[5], ref cOMDT_QQSPROT_EXTRA.bAddType) && uint.TryParse(array[6], ref cOMDT_QQSPROT_EXTRA.dwTimeOutSec))
					{
