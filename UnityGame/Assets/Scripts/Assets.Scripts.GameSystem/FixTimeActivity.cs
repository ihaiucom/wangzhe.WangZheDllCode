	...
	public class FixTimeActivity : Activity
	{
		...
		public override bool CheckTimeState()
		{
			...
			for (int i = 0; i < this._config.astMultipleTime.Length; i++)
			{
				...
				if (resDT_DateTime.ullStartTime > 0uL && resDT_DateTime.ullEndTime > 0uL && dateTime >= Utility.ToUtcTime2Local((long)resDT_DateTime.ullStartTime) && dateTime < Utility.ToUtcTime2Local((long)resDT_DateTime.ullEndTime))
				{
					flag = true;
					break;
				}
