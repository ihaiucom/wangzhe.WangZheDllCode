...
namespace Assets.Scripts.GameSystem
{
	...
	public class CQualifyingSystem : Singleton<CQualifyingSystem>
	{
		...
		private void Qualifying_RankListElementInit(CUIEvent uiEvent)
		{
			...
			component2.set_text(StringHelper.UTF8BytesToString(ref cSDT_CLASSOFRANKDETAIL.stDetail.szAcntName));
			component3.set_text(cSDT_CLASSOFRANKDETAIL.stDetail.dwWinCnt.ToString());
			component4.set_text(cSDT_CLASSOFRANKDETAIL.stDetail.bScore.ToString());
		}
		...
		private void Qualifying_RankListSelect(CUIEvent uiEvent)
		{
			...
			for (int i = 0; i < cSDT_CLASSOFRANKDETAIL.stDetail.astCommonUseHero.Length; i++)
			{
				...
				else
				{
					...
					component3.set_text(cSDT_CLASSOFRANKDETAIL.stDetail.astCommonUseHero[i].dwHeroProficiency.ToString());
					gameObject3.CustomSetActive(true);
				}
			}
		}
		...
		public void RefreshHeroInfo()
		{
			...
			for (int i = 0; i < hostHeroList.Count; i++)
			{
				...
				component4.set_text(hostHeroList[i].proficiency.ToString());
			}
			uint num = 0u;
			...
			component.set_text(CUIUtility.StringReplace(Singleton<CTextManager>.GetInstance().GetText("Qualifying_Title0"), new string[]
			{
				num.ToString()
			}));
		}
		...
		{
			return (info2.stDetail.bScore > info1.stDetail.bScore) ? 1 : 0;
		}
