...
namespace Assets.Scripts.GameSystem
{
	...
	public class VoiceSys : MonoSingleton<VoiceSys>
	{
		...
		{
			VoiceSys.<UpdateEnterRoomState>c__Iterator2F <UpdateEnterRoomState>c__Iterator2F = new VoiceSys.<UpdateEnterRoomState>c__Iterator2F();
			<UpdateEnterRoomState>c__Iterator2F.<>f__this = this;
			return <UpdateEnterRoomState>c__Iterator2F;
		}
		...
		private void EnterRoomReal(SCPKG_CREATE_TVOIP_ROOM_NTF roomInfoSvr)
		{
			...
			if (this.ApolloVoiceMgr != null)
			{
				...
				if (MonoSingleton<CTongCaiSys>.instance.IsCanUseTongCai() && MonoSingleton<CTongCaiSys>.instance.IsUsingTongcaiIp())
				{
					...
					{
						array[j] = "udp://" + ApolloConfig.loginOnlyIpTongCai + ":" + text;
					}
				}
				else
				{
				...
				string text2 = string.Format("roomid is {0}, roomkey is {1}, openid is {2}, memberid is {3}, accIP is {4}, {5}, {6}\n", new object[]
				{
		...
		private void UpdateSoundLevel_HeroSelect(float soudLevel, float leftSecond)
		{
			if (this.m_SoundLevel_HeroSelect)
			{
				...
				this.m_SoundLevel_HeroSelect.Find("CountDown").GetComponent<Text>().set_text(string.Format("{0:0.00}", leftSecond));
			}
		}
		...
		private void UpdateHeroSelectVoiceBtnState(bool bEnable)
		{
			if (this.m_VoiceBtn)
			{
				if (!bEnable)
				{
					...
					this.m_VoiceBtn.GetComponent<Image>().set_color(new Color(this.m_VoiceBtn.GetComponent<Image>().get_color().r, this.m_VoiceBtn.GetComponent<Image>().get_color().g, this.m_VoiceBtn.GetComponent<Image>().get_color().b, 0.37f));
				...
				else
				{
					...
					this.m_VoiceBtn.GetComponent<Image>().set_color(new Color(this.m_VoiceBtn.GetComponent<Image>().get_color().r, this.m_VoiceBtn.GetComponent<Image>().get_color().g, this.m_VoiceBtn.GetComponent<Image>().get_color().b, 1f));
					Text componentInChildren2 = this.m_VoiceBtn.GetComponentInChildren<Text>();
					componentInChildren2.set_color(new Color(componentInChildren2.get_color().r, componentInChildren2.get_color().g, componentInChildren2.get_color().b, 1f));
				}
			}
		}
		...
		public void SetVoiceState(ulong uid, CS_VOICESTATE_TYPE state)
		{
			for (int i = 0; i < this.m_voiceStateList.get_Count(); i++)
			{
				...
				if (voiceState != null && voiceState.uid == uid)
				{
