...
public class CRoleRegisterSys : Singleton<CRoleRegisterSys>
{
	...
	private void OnRoleCreate(CUIEvent cuiEvent)
	{
		...
		case Utility.NameResult.Vaild:
		{
			...
			cSPkg.stPkgData.stAcntRegisterReq.szUserName = Encoding.get_UTF8().GetBytes(roleName);
	...
	public void ShowErrorCode(byte[] szName)
	{
		...
		int num = Math.Max(text.get_Length(), roleName.get_Length());
		for (int i = 0; i < num; i++)
		{
			if (!text.get_Chars(i).Equals(roleName.get_Chars(i)))
			{
				text2 = string.Format("{0}{1}", text2, roleName.get_Chars(i));
			}
		}
		Singleton<CUIManager>.GetInstance().OpenMessageBox(string.Format(Singleton<CTextManager>.GetInstance().GetText("Register_Name_Invalid_Words_2"), text2), false);
	}
