...
namespace Assets.Scripts.Framework
{
	public class DatabinTableBase
	{
		...
		{
			object result;
			if (this.bLoaded && this.mapItems.TryGetValue(key, ref result))
			{
				return result;
			}
			return null;
		...
		protected long GetDataKey(object data, Type InValueType)
		{
			...
			try
			{
				num3 = ((value3 != null) ? Convert.ToInt64(value3) : 0L);
			}
			catch (Exception ex2)
			{
		...
		{
			this.Reload();
			return this.mapItems.get_Count();
		}
