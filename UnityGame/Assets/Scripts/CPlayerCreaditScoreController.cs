...
public class CPlayerCreaditScoreController : Singleton<CPlayerCreaditScoreController>
{
	...
	private void UpdateCreditScore(CUIFormScript form)
	{
		...
		if (gameObject3 != null)
		{
			...
			if (this.m_CreditLevelInfo.bCreditLevelResult == 0)
			{
				...
				if (gameObject5 != null)
				{
					...
					component.set_text(Singleton<CTextManager>.instance.GetText("Credit_Punish_Title"));
				}
			}
			else
			{
				...
				if (gameObject6 != null)
				{
					...
					component2.set_text(Singleton<CTextManager>.instance.GetText("Credit_Reward_Title"));
				}
			}
			int num = 0;
		...
		if (gameObject4 != null)
		{
			...
			gameObject7.GetComponent<Image>().set_fillAmount(num2);
			componetInChild4.set_text(string.Format("{0}/{1}", profile.sumDelCreditValue, dwConfValue));
			componetInChild5.SetSprite(this.GetTipsIconByProportion(num2), form, true, false, false, false);
			componetInChild6.set_text(this.GetTipsByComplaintTypeAndProportion(profile.mostDelCreditType, num2));
		}
	}
