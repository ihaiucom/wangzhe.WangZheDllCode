...
namespace com.tencent.pandora
{
	internal class MsdkTea
	{
		...
		private static void TeaEncryptECB(byte[] inBuf, byte[] key, byte[] outBuf, int outBufStartPos)
		{
			...
			uint num3 = 0u;
			for (int j = 0; j < MsdkTea.kRounds; j++)
			{
				num3 += MsdkTea.kDelta;
			...
			while (i <= MsdkTea.kSaltLen)
			{
				...
				else if (num5 == 8)
				{
					...
					for (int j = 0; j < 8; j++)
					{
						...
						byte[] array5 = array;
						int num6 = j;
						byte[] expr_112_cp_0 = array5;
						int expr_112_cp_1 = num6;
						expr_112_cp_0[expr_112_cp_1] ^= inBuf[num + j];
					}
			...
			while (num7 != 0)
			{
				...
				else if (num5 == 8)
				{
					...
					for (int k = 0; k < 8; k++)
					{
						...
						byte[] array6 = array;
						int num8 = k;
						byte[] expr_1C5_cp_0 = array6;
						int expr_1C5_cp_1 = num8;
						expr_1C5_cp_0[expr_1C5_cp_1] ^= inBuf[num + k];
					}
					MsdkTea.TeaDecryptECB(array, key, array);
					num += 8;
					num3 += 8;
			...
			while (i <= MsdkTea.kZeroLen)
			{
				...
				else if (num5 == 8)
				{
					...
					for (int l = 0; l < 8; l++)
					{
						...
						byte[] array7 = array;
						int num9 = l;
						byte[] expr_270_cp_0 = array7;
						int expr_270_cp_1 = num9;
						expr_270_cp_0[expr_270_cp_1] ^= inBuf[num + l];
					}
					MsdkTea.TeaDecryptECB(array, key, array);
					num += 8;
					num3 += 8;
		...
		{
			byte[] bytes = Encoding.get_UTF8().GetBytes(rawData);
			byte[] bytes2 = Encoding.get_UTF8().GetBytes("msdkmsdkmsdkmsdk");
			int num = 0;
			byte[] array = new byte[bytes.Length + 18];
			MsdkTea.oi_symmetry_encrypt2(bytes, bytes.Length, bytes2, array, ref num);
			return Convert.ToBase64String(array, 0, num);
		}

		public static string Decode(byte[] encodedDataBytes)
		{
			byte[] bytes = Encoding.get_UTF8().GetBytes("msdkmsdkmsdkmsdk");
			...
			return Encoding.get_UTF8().GetString(array, 0, num);
		}
	}
}
