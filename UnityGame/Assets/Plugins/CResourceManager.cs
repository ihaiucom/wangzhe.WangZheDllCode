...
public class CResourceManager : Singleton<CResourceManager>
{
	...
	{
		CResourceManager.<LoadResidentAssetBundles>c__Iterator5 <LoadResidentAssetBundles>c__Iterator = new CResourceManager.<LoadResidentAssetBundles>c__Iterator5();
		<LoadResidentAssetBundles>c__Iterator.<>f__this = this;
		return <LoadResidentAssetBundles>c__Iterator;
	}
	...
	public void RemoveCachedResources(enResourceType resourceType, bool clearImmediately = true)
	{
		...
		while (enumerator.MoveNext())
		{
			...
			CResource value = current.get_Value();
		...
		{
			this.m_cachedResourceMap.Remove(list.get_Item(i));
		}
		if (clearImmediately)
		{
	...
	{
		Type result = null;
		if (string.Equals(extension, ".prefab", 5))
		{
			result = typeof(GameObject);
		}
		else if (string.Equals(extension, ".bytes", 5) || string.Equals(extension, ".xml", 5))
		{
			result = typeof(TextAsset);
		}
		else if (string.Equals(extension, ".asset", 5))
		{
	...
	private CResourcePackerInfo GetResourceBelongedPackerInfo(CResource resource)
	{
		if (this.m_resourcePackerInfoSet != null)
		{
			...
			if (resourceBelongedPackerInfo != null && !resourceBelongedPackerInfo.m_isAssetBundle)
			{
				...
				if (!resourceBelongedPackerInfo.m_fileExtMap.TryGetValue(resource.m_fullPathInResourcesWithoutExtension.JavaHashCodeIgnoreCase(), ref empty))
				{
