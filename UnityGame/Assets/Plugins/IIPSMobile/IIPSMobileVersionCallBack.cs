...
namespace IIPSMobile
{
	public class IIPSMobileVersionCallBack
	{
		...
		internal delegate byte OnNoticeInstallApkFunc(IntPtr callback, [MarshalAs(20)] string url);

		internal delegate byte OnActionMsgFunc(IntPtr callback, [MarshalAs(20)] string url);
		...
		~IIPSMobileVersionCallBack()
		{
			...
			{
				GCHandle.FromIntPtr(IIPSMobileVersionCallBack.GetCallBackGCHandle(this.mCallBack)).Free();
		...
		public static byte OnNoticeInstallApk(IntPtr callback, [MarshalAs(20)] string url)
		{
			IIPSMobileVersionCallBack.mImpCB = (IIPSMobileVersionCallBackInterface)GCHandle.FromIntPtr(callback).get_Target();
