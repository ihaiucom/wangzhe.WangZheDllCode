...
public class ClassEnumerator
{
	...
	public ClassEnumerator(Type InAttributeType, Type InInterfaceType, Assembly InAssembly, bool bIgnoreAbstract = true, bool bInheritAttribute = false, bool bShouldCrossAssembly = false)
	{
		...
		{
			Debug.LogError("Error in enumerate classes :" + ex.get_Message());
		}
	}
	...
	protected void CheckInAssembly(Assembly InAssembly, bool bInIgnoreAbstract, bool bInInheritAttribute)
	{
		...
		if (types != null)
		{
			...
			{
				Type type = types[i];
				if ((this.InterfaceType == null || this.InterfaceType.IsAssignableFrom(type)) && (!bInIgnoreAbstract || (bInIgnoreAbstract && !type.get_IsAbstract())) && type.GetCustomAttributes(this.AttributeType, bInInheritAttribute).Length > 0)
				{
