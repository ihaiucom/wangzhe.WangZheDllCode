...
public class NcSpriteAnimation : NcEffectAniBehaviour, IPooledMonoBehaviour
{
	...
	{
		return (float)((this.m_PlayMode == NcSpriteAnimation.PLAYMODE.PINGPONG) ? (this.m_nFrameCount * 2 - 1) : this.m_nFrameCount) / this.m_fFps;
	}
	...
	private void ResetLocalValue()
	{
		...
		this.m_nFrameCount = ((this.m_nFrameCount <= 0) ? (this.m_nTilingX * this.m_nTilingY) : this.m_nFrameCount);
	...
	private void Update()
	{
		...
		if (this.m_PlayMode != NcSpriteAnimation.PLAYMODE.RANDOM)
		{
			...
			{
				if (((this.m_PlayMode == NcSpriteAnimation.PLAYMODE.PINGPONG) ? (this.m_nFrameCount * 2 - 1) : this.m_nFrameCount) <= num)
				{
					if (!this.m_bLoop)
					{
	...
	private void SetIndex(int nSeqIndex)
	{
		if (this.m_Renderer != null)
		{
			...
			case NcSpriteAnimation.PLAYMODE.PINGPONG:
				nLoopCount = num / (this.m_nFrameCount * 2 - ((num == 0) ? 1 : 2));
				num %= this.m_nFrameCount * 2 - ((num == 0) ? 1 : 2);
				if (this.m_nFrameCount <= num)
				{
