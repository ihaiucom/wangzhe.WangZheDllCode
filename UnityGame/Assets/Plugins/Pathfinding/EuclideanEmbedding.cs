...
namespace Pathfinding
{
	...
	public class EuclideanEmbedding
	{
		...
		{
			using (IEnumerator enumerator = tr.GetEnumerator())
			{
				...
				for (int i = 0; i < graphs.Length; i++)
				{
					graphs[i].GetNodes(delegate(GraphNode node)
					{
						if (!node.Destroyed && node.Walkable)
						{
							...
							{
								if (pivotList.get_Count() < this.spreadOutCount)
								{
									pivotList.Add(node);
								}
								else
								{
									pivotList.set_Item((int)((ulong)this.GetRandom() % (ulong)((long)pivotList.get_Count())), node);
								}
							}
						}
						return true;
		...
		public void RecalculateCosts()
		{
				...
				fp.immediateCallback = delegate(Path _p)
				{
					...
					for (int m = graphs.Length - 1; m >= 0; m--)
					{
						graphs[m].GetNodes(delegate(GraphNode node)
						{
							...
							if (costOffset > 0u)
							{
								this.costs[num6] = ((pathNode.pathID == fp.pathID && pathNode.parent != null) ? Math.Max(pathNode.parent.G - costOffset, 0u) : 0u);
							}
							else
							{
								this.costs[num6] = ((pathNode.pathID == fp.pathID) ? pathNode.G : 0u);
							}
							return true;
						});
					}
					if (this.mode == HeuristicOptimizationMode.RandomSpreadOut && k < this.pivots.Length - 1)
					{
						...
						startCostCalculation.Invoke(k + 1);
					}
					_p.Release(this);
				};
			...
			if (this.mode != HeuristicOptimizationMode.RandomSpreadOut)
			{
				...
				{
					startCostCalculation.Invoke(l);
				}
			}
			else
			{
				startCostCalculation.Invoke(0);
			}
			this.dirty = false;
		}
