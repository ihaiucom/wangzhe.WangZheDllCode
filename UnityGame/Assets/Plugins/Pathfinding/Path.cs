...
namespace Pathfinding
{
	public abstract class Path
	{
		...
		public float GetTotalLength()
		{
			...
			float num = 0f;
			for (int i = 0; i < this.vectorPath.get_Count() - 1; i++)
			{
				num += Vector3.Distance((Vector3)this.vectorPath.get_Item(i), (Vector3)this.vectorPath.get_Item(i + 1));
			}
			return num;
		}
		...
		public uint CalculateHScore(GraphNode node)
		{
			...
			case Heuristic.Manhattan:
			{
				...
				uint num2 = (this.hTargetNode != null) ? AstarPath.active.euclideanEmbedding.GetHeuristic(node.NodeIndex, this.hTargetNode.NodeIndex) : 0u;
			...
			case Heuristic.DiagonalManhattan:
			{
				...
				int num3 = Math.Min(vInt.x, vInt.z);
				int num4 = Math.Max(vInt.x, vInt.z);
				uint num5 = (uint)((float)(14 * num3 / 10 + (num4 - num3) + vInt.y) * this.heuristicScale);
				uint num6 = (this.hTargetNode != null) ? AstarPath.active.euclideanEmbedding.GetHeuristic(node.NodeIndex, this.hTargetNode.NodeIndex) : 0u;
				return Math.Max(num5, num6);
			}
			case Heuristic.Euclidean:
			{
				uint num7 = (uint)((float)(this.GetHTarget() - node.position).costMagnitude * this.heuristicScale);
				uint num8 = (this.hTargetNode != null) ? AstarPath.active.euclideanEmbedding.GetHeuristic(node.NodeIndex, this.hTargetNode.NodeIndex) : 0u;
		...
		{
			return DateTime.get_UtcNow().get_Ticks() >= targetTime;
		}
		...
		public void Claim(object o)
		{
			...
			for (int i = 0; i < this.claimed.get_Count(); i++)
			{
				if (object.ReferenceEquals(this.claimed.get_Item(i), o))
				{
		...
		public void ReleaseSilent(object o)
		{
			...
			for (int i = 0; i < this.claimed.get_Count(); i++)
			{
				if (object.ReferenceEquals(this.claimed.get_Item(i), o))
				{
					...
					if (this.releasedNotSilent && this.claimed.get_Count() == 0)
					{
						this.Recycle();
					}
					return;
		...
		protected virtual void Trace(PathNode from)
		{
			...
			if (this.path.get_Capacity() < num)
			{
				this.path.set_Capacity(num);
			}
			if (this.vectorPath.get_Capacity() < num)
			{
				this.vectorPath.set_Capacity(num);
			}
			pathNode = from;
			for (int i = 0; i < num; i++)
			{
			...
			{
				GraphNode graphNode = this.path.get_Item(j);
				this.path.set_Item(j, this.path.get_Item(num - j - 1));
				this.path.set_Item(num - j - 1, graphNode);
			}
			for (int k = 0; k < num; k++)
			{
				this.vectorPath.Add(this.path.get_Item(k).position);
			}
		}
		...
		public virtual string DebugString(PathLog logMode)
		{
			...
			debugStringBuilder.set_Length(0);
			debugStringBuilder.Append(this.error ? "Path Failed : " : "Path Completed : ");
			debugStringBuilder.Append("Computation Time ");
			debugStringBuilder.Append(this.duration.ToString((logMode == PathLog.Heavy) ? "0.000 ms " : "0.00 ms "));
			...
			if (!this.error)
			{
				...
				debugStringBuilder.Append((this.path == null) ? "Null" : this.path.get_Count().ToString());
			...
			if (logMode == PathLog.Heavy && !AstarPath.IsUsingMultithreading)
			{
				...
				{
					debugStringBuilder.Append(this.callback.get_Target().GetType().get_FullName()).AppendLine();
				}
				else
				{
