	...
	public class ABPath : Path
	{
		...
		public override uint GetConnectionSpecialCost(GraphNode a, GraphNode b, uint currentCost)
		{
			if (this.startNode != null && this.endNode != null)
			{
				if (a == this.startNode)
				{
					return (uint)((double)(this.startPoint - ((b == this.endNode) ? this.hTarget : b.position)).costMagnitude * (currentCost * 1.0 / (double)(a.position - b.position).costMagnitude));
				}
				if (b == this.startNode)
				{
					return (uint)((double)(this.startPoint - ((a == this.endNode) ? this.hTarget : a.position)).costMagnitude * (currentCost * 1.0 / (double)(a.position - b.position).costMagnitude));
				}
				if (a == this.endNode)
				{
		...
		public override void Initialize()
		{
			...
			pathNode2.cost = 0u;
			pathNode2.G = ((this.startNode != null) ? base.GetTraversalCost(this.startNode) : 0u);
			pathNode2.H = ((this.startNode != null) ? base.CalculateHScore(this.startNode) : 0u);
		...
		public override string DebugString(PathLog logMode)
		{
			...
			stringBuilder.Append(base.error ? "Path Failed : " : "Path Completed : ");
			stringBuilder.Append("Computation Time ");
			stringBuilder.Append(this.duration.ToString((logMode == PathLog.Heavy) ? "0.000" : "0.00"));
			stringBuilder.Append(" ms Searched Nodes ");
			stringBuilder.Append(this.searchedNodes);
			if (!base.error)
			{
				stringBuilder.Append(" Path Length ");
				stringBuilder.Append((this.path == null) ? "Null" : this.path.get_Count().ToString());
			...
			if (logMode == PathLog.Heavy && !AstarPath.IsUsingMultithreading)
			{
				...
				{
					stringBuilder.Append(this.callback.get_Target().GetType().get_FullName()).AppendLine();
				}
				else
				{
