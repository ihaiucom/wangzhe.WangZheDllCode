...
namespace Pathfinding
{
	...
	public class RecastMeshObj : MonoBehaviour
	{
		...
		public static void GetAllInBounds(List<RecastMeshObj> buffer, Bounds bounds)
		{
			...
			for (int k = 0; k < RecastMeshObj.dynamicMeshObjs.get_Count(); k++)
			{
				if (RecastMeshObj.dynamicMeshObjs.get_Item(k).GetBounds().Intersects(bounds))
				{
					buffer.Add(RecastMeshObj.dynamicMeshObjs.get_Item(k));
				}
			}
