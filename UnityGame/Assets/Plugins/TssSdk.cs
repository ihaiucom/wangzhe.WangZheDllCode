...
public static class TssSdk
{
	...
	private class OutputUnityBuffer
	{
		...
		public void write(byte b)
		{
			...
			{
				this.data[(int)((uint)((UIntPtr)this.offset))] = b;
				this.offset += 1u;
			}
		}
	...
	private class SerializeUnity
	{
		...
		{
			byte[] bytes = Encoding.get_ASCII().GetBytes(uin);
			byte[] bytes2 = Encoding.get_ASCII().GetBytes(appId);
			byte[] bytes3 = Encoding.get_ASCII().GetBytes(roleId);
