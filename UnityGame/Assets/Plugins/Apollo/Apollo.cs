...
namespace Apollo
{
	internal class Apollo : IApollo
	{
		private event ApolloLogHandler logEvent
		{
			...
			add
			{
				this.logEvent = (ApolloLogHandler)Delegate.Combine(this.logEvent, value);
			}
			[MethodImpl(32)]
			remove
			{
				this.logEvent = (ApolloLogHandler)Delegate.Remove(this.logEvent, value);
			}
		}
		...
		[DllImport("apollo", CallingConvention = CallingConvention.Cdecl)]
		private static extern int apollo_init(int nServiceId, int nMaxMessageBuffSize, [MarshalAs(20)] string pluginName);

		[DllImport("apollo", CallingConvention = CallingConvention.Cdecl)]
		private static extern bool apollo_switchplugin([MarshalAs(20)] string pluginName);
