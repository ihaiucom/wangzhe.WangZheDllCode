...
namespace Apollo
{
	internal sealed class ApolloObjectManager : MonoBehaviour
	{
			...
			get
			{
				if (!ApolloObjectManager.init)
				{
					...
					if (ApolloObjectManager.container == null)
					{
						...
						Object.DontDestroyOnLoad(ApolloObjectManager.container);
					}
		...
		private static void onSendResult(ulong objectId, [MarshalAs(20)] string function, int result)
		{
			ApolloObject apolloObject = ApolloObjectManager.Instance.dictObjectCollection[objectId];
			if (apolloObject != null && function != null)
			{
				...
				{
					ADebug.LogError("onSendResult not exist method:" + function + " " + type.get_FullName());
				}
			}
			else
			{
		...
		private static void onSendBuffer(ulong objectId, [MarshalAs(20)] string function, IntPtr buffer, int len)
		{
			ApolloObject apolloObject = ApolloObjectManager.Instance.dictObjectCollection[objectId];
			if (apolloObject != null && function != null)
			{
				...
				{
					ADebug.LogError("onSendBuffer not exist method:" + function + " " + type.get_FullName());
				}
			}
			else
			{
		...
		private static extern void addApolloObject(ulong objectId, [MarshalAs(20)] string objName);

		[DllImport("apollo", CallingConvention = CallingConvention.Cdecl)]
		private static extern void removeApolloObject(ulong objectId);

		[DllImport("apollo", CallingConvention = CallingConvention.Cdecl)]
		private static extern void setApolloSendMessageCallback([MarshalAs(38)] ApolloSendMessageDelegate callback);

		[DllImport("apollo", CallingConvention = CallingConvention.Cdecl)]
		private static extern void setApolloSendStructCallback([MarshalAs(38)] ApolloSendStructDelegate callback);

		[DllImport("apollo", CallingConvention = CallingConvention.Cdecl)]
		private static extern void setApolloSendResultCallback([MarshalAs(38)] ApolloSendResultDelegate callback);

		[DllImport("apollo", CallingConvention = CallingConvention.Cdecl)]
		private static extern void setApolloSendBufferCallback([MarshalAs(38)] ApolloSendBufferDelegate callback);

		[DllImport("apollo", CallingConvention = CallingConvention.Cdecl)]
		private static extern void setApolloSendResultBufferCallback([MarshalAs(38)] ApolloSendResultBufferDelegate callback);
