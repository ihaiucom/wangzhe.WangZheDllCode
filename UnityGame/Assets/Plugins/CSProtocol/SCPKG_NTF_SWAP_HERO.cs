...
{
	public class SCPKG_NTF_SWAP_HERO : ProtocolObject
	{
		public int iErrCode;
		...
		{
			if (cutVer == 0u || SCPKG_NTF_SWAP_HERO.CURRVERSION < cutVer)
			{
				cutVer = SCPKG_NTF_SWAP_HERO.CURRVERSION;
			}
			if (SCPKG_NTF_SWAP_HERO.BASEVERSION > cutVer)
			{
			...
			errorType = destBuf.writeUInt32(this.dwActiveObjID);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
		...
		public override TdrError.ErrorType unpack(ref TdrReadBuf srcBuf, uint cutVer)
		{
			...
			errorType = srcBuf.readUInt32(ref this.dwActiveObjID);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
		...
		{
			return SCPKG_NTF_SWAP_HERO.CLASS_ID;
		}
