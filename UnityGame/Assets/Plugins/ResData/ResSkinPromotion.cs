...
namespace ResData
{
	public class ResSkinPromotion : tsf4g_csharp_interface, IUnpackable
	{
		...
		{
			if (cutVer == 0u || ResSkinPromotion.CURRVERSION < cutVer)
			{
				cutVer = ResSkinPromotion.CURRVERSION;
			}
			if (ResSkinPromotion.BASEVERSION > cutVer)
			{
			...
			{
				if ((ulong)num > (ulong)ResSkinPromotion.LENGTH_szOnTime)
				{
			...
			{
				if ((ulong)num3 > (ulong)ResSkinPromotion.LENGTH_szOffTime)
				{
					return TdrError.ErrorType.TDR_ERR_STR_LEN_TOO_BIG;
				}
			...
			errorType = srcBuf.readUInt32(ref this.dwDiscount);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			errorType = srcBuf.readUInt32(ref this.dwSortIndex);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			errorType = srcBuf.readUInt8(ref this.bSortIndexOnly);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
		...
		public TdrError.ErrorType load(ref TdrReadBuf srcBuf, uint cutVer)
		{
			...
			{
				this.szOnTime_ByteArray = new byte[ResSkinPromotion.LENGTH_szOnTime];
			}
			...
			{
				this.szOffTime_ByteArray = new byte[ResSkinPromotion.LENGTH_szOffTime];
			}
