...
namespace ResData
{
	public class ResWealExchange : tsf4g_csharp_interface, IUnpackable
	{
		public uint dwID;
		...
		public ResDT_Item_Info stResItemInfo;

		public byte bColItemCnt;

		public ResDT_Item_Info[] astColItemInfo;
		...
		{
			if (cutVer == 0u || ResWealExchange.CURRVERSION < cutVer)
			{
				cutVer = ResWealExchange.CURRVERSION;
			}
			if (ResWealExchange.BASEVERSION > cutVer)
			{
			...
			errorType = srcBuf.readUInt8(ref this.bIsShowHotSpot);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			...
			errorType = this.stResItemInfo.unpack(ref srcBuf, cutVer);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			...
			for (int i = 0; i < 2; i++)
			{
				errorType = this.astColItemInfo[i].unpack(ref srcBuf, cutVer);
				if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
				{
					return errorType;
				}
