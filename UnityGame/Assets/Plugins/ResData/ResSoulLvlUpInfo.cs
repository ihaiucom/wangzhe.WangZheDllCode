...
namespace ResData
{
	public class ResSoulLvlUpInfo : tsf4g_csharp_interface, IUnpackable
	{
		...
		public uint dwKilledExp;

		public uint dwExtraKillExp;

		public byte[] szImagePath_ByteArray;

		public ushort wKillGoldCoinInBattle;

		public ushort wExtraKillGoldCoinInBattle;
		...
		{
			if (cutVer == 0u || ResSoulLvlUpInfo.CURRVERSION < cutVer)
			{
				cutVer = ResSoulLvlUpInfo.CURRVERSION;
			}
			if (ResSoulLvlUpInfo.BASEVERSION > cutVer)
			{
				return TdrError.ErrorType.TDR_ERR_CUTVER_TOO_SMALL;
			}
			TdrError.ErrorType errorType = srcBuf.readUInt32(ref this.dwSoulAllocId);
			...
			errorType = srcBuf.readUInt32(ref this.dwKilledExp);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			errorType = srcBuf.readUInt32(ref this.dwExtraKillExp);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			...
			{
				if ((ulong)num > (ulong)ResSoulLvlUpInfo.LENGTH_szImagePath)
				{
			...
			errorType = srcBuf.readUInt16(ref this.wKillGoldCoinInBattle);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			errorType = srcBuf.readUInt16(ref this.wExtraKillGoldCoinInBattle);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
		...
		public TdrError.ErrorType load(ref TdrReadBuf srcBuf, uint cutVer)
		{
			...
			{
				this.szImagePath_ByteArray = new byte[ResSoulLvlUpInfo.LENGTH_szImagePath];
			}
