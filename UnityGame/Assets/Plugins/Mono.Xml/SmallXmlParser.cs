...
namespace Mono.Xml
{
	public class SmallXmlParser
	{
		...
		private class AttrListImpl : SmallXmlParser.IAttrList
		{
			...
			{
				return (string)this.attrNames.get_Item(i);
			}

			public string GetValue(int i)
			{
				return (string)this.attrValues.get_Item(i);
			}

			public string GetValue(string name)
			{
				for (int i = 0; i < this.attrNames.get_Count(); i++)
				{
					if ((string)this.attrNames.get_Item(i) == name)
					{
						return (string)this.attrValues.get_Item(i);
					}
				}
		...
		{
			string[] array = new string[this.elementNames.get_Count()];
		...
		public void Parse(TextReader input, SmallXmlParser.IContentHandler handler)
		{
			...
			if (this.elementNames.get_Count() > 0)
			{
		...
		public void ReadContent()
		{
			...
			if (num != 33)
			{
				...
				else
				{
					...
					this.SkipWhitespaces();
					string text4 = (string)this.elementNames.Pop();
					this.xmlSpaces.Pop();
					if (this.xmlSpaces.get_Count() > 0)
					{
					...
					if (text3 != text4)
					{
						throw this.Error(string.Format("End tag mismatch: expected {0} but found {1}", text4, text3));
					}
					this.handler.OnEndElement(text3);
					this.Expect(62);
					return;
				}
			}
