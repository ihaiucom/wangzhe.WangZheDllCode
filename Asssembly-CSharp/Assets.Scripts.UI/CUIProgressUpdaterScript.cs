...
namespace Assets.Scripts.UI
{
	public class CUIProgressUpdaterScript : CUIComponent
	{
		...
		protected virtual void Update()
		{
			...
			{
				float fillAmount = this.m_image.get_fillAmount() + this.m_fillAmountPerSecond * this.m_fillRate * Time.deltaTime;
				this.m_image.set_fillAmount(fillAmount);
			...
			{
				float fillAmount2 = this.m_image.get_fillAmount() - this.m_fillAmountPerSecond * this.m_fillRate * Time.deltaTime;
				this.m_image.set_fillAmount(fillAmount2);
				if (this.m_image.get_fillAmount() >= this.m_protectFillAmout)
				{
					this.m_image.set_color(CUIUtility.s_Color_BraveScore_BaojiKedu_On);
				}
		...
		public void ResetFillAmount()
		{
			if (this.m_image != null)
			{
				this.m_image.set_fillAmount(this.m_startFillAmount);
			}
		}
	}
}
