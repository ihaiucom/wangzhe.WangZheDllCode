...
namespace Assets.Scripts.UI
{
	public class CUISliderEventScript : CUIComponent
	{
		...
		public float value
		{
			...
			{
				this.m_value = value;
				if (this.m_slider && this.m_value <= this.m_slider.get_maxValue() && this.m_value >= 0f)
				{
		...
		public bool Enabled
		{
			get
			{
				return this.m_slider && this.m_slider.get_interactable();
			}
			set
			{
				if (this.m_slider)
		...
		public override void Initialize(CUIFormScript formScript)
		{
			...
			this.m_slider.get_onValueChanged().AddListener(new UnityAction<float>(this.OnSliderValueChanged));
