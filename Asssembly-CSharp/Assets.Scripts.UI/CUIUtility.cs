...
namespace Assets.Scripts.UI
{
	public class CUIUtility : Singleton<CUIUtility>
	{
		...
		{
			return (camera == null) ? new Vector3(screenPoint.x, screenPoint.y, z) : camera.ViewportToWorldPoint(new Vector3(screenPoint.x / (float)Screen.width, screenPoint.y / (float)Screen.height, z));
		}

		public static Vector2 WorldToScreenPoint(Camera camera, Vector3 worldPoint)
		{
			return (camera == null) ? new Vector2(worldPoint.x, worldPoint.y) : camera.WorldToScreenPoint(worldPoint);
		}
		...
		private static void SetImageGrey(Graphic graphic, bool isSetGrey, Color defaultColor)
		{
			if (graphic != null)
			{
				graphic.set_color(isSetGrey ? CUIUtility.s_Color_Grey : defaultColor);
			}
		}
		...
		public static void SetToggleEnable(Toggle tgl, bool enable, Color textOrgColor)
		{
			...
			{
				componentInChildren.set_color(enable ? textOrgColor : CUIUtility.s_Text_Color_Disable);
			}
			...
			if (componetInChild != null)
			{
				componetInChild.set_color(enable ? CUIUtility.s_Color_White : CUIUtility.s_Color_GrayShader);
			}
		}
