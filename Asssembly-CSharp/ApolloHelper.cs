...
public class ApolloHelper : Singleton<ApolloHelper>
{
	...
	public int GetRegisterChannelId()
	{
		...
		int result = 0;
		bool flag = int.TryParse(registerChannelId, ref result);
		if (flag)
		{
			return result;
		}
	...
	{
		return (Singleton<ApolloHelper>.GetInstance().CurPlatform == ApolloPlatform.Wechat) ? ApolloConfig.WXAppID : ApolloConfig.QQAppID;
	}

	public string GetAppKey()
	{
		return (Singleton<ApolloHelper>.GetInstance().CurPlatform == ApolloPlatform.Wechat) ? ApolloConfig.WXAppKey : ApolloConfig.QQAppKey;
	}
