...
public abstract class TemplateMarkerBase : MonoBehaviour
{
	...
	public class NamePattern
	{
		...
		public string IgnoreCaseStr
		{
			get
			{
				return this.ignoreCase ? "不区分大小写" : "区分大小写";
			}
		}
	}
	...
	{
		string text = this.wildcard2Regex(wildCard);
		if (ignoreCase)
		{
			return Regex.IsMatch(targetString, text, 1);
		}
		return Regex.IsMatch(targetString, text);
	}
}
