...
public class CChatParser
{
			...
			for (int i = 0; i < count; i++)
			{
				...
				if (!string.IsNullOrEmpty(value2))
				{
					...
					if (num > -1)
					{
						...
						if (!string.IsNullOrEmpty(value2) && value2.IndexOf("[") != -1 && match.get_Groups().get_Count() == 6)
						{
							int num3 = Convert.ToInt32(match.get_Groups().get_Item(3).ToString());
							int num4 = Convert.ToInt32(match.get_Groups().get_Item(4).ToString());
							string text4 = Convert.ToString(match.get_Groups().get_Item(5));
							string text5 = Utility.CreateMD5Hash(string.Format("{0}{1}{2}", num3, num4, "j9cWiS6lPjw4g"), Utility.MD5_STRING_CASE.LOWER);
							if (text4 == text5)
							{
	...
	private void CreateTextFace(string value)
	{
		...
		this.curEntNode.TextObjList.Add(new CTextImageNode(value.Substring(1), CChatParser.InfoType.Face, (float)CChatParser.chatFaceWidth, 0f, (float)this.mPositionX, -(float)CChatParser.chatFaceHeight));
