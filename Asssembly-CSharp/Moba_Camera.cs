...
public class Moba_Camera : MonoBehaviour
{
	...
	public Plane[] frustum
	{
		get
		{
			return (this.requirements != null && this.requirements.camera != null) ? this.CalcFrustum(this.requirements.camera) : null;
		}
	}
	...
	private void CalculateCameraRotation()
	{
		...
		if (this.inputs.useKeyCodeInputs ? (Input.GetKey(this.inputs.keycodes.RotateCamera) && this.inputs.useKeyCodeInputs) : Input.GetButton(this.inputs.axis.button_rotate_camera))
		{
	...
	private void CalculateCameraMovement()
	{
		...
		if ((this.inputs.useKeyCodeInputs ? Input.GetKeyDown(this.inputs.keycodes.LockCamera) : (Input.GetButtonDown(this.inputs.axis.button_lock_camera) && this.settings.lockTarget)) && this.settings.lockTarget)
		{
		...
		else if (this.settings.lockTarget && (this.settings.cameraLocked || (this.inputs.useKeyCodeInputs ? Input.GetKey(this.inputs.keycodes.characterFocus) : Input.GetButton(this.inputs.axis.button_char_focus))))
		{
			...
			if (magnitude > 0.001f)
			{
				...
				{
					x = (bComposePlayerMovement ? this.requirements.pivot.position.x : this.StartLocation.x) + this.RelativeDisplacement.x / 1000f;
					z = (bComposePlayerMovement ? this.requirements.pivot.position.z : this.StartLocation.z) + this.RelativeDisplacement.y / 1000f;
				}
				else
				{
			...
			else if (this.RelativeDisplacement.x != 0f || this.RelativeDisplacement.y != 0f)
			{
				...
				if (this.bEnableDisplacement)
				{
					x2 = (bComposePlayerMovement ? this.requirements.pivot.position.x : this.StartLocation.x) + this.RelativeDisplacement.x / 1000f;
					z2 = (bComposePlayerMovement ? this.requirements.pivot.position.z : this.StartLocation.z) + this.RelativeDisplacement.y / 1000f;
				}
				else
				{
		...
		else
		{
			...
			if ((Input.mousePosition.x < this.settings.movement.edgeHoverOffset && this.settings.movement.edgeHoverMovement) || (this.inputs.useKeyCodeInputs ? Input.GetKey(this.inputs.keycodes.CameraMoveLeft) : Input.GetButton(this.inputs.axis.button_camera_move_left)))
			{
				a -= this.requirements.pivot.transform.right;
			}
			if ((Input.mousePosition.x > (float)Screen.width - this.settings.movement.edgeHoverOffset && this.settings.movement.edgeHoverMovement) || (this.inputs.useKeyCodeInputs ? Input.GetKey(this.inputs.keycodes.CameraMoveRight) : Input.GetButton(this.inputs.axis.button_camera_move_right)))
			{
				a += this.requirements.pivot.transform.right;
			}
			if ((Input.mousePosition.y < this.settings.movement.edgeHoverOffset && this.settings.movement.edgeHoverMovement) || (this.inputs.useKeyCodeInputs ? Input.GetKey(this.inputs.keycodes.CameraMoveBackward) : Input.GetButton(this.inputs.axis.button_camera_move_backward)))
			{
				a -= this.requirements.pivot.transform.forward;
			}
			if ((Input.mousePosition.y > (float)Screen.height - this.settings.movement.edgeHoverOffset && this.settings.movement.edgeHoverMovement) || (this.inputs.useKeyCodeInputs ? Input.GetKey(this.inputs.keycodes.CameraMoveForward) : Input.GetButton(this.inputs.axis.button_camera_move_forward)))
			{
				a += this.requirements.pivot.transform.forward;
			}
