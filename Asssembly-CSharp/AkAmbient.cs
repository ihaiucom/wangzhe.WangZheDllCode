...
public class AkAmbient : AkEvent
{
	...
	{
		AkPositionArray akPositionArray = new AkPositionArray((uint)this.multiPositionArray.get_Count());
		for (int i = 0; i < this.multiPositionArray.get_Count(); i++)
		{
			akPositionArray.Add(base.transform.position + this.multiPositionArray.get_Item(i), base.transform.forward);
		}
		return akPositionArray;
	}
}
