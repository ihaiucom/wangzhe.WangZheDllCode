...
public class CHeroSelectChatView
{
	...
	private void setBubbleText(int index, string text)
	{
		if (index == 0)
		{
			...
			{
				this.bubbleText0.set_text(text);
			}
		}
		else if (index == 1)
		{
			...
			{
				this.bubbleText1.set_text(text);
			}
		}
		else if (index == 2)
		{
			...
			{
				this.bubbleText2.set_text(text);
			}
		}
		else if (index == 3)
		{
			...
			{
				this.bubbleText3.set_text(text);
			}
		}
		else if (index == 4 && this.bubbleText4 != null)
		{
			this.bubbleText4.set_text(text);
		}
	}
	...
	private void _refresh_list(CUIListScript listScript, ListView<CChatEntity> data_list)
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			if (elemenet != null && listScript.IsElementInScrollArea(i))
			{
				elemenet.transform.Find("Text").GetComponent<Text>().set_text(string.Format("{0}：{1}", data_list[i].name, data_list[i].text));
			}
		}
	}
	...
	public void On_List_ElementEnable(CUIEvent uievent)
	{
		...
		{
			uievent.m_srcWidget.transform.Find("Text").GetComponent<Text>().set_text(text);
		}
	}
