			...
			for (int i = 0; i < 3; i++)
			{
				if (i != (int)InActor.TheActorMeta.ActorCamp)
				{
					...
					for (int j = 0; j < count; j++)
					{
						...
						if (((ulong)filter & 1uL << (int)(handle.TheActorMeta.ActorType & (ActorTypeDef)31)) <= 0uL && !handle.ActorControl.IsDeadState && handle.HorizonMarker.IsVisibleFor(InActor.TheActorMeta.ActorCamp) && InActor.CanAttack(handle))
						{
