	...
	internal class StarEvaluation : IStarEvaluation
	{
		...
		public virtual string description
		{
			get
			{
				...
				return this.Description + ((this.Conditions.Count > 0 && this.Conditions[0] != null) ? this.Conditions[0].description : string.Empty);
			}
		}
		...
		public StarEvaluationStatus status
		{
			get
			{
				if (this.logicType == RES_LOGIC_OPERATION_TYPE.RES_LOGIC_OPERATION_AND)
				{
					...
					return flag ? StarEvaluationStatus.Success : StarEvaluationStatus.InProgressing;
				}
