	...
	public class CBattleDeadStat
	{
		...
		private void OnEnterCombat(ref DefaultGameEventParam prm)
		{
			...
			else if (prm.src && prm.src.handle.TheActorMeta.ActorType == ActorTypeDef.Actor_Type_Hero)
			{
				...
				{
					if (heroActors.get_Item(i).handle.ObjID == prm.src.handle.ObjID && i < 10)
					{
		...
		private void OnDeadRecord(ref GameDeadEventParam prm)
		{
			...
			if (src.handle.TheActorMeta.ActorType == ActorTypeDef.Actor_Type_Hero)
			{
				...
				for (int i = 0; i < count; i++)
				{
					if (heroActors.get_Item(i).handle.ObjID == prm.src.handle.ObjID && i < 10)
					{
						deadRecord.fightTime = (int)Singleton<FrameSynchr>.instance.LogicFrameTick - this.m_arrHeroEnterCombatTime[i];
						break;
					}
				}
			...
			else if (src.handle.TheActorMeta.ActorType == ActorTypeDef.Actor_Type_Monster)
			{
				...
				{
					deadRecord2.actorSubType = src.handle.ActorControl.GetActorSubType();
					deadRecord2.actorSubSoliderType = src.handle.ActorControl.GetActorSubSoliderType();
					if (deadRecord2.actorSubType == 2)
					{
						if (deadRecord2.actorSubSoliderType == 7)
						{
							deadRecord2.fightTime = (int)Singleton<FrameSynchr>.instance.LogicFrameTick - this.m_baojunEnterCombatTime;
						}
						else if (deadRecord2.actorSubSoliderType == 8)
						{
							deadRecord2.fightTime = (int)Singleton<FrameSynchr>.instance.LogicFrameTick - this.m_baronEnterCombatTime;
						}
						else if (deadRecord2.actorSubSoliderType == 9)
						{
							deadRecord2.fightTime = (int)Singleton<FrameSynchr>.instance.LogicFrameTick - this.m_bigDragonEnterCombatTime;
						}
					}
				}
			...
			else if (src.handle.TheActorMeta.ActorType == ActorTypeDef.Actor_Type_Organ && (src.handle.TheStaticData.TheOrganOnlyInfo.OrganType == 1 || src.handle.TheStaticData.TheOrganOnlyInfo.OrganType == 4 || src.handle.TheStaticData.TheOrganOnlyInfo.OrganType == 2))
			{
				...
				{
					deadRecord3.iOrder = src.handle.ObjLinker.BattleOrder;
					deadRecord3.actorSubType = (byte)src.handle.TheStaticData.TheOrganOnlyInfo.OrganType;
				}
