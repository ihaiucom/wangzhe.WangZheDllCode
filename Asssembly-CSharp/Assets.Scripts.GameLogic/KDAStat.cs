...
namespace Assets.Scripts.GameLogic
{
	public class KDAStat
	{
		...
		protected void recordDead(PoolObjHandle<ActorRoot> src, PoolObjHandle<ActorRoot> atker)
		{
			...
			{
				this.m_stJudgeStat.DeadNum = this.m_stJudgeStat.DeadNum + 1;
			}
			...
			if (src && src.handle.ActorControl != null)
			{
				...
				while (enumerator.MoveNext())
				{
					if (atker)
					{
						...
						if (current.get_Key() == atker.handle.ObjID)
						{
							continue;
						}
					}
					if (killer)
					{
						...
						if (current2.get_Key() == killer.handle.ObjID)
						{
							continue;
						}
					}
					KeyValuePair<uint, ulong> current3 = enumerator.get_Current();
					if (current3.get_Key() == self.handle.ObjID)
					{
						this.m_numAssist++;
						if (src && !src.handle.ActorAgent.IsAutoAI())
						{
							this.m_stJudgeStat.AssitNum = this.m_stJudgeStat.AssitNum + 1;
						}
						return;
					}
				}
			...
			if (killer && killer.handle.ActorControl != null)
			{
				...
				while (enumerator2.MoveNext())
				{
					if (atker)
					{
						...
						if (current4.get_Key() == atker.handle.ObjID)
						{
							continue;
						}
					}
					KeyValuePair<uint, ulong> current5 = enumerator2.get_Current();
					if (current5.get_Key() != killer.handle.ObjID)
					{
						KeyValuePair<uint, ulong> current6 = enumerator2.get_Current();
						if (current6.get_Key() == self.handle.ObjID)
						{
							this.m_numAssist++;
							if (src && !src.handle.ActorAgent.IsAutoAI())
							{
