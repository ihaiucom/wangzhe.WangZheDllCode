...
namespace Assets.Scripts.GameLogic
{
	public class DialogueProcessor : MonoSingleton<DialogueProcessor>
	{
		...
		private PoolObjHandle<Action> m_curAction = default(PoolObjHandle<Action>);
		...
		{
			DialogueProcessor.<NextPageNoTimeScale>c__IteratorF <NextPageNoTimeScale>c__IteratorF = new DialogueProcessor.<NextPageNoTimeScale>c__IteratorF();
			<NextPageNoTimeScale>c__IteratorF.<>f__this = this;
			return <NextPageNoTimeScale>c__IteratorF;
		}

		[DebuggerHidden]
		private IEnumerator StartFadingOut()
		{
			DialogueProcessor.<StartFadingOut>c__Iterator10 <StartFadingOut>c__Iterator = new DialogueProcessor.<StartFadingOut>c__Iterator10();
			<StartFadingOut>c__Iterator.<>f__this = this;
			return <StartFadingOut>c__Iterator;
		}

		[DebuggerHidden]
		private IEnumerator StartFadingIn()
		{
			DialogueProcessor.<StartFadingIn>c__Iterator11 <StartFadingIn>c__Iterator = new DialogueProcessor.<StartFadingIn>c__Iterator11();
			<StartFadingIn>c__Iterator.<>f__this = this;
			return <StartFadingIn>c__Iterator;
		}

		[DebuggerHidden]
		public static IEnumerator PlayAnimNoTimeScale(Animation animation, string clipName, bool bLoop, Action onComplete)
		{
			DialogueProcessor.<PlayAnimNoTimeScale>c__Iterator12 <PlayAnimNoTimeScale>c__Iterator = new DialogueProcessor.<PlayAnimNoTimeScale>c__Iterator12();
			<PlayAnimNoTimeScale>c__Iterator.animation = animation;
			<PlayAnimNoTimeScale>c__Iterator.clipName = clipName;
			<PlayAnimNoTimeScale>c__Iterator.onComplete = onComplete;
			<PlayAnimNoTimeScale>c__Iterator.bLoop = bLoop;
			<PlayAnimNoTimeScale>c__Iterator.<$>animation = animation;
			<PlayAnimNoTimeScale>c__Iterator.<$>clipName = clipName;
			<PlayAnimNoTimeScale>c__Iterator.<$>onComplete = onComplete;
			<PlayAnimNoTimeScale>c__Iterator.<$>bLoop = bLoop;
			return <PlayAnimNoTimeScale>c__Iterator;
		}
			...
			while (enumerator.MoveNext())
			{
				if (!(enumerator.Current == null))
				{
					...
					if (component != null)
					{
						...
						component.set_color(new Color(color.r, color.g, color.b, inAlpha));
					}
					if (component2 != null)
					{
						Color color2 = component2.get_color();
						component2.set_color(new Color(color2.r, color2.g, color2.b, inAlpha));
					}
				}
			}
		}
		...
		private void UpdatePortrait3d(bool bActive, DialogueProcessor.SActorLineNode node, DialogueProcessor.SActorLineNode preNode)
		{
			...
			if (bActive)
			{
				if (preNode.AnimName != node.AnimName || preNode.bAnimLoop != node.bAnimLoop || preNode.b3dPortrait != node.b3dPortrait || preNode.ActorType != node.ActorType || preNode.CharCfgId != node.CharCfgId)
				{
					...
					TransformConfig transformConfig = (actorInfo != null) ? actorInfo.GetTransformConfig(ETransformConfigUsage.NPCInStory) : null;
					...
					if (gameObject)
					{
						...
						if (component2)
						{
							...
							Coroutine value = base.StartCoroutine(DialogueProcessor.PlayAnimNoTimeScale(component2, clipName, node.bAnimLoop, new Action(this.OnPortrait3dAnimComplete)));
		...
		private void OnPortrait3dAnimComplete()
		{
			...
			GameObject 3dPortraitName = this.m_3dPortraitName;
			if (3dPortraitName)
			{
				Animation component = 3dPortraitName.GetComponent<Animation>();
				if (component)
				{
					Coroutine value = base.StartCoroutine(DialogueProcessor.PlayAnimNoTimeScale(component, "Idleshow", true, null));
					if (this.Portrait3dAnimCoMap.ContainsKey(3dPortraitName))
					{
						this.Portrait3dAnimCoMap[3dPortraitName] = value;
					}
					else
					{
						this.Portrait3dAnimCoMap.Add(3dPortraitName, value);
					}
				}
			}
		}
		...
		public static Action PlayAgeAction(string inActionName, string inHelperName, GameObject inSrc, GameObject inAtker, ActionStopDelegate inCallback = null, int inHelperIndex = -1)
		{
			...
			if (actionHelper != null && inHelperName != null && inHelperName.get_Length() > 0)
			{
			...
			{
				int num = (actionHelperStorage.targets.Length < 2) ? 2 : actionHelperStorage.targets.Length;
			...
			if (action == null && inActionName != null && inActionName.get_Length() > 0)
			{
		...
		private bool PlayAgeActionInternal(GameObject inSrc, GameObject inAtker, int inGroupId)
		{
			...
			this.m_curAction = new PoolObjHandle<Action>(DialogueProcessor.PlayAgeAction(this.m_ageActName, this.m_ageActName, inSrc, inAtker, new ActionStopDelegate(this.OnActionStoped), -1));
			if (this.m_curAction)
			{
		...
		private void OnActionStoped(ref PoolObjHandle<Action> action)
		{
			if (!action)
			{
				return;
		...
		private void DoNextPageInternal()
		{
			...
			if (this.m_curUiForm != null)
			{
				string[] array = (node.DialogStyle == 1) ? DialogueProcessor.BlackDialogBgPaths : DialogueProcessor.ImageDialogBgPaths;
				string[] array2 = array;
				...
				component.set_text(node.DialogContent);
				...
				component2.set_text(node.DialogTitle);
				if (node.b3dPortrait)
				{
		...
		private bool NextPageInternal()
		{
			...
			default(DialogueProcessor.SActorLineNode).CharCfgId = -1;
			if (this.m_curIndex > 0)
			{
				DialogueProcessor.SActorLineNode sActorLineNode2 = this.m_actorLines[this.m_curIndex - 1];
				if (sActorLineNode2.bFadeOut)
				{
					this.m_fadingCoroutine = base.StartCoroutine(this.StartFadingOut());
					return false;
				}
		...
		private void onActorDead(ref GameDeadEventParam prm)
		{
			...
			{
				GameObject inAtker = prm.orignalAtker ? prm.orignalAtker.handle.gameObject : null;
		...
		{
			if (inDialogStyle != 0 && inDialogStyle == 1)
			{
