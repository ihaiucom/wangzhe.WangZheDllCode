		...
		public override void LateUpdate(int nDelta)
		{
			...
			if (this.deltaTime > 500)
			{
				...
				if (num > this.offsetDistance)
				{
					...
					float num2 = (float)this.actorPtr.handle.ValueComponent.actorMoveSpeed / 1000000f;
					if (num < 3f * this.offsetDistance)
					{
						this.moveSpeed = num2;
					}
					else if (num > 3f * this.offsetDistance && num < 10f * this.offsetDistance)
					{
			...
			else if (this.curState == PetState.Run)
			{
				...
				Vector3 position2 = this.petTrans.position;
				Vector3 b = this.parentTrans.localToWorldMatrix.MultiplyPoint(this.offset);
				float num3 = Vector3.Distance(position2, b);
				if (num3 < this.offsetDistance)
				{
					this.curState = PetState.Idle;
					base.PlayAnimation("Idle", 0.05f);
				}
