...
namespace Assets.Scripts.GameLogic
{
	public class AnimPlayComponent : LogicComponent
	{
		...
		public void Stop(string origAnimName, bool bFlag = false)
		{
			...
			bool flag = false;
			int count = this.anims.get_Count();
			for (int i = count - 1; i >= 0; i--)
			{
				if (this.anims.get_Item(i).animName == changeAnimName)
				{
					flag = (i == count - 1);
					this.anims.RemoveAt(i);
				}
			}
			count = this.anims.get_Count();
			if (flag && (bFlag || (count > 0 && this.anims.get_Item(count - 1).forceOutOfStack)))
			{
				if (count > 0)
				{
					this.DoPlay(this.anims.get_Item(count - 1));
				}
				else
				{
		...
		{
			if (this.anims.get_Count() > 0)
			{
				this.DoPlay(this.anims.get_Item(this.anims.get_Count() - 1));
			}
		}
		...
		public override void UpdateLogic(int delta)
		{
			...
			if (num > 0)
			{
				...
				if (num > 0)
				{
					this.DoPlay(this.anims.get_Item(num - 1));
				}
			}
		}
	}
