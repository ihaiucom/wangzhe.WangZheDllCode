...
namespace behaviac
{
	public static class Workspace
	{
		...
		private static event Workspace.BehaviorNodeLoadedHandler_t OnNodeLoaded
		{
			...
			add
			{
				Workspace.OnNodeLoaded = (Workspace.BehaviorNodeLoadedHandler_t)Delegate.Combine(Workspace.OnNodeLoaded, value);
			}
			[MethodImpl(32)]
			remove
			{
				Workspace.OnNodeLoaded = (Workspace.BehaviorNodeLoadedHandler_t)Delegate.Remove(Workspace.OnNodeLoaded, value);
			}
		}
		...
		public static event Workspace.DRespondToBreakHandler RespondToBreakHandler
		{
			...
			add
			{
				Workspace.RespondToBreakHandler = (Workspace.DRespondToBreakHandler)Delegate.Combine(Workspace.RespondToBreakHandler, value);
			}
			[MethodImpl(32)]
			remove
			{
				Workspace.RespondToBreakHandler = (Workspace.DRespondToBreakHandler)Delegate.Remove(Workspace.RespondToBreakHandler, value);
			}
		}
		...
		{
			return (relativePath.get_Chars(0) != '.' || (relativePath.get_Chars(1) != '/' && relativePath.get_Chars(1) != '\\')) && relativePath.get_Chars(0) != '/' && relativePath.get_Chars(0) != '\\';
		}
		...
		public static void RegisterBehaviorNode(Assembly a)
		{
			...
			{
				Type type = array[i];
				if (type.IsSubclassOf(typeof(BehaviorNode)) && !type.get_IsAbstract())
				{
					Workspace.ms_behaviorNodeTypes[type.get_Name()] = type;
				}
			}
		}
