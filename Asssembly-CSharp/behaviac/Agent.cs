...
namespace behaviac
{
	public class Agent : MonoBehaviour
	{
		...
		public static Dictionary<string, Agent.AgentName_t> Names
		{
			get
			{
		...
		{
			return base.GetType().get_FullName();
		}
		...
		public void SetName(string instanceName)
		{
			...
			{
				string fullName = base.GetType().get_FullName();
				...
				{
					num2 = 0;
					Agent.ms_agent_type_index.set_Item(fullName, 1);
				}
				else
				{
					Dictionary<string, int> dictionary;
					string text2;
					int num3 = (dictionary = Agent.ms_agent_type_index).get_Item(text2 = fullName);
					int num4;
					dictionary.set_Item(text2, (num4 = num3) + 1);
					num2 = num4;
				}
				base.name += string.Format("{0}_{1}_{2}", text, num2, this.m_id);
			}
			else
			{
		...
		public static bool RegisterName<TAGENT>(string agentInstanceName, string displayName, string desc) where TAGENT : Agent
		{
			...
			{
				string fullName = typeof(TAGENT).get_FullName();
				Agent.Names.set_Item(text, new Agent.AgentName_t(text, fullName, displayName, desc));
				return true;
			}
			return false;
		}
		...
		public static bool RegisterStaticClass(Type type, string displayName, string desc)
		{
			...
			{
				Agent.Names.set_Item(fullName, new Agent.AgentName_t(fullName, fullName, displayName, desc));
		...
		public static string GetRegisteredClassName(string agentInstanceName)
		{
			...
			{
				return Agent.Names.get_Item(agentInstanceName).ClassName;
			}
			return null;
		}
