...
namespace Assets.Scripts.Framework
{
	public class GameLogic : Singleton<GameLogic>, IGameModule
	{
		...
		{
			if (Singleton<FrameSynchr>.instance.bActive && Singleton<FrameSynchr>.instance.CurFrameNum % this.HashCheckFreq == 0u && Singleton<BattleLogic>.instance.isFighting && !Singleton<WatchController>.GetInstance().IsWatching)
			{
				...
				for (int i = 0; i < heroActors.get_Count(); i++)
				{
					...
					array[num2++] = (int)(handle.ActorControl.myBehavior | (ObjBehaviMode)((handle.HorizonMarker.IsVisibleFor(COM_PLAYERCAMP.COM_PLAYERCAMP_1) ? 1 : 0) << 5) | (ObjBehaviMode)((handle.HorizonMarker.IsVisibleFor(COM_PLAYERCAMP.COM_PLAYERCAMP_2) ? 1 : 0) << 6));
				}
