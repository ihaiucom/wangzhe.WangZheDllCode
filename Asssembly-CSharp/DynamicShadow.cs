...
public class DynamicShadow : MonoBehaviour
{
	...
	private void InitShadow()
	{
		...
		{
			Shader.EnableKeyword("_SGAME_HEROSHOW_SHADOW_ON");
			RenderTextureFormat format = this.useDepthTex ? RenderTextureFormat.Depth : RenderTextureFormat.ARGB32;
			...
			for (int i = 0; i < this.depthTextures_.Length; i++)
			{
				...
				this.depthTextures_[i].filterMode = (flag ? FilterMode.Bilinear : FilterMode.Point);
			}
