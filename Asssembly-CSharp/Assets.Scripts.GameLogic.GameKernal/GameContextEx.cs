...
namespace Assets.Scripts.GameLogic.GameKernal
{
	public class GameContextEx : Singleton<GameContextEx>
	{
		...
		private void _SetSingleGame(ResLevelCfgInfo levelCfg)
		{
			...
			this._gameContextCommonData.HorizonEnableMethod = (Horizon.EnableMethod)(this._gameContextCommonData.IsMobaGame ? 1 : levelCfg.bEnableHorizon);
		...
		private void _InitMultiGame(COM_GAME_TYPE GameType, int mapType, uint mapID, bool isWarmBattle = false, int warmAiLv = 0)
		{
			...
			this._gameContextMobaData.WarmHeroAiDiffInfo = (isWarmBattle ? GameDataMgr.battleDynamicDifficultyDB.GetDataByKey((long)warmAiLv) : null);
			if (mapType == 4)
			{
