	...
	public class GamePlayerCenter : Singleton<GamePlayerCenter>
	{
		...
		public COM_PLAYERCAMP hostPlayerCamp
		{
			get
			{
				...
				return (hostPlayer != null) ? hostPlayer.PlayerCamp : COM_PLAYERCAMP.COM_PLAYERCAMP_MID;
			}
		}
		...
		public void SetHostPlayer(uint playerId)
		{
			...
			this._players.TryGetValue(playerId, ref this._hostPlayer);
		...
		public List<Player> GetDiffCampPlayers(COM_PLAYERCAMP camp)
		{
			...
			{
				KeyValuePair<uint, Player> current = enumerator.get_Current();
				Player value = current.get_Value();
		...
		{
			Player result = null;
			this._players.TryGetValue(playerId, ref result);
			return result;
		}
		...
		public Player GetPlayerByUid(ulong uid)
		{
			...
			{
				KeyValuePair<uint, Player> current = enumerator.get_Current();
				if (current.get_Value().PlayerUId == uid)
				{
					KeyValuePair<uint, Player> current2 = enumerator.get_Current();
		...
		public int GetPlayerCampPosIndex(uint playerId)
		{
			...
			{
				KeyValuePair<uint, Player> current = enumerator.get_Current();
				if (current.get_Value().PlayerId == playerId)
				{
					break;
				}
				KeyValuePair<uint, Player> current2 = enumerator.get_Current();
				if (current2.get_Value().PlayerCamp == player.PlayerCamp)
				{
					num++;
				}
			}
