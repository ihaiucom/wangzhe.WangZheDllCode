...
public class ApolloVoice_lib : IApolloVoice
{
	...
	private static extern int ApolloVoiceCreateEngine([MarshalAs(42)] string appID);

	[DllImport("apollo_voice")]
	private static extern int ApolloVoiceDestoryEngine();

	[DllImport("apollo_voice")]
	private static extern int ApolloVoiceJoinRoom([MarshalAs(42)] string url1, [MarshalAs(42)] string url2, [MarshalAs(42)] string url3, long roomId, long roomKey, short memberId, [MarshalAs(42)] string openId, int nTimeOut);
	...
	[DllImport("apollo_voice")]
	private static extern int ApolloVoiceGetMemberState([MarshalAs(20)] StringBuilder memberState, int nSize);

	[DllImport("apollo_voice")]
	private static extern int ApolloVoiceSetMemberCount(int nCount);

	[DllImport("apollo_voice")]
	private static extern int ApolloVoiceStartRecord([MarshalAs(42)] string strFullPath);
	...
	[DllImport("apollo_voice")]
	private static extern int ApolloVoiceSendRecFile([MarshalAs(42)] string strFullPath);

	[DllImport("apollo_voice")]
	private static extern int ApolloVoicePlayFile([MarshalAs(42)] string strFullPath);
	...
	public int _QuitRoom(long roomId, short memberId, string OpenId)
	{
		...
		byte[] bytes = Encoding.get_ASCII().GetBytes(OpenId);
