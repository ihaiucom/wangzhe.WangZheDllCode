...
namespace Assets.Scripts.GameSystem
{
	public class ReplayControl
	{
		...
		public void LateUpdate()
		{
			...
			if (this._progress)
			{
				this._progress.set_value((num2 > 0u) ? (num / num2) : 0f);
			}
			if (this._currentTimeTxt)
			{
				this._currentTimeTxt.set_text(string.Format("{0:D2}:{1:D2}", num / 60000u, num / 1000u % 60u));
			}
			if (this._totalTimeTxt)
			{
				this._totalTimeTxt.set_text(string.Format("{0:D2}:{1:D2}", num2 / 60000u, num2 / 1000u % 60u));
			}
			if (this._speedTxt)
			{
				this._speedTxt.set_text(Singleton<WatchController>.GetInstance().SpeedRate.ToString() + "X");
		...
		private void OnClickSpeedUp(CUIEvent evt)
		{
			...
			WatchController expr_07 = instance;
			expr_07.SpeedRate += 1;
			this.ValidateSpeedBtnState();
		}
