	...
	public class CMallRouletteController : Singleton<CMallRouletteController>
	{
		...
		public void RefreshData(uint targetPoolId = 0u)
		{
			...
			while (enumerator.MoveNext())
			{
				...
				ResLuckyDrawPrice value = current.get_Value();
				if (value != null)
				{
					...
					while (enumerator2.MoveNext())
					{
						...
						ResLuckyDrawRewardForClient value2 = current2.get_Value();
						if (value2 != null && value2.dwRewardPoolID == num)
						{
							...
							{
								uint num2 = 0u;
								if (this.m_RewardPoolDic.TryGetValue((uint)value.bMoneyType, ref num2))
								{
									num2 = num;
								}
							}
					...
					while (enumerator3.MoveNext())
					{
						...
						ResLuckyDrawExternReward value3 = current3.get_Value();
		...
		private void InitTab()
		{
			...
			while ((int)b < array.Length)
			{
				if (array[(int)b] != CMallRouletteController.Tab.None)
				{
					...
					{
						if (tab == CMallRouletteController.Tab.Diamond && this.m_RewardDic.ContainsKey(10u))
						{
			...
			DebugHelper.Assert(this.m_UsedTabs.get_Count() != 0, "夺宝单价设定数据档不对");
			if (this.m_UsedTabs.get_Count() == 0)
			{
			...
			{
				componetInChild.SetElementAmount(this.m_UsedTabs.get_Count());
				for (int i = 0; i < componetInChild.m_elementAmount; i++)
				{
					CUIListElementScript elemenet = componetInChild.GetElemenet(i);
					if (elemenet != null)
					{
						CMallRouletteController.Tab tab2 = this.m_UsedTabs.get_Item(i);
						...
						bool flag = false;
						CMallRouletteController.Tab tab3 = tab2;
						if (tab3 != CMallRouletteController.Tab.DianQuan)
						{
							if (tab3 == CMallRouletteController.Tab.Diamond)
							{
		...
		{
			CMallSystem.luckyDrawDic.TryGetValue(payType, ref drawInfo);
			CUIFormScript mallForm = Singleton<CMallSystem>.GetInstance().m_MallForm;
			if (mallForm == null || !Singleton<CMallSystem>.GetInstance().m_IsMallFormOpen)
			{
				return;
				...
				for (byte b = 0; b < 5; b += 1)
				{
					...
					if ((int)b < listView.Count)
					{
						if (gameObject2 != null)
						{
							...
							byte b2 = 0;
							byte b3 = 0;
							byte.TryParse(text.Substring((int)(32 - (b + 1)), 1), ref b2);
							byte.TryParse(text2.Substring((int)(32 - (b + 1)), 1), ref b3);
							...
							{
								componetInChild2.set_text(resDT_LuckyDrawExternReward.dwDrawCnt.ToString());
							}
							if (componetInChild3 != null)
							{
								componetInChild3.set_text("个");
							}
							if (b2 > 0 && b3 == 0)
							{
		...
		private void RefreshLuck(enPayType payType, CSDT_LUCKYDRAW_INFO drawInfo)
		{
			...
			else
			{
				...
				{
					float fillAmount = componetInChild.get_fillAmount();
					componetInChild.set_fillAmount((float)num3);
					componetInChild2.set_fillAmount((float)num3);
					if (this.m_IsLuckyBarInited && Math.Abs(fillAmount - componetInChild.get_fillAmount()) > 1.401298E-45f)
					{
						CUICommonSystem.PlayAnimator(gameObject, "BarLight_Anim");
					}
					componetInChild3.set_text(num2.ToString());
				}
			}
			...
			for (byte b3 = 0; b3 < 14; b3 += 1)
			{
				...
				if (!(gameObject == null))
				{
					...
					if (componetInChild3 != null && cUseable != null)
					{
						...
						{
							obj.CustomSetActive(true);
							componetInChild3.set_text(cUseable.m_stackCount.ToString());
						}
					}
					if (componetInChild4 != null && cUseable != null)
					{
						componetInChild4.set_text(cUseable.m_name);
					}
					if (resLuckyDrawRewardForClient.dwItemTag == 1u)
					{
						...
						if (gameObject4 != null)
						{
							...
							if (transform != null)
							{
								...
								{
									component.set_text(cUseable.m_name);
								}
							}
						}
						b += 1;
					...
					else
					{
						...
						if (componetInChild5 != null)
						{
							...
							if (flag)
							{
								componetInChild5.set_text("拥有");
							}
							else
							{
								componetInChild5.set_text(text);
							}
						}
					}
					if (gameObject5 != null)
		...
		private int GetNextRefreshTime()
		{
			...
			DateTime dateTime2 = new DateTime(1970, 1, 1, 0, 0, 0, 1);
			ResGlobalInfo dataByKey = GameDataMgr.globalInfoDatabin.GetDataByKey(103u);
			int num;
			if (dateTime.get_DayOfWeek() - dataByKey.dwConfValue > 0)
			{
				num = 7 - (dateTime.get_DayOfWeek() - dataByKey.dwConfValue);
			}
			else
			{
				num = dataByKey.dwConfValue - dateTime.get_DayOfWeek();
			}
			DateTime dateTime3 = new DateTime(dateTime.get_Year(), dateTime.get_Month(), dateTime.get_Day(), 0, 0, 0, 1);
			...
			dateTime3 = dateTime3.AddSeconds((double)(num2 * 3600) + (double)(num3 * 60));
			if (dateTime.Subtract(dateTime3).get_TotalSeconds() > 0.0)
			{
				dateTime3 = dateTime3.AddDays(7.0);
			}
			return (int)dateTime3.Subtract(dateTime).get_TotalSeconds();
		}
		...
		public bool IsProbabilityDoubled(RES_SHOPBUY_COINTYPE coinType = RES_SHOPBUY_COINTYPE.RES_SHOPBUY_TYPE_NULL)
		{
			...
			while (enumerator.MoveNext())
			{
				if (coinType != RES_SHOPBUY_COINTYPE.RES_SHOPBUY_TYPE_NULL)
				{
					...
					if (current.get_Key() != (uint)((byte)coinType))
					{
						continue;
					}
				}
				...
				if (current2.get_Value() != null)
				{
					...
					if (current3.get_Value().Count > 0)
					{
						KeyValuePair<uint, ListView<ResLuckyDrawRewardForClient>> current4 = enumerator.Current;
						ListView<ResLuckyDrawRewardForClient> value = current4.get_Value();
					...
					if (cUseable != null)
					{
						...
						case COM_ITEM_TYPE.COM_OBJTYPE_HERO:
						{
							...
							if (heroData.bPlayerOwn)
							{
								...
								cUseable = CUseableManager.CreateUseable(COM_ITEM_TYPE.COM_OBJTYPE_ITEMPROP, dwConfValue, (int)((cHeroInfoData.m_info.shopCfgInfo != null) ? cHeroInfoData.m_info.shopCfgInfo.dwChgItemCnt : 1u));
						...
						case COM_ITEM_TYPE.COM_OBJTYPE_HEROSKIN:
						{
							...
							if (masterRoleInfo != null && masterRoleInfo.IsHaveHeroSkin(heroId, skinId, false))
							{
								...
								cUseable = CUseableManager.CreateUseable(COM_ITEM_TYPE.COM_OBJTYPE_ITEMPROP, dwConfValue2, (int)((resHeroSkinShop != null) ? resHeroSkinShop.dwChgItemCnt : 1u));
		...
		public void OpenAwardTip(RES_SHOPDRAW_SUBTYPE drawType, ListView<CUseable> items, string title = null, bool playSound = false, enUIEventID eventID = enUIEventID.None, bool displayAll = false)
		{
			...
			if (title != null)
			{
				Utility.GetComponetInChild<Text>(cUIFormScript.gameObject, "Panel/bg/Title").set_text(title);
			}
			for (int i = 0; i < num; i++)
			{
		...
		private void OnRouletteTabChange(CUIEvent uiEvent)
		{
			...
			if (selectedIndex > this.m_UsedTabs.get_Count())
			{
				return;
			}
			int lastSelectedIndex = component.GetLastSelectedIndex();
			if (lastSelectedIndex >= 0 && lastSelectedIndex < this.m_UsedTabs.get_Count())
			{
				CMallRouletteController.Tab tab = this.m_UsedTabs.get_Item(lastSelectedIndex);
				RES_SHOPBUY_COINTYPE coinType = RES_SHOPBUY_COINTYPE.RES_SHOPBUY_TYPE_NULL;
				bool flag = false;
				CMallRouletteController.Tab tab2 = tab;
				if (tab2 != CMallRouletteController.Tab.DianQuan)
			...
			this.CurTab = this.m_UsedTabs.get_Item(selectedIndex);
			this.RefreshRewards();
			this.m_IsLuckyBarInited = false;
		...
		private void OnSpinInterval(CUIEvent uiEvent)
		{
			...
			if (this.m_CurState == CMallRouletteController.Roulette_State.ACCELERATE)
			{
				...
				if (num2 > 0.03f)
				{
					if (cUITimerScript != null)
					{
						cUITimerScript.SetOnChangedIntervalTime((num2 <= 0.03f) ? 0.03f : num2);
					}
				}
				else
				{
			...
			if (this.m_CurState == CMallRouletteController.Roulette_State.DECELERATE)
			{
				...
				if (cUITimerScript != null)
				{
					cUITimerScript.SetOnChangedIntervalTime((num3 >= 0.1f) ? 0.1f : num3);
				}
			}
		...
		private void OnOpenExternRewardTip(CUIEvent uiEvent)
		{
			...
			if (curTab != CMallRouletteController.Tab.DianQuan)
			{
				if (curTab == CMallRouletteController.Tab.Diamond)
				{
					CMallSystem.luckyDrawDic.TryGetValue(enPayType.Diamond, ref cSDT_LUCKYDRAW_INFO);
					this.m_ExternRewardDic.TryGetValue(10u, out listView);
				}
			}
			else
			{
				CMallSystem.luckyDrawDic.TryGetValue(enPayType.DianQuan, ref cSDT_LUCKYDRAW_INFO);
				this.m_ExternRewardDic.TryGetValue(2u, out listView);
			}
			...
			if (cSDT_LUCKYDRAW_INFO != null)
			{
				string text = Convert.ToString((long)((ulong)cSDT_LUCKYDRAW_INFO.dwReachMask), 2).PadLeft(32, '0');
				string text2 = Convert.ToString((long)((ulong)cSDT_LUCKYDRAW_INFO.dwDrawMask), 2).PadLeft(32, '0');
				byte.TryParse(text.Substring(32 - (tag + 1), 1), ref b);
				byte.TryParse(text2.Substring(32 - (tag + 1), 1), ref b2);
			}
			...
			if (b == 0)
			{
				...
				if (componetInChild2 != null)
				{
					componetInChild2.set_text("去完成");
				}
				if (componetInChild3 != null && cSDT_LUCKYDRAW_INFO != null)
				{
					componetInChild3.set_text(string.Format("{0}", resDT_LuckyDrawExternReward.dwDrawCnt - cSDT_LUCKYDRAW_INFO.dwCnt));
				}
			}
			else
			{
			...
			for (int j = 0; j < num2; j++)
			{
				...
				gameObject3.CustomSetActive(true);
				gameObject3.transform.FindChild("ItemName").GetComponent<Text>().set_text(listView2[j].m_name);
			}
		...
		{
			CSDT_LUCKYDRAW_INFO cSDT_LUCKYDRAW_INFO = new CSDT_LUCKYDRAW_INFO();
			enPayType enPayType = enPayType.Diamond;
			CMallRouletteController.Tab curTab = this.CurTab;
			if (curTab != CMallRouletteController.Tab.DianQuan)
			{
				if (curTab == CMallRouletteController.Tab.Diamond)
				{
					enPayType = enPayType.Diamond;
				}
			}
			else
			{
			...
			CMallSystem.luckyDrawDic.TryGetValue(enPayType, ref cSDT_LUCKYDRAW_INFO);
			if (cSDT_LUCKYDRAW_INFO != null)
			{
		...
		private void InitCryStalData()
		{
			...
			while (enumerator.MoveNext())
			{
				...
				ResRareExchange value = current.get_Value();
		...
		private void InitCryStalItemID()
		{
			...
			while ((ulong)num < (ulong)((long)dictionaryView.Count))
			{
				...
				{
					this.m_CrystalItemID[(int)((uint)((UIntPtr)num))] = (int)listView2[0].dwRareItemID;
				}
				else
				{
					this.m_CrystalItemID[(int)((uint)((UIntPtr)num))] = 0;
				}
				num += 1u;
			}
		}
		...
		private void InitCryStalTabMenu(uint tabID)
		{
			...
			for (int i = 0; i < this.m_nTotalCrySalTab; i++)
			{
				...
				{
					componetInChild2.set_text(array[i]);
				}
			}
		...
		private void OnCrystalHeroItem(CUIEvent uiEvent, ushort resItemIdx, ResRareExchange resRareExchange)
		{
			...
			if (masterRoleInfo.IsHaveHero(dataByKey.dwCfgID, false))
			{
				...
				component4.set_text(Singleton<CTextManager>.GetInstance().GetText("Mall_Hero_State_Own"));
				gameObject2.CustomSetActive(false);
			...
			else
			{
				...
				componetInChild2.set_text(resRareExchange.dwRareItemCnt.ToString());
				string prefabPath = this.m_tab0ImagePath;
				...
				transform6.gameObject.CustomSetActive(true);
				component4.set_text(Singleton<CTextManager>.GetInstance().GetText("Crystal_Exchange_Btn"));
		...
		private void OnCrystalHeroSkinItem(CUIEvent uiEvent, ushort resItemIdx, ResRareExchange resRareExchange)
		{
			...
			component2.set_text(heroSkin.szSkinName);
			...
			{
				transform5.gameObject.CustomSetActive(true);
				component5.set_text(instance.GetText("Mall_Skin_State_Own"));
				gameObject2.CustomSetActive(false);
			}
			else
			{
				...
				if (!masterRoleInfo.IsHaveHero(heroSkin.dwHeroID, false))
				{
					...
					transform5.gameObject.CustomSetActive(true);
					component5.set_text(Singleton<CTextManager>.GetInstance().GetText("Mall_Skin_State_Buy_hero"));
				...
				component6.enabled = true;
				transform5.gameObject.CustomSetActive(true);
				component5.set_text(Singleton<CTextManager>.GetInstance().GetText("Crystal_Exchange_Btn"));
				component4.enabled = true;
		...
		private void OnCrystalExchange(CUIEvent uiEvent)
		{
			...
			{
				text = array[(int)((uint)((UIntPtr)this.m_nCurCryStalTabID))];
			}
		...
		public static void ReceiveLotteryRes(CSPkg msg)
		{
			...
			uint num = 0u;
			if (Singleton<CMallRouletteController>.GetInstance().m_RewardPoolDic.TryGetValue((uint)msg.stPkgData.stLuckyDrawRsp.bMoneyType, ref num))
			{
