...
namespace Assets.Scripts.GameSystem
{
	...
	public class CPlayerInfoSystem : Singleton<CPlayerInfoSystem>
	{
		...
		private void ImpResIntimacyRelation(SCPKG_GET_INTIMACY_RELATION_RSP rsp)
		{
			...
			while ((long)num < (long)((ulong)dwIntimacyNum))
			{
				...
				if (cSDT_INTIMACY_RELATION_INFO != null)
				{
					...
					{
						componetInChild6.set_text(UT.Bytes2String(cSDT_INTIMACY_RELATION_INFO.szUserName));
					}
					...
					if (componetInChild8 != null)
					{
						...
						if (relaTextCfg != null)
						{
							componetInChild8.set_text(relaTextCfg.IntimRela_Type);
						}
					}
					...
					componetInChild9.set_text(string.Format("{0}/{1}", cSDT_INTIMACY_RELATION_INFO.wIntimacyValue, nxtLevelValue));
					...
					{
						componetInChild10.set_fillAmount((float)cSDT_INTIMACY_RELATION_INFO.wIntimacyValue / (float)nxtLevelValue);
					}
					else
					{
						componetInChild10.set_fillAmount(1f);
					}
					Text componetInChild11 = Utility.GetComponetInChild<Text>(elemenet.gameObject, "middle/descText");
					if (componetInChild11 != null)
					{
						componetInChild11.set_text(UT.Bytes2String(cSDT_INTIMACY_RELATION_INFO.szSignatureInfo));
					}
				}
				num++;
			}
		...
		private void ProcessQQVIP(CUIFormScript form, bool bShow)
		{
			...
			component.set_text(Singleton<CTextManager>.GetInstance().GetText("QQ_Vip_Buy_Vip"));
			CRoleInfo masterRoleInfo = Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo();
			if (masterRoleInfo != null)
			{
		...
		private void OnPlayerInfoMostUsedHeroItemEnable(CUIEvent uiEvent)
		{
			...
			{
				componetInChild.set_text(string.Format(Singleton<CTextManager>.GetInstance().GetText("Player_Info_Games_Cnt_Label"), cOMDT_MOST_USED_HERO_INFO.dwGameWinNum + cOMDT_MOST_USED_HERO_INFO.dwGameLoseNum));
			}
		}
		...
		private void OnLicenseListItemEnable(CUIEvent uiEvent)
		{
			...
			if (masterRoleInfo != null && masterRoleInfo.m_licenseInfo != null)
			{
				...
				if (srcWidget != null && licenseItemByIndex != null && licenseItemByIndex.m_resLicenseInfo != null)
				{
					...
					transform2.GetComponent<Text>().set_text(licenseItemByIndex.m_resLicenseInfo.szLicenseName);
					...
					if (licenseItemByIndex.m_getSecond > 0u)
					{
						...
						transform3.GetComponent<Text>().set_text(string.Format("<color=#00d519>{0}/{1}/{2}</color>", dateTime.get_Year(), dateTime.get_Month(), dateTime.get_Day()));
					}
					else
					{
					...
					transform4.GetComponent<Text>().set_text(licenseItemByIndex.m_resLicenseInfo.szDesc);
				}
			}
		}
		...
		private void InitTab()
		{
			...
			if (CSysDynamicBlock.bLobbyEntryBlocked)
			{
				...
				while ((int)b2 < array.Length)
				{
					...
					case CPlayerInfoSystem.Tab.Mentor_Info:
					{
						CPlayerInfoSystem.Tab[] array3 = array2;
						byte b3 = b;
						b = b3 + 1;
						array3[(int)b3] = array[(int)b2];
						break;
					}
					}
					b2 += 1;
			...
			while ((int)b5 < array.Length)
			{
				...
				{
				case CPlayerInfoSystem.Tab.Base_Info:
					array4[(int)b5] = Singleton<CTextManager>.GetInstance().GetText("Player_Info_Tab_Base_Info");
					break;
				case CPlayerInfoSystem.Tab.Pvp_Info:
					array4[(int)b5] = Singleton<CTextManager>.GetInstance().GetText("Player_Info_Tab_Pvp_Info");
					break;
				case CPlayerInfoSystem.Tab.Honor_Info:
					array4[(int)b5] = Singleton<CTextManager>.GetInstance().GetText("Player_Info_Tab_Honor_Info");
					break;
				case CPlayerInfoSystem.Tab.Common_Hero:
					array4[(int)b5] = Singleton<CTextManager>.GetInstance().GetText("Player_Info_Tab_Common_Hero_Info");
					break;
				case CPlayerInfoSystem.Tab.PvpHistory_Info:
					array4[(int)b5] = Singleton<CTextManager>.GetInstance().GetText("Player_Info_Tab_PvpHistory_Info");
					break;
				case CPlayerInfoSystem.Tab.PvpCreditScore_Info:
					array4[(int)b5] = Singleton<CTextManager>.GetInstance().GetText("Player_Info_Tab_Credit_Info");
					break;
				case CPlayerInfoSystem.Tab.Mentor_Info:
					array4[(int)b5] = Singleton<CTextManager>.GetInstance().GetText("Player_Info_Tab_Mentor_Info");
					break;
				case CPlayerInfoSystem.Tab.Social_Info:
					if (!CSysDynamicBlock.bLobbyEntryBlocked)
					{
						array4[(int)b5] = Singleton<CTextManager>.GetInstance().GetText("Player_Info_Tab_Social_Info");
					}
					break;
				case CPlayerInfoSystem.Tab.Relation_Info:
					if (!CSysDynamicBlock.bLobbyEntryBlocked)
					{
						array4[(int)b5] = Singleton<CTextManager>.GetInstance().GetText("Player_Info_Tab_Relation_Info");
					}
					break;
				}
				b5 += 1;
			...
			if (component != null)
			{
				component.SetElementAmount(array4.Length);
		...
		private void SetBaseInfoScrollable(bool scrollable = false)
		{
			...
			if (gameObject != null)
			{
				...
				if (component2 != null)
				{
					component2.set_verticalNormalizedPosition(1f);
				}
			}
		}
		...
		{
			CUICommonSystem.SetButtonName(btnAppointOrCancelMatchLeader, CGuildHelper.IsGuildMatchLeaderPosition(this.m_PlayerProfile.m_uuid) ? Singleton<CTextManager>.GetInstance().GetText("GuildMatch_Cancel_Leader") : Singleton<CTextManager>.GetInstance().GetText("GuildMatch_Apooint_Leader"));
		}
		...
		private void UpdateHeadInfo()
		{
			...
			{
				componetInChild2.set_text(this.m_PlayerProfile.Name());
			}
			...
			if (gameObject3 != null && gameObject4 != null)
			{
				...
				if (!CGuildSystem.IsInNormalGuild(this.m_PlayerProfile.GuildState) || string.IsNullOrEmpty(this.m_PlayerProfile.GuildName))
				{
					if (component3 != null)
					{
						component3.set_text(Singleton<CTextManager>.GetInstance().GetText("PlayerInfo_Guild"));
					}
				...
				else
				{
					if (component3 != null)
					{
						component3.set_text(this.m_PlayerProfile.GuildName);
					}
					gameObject4.CustomSetActive(true);
					if (componetInChild3 != null)
					{
						componetInChild3.set_text(CGuildHelper.GetPositionName(this.m_PlayerProfile.GuildState));
					}
				}
			}
		}
		...
		private void UpdateBaseInfo()
		{
			...
			COM_PRIVILEGE_TYPE privilegeType = (cOM_PRIVILEGE_TYPE == COM_PRIVILEGE_TYPE.COM_PRIVILEGE_TYPE_WXGAMECENTER_LOGIN) ? COM_PRIVILEGE_TYPE.COM_PRIVILEGE_TYPE_NONE : COM_PRIVILEGE_TYPE.COM_PRIVILEGE_TYPE_WXGAMECENTER_LOGIN;
			...
			COM_PRIVILEGE_TYPE privilegeType2 = (cOM_PRIVILEGE_TYPE == COM_PRIVILEGE_TYPE.COM_PRIVILEGE_TYPE_QQGAMECENTER_LOGIN) ? COM_PRIVILEGE_TYPE.COM_PRIVILEGE_TYPE_NONE : COM_PRIVILEGE_TYPE.COM_PRIVILEGE_TYPE_QQGAMECENTER_LOGIN;
			...
			if (componetInChild != null)
			{
				componetInChild.set_text((this.m_PlayerProfile.Pvp1V1TotalGameCnt() + this.m_PlayerProfile.Pvp3V3TotalGameCnt() + this.m_PlayerProfile.Pvp5V5TotalGameCnt() + this.m_PlayerProfile.EntertainmentTotalGameCnt() + this.m_PlayerProfile.PvpGuildTotalGameCnt() + this.m_PlayerProfile.RankTotalGameCnt()).ToString());
			}
			Text componetInChild2 = Utility.GetComponetInChild<Text>(widget, "pnlContainer/pnlInfo/labelKeJingDuiShouCnt/numCnt");
			if (componetInChild2 != null)
			{
				componetInChild2.set_text((this.m_PlayerProfile._geiLiDuiYou + this.m_PlayerProfile._keJingDuiShou).ToString());
			}
			Text componetInChild3 = Utility.GetComponetInChild<Text>(widget, "pnlContainer/pnlInfo/labelHeroCnt/numCnt");
			if (componetInChild3 != null)
			{
				componetInChild3.set_text(this.m_PlayerProfile.HeroCnt().ToString());
			}
			...
			{
				componetInChild4.set_text(this.m_PlayerProfile.SkinCnt().ToString());
			}
			bool flag = CPlayerInfoSystem.isSelf(this.m_PlayerProfile.m_uuid);
			...
			if (component2)
			{
				...
				{
					component2.set_text(this.m_PlayerProfile.m_personSign);
				}
				if (flag)
				{
					component2.set_interactable(true);
					component2.get_onEndEdit().RemoveAllListeners();
					component2.get_onEndEdit().AddListener(new UnityAction<string>(this.OnPersonSignEndEdit));
				}
				else
				{
			...
			if (componetInChild6 != null)
			{
				componetInChild6.set_text(this.m_PlayerProfile._trophyRewardInfoLevel.ToString());
			}
			if (componetInChild7 != null)
			{
				...
				else
				{
					...
					obj3.CustomSetActive(true);
					componetInChild7.set_text(this.m_PlayerProfile._trophyRank.ToString());
				}
			}
			this.ProcessSelectedTrophies();
		}
		...
		private void ProcessSelectedTrophies()
		{
			...
			if (flag2)
			{
				...
				if (flag)
				{
					...
					component.set_text(Singleton<CTextManager>.GetInstance().GetText("Achievement_Player_Info_Selected_Trophies_Self_No_Data"));
				}
				else
				{
					Text component2 = componetInChild.GetWidget(0).GetComponent<Text>();
					component2.set_text(Singleton<CTextManager>.GetInstance().GetText("Achievement_Player_Info_Selected_Trophies_Other_No_Data"));
				}
			}
			else
			{
		...
		private void RefreshSelectedAchieveElement(CUIListElementScript elementScript, CAchieveItem2 item, int index, bool isSelf, ListView<CAchieveItem2> filteredTrophies)
		{
			...
			if (item == null)
			{
				...
				component3.set_text(isSelf ? Singleton<CTextManager>.GetInstance().GetText("Achievement_Status_Not_Done") : Singleton<CTextManager>.GetInstance().GetText("Achievement_Status_Not_Chosen"));
			...
			else
			{
				...
				component2.set_text(cAchieveItem.Cfg.szName);
				...
				{
					component3.set_text(Singleton<CTextManager>.GetInstance().GetText("Achievement_Status_Done"));
				}
				else
				{
					component3.set_text(string.Format("{0:yyyy.M.d} {1}", Utility.ToUtcTime2Local((long)((ulong)cAchieveItem.DoneTime)), Singleton<CTextManager>.GetInstance().GetText("Achievement_Status_Done")));
				}
				component4.enabled = true;
