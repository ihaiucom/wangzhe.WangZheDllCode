	...
	internal class Lobby3DInteraction
	{
		...
		private void onMouseClick(CUIEvent uiEvent)
		{
			...
			if (Physics.Raycast(ray, out raycastHit, float.PositiveInfinity, this.InteractLayerMask))
			{
				...
				if (text2 != null)
				{
					if (Lobby3DInteraction.<>f__switch$map0 == null)
					{
						...
						Lobby3DInteraction.<>f__switch$map0 = dictionary;
					}
					int num;
					if (Lobby3DInteraction.<>f__switch$map0.TryGetValue(text2, ref num))
					{
						switch (num)
						{
						case 0:
		...
		private void onDragging(CUIEvent uiEvent)
		{
			...
			transform.position = new Vector3(transform.position.x - uiEvent.m_pointerEventData.get_delta().x / 100f, transform.position.y, transform.position.z);
