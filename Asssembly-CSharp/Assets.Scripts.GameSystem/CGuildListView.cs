	...
	public class CGuildListView : Singleton<CGuildListView>
	{
		...
		private void On_Guild_Guild_Select(CUIEvent uiEvent)
		{
			...
			if (guildInfoByIndex != null)
			{
				...
				this.m_form.GetWidget(39).GetComponent<Text>().set_text(Singleton<CTextManager>.GetInstance().GetText("Common_Level_Format", new string[]
				{
				...
				component3.set_text(CGuildHelper.GetGradeName(guildInfoByIndex.RankInfo.totalRankPoint));
				component4.set_text(Singleton<CTextManager>.GetInstance().GetText("Guild_Profit_Desc", new string[]
				{
		...
		private void On_Guild_Prepare_Guild_Select(CUIEvent uiEvent)
		{
			...
			component4.set_text(prepareGuildInfoByIndex.stBriefInfo.stCreatePlayer.sName);
			...
			component6.set_text(prepareGuildInfoByIndex.stBriefInfo.stCreatePlayer.dwGameEntity.ToString());
			Text component7 = this.m_form.GetWidget(21).GetComponent<Text>();
			component7.set_text(prepareGuildInfoByIndex.stBriefInfo.sBulletin);
		}
		...
		private void On_Guild_Guild_Search_In_Guild_List_Panel(CUIEvent uiEvent)
		{
			...
			string text = srcFormScript.GetWidget(9).GetComponent<InputField>().get_text();
		...
		private void On_Guild_Guild_Search_In_Prepare_Guild_List_Panel(CUIEvent uiEvent)
		{
			...
			string text = srcFormScript.GetWidget(22).GetComponent<InputField>().get_text();
			if (!string.IsNullOrEmpty(text))
			{
		...
		private void On_Guild_PrepareGuild_Create_Confirm(CUIEvent uiEvent)
		{
			...
			else
			{
				...
				arg.sName = component.get_text().Trim();
				arg.sBulletin = component2.get_text().Trim();
				arg.dwHeadId = Convert.ToUInt32(component3.get_text());
		...
		private void On_Guild_Requesst_More_Prepare_Guild_List(CUIEvent uiEvent)
		{
			...
			if (elementAmount > 0)
			{
				...
				int arg = int.Parse(component2.get_text()) + 1;
		...
		public void RefreshCreateGuildPanel()
		{
			...
			if (dataByIndex != null)
			{
				...
				component3.set_text(dataByIndex.dwIcon.ToString());
			}
		}
		...
		private void RefreshPrepareGuildPanelPrepareGuildList(byte pageId, bool isHideListExtraContent)
		{
			...
			component2.set_text(pageId.ToString());
		}
		...
		private void RefreshPrepareGuildPanelMemberList()
		{
			...
			component5.set_text(this.m_Model.CurrentPrepareGuildInfo.stBriefInfo.stCreatePlayer.sName);
			component6.set_text(Singleton<CTextManager>.GetInstance().GetText("Common_Level_Format", new string[]
			{
				this.m_Model.CurrentPrepareGuildInfo.stBriefInfo.stCreatePlayer.dwLevel.ToString()
			}));
			component7.set_text(this.m_Model.CurrentPrepareGuildInfo.stBriefInfo.stCreatePlayer.dwAbility.ToString());
			...
			component8.set_text(this.m_Model.CurrentPrepareGuildInfo.stBriefInfo.sName);
			...
			component9.set_text(num.ToString() + "/" + num3.ToString());
		}
		...
		private void SetGuildListItem(CUIListElementScript listElementScript, GuildInfo info)
		{
			...
			component3.set_text(info.briefInfo.bMemberNum + "/" + CGuildHelper.GetMaxGuildMemberCountByLevel((int)info.briefInfo.bLevel));
			component4.set_text(info.chairman.stBriefInfo.sName);
			component5.set_text(info.RankInfo.totalRankPoint.ToString());
			component6.set_text(CGuildHelper.GetGuildJoinLimitText((int)info.briefInfo.LevelLimit, (int)info.briefInfo.GradeLimit, info.briefInfo.dwSettingMask));
		...
		private void SetPrepareGuildListItem(CUIListElementScript listElementScript, PrepareGuildInfo info)
		{
			...
			component.SetSprite(prefabPath, this.m_form, true, false, false, false);
			component2.set_text(info.stBriefInfo.sName);
			component3.set_text(info.stBriefInfo.stCreatePlayer.sName);
			component4.set_text((int)(info.stBriefInfo.bMemCnt - 1) + "/" + GameDataMgr.guildMiscDatabin.GetDataByKey(1u).dwConfValue);
		...
		private void SetPrepareGuildMemListItem(CUIListElementScript listElementScript, GuildMemInfo info)
		{
			...
			component5.set_text(info.stBriefInfo.dwLevel.ToString());
			component6.set_text(info.stBriefInfo.dwAbility.ToString());
		}
