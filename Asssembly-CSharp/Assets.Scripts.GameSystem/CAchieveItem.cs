...
namespace Assets.Scripts.GameSystem
{
	public class CAchieveItem : IComparable
	{
		...
		{
			string text = StringHelper.UTF8BytesToString(ref this.m_cfgInfo.szDesc);
			if (this.m_cfgInfo.dwDoneParm == 0u)
			{
				return string.Format(text, this.m_cfgInfo.dwDoneCondi);
			}
			return string.Format(text, this.m_cfgInfo.dwDoneCondi, this.m_cfgInfo.dwDoneParm);
		}

		public string GetAchievementTips()
		{
			string text = StringHelper.UTF8BytesToString(ref this.m_cfgInfo.szTips);
			return string.Format(text, Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo().Name);
		}
