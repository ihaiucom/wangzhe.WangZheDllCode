...
namespace Assets.Scripts.GameSystem
{
	public class InBattleMsgMgr : Singleton<InBattleMsgMgr>
	{
		...
		{
			if (tabIndex < 0 || tabIndex >= Singleton<InBattleMsgMgr>.instance.title_list.get_Count())
			{
				return null;
			}
			ListView<TabElement> listView = null;
			string key = Singleton<InBattleMsgMgr>.instance.title_list.get_Item(tabIndex);
		...
		public void ParseServerData(COMDT_SELFDEFINE_CHATINFO chatInfo)
		{
			...
			for (int j = 0; j < (int)chatInfo.bMsgCnt; j++)
			{
				...
				{
					string text = StringHelper.BytesToString(cOMDT_SELFDEFINE_DETAIL_CHATINFO2.stChatInfo.stSelfDefineStr.szContent);
		...
		public void ParseCfgData()
		{
			...
			if (!int.TryParse(Singleton<CTextManager>.instance.GetText("InBat_Bubble_CDTime"), ref InBattleShortcut.InBat_Bubble_CDTime))
			{
		...
		public void Handle_InBattleMsg_Ntf(COMDT_CHAT_MSG_INBATTLE obj)
		{
			...
			{
				uint dwTextID2 = obj.stChatInfo.stBubbleID.dwTextID;
				if (this.m_shortcutChat != null)
				{
					this.m_shortcutChat.InnerHandle_InBat_PreConfigMsg((COM_INBATTLE_CHAT_TYPE)obj.bChatType, dwAcntHeroID, dwTextID2, ullUid);
				}
			}
