...
namespace Assets.Scripts.GameSystem
{
	public class CEquipRelationPath
	{
		...
		private void SetEquipPositionDictionary(List<ushort>[] equipList)
		{
			...
			{
				for (int j = 0; j < equipList[i].get_Count(); j++)
				{
					if (equipList[i].get_Item(j) > 0 && !this.m_equipPositionDictionary.ContainsKey(equipList[i].get_Item(j)))
					{
						this.m_equipPositionDictionary.Add(equipList[i].get_Item(j), new Vector2((float)i, (float)j));
					}
				}
			}
		}
		...
		private void EnableEquipNode(ushort equipID)
		{
			...
			if (this.m_equipPositionDictionary.TryGetValue(equipID, ref zero) && zero.x >= 0f && zero.x < (float)this.m_equipNodes.Length && zero.y >= 0f && zero.y < (float)this.m_equipNodes[(int)zero.x].Length)
			{
		...
		private void EnablePreEquipNode(ushort equipID, Dictionary<ushort, CEquipInfo> equipInfoDictionary)
		{
			...
			if (equipInfoDictionary.TryGetValue(equipID, ref cEquipInfo) && cEquipInfo.m_resEquipInBattle != null)
			{
		...
		{
			CEquipInfo cEquipInfo = null;
			if (equipInfoDictionary.TryGetValue(equipID, ref cEquipInfo) && cEquipInfo.m_backEquipIDs != null)
			{
				for (int i = 0; i < cEquipInfo.m_backEquipIDs.get_Count(); i++)
				{
					this.EnableEquipNode(cEquipInfo.m_backEquipIDs.get_Item(i));
					this.DisplayEquipLineSet(equipID, cEquipInfo.m_backEquipIDs.get_Item(i));
					this.EnableBackEquipNode(cEquipInfo.m_backEquipIDs.get_Item(i), equipInfoDictionary);
				}
			}
		}
		...
		private void DisplayEquipLineSet(ushort equipID1, ushort equipID2)
		{
			...
			if (this.m_equipPositionDictionary.TryGetValue(equipID1, ref zero) && this.m_equipPositionDictionary.TryGetValue(equipID2, ref zero2) && Mathf.Abs(zero.x - zero2.x) == 1f)
			{
				Vector2 vector = (zero.x < zero2.x) ? zero : zero2;
				Vector2 vector2 = (zero.x < zero2.x) ? zero2 : zero;
