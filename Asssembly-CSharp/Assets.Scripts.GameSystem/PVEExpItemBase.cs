...
namespace Assets.Scripts.GameSystem
{
	internal abstract class PVEExpItemBase
	{
		...
		{
			this.m_NameText.set_text(this.m_Name);
			this.m_ExpTxt.set_text(this.getExpString(this.ExpToAdd));
			this.m_LevelTxt.set_text(string.Format("Lv{0}", this.m_level));
		...
		protected void TweenExpBar(float dst, bool bUpdate)
		{
			...
			LeanTween.value(this.m_Root, new Action<float>(this.TweenUpdate), this.m_exp, dst, bUpdate ? 1f : 1.6f).setEase(LeanTweenType.linear);
		}
