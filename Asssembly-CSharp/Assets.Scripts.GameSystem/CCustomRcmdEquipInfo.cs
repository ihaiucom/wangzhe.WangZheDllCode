...
namespace Assets.Scripts.GameSystem
{
	public class CCustomRcmdEquipInfo
	{
		...
		public bool GetSelfDefineRcmdEquip(uint heroId, uint rcmdId, ref ushort[] equipIds)
		{
			...
			if (this.m_customRecommendEquipDictionary.TryGetValue(heroId, ref stRcmdEquipListInfo) && stRcmdEquipListInfo.ListItem[(int)((uint)((UIntPtr)rcmdId))].bSelfDefine)
			{
				equipIds = stRcmdEquipListInfo.ListItem[(int)((uint)((UIntPtr)rcmdId))].EquipId;
				return true;
			}
			return false;
		}
		...
		{
			stRcmdEquipListInfo stRcmdEquipListInfo;
			if (this.m_customRecommendEquipDictionary.TryGetValue(heroId, ref stRcmdEquipListInfo))
			{
		...
		{
			stRcmdEquipListInfo stRcmdEquipListInfo;
			if (this.m_customRecommendEquipDictionary.TryGetValue(recoverDefaultRcmdEquip.dwHeroId, ref stRcmdEquipListInfo))
			{
				stRcmdEquipListInfo.ListItem[(int)((uint)((UIntPtr)recoverDefaultRcmdEquip.dwEquipIndex))].bSelfDefine = false;
				CCustomRcmdEquipInfo.SetDefaultRcmdEquip(ref stRcmdEquipListInfo.ListItem[(int)((uint)((UIntPtr)recoverDefaultRcmdEquip.dwEquipIndex))].EquipId, recoverDefaultRcmdEquip.dwHeroId, recoverDefaultRcmdEquip.dwEquipIndex);
				this.m_customRecommendEquipDictionary.set_Item(recoverDefaultRcmdEquip.dwHeroId, stRcmdEquipListInfo);
			}
		}
		...
		public void OnChangeUsedHeroRcmdEquipID(uint heroId, uint useRcmdId)
		{
			...
			if (this.m_customRecommendEquipDictionary.TryGetValue(heroId, ref defaultRcmdEquipListInfo))
			{
				defaultRcmdEquipListInfo.CurUseID = useRcmdId;
				this.m_customRecommendEquipDictionary.set_Item(heroId, defaultRcmdEquipListInfo);
			}
			else
			{
		...
		public void OnChgHeroRcmdEquipPlanName(uint heroId, uint index, COMDT_HERO_EQUIP_NAME equipName)
		{
			...
			if (this.m_customRecommendEquipDictionary.TryGetValue(heroId, ref defaultRcmdEquipListInfo))
			{
				...
				{
					defaultRcmdEquipListInfo.ListItem[(int)((uint)((UIntPtr)index))].Name = StringHelper.UTF8BytesToString(ref equipName.szNameStr);
				}
				else
				{
					defaultRcmdEquipListInfo.ListItem[(int)((uint)((UIntPtr)index))].Name = string.Format(Singleton<CTextManager>.GetInstance().GetText("CustomEquip_RcmdEquipPlan_Name"), index + 1u);
				}
				this.m_customRecommendEquipDictionary.set_Item(heroId, defaultRcmdEquipListInfo);
			}
			else
		...
		public string GetRcmdEquipPlanName(uint heroId, uint index)
		{
			...
			if (this.m_customRecommendEquipDictionary.TryGetValue(heroId, ref stRcmdEquipListInfo))
			{
				return stRcmdEquipListInfo.ListItem[(int)((uint)((UIntPtr)index))].Name;
			}
