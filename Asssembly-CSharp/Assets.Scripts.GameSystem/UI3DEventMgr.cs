	...
	public class UI3DEventMgr
	{
		...
		public bool HandleClickEvent(PointerEventData pointerEventData)
		{
			for (int i = 0; i < this.m_evtComsHeros.Count; i++)
			{
				...
				if (uI3DEventCom != null && !uI3DEventCom.isDead && uI3DEventCom.m_screenSize.Contains(pointerEventData.get_pressPosition()))
				{
			...
			for (int j = 0; j < this.m_evtComsTowers.Count; j++)
			{
				...
				if (uI3DEventCom2 != null && !uI3DEventCom2.isDead && uI3DEventCom2.m_screenSize.Contains(pointerEventData.get_pressPosition()))
				{
