...
namespace Assets.Scripts.GameSystem
{
	public class CMallSymbolGiftController : Singleton<CMallSymbolGiftController>
	{
		...
		private void PlayLotteryAnimation(CUIFormScript form, string state, bool PlayAni = true, bool PlayFx = false, bool enableEndEvent = false)
		{
			if (state != null)
			{
				if (CMallSymbolGiftController.__f__switch_map2 == null)
				{
					...
					CMallSymbolGiftController.__f__switch_map2 = dictionary;
				}
				int num;
				if (CMallSymbolGiftController.__f__switch_map2.TryGetValue(state, ref num) && num == 0)
				{
		...
		private void RefreshDesc(CUIFormScript form)
		{
			...
			{
				componetInChild.set_text(string.Format(Singleton<CTextManager>.GetInstance().GetText("Mall_Symbol_Gifts_Common_Buy_Dsc"), (int)this.m_CommonDrawSymbolCycle - CMallSymbolGiftController.GetTotalDrawedCnt(RES_SHOPBUY_TYPE.RES_BUYTYPE_DRAWSYMBOLCOMMON) % (int)this.m_CommonDrawSymbolCycle));
			}
			if (componetInChild2 != null)
			{
				componetInChild2.set_text(string.Format(Singleton<CTextManager>.GetInstance().GetText("Mall_Symbol_Gifts_Senior_Buy_Dsc"), (int)this.m_SeniorDrawSymbolCycle - CMallSymbolGiftController.GetTotalDrawedCnt(RES_SHOPBUY_TYPE.RES_BUYTYPE_DRAWSYMBOLSENIOR) % (int)this.m_SeniorDrawSymbolCycle));
			}
		}
