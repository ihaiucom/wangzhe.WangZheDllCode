	...
	public class BattleDragonView
	{
		...
		private void OnActorDead(ref GameDeadEventParam prm)
		{
			...
			if (src && orignalAtker && src.handle.TheActorMeta.ConfigId == Singleton<BattleLogic>.instance.DragonId && src.handle.TheActorMeta.ActorType == ActorTypeDef.Actor_Type_Monster)
			{
				...
				{
					byte[] dragonBuffCount = this.m_dragonBuffCount;
					int num = 0;
					byte[] expr_82_cp_0 = dragonBuffCount;
					int expr_82_cp_1 = num;
					expr_82_cp_0[expr_82_cp_1] += 1;
				}
				else if (orignalAtker.handle.TheActorMeta.ActorCamp == COM_PLAYERCAMP.COM_PLAYERCAMP_2)
				{
					byte[] dragonBuffCount2 = this.m_dragonBuffCount;
					int num2 = 1;
					byte[] expr_B8_cp_0 = dragonBuffCount2;
					int expr_B8_cp_1 = num2;
					expr_B8_cp_0[expr_B8_cp_1] += 1;
				}
			}
			this.Draw();
		}
