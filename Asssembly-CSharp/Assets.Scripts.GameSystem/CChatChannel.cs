...
namespace Assets.Scripts.GameSystem
{
	public class CChatChannel
	{
		...
		public CChatEntity GetLast()
		{
			...
			{
				return (this.list[this.list.Count - 1].type == EChaterType.System || this.list[this.list.Count - 1].type == EChaterType.OfflineInfo) ? null : this.list[this.list.Count - 1];
			}
		...
		public bool HasAnyValidChatEntity()
		{
			for (int i = 0; i < this.list.Count; i++)
			{
				...
				if (cChatEntity != null && (cChatEntity.type == EChaterType.Self || cChatEntity.type == EChaterType.Friend || cChatEntity.type == EChaterType.Strenger))
				{
					return true;
				}
			}
		...
		public int GetUnreadMeanbleChatEntCount(int start_index = 0)
		{
			...
			{
				CChatEntity cChatEntity = this.list[i];
				if (cChatEntity != null && this.IsMeanbleChatEnt(cChatEntity) && !cChatEntity.bHasReaded)
				{
					num++;
				}
			}
