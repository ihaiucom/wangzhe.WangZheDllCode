	...
	internal class PVESettleView
	{
		...
		private static void ShowReward(CUIFormScript belongForm, COMDT_SETTLE_RESULT_DETAIL settleData)
		{
			...
			if (stAcntInfo != null)
			{
				...
				component8.set_text(string.Format("Lv.{0}", stAcntInfo.dwPvpLv.ToString()));
				...
				component2.set_text(string.Format("{0}/{1}", stAcntInfo.dwPvpExp, dataByKey.dwNeedExp));
				component3.set_text(string.Format("+{0}", stAcntInfo.dwPvpSettleExp));
				...
				component6.set_text(Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo().Name);
			...
			{
				COMDT_REWARD_INFO cOMDT_REWARD_INFO2 = stReward.astRewardDetail[j];
				byte bType2 = cOMDT_REWARD_INFO2.bType;
				if (bType2 != 6)
				{
					if (bType2 == 11)
					{
						...
						if (gameObject5 != null)
						{
							...
							CRoleInfo masterRoleInfo = Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo();
							uint qqVipExtraCoin = CUseable.GetQqVipExtraCoin(cOMDT_REWARD_INFO2.stRewardInfo.dwPvpCoin, ref settleData.stMultipleDetail, 0);
							if (masterRoleInfo != null && qqVipExtraCoin > 0u)
							{
								component11.set_text(string.Format("+{0}", qqVipExtraCoin));
			...
			if (listView.Count > 0)
			{
				...
				COMDT_REWARD_INFO rewardInfo = listView[0];
				PVESettleView.SetItemEtcCell(belongForm, gameObject9, component12, rewardInfo, settleData);
			}
		}
		...
		private static void DoCoinAndExpTween()
		{
			...
			if (PVESettleView._coinTweenText != null && PVESettleView._coinTweenText.gameObject != null)
			{
				PVESettleView._coinLTD = LeanTween.value(PVESettleView._coinTweenText.gameObject, delegate(float value)
				{
					if (PVESettleView._coinTweenText != null && PVESettleView._coinTweenText.gameObject != null)
					{
						PVESettleView._coinTweenText.set_text(string.Format("+{0}", value.ToString("N0")));
		...
		public static void DoCoinTweenEnd()
		{
			...
			{
				PVESettleView._coinTweenText.set_text(string.Format("+{0}", PVESettleView._coinTo.ToString("N0")));
		...
		public static void ShowPlayerLevelUp(CUIFormScript form, int oldLvl, int newLvl)
		{
			if (form != null)
			{
				...
				if (!hasBuy && symbolPosOpenCnt2 > symbolPosOpenCnt)
				{
					...
					component3.set_text(symbolPosOpenCnt.ToString());
					Text component4 = transform.Find("levelUpCntText").gameObject.GetComponent<Text>();
					component4.set_text(symbolPosOpenCnt2.ToString());
				}
				...
				if (symbolLvlLimit2 > symbolLvlLimit)
				{
					...
					component5.set_text(symbolLvlLimit.ToString());
					component6.set_text(symbolLvlLimit2.ToString());
				}
				...
				if (dataByKey3 != null)
				{
					...
					if (dataByKey3.bPresentSymbolPage > 0 && masterRoleInfo != null)
					{
						...
						component7.set_text((masterRoleInfo.m_symbolInfo.m_pageCount - 1).ToString());
						component8.set_text(masterRoleInfo.m_symbolInfo.m_pageCount.ToString());
					}
				}
			}
		}
