		...
		private class DragonNode
		{
			...
			public void ValidateCD()
			{
				...
				if (flag2)
				{
					...
					this.cdTxtInMini.set_text(text);
					this.cdTxtInBig.set_text(text);
				}
				else
				{
		...
		private void onDragon_Born(ActorRoot actor)
		{
			...
			if (actorSubSoliderType == 8 || actorSubSoliderType == 9 || actorSubSoliderType == 13)
			{
				...
				if (flag)
				{
					...
					{
						Vector3 v = currentCamera.WorldToScreenPoint(flag ? dragonNode.mmDragonNode_3dui.transform.position : dragonNode.bmDragonNode_3dui.transform.position);
						Singleton<CUIParticleSystem>.instance.AddParticle(DragonIcon.dragonBornEffect, 3f, v, default(Quaternion?));
					}
				}
			}
