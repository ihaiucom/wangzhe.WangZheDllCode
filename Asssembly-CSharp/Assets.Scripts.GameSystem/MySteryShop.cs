...
namespace Assets.Scripts.GameSystem
{
	...
	public class MySteryShop : Singleton<MySteryShop>
	{
		...
		public int GetTimeToClose()
		{
			...
			{
				return (timeSpan.get_TotalSeconds() > 2147483647.0) ? 2147483647 : ((int)timeSpan.get_TotalSeconds());
			}
			return 0;
		}
		...
		private void RefreshDiscount()
		{
			...
			if (this.IsGetDisCount())
			{
				...
				if (gameObject != null)
				{
					...
					{
						componetInChild2.set_text(string.Format("{0}/{1}", this.m_bRequestBuyCount, this.m_AkaliShopDetail.bMaxBuyCnt));
					}
				}
			}
			else
		...
		public static void ReceiveBuyItem(CSPkg msg)
		{
			...
			if (msg.stPkgData.stAkaliShopBuyRsp.iResult == 0)
			{
				...
				MySteryShop expr_26 = instance;
				expr_26.m_bRequestBuyCount += 1;
				Singleton<MySteryShop>.GetInstance().UpdateUI();
