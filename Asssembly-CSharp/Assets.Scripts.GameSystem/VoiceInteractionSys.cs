...
namespace Assets.Scripts.GameSystem
{
	public class VoiceInteractionSys : MonoSingleton<VoiceInteractionSys>
	{
		...
		private void StartupSys()
		{
			...
			for (int i = 0; i < allPlayers.get_Count(); i++)
			{
				...
				if (player.Captain)
				{
					...
					if (!this.HeroStatInfo.TryGetValue(configId, ref hashSet))
					{
		...
		{
			HashSet<int> hashSet = null;
			return this.HeroStatInfo.TryGetValue(InCfgID, ref hashSet) && hashSet.Contains(InTestCamp);
		}

		private bool HasReceiverNotInCamp(int InCfgID, int InTestCamp)
		{
			HashSet<int> hashSet = null;
			if (this.HeroStatInfo.TryGetValue(InCfgID, ref hashSet))
			{
