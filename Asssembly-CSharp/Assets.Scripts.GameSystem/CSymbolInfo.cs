	...
	public class CSymbolInfo
	{
		...
		private void InitSymbolPvpLevelList()
		{
			...
			while (enumerator.MoveNext())
			{
				...
				ResHeroSymbolLvl resHeroSymbolLvl = (ResHeroSymbolLvl)current.get_Value();
		...
		public enSymbolWearState GetSymbolPosWearState(int page, int pos, out int param)
		{
			...
			while (i < num)
			{
				...
				if (pvpLevel < (uint)symbolPos2.wOpenLevel && this.GridBuyInfo[(int)(symbolPos2.bSymbolPos - 1)].bBuyFlag == 0 && symbolPos.dwSymbolColor == symbolPos2.dwSymbolColor)
				{
					...
					{
						return ((long)param == (long)((ulong)latestOpenLevel)) ? enSymbolWearState.CanBuyAndWillOpen : enSymbolWearState.CanBuy;
					}
		...
		public bool GetWearPos(CSymbolItem item, int page, out int pos, out enFindSymbolWearPosCode findCode)
		{
			...
			{
				ResSymbolPos symbolPos = CSymbolInfo.GetSymbolPos(i);
				if (((ulong)symbolPos.dwSymbolColor & 1uL << (int)(item.m_SymbolData.bColor & 31)) != 0uL)
				{
					flag = (flag && array[i] != 0uL);
				}
				if (array[i] == 0uL && (masterRoleInfo.PvpLevel >= (uint)symbolPos.wOpenLevel || this.GridBuyInfo[i].bBuyFlag == 1) && ((ulong)symbolPos.dwSymbolColor & 1uL << (int)(item.m_SymbolData.bColor & 31)) != 0uL)
				{
					pos = i;
			...
			findCode = (flag ? enFindSymbolWearPosCode.FindSymbolPosFull : enFindSymbolWearPosCode.FindSymbolNotOpen);
			return false;
		}
		...
		{
			return symbolPos != null && ((ulong)symbolPos.dwSymbolColor & 1uL << (int)(color & 31)) != 0uL;
		}
		...
		{
			return (uint)((long)(compType * (RES_SYMBOLCOMP_TYPE)10000 + level * 1000) + (long)((ulong)cfgId));
		}
		...
		public void GetSymbolPageProp(int pageIndex, ref int[] propArr, ref int[] propPctArr, bool bPvp)
		{
			...
			if (masterRoleInfo != null && array != null)
			{
				...
				for (int j = 0; j < array.Length; j++)
				{
					if (array[j] > 0uL)
					{
						CUseable useableByObjID = useableContainer.GetUseableByObjID(array[j]);
						if (useableByObjID != null)
						{
							ResSymbolInfo dataByKey = GameDataMgr.symbolInfoDatabin.GetDataByKey(useableByObjID.m_baseID);
							if (dataByKey != null)
							{
								ResDT_FuncEft_Obj[] array2 = bPvp ? dataByKey.astFuncEftList : dataByKey.astPveEftList;
								if (array2 != null)
								{
									for (int k = 0; k < array2.Length; k++)
									{
										if (array2[k].wType != 0 && array2[k].wType < 37 && array2[k].iValue != 0)
										{
											if (array2[k].bValType == 0)
											{
												propArr[(int)array2[k].wType] += array2[k].iValue;
											}
											else if (array2[k].bValType == 1)
											{
												propPctArr[(int)array2[k].wType] += array2[k].iValue;
											}
										}
									}
								}
