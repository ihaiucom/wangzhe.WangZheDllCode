...
namespace Assets.Scripts.GameSystem
{
	public class HeroInfoPanel
	{
		...
		private void AddHeroToDic(PoolObjHandle<ActorRoot> actor, int listIndex)
		{
			...
			{
				this.m_HeroToIndexDic.set_Item(actor.handle.ObjID, listIndex);
			}
		}
		...
		private void RefreshHeroIconImageColor(CUIListElementScript elementScript, PoolObjHandle<ActorRoot> actor)
		{
			...
			if (widget != null)
			{
				...
				{
					component.set_color(actor.handle.ActorControl.IsDeadState ? CUIUtility.s_Color_GrayShader : CUIUtility.s_Color_White);
				}
				else
				{
		...
		{
			int num = 0;
			if (!actor || !this.m_HeroToIndexDic.TryGetValue(actor.handle.ObjID, ref num))
			{
				return;
			}
		...
		private void OnActorDead(ref GameDeadEventParam eventParam)
		{
			if (eventParam.src && eventParam.src.handle.TheActorMeta.ActorType == ActorTypeDef.Actor_Type_Hero)
			{
				...
				if (!flag)
				{
					...
					num = this.m_enemyHeroList.get_Count() - 1;
					cUIListElementScript = this.GetHeroListElement(num + 5);
				}
				if (this.m_HeroToIndexDic.TryGetValue(eventParam.src.handle.ObjID, ref num))
				{
		...
		private void OnActorRevive(ref DefaultGameEventParam eventParam)
		{
			if (eventParam.src && eventParam.src.handle.TheActorMeta.ActorType == ActorTypeDef.Actor_Type_Hero)
			{
				...
				if (flag)
				{
					if (this.m_HeroToIndexDic.TryGetValue(eventParam.src.handle.ObjID, ref listIndex))
					{
		...
		{
			int count = this.m_enemyHeroList.get_Count();
			for (int i = 0; i < 5; i++)
			{
				...
				if (heroListElement != null)
				{
					...
					if (i < count)
					{
						this.RefreshHeroIconImage(heroListElement, this.m_enemyHeroList.get_Item(i));
						this.RefreshHeroReviveCntDown(heroListElement, this.m_enemyHeroList.get_Item(i));
					}
				}
			}
		}
