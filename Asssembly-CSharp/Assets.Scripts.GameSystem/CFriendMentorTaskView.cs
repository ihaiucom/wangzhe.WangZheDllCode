...
namespace Assets.Scripts.GameSystem
{
	public class CFriendMentorTaskView
	{
		...
		public void showTask(uint taskid)
		{
			...
			componetInChild.set_text(task.m_taskTitle);
			...
			for (int i = 0; i < num; i++)
			{
				...
				if (!(gameObject == null))
				{
					...
					{
						gameObject.CustomSetActive(true);
						gameObject.transform.FindChild("desc").GetComponent<Text>().set_text(taskViewConditionData.condition);
						gameObject.transform.FindChild("progress").GetComponent<Text>().set_text(taskViewConditionData.progress);
