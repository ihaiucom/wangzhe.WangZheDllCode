	...
	public class CArenaSystem : Singleton<CArenaSystem>
	{
		...
		private void Arena_OpenForm(CUIEvent uiEvent)
		{
			if (Singleton<CFunctionUnlockSys>.instance.FucIsUnlock(RES_SPECIALFUNCUNLOCK_TYPE.RES_SPECIALFUNCUNLOCKTYPE_ARENA))
			{
				...
				if (Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo().m_arenaDefHeroList.get_Count() == 0)
				{
		...
		public void RefreshArenaForm()
		{
			...
			component.SetTotalTime((ulong)this.m_serverInfo.dwNextRefreshLeftTime - (ulong)((long)CRoleInfo.GetElapseSecondsSinceLogin()));
			...
			component3.set_text(Singleton<CTextManager>.GetInstance().GetText("Arena_Fighting_Power", new string[]
			{
				masterRoleInfo.PvpLevel.ToString()
			}));
			if (this.m_fightHeroInfoList.stArenaInfo.dwSelfRank > 0u)
			{
				component4.set_text(Singleton<CTextManager>.GetInstance().GetText("Arena_My_Rank", new string[]
				{
			...
			{
				component4.set_text(Singleton<CTextManager>.GetInstance().GetText("Arena_Not_In_Rank"));
			}
			if (this.m_fightHeroInfoList.stArenaInfo.dwSelfRank == 1u)
			{
				component5.gameObject.CustomSetActive(true);
			}
			component2.SetElementAmount(arenaDefHeroList.get_Count());
			for (int i = 0; i < arenaDefHeroList.get_Count(); i++)
			{
			...
			for (int j = 0; j < (int)this.m_fightHeroInfoList.stArenaInfo.stMatchFighter.bFigterNum; j++)
			{
				...
				component6.set_text(StringHelper.UTF8BytesToString(ref fighterInfo.szName));
				component7.set_text(Singleton<CTextManager>.GetInstance().GetText("Arena_13", new string[]
				{
					this.m_fightHeroInfoList.stArenaInfo.stMatchFighter.astFigterDetail[j].dwRank.ToString()
				}));
				component8.set_text(Singleton<CTextManager>.GetInstance().GetText("Arena_Fighting_Power", new string[]
				{
					fighterInfo.dwPVPLevel.ToString()
				}));
				component9.set_text(fighterInfo.dwPVPLevel.ToString());
			...
			component12.set_text(Singleton<CTextManager>.GetInstance().GetText("Arena_Today_Challenge_Count", new string[]
			{
		...
		public void BattleReturn(bool isWin)
		{
			...
			CSDT_ACNT_ARENADATA serverInfo = Singleton<CArenaSystem>.GetInstance().m_serverInfo;
			CSDT_ACNT_ARENADATA expr_36 = serverInfo;
			expr_36.chAlreadyFightCnt = (sbyte)((int)expr_36.chAlreadyFightCnt + 1);
		}
		...
		private void SetResultCell(GameObject root, string name, uint pvpLevel, int battleValue, uint rank, int memberType, string headPath, CUIFormScript form)
		{
			...
			component2.set_text(pvpLevel.ToString());
			component3.set_text(rank.ToString());
		...
		private void Arena_OpenPlayerInfoForm(CUIEvent uiEvent)
		{
			...
			component2.set_text(Singleton<CTextManager>.GetInstance().GetText("Arena_Fighting_Power", new string[]
			{
				fighterInfo.dwPVPLevel.ToString()
			}));
			component3.set_text(Singleton<CTextManager>.GetInstance().GetText("Arena_13", new string[]
			{
		...
		private void Arena_RankInfoMenuClick(CUIEvent uiEvent)
		{
			...
			else if (selectedIndex == 1)
			{
				...
				for (int i = 0; i < component2.GetElementAmount(); i++)
				{
					...
					component6.set_text(Singleton<CTextManager>.GetInstance().GetText("Arena_13", new string[]
					{
						(i + 1).ToString()
					}));
				...
				component7.set_text(Singleton<CTextManager>.GetInstance().GetText("Arena_My_Rank", new string[]
				{
					this.m_fightHeroInfoList.stArenaInfo.dwSelfRank.ToString()
				}));
		...
		public static void Arena_RankElementEnable(CUIEvent uiEvent)
		{
			...
			component.set_text(Singleton<CTextManager>.GetInstance().GetText("Arena_13", new string[]
			{
				(srcWidgetIndexInBelongedList + 1).ToString()
			}));
			component2.set_text(StringHelper.UTF8BytesToString(ref fighterInfo.szName));
			component3.set_text(Singleton<CTextManager>.GetInstance().GetText("Arena_Fighting_Power", new string[]
			{
				fighterInfo.dwPVPLevel.ToString()
			}));
		...
		private void Arena_RecordlementEnable(CUIEvent uiEvent)
		{
			...
			if (this.m_recordList[srcWidgetIndexInBelongedList].ullAtkerUid == playerUllUID)
			{
				...
				{
					component3.set_text(Math.Abs((int)(this.m_recordList[srcWidgetIndexInBelongedList].dwAtkerRank - this.m_recordList[srcWidgetIndexInBelongedList].dwTargetRank)).ToString());
					component6.gameObject.CustomSetActive(true);
					component4.gameObject.CustomSetActive(true);
				}
				else
		...
		public static void SendSetDefTeamConfigMSG(List<uint> heroList)
		{
			...
			cSPkg.stPkgData.stSetBattleListOfArenaReq.stBattleList.wHeroCnt = (ushort)heroList.get_Count();
		...
		public static void ReciveRankListInfo(CSPkg msg)
		{
			...
			for (int i = 0; i < (int)stGetTopFighterOfArenaRsp.stTopFighter.bFigterNum; i++)
			{
				if (stGetTopFighterOfArenaRsp.stTopFighter.astFigterDetail[i].stFigterData.bMemberType == 1 && masterRoleInfo.playerUllUID == stGetTopFighterOfArenaRsp.stTopFighter.astFigterDetail[i].stFigterData.stMember.stAcnt.ullUid)
				{
					...
					for (int j = 0; j < masterRoleInfo.m_arenaDefHeroList.get_Count(); j++)
					{
						COMDT_ARENA_HERODETAIL cOMDT_ARENA_HERODETAIL = new COMDT_ARENA_HERODETAIL();
						cOMDT_ARENA_HERODETAIL.dwHeroId = masterRoleInfo.m_arenaDefHeroList.get_Item(j);
