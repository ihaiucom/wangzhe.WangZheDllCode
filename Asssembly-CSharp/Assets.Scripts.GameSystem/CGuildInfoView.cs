	...
	public class CGuildInfoView : Singleton<CGuildInfoView>
	{
		...
		private void On_Guild_Guild_Setting_Guild_Icon_Selected(CUIEvent uiEvent)
		{
			...
			if (form != null)
			{
				...
				component2.set_text(component.get_text());
		...
		private void On_Guild_Guild_Setting_Confirm(CUIEvent uiEvent)
		{
			...
			if (component != null)
			{
				try
				{
					arg = Convert.ToUInt32(component.get_text());
				}
				catch (Exception ex)
				{
			...
			uint num = Convert.ToUInt32((int)component2.get_value() == 0);
			...
			int arg2;
			if (!int.TryParse(component3.get_text(), ref arg2))
			{
			...
			int arg3;
			if (!int.TryParse(component4.get_text(), ref arg3))
			{
		...
		private void On_Guild_Setting_Down_Join_Level_Num(CUIEvent uiEvent)
		{
			...
			int num;
			if (int.TryParse(component.get_text(), ref num))
			{
				int num2 = num - 1;
		...
		private void On_Guild_Setting_Down_Join_Grade(CUIEvent uiEvent)
		{
			...
			int inShowGrade;
			if (int.TryParse(component.get_text(), ref inShowGrade))
			{
				...
				if (num >= 1)
				{
					...
					component.set_text(gradeDataByLogicGrade.bGrade.ToString());
					component2.set_text((num == 1) ? Singleton<CTextManager>.GetInstance().GetText("Guild_No_Grade_Limit_Tip") : StringHelper.UTF8BytesToString(ref gradeDataByLogicGrade.szGradeDesc));
				}
				if (num <= 1)
				{
		...
		private void On_Guild_Setting_Up_Join_Grade(CUIEvent uiEvent)
		{
			...
			if (int.TryParse(component.get_text(), ref inShowGrade))
			{
				...
				if (num <= CLadderSystem.MAX_RANK_LEVEL)
				{
					ResRankGradeConf gradeDataByLogicGrade = CLadderSystem.GetGradeDataByLogicGrade(num);
					component.set_text(gradeDataByLogicGrade.bGrade.ToString());
					component2.set_text(StringHelper.UTF8BytesToString(ref gradeDataByLogicGrade.szGradeDesc));
				}
				if (num >= CLadderSystem.MAX_RANK_LEVEL)
				{
		...
		private void On_Guild_Need_Approval_Slider_Value_Changed(CUIEvent uiEvent)
		{
			...
			component.set_text((num == 0) ? Singleton<CTextManager>.GetInstance().GetText("Common_Yes") : Singleton<CTextManager>.GetInstance().GetText("Common_No"));
		}
		...
		private void On_Guild_Application_Reject(CUIEvent uiEvent)
		{
			...
			Text component2 = elemenet.transform.Find("txtUidData").GetComponent<Text>();
			ulong arg;
			if (ulong.TryParse(component2.get_text(), ref arg))
			{
		...
		private void On_Guild_Recommend_Invite(CUIEvent uiEvent)
		{
			...
			ulong num;
			int arg;
			if (ulong.TryParse(component2.get_text(), ref num) && int.TryParse(component3.get_text(), ref arg))
			{
		...
		private void On_Guild_Recommend_Reject(CUIEvent uiEvent)
		{
			...
			ulong arg;
			int arg2;
			if (ulong.TryParse(component2.get_text(), ref arg) && int.TryParse(component3.get_text(), ref arg2))
			{
		...
		private void On_Guild_ApplyList_Save_Pref(CUIEvent uiEvent)
		{
			...
			PlayerPrefs.SetInt("Guild_ApplyList_CloseRedDot", component.get_isOn() ? 1 : 0);
		...
		private void On_Guild_Guild_Apply_Quit(CUIEvent uiEvent)
		{
			...
			uint num = (playerGuildMemberInfo == null) ? 0u : (playerGuildMemberInfo.dwConstruct * GameDataMgr.guildMiscDatabin.GetDataByKey(20u).dwConfValue / 100u);
			uint num2 = (playerGuildMemberInfo == null) ? 0u : (playerGuildMemberInfo.dwConstruct - num);
			string text;
		...
		private void OnGuildPositionAppointViceChairmanConfirm(CUIEvent uiEvent)
		{
			...
			if (Singleton<CUIManager>.GetInstance().GetForm("UGUI/Form/System/Guild/Form_Guild_Exchange_Position.prefab") != null)
			{
				...
				arg2 = ulong.Parse(component2.get_text());
			}
		...
		public void RefreshFacePanel()
		{
			...
			component2.set_text(currentGuildInfo.briefInfo.sName);
			CGuildHelper.SetStarLevelPanel(currentGuildInfo.star, transform, this.m_form);
			component3.set_text(currentGuildInfo.chairman.stBriefInfo.sName);
		}
		...
		public void RefreshBulletinPanel()
		{
			...
			component.set_text(currentGuildInfo.briefInfo.sBulletin);
		...
		public void RefreshMemberCntPanel()
		{
			...
			component.set_text(Singleton<CTextManager>.GetInstance().GetText("Guild_Member_Count_Format", new string[]
			{
		...
		public void RefreshApplyListPanel()
		{
			...
			object arg_60_0 = (recommendInfoCount == 0) ? null : this.m_Model.GetRecommendInfo();
			object arg_7A_0 = (applicantsCount == 0) ? null : this.m_Model.GetApplicants();
		...
		private void OpenSettingForm()
		{
			...
			component2.set_text(this.m_Model.CurrentGuildInfo.briefInfo.dwHeadId.ToString());
			...
			component3.set_value((float)(flag ? 0 : 1));
			Text component4 = cUIFormScript.GetWidget(5).GetComponent<Text>();
			component4.set_text(this.m_Model.CurrentGuildInfo.briefInfo.sName);
			...
			component5.set_text(this.m_Model.CurrentGuildInfo.briefInfo.LevelLimit.ToString());
			...
			component7.set_text(CGuildHelper.GetLadderGradeLimitText((int)this.m_Model.CurrentGuildInfo.briefInfo.GradeLimit));
			...
			component9.set_text(this.m_Model.CurrentGuildInfo.briefInfo.GradeLimit.ToString());
		...
		private void SetApplicantItem(CUIListElementScript listElementScript, stApplicantInfo info, bool isAdmin)
		{
			...
			component2.set_text(info.stBriefInfo.uulUid.ToString());
			component6.set_text(info.stBriefInfo.sName);
			Text text = component7;
			string text2 = Singleton<CTextManager>.GetInstance().GetText("Guild_Rankpoint_Level_Format", new string[]
			{
				info.stBriefInfo.dwLevel.ToString()
			});
			component7.set_text(text2);
			text.set_text(text2);
			...
			component10.set_text(string.Empty);
		}
		...
		private void SetRecommendItem(CUIListElementScript listElementScript, stRecommendInfo info, bool isAdmin)
		{
			...
			component2.set_text(info.logicWorldID.ToString());
			component6.set_text(info.name);
			component7.set_text(Singleton<CTextManager>.GetInstance().GetText("Guild_Rankpoint_Level_Format", new string[]
			{
				info.level.ToString()
			}));
			...
			component9.set_text(info.recommendName);
			if (transform2 != null)
			{
			...
			if (isAdmin)
			{
				...
				gameObject3.transform.Find("Text").GetComponent<Text>().set_text(Singleton<CTextManager>.GetInstance().GetText("Common_Invite"));
		...
		public void OpenExchangePositionForm(List<string> names, List<ulong> uids)
		{
			...
			for (int i = 0; i < names.get_Count(); i++)
			{
				...
				transform.Find("Name").GetComponent<Text>().set_text(names.get_Item(i));
				transform.Find("Uid").GetComponent<Text>().set_text(uids.get_Item(i).ToString());
			}
			component.SelectElement(0, true);
		}
		...
		public void RefreshRankpointInfoPanel()
		{
			...
			component3.set_text(CGuildHelper.GetGradeName(this.m_Model.CurrentGuildInfo.RankInfo.totalRankPoint));
			...
			{
				component4.set_text(playerGuildMemberInfo.RankInfo.weekRankPoint.ToString());
			}
			component5.set_text(CGuildHelper.GetRankpointClearTimeFormatString());
		}
		...
		private void SetRankpointMemberItem(Transform itemTransform, GuildMemInfo info, int elementIndex, CUIFormScript form, bool headParticleMask)
		{
			...
			component4.set_text(info.stBriefInfo.sName);
			component5.set_text(info.RankInfo.totalRankPoint.ToString());
			component6.set_text(info.RankInfo.weekRankPoint.ToString());
		}
		...
		private void SetRankpointGuildRankListItem(CUIListElementScript listElement, RankpointRankInfo info, enGuildRankpointRankListType rankListType, uint firstRankNo)
		{
			...
			component2.set_text(info.guildName);
			component3.set_text(info.rankScore.ToString());
			component4.set_text(Singleton<CTextManager>.GetInstance().GetText("Guild_Rankpoint_Member_Format", new string[]
			{
		...
		private void SetRankpointAwardPanel(CUIFormScript form, GameObject awardPanel, bool isWeekAward, uint rankNo, uint grade)
		{
			...
			if (isWeekAward)
			{
				...
				component2.set_text(CGuildHelper.GetRankpointWeekAwardCoin(rankNo).ToString());
			}
			else
			{
				component.SetSprite(CUIUtility.s_Sprite_Dynamic_Icon_Dir + "90005", form, true, false, false, false);
				component2.set_text(CGuildHelper.GetRankpointSeasonAwardDiamond(grade).ToString());
			}
		}
		...
		private void SetGuildIcon(CUIListElementScript listElementScript, ResGuildIcon info)
		{
			...
			component2.set_text(info.dwIcon.ToString());
		}
		...
		public void RefreshInfoPanelGuildBulletin()
		{
			...
			Text component = this.m_form.GetWidget(7).GetComponent<Text>();
			component.set_text(this.m_Model.CurrentGuildInfo.briefInfo.sBulletin);
		}
		...
		public void RefreshProfitPanel()
		{
			...
			component.set_text(Singleton<CTextManager>.GetInstance().GetText("Guild_Profit_Desc", new string[]
			{
		...
		private void SetPreviewGuildListItem(CUIListElementScript listElementScript, GuildInfo info)
		{
			...
			component.SetSprite(prefabPath, this.m_form, true, false, false, false);
			component2.set_text(info.briefInfo.sName);
			...
			component4.set_text(info.briefInfo.bMemberNum + "/" + CGuildHelper.GetMaxGuildMemberCountByLevel((int)info.briefInfo.bLevel));
			component5.set_text(info.chairman.stBriefInfo.sName);
			CGuildHelper.SetStarLevelPanel(info.star, panelTransform, listElementScript.m_belongedFormScript);
			component6.set_text(info.RankInfo.totalRankPoint.ToString());
		...
		private void SetGuildMemberListItem(CUIListElementScript listElementScript, GuildMemInfo memberInfo)
		{
			...
			component4.set_text(memberInfo.stBriefInfo.sName);
			...
			component5.set_text(CGuildHelper.GetPositionName(memberInfo.enPosition));
			gameObject3.CustomSetActive(Convert.ToBoolean(memberInfo.GuildMatchInfo.bIsLeader));
			component6.set_text(memberInfo.RankInfo.weekRankPoint.ToString());
			component7.set_text(memberInfo.RankInfo.totalRankPoint.ToString());
			if (CGuildHelper.IsMemberOnline(memberInfo))
			{
				component8.set_text(Singleton<CTextManager>.GetInstance().GetText("Common_Online"));
			}
			else
			{
				component8.set_text(Utility.GetRecentOnlineTimeString((int)memberInfo.LastLoginTime));
			}
		}
		...
		public void RefreshGuildName()
		{
			...
			if (form != null)
			{
				...
				component.set_text(this.m_Model.CurrentGuildInfo.briefInfo.sName);
			}
			if (this.m_form != null)
			{
				Text component2 = this.m_form.GetWidget(4).GetComponent<Text>();
				component2.set_text(this.m_Model.CurrentGuildInfo.briefInfo.sName);
			}
		}
		...
		public void On_Guild_Preview_Guild_List_Element_Select(CUIEvent uiEvent)
		{
			...
			component4.set_text(guildInfoByIndex.chairman.stBriefInfo.sName);
			component5.set_text(Singleton<CTextManager>.GetInstance().GetText("Common_Level_Format", new string[]
			{
				guildInfoByIndex.chairman.stBriefInfo.dwLevel.ToString()
			}));
			component6.set_text(guildInfoByIndex.briefInfo.sBulletin);
		...
		private void On_Guild_Guild_Search(CUIEvent uiEvent)
		{
			...
			string text = srcFormScript.GetWidget(4).GetComponent<InputField>().get_text();
			if (!string.IsNullOrEmpty(text))
			{
		...
		public void RefreshInfoPanelSignBtn()
		{
			...
			else
			{
				...
				{
					componentInChildren.set_color(CGuildInfoView.s_Text_Color_Btn_Sign_Default);
				}
		...
		public void OpenLogForm(SCPKG_GUILD_EVENT_RSP rsp)
		{
			...
			for (int i = (int)(rsp.bNum - 1); i >= 0; i--)
			{
				...
				text3.set_text(text3.get_text() + text + text2 + "\n");
			}
		}
		...
		private void On_Guild_JoinPlatformGroup(CUIEvent uiEvent)
		{
			...
			else
			{
				...
				string text = (masterRoleInfo == null) ? null : masterRoleInfo.Name;
		...
		public void SendRemindMsg()
		{
			...
			{
				string text2 = Utility.CreateMD5Hash(string.Format("{0}{1}{2}", currentUTCTime2, 4042, "j9cWiS6lPjw4g"), Utility.MD5_STRING_CASE.LOWER);
				text = string.Format("[{0}|{1}|{2}]", currentUTCTime2, 4042, text2);
			}
		...
		public void BindPlatformGroup()
		{
			...
			else
			{
				...
				string text4 = (masterRoleInfo == null) ? null : masterRoleInfo.Name;
				string wxGroupUnionId = this.GetWxGroupUnionId(false);
		...
		public string GetWxGroupUnionId(bool needIncrease = false)
		{
			...
			string text2 = text.PadLeft(5, '0');
			string text3 = Convert.ToString(CGuildHelper.GetGroupGuildId()).PadLeft(32, '0');
			string text4 = Convert.ToString(this.GetWxIncreasingId(false)).PadLeft(9, '0');
			return string.Format("{0}{1}{2}", text2, text3, text4);
		}
		...
		private void UnLinkWxGroupRsConvert(string s)
		{
			...
			{
				int num = Convert.ToInt32(dictionary.get_Item("ret"));
				string text = Convert.ToString(dictionary.get_Item("msg"));
				if (num != 0)
				{
					int num2 = num;
					if (num2 != -20000)
		...
		{
			CGuildInfoView.<UnLinkWxGroup>c__Iterator23 <UnLinkWxGroup>c__Iterator = new CGuildInfoView.<UnLinkWxGroup>c__Iterator23();
			<UnLinkWxGroup>c__Iterator.callback = callback;
			<UnLinkWxGroup>c__Iterator.<$>callback = callback;
			<UnLinkWxGroup>c__Iterator.<>f__this = this;
			return <UnLinkWxGroup>c__Iterator;
		}
		...
		public void RefreshPlatformGroupPanel(CGuildSystem.enPlatformGroupStatus status, bool isSelfInGroup = false)
		{
			...
			else
			{
				...
				string text8 = Singleton<CTextManager>.GetInstance().GetText("Guild_WXGroup_Operation_Text_UnBind");
