	...
	public class BattleStatView
	{
		private class HeroItem
		{
			...
			public void updateReviceCD(Player curPlayer)
			{
				...
				{
					this.reviveTime.set_text(SimpleNumericString.GetNumeric(Mathf.FloorToInt((float)curPlayer.Captain.handle.ActorControl.ReviveCooldown * 0.001f)));
			...
			public void updateTalentSkillCD(Player curPlayer, CUIFormScript parentFormScript)
			{
				...
				else
				{
					...
					{
						this.talentSkillCD.set_text(SimpleNumericString.GetNumeric(Mathf.FloorToInt((float)skillSlot.CurSkillCD * 0.001f)));
		...
		private class ChartView
		{
			...
			public void Show(BattleStatView.ChartView.ChartType chartType, SampleData data, CUIFormScript form, ArrayList dragonKillInfos)
			{
				...
				this.title.set_text(Singleton<CTextManager>.GetInstance().GetText(text + "_Title"));
				...
				float num = (data.count > 1) ? (data.step * (float)(data.count - 1)) : 1f;
				int num2 = Mathf.CeilToInt(num / 4f);
				for (int i = 0; i <= 4; i++)
				{
					int num3 = i * num2;
					this.xAxis[i].set_text(string.Format("{0:D2}:{1:D2}", num3 / 60, num3 % 60));
				}
				...
				{
					string text2 = "+" + num5 * j;
					this.yAxis[num6 + j].set_text(text2);
					this.yAxis[num6 - j].set_text(text2);
				}
				...
				Vector3[][] array = new Vector3[dragonKillInfos.get_Count() + 1][];
				Color[] array2 = new Color[dragonKillInfos.get_Count() + 1];
				array[0] = new Vector3[data.count];
				array2[0] = ((chartType == BattleStatView.ChartView.ChartType.ExpTrend) ? Color.green : Color.yellow);
				int num11 = 0;
				for (int k = 0; k < data.count; k++)
				{
					...
					if (k > 0 && num11 < dragonKillInfos.get_Count())
					{
						DragonKillInfo dragonKillInfo = dragonKillInfos.get_Item(num11) as DragonKillInfo;
		...
		public bool SortByCoin
		{
			...
			set
			{
				...
				PlayerPrefs.SetInt(string.Format("Sgmae_Battle_SortByCoin_{0}", masterRoleInfo.playerUllUID), value ? 1 : 0);
				PlayerPrefs.Save();
		...
		public void RefreshVoiceStateIfNess()
		{
			...
			if (!statViewFormScript.IsHided())
			{
				...
				for (int j = 0; j < this.m_playerListCamp2.get_Count(); j++)
				{
					if (j >= this._heroList1.Length)
					{
						break;
					}
					Player curPlayer2 = this.m_playerListCamp2.get_Item(j);
					BattleStatView.HeroItem heroItem2 = this._heroList1[j];
					heroItem2.updateHeroVoiceState(curPlayer2);
				}
			}
		}
		...
		private void UpdateBattleState(CUIEvent evt = null)
		{
			...
			if (!statViewFormScript.IsHided())
			{
				...
				for (int j = 0; j < this.m_playerListCamp2.get_Count(); j++)
				{
					...
					Player curPlayer2 = this.m_playerListCamp2.get_Item(j);
					BattleStatView.HeroItem heroItem2 = this._heroList1[j];
					heroItem2.updateReviceCD(curPlayer2);
					heroItem2.updateTalentSkillCD(curPlayer2, this.m_statViewFormScript);
					if (this.m_battleHeroPropertyChange)
					{
						heroItem2.updateHeroValue(curPlayer2);
					}
				}
				if (this.m_battleHeroPropertyChange)
				{
		...
		private void SortHeroAndPlayer()
		{
			...
			if (this.m_heroListCamp1.get_Count() > 0 || this.m_heroListCamp2.get_Count() > 0 || this.m_playerListCamp1.get_Count() > 0 || this.m_playerListCamp2.get_Count() > 0)
			{
				forceUpdate = false;
			}
		...
		private void OnBttleMatchInfoTabChange(CUIEvent uiEvent)
		{
			...
			if (statViewFormScript != null)
			{
				...
				else
				{
					...
					{
						this.chartView.Show((this._defaultSelectIndex == 2) ? BattleStatView.ChartView.ChartType.ExpTrend : BattleStatView.ChartView.ChartType.MoneyTrend, (this._defaultSelectIndex == 2) ? watchForm.expSample : watchForm.moneySample, Singleton<CUIManager>.GetInstance().GetForm(BattleStatView.s_battleStateViewUIForm), watchForm.dragonKillInfos);
		...
		private void onSoulLvlChange(PoolObjHandle<ActorRoot> act, int curVal)
		{
			...
			for (int i = 0; i < array.Length; i++)
			{
				...
				{
					heroItem.level.set_text(curVal.ToString());
				}
		...
		private void UpdateListCamp(bool forceUpdate)
		{
			if (forceUpdate || !this.m_bListCampInited)
			{
				...
				{
					if (allPlayers.get_Item(i).PlayerCamp == COM_PLAYERCAMP.COM_PLAYERCAMP_1)
					{
						this.m_playerListCamp1.Add(allPlayers.get_Item(i));
					}
					else if (allPlayers.get_Item(i).PlayerCamp == COM_PLAYERCAMP.COM_PLAYERCAMP_2)
					{
		...
		public static void PaintLineEnd(int segIdx, Vector3 pos)
		{
			...
			if (segIdx > 0)
			{
				...
				WatchForm watchForm = Singleton<CBattleSystem>.GetInstance().WatchForm;
				if (segIdx <= watchForm.dragonKillInfos.get_Count())
				{
					DragonKillInfo dragonKillInfo = watchForm.dragonKillInfos.get_Item(segIdx - 1) as DragonKillInfo;
				...
				position.y = (flag ? rectTransform.position.y : rectTransform2.position.y);
