...
namespace Assets.Scripts.GameSystem
{
	public class CBattleEquipSystem
	{
		...
		public class CEquipBuyPrice
		{
			...
			public void AddSwappedPreEquipInfo(ushort equipID)
			{
				for (int i = 0; i < this.m_swappedPreEquipInfoCount; i++)
				{
					...
					{
						CBattleEquipSystem.CEquipBuyPrice.stSwappedPreEquipInfo[] swappedPreEquipInfos = this.m_swappedPreEquipInfos;
						int num = i;
						swappedPreEquipInfos[num].m_swappedAmount = swappedPreEquipInfos[num].m_swappedAmount + 1u;
						return;
					}
				}
		...
		public CBattleEquipSystem()
		{
			...
			{
				Dictionary<ushort, CBattleEquipSystem.CEquipBuyPrice> hostPlayerCachedEquipBuyPrice = this.m_hostPlayerCachedEquipBuyPrice;
				KeyValuePair<ushort, CEquipInfo> current = enumerator.get_Current();
				ushort key = current.get_Key();
				KeyValuePair<ushort, CEquipInfo> current2 = enumerator.get_Current();
				hostPlayerCachedEquipBuyPrice.Add(key, new CBattleEquipSystem.CEquipBuyPrice(current2.get_Value().m_resEquipInBattle.dwBuyPrice));
			}
		}
		...
		public bool IsEquipCanBought(ushort equipID, ref PoolObjHandle<ActorRoot> actor, ref CBattleEquipSystem.CEquipBuyPrice equipBuyPrice)
		{
			if (!this.m_isEnabled || equipID == 0 || false)
			{
				return false;
			}
			...
			if (requiredEquipIDs != null && requiredEquipIDs.Length > 0)
			{
				...
				{
					if (actor.handle.EquipComponent.HasEquip(requiredEquipIDs[i], 1u))
					{
						flag = true;
						break;
					}
			...
			return (long)actor.handle.ValueComponent.GetGoldCoinInBattle() >= (long)equipBuyPrice.m_buyPrice && actor.handle.EquipComponent.IsEquipCanAddedToGrid(equipID, ref equipBuyPrice);
		}
		...
		public bool IsHorizonEquipCanBought(ushort equipID, ref PoolObjHandle<ActorRoot> actor, ref int price)
		{
			...
			if (curLvelContext != null && curLvelContext.m_bEnableOrnamentSlot)
			{
				...
				if (dataByKey.dwActiveSkillID > 0u && (skillSlot == null || (ulong)dataByKey.dwActiveSkillID != (ulong)((long)skillSlot.SkillObj.SkillID)) && actor.handle.ValueComponent.GetGoldCoinInBattle() >= dataByKey.dwBuyPrice)
				{
					return true;
				}
			}
		...
		public ushort[] GetQuicklyBuyEquipList(ref PoolObjHandle<ActorRoot> actor)
		{
			...
			if (usageLevelEquipList != null)
			{
				...
				{
					if (equipBoughtHistory.ContainsKey(usageLevelEquipList.get_Item(i)))
					{
						flag = true;
						break;
					}
		...
		private void AddRecommendPreEquip(ushort equipId, bool bRootRecommend)
		{
			...
			if (this.m_equipList[0][(int)(dataByKey.bLevel - 1)].get_Count() >= 12)
			{
				return;
			}
		...
		private void RefreshHostPlayerCachedEquipBuyPrice()
		{
			...
			while (enumerator.MoveNext())
			{
				KeyValuePair<ushort, CEquipInfo> current = enumerator.get_Current();
				ushort key = current.get_Key();
				CBattleEquipSystem.CEquipBuyPrice cEquipBuyPrice = null;
				if (this.m_hostPlayerCachedEquipBuyPrice.TryGetValue(key, ref cEquipBuyPrice))
				{
					this.GetEquipBuyPrice(key, ref this.m_hostCtrlHero, ref cEquipBuyPrice);
				}
				else
				{
					cEquipBuyPrice = new CBattleEquipSystem.CEquipBuyPrice(0u);
					this.GetEquipBuyPrice(key, ref this.m_hostCtrlHero, ref cEquipBuyPrice);
					this.m_hostPlayerCachedEquipBuyPrice.Add(key, cEquipBuyPrice);
				}
			}
		}
		...
		private CBattleEquipSystem.CEquipBuyPrice GetHostPlayerCachedEquipBuyPrice(ushort equipID)
		{
			...
			if (this.m_hostPlayerCachedEquipBuyPrice.TryGetValue(equipID, ref result))
			{
				return result;
			}
			return null;
		...
		public void GetRelatedRecommondEquips(ushort equipID, bool isRootEquip, ref PoolObjHandle<ActorRoot> actor, ref CExistEquipInfoSet existEquipInfoSet, ref ListView<CEquipInfo> relatedRecommondEquips)
		{
			...
			if (!this.m_equipInfoDictionary.TryGetValue(equipID, ref cEquipInfo))
			{
				return;
			}
			if (!isRootEquip)
			{
				for (int i = 0; i < existEquipInfoSet.m_existEquipInfoCount; i++)
				{
					...
					{
						stExistEquipInfo[] existEquipInfos = existEquipInfoSet.m_existEquipInfos;
						int num = i;
						existEquipInfos[num].m_calculateAmount = existEquipInfos[num].m_calculateAmount - 1u;
						return;
					}
				}
			}
			...
			for (int i = 0; i < freeEquipBuyPrice.m_swappedPreEquipInfoCount; i++)
			{
				if (freeEquipBuyPrice.m_swappedPreEquipInfos[i].m_equipID > 0 && freeEquipBuyPrice.m_swappedPreEquipInfos[i].m_swappedAmount > 0u)
				{
					while (freeEquipBuyPrice.m_swappedPreEquipInfos[i].m_swappedAmount > 0u)
					{
						...
						CBattleEquipSystem.CEquipBuyPrice.stSwappedPreEquipInfo[] swappedPreEquipInfos = freeEquipBuyPrice.m_swappedPreEquipInfos;
						int num = i;
						swappedPreEquipInfos[num].m_swappedAmount = swappedPreEquipInfos[num].m_swappedAmount - 1u;
					}
				}
		...
		private CEquipInfo GetEquipInfo(ushort equipID)
		{
			...
			if (this.m_equipInfoDictionary.TryGetValue(equipID, ref result))
			{
				return result;
			}
			return null;
				...
				if (dataByKey.PreEquipID[i] > 0)
				{
					...
					for (int j = 0; j < existEquipInfoSet.m_existEquipInfoCount; j++)
					{
						if (existEquipInfoSet.m_existEquipInfos[j].m_equipID == dataByKey.PreEquipID[i] && existEquipInfoSet.m_existEquipInfos[j].m_calculateAmount > 0u)
						{
							...
							stExistEquipInfo[] existEquipInfos = existEquipInfoSet.m_existEquipInfos;
							int num2 = j;
							existEquipInfos[num2].m_calculateAmount = existEquipInfos[num2].m_calculateAmount - 1u;
			...
			for (int i = 0; i < this.m_hostPlayerQuicklyBuyEquipIDs.Length; i++)
			{
				...
				if (widget != null)
				{
					...
					if (equipInfo != null)
					{
						...
						component2.set_text(string.Format("<color=#ffa500>{0}</color>\n{1}", equipInfo.m_equipName, equipInfo.m_equipDesc));
						Text component3 = transform.Find("moneyText").GetComponent<Text>();
						component3.set_text(((uint)this.GetHostPlayerCachedEquipBuyPrice(equipInfo.m_equipID).m_buyPrice).ToString());
			...
			for (int i = 0; i < 12; i++)
			{
				...
				if (!(equipItem == null))
				{
					if (i < count)
					{
						...
						this.RefreshEquipItem(equipItem, list.get_Item(i), onlyRefreshPriceAndOwned);
					}
		...
		private void RefreshEquipItem(Transform equipItem, ushort equipID, bool onlyRefreshPriceAndOwned)
		{
			...
			if (resEquipInBattle != null)
			{
				...
				else
				{
					...
					component2.set_text(freeEquipBuyPrice.m_buyPrice.ToUintString());
				}
				component.set_color((flag || flag2) ? CUIUtility.s_Color_White : CUIUtility.s_Color_GrayShader);
				...
				if (!onlyRefreshPriceAndOwned)
				{
					...
					component3.set_text(equipInfo.m_equipName);
		...
		private void RefreshEquipBuyPanel(CEquipInfo equipInfo, Transform buyPanel)
		{
			...
			else
			{
				...
				component.set_text(freeEquipBuyPrice.m_buyPrice.ToString());
			}
		...
		private void RefreshEquipSalePanel(CEquipInfo equipInfo, int positionInBag, Transform salePanel)
		{
			...
			component.set_text(equipInfo.m_resEquipInBattle.dwSalePrice.ToString());
			stUIEventParams eventParams = default(stUIEventParams);
			...
			for (int i = 0; i < 12; i++)
			{
				...
				if (!(null == equipItem))
				{
					...
					if (level < 3)
					{
						...
						this.m_equipRelationPath.InitializeHorizontalLine(index2, i, CEquipLineSet.enHorizontalLineType.Left, transform2.gameObject);
					}
				}
			}
		}
		...
		private void RefreshRcmdEquipPlanPanel()
		{
			...
			if (widget != null)
			{
				widget.transform.Find("Text").GetComponent<Text>().set_text(this.m_hostCtrlHero.handle.EquipComponent.GetCurRcmdEquipPlanName());
			}
		}
		...
		public void OnBattleEquipBackEquipListSelectedChanged(CUIEvent uiEvent)
		{
			...
			if (this.m_selectedEquipOrigin != CBattleEquipSystem.enSelectedEquipOrigin.None && this.m_selectedEquipItems[(int)this.m_selectedEquipOrigin].m_equipInfo != null)
			{
				...
				num = ((list == null) ? 0 : list.get_Count());
			}
			...
			if (cUIListScript != null)
			{
				...
				{
					ushort num2 = list.get_Item(selectedIndex);
					...
					Transform equipItemTransform = (widget != null) ? widget.transform.Find("rootEquipItem") : null;
		...
		private void RefreshHostPlayerQuicklyBuyEquipPanel(bool hostCtrlHeroPermitedToBuyEquip)
		{
			...
			if (widget != null)
			{
				...
				if (component != null)
				{
					component.set_color(new Color((float)(hostCtrlHeroPermitedToBuyEquip ? 1 : 0), 1f, 1f, 1f));
				}
			}
		...
		private void OpenEquipTreePanel(CEquipInfo rootEquipInfo, bool selectRootItem)
		{
			...
			if (selectRootItem)
			{
				Transform equipItemTransform = (widget2 != null) ? widget2.transform.Find("rootEquipItem") : null;
		...
		private void RefreshBackEquipList(bool onlyRefreshPriceAndOwned)
		{
			...
			if (this.m_backEquipListScript != null)
			{
				...
				for (int i = 0; i < num; i++)
				{
					this.RefreshEquipTreeItem(this.m_backEquipListScript.GetElemenet(i).transform, list.get_Item(i), onlyRefreshPriceAndOwned);
				}
			}
		}
		...
		private void RefreshEquipTreeItem(Transform equipItemTransform, ushort equipID, bool onlyRefreshPriceAndOwned)
		{
			...
			Image image = (transform != null) ? transform.gameObject.GetComponent<Image>() : null;
			...
			if (image != null)
			{
				image.set_color((flag || flag2) ? CUIUtility.s_Color_White : CUIUtility.s_Color_GrayShader);
			}
		...
		private void SetEquipActiveSkillCdState(int iEquipSlotIndex, int icd)
		{
			...
			if (this.m_objEquipActiveTimerTxt[iEquipSlotIndex] != null)
			{
				if (icd > 0)
				{
					...
					if (component != null)
					{
						component.set_text(SimpleNumericString.GetNumeric(Mathf.CeilToInt((float)(icd / 1000)) + 1));
					}
