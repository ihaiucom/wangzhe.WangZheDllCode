...
namespace Assets.Scripts.GameSystem
{
	public class PointsExchangeActivity : Activity
	{
		...
		public uint PerConsume
		{
			get
			{
				return (this.PointsConfig == null) ? 1u : this.PointsConfig.dwPointGetParam;
			}
		}
		...
		public uint PerPoint
		{
			get
			{
				return (this.PointsConfig == null) ? 1u : this.PointsConfig.dwPointGetCnt;
			}
		}
		...
		public PointsExchangeActivity(ActivitySys mgr, ResWealPointExchange config) : base(mgr, config.stCommon)
		{
			...
			{
				PointsExchangePhase ap = new PointsExchangePhase(this, num, config.astExchangeInfo[(int)((uint)((UIntPtr)num))]);
				base.AddPhase(ap);
			}
		}
		...
		{
			uint num = 0u;
			if (this._exchangeCount.TryGetValue(index, ref num))
			{
		...
		public void ResetExchangeCount()
		{
			...
			{
				KeyValuePair<int, uint> current = enumerator.get_Current();
		...
		{
			uint num = 0u;
			return this._exchangeCount.TryGetValue(index, ref num) ? num : 0u;
		}
