...
namespace Assets.Scripts.GameSystem
{
	public class CUICommonSystem : Singleton<CUICommonSystem>
	{
		...
		public override void Init()
		{
			...
			int num = 0;
			int.TryParse(Singleton<CTextManager>.instance.GetText("LobbyFadeAniDisableFlag"), ref num);
		...
		private void OnSendMsgAlertOpen(CUIEvent uiEvent)
		{
			...
			if (!string.IsNullOrEmpty(uiEvent.m_eventParams.tagStr))
			{
				...
				component2.set_text(uiEvent.m_eventParams.tagStr);
		...
		public void OpenUseableTips(CUseable iconUseable, float srcX, float srcY, enUseableTipsPos pos = enUseableTipsPos.enTop)
		{
			...
			component5.set_text(iconUseable.m_description);
			...
			if (iconUseable.m_type == COM_ITEM_TYPE.COM_OBJTYPE_ITEMPROP)
			{
				...
				{
					component3.set_text("当前拥有 <color=#a52a2aff>" + useableStackCount + "</color>");
					component3.gameObject.CustomSetActive(true);
				}
				component4.set_text(cItem.m_coinSale.ToString());
				component5.set_text(CUIUtility.StringReplace(component5.get_text(), new string[]
				{
			...
			else if (iconUseable.m_type == COM_ITEM_TYPE.COM_OBJTYPE_ITEMEQUIP)
			{
				...
				component3.set_text("当前拥有 <color=#a52a2aff>" + useableStackCount2 + "</color>");
				component3.gameObject.CustomSetActive(true);
				component4.set_text(iconUseable.m_coinSale.ToString());
				component5.set_text(CUIUtility.StringReplace(component5.get_text(), new string[]
				{
			...
			else if (iconUseable.m_type == COM_ITEM_TYPE.COM_OBJTYPE_ITEMSYMBOL)
			{
				...
				component3.set_text("当前拥有 <color=#a52a2aff>" + useableStackCount3 + "</color>");
				component3.gameObject.CustomSetActive(true);
				component4.set_text(iconUseable.m_coinSale.ToString());
				component5.set_text(CUIUtility.StringReplace(component5.get_text(), new string[]
				{
			...
			if (cVirtualItem != null)
			{
				...
				if (virtualType == enVirtualItemType.enExp)
				{
					...
					component3.set_text(string.Concat(new object[]
					{
						"当前经验",
			...
			rectTransform.sizeDelta = new Vector2(rectTransform.sizeDelta.x, height + component5.get_preferredHeight());
			float num = srcX;
			float num2 = srcY;
			float num3 = 60f;
		...
		{
			CUICommonSystem.OpenCommonTips(uiEvent.m_pointerEventData.get_pressPosition().x, uiEvent.m_pointerEventData.get_pressPosition().y, uiEvent.m_eventParams.tagStr, (enUseableTipsPos)uiEvent.m_eventParams.tag);
		}
		...
		private void OnGiftBag_ShowDetail(CUIEvent uiEvent)
		{
			...
			if (iconUseable.m_type == COM_ITEM_TYPE.COM_OBJTYPE_ITEMPROP)
			{
				...
				{
					componetInChild.set_text(cItem.m_description);
				}
		...
		private static void OpenCommonTips(float srcX, float srcY, string strContent, enUseableTipsPos pos = enUseableTipsPos.enTop)
		{
			...
			RectTransform rectTransform = (RectTransform)gameObject.transform;
			float height = ((RectTransform)gameObject2.transform).rect.height;
			rectTransform.sizeDelta = new Vector2(rectTransform.sizeDelta.x, height + component.get_preferredHeight());
			float num = srcX;
			float num2 = srcY;
			float num3 = 60f;
			float num4 = cUIFormScript.ChangeFormValueToScreen(rectTransform.rect.width);
		...
		public static void SetItemCell(CUIFormScript formScript, GameObject itemCell, CUseable itemUseable, bool isHaveClickEvent = true, bool displayAll = false, bool cntForceDisplay = false, bool isClickToShowDetail = false)
		{
			...
			if (transform != null)
			{
				...
				{
					component2.set_text(itemUseable.m_stackCount / 10000 + "万");
				}
				...
				if (itemUseable.m_type == COM_ITEM_TYPE.COM_OBJTYPE_ITEMSYMBOL)
				{
					...
					{
						component2.set_text(itemUseable.GetSalableCount().ToString());
					}
				}
			}
			...
			if (transform3 != null)
			{
				...
				if (component4 != null)
				{
					component4.set_text(itemUseable.m_name);
				}
			}
			...
			else
			{
				...
				for (int j = 0; j < 5; j++)
				{
					...
					if (skillPropertyParams[j].name != null)
					{
						...
						componetInChild.set_text(skillPropertyParams[j].name);
						for (int k = 1; k <= 6; k++)
						{
		...
		public static void AppendMultipleText(Text target, int multiple)
		{
			if (multiple > 0)
			{
				target.set_color((multiple > 1) ? Color.green : Color.yellow);
			}
		}
		...
		public static void SetProgressBarData(GameObject progressBar, int curVal, int totalVal, bool bShowTxt = false)
		{
			...
			if (bShowTxt)
			{
				component2.set_text(curVal.ToString() + "/" + totalVal.ToString());
			}
			else
			{
				component2.set_text(string.Empty);
		...
		public static void SetHeroItemImage(CUIFormScript formScript, GameObject item, string imgPath, enHeroHeadType headType = enHeroHeadType.enBust, bool bGray = false, bool isShowSpecMatrial = false)
		{
			...
			if (transform != null)
			{
				...
				if (component != null)
				{
					component.set_color(bGray ? CUIUtility.s_Color_GrayShader : CUIUtility.s_Color_White);
		...
		public static void SetHeroProficiencyBgImage(CUIFormScript formScript, GameObject proficiencyBg, int proficiencyLV, bool bLoading)
		{
			...
			if (component != null)
			{
				...
				{
					text = ((!bLoading) ? "HeroProficiency_Bg_Level_1" : string.Format("{0}{1}", "HeroProficiency_Bg_Level_1", "_loading"));
					component.SetSprite(string.Format("{0}{1}", CUIUtility.s_Sprite_Dynamic_Proficiency_Dir, text), formScript, true, false, false, false);
				}
				else
				{
					text = ((!bLoading) ? "HeroProficiency_Bg_Level_Max" : string.Format("{0}{1}", "HeroProficiency_Bg_Level_Max", "_loading"));
					component.SetSprite(string.Format("{0}{1}", CUIUtility.s_Sprite_Dynamic_Proficiency_Dir, text), formScript, true, false, false, false);
				}
			}
		}
		...
		{
			return (roleID == 1) ? CUICommonSystem.s_womanPath : CUICommonSystem.s_manPath;
		}
		...
		public static void SetButtonEnable(Button btn, bool isEnable, bool isEventEnable, bool bChangeTextColor = true)
		{
			...
			if (bChangeTextColor)
			{
				...
				{
					componentInChildren.set_color(isEnable ? CUIUtility.s_Text_Color_White : CUIUtility.s_Text_Color_Disable);
				}
			}
		}
		...
		public static void InitMenuPanel(GameObject listObj, string[] titleList, int selectIndex, bool isDispatchEvent = true)
		{
			if (listObj != null)
			{
				...
				for (int i = 0; i < component.m_elementAmount; i++)
				{
					...
					component2.set_text(titleList[i]);
				}
		...
		{
			CUIAnimatorScript cUIAnimatorScript = (target != null) ? target.GetComponent<CUIAnimatorScript>() : null;
		...
		public static void SetListProp(GameObject listObj, ref int[] propArr, ref int[] propPctArr)
		{
			...
			for (int j = 0; j < num; j++)
			{
				if (propArr[j] > 0 || propPctArr[j] > 0)
				{
					...
					Text text = (elemenet != null) ? elemenet.gameObject.transform.Find("titleText").GetComponent<Text>() : null;
					Text text2 = (elemenet != null) ? elemenet.gameObject.transform.Find("valueText").GetComponent<Text>() : null;
					if (text != null)
					{
					...
					if (text2 != null)
					{
						if (propArr[j] > 0)
						{
							text2.set_text(string.Format("+{0}", propArr[j]));
						}
		...
		public static void SetListProp(GameObject listObj, ref int[] propArr, ref int[] propPctArr, ref int[] propValAddArr, ref int[] propPctAddArr, bool bShowAdd = true)
		{
			...
			for (int j = 0; j < num; j++)
			{
				if (propArr[j] > 0 || propPctArr[j] > 0 || propValAddArr[j] / 10000 > 0 || propPctAddArr[j] > 0)
				{
					CUIListElementScript elemenet = component.GetElemenet(num2);
					if (elemenet != null)
					{
						...
						{
							if (propArr[j] > 0 || propValAddArr[j] > 0)
							{
								component3.set_text(string.Format("{0}", propArr[j]));
								component4.set_text(string.Format("{0}", propValAddArr[j]));
							}
							else if (propPctArr[j] > 0 || propPctAddArr[j] > 0)
							{
								component3.set_text(CUICommonSystem.GetValuePercent(propPctArr[j]));
								component4.set_text(string.Format("{0}", CUICommonSystem.GetValuePercent(propPctAddArr[j])));
							}
						}
					}
					num2++;
		...
		private static void OnLoadNewHeroOrSkin3DModel(CUIEvent uiEvent)
		{
			...
			CUI3DImageScript cUI3DImageScript = (widget != null) ? widget.GetComponent<CUI3DImageScript>() : null;
			...
			GameObject gameObject = (cUI3DImageScript != null) ? cUI3DImageScript.AddGameObject(CUICommonSystem.s_last3DModelPath, false, false) : null;
		...
		private static void ProcessShowNewHeroOrSkin()
		{
				...
				if (rewardType == COM_REWARDS_TYPE.COM_REWARDS_TYPE_SKIN)
				{
					...
					if (widget2 != null)
					{
						if (newHeroOrSkinParams.experienceDays > 0)
						{
							...
							widget5.GetComponent<Text>().set_text(Singleton<CTextManager>.GetInstance().GetText("ExpCard_NewHeroOrSkin_ExpTime", new string[]
							{
					...
					if (newHeroOrSkinParams.convertCoin > 0u && widget2 != null)
					{
						...
						{
							component.set_text(string.Format(Singleton<CTextManager>.GetInstance().GetText("SkinConvertItem"), newHeroOrSkinParams.convertCoin));
						}
					}
					else
					{
			...
			else
			{
				...
				if (newHeroOrSkinParams.convertCoin > 0u && widget != null)
				{
					...
					{
						component3.set_text(string.Format(Singleton<CTextManager>.GetInstance().GetText("HeroConvertItem"), newHeroOrSkinParams.convertCoin));
					}
				}
				else
				{
				...
				if (widget6 != null)
				{
					...
					component4.set_text(heroData.heroName);
				}
				MonoSingleton<ShareSys>.instance.m_ShareInfo.heroId = newHeroOrSkinParams.heroId;
				MonoSingleton<ShareSys>.instance.m_ShareInfo.skinId = newHeroOrSkinParams.skinId;
				MonoSingleton<ShareSys>.instance.m_ShareInfo.rewardType = newHeroOrSkinParams.rewardType;
			...
			for (int i = 0; i < curUseableCount; i++)
			{
				...
				if (gameObject4 != null)
				{
					...
					if (cSymbolItem != null)
					{
						...
						componetInChild2.set_text(cSymbolItem.m_name);
						componetInChild3.set_text(CSymbolSystem.GetSymbolAttString(cSymbolItem, true));
					}
				}
			}
			return true;
		...
		public static void SetGetInfoToList(CUIFormScript formScript, CUIListScript list, CUseable itemUseable)
		{
			...
			if (CSysDynamicBlock.bLobbyEntryBlocked)
			{
				...
				for (int i = 0; i < astSrcInfo.Length; i++)
				{
					...
					listView.Add((resDT_ItemSrc_Info.bType == 2 || resDT_ItemSrc_Info.bType == 3) ? null : resDT_ItemSrc_Info);
				}
			...
			if (list != null)
			{
				...
				for (int k = 0; k < num; k++)
				{
					...
					if (bType == 1)
					{
						...
						component.set_text(Singleton<CTextManager>.instance.GetText("PVE_Level_Chapter_Number", new string[]
						{
							dataByKey.iChapterId.ToString()
						}));
						component2.set_text(Utility.UTF8Convert(dataByKey.szName));
						component3.gameObject.CustomSetActive(false);
						uint num2 = flag ? Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo().pveLevelDetail[(int)(dataByKey.bLevelDifficulty - 1)].ChapterDetailList[dataByKey.iChapterId - 1].LevelDetailList[(int)(dataByKey.bLevelNo - 1)].PlayNum : 0u;
						uint dwChallengeNum = dataByKey.dwChallengeNum;
						if (dataByKey.bLevelDifficulty == 2)
						{
							component3.set_text(Singleton<CTextManager>.instance.GetText("Hero_Info_Text4", new string[]
							{
					...
					else if (bType == 2)
					{
						...
						gameObject2.CustomSetActive(true);
						gameObject2.GetComponentInChildren<Text>().set_text(Singleton<CTextManager>.instance.GetText(string.Format("Shop_Type_{0}", dwID)));
					...
					else if (bType == 3)
					{
						...
						obj.CustomSetActive(false);
						gameObject2.CustomSetActive(true);
						gameObject2.GetComponentInChildren<Text>().set_text(Singleton<CTextManager>.instance.GetText("Mall"));
					}
		...
		public static void RefreshExperienceHeroLeftTime(GameObject txtHeroLeftTimeGo, GameObject timerGo, uint heroId)
		{
			...
			{
				txtHeroLeftTimeGo.GetComponent<Text>().set_text(Singleton<CTextManager>.GetInstance().GetText("ExpCard_Hero_ExpTime") + "<color=#ffb900>" + timeSpanFormatString + "</color>");
			}
			else
			{
		...
		public static void RefreshExperienceSkinLeftTime(GameObject txtSkinLeftTimeGo, GameObject timerGo, uint heroId, uint skinId, string tipText = null, bool isTipTextDisplayedWithTwoLines = false)
		{
			...
			if (masterRoleInfo.IsValidExperienceSkin(heroId, skinId))
			{
				...
				if (Utility.IsOverOneDay(experienceSkinLeftTime))
				{
					...
					txtSkinLeftTimeGo.GetComponent<Text>().set_text(string.Concat(new string[]
					{
						tipText,
						isTipTextDisplayedWithTwoLines ? "\n" : string.Empty,
						"<color=#ffb900>",
		...
		public static bool JumpForm(RES_GAME_ENTRANCE_TYPE entranceType, int targetId = 0, int targetId2 = 0, List<uint> tagList = null)
		{
			...
			{
				CUIFormScript form2 = Singleton<CUIManager>.GetInstance().GetForm(CMatchingSystem.PATH_MATCHING_ENTRY);
				if (form2 == null)
				{
					Singleton<CUIEventManager>.GetInstance().DispatchUIEvent(enUIEventID.Matching_OpenEntry);
					form2 = Singleton<CUIManager>.GetInstance().GetForm(CMatchingSystem.PATH_MATCHING_ENTRY);
				}
				cUIEvent.m_srcFormScript = form2;
				cUIEvent.m_eventID = enUIEventID.Matching_BtnGroup_Click;
			...
			case RES_GAME_ENTRANCE_TYPE.RES_GAME_ENTRANCE_PVP_HISTORY:
				Singleton<CPlayerPvpHistoryController>.instance.OpenDetailPvpHistory(tagList.get_Item(0), tagList.get_Item(1));
				break;
		...
		private static int GetSecUtilOpenOfDay(DateTime dateTimeNow, ResActTime[] ActTime)
		{
			...
			if (ActTime != null)
			{
				...
				for (int i = 0; i < ActTime.Length; i++)
				{
					...
					num = ((num == 19700101240000uL) ? 19700101235959uL : num);
		...
		public static void GetTimeUtilOpen(ResRewardMatchTimeInfo inMatchTimeInfo, out uint utilOpenSec, out int utilOpenDay)
		{
			...
			if (inMatchTimeInfo != null)
			{
				...
				if (bCycleType != RES_CYCLETIME_TYPE.RES_CYCLETIME_DAY)
				{
					if (bCycleType == RES_CYCLETIME_TYPE.RES_CYCLETIME_WEEK)
					{
						...
						while (i < 0)
						{
							...
							{
								if ((ulong)CUICommonSystem.GetDayOfWeek(inMatchTimeInfo.CycleParm[j]) == (ulong)dateTime.get_DayOfWeek())
								{
				...
				else
				{
					...
					while (k < 0)
					{
						k = CUICommonSystem.GetSecUtilOpenOfDay(dateTime, inMatchTimeInfo.astActTime);
						if (k < 0)
						{
		...
		public static bool IsInSignUpTime(RES_BATTLE_MAP_TYPE mapType, uint mapId)
		{
			...
			if (resRewardMatchTimeInfo.bCycleType == 2)
			{
				...
				{
					if ((ulong)CUICommonSystem.GetDayOfWeek(resRewardMatchTimeInfo.SignUpCycleParm[i]) == (ulong)dateTime.get_DayOfWeek())
					{
						DateTime dateTime2 = new DateTime(dateTime.get_Year(), dateTime.get_Month(), dateTime.get_Day(), 0, 0, 0);
		...
		public static uint GetGuildMatchNextEndTime()
		{
			...
			if (resRewardMatchTimeInfo.bCycleType == 2)
			{
				if (resRewardMatchTimeInfo.bCycleParmNum > 0)
				{
					...
					int dayOfWeek = baseDate.get_DayOfWeek();
					for (int k = dayOfWeek; k <= dayOfWeek + array.Length; k++)
					{
						...
						if (num >= 0 && num < resRewardMatchTimeInfo.astActTime.Length)
						{
							...
							if ((long)(baseDate.get_Hour() * 3600 + baseDate.get_Minute() * 60 + baseDate.get_Second()) < (long)((ulong)secondsFromHourMinuteSecond))
							{
		...
		{
			DateTime dateTime = new DateTime(baseDate.get_Year(), baseDate.get_Month(), baseDate.get_Day(), 0, 0, 0);
