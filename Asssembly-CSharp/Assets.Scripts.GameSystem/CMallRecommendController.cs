	...
	public class CMallRecommendController : Singleton<CMallRecommendController>
	{
		...
		private bool SetCurrentLotteryCtrl(uint drawID = 0u)
		{
			...
			while (enumerator.MoveNext())
			{
				...
				ResRandDrawInfo value = current.get_Value();
		...
		private bool SetCurrentPoolData()
		{
			...
			for (byte b2 = 1; b2 < b; b2 += 1)
			{
				...
				while (enumerator.MoveNext())
				{
					...
					ResRewardPoolInfo value = current.get_Value();
		...
		private bool SetCurrentRecommendData()
		{
			...
			while (enumerator.MoveNext())
			{
				...
				ResSaleRecommend value = current.get_Value();
		...
		private void InitRecommendTab()
		{
			...
			while ((int)b < array.Length)
			{
				...
				CMallRecommendController.Tab tab2 = tab + 1;
				if (tab2 != CMallRecommendController.Tab.Skin)
				{
					if (tab2 == (CMallRecommendController.Tab)2)
					{
						if ((this.m_HasHeroAndSkin & 2) > 0)
						{
				...
				else if ((this.m_HasHeroAndSkin & 1) > 0)
				{
					list.Add(array[(int)b]);
				}
				b += 1;
			}
			string[] array2 = new string[list.get_Count()];
			byte b2 = 0;
			while ((int)b2 < array2.Length)
			{
				CMallRecommendController.Tab tab3 = list.get_Item((int)b2);
				if (tab3 != CMallRecommendController.Tab.Hero)
				{
					if (tab3 == CMallRecommendController.Tab.Skin)
					{
		...
		private void InitListTab()
		{
			...
			while ((int)b < array.Length)
			{
				...
				CMallRecommendController.ListTab listTab2 = listTab + 1;
				if (listTab2 != CMallRecommendController.ListTab.Skin)
				{
					if (listTab2 == (CMallRecommendController.ListTab)2)
					{
						if ((this.m_HasHeroAndSkin & 2) > 0)
						{
							list.Add(array[(int)b]);
			...
			while ((int)b2 < array2.Length)
			{
				CMallRecommendController.ListTab listTab3 = list.get_Item((int)b2);
				if (listTab3 != CMallRecommendController.ListTab.Hero)
				{
					if (listTab3 == CMallRecommendController.ListTab.Skin)
					{
						array2[(int)b2] = Singleton<CTextManager>.GetInstance().GetText("Mall_NewSkin");
					}
				}
		...
		private void UpdateExchangeCouponsInfo(CUIFormScript form, Transform container, enExchangeType exchangeType)
		{
			...
			componetInChild2.set_text(CMallRecommendController.GetCurrencyValueFromRoleInfo(masterRoleInfo, exchangeType).ToString());
		}
		...
		private void RefreshSkinBaseInfo(ResHeroSkin skinCfg)
		{
			...
			if (component != null)
			{
				component.set_text(string.Format("{0} {1}", heroData.heroName, StringHelper.UTF8BytesToString(ref skinCfg.szSkinName)));
			}
		}
		...
		private void RefreshSkillPanel(ResHeroCfgInfo heroCfg)
		{
			...
			for (int i = 0; i < skillArr.Length - 1; i++)
			{
				...
				if (!(gameObject2 == null))
				{
					...
					eventParams.skillTipParam.skillEnergyCost = ((i == 0 || heroCfg.dwEnergyType == 6u) ? string.Empty : Singleton<CTextManager>.instance.GetText(EnergyCommon.GetEnergyShowText(heroCfg.dwEnergyType, EnergyShowType.CostValue), new string[]
					{
		...
		private void RefreshHeroPricePnl(ResHeroCfgInfo heroCfg)
		{
			...
			if (gameObject4 != null)
			{
				...
				if (resHeroShop != null)
				{
					uint num = (resHeroShop.bIsBuyItem > 0) ? resHeroShop.dwBuyItemCnt : 0u;
		...
		private void RefreshSkinPricePnl(ResHeroSkin skinCfg)
		{
			...
			if (masterRoleInfo.IsCanBuySkinButNotHaveHero(skinCfg.dwHeroID, skinCfg.dwSkinID))
			{
				...
				if (gameObject5 != null)
				{
					...
					componetInChild.set_text(Singleton<CTextManager>.GetInstance().GetText("Mall_Skin_State_Buy_hero"));
					stUIEventParams eventParams = default(stUIEventParams);
			...
			if (gameObject4 != null)
			{
				...
				{
					uint num = (resHeroSkinShop.bIsBuyItem > 0) ? resHeroSkinShop.dwBuyItemCnt : 0u;
					Transform transform2 = gameObject4.transform.Find("BuyButton");
					stUIEventParams stUIEventParams2 = default(stUIEventParams);
		...
		public void SetExchangeButton(CUIFormScript formScript, RectTransform buttonTransform, enExchangeType exchangeType, uint exchangeValue, enUIEventID eventID, ref stUIEventParams eventParams)
		{
			...
			if (transform2 != null)
			{
				...
				if (component2 != null)
				{
					component2.set_text(exchangeValue.ToString());
				}
			}
		...
		public string GetExchangeTypeIconPath(enExchangeType type)
		{
			...
			else
			{
				...
				CUseable cUseable2 = CUseableManager.CreateUseable(COM_ITEM_TYPE.COM_OBJTYPE_ITEMPROP, dwConfValue2, 0);
				if (cUseable2 != null)
				{
		...
		private void OnRecommendListHeroEnable(CUIEvent uiEvent)
		{
			...
			if (!this.m_CurrentRecommendProductMap.TryGetValue(doubleKey, ref key))
			{
			...
			component.set_text(StringHelper.UTF8BytesToString(ref resHeroCfgInfo.szName));
			...
			else
			{
				...
				if (resHeroShop != null)
				{
					componetInChild2.set_text(resHeroShop.dwBuyItemCnt.ToString());
				}
				transform3.gameObject.CustomSetActive(true);
				component4.set_text(Singleton<CTextManager>.GetInstance().GetText("Exchange_Btn"));
		...
		private void OnRecommendListSkinEnable(CUIEvent uiEvent)
		{
			...
			component2.set_text(resHeroSkin.szSkinName);
			...
			else
			{
				...
				if (resHeroSkinShop != null)
				{
					componetInChild2.set_text(resHeroSkinShop.dwBuyItemCnt.ToString());
				}
				component6.enabled = true;
				transform4.gameObject.CustomSetActive(true);
				component5.set_text(Singleton<CTextManager>.GetInstance().GetText("Exchange_Btn"));
				component4.enabled = true;
				stUIEventParams eventParams2 = default(stUIEventParams);
				eventParams2.tag = 2;
				if (resHeroSkinShop != null)
		...
		private void OnRecommendBuyConfirm(CUIEvent uiEvent)
		{
			...
			if (curTab != CMallRecommendController.Tab.Hero)
			{
				if (curTab == CMallRecommendController.Tab.Skin)
				{
					...
					if (!this.m_CurrentRecommendProductMap.TryGetValue(doubleKey, ref recommendProductID))
					{
						return;
					}
				}
			...
			{
				long doubleKey2 = GameDataMgr.GetDoubleKey(4u, uiEvent.m_eventParams.heroSkinParam.heroId);
				if (!this.m_CurrentRecommendProductMap.TryGetValue(doubleKey2, ref recommendProductID))
				{
					return;
				}
			}
		...
		private void HeroSelect_Skill_Down(CUIEvent uiEvent)
		{
			...
			componetInChild.set_text(uiEvent.m_eventParams.skillTipParam.skillName);
			componetInChild2.set_text(uiEvent.m_eventParams.skillTipParam.strTipText);
			componetInChild3.set_text(uiEvent.m_eventParams.skillTipParam.skillCoolDown);
			componetInChild4.set_text(uiEvent.m_eventParams.skillTipParam.skillEnergyCost);
			...
			for (int i = 1; i <= 2; i++)
			{
				...
				if (!(gameObject2 == null))
				{
					if (i <= skillEffect.Length && skillEffect[i - 1] != 0)
					{
						...
						gameObject2.transform.Find("Text").GetComponent<Text>().set_text(CSkillData.GetEffectDesc((SkillEffectType)skillEffect[i - 1]));
					}
					else
					{
						gameObject2.CustomSetActive(false);
