	...
	public class CSymbolWearController
	{
		...
		private void SetSymbolItem(CUIFormScript formScript, GameObject item, int i, ulong objId)
		{
			...
			case enSymbolWearState.WillOpen:
				component2.set_text("Lv." + tag.ToString());
		...
		private void RefreshSymbolPage()
		{
			...
			if (transform3 != null)
			{
				...
				if (component != null)
				{
					...
					component2.set_text(masterRoleInfo.m_symbolInfo.GetSymbolPageMaxLvl(this.m_symbolPageIndex).ToString());
				}
			}
		}
		...
		public void OpenSymbolPageForm(enSymbolPageOpenSrc symbolPageOpenSrc, int usedPageIndex)
		{
			...
			if (cUIFormScript != null)
			{
				...
				int elementAmount = ((long)nextFreePageLevel > (long)((ulong)masterRoleInfo.PvpLevel)) ? (pageCountWithAllCanBuy + 1) : pageCountWithAllCanBuy;
		...
		private void OnSymbolPageItemSelect(CUIEvent uiEvent)
		{
			...
			if (tag < masterRoleInfo.m_symbolInfo.m_pageCount)
			{
				...
				component.set_text(masterRoleInfo.m_symbolInfo.GetSymbolPageName(this.m_symbolPageIndex));
		...
		private void OnChangeSymbolPageName(CUIEvent uiEvent)
		{
			...
			if (form != null)
			{
				...
				if (transform != null && masterRoleInfo != null)
				{
					...
					transform.GetComponent<InputField>().set_text(symbolPageName);
				}
			}
		}
		...
		private void ShowSymbolView(CUIEvent uiEvent)
		{
			...
			if (component != null)
			{
				...
				component2.set_text(symbol.m_name);
		...
		public void SetSymbolListItem(CUIFormScript formScript, GameObject itemObj, CSymbolItem item)
		{
			...
			component2.set_text(item.m_name);
			Text component3 = itemObj.transform.Find("SymbolDesc").GetComponent<Text>();
			component3.set_text(CSymbolSystem.GetSymbolAttString(item, true));
			...
			component4.set_text(string.Format("x{0}", num));
			CRoleInfo masterRoleInfo = Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo();
		...
		private void RefreshSymbolBag(bool bNoWeatItem)
		{
			...
			if (transform2 != null)
			{
				transform2.GetComponent<Text>().set_text(bNoWeatItem ? Singleton<CTextManager>.GetInstance().GetText("Symbol_Title_NoWear") : Singleton<CTextManager>.GetInstance().GetText("Symbol_Title_SymbolBagList"));
			}
		...
		{
			string text = bWear ? Singleton<CTextManager>.GetInstance().GetText("Symbol_WearTip") : Singleton<CTextManager>.GetInstance().GetText("Symbol_TakeOffTip");
