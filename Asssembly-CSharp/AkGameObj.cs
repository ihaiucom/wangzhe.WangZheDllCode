...
public class AkGameObj : MonoBehaviour
{
	...
	private void UpdateAuxSend()
	{
		...
		{
			this.m_envData.auxSendValues = new AkAuxSendArray((uint)((this.m_envData.activeEnvironments.Count < AkEnvironment.MAX_NB_ENVIRONMENTS) ? this.m_envData.activeEnvironments.Count : AkEnvironment.MAX_NB_ENVIRONMENTS));
		}
		else
		{
		...
		if ((ulong)this.m_envData.auxSendValues.m_Count < (ulong)((long)AkEnvironment.MAX_NB_ENVIRONMENTS) && (ulong)this.m_envData.auxSendValues.m_Count < (ulong)((long)this.m_envData.activeEnvironments.Count))
		{
			...
			for (int k = 0; k < num; k++)
			{
				if (!this.m_envData.auxSendValues.Contains(listView[k].GetAuxBusID()) && (!listView[k].isDefault || k == 0))
				{
