...
namespace AGE
{
	...
	public class TriggerParticlePerioidc : TriggerParticle
	{
		...
		private GameObject InstParObj(string prefabName, Action _action, bool bCheckParLife)
		{
			...
			{
				Transform transform4 = SubObject.FindSubObject(gameObject2, this.bindPointName).transform;
				if (transform4 != null)
				{
		...
		public override void Leave(Action _action, Track _track)
		{
			...
			if (this.FinalEffectName.get_Length() > 0)
			{
			...
			{
				this.DestroyParObj(_action, enumerator.get_Current());
			}
			this.NonAutoDestructParList.Clear();
		}
		...
		public override void Process(Action _action, Track _track, int _localTime)
		{
			...
			{
				this.intervalTimer = 0;
				if (this.PeriodicEffectName.get_Length() > 0)
				{
