	...
	public class RecastGraph : NavGraph, IUpdatableGraph, IRaycastableGraph, INavmesh, INavmeshHolder
	{
				...
				for (int j = 0; j < this.nodes.Length; j++)
				{
					...
					if (triangleMeshNode3.connections != null)
					{
						...
						for (int k = 0; k < triangleMeshNode3.connections.Length; k++)
						{
							...
							dictionary.TryGetValue(nodeIndex, out triangleMeshNode5);
		...
		public void GetRecastMeshObjs(Bounds bounds, List<ExtraMesh> buffer)
		{
			...
			for (int i = 0; i < list.Count; i++)
			{
				...
				if (meshFilter != null && renderer != null)
				{
					...
					{
						item.vertices = dictionaryObjectView[sharedMesh];
						item.triangles = dictionaryObjectView2[sharedMesh];
					}
					else
					{
						item.vertices = sharedMesh.vertices;
						item.triangles = sharedMesh.triangles;
						dictionaryObjectView[sharedMesh] = item.vertices;
						dictionaryObjectView2[sharedMesh] = item.triangles;
					}
		...
		private void CreateNodeConnections(TriangleMeshNode[] nodes)
		{
			...
			for (int k = 0; k < nodes.Length; k++)
			{
				...
				for (int l = 0; l < vertexCount2; l++)
				{
					...
					int num;
					if (dictionary.TryGetValue(new VInt2(vertexIndex2, vertexIndex), out num))
					{
		...
		private void CollectColliderMeshes(Bounds bounds, List<ExtraMesh> extraMeshes)
		{
			...
			for (int i = 0; i < array.Length; i++)
			{
				...
				if ((1 << collider.gameObject.layer & this.mask) != 0 && collider.enabled && !collider.isTrigger && collider.bounds.Intersects(bounds))
				{
