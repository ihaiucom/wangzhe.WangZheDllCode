	...
	public class Agent : IAgent
	{
		...
		static Agent()
		{
			...
			for (int j = 0; j < 6; j++)
			{
				VFactor vFactor;
				VFactor vFactor2;
				IntMath.sincos(out vFactor, out vFactor2, (long)((2 * j + 1) * 31416), 60000L);
				vFactor += inverse;
				Agent.adpSamp_Steps2_Den[j] = vFactor2.den * vFactor.den * Agent.adpSamp_Steps2_InnerScale.den;
				Agent.adpSamp_Steps2_Nom_rw[j] = vFactor2.nom * vFactor.den;
				Agent.adpSamp_Steps2_Nom_fw[j] = vFactor.nom * vFactor2.den;
			}
			...
			for (int k = 0; k < 6; k++)
			{
				VFactor vFactor;
				VFactor vFactor2;
				IntMath.sincos(out vFactor, out vFactor2, (long)((2 * k + 1) * 31416), 60000L);
				vFactor.den *= 5L;
				vFactor2.den *= 5L;
				Agent.adpSamp_Steps3_Sin[k] = vFactor;
				Agent.adpSamp_Steps3_Cos[k] = vFactor2;
			}
		}
		...
		public void InsertObstacleNeighbour(ObstacleVertex ob1, long rangeSq)
		{
			...
			if (num < rangeSq)
			{
				...
				while (num2 != 0 && (float)num < this.obstacleDists[num2 - 1])
				{
				...
				this.obstacleDists[num2] = (float)num;
			}
		}
		...
		private VInt2 Trace(Agent.VO[] vos, int voCount, VInt2 p, VFactor cutoff, out float score)
		{
			...
			for (int i = 0; i < 50; i++)
			{
				...
				vFactor = ((!(vFactor > vFactor3)) ? vFactor3 : vFactor);
		...
		public void InsertNeighbour(Agent agent, long distSq)
		{
			if (this.neighbours.Count > 0)
			{
				if (this.neighbours.Count == this.maxNeighbours)
				{
					...
					if (this.neighbourDists[index] <= distSq)
					{
						return;
					}
					this.neighbours[index] = agent;
					this.neighbourDists[index] = distSq;
				}
				else
				{
					this.neighbours.Add(agent);
				...
				while (num != 0 && distSq < this.neighbourDists[num - 1])
				{
			...
			for (int i = 0; i < this.neighbours.Count; i++)
			{
				...
				if (num > num3)
				{
					...
					if (num4 < 0L && num4 * num4 > num3 * sqrMagnitudeLong)
					{
						VInt2 vInt2 = vInt;
						vInt2.Normalize();
						item.direction.x = vInt2.y;
						item.direction.y = -vInt2.x;
					...
					else
					{
						...
						{
							item.direction.x = (int)IntMath.Divide(((long)xz.x * num6 - (long)xz.y * num2) * 1000L, num);
							item.direction.y = (int)IntMath.Divide(((long)xz.y * num2 - (long)xz.y * num6) * 1000L, num);
						}
						else
						{
							item.direction.x = -(int)IntMath.Divide(((long)xz.x * num6 + (long)xz.y * num2) * 1000L, num);
							item.direction.y = -(int)IntMath.Divide(((long)(-(long)xz.x) * num2 + (long)xz.y * num6) * 1000L, num);
						}
						item.direction.Normalize();
						long num7 = VInt2.DotLong(ref xz2, ref item.direction);
						a.x = (int)(IntMath.Divide((long)item.direction.x * num7, 1000000L) - (long)xz2.x);
						a.y = (int)(IntMath.Divide((long)item.direction.y * num7, 1000000L) - (long)xz2.y);
					}
				}
				else
				{
		...
		private static bool linearProgram1(List<VLine> lines, int lineNo, int radius, VInt2 optVelocity, bool directionOpt, ref VInt2 result)
		{
			...
			for (int i = 0; i < lineNo; i++)
			{
				...
				else
				{
					if (vFactor3.den > 0L)
					{
						vFactor2 = ((!(vFactor2 > vFactor3)) ? vFactor3 : vFactor2);
					}
					else
					{
						vFactor = ((!(vFactor > vFactor3)) ? vFactor3 : vFactor);
		...
		private static void linearProgram3(List<VLine> lines, int numObstLines, int beginLine, int radius, ref VInt2 result)
		{
			...
			for (int i = beginLine; i < lines.Count; i++)
			{
				...
				if (VInt2.DetLong(vLine.direction, vLine.point - result) > num)
				{
					...
					{
						Agent.projLines.Add(lines[j]);
					}
					int k = numObstLines;
					while (k < i)
					{
						VLine vLine2 = lines[k];
						...
						if (num2 != 0L)
						{
							...
							item.point = vLine.point + b;
							goto IL_13C;
						}
						if (VInt2.DetLong(vLine.direction, vLine2.direction) <= 0L)
						{
							item.point = vLine.point + vLine2.point;
							item.point.x = item.point.x / 2;
							item.point.y = item.point.y / 2;
							goto IL_13C;
						}
						IL_16B:
						k++;
						continue;
						IL_13C:
						item.direction = (vLine2.direction - vLine.direction).normalized;
						Agent.projLines.Add(item);
						goto IL_16B;
					}
					VInt2 vInt = result;
					if (Agent.linearProgram2(Agent.projLines, radius, new VInt2(-vLine.direction.y, vLine.direction.x), true, ref result) < Agent.projLines.Count)
					{
						result = vInt;
					}
					Agent.projLines.Clear();
