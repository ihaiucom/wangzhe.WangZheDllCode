	...
	public class NavmeshCut : MonoBehaviour
	{
		...
		public class DrawGizmos
		{
			...
			public void OnDrawGismos(Transform tr)
			{
				...
				{
					Vector3 vector = NavmeshCut.IntPointToV3(list[i]);
					vector.y = y;
					Vector3 vector2 = NavmeshCut.IntPointToV3(list[(i + 1) % list.Count]);
					vector2.y = y;
		...
		private void CalculateMeshContour()
		{
			...
			for (int i = 0; i < triangles.Length; i += 3)
			{
				...
				NavmeshCut.edges[new VInt2(triangles[i], triangles[i + 1])] = i;
				NavmeshCut.edges[new VInt2(triangles[i + 1], triangles[i + 2])] = i;
				NavmeshCut.edges[new VInt2(triangles[i + 2], triangles[i])] = i;
			}
		...
		public void GetContour(List<List<IntPoint>> buffer)
		{
			...
			case NavmeshCut.MeshType.Circle:
			{
				...
				NavmeshCut.GetContour_Circle(list, this.tr, this.circleResolution, this.circleRadius, this.center, this.useRotation);
				buffer.Add(list);
				break;
			}
			case NavmeshCut.MeshType.CustomMesh:
				...
				if (this.contours != null)
				{
					...
					for (int i = 0; i < this.contours.Length; i++)
					{
						...
						if (this.useRotation)
						{
							...
							{
								list.Add(NavmeshCut.V3ToIntPoint(localToWorldMatrix.MultiplyPoint3x4(this.center + array[j] * this.meshScale)));
							}
						}
						else
						{
