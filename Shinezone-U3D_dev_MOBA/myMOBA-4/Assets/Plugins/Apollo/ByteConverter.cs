	...
	internal class ByteConverter
	{
		...
		public static string Bytes2String(byte[] bytes)
		{
			...
			if (list.Count > 0)
			{
				...
				text += (char)array[list[0]];
				text += (char)array[list[0] + 1];
			}
			for (int k = 1; k < list.Count; k++)
			{
				int num = list[k] - list[k - 1] - 2;
				if (num > 0)
				{
					text += Encoding.UTF8.GetString(array, list[k - 1] + 2, num);
				}
				text += (char)array[list[k]];
				text += (char)array[list[k] + 1];
			}
			int num2 = 0;
			if (list.Count > 0)
			{
				num2 = list[list.Count - 1] + 2;
			}
			if (num2 < array.Length)
			{
				text += Encoding.UTF8.GetString(array, num2, array.Length - num2);
			}
			return text;
		}
