...
namespace Apollo
{
	public class ApolloTdir : ApolloObject, IApolloTdir
	{
		...
		public string GetErrorString()
		{
			...
			{
				IntPtr ptr = ApolloTdir.tcls_get_errString(base.ObjectId);
		...
		private void RecvNode(IntPtr ptr)
		{
			...
			TdirTreeNode item = default(TdirTreeNode);
			item.userRoleInfo = new List<TdirUserRoleInfo>();
			item.nodeID = subTdirTreeNode.nodeID;
			item.parentID = subTdirTreeNode.parentID;
			item.flag = subTdirTreeNode.flag;
			item.name = subTdirTreeNode.name;
			item.status = subTdirTreeNode.status;
			item.nodeType = subTdirTreeNode.nodeType;
			item.svrFlag = subTdirTreeNode.svrFlag;
			item.staticInfo = subTdirTreeNode.staticInfo;
			item.dynamicInfo = subTdirTreeNode.dynamicInfo;
			if (this.m_treeRoleInfoDic.ContainsKey(item.nodeID))
			{
				item.userRoleInfo = this.m_treeRoleInfoDic[item.nodeID];
				this.TdirLog(string.Format("the size of node ID[{0}]'s user role info is [{1}]", Convert.ToString(item.nodeID), item.userRoleInfo.Count));
			}
			else
			{
		...
		private void RecvRoleInfo(byte[] data)
		{
			...
			TdirUserRoleInfo item = default(TdirUserRoleInfo);
			item.zoneID = cTdirUserRoleInfo.zoneID;
			item.roleID = cTdirUserRoleInfo.roleID;
			item.lastLoginTime = cTdirUserRoleInfo.lastLoginTime;
			item.roleName = cTdirUserRoleInfo.roleName;
			item.roleLevel = cTdirUserRoleInfo.roleLevel;
			item.appLen = cTdirUserRoleInfo.appLen;
			if (0u < cTdirUserRoleInfo.appLen)
			{
				...
				{
					item.appBuff[num] = cTdirUserRoleInfo.appBuff[num];
					num++;
				}
			}
		...
		[DllImport("apollo", CallingConvention = CallingConvention.Cdecl)]
		private static extern TdirResult tcls_init_query_isp(ulong objId, int appID, [MarshalAs(UnmanagedType.LPStr)] string ipList, [MarshalAs(UnmanagedType.LPStr)] string portList, [MarshalAs(UnmanagedType.LPStr)] string lastSuccessIP, [MarshalAs(UnmanagedType.LPStr)] string lastSuccessPort, [MarshalAs(UnmanagedType.LPStr)] string openID);
