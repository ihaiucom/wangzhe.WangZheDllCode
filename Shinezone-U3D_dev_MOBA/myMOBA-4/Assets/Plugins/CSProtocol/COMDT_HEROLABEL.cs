...
{
	public class COMDT_HEROLABEL : ProtocolObject
	{
		public ulong ullUid;
		...
		public static readonly uint LENGTH_szHealUrl = 256u;

		public static readonly int CLASS_ID = 606;

		public COMDT_HEROLABEL()
		{
			this.szRoleName = new byte[64];
			this.szHealUrl = new byte[256];
		}
		...
		public override TdrError.ErrorType unpack(ref TdrReadBuf srcBuf, uint cutVer)
		{
			...
			errorType = srcBuf.readUInt32(ref this.dwGrade);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			...
			{
				if ((long)num > (long)((ulong)COMDT_HEROLABEL.LENGTH_szRoleName))
				{
					return TdrError.ErrorType.TDR_ERR_STR_LEN_TOO_BIG;
				}
			...
			if (num3 > (uint)this.szHealUrl.GetLength(0))
			{
				if ((long)num3 > (long)((ulong)COMDT_HEROLABEL.LENGTH_szHealUrl))
				{
					return TdrError.ErrorType.TDR_ERR_STR_LEN_TOO_BIG;
				}
				this.szHealUrl = new byte[num3];
			}
			if (1u > num3)
			{
				return TdrError.ErrorType.TDR_ERR_STR_LEN_TOO_SMALL;
			}
			errorType = srcBuf.readCString(ref this.szHealUrl, (int)num3);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			if (this.szHealUrl[(int)(num3 - 1u)] != 0)
			{
				return TdrError.ErrorType.TDR_ERR_STR_LEN_CONFLICT;
			}
			int num4 = TdrTypeUtil.cstrlen(this.szHealUrl) + 1;
		...
		{
			return COMDT_HEROLABEL.CLASS_ID;
		}
