...
{
	public class COMDT_CREATEINFO_EXTRA : ProtocolObject
	{
		...
		public static readonly int CLASS_ID = 643;

		public COMDT_QQSPROT_EXTRA stQQSprotExtra
		{
			get
			{
				return this.dataObject as COMDT_QQSPROT_EXTRA;
			}
			set
			{
		...
		{
			if (cutVer == 0u || COMDT_CREATEINFO_EXTRA.CURRVERSION < cutVer)
			{
				cutVer = COMDT_CREATEINFO_EXTRA.CURRVERSION;
			}
			if (COMDT_CREATEINFO_EXTRA.BASEVERSION > cutVer)
			{
		...
		private void select_1_1(long selector)
		{
			...
			else if (!(this.dataObject is COMDT_QQSPROT_EXTRA))
			{
				if (this.dataObject != null)
				{
					this.dataObject.Release();
				}
				this.dataObject = (COMDT_QQSPROT_EXTRA)ProtocolObjectPool.Get(COMDT_QQSPROT_EXTRA.CLASS_ID);
			}
		}

		public override int GetClassID()
		{
			return COMDT_CREATEINFO_EXTRA.CLASS_ID;
		}
