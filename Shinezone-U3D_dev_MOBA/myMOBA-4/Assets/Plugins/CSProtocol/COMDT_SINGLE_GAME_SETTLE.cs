...
{
	public class COMDT_SINGLE_GAME_SETTLE : ProtocolObject
	{
		...
		public static readonly int CLASS_ID = 184;

		public COMDT_SINGLE_GAME_SETTLE()
		{
		...
		{
			if (cutVer == 0u || COMDT_SINGLE_GAME_SETTLE.CURRVERSION < cutVer)
			{
				cutVer = COMDT_SINGLE_GAME_SETTLE.CURRVERSION;
			}
			if (COMDT_SINGLE_GAME_SETTLE.BASEVERSION > cutVer)
			{
				return TdrError.ErrorType.TDR_ERR_CUTVER_TOO_SMALL;
			}
			TdrError.ErrorType errorType = this.stBattleParam.pack(ref destBuf, cutVer);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
		...
		public override TdrError.ErrorType unpack(ref TdrReadBuf srcBuf, uint cutVer)
		{
			...
			TdrError.ErrorType errorType = this.stBattleParam.unpack(ref srcBuf, cutVer);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
		...
		{
			return COMDT_SINGLE_GAME_SETTLE.CLASS_ID;
		}
