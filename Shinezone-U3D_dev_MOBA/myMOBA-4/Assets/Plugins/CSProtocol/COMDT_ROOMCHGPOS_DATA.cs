...
{
	public class COMDT_ROOMCHGPOS_DATA : ProtocolObject
	{
		...
		public static readonly int CLASS_ID = 55;

		public COMDT_CHGMEMBER_INFO stMemberInfo
		{
			get
			{
				return this.dataObject as COMDT_CHGMEMBER_INFO;
			}
			set
			{
		...
		{
			if (cutVer == 0u || COMDT_ROOMCHGPOS_DATA.CURRVERSION < cutVer)
			{
				cutVer = COMDT_ROOMCHGPOS_DATA.CURRVERSION;
			}
			if (COMDT_ROOMCHGPOS_DATA.BASEVERSION > cutVer)
			{
		...
		private void select_1_1(long selector)
		{
			...
			else if (!(this.dataObject is COMDT_CHGMEMBER_INFO))
			{
				if (this.dataObject != null)
				{
					this.dataObject.Release();
		...
		{
			return COMDT_ROOMCHGPOS_DATA.CLASS_ID;
		}
