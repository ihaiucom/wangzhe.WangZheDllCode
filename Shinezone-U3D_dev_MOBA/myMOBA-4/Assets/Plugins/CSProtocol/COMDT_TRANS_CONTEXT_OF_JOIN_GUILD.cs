...
{
	public class COMDT_TRANS_CONTEXT_OF_JOIN_GUILD : ProtocolObject
	{
		...
		public static readonly int CLASS_ID = 400;
		...
		{
			if (cutVer == 0u || COMDT_TRANS_CONTEXT_OF_JOIN_GUILD.CURRVERSION < cutVer)
			{
				cutVer = COMDT_TRANS_CONTEXT_OF_JOIN_GUILD.CURRVERSION;
			}
			if (COMDT_TRANS_CONTEXT_OF_JOIN_GUILD.BASEVERSION > cutVer)
			{
		...
		public override TdrError.ErrorType unpack(ref TdrReadBuf srcBuf, uint cutVer)
		{
			...
			{
				if ((long)num > (long)((ulong)COMDT_TRANS_CONTEXT_OF_JOIN_GUILD.LENGTH_szGuildName))
				{
					return TdrError.ErrorType.TDR_ERR_STR_LEN_TOO_BIG;
				}
		...
		{
			return COMDT_TRANS_CONTEXT_OF_JOIN_GUILD.CLASS_ID;
		}
