...
{
	public class COMDT_GUILD_EVENT_JOIN : ProtocolObject
	{
		public ulong ullUid;
		...
		{
			if (cutVer == 0u || COMDT_GUILD_EVENT_JOIN.CURRVERSION < cutVer)
			{
				cutVer = COMDT_GUILD_EVENT_JOIN.CURRVERSION;
			}
			if (COMDT_GUILD_EVENT_JOIN.BASEVERSION > cutVer)
			{
		...
		public override TdrError.ErrorType unpack(ref TdrReadBuf srcBuf, uint cutVer)
		{
			...
			{
				if ((long)num > (long)((ulong)COMDT_GUILD_EVENT_JOIN.LENGTH_szName))
				{
					return TdrError.ErrorType.TDR_ERR_STR_LEN_TOO_BIG;
				}
		...
		{
			return COMDT_GUILD_EVENT_JOIN.CLASS_ID;
		}
