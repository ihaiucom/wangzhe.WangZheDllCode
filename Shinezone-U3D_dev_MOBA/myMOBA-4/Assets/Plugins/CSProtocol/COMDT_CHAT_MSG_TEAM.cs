...
namespace CSProtocol
{
	public class COMDT_CHAT_MSG_TEAM : ProtocolObject
	{
		...
		{
			if (cutVer == 0u || COMDT_CHAT_MSG_TEAM.CURRVERSION < cutVer)
			{
				cutVer = COMDT_CHAT_MSG_TEAM.CURRVERSION;
			}
			if (COMDT_CHAT_MSG_TEAM.BASEVERSION > cutVer)
			{
		...
		public override TdrError.ErrorType unpack(ref TdrReadBuf srcBuf, uint cutVer)
		{
			...
			{
				if ((long)num > (long)((ulong)COMDT_CHAT_MSG_TEAM.LENGTH_szContent))
				{
					return TdrError.ErrorType.TDR_ERR_STR_LEN_TOO_BIG;
				}
		...
		{
			return COMDT_CHAT_MSG_TEAM.CLASS_ID;
		}
