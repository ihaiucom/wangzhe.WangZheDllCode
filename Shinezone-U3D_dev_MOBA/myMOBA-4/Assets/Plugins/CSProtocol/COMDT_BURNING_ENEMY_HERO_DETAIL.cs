...
namespace CSProtocol
{
	public class COMDT_BURNING_ENEMY_HERO_DETAIL : ProtocolObject
	{
		...
		public static readonly int CLASS_ID = 178;

		public COMDT_BURNING_ENEMY_HERO_DETAIL()
		{
			this.astHeroList = new COMDT_BURNING_HERO_INFO[3];
			for (int i = 0; i < 3; i++)
			{
		...
		{
			if (cutVer == 0u || COMDT_BURNING_ENEMY_HERO_DETAIL.CURRVERSION < cutVer)
			{
				cutVer = COMDT_BURNING_ENEMY_HERO_DETAIL.CURRVERSION;
			}
			if (COMDT_BURNING_ENEMY_HERO_DETAIL.BASEVERSION > cutVer)
			{
				return TdrError.ErrorType.TDR_ERR_CUTVER_TOO_SMALL;
			}
			TdrError.ErrorType errorType = destBuf.writeUInt8(this.bHeroNum);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			...
			if (this.astHeroList.Length < (int)this.bHeroNum)
			{
		...
		public override TdrError.ErrorType unpack(ref TdrReadBuf srcBuf, uint cutVer)
		{
			...
			TdrError.ErrorType errorType = srcBuf.readUInt8(ref this.bHeroNum);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
		...
		{
			return COMDT_BURNING_ENEMY_HERO_DETAIL.CLASS_ID;
		}
