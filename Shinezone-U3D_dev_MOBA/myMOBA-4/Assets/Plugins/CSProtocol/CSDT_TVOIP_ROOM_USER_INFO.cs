...
namespace CSProtocol
{
	public class CSDT_TVOIP_ROOM_USER_INFO : ProtocolObject
	{
		...
		public static readonly int CLASS_ID = 1467;

		public CSDT_TVOIP_ROOM_USER_INFO()
		{
		...
		{
			if (cutVer == 0u || CSDT_TVOIP_ROOM_USER_INFO.CURRVERSION < cutVer)
			{
				cutVer = CSDT_TVOIP_ROOM_USER_INFO.CURRVERSION;
			}
			if (CSDT_TVOIP_ROOM_USER_INFO.BASEVERSION > cutVer)
			{
				return TdrError.ErrorType.TDR_ERR_CUTVER_TOO_SMALL;
			}
			TdrError.ErrorType errorType = destBuf.writeUInt32(this.dwMemberID);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
		...
		public override TdrError.ErrorType unpack(ref TdrReadBuf srcBuf, uint cutVer)
		{
			...
			TdrError.ErrorType errorType = srcBuf.readUInt32(ref this.dwMemberID);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			...
			{
				if ((long)num > (long)((ulong)CSDT_TVOIP_ROOM_USER_INFO.LENGTH_szOpenID))
				{
					return TdrError.ErrorType.TDR_ERR_STR_LEN_TOO_BIG;
				}
		...
		{
			return CSDT_TVOIP_ROOM_USER_INFO.CLASS_ID;
		}
