...
namespace CSProtocol
{
	public class COMDT_TRANS_CONTEXT_OF_DEAL_GUILD_INVITE : ProtocolObject
	{
		...
		public COMDT_GUILD_MEMBER_BRIEF_INFO stBeInvitePlayer;
		...
		public static readonly int CLASS_ID = 407;
		...
		{
			if (cutVer == 0u || COMDT_TRANS_CONTEXT_OF_DEAL_GUILD_INVITE.CURRVERSION < cutVer)
			{
				cutVer = COMDT_TRANS_CONTEXT_OF_DEAL_GUILD_INVITE.CURRVERSION;
			}
			if (COMDT_TRANS_CONTEXT_OF_DEAL_GUILD_INVITE.BASEVERSION > cutVer)
			{
			...
			errorType = this.stBeInvitePlayer.pack(ref destBuf, cutVer);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
		...
		public override TdrError.ErrorType unpack(ref TdrReadBuf srcBuf, uint cutVer)
		{
			...
			errorType = this.stBeInvitePlayer.unpack(ref srcBuf, cutVer);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			...
			{
				if ((long)num > (long)((ulong)COMDT_TRANS_CONTEXT_OF_DEAL_GUILD_INVITE.LENGTH_szGuildName))
				{
					return TdrError.ErrorType.TDR_ERR_STR_LEN_TOO_BIG;
				}
		...
		{
			return COMDT_TRANS_CONTEXT_OF_DEAL_GUILD_INVITE.CLASS_ID;
		}
