...
namespace CSProtocol
{
	public class COMDT_MATCH_ERR_PARAM : ProtocolObject
	{
		...
		public static readonly int CLASS_ID = 249;
		...
		public COMDT_MATCH_ERR_OF_FORBIDLADDER stForBidLadder
		{
			get
			{
				return this.dataObject as COMDT_MATCH_ERR_OF_FORBIDLADDER;
			}
			set
			{
				this.dataObject = value;
		...
		private void select_4_9(long selector)
		{
			if (selector != 4L)
			{
				if (selector != 9L)
				...
				else if (!(this.dataObject is COMDT_MATCH_ERR_OF_FORBIDLADDER))
				{
					if (this.dataObject != null)
					{
						this.dataObject.Release();
					}
					this.dataObject = (COMDT_MATCH_ERR_OF_FORBIDLADDER)ProtocolObjectPool.Get(COMDT_MATCH_ERR_OF_FORBIDLADDER.CLASS_ID);
				}
			}
