...
{
	public class COMDT_CHAT_MSG_PRIVATE : ProtocolObject
	{
		public COMDT_CHAT_PLAYER_INFO stFrom;

		public COMDT_CHAT_PLAYER_INFO stTo;
		...
		public static readonly int CLASS_ID = 279;

		public COMDT_CHAT_MSG_PRIVATE()
		{
			this.stFrom = (COMDT_CHAT_PLAYER_INFO)ProtocolObjectPool.Get(COMDT_CHAT_PLAYER_INFO.CLASS_ID);
			this.stTo = (COMDT_CHAT_PLAYER_INFO)ProtocolObjectPool.Get(COMDT_CHAT_PLAYER_INFO.CLASS_ID);
		...
		{
			if (cutVer == 0u || COMDT_CHAT_MSG_PRIVATE.CURRVERSION < cutVer)
			{
				cutVer = COMDT_CHAT_MSG_PRIVATE.CURRVERSION;
			}
			if (COMDT_CHAT_MSG_PRIVATE.BASEVERSION > cutVer)
			{
			...
			errorType = this.stTo.pack(ref destBuf, cutVer);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
		...
		public override TdrError.ErrorType unpack(ref TdrReadBuf srcBuf, uint cutVer)
		{
			...
			errorType = this.stTo.unpack(ref srcBuf, cutVer);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			...
			{
				if ((long)num > (long)((ulong)COMDT_CHAT_MSG_PRIVATE.LENGTH_szContent))
				{
					return TdrError.ErrorType.TDR_ERR_STR_LEN_TOO_BIG;
				}
		...
		{
			return COMDT_CHAT_MSG_PRIVATE.CLASS_ID;
		}
