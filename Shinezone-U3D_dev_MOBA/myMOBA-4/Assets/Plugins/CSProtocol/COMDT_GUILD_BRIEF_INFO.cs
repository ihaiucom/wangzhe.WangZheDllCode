...
{
	public class COMDT_GUILD_BRIEF_INFO : ProtocolObject
	{
		...
		public COMDT_GUILD_MEMBER_BRIEF_INFO stChairMan;

		public byte bLevelLimit;

		public byte bGradeLimit;
		...
		public static readonly int CLASS_ID = 372;
		...
		{
			if (cutVer == 0u || COMDT_GUILD_BRIEF_INFO.CURRVERSION < cutVer)
			{
				cutVer = COMDT_GUILD_BRIEF_INFO.CURRVERSION;
			}
			if (COMDT_GUILD_BRIEF_INFO.BASEVERSION > cutVer)
			{
			...
			errorType = destBuf.writeUInt8(this.bLevel);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			...
			errorType = this.stChairMan.pack(ref destBuf, cutVer);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			errorType = destBuf.writeUInt8(this.bLevelLimit);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			errorType = destBuf.writeUInt8(this.bGradeLimit);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
		...
		public override TdrError.ErrorType unpack(ref TdrReadBuf srcBuf, uint cutVer)
		{
			...
			{
				if ((long)num > (long)((ulong)COMDT_GUILD_BRIEF_INFO.LENGTH_szName))
				{
					return TdrError.ErrorType.TDR_ERR_STR_LEN_TOO_BIG;
				}
			...
			if (num3 > (uint)this.szNotice.GetLength(0))
			{
				if ((long)num3 > (long)((ulong)COMDT_GUILD_BRIEF_INFO.LENGTH_szNotice))
				{
					return TdrError.ErrorType.TDR_ERR_STR_LEN_TOO_BIG;
				}
			...
			errorType = srcBuf.readCString(ref this.szNotice, (int)num3);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			if (this.szNotice[(int)(num3 - 1u)] != 0)
			{
				return TdrError.ErrorType.TDR_ERR_STR_LEN_CONFLICT;
			}
			...
			errorType = this.stChairMan.unpack(ref srcBuf, cutVer);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			errorType = srcBuf.readUInt8(ref this.bLevelLimit);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			errorType = srcBuf.readUInt8(ref this.bGradeLimit);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
		...
		{
			return COMDT_GUILD_BRIEF_INFO.CLASS_ID;
		}
