...
{
	public class COMDT_ACNT_ASKFORREQ_REFUSEDEL : ProtocolObject
	{
		public byte[] szAcntName;

		public byte[] szReqName;
		...
		public static readonly uint LENGTH_szReqName = 64u;

		public static readonly int CLASS_ID = 705;

		public COMDT_ACNT_ASKFORREQ_REFUSEDEL()
		{
			this.szAcntName = new byte[64];
			this.szReqName = new byte[64];
		...
		{
			if (cutVer == 0u || COMDT_ACNT_ASKFORREQ_REFUSEDEL.CURRVERSION < cutVer)
			{
				cutVer = COMDT_ACNT_ASKFORREQ_REFUSEDEL.CURRVERSION;
			}
			if (COMDT_ACNT_ASKFORREQ_REFUSEDEL.BASEVERSION > cutVer)
			{
			...
			int num2 = TdrTypeUtil.cstrlen(this.szReqName);
			if (num2 >= 64)
			{
				return TdrError.ErrorType.TDR_ERR_STR_LEN_TOO_BIG;
			}
			errorType = destBuf.writeCString(this.szReqName, num2);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
		...
		public override TdrError.ErrorType unpack(ref TdrReadBuf srcBuf, uint cutVer)
		{
			...
			{
				if ((long)num > (long)((ulong)COMDT_ACNT_ASKFORREQ_REFUSEDEL.LENGTH_szAcntName))
				{
					return TdrError.ErrorType.TDR_ERR_STR_LEN_TOO_BIG;
				}
			...
			if (num3 > (uint)this.szReqName.GetLength(0))
			{
				if ((long)num3 > (long)((ulong)COMDT_ACNT_ASKFORREQ_REFUSEDEL.LENGTH_szReqName))
				{
					return TdrError.ErrorType.TDR_ERR_STR_LEN_TOO_BIG;
				}
				this.szReqName = new byte[num3];
			}
			if (1u > num3)
			{
				return TdrError.ErrorType.TDR_ERR_STR_LEN_TOO_SMALL;
			}
			errorType = srcBuf.readCString(ref this.szReqName, (int)num3);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			if (this.szReqName[(int)(num3 - 1u)] != 0)
			{
				return TdrError.ErrorType.TDR_ERR_STR_LEN_CONFLICT;
			}
			int num4 = TdrTypeUtil.cstrlen(this.szReqName) + 1;
		...
		{
			return COMDT_ACNT_ASKFORREQ_REFUSEDEL.CLASS_ID;
		}
