...
{
	public class COMDT_ROOMMEMBER_OF_ACNT : ProtocolObject
	{
		public ulong ullUid;
		...
		public COMDT_GAME_VIP_CLIENT stMemberVip;

		public byte bIsLikeFlag;
		...
		public static readonly int CLASS_ID = 48;

		public COMDT_ROOMMEMBER_OF_ACNT()
		{
			this.szMemberName = new byte[64];
			this.szMemberHeadUrl = new byte[256];
			this.stMemberVip = (COMDT_GAME_VIP_CLIENT)ProtocolObjectPool.Get(COMDT_GAME_VIP_CLIENT.CLASS_ID);
		}
		...
		public override TdrError.ErrorType pack(ref TdrWriteBuf destBuf, uint cutVer)
		{
			...
			errorType = destBuf.writeInt32(this.iFromGameEntity);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			...
			errorType = this.stMemberVip.pack(ref destBuf, cutVer);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			errorType = destBuf.writeUInt8(this.bIsLikeFlag);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
		...
		{
			return COMDT_ROOMMEMBER_OF_ACNT.CLASS_ID;
		}

		public override void OnRelease()
		{
			this.ullUid = 0uL;
			this.iFromGameEntity = 0;
			this.iLogicWorldID = 0;
			this.dwMemberLevel = 0u;
			this.dwMemberPvpLevel = 0u;
			this.dwMemberHeadId = 0u;
			if (this.stMemberVip != null)
			{
				this.stMemberVip.Release();
				this.stMemberVip = null;
			}
			this.bIsLikeFlag = 0;
		}
