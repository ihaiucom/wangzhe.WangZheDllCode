...
{
	public class COMDT_RECRUITMENT_SOURCE : ProtocolObject
	{
		...
		public static readonly int CLASS_ID = 294;

		public COMDT_RECRUITMENT_SOURCE()
		{
		...
		{
			if (cutVer == 0u || COMDT_RECRUITMENT_SOURCE.CURRVERSION < cutVer)
			{
				cutVer = COMDT_RECRUITMENT_SOURCE.CURRVERSION;
			}
			if (COMDT_RECRUITMENT_SOURCE.BASEVERSION > cutVer)
			{
			...
			errorType = destBuf.writeInt32(this.iPlatId);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
		...
		public override TdrError.ErrorType unpack(ref TdrReadBuf srcBuf, uint cutVer)
		{
			...
			errorType = srcBuf.readInt32(ref this.iPlatId);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			...
			{
				if ((long)num > (long)((ulong)COMDT_RECRUITMENT_SOURCE.LENGTH_szOpenID))
				{
					return TdrError.ErrorType.TDR_ERR_STR_LEN_TOO_BIG;
				}
		...
		{
			return COMDT_RECRUITMENT_SOURCE.CLASS_ID;
		}
