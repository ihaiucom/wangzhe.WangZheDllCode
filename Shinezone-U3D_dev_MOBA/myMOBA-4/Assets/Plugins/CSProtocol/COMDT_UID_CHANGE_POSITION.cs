...
namespace CSProtocol
{
	public class COMDT_UID_CHANGE_POSITION : ProtocolObject
	{
		...
		public byte[] szMemName;

		public static readonly uint BASEVERSION = 1u;

		public static readonly uint CURRVERSION = 1u;

		public static readonly uint LENGTH_szMemName = 64u;

		public static readonly int CLASS_ID = 420;

		public COMDT_UID_CHANGE_POSITION()
		{
			this.szMemName = new byte[64];
		}
		...
		public override TdrError.ErrorType unpack(ref TdrReadBuf srcBuf, uint cutVer)
		{
			...
			errorType = srcBuf.readUInt8(ref this.bOrgPosition);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			errorType = srcBuf.readUInt8(ref this.bChgPosition);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			...
			if (num > (uint)this.szMemName.GetLength(0))
			{
				if ((long)num > (long)((ulong)COMDT_UID_CHANGE_POSITION.LENGTH_szMemName))
				{
					return TdrError.ErrorType.TDR_ERR_STR_LEN_TOO_BIG;
				}
				this.szMemName = new byte[num];
			}
			if (1u > num)
			{
				return TdrError.ErrorType.TDR_ERR_STR_LEN_TOO_SMALL;
			}
			errorType = srcBuf.readCString(ref this.szMemName, (int)num);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			if (this.szMemName[(int)(num - 1u)] != 0)
			{
				return TdrError.ErrorType.TDR_ERR_STR_LEN_CONFLICT;
			}
			int num2 = TdrTypeUtil.cstrlen(this.szMemName) + 1;
		...
		{
			return COMDT_UID_CHANGE_POSITION.CLASS_ID;
		}
