...
namespace CSProtocol
{
	public class COMDT_GUILD_RECOMMEND_INFO : ProtocolObject
	{
		...
		public static readonly uint LENGTH_szRecommendName = 64u;

		public static readonly int CLASS_ID = 382;

		public COMDT_GUILD_RECOMMEND_INFO()
		{
			...
			this.szRecommendName = new byte[64];
		...
		{
			if (cutVer == 0u || COMDT_GUILD_RECOMMEND_INFO.CURRVERSION < cutVer)
			{
				cutVer = COMDT_GUILD_RECOMMEND_INFO.CURRVERSION;
			}
			if (COMDT_GUILD_RECOMMEND_INFO.BASEVERSION > cutVer)
			{
			...
			int num3 = TdrTypeUtil.cstrlen(this.szRecommendName);
			if (num3 >= 64)
			{
				return TdrError.ErrorType.TDR_ERR_STR_LEN_TOO_BIG;
			}
			errorType = destBuf.writeCString(this.szRecommendName, num3);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
		...
		public override TdrError.ErrorType unpack(ref TdrReadBuf srcBuf, uint cutVer)
		{
			...
			{
				if ((long)num > (long)((ulong)COMDT_GUILD_RECOMMEND_INFO.LENGTH_szName))
				{
					return TdrError.ErrorType.TDR_ERR_STR_LEN_TOO_BIG;
				}
			...
			{
				if ((long)num3 > (long)((ulong)COMDT_GUILD_RECOMMEND_INFO.LENGTH_szHeadUrl))
				{
					return TdrError.ErrorType.TDR_ERR_STR_LEN_TOO_BIG;
				}
			...
			if (num5 > (uint)this.szRecommendName.GetLength(0))
			{
				if ((long)num5 > (long)((ulong)COMDT_GUILD_RECOMMEND_INFO.LENGTH_szRecommendName))
				{
					return TdrError.ErrorType.TDR_ERR_STR_LEN_TOO_BIG;
				}
			...
			errorType = srcBuf.readCString(ref this.szRecommendName, (int)num5);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			if (this.szRecommendName[(int)(num5 - 1u)] != 0)
			{
				return TdrError.ErrorType.TDR_ERR_STR_LEN_CONFLICT;
			}
		...
		{
			return COMDT_GUILD_RECOMMEND_INFO.CLASS_ID;
		}
