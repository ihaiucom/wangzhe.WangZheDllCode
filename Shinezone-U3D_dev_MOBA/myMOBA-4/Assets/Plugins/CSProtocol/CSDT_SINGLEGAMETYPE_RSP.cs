...
namespace CSProtocol
{
	public class CSDT_SINGLEGAMETYPE_RSP : ProtocolObject
	{
		...
		public static readonly int CLASS_ID = 777;
		...
		{
			if (cutVer == 0u || CSDT_SINGLEGAMETYPE_RSP.CURRVERSION < cutVer)
			{
				cutVer = CSDT_SINGLEGAMETYPE_RSP.CURRVERSION;
			}
			if (CSDT_SINGLEGAMETYPE_RSP.BASEVERSION > cutVer)
			{
		...
		private void select_1_1(long selector)
		{
			...
			else if (!(this.dataObject is CSDT_SINGLEGAME_OF_COMBAT_DETAIL))
			{
				if (this.dataObject != null)
				{
					this.dataObject.Release();
		...
		{
			return CSDT_SINGLEGAMETYPE_RSP.CLASS_ID;
		}
