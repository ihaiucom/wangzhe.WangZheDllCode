...
{
	public class COMDT_TRANS_CONTEXT_OF_APPLYMASTERREQ : ProtocolObject
	{
		...
		public uint dwMaxStudentNum;

		public byte bStudentMaxGrade;
		...
		public COMDT_TRANS_CONTEXT_OF_APPLYMASTERREQ()
		{
		...
		{
			if (cutVer == 0u || COMDT_TRANS_CONTEXT_OF_APPLYMASTERREQ.CURRVERSION < cutVer)
			{
				cutVer = COMDT_TRANS_CONTEXT_OF_APPLYMASTERREQ.CURRVERSION;
			}
			if (COMDT_TRANS_CONTEXT_OF_APPLYMASTERREQ.BASEVERSION > cutVer)
			{
			...
			errorType = destBuf.writeUInt32(this.dwMaxStudentNum);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			errorType = destBuf.writeUInt8(this.bStudentMaxGrade);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
		...
		public override TdrError.ErrorType unpack(ref TdrReadBuf srcBuf, uint cutVer)
		{
			...
			{
				if ((long)num > (long)((ulong)COMDT_TRANS_CONTEXT_OF_APPLYMASTERREQ.LENGTH_szVerificationInfo))
				{
					return TdrError.ErrorType.TDR_ERR_STR_LEN_TOO_BIG;
				}
		...
		{
			return COMDT_TRANS_CONTEXT_OF_APPLYMASTERREQ.CLASS_ID;
		}
		...
		public override void OnRelease()
		{
			...
			this.dwMaxStudentNum = 0u;
			this.bStudentMaxGrade = 0;
		}
