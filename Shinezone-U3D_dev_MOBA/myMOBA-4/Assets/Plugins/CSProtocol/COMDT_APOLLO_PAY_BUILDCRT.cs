...
{
	public class COMDT_APOLLO_PAY_BUILDCRT : ProtocolObject
	{
		...
		public static readonly int CLASS_ID = 458;
		...
		public override TdrError.ErrorType unpack(ref TdrReadBuf srcBuf, uint cutVer)
		{
			...
			TdrError.ErrorType errorType = this.stCreatePlayer.unpack(ref srcBuf, cutVer);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			...
			{
				if ((long)num > (long)((ulong)COMDT_APOLLO_PAY_BUILDCRT.LENGTH_szName))
				{
					return TdrError.ErrorType.TDR_ERR_STR_LEN_TOO_BIG;
				}
			...
			{
				if ((long)num3 > (long)((ulong)COMDT_APOLLO_PAY_BUILDCRT.LENGTH_szNotice))
				{
					return TdrError.ErrorType.TDR_ERR_STR_LEN_TOO_BIG;
				}
			...
			errorType = srcBuf.readUInt8(ref this.bIsFriendOnly);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
		...
		{
			return COMDT_APOLLO_PAY_BUILDCRT.CLASS_ID;
		}
