...
namespace ResData
{
	public class ResRobotName : IUnpackable, tsf4g_csharp_interface
	{
		public uint dwObjId;
		...
		{
			if (cutVer == 0u || ResRobotName.CURRVERSION < cutVer)
			{
				cutVer = ResRobotName.CURRVERSION;
			}
			if (ResRobotName.BASEVERSION > cutVer)
			{
			...
			{
				if ((long)num > (long)((ulong)ResRobotName.LENGTH_szName))
				{
		...
		public TdrError.ErrorType load(ref TdrReadBuf srcBuf, uint cutVer)
		{
			...
			{
				this.szName_ByteArray = new byte[ResRobotName.LENGTH_szName];
			}
