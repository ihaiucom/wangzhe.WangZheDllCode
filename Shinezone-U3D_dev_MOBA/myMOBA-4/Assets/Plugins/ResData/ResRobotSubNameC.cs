...
namespace ResData
{
	public class ResRobotSubNameC : IUnpackable, tsf4g_csharp_interface
	{
		public uint dwObjId;
		...
		{
			if (cutVer == 0u || ResRobotSubNameC.CURRVERSION < cutVer)
			{
				cutVer = ResRobotSubNameC.CURRVERSION;
			}
			if (ResRobotSubNameC.BASEVERSION > cutVer)
			{
			...
			{
				if ((long)num > (long)((ulong)ResRobotSubNameC.LENGTH_szName))
				{
		...
		public TdrError.ErrorType load(ref TdrReadBuf srcBuf, uint cutVer)
		{
			...
			{
				this.szName_ByteArray = new byte[ResRobotSubNameC.LENGTH_szName];
			}
