...
namespace ResData
{
	public class ResRollingGift : IUnpackable, tsf4g_csharp_interface
	{
		public uint dwID;
		...
		{
			if (cutVer == 0u || ResRollingGift.CURRVERSION < cutVer)
			{
				cutVer = ResRollingGift.CURRVERSION;
			}
			if (ResRollingGift.BASEVERSION > cutVer)
			{
			...
			{
				if ((long)num > (long)((ulong)ResRollingGift.LENGTH_szName))
				{
		...
		public TdrError.ErrorType load(ref TdrReadBuf srcBuf, uint cutVer)
		{
			...
			{
				this.szName_ByteArray = new byte[ResRollingGift.LENGTH_szName];
			}
