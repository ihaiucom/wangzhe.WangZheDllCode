...
namespace ResData
{
	public class ResSocialTags : IUnpackable, tsf4g_csharp_interface
	{
		public uint dwID;
		...
		{
			if (cutVer == 0u || ResSocialTags.CURRVERSION < cutVer)
			{
				cutVer = ResSocialTags.CURRVERSION;
			}
			if (ResSocialTags.BASEVERSION > cutVer)
			{
			...
			{
				if ((long)num > (long)((ulong)ResSocialTags.LENGTH_szText))
				{
		...
		public TdrError.ErrorType load(ref TdrReadBuf srcBuf, uint cutVer)
		{
			...
			{
				this.szText_ByteArray = new byte[ResSocialTags.LENGTH_szText];
			}
