...
namespace ResData
{
	public class ResRareExchange : IUnpackable, tsf4g_csharp_interface
	{
		public uint dwID;

		public ushort wExchangeType;

		public uint dwExchangeID;
		...
		public uint dwRareItemID;

		public uint dwRareItemCnt;
		...
		{
			if (cutVer == 0u || ResRareExchange.CURRVERSION < cutVer)
			{
				cutVer = ResRareExchange.CURRVERSION;
			}
			if (ResRareExchange.BASEVERSION > cutVer)
			{
			...
			errorType = srcBuf.readUInt16(ref this.wExchangeType);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			errorType = srcBuf.readUInt32(ref this.dwExchangeID);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			...
			{
				if ((long)num > (long)((ulong)ResRareExchange.LENGTH_szOnTime))
				{
			...
			{
				if ((long)num3 > (long)((ulong)ResRareExchange.LENGTH_szOffTime))
				{
					return TdrError.ErrorType.TDR_ERR_STR_LEN_TOO_BIG;
				}
			...
			errorType = srcBuf.readUInt32(ref this.dwRareItemID);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			errorType = srcBuf.readUInt32(ref this.dwRareItemCnt);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			errorType = srcBuf.readUInt32(ref this.dwTabID);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
		...
		public TdrError.ErrorType load(ref TdrReadBuf srcBuf, uint cutVer)
		{
			...
			{
				this.szOnTime_ByteArray = new byte[ResRareExchange.LENGTH_szOnTime];
			}
			...
			{
				this.szOffTime_ByteArray = new byte[ResRareExchange.LENGTH_szOffTime];
			}
