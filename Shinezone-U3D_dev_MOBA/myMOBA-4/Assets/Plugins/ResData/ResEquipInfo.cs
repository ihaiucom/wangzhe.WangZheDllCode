...
namespace ResData
{
	public class ResEquipInfo : IUnpackable, tsf4g_csharp_interface
	{
		public uint dwID;
		...
		public uint dwSmeltCoin;

		public ushort wEnchantPtGet;

		public byte bEnchantPos;

		public ushort wEnchantConsume;

		public uint dwEnchantID;

		public ushort wAdvEnchantRate;
		...
		public ResEquipInfo()
		{
			...
			this.astFuncEftList = new ResDT_FuncEft_Obj[6];
			for (int i = 0; i < 6; i++)
			{
			...
			for (int j = 0; j < 10; j++)
			{
				this.astSrcInfo[j] = new ResDT_ItemSrc_Info();
			}
		...
		{
			if (cutVer == 0u || ResEquipInfo.CURRVERSION < cutVer)
			{
				cutVer = ResEquipInfo.CURRVERSION;
			}
			if (ResEquipInfo.BASEVERSION > cutVer)
			{
			...
			{
				if ((long)num > (long)((ulong)ResEquipInfo.LENGTH_szName))
				{
			...
			{
				if ((long)num3 > (long)((ulong)ResEquipInfo.LENGTH_szDesc))
				{
					return TdrError.ErrorType.TDR_ERR_STR_LEN_TOO_BIG;
				}
			...
			errorType = srcBuf.readUInt16(ref this.wWearLevel);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			...
			errorType = srcBuf.readUInt32(ref this.dwSmeltCoin);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			errorType = srcBuf.readUInt16(ref this.wEnchantPtGet);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			...
			errorType = srcBuf.readUInt16(ref this.wEnchantConsume);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			errorType = srcBuf.readUInt32(ref this.dwEnchantID);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			errorType = srcBuf.readUInt16(ref this.wAdvEnchantRate);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
		...
		public TdrError.ErrorType load(ref TdrReadBuf srcBuf, uint cutVer)
		{
			...
			{
				this.szName_ByteArray = new byte[ResEquipInfo.LENGTH_szName];
			}
			...
			{
				this.szDesc_ByteArray = new byte[ResEquipInfo.LENGTH_szDesc];
			}
