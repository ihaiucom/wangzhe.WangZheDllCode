...
namespace ResData
{
	public class ResRobotSubNameA : IUnpackable, tsf4g_csharp_interface
	{
		public uint dwObjId;
		...
		{
			if (cutVer == 0u || ResRobotSubNameA.CURRVERSION < cutVer)
			{
				cutVer = ResRobotSubNameA.CURRVERSION;
			}
			if (ResRobotSubNameA.BASEVERSION > cutVer)
			{
			...
			{
				if ((long)num > (long)((ulong)ResRobotSubNameA.LENGTH_szName))
				{
		...
		public TdrError.ErrorType load(ref TdrReadBuf srcBuf, uint cutVer)
		{
			...
			{
				this.szName_ByteArray = new byte[ResRobotSubNameA.LENGTH_szName];
			}
