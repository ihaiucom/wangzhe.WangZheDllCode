...
namespace ResData
{
	public class ResTextData : IUnpackable, tsf4g_csharp_interface
	{
		public uint dwTag;
		...
		{
			if (cutVer == 0u || ResTextData.CURRVERSION < cutVer)
			{
				cutVer = ResTextData.CURRVERSION;
			}
			if (ResTextData.BASEVERSION > cutVer)
			{
				return TdrError.ErrorType.TDR_ERR_CUTVER_TOO_SMALL;
			}
			TdrError.ErrorType errorType = srcBuf.readUInt32(ref this.dwTag);
			...
			{
				if ((long)num > (long)((ulong)ResTextData.LENGTH_szContent))
				{
		...
		public TdrError.ErrorType load(ref TdrReadBuf srcBuf, uint cutVer)
		{
			...
			{
				this.szContent_ByteArray = new byte[ResTextData.LENGTH_szContent];
			}
