...
namespace ResData
{
	public class ResShopInfo : IUnpackable, tsf4g_csharp_interface
	{
		...
		{
			if (cutVer == 0u || ResShopInfo.CURRVERSION < cutVer)
			{
				cutVer = ResShopInfo.CURRVERSION;
			}
			if (ResShopInfo.BASEVERSION > cutVer)
			{
			...
			{
				if ((long)num > (long)((ulong)ResShopInfo.LENGTH_szDesc))
				{
			...
			errorType = srcBuf.readUInt32(ref this.dwCoinPrice);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			errorType = srcBuf.readUInt8(ref this.bBuyResCnt);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			for (int i = 0; i < 5; i++)
			{
				errorType = srcBuf.readUInt32(ref this.BuyResID[i]);
				if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
				{
					return errorType;
				}
		...
		public TdrError.ErrorType load(ref TdrReadBuf srcBuf, uint cutVer)
		{
			...
			{
				this.szDesc_ByteArray = new byte[ResShopInfo.LENGTH_szDesc];
			}
