...
namespace ResData
{
	public class ResDeadInfoText : IUnpackable, tsf4g_csharp_interface
	{
		public byte bId;
		...
		public static readonly uint LENGTH_szText = 256u;

		public ResDeadInfoText()
		{
		...
		{
			if (cutVer == 0u || ResDeadInfoText.CURRVERSION < cutVer)
			{
				cutVer = ResDeadInfoText.CURRVERSION;
			}
			if (ResDeadInfoText.BASEVERSION > cutVer)
			{
			...
			errorType = srcBuf.readUInt8(ref this.bConditionId);
			if (errorType != TdrError.ErrorType.TDR_NO_ERROR)
			{
				return errorType;
			}
			...
			{
				if ((long)num > (long)((ulong)ResDeadInfoText.LENGTH_szText))
				{
		...
		public TdrError.ErrorType load(ref TdrReadBuf srcBuf, uint cutVer)
		{
			...
			{
				this.szText_ByteArray = new byte[ResDeadInfoText.LENGTH_szText];
			}
