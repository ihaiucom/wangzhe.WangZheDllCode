...
namespace ResData
{
	public class ResContinuKill : IUnpackable, tsf4g_csharp_interface
	{
		...
		{
			if (cutVer == 0u || ResContinuKill.CURRVERSION < cutVer)
			{
				cutVer = ResContinuKill.CURRVERSION;
			}
			if (ResContinuKill.BASEVERSION > cutVer)
			{
			...
			{
				if ((long)num > (long)((ulong)ResContinuKill.LENGTH_szAchievementName))
				{
		...
		public TdrError.ErrorType load(ref TdrReadBuf srcBuf, uint cutVer)
		{
			...
			{
				this.szAchievementName_ByteArray = new byte[ResContinuKill.LENGTH_szAchievementName];
			}
