...
namespace TMPro
{
	...
	public class TextMeshProFont : ScriptableObject
	{
		...
		public void ReadFontDefinition()
		{
			...
			foreach (GlyphInfo current in this.m_glyphInfoList)
			{
			...
			{
				this.m_characterDictionary[32].width = this.m_fontInfo.Ascender / 5f;
			...
			if (!this.m_characterDictionary.ContainsKey(9))
			{
				glyphInfo = new GlyphInfo();
				glyphInfo.id = 9;
				glyphInfo.x = this.m_characterDictionary[32].x;
				glyphInfo.y = this.m_characterDictionary[32].y;
				glyphInfo.width = this.m_characterDictionary[32].width * (float)this.TabSize;
				glyphInfo.height = this.m_characterDictionary[32].height;
				glyphInfo.xOffset = this.m_characterDictionary[32].xOffset;
				glyphInfo.yOffset = this.m_characterDictionary[32].yOffset;
				glyphInfo.xAdvance = this.m_characterDictionary[32].xAdvance * (float)this.TabSize;
				this.m_characterDictionary.Add(9, glyphInfo);
			}
			this.m_fontInfo.TabWidth = this.m_characterDictionary[32].xAdvance;
			...
			{
				this.fontHashCode = (this.fontHashCode << 5) - this.fontHashCode + (int)name[j];
			}
			string name2 = this.material.name;
			this.materialHashCode = 0;
			for (int k = 0; k < name2.Length; k++)
			{
				this.materialHashCode = (this.materialHashCode << 5) - this.materialHashCode + (int)name2[k];
			}
		}
