...
public class Seeker : MonoBehaviour, IPooledMonoBehaviour
{
	...
	public Path StartPath(Path p, OnPathDelegate callback = null, int graphMask = -1)
	{
		...
		this.path = p;
		Path expr_77 = this.path;
		expr_77.callback = (OnPathDelegate)Delegate.Combine(expr_77.callback, this.onPathDelegate);
    ...
    {
        yield return null;
        RunModifiers(Seeker.ModifierPass.PreProcess, p);
