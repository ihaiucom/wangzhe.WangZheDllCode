		...
		get
		{
			...
			if (this.argumentsTypes != null && this.argumentsTypes.Length > 0)
			{
				for (int i = 0; i < this.argumentsTypes.Length; i++)
				{
					...
					text += string.Format(" <{0}{2}|{1}>", this.argumentsTypes[i].name, argumentType.Name, (!this.argumentsTypes[i].isOptional) ? string.Empty : "(Optional)");
				}
			}
		...
		if (this.argumentsTypes != null && this.argumentsTypes.Length > 0)
		{
			for (int i = 0; i < this.argumentsTypes.Length; i++)
			{
				...
				if (InArguments == null || i >= InArguments.Length)
				{
					...
					{
						OutMessage = string.Format("无法执行命令，因为缺少参数<{0}>, 类型为:<{1}>", argumentDescriptionAttribute.name, argumentDescriptionAttribute.argumentType.Name);
						return false;
					}
				...
				else if (!argumentDescriptionAttribute.isOptional)
				{
					...
					if (!CheatCommandBase.TypeCastCheck(InArguments[i], this.argumentsTypes[i], out text))
					{
						...
						{
							InArguments[i],
							this.argumentsTypes[i].argumentType.Name,
