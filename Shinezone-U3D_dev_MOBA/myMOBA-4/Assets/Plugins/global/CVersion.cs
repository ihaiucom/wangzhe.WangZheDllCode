...
public class CVersion
{
	...
	public static bool IsSynchronizedVersion(string appVersion, string usedResourceVersion)
	{
		...
		return string.Equals(a, CVersion.s_codeVersion);
	}
	...
	private static void Initialize()
	{
		...
		if (textAsset != null)
		{
			// bsh: hack, set version to 1.20.1.17 to avoid downloading new apk
			string text = "[Publish = 20170618_v20]\r\n[CodeVersion = 1.20.1]\r\n[ResourceVersion = 17]\r\n[Build = 0001]";//textAsset.text;
		...
		CVersion.s_iOSBundleVersion = string.Format("{0}{1}", CVersion.s_codeVersion, (CVersion.s_resourceVersion.Length != 1) ? CVersion.s_resourceVersion : ("0" + CVersion.s_resourceVersion));
		int num = 0;
		int.TryParse(CVersion.s_buildNumber, out num);
		if (num <= 99)
		{
