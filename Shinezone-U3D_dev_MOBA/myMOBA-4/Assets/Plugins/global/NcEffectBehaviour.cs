...
public class NcEffectBehaviour : MonoBehaviour
{
	...
	private static Texture[] PreloadTexture(GameObject tarObj, List<GameObject> parentPrefabList)
	{
		...
		for (int l = 0; l < array4.Length; l++)
		{
			...
			{
				for (int m = 0; m < ncSpriteFactory.m_SpriteList.Count; m++)
				{
					if (ncSpriteFactory.m_SpriteList[m].m_EffectPrefab != null)
					{
						Texture[] array5 = NcEffectBehaviour.PreloadPrefab(ncSpriteFactory.m_SpriteList[m].m_EffectPrefab, parentPrefabList, true);
						if (array5 == null)
						{
							ncSpriteFactory.m_SpriteList[m].m_EffectPrefab = null;
						}
						else
						{
							listView.AddRange(array5);
						}
						if (ncSpriteFactory.m_SpriteList[m].m_AudioClip != null)
						{
						}
					}
				}
