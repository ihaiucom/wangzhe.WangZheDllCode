...
public class DebugHelper : MonoBehaviour
{
	...
	public static void BeginLogs()
	{
		...
		DebugHelper.OpenLoger(SLogCategory.Normal, string.Format("{0}/{1}_normal.log", logRootPath, arg));
		DebugHelper.OpenLoger(SLogCategory.Skill, string.Format("{0}/{1}_skill.log", logRootPath, arg));
		DebugHelper.OpenLoger(SLogCategory.Misc, string.Format("{0}/{1}_misc.log", logRootPath, arg));
		DebugHelper.OpenLoger(SLogCategory.Msg, string.Format("{0}/{1}_msg.log", logRootPath, arg));
		DebugHelper.OpenLoger(SLogCategory.Actor, string.Format("{0}/{1}_actor.log", logRootPath, arg));
		DebugHelper.OpenLoger(SLogCategory.Rvo, string.Format("{0}/{1}_rvo.log", logRootPath, arg));
		DebugHelper.OpenLoger(SLogCategory.Fow, string.Format("{0}/{1}_fow.log", logRootPath, arg));
	}
	...
	public static void ClearLogs(int passedMinutes = 60)
	{
		...
		{
			string[] files = Directory.GetFiles(directoryInfo.FullName, "*.log", SearchOption.TopDirectoryOnly);
	...
	[Conditional("UNITY_ANDROID"), Conditional("UNITY_STANDALONE_WIN"), Conditional("UNITY_EDITOR"), Conditional("FORCE_LOG"), Conditional("UNITY_IPHONE")]
	public static void Assert(bool InCondition)
	{
		DebugHelper.Assert(InCondition, null, null);
	}

	[Conditional("UNITY_STANDALONE_WIN"), Conditional("UNITY_EDITOR"), Conditional("FORCE_LOG"), Conditional("UNITY_IPHONE"), Conditional("UNITY_ANDROID")]
	public static void Assert(bool InCondition, string InFormat)
	{
		DebugHelper.Assert(InCondition, InFormat, null);
	}

	[Conditional("UNITY_EDITOR"), Conditional("FORCE_LOG"), Conditional("UNITY_ANDROID"), Conditional("UNITY_IPHONE"), Conditional("UNITY_STANDALONE_WIN")]
	...
	[Conditional("UNITY_STANDALONE_WIN"), Conditional("UNITY_EDITOR"), Conditional("UNITY_ANDROID"), Conditional("UNITY_IPHONE"), Conditional("FORCE_LOG")]
	...
	[Conditional("UNITY_IPHONE"), Conditional("FORCE_LOG"), Conditional("UNITY_ANDROID"), Conditional("UNITY_STANDALONE_WIN"), Conditional("UNITY_EDITOR")]
