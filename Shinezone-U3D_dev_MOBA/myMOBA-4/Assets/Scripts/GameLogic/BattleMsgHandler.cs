				...
				if (cSPkg != null)
				{
					...
					if (allPlayers != null)
					{
						...
						for (int i = 0; i < count; i++)
						{
							...
							{
								cSPkg.stPkgData.stCltUploadDataRsp.astMemberData[(int)b].dwAcntObjID = allPlayers[i].PlayerId;
								PlayerKDA playerKDA = Singleton<BattleStatistic>.GetInstance().m_playerKDAStat.GetPlayerKDA(allPlayers[i].PlayerId);
								if (playerKDA != null)
								{
								...
								if (allPlayers[i].Captain && allPlayers[i].Captain.handle.ValueComponent != null)
								{
									cSPkg.stPkgData.stCltUploadDataRsp.astMemberData[(int)b].dwBattleCoin = (uint)allPlayers[i].Captain.handle.ValueComponent.GetGoldCoinIncomeInBattle();
								}
								b += 1;
							}
						}
