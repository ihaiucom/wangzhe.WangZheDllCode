	...
	public class SkillSlot
	{
		...
		public int GetSkillCDMax()
		{
			...
			if (this.Actor)
			{
				...
				{
					num4 = ((num4 <= valueDataInfo.maxLimitValue) ? num4 : valueDataInfo.maxLimitValue);
				}
			...
			{
				ValueDataInfo valueDataInfo = this.Actor.handle.ValueComponent.mActorValue[RES_FUNCEFT_TYPE.RES_FUNCEFT_ATKSPDADD];
				DebugHelper.Assert(valueDataInfo != null, "Failed get value data skill 0");
				num = num * 10000 / (10000 + valueDataInfo.totalValue);
				num = ((num >= 0) ? num : 0);
			}
		...
		{
			Skill skill = (this.NextSkillObj == null) ? this.SkillObj : this.NextSkillObj;
			return skill != null && skill.cfgData != null && this.IsEnableSkillSlot() && skill.cfgData.bSkillUseRule == 1 && !false;
		}
		...
		private void RequestUseSkillEnemyHeroSelectMode(uint objID)
		{
			...
			{
				Skill arg_5C_0 = (this.NextSkillObj == null) ? this.SkillObj : this.NextSkillObj;
				FrameCommand<UseObjectiveSkillCommand> frameCommand = FrameCommandFactory.CreateCSSyncFrameCommand<UseObjectiveSkillCommand>();
				frameCommand.cmdData.ObjectID = objID;
				frameCommand.cmdData.SlotType = this.SlotType;
				frameCommand.Send();
		...
		{
			Skill skill = (this.NextSkillObj == null) ? this.SkillObj : this.NextSkillObj;
			return skill.cfgData.dwSkillTargetFilter == 0u || ((ulong)skill.cfgData.dwSkillTargetFilter & (ulong)(1L << (int)(_target.handle.TheActorMeta.ActorType & (ActorTypeDef)31))) <= 0uL;
		}
		...
		public bool IsSkillUseValid(ref SkillUseParam _param)
		{
			...
			if (skill != null && skill.cfgData != null && this.Actor)
			{
				...
				if (skill.AppointType == SkillRangeAppointType.Target || skill.AppointType == SkillRangeAppointType.Auto)
				{
					...
					{
						num += ((_param.TargetActor.handle.ActorControl == null) ? 0L : ((long)_param.TargetActor.handle.ActorControl.GetDetectedRadius()));
					}
					else
					{
						num += ((_param.TargetActor.handle.shape == null) ? 0L : ((long)_param.TargetActor.handle.shape.AvgCollisionRadius));
					}
					long num2 = num * num;
				...
				else if (skill.AppointType == SkillRangeAppointType.Pos)
				{
					long num = skill.cfgData.iMaxAttackDistance * 12000L / 10000L;
					long num2 = num * num;
					long sqrMagnitudeLong2D = (this.Actor.handle.location - _param.UseVector).sqrMagnitudeLong2D;
					if (sqrMagnitudeLong2D <= num2)
					{
						return true;
					}
