		...
		public void BuildSoloGamePlayers(SCPKG_STARTSINGLEGAMERSP svrGameInfo)
		{
			...
			for (int i = 0; i < num; i++)
			{
				...
				if (cOMDT_PLAYERINFO.bObjType != 0)
				{
					...
					{
						uid = ((cOMDT_PLAYERINFO.bObjType != 1) ? 0uL : cOMDT_PLAYERINFO.stDetail.stPlayerOfAcnt.ullUid);
						logicWrold = (uint)((cOMDT_PLAYERINFO.bObjType != 1) ? 0 : cOMDT_PLAYERINFO.stDetail.stPlayerOfAcnt.iLogicWorldID);
						level = cOMDT_PLAYERINFO.dwLevel;
						honorId = cOMDT_PLAYERINFO.stDetail.stPlayerOfAcnt.iHonorID;
						honorLevel = cOMDT_PLAYERINFO.stDetail.stPlayerOfAcnt.iHonorLevel;
						gradeOfRank = (uint)((svrGameInfo.stGameParam.stSingleGameRspOfCombat != null) ? svrGameInfo.stGameParam.stSingleGameRspOfCombat.bGradeOfRank : (byte)0);
			...
			for (int i = 0; i < 2; i++)
			{
				...
				while ((long)num2 < (long)((ulong)svrGameInfo.astCampInfo[i].dwPlayerNum))
				{
					...
					for (int j = 0; j < svrGameInfo.astCampInfo[i].astCampPlayerInfo[num2].stPlayerInfo.astChoiceHero.Length; j++)
					{
						...
						if (dwHeroID != 0)
						{
							bool arg_3B7_0 = (cOMDT_CHOICEHERO.stBaseInfo.stCommonInfo.dwMaskBits & 4u) > 0u && (cOMDT_CHOICEHERO.stBaseInfo.stCommonInfo.dwMaskBits & 1u) == 0u;
							if (player != null)
							{
