	...
	public class Player
	{
		...
		public void AddHero(uint heroCfgId)
		{
			...
			else if (string.IsNullOrEmpty(this.Name))
			{
				...
				string arg = (!actorDataProvider.GetActorStaticData(ref actorMeta, ref actorStaticData)) ? null : actorStaticData.TheResInfo.Name;
				this.Name = string.Format("{0}[{1}]", arg, Singleton<CTextManager>.GetInstance().GetText("PVP_NPC"));
			}
		...
		public bool IsAllHeroesDead()
		{
			...
			{
				if (!this._heroes[i].handle.ActorControl.IsDeadState)
				{
					return false;
				}
			}
		...
		public bool IsMyTeamOutOfBattle()
		{
			for (int i = 0; i < this._heroes.Count; i++)
			{
				if (this._heroes[i].handle.ActorControl.IsInBattle)
				{
					return false;
				}
			}
		...
		public int GetAllHeroCombatEft()
		{
			...
			for (int i = 0; i < this._heroes.Count; i++)
			{
				num += Player.GetHeroCombatEft(this._heroes[i]);
			}
			return num;
		}
