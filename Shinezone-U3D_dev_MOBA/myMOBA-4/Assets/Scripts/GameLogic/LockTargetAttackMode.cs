...
namespace Assets.Scripts.GameLogic
{
	public class LockTargetAttackMode : BaseAttackMode
	{
		...
		public bool IsValidSkillTargetID(uint _targetID, uint _targetMask)
		{
			...
			return actor && ((ulong)_targetMask & (ulong)(1L << (int)(actor.handle.TheActorMeta.ActorType & (ActorTypeDef)31))) <= 0uL;
		}
		...
		public override uint SelectSkillTarget(SkillSlot _slot)
		{
			...
			{
				ActorRoot actorRoot = _slot.skillIndicator.GetUseSkillTargetLockAttackMode();
				if (actorRoot != null)
				{
					if (this.IsValidLockTargetID(actorRoot.ObjID))
					{
						num = actorRoot.ObjID;
						Singleton<NetLockAttackTarget>.GetInstance().SendLockAttackTarget(num);
					}
		...
		{
			bool result = false;
			Skill arg_1E_0 = (_slot.NextSkillObj == null) ? _slot.SkillObj : _slot.NextSkillObj;
