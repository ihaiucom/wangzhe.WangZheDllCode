...
namespace Assets.Scripts.GameLogic
{
	public class GameInput : Singleton<GameInput>
	{
		...
		public void UpdateEscape()
		{
			...
			{
				if ((DateTime.Now - this.lastClickEscape).TotalMilliseconds < 1500.0)
				{
		...
		private void SendMoveDirection(int moveDegree)
		{
            ...
            if ((int)nDirMoveSeq < FrameSynchr.MoveCmdBuff_Size && nDirMoveSeq >= 0)
			{
				Singleton<FrameSynchr>.GetInstance().m_MoveCMDSendTime[(int)nDirMoveSeq] = (uint)(Time.realtimeSinceStartup * 1000f);
			}
            byte nSeq = nDirMoveSeq;
            nDirMoveSeq = (byte)(nSeq + 1);
