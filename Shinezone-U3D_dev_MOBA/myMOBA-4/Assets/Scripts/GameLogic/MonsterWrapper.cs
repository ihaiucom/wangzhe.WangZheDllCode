	...
	public class MonsterWrapper : ObjWrapper
	{
		...
		public override void UpdateLogic(int delta)
		{
			...
			else
			{
				...
				if (this.actorSubSoliderType == 8 && this.myBehavior == ObjBehaviMode.State_AutoAI && !this.actor.SkillControl.isUsing)
				{
					...
					{
						VInt3 location = heroActors[i].handle.location;
		...
		protected override void OnDead()
		{
			...
			if (this.actorSubSoliderType == 8 && this.myLastAtker)
			{
				...
				int num = (actorCamp != COM_PLAYERCAMP.COM_PLAYERCAMP_1) ? this.cfgInfo.iKillByCamp2ChangeSoldierWave : this.cfgInfo.iKillByCamp1ChangeSoldierWave;
				if (num > 0)
				{
						...
						if (this.actorSubSoliderType == 8)
						{
							...
							if (Vector3.Dot(vec, rhs) < 0f)
							{
								...
								else
								{
									...
									{
										Vector3 rhs2 = Singleton<GameObjMgr>.instance.HeroActors[i].handle.location.vec3 - this.actor.location.vec3;
										VInt3 vInt = Singleton<GameObjMgr>.instance.HeroActors[i].handle.location - this.actor.location;
