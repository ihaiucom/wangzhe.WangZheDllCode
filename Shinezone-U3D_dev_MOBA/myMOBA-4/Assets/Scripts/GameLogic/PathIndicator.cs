...
namespace Assets.Scripts.GameLogic
{
	public class PathIndicator : MonoBehaviour
	{
		...
		private void Clear()
		{
			...
			{
				GameObject gameObject = this.m_parCachePool[i] as GameObject;
				if (gameObject != null)
				{
		...
		private void GetParFromCache(int inNeedNum)
		{
			...
			if (i < this.m_parCachePool.Count)
			{
				for (int j = this.m_parCacheIndex + 1; j <= i; j++)
				{
					GameObject gameObject = this.m_parCachePool[j] as GameObject;
					if (gameObject)
					{
			...
			{
				while (i >= this.m_parCachePool.Count)
				{
					for (int k = 0; k < 25; k++)
					{
						bool flag = false;
		...
		private void ReturnToParCache(int inReturnNum)
		{
			...
			int num2 = this.m_parCachePool.Count - this.m_parCacheIndex;
			if (num2 >= 50)
			{
				int num3 = this.m_parCachePool.Count - 1;
				int num4 = this.m_parCachePool.Count - 50;
				for (int j = num3; j > num4; j--)
				{
					GameObject gameObject2 = this.m_parCachePool[j] as GameObject;
					if (gameObject2 != null)
					{
