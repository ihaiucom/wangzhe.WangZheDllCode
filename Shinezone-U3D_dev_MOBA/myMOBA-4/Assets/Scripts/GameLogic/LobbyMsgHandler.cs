...
namespace Assets.Scripts.GameLogic
{
	...
	public class LobbyMsgHandler
	{
		...
		public static void onLobbyOffingRestartReq(CSPkg msg)
		{
			...
			if (bytes.Length > 0)
			{
				Buffer.BlockCopy(bytes, 0, cSPkg.stPkgData.stOffingRestartRsp.szCltIMEI, 0, (bytes.Length <= 64) ? bytes.Length : 64);
			}
		...
		public static void onMultiGameSettle(CSPkg msg)
		{
			...
			for (int i = 0; i < 8; i++)
			{
				if (((ulong)msg.stPkgData.stMultGameSettleGain.stDetail.dwTipsMask & (ulong)(1L << (i & 31))) != 0uL)
				{
					int num = i;
					if (num == 1)
					{
