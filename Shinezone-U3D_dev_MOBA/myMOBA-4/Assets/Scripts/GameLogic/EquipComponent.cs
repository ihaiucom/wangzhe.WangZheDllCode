...
namespace Assets.Scripts.GameLogic
{
	public class EquipComponent : LogicComponent
	{
		...
		{
			int i = 0;
			while (i < this.m_equipPassiveCdList.Count)
			{
				this.m_equipPassiveCdList[i].m_passiveCd -= nDelta;
				if (this.m_equipPassiveCdList[i].m_passiveCd <= 0)
				{
			...
			int j = 0;
			while (j < this.m_equipActiveSkillCdList.Count)
			{
				this.m_equipActiveSkillCdList[j].m_passiveCd -= nDelta;
				if (this.m_equipActiveSkillCdList[j].m_passiveCd <= 0)
				{
		...
		{
			return this.m_recommendEquipInfos[(int)((UIntPtr)this.m_usedRecommendEquipGroup)];
		}
		...
		public void UpdateEquipEffect()
		{
			...
			{
				KeyValuePair<ushort, CEquipBuffInfoGroup> current2 = enumerator2.Current;
				this.UpdateEquipBuff(current2.Value);
			}
		}
		...
		private void AddEquipPassiveSkill(ushort equipID, uint passiveSkillID, ushort passiveSkillGroupID, uint equipBuyPrice, uint[] passiveSkillRelatedFuncIDs, uint sequence)
		{
			...
			{
				this.m_equipPassiveSkillInfoMap.TryGetValue(passiveSkillGroupID, out cEquipPassiveSkillInfoGroup);
			}
			else
			{
		...
		private void AddEquipBuff(ushort equipID, uint buffID, ushort buffGroupID, uint equipBuyPrice, uint sequence)
		{
			...
			{
				this.m_equipBuffInfoMap.TryGetValue(buffGroupID, out cEquipBuffInfoGroup);
			}
			else
			{
		...
		private void InitHorizonEquipID()
		{
			...
			if (usageLevelEquipList != null)
			{
				...
				{
					ResEquipInBattle dataByKey = GameDataMgr.m_equipInBattleDatabin.GetDataByKey((uint)usageLevelEquipList[i]);
		...
		private void AddEquipToBuyHistory(ushort equipID)
		{
			...
			{
				uint value = (uint)(Singleton<FrameSynchr>.instance.LogicFrameTick / 1000uL);
				this.m_equipBoughtHistory.Add(equipID, value);
			}
			int num = 0;
			while ((long)num < 3L)
			{
				...
				while ((long)num2 < 6L)
				{
					if (this.m_recommendEquipInfos[num][num2].m_equipID == equipID && !this.m_recommendEquipInfos[num][num2].m_hasBeenBought)
					{
						this.m_recommendEquipInfos[num][num2].m_hasBeenBought = true;
						break;
					}
					num2++;
				}
		...
		public void AddEquipActiveSkillCdInfo(int iEquipSlotIndex, int cd)
		{
			if (this.m_equipInfos != null && iEquipSlotIndex >= 0 && iEquipSlotIndex < 6)
			{
				...
				if (equipResInfo != null)
				{
					uint passiveSkillId = (equipResInfo.dwActiveSkillGroupID == 0u) ? equipResInfo.dwActiveSkillID : equipResInfo.dwActiveSkillGroupID;
		...
		public bool GetEquipActiveSkillCdInfo(int iEquipSlotIndex, out int cd)
		{
			...
			if (this.m_equipInfos != null && iEquipSlotIndex >= 0 && iEquipSlotIndex < 6 && this.m_equipActiveSkillCdList != null)
			{
				...
				uint num = (equipResInfo.dwActiveSkillGroupID == 0u) ? equipResInfo.dwActiveSkillID : equipResInfo.dwActiveSkillGroupID;
		...
		{
			return this.m_rcmdEquipPlanName[(int)((UIntPtr)this.m_usedRecommendEquipGroup)];
		}
