...
namespace Assets.Scripts.GameLogic
{
	public class LobbyLogic : Singleton<LobbyLogic>, IUpdateLogic
	{
		...
		public override void Init()
		{
			...
			CUIManager expr_2F = Singleton<CUIManager>.GetInstance();
			expr_2F.onFormSorted = (CUIManager.OnFormSorted)Delegate.Combine(expr_2F.onFormSorted, new CUIManager.OnFormSorted(this.OnUiFormUpdate));
		...
		public void LoginGame()
		{
			if (!this.isLogin)
			{
                Singleton<Assets.Scripts.Framework.GameLogic>.GetInstance().ClearLogicData();
				...
				if (bytes.Length > 0)
				{
					Buffer.BlockCopy(bytes, 0, cSPkg.stPkgData.stGameLoginReq.szCltIMEI, 0, (bytes.Length <= 64) ? bytes.Length : 64);
				}
		...
		public void ReqSingleGameFinish(bool ClickGameOver = false, bool bGmWin = false)
		{
			if (this.isLogin)
			{
				...
				{
					b = ((!Singleton<WinLose>.instance.LastSingleGameWin) ? (byte)2 : (byte)1);
				}
		...
		public void ReqMultiGameOver(bool ClickGameOver = false)
		{
			...
			if (this.isLogin && battleStat != null)
			{
				...
				for (int i = 0; i < list.Count; i++)
				{
					if (!list[i].Computer)
					{
						if (list[i].PlayerCamp == COM_PLAYERCAMP.COM_PLAYERCAMP_1)
						{
						...
						if (list[i] == Singleton<GamePlayerCenter>.instance.GetHostPlayer())
						{
							cSPkg.stPkgData.stMultGameOverReq.astAcntInfo[num3].stBattleParam.wNetworkMode = (ushort)this.CheckWifi();
							Singleton<DataReportSys>.GetInstance().Report(ref cSPkg.stPkgData.stMultGameOverReq.stClientRecordData, list[i].PlayerId);
						}
						else
						{
						...
						cSPkg.stPkgData.stMultGameOverReq.astAcntInfo[num3].dwAcntObjID = list[i].PlayerId;
						cSPkg.stPkgData.stMultGameOverReq.astAcntInfo[num3].stCommonData = this.CreateReportData(list[i].PlayerId, flag, flag);
						num3++;
						if (num3 > 4)
						{
			...
			for (int i = 0; i < curLvelContext.m_starDetail.Length; i++)
			{
				...
				if (dataByKey != null)
				{
					...
					if (cOMDT_STATISTIC_STRUCT_PILE == null)
					{
						...
						COMDT_STATISTIC_DATA expr_D5 = CommonData.stStatisticData;
						expr_D5.bNum += 1;
					}
		...
		private void CreateHeroBaseInfoData(HeroKDA HeroInfo, ref COMDT_HERO_BASE_INFO stHeroDetailInfo)
		{
			...
			if (HeroInfo.actorHero.handle.MovementComponent != null)
			{
				...
				playerMovement.m_uiMoveIntervalMax = ((playerMovement.m_uiMoveIntervalMax <= num) ? num : playerMovement.m_uiMoveIntervalMax);
		...
		private void CreateBattleStatisticInfoData(HeroKDA HeroInfo, ref COMDT_HERO_BATTLE_STATISTIC_INFO stBattleStatisticInfo)
		{
			...
			{
                stBattleStatisticInfo.bEyeNum = ((HeroInfo.actorHero.handle.SkillControl.stSkillStat.m_uiSpawnEyeTimes >= 15u) ? (byte)15 : ((byte)HeroInfo.actorHero.handle.SkillControl.stSkillStat.m_uiSpawnEyeTimes));
			...
			if (HeroInfo.actorHero && HeroInfo.actorHero.handle.SkillControl != null && HeroInfo.actorHero.handle.SkillControl.stSkillStat != null && HeroInfo.actorHero.handle.SkillControl.stSkillStat.SkillStatistictInfo != null)
			{
				...
				for (int i = 0; i < 5; i++)
				{
					if (skillSlotArray[i] != null)
					{
						...
						skillStatistictInfo[i].uiCDIntervalMax = ((skillStatistictInfo[i].uiCDIntervalMax <= num) ? num : skillStatistictInfo[i].uiCDIntervalMax);
						if (i > 0)
						{
			...
			while (enumerator.MoveNext())
			{
				if (enumerator.Current.actorHero && enumerator.Current.actorHero.handle.ValueComponent != null)
				{
					...
					if (captain.handle.EquipComponent != null)
					{
						...
						byte b2 = (byte)equipBoughtHistory.Count;
                        b2 = ((b2 >= 30) ? (byte)30 : (byte)b2);
						...
						{
							COMDT_SETTLE_INBATTLE_EQUIP_INFO arg_28E_0 = CommonData.stHeroData.astHeroList[(int)b].astInBattleEquipInfo[num];
							KeyValuePair<ushort, uint> current = enumerator2.Current;
							arg_28E_0.dwEquipID = (uint)current.Key;
							COMDT_SETTLE_INBATTLE_EQUIP_INFO arg_2B9_0 = CommonData.stHeroData.astHeroList[(int)b].astInBattleEquipInfo[num];
							KeyValuePair<ushort, uint> current2 = enumerator2.Current;
							arg_2B9_0.dwFirstBuyTime = current2.Value;
							if (++num >= (int)b2)
							{
								break;
							}
					...
					CommonData.stHeroData.astHeroList[(int)b].stHeroBattleInfo.iHeroEndLive = ((enumerator.Current.actorHero.handle.ActorControl.myBehavior != ObjBehaviMode.State_Dead) ? 1 : 0);
		...
		private void CreateDestroyReportData(ref COMDT_SETTLE_COMMON_DATA CommonData, bool bGMWin)
		{
			...
			if (destroyStat.Count != 0)
			{
				...
				while (enumerator.MoveNext())
				{
					...
					Dictionary<int, DestroyStat> value = current.Value;
					...
					while (enumerator2.MoveNext())
					{
						...
						int[] arg_AC_0 = astDetail[num].stKeyInfo.KeyDetail;
						int arg_AC_1 = 0;
						KeyValuePair<uint, Dictionary<int, DestroyStat>> current2 = enumerator.Current;
						arg_AC_0[arg_AC_1] = (int)current2.Key;
						int[] arg_CC_0 = astDetail[num].stKeyInfo.KeyDetail;
						int arg_CC_1 = 1;
						KeyValuePair<int, DestroyStat> current3 = enumerator2.Current;
						arg_CC_0[arg_CC_1] = current3.Key;
						astDetail[num].stKeyInfo.KeyDetail[2] = 1;
						astDetail[num].stValueInfo.bValueNum = 1;
						int[] arg_115_0 = astDetail[num].stValueInfo.ValueDetail;
						int arg_115_1 = 0;
						KeyValuePair<int, DestroyStat> current4 = enumerator2.Current;
						arg_115_0[arg_115_1] = current4.Value.CampEnemyNum;
						num++;
						int arg_134_0 = num2;
						KeyValuePair<int, DestroyStat> current5 = enumerator2.Current;
						num2 = arg_134_0 + current5.Value.CampEnemyNum;
						astDetail[num].stKeyInfo.bKeyNum = 3;
						int[] arg_164_0 = astDetail[num].stKeyInfo.KeyDetail;
						int arg_164_1 = 0;
						KeyValuePair<uint, Dictionary<int, DestroyStat>> current6 = enumerator.Current;
						arg_164_0[arg_164_1] = (int)current6.Key;
						int[] arg_184_0 = astDetail[num].stKeyInfo.KeyDetail;
						int arg_184_1 = 1;
						KeyValuePair<int, DestroyStat> current7 = enumerator2.Current;
						arg_184_0[arg_184_1] = current7.Key;
						astDetail[num].stKeyInfo.KeyDetail[2] = 0;
						astDetail[num].stValueInfo.bValueNum = 1;
						int[] arg_1CD_0 = astDetail[num].stValueInfo.ValueDetail;
						int arg_1CD_1 = 0;
						KeyValuePair<int, DestroyStat> current8 = enumerator2.Current;
						arg_1CD_0[arg_1CD_1] = current8.Value.CampSelfNum;
						num++;
						int arg_1EC_0 = num3;
						KeyValuePair<int, DestroyStat> current9 = enumerator2.Current;
						num3 = arg_1EC_0 + current9.Value.CampSelfNum;
					}
					astDetail[num].stKeyInfo.bKeyNum = 3;
					int[] arg_228_0 = astDetail[num].stKeyInfo.KeyDetail;
					int arg_228_1 = 0;
					KeyValuePair<uint, Dictionary<int, DestroyStat>> current10 = enumerator.Current;
					arg_228_0[arg_228_1] = (int)current10.Key;
					...
					num++;
					astDetail[num].stKeyInfo.bKeyNum = 3;
					int[] arg_29D_0 = astDetail[num].stKeyInfo.KeyDetail;
					int arg_29D_1 = 0;
					KeyValuePair<uint, Dictionary<int, DestroyStat>> current11 = enumerator.Current;
					arg_29D_0[arg_29D_1] = (int)current11.Key;
					astDetail[num].stKeyInfo.KeyDetail[1] = 0;
					astDetail[num].stKeyInfo.KeyDetail[2] = 0;
					astDetail[num].stValueInfo.bValueNum = 1;
				...
				COMDT_STATISTIC_DATA expr_317 = CommonData.stStatisticData;
				expr_317.bNum += 1;
			}
		...
		private void FakeWithCondition(ref ResDT_ConditionInfo ConditionInfo, ref COMDT_STATISTIC_STRUCT_PILE StatRef, ref COMDT_STATISTIC_BASE_STRUCT[] DetailData)
		{
			...
			COMDT_STATISTIC_STRUCT_PILE expr_107 = StatRef;
			expr_107.wNum += 1;
		}
		...
		private void CreateSettleReportData(uint playerId, ref COMDT_SETTLE_COMMON_DATA CommonData, bool bGMWin, bool bWin)
		{
			...
			else
			{
				...
				if (mvpPlayer2 != 0u)
				{
					num7 = ((mvpPlayer2 != playerKDA.PlayerId) ? 0 : 1);
				}
			}
			int num8 = 0;
			...
			astDetail[num8].stValueInfo.bValueNum = 1;
			astDetail[num8].stValueInfo.ValueDetail[0] = ((!flag) ? 0 : 1);
			num8++;
			astDetail[num8].stKeyInfo.bKeyNum = 1;
			astDetail[num8].stKeyInfo.KeyDetail[0] = 30;
			astDetail[num8].stValueInfo.bValueNum = 1;
			astDetail[num8].stValueInfo.ValueDetail[0] = ((!flag2) ? 0 : 1);
			num8++;
			astDetail[num8].stKeyInfo.bKeyNum = 1;
			astDetail[num8].stKeyInfo.KeyDetail[0] = 31;
			astDetail[num8].stValueInfo.bValueNum = 1;
			astDetail[num8].stValueInfo.ValueDetail[0] = ((!flag3) ? 0 : 1);
			num8++;
			astDetail[num8].stKeyInfo.bKeyNum = 1;
			astDetail[num8].stKeyInfo.KeyDetail[0] = 32;
			astDetail[num8].stValueInfo.bValueNum = 1;
			astDetail[num8].stValueInfo.ValueDetail[0] = ((!flag5) ? 0 : 1);
			num8++;
			astDetail[num8].stKeyInfo.bKeyNum = 1;
			astDetail[num8].stKeyInfo.KeyDetail[0] = 33;
			astDetail[num8].stValueInfo.bValueNum = 1;
			astDetail[num8].stValueInfo.ValueDetail[0] = ((!flag4) ? 0 : 1);
			num8++;
			astDetail[num8].stKeyInfo.bKeyNum = 1;
			astDetail[num8].stKeyInfo.KeyDetail[0] = 34;
			astDetail[num8].stValueInfo.bValueNum = 1;
			astDetail[num8].stValueInfo.ValueDetail[0] = ((!flag6) ? 0 : 1);
			num8++;
			astDetail[num8].stKeyInfo.bKeyNum = 1;
			...
			astDetail[num8].stValueInfo.bValueNum = 1;
			astDetail[num8].stValueInfo.ValueDetail[0] = ((battleDeadStat.m_uiTakeFBPlayerId != playerId) ? 0 : 1);
			num8++;
			astDetail[num8].stKeyInfo.bKeyNum = 1;
			...
			COMDT_STATISTIC_DATA expr_E9C = CommonData.stStatisticData;
			expr_E9C.bNum += 1;
		...
		private void CreateAttrReportData(ref COMDT_SETTLE_COMMON_DATA CommonData, bool bGMWin)
		{
			...
			CommonData.stStatisticData.astDetail[(int)CommonData.stStatisticData.bNum].wNum = (ushort)num;
			COMDT_STATISTIC_DATA expr_1C4 = CommonData.stStatisticData;
			expr_1C4.bNum += 1;
		}
		...
		private void CreateGeneralData(uint playerId, ref COMDT_SETTLE_COMMON_DATA CommonData, bool bGMWin)
		{
			...
			{
				CShenFuStat.ShenFuRecord shenFuRecord2 = shenFuRecord[k];
				...
				{
					List<int> list2 = dictionary[(int)shenFuRecord2.shenFuId];
			...
			foreach (int current in dictionary.Keys)
			{
				...
				stGeneralData.astRuneTypePickUpNum[num].dwPickUpNum = (uint)dictionary[current].Count;
				num++;
			}
			...
			{
				stGeneralData.BigDragonBattleToDeadTimeDtail[(int)((UIntPtr)num9)] = (uint)Singleton<BattleStatistic>.instance.m_battleDeadStat.GetRecordAtIndex(COM_PLAYERCAMP.COM_PLAYERCAMP_MID, ActorTypeDef.Actor_Type_Monster, 2, 8, (int)num9).fightTime;
			}
			if (curLvelContext != null && curLvelContext.IsMobaModeWithOutGuide())
			{
				if (curLvelContext.m_pvpPlayerNum == 10)
				{
					...
					{
						stGeneralData.DragonBattleToDeadTimeDtail[(int)((UIntPtr)num11)] = (uint)Singleton<BattleStatistic>.instance.m_battleDeadStat.GetRecordAtIndex(COM_PLAYERCAMP.COM_PLAYERCAMP_MID, ActorTypeDef.Actor_Type_Monster, 2, 9, (int)num11).fightTime;
					}
				}
				else
				{
					...
					{
						stGeneralData.DragonBattleToDeadTimeDtail[(int)((UIntPtr)num13)] = (uint)Singleton<BattleStatistic>.instance.m_battleDeadStat.GetRecordAtIndex(COM_PLAYERCAMP.COM_PLAYERCAMP_MID, ActorTypeDef.Actor_Type_Monster, 2, 7, (int)num13).fightTime;
					}
				}
				if (curLvelContext.m_pvpPlayerNum == 10)
				{
					...
					{
						stGeneralData.MonsterDeadDetail[(int)((UIntPtr)num15)] = (uint)Singleton<BattleStatistic>.instance.m_battleDeadStat.GetDeadNum(COM_PLAYERCAMP.COM_PLAYERCAMP_MID, ActorTypeDef.Actor_Type_Monster, 2, array[(int)((UIntPtr)num15)]);
					}
				}
				else
				{
					...
					{
						stGeneralData.MonsterDeadDetail[(int)((UIntPtr)num17)] = (uint)Singleton<BattleStatistic>.instance.m_battleDeadStat.GetDeadNum(COM_PLAYERCAMP.COM_PLAYERCAMP_MID, ActorTypeDef.Actor_Type_Monster, 2, array2[(int)((UIntPtr)num17)]);
					}
				}
			}
			...
			if (nonHeroInfo.TryGetValue(2u, out dictionaryView))
			{
				...
				else
				{
					...
					{
						PoolObjHandle<ActorRoot> poolObjHandle = organActors[n];
			...
			if (nonHeroInfo.TryGetValue(2u, out dictionaryView))
			{
				COM_PLAYERCAMP key = (playerCamp2 != COM_PLAYERCAMP.COM_PLAYERCAMP_1) ? COM_PLAYERCAMP.COM_PLAYERCAMP_1 : COM_PLAYERCAMP.COM_PLAYERCAMP_2;
				NONHERO_STATISTIC_INFO nONHERO_STATISTIC_INFO;
				if (dictionaryView.TryGetValue((uint)key, out nONHERO_STATISTIC_INFO))
				{
				...
				else
				{
					...
					for (int num18 = 0; num18 < organActors2.Count; num18++)
					{
						PoolObjHandle<ActorRoot> poolObjHandle2 = organActors2[num18];
			...
			{
				COM_PLAYERCAMP key2 = (playerCamp2 != COM_PLAYERCAMP.COM_PLAYERCAMP_1) ? COM_PLAYERCAMP.COM_PLAYERCAMP_1 : COM_PLAYERCAMP.COM_PLAYERCAMP_2;
				NONHERO_STATISTIC_INFO nONHERO_STATISTIC_INFO;
				if (dictionaryView.TryGetValue((uint)key2, out nONHERO_STATISTIC_INFO))
				{
			...
			stGeneralData.iBossCount = list3.Count;
			for (int num19 = 0; num19 < list3.Count; num19++)
			{
				stGeneralData.iBossHPMax = Math.Max(stGeneralData.iBossHPMax, list3[num19].handle.ValueComponent.ObjValueStatistic.iActorMaxHp);
				if (list3[num19].handle.ValueComponent.ObjValueStatistic.iActorMinHp != -1)
				{
					if (num19 == 0)
					{
						stGeneralData.iBossHPMin = list3[num19].handle.ValueComponent.ObjValueStatistic.iActorMinHp;
					}
					else
					{
						stGeneralData.iBossHPMin = Math.Min(stGeneralData.iBossHPMin, list3[num19].handle.ValueComponent.ObjValueStatistic.iActorMinHp);
					}
				}
				for (int num20 = 0; num20 < 10; num20++)
				{
					if (list3[num19] && list3[num19].handle.SkillControl != null && list3[num19].handle.SkillControl.stSkillStat != null && list3[num19].handle.SkillControl.stSkillStat.SkillStatistictInfo != null)
					{
						SKILLSTATISTICTINFO[] skillStatistictInfo = list3[num19].handle.SkillControl.stSkillStat.SkillStatistictInfo;
			...
			for (int num21 = 0; num21 < organActors3.Count; num21++)
			{
				PoolObjHandle<ActorRoot> poolObjHandle3 = organActors3[num21];
			...
			stGeneralData.OperateType[6] = ((!GameSettings.ShowEnemyHeroHeadBtnMode) ? 0u : 1u);
			stGeneralData.OperateType[7] = ((!GameSettings.LunPanLockEnemyHeroMode) ? 0u : 1u);
			stGeneralData.OperateType[8] = (uint)GameSettings.TheLastHitMode;
			stGeneralData.OperateType[9] = ((!GameSettings.ShowEquipInfo) ? 0u : 1u);
		...
		private void FillVDStat(ref COMDT_SETTLE_GAME_GENERAL_INFO stGeneralData, uint playerID)
		{
			if (Singleton<BattleStatistic>.instance.m_vdStat != null)
			{
				...
				if (player != null)
				{
                    ...
                    stGeneralData.bAdvantageNum = (((byte)Singleton<BattleStatistic>.instance.m_vdStat.count >= 16) ? (byte)16 : ((byte)Singleton<BattleStatistic>.instance.m_vdStat.count));
		...
		public bool ExecOpenForm(string[] prms = null)
		{
			...
			int targetId = 0;
			int targetId2 = 0;
			int.TryParse(this._lobbyOpenFormCmd[1], out entranceType);
			if (this._lobbyOpenFormCmd.Length > 2)
			{
				int.TryParse(this._lobbyOpenFormCmd[2], out targetId);
			}
			if (this._lobbyOpenFormCmd.Length > 3)
			{
				int.TryParse(this._lobbyOpenFormCmd[3], out targetId2);
			}
			this._lobbyOpenFormCmd = null;
