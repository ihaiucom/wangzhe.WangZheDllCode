...
namespace Assets.Scripts.GameLogic
{
	public class EffectPlayComponent : LogicComponent
	{
		...
		private void ClearVariables()
		{
			...
			{
				Singleton<CGameObjectPool>.GetInstance().RecycleGameObject(this.soulSuckObjList[i].EftObj);
			}
		...
		public override void UpdateLogic(int delta)
		{
			...
			int i = 0;
			while (i < this.soulSuckObjList.Count)
			{
				DuraEftPlayParam value = this.soulSuckObjList[i];
				if (this.UpdateSuckSoulEftMove(ref value, delta))
				{
					Singleton<CGameObjectPool>.GetInstance().RecycleGameObject(value.EftObj);
					this.soulSuckObjList.RemoveAt(i);
					this.PlayHunHurtEft();
				}
				else
				{
					this.soulSuckObjList[i] = value;
					i++;
				}
			}
		}
		...
		public void PlaySuckSoulEft(PoolObjHandle<ActorRoot> src)
		{
			...
			if (pooledGameObjLOD != null)
			{
				DuraEftPlayParam item = default(DuraEftPlayParam);
				item.EftObj = pooledGameObjLOD;
				item.RemainMSec = EffectPlayComponent.s_suckSoulMSec;
