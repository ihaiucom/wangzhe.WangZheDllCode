...
namespace Assets.Scripts.GameLogic
{
	public class MapWrapper : MonoBehaviour, IUpdateLogic
	{
				...
				if (!(funcRegion == null))
				{
					if (funcRegion.enabled && funcRegion.gameObject.activeInHierarchy && funcRegion.gameObject.activeSelf)
					{
						...
						else if (funcRegion is AreaEventTrigger)
						{
							...
							for (int j = this.objTriggerMultiMap.Count - 1; j >= 0; j--)
							{
								MapWrapper.ObjTriggerKeyValuePair objTriggerKeyValuePair = this.objTriggerMultiMap[j];
		...
		public void UpdateLogic(int delta)
		{
			...
			int count3 = this.objTriggerMultiMap.Count;
			for (int l = 0; l < count3; l++)
			{
				MapWrapper.ObjTriggerKeyValuePair objTriggerKeyValuePair = this.objTriggerMultiMap[l];
