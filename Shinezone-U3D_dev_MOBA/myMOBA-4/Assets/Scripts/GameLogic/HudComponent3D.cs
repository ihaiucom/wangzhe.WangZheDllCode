...
namespace Assets.Scripts.GameLogic
{
	public class HudComponent3D : LogicComponent
	{
		...
		private bool IsHostView
		{
			get
			{
				return (!Singleton<WatchController>.GetInstance().IsWatching) ? this.actor.IsHostCamp() : (this.actor.TheActorMeta.ActorCamp == COM_PLAYERCAMP.COM_PLAYERCAMP_1);
			}
		}
		...
		private void ShowBuffListTop()
		{
			...
			if (this.m_buffText.Count == 1)
			{
				...
				this.ShowPlayerName(this.m_buffText[0].buffIcon);
			}
			else
			{
				if (this.m_buffText.Count <= 1)
				...
				BuffIcon buffIcon = this.m_buffText[this.m_buffText.Count - 1];
				uint num;
				if (uint.TryParse(buffIcon.color, NumberStyles.AllowHexSpecifier, null, out num))
				{
		...
		public void RemoveBuffText(string txt, string color = "")
		{
			...
			for (int i = 0; i < this.m_buffText.Count; i++)
			{
				if (this.m_buffText[i].buffIcon == txt)
				{
			...
			if (this.HudType != HudCompType.Type_Hide && this.m_hud == null)
			{
				...
				if (flag)
				{
					...
					if (this.m_bloodImage != null)
					{
						...
						{
							this.m_bloodImage.spriteName = ((this.actor.TheActorMeta.ActorCamp != COM_PLAYERCAMP.COM_PLAYERCAMP_1) ? "Battle_redHp" : "Battle_blueHp");
						}
						else if (flag2)
						{
						...
						{
							this.m_bloodImage.spriteName = ((!this.actor.IsHostCamp()) ? "Battle_redHp" : "Battle_blueHp");
						}
				...
				else if (this.HudType != HudCompType.Type_Eye)
				{
					...
					component.spriteName = Enum.GetName(typeof(SpriteNameEnum), (int)ActorTypeDef.Actor_Type_EYE * (int)actorTypeDef);
					...
					{
						this.m_bloodImage.spriteName = Enum.GetName(typeof(SpriteNameEnum), (int)ActorTypeDef.Actor_Type_EYE * (int)actorTypeDef + ((!this.IsHostView) ? 2 : 1));
					}
					...
					if (componetInChild != null)
					{
						if (flag4)
						{
							componetInChild.spriteName = ((!this.IsHostView) ? SpriteNameEnum.bl_icon_organ_enemy.ToString() : SpriteNameEnum.bl_icon_organ_self.ToString());
				...
				else
				{
					...
					component2.spriteName = Enum.GetName(typeof(SpriteNameEnum), (int)ActorTypeDef.Actor_Type_EYE * (int)actorType);
					this.m_bloodImage = this.m_hud.transform.Find("Fore").GetComponent<Sprite3D>();
					this.m_bloodImage.spriteName = Enum.GetName(typeof(SpriteNameEnum), (int)ActorTypeDef.Actor_Type_EYE * (int)actorType + ((!this.IsHostView) ? 2 : 1));
					if (this.actor.ValueComponent.actorHpTotal > 0)
					{
						this.m_bloodImage.fillAmount = (float)this.actor.ValueComponent.actorHp / (float)this.actor.ValueComponent.actorHpTotal;
					}
		...
		private void initHudIcon(SLevelContext levelContext, bool bMiniMap, out GameObject mapPointer)
		{
			...
			if (this.actor.TheActorMeta.ActorType == ActorTypeDef.Actor_Type_Hero || this.actor.TheActorMeta.ActorType == ActorTypeDef.Actor_Type_Call)
			{
				...
				if (heroIconObj != null)
				{
					heroIconObj.CustomSetActive((!bMiniMap) ? this.m_mapPointer_big_Trans.gameObject.activeSelf : this.m_mapPointer_small_Trans.gameObject.activeSelf);
					if (bMiniMap)
					{
			...
			{
				string prefabPath = (!this.IsHostView) ? MinimapSys.enemy_Eye : MinimapSys.self_Eye;
		...
		private void ResetHudUI()
		{
			...
			this.m_bloodImage.spriteName = Enum.GetName(typeof(SpriteNameEnum), (int)ActorTypeDef.Actor_Type_EYE * (int)actorTypeDef + ((!isHostView) ? 2 : 1));
			if (this.HudType != HudCompType.Type_Eye)
			{
				this.m_bloodImage.SetNativeSize(Singleton<Camera_UI3D>.GetInstance().GetCurrentCamera(), 30f);
			}
		...
		public void OnActorDead()
		{
			...
			{
				MiniMapEffectModule miniMapEffectModule = (Singleton<CBattleSystem>.instance.TheMinimapSys == null) ? null : Singleton<CBattleSystem>.instance.TheMinimapSys.miniMapEffectModule;
		...
		protected void UpdateHpAndShieldBarEffect()
		{
			...
			int num = (this._curShield1 <= 0) ? this.actor.ValueComponent.actorHpTotal : Math.Max(this.actor.ValueComponent.actorHpTotal, this._curShield1 + this.actor.ValueComponent.actorHp);
			if ((float)num > 0f)
			{
		...
		private Transform GetHudPanel(HudCompType hudType, bool isHostCamp)
		{
			...
			{
				text2 = ((!isHostCamp) ? enBattleFormWidget.Panel_EnemySoliderHud.ToString() : enBattleFormWidget.Panel_SelfSoliderHud.ToString());
			}
			else if (hudType == HudCompType.Type_Hero || hudType == HudCompType.Type_Organ)
			{
				text2 = ((!isHostCamp) ? enBattleFormWidget.Panel_EnemyHeroHud.ToString() : enBattleFormWidget.Panel_SelfHeroHud.ToString());
			}
		...
		public void ShowStatus(StatusHudType st)
		{
			...
			if (!gameObject)
			{
				UnityEngine.Object content = Singleton<CResourceManager>.GetInstance().GetResource(HudComponent3D._statusResPath[(int)st], typeof(GameObject), enResourceType.BattleScene, false, false).m_content;
				if (content)
				{
		...
		public void HideStatus(StatusHudType st)
		{
			...
			GameObject gameObject = (this._statusGo == null) ? null : this._statusGo[(int)st];
			if (gameObject)
			{
				gameObject.CustomSetActive(false);
			}
		...
		public void UpdateTowerMinimapBloodBar(float v)
		{
			...
			{
				Sprite3D arg_37_0 = this.smallTower_spt3d;
				this.bigTower_spt3d.fillAmount = v;
				arg_37_0.fillAmount = v;
			}
		}
		...
		public void ChangeCallActorBloodImg(bool bIsTrueType)
		{
			...
			this.m_bloodImage.spriteName = ((!bIsTrueType) ? "Battle_blueHp" : "Battle_redHp");
