			...
			for (int i = 0; i < equipInfos.Length; i++)
			{
				if (equipInfos[i].m_equipID != 0)
				{
					...
					for (int j = 0; j < this.m_existEquipInfoCount; j++)
					{
						...
						{
							flag = true;
							stExistEquipInfo[] expr_63_cp_0 = this.m_existEquipInfos;
							int expr_63_cp_1 = j;
							expr_63_cp_0[expr_63_cp_1].m_amount = expr_63_cp_0[expr_63_cp_1].m_amount + equipInfos[i].m_amount;
							break;
						}
					}
					if (!flag)
					{
						...
						this.m_existEquipInfos[this.m_existEquipInfoCount].m_unitBuyPrice = ((dataByKey == null) ? (CrypticInt32)0 : (CrypticInt32)((int)dataByKey.dwBuyPrice));
