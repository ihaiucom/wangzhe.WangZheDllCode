	...
	public class CallActorWrapper : ObjWrapper
	{
		...
		private void CopyActorBuff(ref PoolObjHandle<ActorRoot> targetActor)
		{
			...
			if (spawnedBuffList != null)
			{
				...
				{
					this.actor.BuffHolderComp.RemoveBuff(spawnedBuffList[i].SkillID);
				}
			}
			List<BuffSkill> spawnedBuffList2 = targetActor.handle.BuffHolderComp.SpawnedBuffList;
			if (spawnedBuffList2 != null)
			{
				int count2 = spawnedBuffList2.Count;
				for (int j = 0; j < count2; j++)
				{
					if (spawnedBuffList2[j] != null && spawnedBuffList2[j].GetSkillUseContext() != null)
					{
						SkillUseParam skillUseParam = default(SkillUseParam);
						skillUseParam.SetOriginator(spawnedBuffList2[j].GetSkillUseContext().Originator);
						skillUseParam.bExposing = spawnedBuffList2[j].skillContext.bExposing;
						skillUseParam.uiFromId = spawnedBuffList2[j].skillContext.uiFromId;
						skillUseParam.skillUseFrom = spawnedBuffList2[j].skillContext.skillUseFrom;
						this.actor.SkillControl.SpawnBuff(this.actorPtr, ref skillUseParam, spawnedBuffList2[j].SkillID, true);
					}
				}
			}
		}
		...
		public void CreateImposterActor()
		{
			...
			int count = diffCampPlayers.Count;
			if (count > 0)
			{
				int index = (int)FrameRandom.Random((uint)count);
				Player player = diffCampPlayers[index];
				if (player.Captain)
				{
		...
		private void SetSkillEffectPath()
		{
			...
			string value = "prefab_skill_effects/hero_skill_effects/";
		...
		{
			PoolObjHandle<ActorRoot> attack = (!this.myLastAtker) ? this.myLastAtker : this.myLastAtker.handle.ActorControl.GetOrignalActor();
		...
		public string GetLevelUpEftPath(int level)
		{
			...
			{
				SkinElement skinElement = this.actor.CharInfo.SkinPrefab[(int)((UIntPtr)(this.m_skinId - 1u))];
