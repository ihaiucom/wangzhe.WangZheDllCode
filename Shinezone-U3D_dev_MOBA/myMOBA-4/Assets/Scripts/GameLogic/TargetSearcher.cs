...
namespace Assets.Scripts.GameLogic
{
	public class TargetSearcher : Singleton<TargetSearcher>
	{
		...
		public ActorRoot GetNearestFriendly(ActorRoot InActor, int srchR, uint filter)
		{
			...
			{
				ActorRoot handle = campActors[i].handle;
				if (((ulong)filter & (ulong)(1L << (int)(handle.TheActorMeta.ActorType & (ActorTypeDef)31))) <= 0uL && !handle.ActorControl.IsDeadState)
				{
			...
			for (int i = 0; i < 3; i++)
			{
				if (i != (int)InActor.TheActorMeta.ActorCamp)
				{
					...
					for (int j = 0; j < count; j++)
					{
						...
						if (((ulong)filter & (ulong)(1L << (int)(actorType & (ActorTypeDef)31))) <= 0uL && handle.HorizonMarker.IsVisibleFor(InActor.TheActorMeta.ActorCamp))
						{
		...
		public ActorRoot GetNearestEnemyIgnoreVisible(ActorRoot InActor, int srchR, uint filter)
		{
			...
			for (int i = 0; i < 3; i++)
			{
				if (i != (int)InActor.TheActorMeta.ActorCamp)
				{
					...
					{
						ActorRoot handle = campActors[j].handle;
						if (((ulong)filter & (ulong)(1L << (int)(handle.TheActorMeta.ActorType & (ActorTypeDef)31))) <= 0uL)
						{
							ulong sqrMagnitudeLong2D = (ulong)(handle.location - InActor.location).sqrMagnitudeLong2D;
							if (sqrMagnitudeLong2D < num && InActor.CanAttack(handle))
							{
		...
		public ActorRoot GetLowestHpTarget(ActorRoot InActor, int srchR, TargetPriority priotity, uint filter, bool bEnemy = true, bool bWithMonsterNotInBattle = true)
		{
			...
			if (bEnemy)
			{
				for (int i = 0; i < 3; i++)
				{
					if (i != (int)InActor.TheActorMeta.ActorCamp)
					{
						...
						{
							ActorRoot handle = campActors[j].handle;
							if (((ulong)filter & (ulong)(1L << (int)(handle.TheActorMeta.ActorType & (ActorTypeDef)31))) <= 0uL && handle.HorizonMarker.IsVisibleFor(InActor.TheActorMeta.ActorCamp) && InActor.CanAttack(handle))
							{
			...
			else
			{
				...
				int count2 = campActors2.Count;
				for (int k = 0; k < count2; k++)
				{
					ActorRoot handle2 = campActors2[k].handle;
					if (((ulong)filter & (ulong)(1L << (int)(handle2.TheActorMeta.ActorType & (ActorTypeDef)31))) <= 0uL && !handle2.ActorControl.IsDeadState && handle2.HorizonMarker.IsVisibleFor(InActor.TheActorMeta.ActorCamp))
					{
			...
			int iTargetCount = list.Count + list3.Count + list2.Count;
		...
		public ActorRoot GetNearestEnemy(ActorRoot InActor, int srchR, TargetPriority priotity, uint filter, bool bWithMonsterNotInBattle = true)
		{
			...
			for (int i = 0; i < 3; i++)
			{
				if (i != (int)InActor.TheActorMeta.ActorCamp)
				{
					...
					{
						ActorRoot handle = campActors[j].handle;
						if (((ulong)filter & (ulong)(1L << (int)(handle.TheActorMeta.ActorType & (ActorTypeDef)31))) <= 0uL && handle.HorizonMarker.IsVisibleFor(InActor.TheActorMeta.ActorCamp))
						{
							if (!bWithMonsterNotInBattle)
							{
								MonsterWrapper monsterWrapper = handle.ActorControl as MonsterWrapper;
			...
			return (actorRoot == null) ? actorRoot2 : actorRoot;
		}
		...
		public ActorRoot GetNearestEnemyDogfaceFirst(ActorRoot InActor, int srchR)
		{
			...
			return (actorRoot2 == null) ? actorRoot : actorRoot2;
		}
		...
		public ActorRoot GetNearestEnemyWithTwoPriorityWithoutJungleMonster(ActorRoot InActor, int srchR, TargetPriority priotity_1, TargetPriority priotity_2)
		{
			...
			return (actorRoot == null) ? ((actorRoot2 == null) ? actorRoot3 : actorRoot2) : actorRoot;
		}
		...
		public List<PoolObjHandle<ActorRoot>> GetActorsInPolygon(GeoPolygon collisionPolygon, AreaEventTrigger InTriggerRef)
		{
			...
			for (int i = 0; i < count; i++)
			{
				...
				if (poolObjHandle)
				{
					if (!(null != InTriggerRef) || InTriggerRef.ActorFilter(ref poolObjHandle))
					{
						if (collisionPolygon.IsPointIn(poolObjHandle.handle.location))
						{
							if (list == null)
							{
		...
		public PoolObjHandle<ActorRoot> GetCommanderTowerTarget(COM_PLAYERCAMP camp, int friendRange, int enemyRange)
		{
			...
			for (int i = 0; i < count; i++)
			{
				ActorRoot handle = towerActors[i].handle;
				if (handle.TheActorMeta.ActorCamp != camp && !handle.ActorControl.IsDeadState)
				{
					if (handle.ObjLinker.theOrganPos < organPos)
					{
			...
			for (int j = 0; j < count; j++)
			{
				ActorRoot handle2 = towerActors[j].handle;
				if (handle2.TheActorMeta.ActorCamp != camp && !handle2.ActorControl.IsDeadState)
				{
					if (handle2.ObjLinker.theOrganPos == organPos)
					{
		...
		public bool HasSelfSoldierCountRoundEnemyBuilding(ActorRoot InActor, uint buildId)
		{
			...
			for (int i = 0; i < count; i++)
			{
				...
				if (handle.TheActorMeta.ActorType == ActorTypeDef.Actor_Type_Organ || handle.ObjID == buildId)
				{
					...
					int count2 = soldierActors.Count;
					for (int j = 0; j < count2; j++)
					{
						ActorRoot handle2 = soldierActors[j].handle;
