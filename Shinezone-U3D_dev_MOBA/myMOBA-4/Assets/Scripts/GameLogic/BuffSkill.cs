...
namespace Assets.Scripts.GameLogic
{
	public class BuffSkill : BaseSkill
	{
		...
		private bool UseImpl(PoolObjHandle<ActorRoot> user)
		{
			...
			GameObject gameObject = (!this.skillContext.Originator) ? null : this.skillContext.Originator.handle.gameObject;
			GameObject gameObject2 = (!this.skillContext.TargetActor) ? null : this.skillContext.TargetActor.handle.gameObject;
			this.curAction = new PoolObjHandle<AGE.Action>(ActionManager.Instance.PlayAction(this.ActionName, true, false, new GameObject[]
			{
				gameObject,
				gameObject2
			}));
			...
			if (this.cfgData.bShowType != 0 || this.cfgData.bFloatTextID > 0)
			{
				...
				for (int i = 0; i < this.skillContext.TargetActor.handle.BuffHolderComp.SpawnedBuffList.Count; i++)
				{
					BuffSkill buffSkill = this.skillContext.TargetActor.handle.BuffHolderComp.SpawnedBuffList[i];
