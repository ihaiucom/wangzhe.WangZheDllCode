...
namespace Assets.Scripts.Framework
{
	...
	public class FrameSynchr : Singleton<FrameSynchr>, IGameModule
	{
        ...
        private int[] StatDelaySet = new int[StatDelayCnt];
        ...
        // 最大缓存MoveDirectionCommand数目
        public const int MoveCmdBuff_Size = 256;
        // 缓存发送的MoveDirectionCommand时间戳（毫秒）
        public uint[] m_MoveCMDSendTime = new uint[MoveCmdBuff_Size];
        // 缓存收到的MoveDirectionCommand时间戳（毫秒）
        public uint[] m_MoveCMDReceiveTime = new uint[MoveCmdBuff_Size];
		...
		public byte FrameSpeed
		{
			...
			set
			{
                this.frameSpeed = (byte)Mathf.Clamp((int)value, MIN_FRAME_SPEED, MAX_FRAME_SPEED);
				if (this._bActive)
				{
					this.ResetStartTime();
				}
		...
		public override void Init()
		{
			...
			Assembly assembly = typeof(FrameSynchr).Assembly;
		...
		private void UpdateMultiFrame()
		{
			...
			long nClientMs = (long)((double)(Time.realtimeSinceStartup - this.startFrameTime) * 1000.0);
			long nDelayMs = nClientMs - (long)((ulong)((this.SvrFrameIndex + 1u) * this.SvrFrameDelta));
			this.nJitterDelay = this.CalculateJitterDelay(nDelayMs);
			nClientMs *= (long)this.frameSpeed;
			int execFrame = 0;
			while (i > 0)
			{
				long nCurFrameMs = (long)((ulong)this.CurFrameNum * (ulong)this.FrameDelta);
				this.nMultiFrameDelta = nClientMs - nCurFrameMs;
                ...
                while (this.commandQueue.Count > 0)
                {
                    ...
                    uint fixedFrameNum = (frameCommand.frameNum + this.SvrFrameLater) * this.KeyFrameRate;
                    if (fixedFrameNum > this.CurFrameNum)
                    {
                        break;
                    }
                    frameCommand.frameNum = fixedFrameNum;
                    frameCommand = this.commandQueue.Dequeue();
                    frameCommand.ExecCommand();
                }
                this.isCmdExecuting = false;
                // update GameLogic: update part1 for last frame, full update for others 
                Singleton<GameLogic>.GetInstance().UpdateLogic((int)this.FrameDelta, i == 1 && this.nMultiFrameDelta < (long)(2uL * (ulong)this.FrameDelta));
                i--;
            }
			this.m_maxExcuteFrameOnce = Mathf.Max(this.m_maxExcuteFrameOnce, execFrame);
		}
