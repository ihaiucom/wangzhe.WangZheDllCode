...
namespace Assets.Scripts.Framework
{
	public class LobbyConnector : BaseConnector
	{
		...
		{
			return this.connected && this.lobbySendQueue.Count == 0 && this.curSvrPkgSeq > 0u;
		}
		...
		public void HandleSending()
		{
			...
			{
				int num = 0;
				while (this.connected && num < this.lobbySendQueue.Count)
				{
					CSPkg cSPkg = this.lobbySendQueue[num];
		...
		private uint onTryReconnect(uint nCount, uint nMax)
		{
			...
			{
				listView.Add(this.lobbySendQueue[i]);
			}
			this.lobbySendQueue.Clear();
