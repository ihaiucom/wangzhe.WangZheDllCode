...
namespace Assets.Scripts.Framework
{
	...
	public class SynchrReport
	{
		...
		private static void Upload(long offset)
		{
			...
			bool flag = false;
			while (SynchrReport._uploadList != null && SynchrReport._uploadIndex >= 0 && SynchrReport._uploadIndex < SynchrReport._uploadList.Count && SynchrReport._uploadList[SynchrReport._uploadIndex] == null)
			{
				SynchrReport._uploadIndex++;
			}
			if (SynchrReport._uploadList != null && SynchrReport._uploadIndex >= 0 && SynchrReport._uploadIndex < SynchrReport._uploadList.Count)
			{
				MemoryStream memoryStream = SynchrReport._uploadList[SynchrReport._uploadIndex];
				if (offset < memoryStream.Length)
				{
					if (offset != memoryStream.Position)
					{
					...
					{
						flag = (++SynchrReport._uploadIndex >= SynchrReport._uploadList.Count);
		...
		private static void CloseUpload()
		{
			...
			{
				for (int i = 0; i < SynchrReport._uploadList.Count; i++)
				{
					if (SynchrReport._uploadList[i] != null)
					{
						SynchrReport._uploadList[i].Close();
					}
				}
				SynchrReport._uploadList = null;
			}
