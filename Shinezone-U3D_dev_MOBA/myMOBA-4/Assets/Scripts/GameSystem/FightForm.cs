...
namespace Assets.Scripts.GameSystem
{
	public class FightForm : IBattleForm
	{
		...
		private void UpdateDominateVanguardWavesText(bool bFriendly)
		{
			...
			{
				this.DominateVanguardWavesText.text = string.Format("强化兵线波次： <color=#f457e5>{0}/{1}</color>", this.DominateVanguardRepeatCount, MonoSingleton<GlobalConfig>.instance.DominateVanguardRepeatTotal);
			}
		}
		...
		public void OnToggleAutoAI(CUIEvent uiEvent)
		{
			...
			if (hostPlayer != null && hostPlayer.Captain && hostPlayer.Captain.handle.ActorControl != null)
			{
				...
				frameCommand.cmdData.IsAutoAI = ((!hostPlayer.Captain.handle.ActorControl.m_isAutoAI) ? (sbyte)1 : (sbyte)0);
				frameCommand.Send();
		...
		public void OnSwitchAutoAI(ref DefaultGameEventParam param)
		{
			...
			{
				GameObject gameObject = (!(this._formScript != null)) ? null : Utility.FindChild(this._formScript.gameObject, "PVPTopRightPanel/PanelBtn/ToggleAutoBtn");
				if (gameObject == null)
				{
					return;
				}
		...
		public void OnDragonTipFormOpen(CUIEvent cuiEvent)
		{
			...
			if (dataByKey2 != null)
			{
				component2.text = Utility.UTF8Convert(dataByKey2.szSkillCombineDesc);
			}
			if (dataByKey3 != null)
			{
				component3.text = Utility.UTF8Convert(dataByKey3.szSkillCombineDesc);
			}
		}
		...
		private void OnCommon_BattleWifiCheckTimer(CUIEvent uiEvent)
		{
			...
			if (this.m_wifiIcon != null && this.m_wifiIconCheckTicks >= this.m_wifiIconCheckMaxTicks)
			{
				...
				{
					this.m_wifiIcon.SetSprite(CUIUtility.s_Sprite_System_Wifi_Dir + CLobbySystem.s_wifiStateName[(int)((UIntPtr)num2)], this._formScript, true, true, false, false);
				}
				else if (netWorkType == enNetWorkType.enNet)
				{
					this.m_wifiIcon.SetSprite(CUIUtility.s_Sprite_System_Wifi_Dir + CLobbySystem.s_netStateName[(int)((UIntPtr)num2)], this._formScript, true, true, false, false);
				}
			...
			{
				this.m_wifiText.text = string.Format("{0}ms", this.m_displayPing);
				this.m_wifiText.color = CLobbySystem.s_WifiStateColor[(int)((UIntPtr)num2)];
			}
		}
		...
		public void DisableCameraDragPanelForRevive()
		{
			...
			if (this._formScript != null && curLvelContext != null)
			{
				...
				if (curLvelContext.IsMobaMode())
				{
					...
					if (captain && this._formScript != null)
					{
						Transform transform = (!(gameObject != null)) ? null : gameObject.transform.Find("panelDeadInfo");
		...
		protected void UpdateFpsAndLag(CUIEvent uiEvent)
		{
			...
			if (widget != null)
			{
				...
				component.text = string.Format("FPS {0}", num);
		...
		private void OnBattleHeroSkillTipOpen(SkillSlot skillSlot, Vector3 Pos, uint heroId)
		{
			...
			if (component2 != null && skillSlot.SkillObj.cfgData.szSkillDesc.Length > 0)
			{
				component2.text = CUICommonSystem.GetSkillDesc(skillSlot.SkillObj.cfgData.szSkillDesc, actorValue, skillSlot.GetSkillLevel(), captain.handle.ValueComponent.actorSoulLevel, heroId);
			}
			Text component3 = this.skillTipDesc.transform.Find("SkillCDText").GetComponent<Text>();
			component3.text = Singleton<CTextManager>.instance.GetText("Skill_Cool_Down_Tips", new string[]
			{
			...
			component4.text = ((captain.handle.ValueComponent.mActorValue.EnergyType != EnergyType.BloodResource) ? Singleton<CTextManager>.instance.GetText(EnergyCommon.GetEnergyShowText((uint)captain.handle.ValueComponent.mActorValue.EnergyType, EnergyShowType.CostValue), new string[]
			{
		...
		private void ShowSkillDescInfo(string strSkillDesc, Vector3 Pos)
		{
			...
			float num = component.preferredHeight;
			...
			componetInChild.rectTransform.sizeDelta = new Vector2(sizeDelta.x, num);
			RectTransform component2 = this.BuffDesc.GetComponent<RectTransform>();
			if (component2 != null)
			{
				component2.sizeDelta = componetInChild.rectTransform.sizeDelta;
			}
			Vector3 position = Pos;
			position.x -= 4f;
			position.y += 4f;
		...
		private void OnBattleAtkSelectSoldierBtnDown(CUIEvent uiEvent)
		{
			...
			Singleton<CUIParticleSystem>.GetInstance().AddParticle(CUIParticleSystem.s_particleSkillBtnEffect_Path, 0.5f, uiEvent.m_srcWidget, uiEvent.m_srcFormScript, null);
		...
		private void OnClickBattleScene(CUIEvent uievent)
		{
			...
			Singleton<TeleportTargetSelector>.GetInstance().OnClickBattleScene(uievent.m_pointerEventData.position);
		}
		...
		public void UpdateAdValueInfo()
		{
			...
			if (this.m_AdTxt != null)
			{
				if (totalValue >= 1000)
				{
					this.m_AdTxt.text = Math.Round((double)totalValue / 1000.0, 2) + "k";
				}
				else
				{
		...
		public void UpdateApValueInfo()
		{
			...
			if (this.m_ApTxt != null)
			{
				if (totalValue >= 1000)
				{
					this.m_ApTxt.text = Math.Round((double)totalValue / 1000.0, 2) + "k";
				}
				else
				{
		...
		public void UpdatePhyDefValueInfo()
		{
			...
			if (this.m_PhyDefTxt != null)
			{
				if (totalValue >= 1000)
				{
					this.m_PhyDefTxt.text = Math.Round((double)totalValue / 1000.0, 2) + "k";
				}
				else
				{
		...
		public void UpdateMgcDefValueInfo()
		{
			...
			if (this.m_MgcDefTxt != null)
			{
				if (totalValue >= 1000)
				{
					this.m_MgcDefTxt.text = Math.Round((double)totalValue / 1000.0, 2) + "k";
				}
				else
				{
