	...
	public class SettlementSystem : Singleton<SettlementSystem>
	{
		...
		private void OnClickDianZanLaHei(CUIEvent uiEvent)
		{
			...
			while (enumerator.MoveNext())
			{
				...
				{
					arg = value.PlayerName;
					break;
				}
			}
			if (commonUInt32Param == commonUInt16Param)
			{
				Singleton<CUIManager>.instance.OpenTips(string.Format(Singleton<CTextManager>.GetInstance().GetText("ZanTeam"), arg), false, 1.5f, null, new object[0]);
			}
			else
			{
				Singleton<CUIManager>.instance.OpenTips(string.Format(Singleton<CTextManager>.GetInstance().GetText("ZanEnemyTeam"), arg), false, 1.5f, null, new object[0]);
			}
		...
		private void SetExpProfit()
		{
			...
			gameObject.transform.FindChild("PlayerName").gameObject.GetComponent<Text>().text = masterRoleInfo.Name;
			gameObject.transform.FindChild("PlayerLv").gameObject.GetComponent<Text>().text = string.Format("Lv.{0}", masterRoleInfo.PvpLevel);
			Text component = gameObject.transform.FindChild("ExpMaxTip").gameObject.GetComponent<Text>();
			component.text = ((acntInfo.bExpDailyLimit <= 0) ? string.Empty : Singleton<CTextManager>.GetInstance().GetText("GetExp_Limit"));
			Text component2 = gameObject.transform.FindChild("PvpExpTxt").gameObject.GetComponent<Text>();
			component2.text = string.Format("{0}/{1}", acntInfo.dwPvpExp, dataByKey.dwNeedExp);
			Text component3 = gameObject.transform.FindChild("AddPvpExpTxt").gameObject.GetComponent<Text>();
			component3.text = ((acntInfo.dwPvpSettleExp < 0u) ? acntInfo.dwPvpSettleExp.ToString() : string.Format("+{0}", acntInfo.dwPvpSettleExp));
			...
			SettlementSystem._lvUpGrade = ((num >= 0) ? 0u : acntInfo.dwPvpLv);
			float num2 = Mathf.Max(0f, (float)num / dataByKey.dwNeedExp);
			float num3 = Mathf.Max(0f, ((num >= 0) ? acntInfo.dwPvpSettleExp : acntInfo.dwPvpExp) / dataByKey.dwNeedExp);
			...
			component9.text = Singleton<CTextManager>.GetInstance().GetText(key, new string[]
			{
			...
			component10.text = Singleton<CTextManager>.GetInstance().GetText(key, new string[]
			{
			...
			if (array != null)
			{
				...
				while ((long)num4 < (long)((ulong)multipleInfo))
				{
					...
					if (num6 >= 0)
					{
						string[] expr_676_cp_0 = SettlementSystem.StrHelper2;
						int expr_676_cp_1 = num4 + 1;
						expr_676_cp_0[expr_676_cp_1] += "+";
					}
					string[] expr_691_cp_0 = SettlementSystem.StrHelper2;
					int expr_691_cp_1 = num4 + 1;
					expr_691_cp_0[expr_691_cp_1] += num6;
			...
			{
				gameObject2.GetComponentInChildren<Text>().text = string.Format("{0}", text);
			}
		...
		private void SetGoldCoinProfit()
		{
			...
			if (array != null)
			{
				...
				while ((long)num2 < (long)((ulong)multipleInfo))
				{
					...
					if (num4 >= 0)
					{
						string[] expr_318_cp_0 = SettlementSystem.StrHelper2;
						int expr_318_cp_1 = num2 + 1;
						expr_318_cp_0[expr_318_cp_1] += "+";
					}
					string[] expr_333_cp_0 = SettlementSystem.StrHelper2;
					int expr_333_cp_1 = num2 + 1;
					expr_333_cp_0[expr_333_cp_1] += num4;
					...
					case 2:
					{
						...
						if (transform2 != null)
						{
							...
							transform.FindChild("Text").GetComponent<Text>().text = string.Format("{0}", text2);
					...
					case 5:
					{
						...
						transform3.FindChild("Text").GetComponent<Text>().text = string.Format("{0}", text2);
						break;
					}
					case 6:
					...
					case 9:
					{
						...
						if (!flag)
						{
							...
							transform4.FindChild("Text").GetComponent<Text>().text = string.Format("{0}", text2);
						}
						else
						{
					...
					case 10:
					{
						...
						if (!flag && !flag2)
						{
							...
							transform5.FindChild("Text").GetComponent<Text>().text = string.Format("{0}", text2);
						}
						else
						{
			...
			else
			{
				...
				{
					gameObject3.GetComponentInChildren<Text>().text = string.Format("{0}", text);
				}
		...
		private void SetProficiencyInfo()
		{
			...
			gameObject.CustomSetActive(true);
			component3.text = actorStaticData.TheResInfo.Name;
			component6.text = ((heroSettleInfo.dwSettleProficiency <= 0u) ? null : string.Format("+{0}", heroSettleInfo.dwSettleProficiency));
			...
			else
			{
				...
				num3 = Mathf.Max(0f, ((num4 >= 0) ? heroSettleInfo.dwSettleProficiency : heroSettleInfo.dwProficiency) / heroProficiency.dwTopPoint);
				component5.text = string.Format("{0} / {1}", heroSettleInfo.dwProficiency, heroProficiency.dwTopPoint);
			}
			component4.text = SettlementSystem.GetProficiencyLvTxt(actorStaticData.TheHeroOnlyInfo.HeroCapability, heroSettleInfo.dwProficiencyLv);
		...
		public static COMDT_SETTLE_HERO_RESULT_INFO GetHeroSettleInfo(uint heroId)
		{
			...
			{
				if (heroSettleInfo.astHeroList[i] != null)
				{
					if (heroSettleInfo.astHeroList[i].dwHeroConfID == heroId)
					{
		...
		private void SetLadderInfo()
		{
			...
			if (transform != null)
			{
				...
				component.text = CLadderView.GetRankName(rankInfo.bNowShowGrade, Singleton<CRoleInfoManager>.GetInstance().GetSelfRankClass());
			}
			...
			if ((int)CLadderSystem.GetGradeDataByShowGrade((int)rankInfo.bNowShowGrade).bLogicGrade == CLadderSystem.MAX_RANK_LEVEL)
			{
				...
				component2.text = string.Format("X{0}", rankInfo.dwNowScore);
			}
			else
			{
			...
			try
			{
				...
				if (this._symbolCoinTweenText != null && this._symbolCoinTweenText.gameObject != null)
				{
					this._symbolCoinLtd = LeanTween.value(this._symbolCoinTweenText.gameObject, delegate(float value)
					{
						...
						this._symbolCoinTweenText.text = string.Format("+{0}", value.ToString("N0"));
		...
		public void DoSymbolCoinTweenEnd()
		{
			...
			this._symbolCoinTweenText.text = string.Format("+{0}", this._symbolCoinTo.ToString("N0"));
			COMDT_ACNT_INFO acntInfo = Singleton<BattleStatistic>.GetInstance().acntInfo;
			if (Singleton<BattleStatistic>.GetInstance().multiDetail != null)
			{
		...
		public void ShowSettlementPanel(bool neutralShow = false)
		{
			...
			this._settleFormScript.m_formWidgets[21].GetComponent<Text>().text = this._startTime;
		...
		private void SetSettlementTitle()
		{
			...
			Color color = (!this._win || this._neutral) ? new Color(1f, 1f, 1f, 0.75f) : Color.white;
			Utility.GetComponetInChild<Image>(p, "BgRed").color = color;
			Utility.GetComponetInChild<Image>(p, "BgBlue").color = color;
		...
		private void SetPlayerSettlement()
		{
			...
			int count2 = Singleton<GamePlayerCenter>.GetInstance().GetAllCampPlayers(COM_PLAYERCAMP.COM_PLAYERCAMP_2).Count;
			...
			gameObject.transform.FindChild("LeftTotalKill").gameObject.GetComponent<Text>().text = this._camp1TotalKill.ToString(CultureInfo.InvariantCulture);
			gameObject.transform.FindChild("RightTotalKill").gameObject.GetComponent<Text>().text = this._camp2TotalKill.ToString(CultureInfo.InvariantCulture);
			...
			while (enumerator2.MoveNext())
			{
				...
				PlayerKDA value2 = current2.Value;
		...
		private void SetCreditSettlement()
		{
			...
			{
				componetInChild.text = Singleton<CTextManager>.instance.GetText("Credit_Change_Tips_1", new string[]
				{
			...
			{
				componetInChild.text = Singleton<CTextManager>.instance.GetText("Credit_Change_Tips_2", new string[]
				{
					masterRoleInfo.creditScore.ToString(),
					acntInfo.iSettleCreditValue.ToString()
				});
		...
		private void UpdatePlayerKda(PlayerKDA kda)
		{
			...
			{
				num = ((!flag2) ? (kda.MvpValue * (GameDataMgr.globalInfoDatabin.GetDataByKey(296u).dwConfValue / 100f)) : kda.MvpValue);
			}
			if (flag2)
			{
			...
			if (this.playerNum > 2)
			{
				if (kda.PlayerId == Singleton<BattleStatistic>.instance.GetMvpPlayer(kda.PlayerCamp, flag2))
				{
					if (flag2)
					{
						...
						component.WinMvpTxt.text = Math.Max(num, 0f).ToString("F1");
					}
					else
					{
						...
						component.NormalMvpIcon.SetActive(false);
						component.NormalMvpTxt.gameObject.SetActive(false);
						component.LoseMvpTxt.text = Math.Max(num, 0f).ToString("F1");
					}
				}
				else
				{
					...
					component.NormalMvpTxt.text = Math.Max(num, 0f).ToString("F1");
				}
			}
			else
			{
			...
			while (enumerator.MoveNext())
			{
				...
				if (current != null)
				{
					...
					component.Damage.transform.FindChild("TotalDamageBg/TotalDamageBar").gameObject.GetComponent<Image>().fillAmount = (float)current.hurtToEnemy / num6;
					component.Damage.transform.FindChild("TotalDamageBg/Percent").gameObject.GetComponent<Text>().text = string.Format("{0:P1}", (float)current.hurtToEnemy / num6);
					...
					component.Damage.transform.FindChild("TotalTakenDamageBg/TotalTakenDamageBar").gameObject.GetComponent<Image>().fillAmount = (float)current.hurtTakenByEnemy / num7;
					component.Damage.transform.FindChild("TotalTakenDamageBg/Percent").gameObject.GetComponent<Text>().text = string.Format("{0:P1}", (float)current.hurtTakenByEnemy / num7);
					...
					component.Damage.transform.FindChild("TotalDamageHeroBg/TotalDamageHeroBar").gameObject.GetComponent<Image>().fillAmount = (float)current.hurtToHero / num8;
					component.Damage.transform.FindChild("TotalDamageHeroBg/Percent").gameObject.GetComponent<Text>().text = string.Format("{0:P1}", (float)current.hurtToHero / num8);
		...
		private int GetHostPlayerHeroId(DictionaryView<uint, PlayerKDA>.Enumerator playerKdaEmr)
		{
			while (playerKdaEmr.MoveNext())
			{
				...
				ulong arg_2D_0 = current.Value.PlayerUid;
				KeyValuePair<uint, PlayerKDA> current2 = playerKdaEmr.Current;
				if (Utility.IsSelf(arg_2D_0, current2.Value.WorldId))
				{
					KeyValuePair<uint, PlayerKDA> current3 = playerKdaEmr.Current;
					ListView<HeroKDA>.Enumerator enumerator = current3.Value.GetEnumerator();
		...
		private void ImpSettlementTimerEnd()
		{
			...
			{
				MonoSingleton<PandroaSys>.GetInstance().StartOpeninSettlement(this.m_TotalPlayerNum, this.m_GameMode, (!this.m_bIsWarmBattle) ? 0 : 1, (!this.m_bHaveCpu) ? 0 : 1, this.m_selectTypeHero, this.m_mapID, (!this.m_bLuanDou) ? 0 : 1, (!this.m_bFireHole) ? 0 : 1, this.m_bWin, this.m_bMvp, this.m_bLegaendary, this.m_bPENTAKILL, this.m_bQUATARYKIL, this.m_bTRIPLEKILL);
			}
			this.ClearSendRedBag();
		}
		...
		protected void ImpShowReport(CUIEvent uiEvent)
		{
			...
			if (playerKDAStat != null)
			{
				...
				while (enumerator.MoveNext())
				{
					...
					{
						KeyValuePair<uint, PlayerKDA> current2 = enumerator.Current;
						if (current2.Value.WorldId == this._reportWordId)
						{
							KeyValuePair<uint, PlayerKDA> current3 = enumerator.Current;
							arg = current3.Value.PlayerName;
							break;
						}
					}
				}
			}
			gameObject.transform.FindChild("ReportToggle/ReportName").gameObject.GetComponent<Text>().text = string.Format(Singleton<CTextManager>.GetInstance().GetText("Report_PlayerName"), arg);
		}
		...
		protected void ImpDoReport(CUIEvent uiEvent)
		{
			...
			while (enumerator.MoveNext())
			{
				...
				if (value != null && value.PlayerUid == this._reportUid && value.WorldId == this._reportWordId)
				{
					...
					cSPkg.stPkgData.stUserComplaintReq.iComplaintPlayerCamp = ((value.PlayerCamp != this._myCamp) ? 2 : 1);
				}
				num++;
			}
			GameObject gameObject2 = gameObject.transform.FindChild("ReportToggle").gameObject;
			if (gameObject2.transform.FindChild("ReportGuaJi").gameObject.GetComponent<Toggle>().isOn)
			{
				cSPkg.stPkgData.stUserComplaintReq.dwComplaintReason = 1u;
			}
			else if (gameObject2.transform.FindChild("ReportSong").gameObject.GetComponent<Toggle>().isOn)
			{
				cSPkg.stPkgData.stUserComplaintReq.dwComplaintReason = 2u;
			}
			else if (gameObject2.transform.FindChild("ReportXiaoJi").gameObject.GetComponent<Toggle>().isOn)
			{
				cSPkg.stPkgData.stUserComplaintReq.dwComplaintReason = 3u;
			}
			else if (gameObject2.transform.FindChild("ReportMaRen").gameObject.GetComponent<Toggle>().isOn)
			{
				cSPkg.stPkgData.stUserComplaintReq.dwComplaintReason = 4u;
			}
			else if (gameObject2.transform.FindChild("ReportYanYuan").gameObject.GetComponent<Toggle>().isOn)
			{
				cSPkg.stPkgData.stUserComplaintReq.dwComplaintReason = 5u;
			}
			else if (gameObject2.transform.FindChild("ReportGua").gameObject.GetComponent<Toggle>().isOn)
			{
				cSPkg.stPkgData.stUserComplaintReq.dwComplaintReason = 6u;
			}
			string str = CUIUtility.RemoveEmoji(gameObject2.transform.FindChild("InputField").gameObject.GetComponent<InputField>().text);
		...
		private void ShowBraveScoreDetailPanel()
		{
			...
			if (num > 0u)
			{
				...
				else
				{
					...
					if (dwConWinScore > 0u)
					{
						...
						component4.text = "+" + dwConWinScore;
						COMDT_RANKDETAIL currentRankDetail = Singleton<CLadderSystem>.GetInstance().GetCurrentRankDetail();
						if (currentRankDetail != null)
						{
							widget5.GetComponent<Text>().text = Singleton<CTextManager>.GetInstance().GetText("Ladder_Brave_Liansheng", new string[]
							{
					...
					if (dwMVPScore > 0u)
					{
						...
						component5.text = "+" + dwMVPScore;
						widget6.GetComponent<Text>().text = Singleton<CTextManager>.GetInstance().GetText("Ladder_Brave_Xianfeng", new string[]
						{
			...
			else
			{
				...
				component.text = Singleton<CTextManager>.GetInstance().GetText("Ladder_Brave_Guaji_Tip");
		...
		private void ShowBraveScoreProcessPanel()
		{
			...
			component.fillAmount = CLadderView.GetProcessCircleFillAmount((int)dwOldAddStarScore, (int)braveScoreMax);
			component2.text = dwOldAddStarScore + "/" + braveScoreMax;
			...
			{
				component.color = CUIUtility.s_Color_BraveScore_BaojiKedu_Off;
			}
			else
			{
				component.color = CUIUtility.s_Color_BraveScore_BaojiKedu_On;
			}
		...
		private void ShowLadderResultPanel(bool isSettle)
		{
			...
			if (isSettle)
			{
				...
				component.text = this.GetLadderResultDesc();
				COMDT_RANK_SETTLE_INFO rankInfo = Singleton<BattleStatistic>.GetInstance().rankInfo;
				...
				if (this._oldLogicGrade > this._newLogicGrade)
				{
					...
					component2.text = Singleton<CTextManager>.GetInstance().GetText("Ladder_Brave_KeepGrade_Txt3", new string[]
					{
						num.ToString()
					});
				}
		...
		private void OnAnimatiorBravePanelShowInEnd(CUIEvent uiEvent)
		{
			...
			else
			{
				...
				component.StartFill(processCircleFillAmount3, processCircleFillAmount4, (!flag) ? CUIProgressUpdaterScript.enFillDirection.Clockwise : CUIProgressUpdaterScript.enFillDirection.CounterClockwise, -1f);
			}
		...
		private void OnAnimatiorBraveProgressFillFull(CUIEvent uiEvent)
		{
			if (this.IsBraveScoreExchange())
			{
				...
				else
				{
					...
					component4.text = dwNowAddStarScore + "/" + selfBraveScoreMax;
		...
		private void OnAnimatiorBraveProgressFillEnd(CUIEvent uiEvent)
		{
			...
			component.text = Singleton<CTextManager>.GetInstance().GetText("Ladder_Brave_Exchange_Tip", new string[]
			{
				selfBraveScoreMax.ToString()
			});
			Text component2 = this._ladderForm.GetWidget(12).GetComponent<Text>();
			component2.text = dwNowAddStarScore + "/" + selfBraveScoreMax;
			GameObject widget = this._ladderForm.GetWidget(28);
		...
		private void OnAnimatiorBraveDigitalJitterEnd(CUIEvent uiEvent)
		{
			...
			CUIAnimatorScript cUIAnimatorScript = (!(this._ladderForm != null)) ? null : this._ladderForm.GetWidget(4).GetComponent<CUIAnimatorScript>();
			if (this.IsBraveScoreExchange())
			{
		...
		private void LadderAllDisplayEnd()
		{
			...
			component.text = Singleton<CTextManager>.GetInstance().GetText("Ladder_Inherit_Last_Season_Grade_Finish");
			Transform transform = this._ladderForm.gameObject.transform.FindChild("ShareGroup/Btn_Continue");
			if (transform != null && transform.gameObject != null)
			{
				transform.gameObject.CustomSetActive(true);
		...
		private void ResetAllXing(uint targetScore, uint targetMax, bool inverseShow = false)
		{
			...
			if (targetMax > 5u)
			{
				...
				component.text = string.Format("X{0}", this._curDian);
			}
			else
			{
		...
		private void PlayXingAnim(uint targetScore, uint targetMax, bool disappear = false)
		{
			...
			if (xing != null)
			{
				...
				component3.Play((!disappear) ? "Base Layer.Start_ShowIn" : "Base Layer.Start_Disappear");
				Singleton<CSoundManager>.GetInstance().PostEvent((!disappear) ? "UI_paiwei_dexing" : "UI_paiwei_diuxing", null);
			}
		}
