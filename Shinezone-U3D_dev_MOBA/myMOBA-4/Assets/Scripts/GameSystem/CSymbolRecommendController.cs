...
namespace Assets.Scripts.GameSystem
{
	...
	public class CSymbolRecommendController
	{
		...
		private void GetHeroRcmdSymbolList()
		{
			...
			{
				ResSymbolRcmd resSymbolRcmd = (ResSymbolRcmd)enumerator.Current;
		...
		private void RefreshRcmdSymbolListPanel()
		{
			...
			{
				gameObject.GetComponent<CUIListScript>().SetElementAmount(this.m_rcmdSymbolList[(int)(this.m_symbolRcmdLevel - 1)].Count);
		...
		private void OnRcmdListItemEnable(CUIEvent uiEvent)
		{
			...
			if (uiEvent.m_srcWidgetIndexInBelongedList < 0 || uiEvent.m_srcWidgetIndexInBelongedList >= this.m_rcmdSymbolList[(int)(this.m_symbolRcmdLevel - 1)].Count)
			{
				DebugHelper.Assert(false, "OnRcmdListItemEnable index out of range");
				return;
			}
			ResSymbolInfo dataByKey = GameDataMgr.symbolInfoDatabin.GetDataByKey(this.m_rcmdSymbolList[(int)(this.m_symbolRcmdLevel - 1)][uiEvent.m_srcWidgetIndexInBelongedList]);
