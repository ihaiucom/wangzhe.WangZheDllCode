...
namespace Assets.Scripts.GameSystem
{
	public class WatchForm : IBattleForm
	{
				...
				else if (!this._isBottomInAnimDoing)
				{
					...
					if (componetInChild3)
					{
						...
						if (flag)
						{
							...
							{
								componetInChild3.PlayAnimator((!this._isBottomLong) ? "UpAndDown_2_1" : "UpAndDown_1_1");
							}
							else
							{
								componetInChild3.PlayAnimator((!this._isBottomLong) ? "UpAndDown_2_2" : "UpAndDown_1_2");
							}
						}
					}
				}
				this.m_targetScaleVal = ((!this._isViewHide) ? 1.2f : 1.6f);
			}
		}
		...
		private void FocusHero(COM_PLAYERCAMP listCamp, int listIndex)
		{
			...
			{
				KeyValuePair<uint, HeroInfoItem> current = enumerator.Current;
				HeroInfoItem value = current.Value;
		...
		private void OnClickBottomFold(CUIEvent evt)
		{
			if (WatchForm.IsNeedShowCampMidInterface())
			{
				...
				if (componetInChild && !this._isBottomInAnimDoing)
				{
					...
					if (this._isBottomFold)
					{
						componetInChild.PlayAnimator((!this._isBottomLong) ? "UpAndDown_2_1" : "UpAndDown_1_1");
					}
					else
					{
						componetInChild.PlayAnimator((!this._isBottomLong) ? "UpAndDown_2_2" : "UpAndDown_1_2");
					}
				}
			}
			else
			{
				...
				if (componetInChild2 && componetInChild3)
				{
					...
					componetInChild3.PlayAnimator((!this._isBottomFold) ? "Open" : "Close");
				}
			}
		}
		...
		private void OnClickBottomEquipFold(CUIEvent evt)
		{
			...
			if (componetInChild && !this._isBottomInAnimDoing)
			{
				...
				componetInChild.PlayAnimator((!this._isBottomLong) ? "Expansion_1" : "Expansion_2");
			}
		}
		...
		private void SwitchCameraZoom()
		{
			if (Singleton<WatchController>.GetInstance().IsLiveCast)
			{
				...
				if (null != mobaCamera)
				{
					mobaCamera.currentZoomRate = ((mobaCamera.currentZoomRate <= 1f) ? 2f : 1f);
					mobaCamera.CameraUpdate();
				}
			}
		}
