...
namespace Assets.Scripts.GameSystem
{
	public class BeInviteMenu
	{
		...
		public void ShowNormal(SCPKG_INVITE_JOIN_GAME_REQ info)
		{
			...
			int num = 15;
			int.TryParse(Singleton<CTextManager>.instance.GetText("MessageBox_Close_Time"), out num);
			...
			this.form.transform.Find("Panel/Panel/refuse/MatchInfo").GetComponent<Text>().text = Singleton<CTextManager>.instance.GetText("Invite_Refuse_Title");
			if (this.form != null)
			{
				string text2 = null;
				string text3 = null;
				...
				else if (info.bInviteType == 2)
				{
					...
					{
						text3 = Singleton<CTextManager>.GetInstance().GetText((info.stInviteDetail.stTeamDetail.bPkAI != 1) ? "Invite_Match_Type_3" : "Invite_Match_Type_2");
					}
				}
		...
		private void ShowRefuseSendState(CUIEvent uievt = null)
		{
			...
			if (string.IsNullOrEmpty(this.m_inputFiled.text))
			{
				this.m_inputFiled.text = this.m_configTexts[0];
			}
		}
		...
		public void On_InBattleMsg_ListElement_Enable(CUIEvent uievent)
		{
			...
			string text = this.m_configTexts[srcWidgetIndexInBelongedList];
			if (text == null)
			{
				return;
			}
		...
		private void On_Invite_RefuseReason_Send(CUIEvent uievent)
		{
			...
			string str = CUIUtility.RemoveEmoji(this.m_inputFiled.text);
		...
		{
			this.ShowRefuseSendState(null);
			string text = this.m_configTexts[uievent.m_srcWidgetIndexInBelongedList];
			this.m_inputFiled.text = text;
		}
