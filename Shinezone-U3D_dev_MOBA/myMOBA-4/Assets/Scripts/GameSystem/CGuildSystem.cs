...
namespace Assets.Scripts.GameSystem
{
	...
	public class CGuildSystem : Singleton<CGuildSystem>
	{
					...
					if (this.IsOpenPlatformGroupFunc())
					{
						...
						if (masterRoleInfo != null && !masterRoleInfo.IsClientBitsSet(7))
						{
							...
							if (Singleton<ApolloHelper>.GetInstance().CurPlatform == ApolloPlatform.QQ)
							{
								...
								{
									string text3 = Singleton<CTextManager>.GetInstance().GetText("Guild_QQGroup_Guide_Title");
									string text4 = Singleton<CTextManager>.GetInstance().GetText("Guild_QQGroup_Guide_Join_Content");
									Singleton<CUIManager>.GetInstance().OpenMessageBoxWithCancel(text4, enUIEventID.Guild_Guide_Join_PlatformGroup, enUIEventID.None, default(stUIEventParams), text2, text, false, text3);
								}
							}
							else if (CGuildSystem.HasManageQQGroupAuthority())
							{
								string text3 = Singleton<CTextManager>.GetInstance().GetText("Guild_WXGroup_Guide_Title");
								string text4 = Singleton<CTextManager>.GetInstance().GetText("Guild_WXGroup_Guide_Create_Content");
								Singleton<CUIManager>.GetInstance().OpenMessageBoxWithCancel(text4, enUIEventID.Guild_Guide_Bind_PlatformGroup, enUIEventID.None, default(stUIEventParams), text2, text, false, text3);
							}
							else
							{
								string text3 = Singleton<CTextManager>.GetInstance().GetText("Guild_WXGroup_Guide_Title");
								string text4 = Singleton<CTextManager>.GetInstance().GetText("Guild_WXGroup_Guide_Join_Content");
								Singleton<CUIManager>.GetInstance().OpenMessageBoxWithCancel(text4, enUIEventID.Guild_Guide_Join_PlatformGroup, enUIEventID.None, default(stUIEventParams), text2, text, false, text3);
							}
		...
		public static void ReceiveNewMemberNtf(CSPkg msg)
		{
			...
			{
				currentGuildInfo.listMemInfo.Add(guildMemInfo);
				GuildInfo expr_26E_cp_0 = currentGuildInfo;
                expr_26E_cp_0.briefInfo.bMemberNum = (byte)(expr_26E_cp_0.briefInfo.bMemberNum + (byte)1);
			}
		...
		public static void ReceiveMemberQuitNtf(CSPkg msg)
		{
			...
			for (int i = Singleton<CGuildSystem>.GetInstance().Model.CurrentGuildInfo.listMemInfo.Count - 1; i >= 0; i--)
			{
				...
				if (guildMemInfo.stBriefInfo.uulUid == ullQuitUid)
				{
                    ...
                    expr_8C_cp_0.briefInfo.bMemberNum = (byte)(expr_8C_cp_0.briefInfo.bMemberNum - (byte)1);
				}
			}
		...
		public static void ReceiveApplyListRsp(CSPkg msg)
		{
			...
			int currentUTCTime = CRoleInfo.GetCurrentUTCTime();
			stApplicantInfo item = default(stApplicantInfo);
			...
			for (byte b = 0; b < stGetGuildApplyListRsp.bApplyCnt; b += 1)
			{
				...
				item.stBriefInfo.mentorLvl = (int)cOMDT_GUILD_MEMBER_BRIEF_INFO.dwMasterLevel;
				item.stBriefInfo.rankGrade = cOMDT_GUILD_MEMBER_BRIEF_INFO.stRankShowGrade;
		...
		{
			stRecommendInfo item = default(stRecommendInfo);
			...
			for (int i = 0; i < (int)stGetGuildRecommendListRsp.bCount; i++)
			{
				...
				item.rankGrade = cOMDT_GUILD_RECOMMEND_INFO.stRankShowGrade;
				list.Add(item);
			}
			...
			else
			{
				...
				for (int i = listMemInfo.Count - 1; i >= 0; i--)
				{
					if (msg.stPkgData.stFireGuildMemberNtf.ullUid == listMemInfo[i].stBriefInfo.uulUid)
					{
						...
						GuildInfo expr_F1_cp_0 = Singleton<CGuildSystem>.GetInstance().Model.CurrentGuildInfo;
                        expr_F1_cp_0.briefInfo.bMemberNum = (byte)(expr_F1_cp_0.briefInfo.bMemberNum - (byte)1);
						break;
					}
				}
