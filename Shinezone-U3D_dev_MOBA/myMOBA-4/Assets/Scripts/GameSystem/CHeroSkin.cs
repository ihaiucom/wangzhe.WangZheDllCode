	...
	public class CHeroSkin : CUseable
	{
		...
		public CHeroSkin(ulong objID, uint baseID, int stackCount = 0, int addTime = 0)
		{
			...
			if (dataByKey != null)
			{
				...
				this.m_stackMax = 1;
				this.m_skinCoinBuy = ((resHeroSkinShop == null) ? 1u : resHeroSkinShop.dwChgItemCnt);
				this.m_dianQuanBuy = ((resHeroSkinShop == null) ? 1u : resHeroSkinShop.dwBuyCoupons);
				this.m_diamondBuy = ((resHeroSkinShop == null) ? 1u : resHeroSkinShop.dwBuyDiamond);
				this.m_coinSale = 0u;
				this.m_grade = 3;
				this.m_isSale = 0;
				this.m_addTime = 0;
