	...
	internal class CBuyCoinInfoPanel
	{
		...
		private void refreshPanel()
		{
			...
			if (cUIListScript != null)
			{
				...
				cUIListScript.SetElementAmount(this.m_stInfos.Count);
				...
				for (int i = 0; i < this.m_stInfos.Count; i++)
				{
					...
					component.text = this.GetDescribeStr(this.m_stInfos[i]);
				}
				if (flag)
				{
		...
		public void BuyCoinRsp(CSPkg msg)
		{
			for (int i = 0; i < (int)msg.stPkgData.stCoinBuyRsp.stBuyList.bCoinGetCnt; i++)
			{
				...
				info.m_CritTime = (int)(msg.stPkgData.stCoinBuyRsp.stBuyList.CoinGetVal[i] / (uint)((this.m_GainCoin != 0) ? this.m_GainCoin : 1));
				this.addInfo(info);
			}
		}
	}
