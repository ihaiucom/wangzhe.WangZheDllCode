	...
	public class CMiShuSystem : Singleton<CMiShuSystem>
	{
		...
		public void InitList(int TabIndex, CUIListScript list)
		{
			...
			for (int i = 0; i < resList.Length; i++)
			{
				...
				component2.text = resList[i].szName;
				component3.text = resList[i].szUnOpenDesc;
				component4.text = resList[i].szDesc;
		...
		public ResMiShuInfo[] GetResList(int TabIndex)
		{
			...
			while (enumerator.MoveNext())
			{
				...
				ResMiShuInfo resMiShuInfo = (ResMiShuInfo)current.Value;
		...
		{
			uint num = 0u;
			uint.TryParse(Singleton<CTextManager>.instance.GetText("MapID_PVP_Fire"), out num);
		...
		public bool IsHaveEntertrainModeOpen()
		{
			...
			uint.TryParse(Singleton<CTextManager>.instance.GetText("MapID_PVP_Fire"), out array[0]);
			uint.TryParse(Singleton<CTextManager>.instance.GetText("MapID_PVP_5V5Clone"), out array[1]);
			uint.TryParse(Singleton<CTextManager>.instance.GetText("MapID_PVP_5V5CD"), out array[2]);
			bool result = false;
			for (int i = 0; i < 3; i++)
			{
