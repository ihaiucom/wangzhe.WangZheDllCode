	...
	public class CPlayerProfile
	{
		...
		public void ConvertRoleInfoData(CRoleInfo roleInfo)
		{
			...
			while (enumerator.MoveNext())
			{
				...
				int key = current.Key;
				Dictionary<int, COMDT_HONORINFO> arg_492_0 = this._honorDic;
				KeyValuePair<int, COMDT_HONORINFO> current2 = enumerator.Current;
				if (!arg_492_0.ContainsKey(current2.Key))
				{
					Dictionary<int, COMDT_HONORINFO> arg_4B4_0 = this._honorDic;
					int arg_4B4_1 = key;
					KeyValuePair<int, COMDT_HONORINFO> current3 = enumerator.Current;
					arg_4B4_0.Add(arg_4B4_1, current3.Value);
				}
			}
