	...
	public class CFriendContoller : Singleton<CFriendContoller>
	{
		...
		private void OpenSingleList(enFriendSingleListType listType)
		{
			...
			if (listType != enFriendSingleListType.requestList)
			{
				...
				{
					component.text = Singleton<CTextManager>.GetInstance().GetText("Friend_Title_Blacklist");
					component2.text = Singleton<CTextManager>.instance.GetText("Friend_NoBlackList_Tip");
					widget.CustomSetActive(this.model.GetBlackList().Count == 0);
			...
			{
				component.text = Singleton<CTextManager>.GetInstance().GetText("Friend_Title_Requests");
				component2.text = Singleton<CTextManager>.instance.GetText("Friend_NoRequest_Tip");
		...
		{
			stFriendByUUIDAndLogicID key = new stFriendByUUIDAndLogicID(ullUid, dwLogicWorldID, CFriendModel.FriendType.GameFriend);
			if (!CFriendModel.IsNoAskForDic.ContainsKey(key))
			{
				CFriendModel.IsNoAskForDic.Add(key, bNoAskFor > 0);
			}
			else
			{
				CFriendModel.IsNoAskForDic[key] = (bNoAskFor > 0);
			}
		}
		...
		{
			stFriendByUUIDAndLogicID key = new stFriendByUUIDAndLogicID(ullUid, dwLogicWorldId, CFriendModel.FriendType.GameFriend);
			return CFriendModel.IsNoAskForDic.ContainsKey(key) && CFriendModel.IsNoAskForDic[key];
		}
		...
		private void Update_Send_Coin_Data(COMDT_ACNT_UNIQ uin, ulong donateAPSec)
		{
			...
			if (dateTime2.Year == dateTime.Year && dateTime2.Month == dateTime.Month && dateTime2.Day == dateTime.Day)
			{
				this.model.HeartData.Add(uin);
			}
			if (dateTime2.Year < dateTime.Year || (dateTime2.Year == dateTime.Year && dateTime2.Month < dateTime.Month) || dateTime2.Year != dateTime.Year || dateTime2.Month != dateTime.Month || dateTime2.Day < dateTime.Day)
			{
			}
		}
        ...
        private void Add_And_Refresh(CFriendModel.FriendType type, COMDT_FRIEND_INFO data)
        {
            ...
            switch (type2)
            {
                case CFriendModel.FriendType.Mentor:
                    if (m_mentorInfo != null)
                    {
                        ...
                        this.Handle_NoAskFor_Flag(m_mentorInfo.stMaster.stFriendInfo.stUin.ullUid, m_mentorInfo.stMaster.stFriendInfo.stUin.dwLogicWorldId, m_mentorInfo.stMaster.bNoAskforFlag);
            ...
            if (flag)
            {
                ...
                this.Handle_NoAskFor_Flag(m_mentorInfo.astStudentList[m_mentorInfo.bStudentNum - 1].stFriendInfo.stUin.ullUid, m_mentorInfo.astStudentList[m_mentorInfo.bStudentNum - 1].stFriendInfo.stUin.dwLogicWorldId, m_mentorInfo.astStudentList[m_mentorInfo.bStudentNum - 1].bNoAskforFlag);
		...
		{
			FriendShower component = evt.m_srcWidget.transform.parent.parent.parent.GetComponent<FriendShower>();
			evt.m_eventParams.commonUInt64Param1 = ((!(component != null)) ? 0uL : component.ullUid);
			evt.m_eventParams.tagUInt = ((!(component != null)) ? 0u : component.dwLogicWorldID);
		...
		public void OnMasterStudentInfo(CSPkg msg)
		{
			...
			{
				COMDT_FRIEND_INFO expr_116 = stMasterStudentInfo.astStudentList[i].stFriendInfo;
				expr_116.bStudentType |= 16;
		...
		public void OnMentor_GraduateNtf(CSPkg msg)
		{
			...
			if (bType != 1)
			{
				if (bType == 2)
				{
					...
					if (info != null)
					{
                        info.bStudentType = (byte)((info.bStudentType & (byte)240) | (byte)2);
					}
					CPlayerProfile profile = Singleton<CPlayerInfoSystem>.instance.GetProfile();
					if (profile != null && profile._mentorInfo != null)
					{
		...
		public void OnMentor_GetStudentList(CSPkg msg)
		{
			...
			while ((long)num < (long)((ulong)msg.stPkgData.stGetStudentListRsp.dwStudentNum))
			{
				...
				expr_92.bStudentType |= (byte)((byte)(enMentorRelationType) << 4);
			...
			if (component != null)
			{
				...
				if (singleListType != enFriendSingleListType.requestList)
				{
					if (singleListType == enFriendSingleListType.blackList)
					{
						...
						{
							CFriendModel.stBlackName stBlackName = blackList[srcWidgetIndexInBelongedList];
							if (component != null)
							{
