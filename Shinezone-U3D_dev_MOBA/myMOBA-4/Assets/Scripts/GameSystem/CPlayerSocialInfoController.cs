	...
	public class CPlayerSocialInfoController : Singleton<CPlayerSocialInfoController>
	{
		...
		public void CheckLoadAddressBook()
		{
			if (this.m_addressBook == null)
			{
				...
				else
				{
					...
					{
						int num = 0;
						this.m_searchStrings[5] = new string[this.m_addressBook.Count];
						using (List<object>.Enumerator enumerator = this.m_addressBook.GetEnumerator())
						{
							while (enumerator.MoveNext())
							{
								...
								this.m_searchStrings[5][num] = (dictionary["name"] as string);
								num++;
							}
						}
					}
		...
		public string[] GetLocations(string srvAddr)
		{
			...
			for (int i = 0; i < this.m_searchStrings[5].Length; i++)
			{
				...
				{
					array[0] = array2[0];
					Dictionary<string, object> dictionary = this.m_addressBook[i] as Dictionary<string, object>;
					object obj = null;
					if (dictionary.TryGetValue("sub", out obj))
					{
		...
		public void UpdateUI()
		{
			...
			if (!this.m_isInEditmode)
			{
				...
				gameObject.GetComponent<Text>().color = this.m_colorHighlight;
				gameObject.transform.parent.Find("Title").GetComponent<Text>().color = this.m_colorHighlight;
			}
			else
			{
				gameObject.GetComponent<Text>().color = this.m_colorNormal;
				gameObject.transform.parent.Find("Title").GetComponent<Text>().color = this.m_colorNormal;
			}
			try
			{
				...
				for (int j = 0; j < this.m_searchTypeNodes.Length; j++)
				{
					if (this.m_displyTypeNodes[j] != null)
					{
						...
						if (!this.m_isInEditmode)
						{
							...
							{
							case 0:
								component.text = this.GetSearchStr(j, (int)profile._socialCardInfo.dwSearchType);
								break;
							case 1:
								component.text = this.GetSearchStr(j, (int)profile._socialCardInfo.dwDay);
								break;
							case 2:
								component.text = this.GetSearchStr(j, (int)profile._socialCardInfo.dwHour);
								break;
							case 3:
								try
								{
									...
									{
										component.text = CHeroInfo.GetHeroJobStr((RES_HERO_JOB)num);
							...
							case 4:
								try
								{
									...
									{
										component.text = CHeroInfo.GetHeroJobStr((RES_HERO_JOB)num2);
							...
							{
								this.m_searchTypeNodes[j].transform.FindChild("List").GetComponent<CUIListScript>().SelectElement(this.GetIndexByString(j, component.text), true);
							}
						}
					}
				}
			...
			{
				Debug.LogError("social card setting error:" + arg);
			}
			if (transform != null)
			{
			...
			if (transform2 != null)
			{
				...
				if (!flag)
				{
					...
					{
						gameObject6.transform.Find("Text").GetComponent<Text>().text = Singleton<CPlayerMentorInfoController>.GetInstance().mentorBtnStr;
					}
				}
			}
			if (transform4 != null)
			{
				...
				gameObject7.GetComponent<InputField>().interactable = this.m_isInEditmode;
			}
			if (!this.m_isInEditmode)
			{
		...
		private void UpdateTags()
		{
			...
			if (transform != null)
			{
				...
				for (int j = 0; j < this.m_tags.Count; j++)
				{
					...
					int id = Convert.ToInt32(this.m_tags[j]);
					ResSocialTags dataByIndex = GameDataMgr.socialTagsDataBin.GetDataByIndex(id);
					component.text = dataByIndex.szText;
					byte bType = dataByIndex.bType;
					if (bType != 1)
					{
						if (bType == 2)
						{
							component.color = this.m_colorTagKawaii;
						}
					}
					else
					{
						component.color = this.m_colorTagCool;
					}
				}
				for (int k = this.m_tags.Count; k < 4; k++)
				{
		...
		public void OnLocation1Choose(CUIEvent evt)
		{
			...
			if (selectedIndex < this.m_addressBook.Count)
			{
				Dictionary<string, object> dictionary = this.m_addressBook[selectedIndex] as Dictionary<string, object>;
				object obj = null;
				if (dictionary.TryGetValue("sub", out obj))
				{
					List<object> list = obj as List<object>;
					...
					{
						int num = 0;
						this.m_searchStrings[6] = new string[list.Count];
						using (List<object>.Enumerator enumerator = list.GetEnumerator())
						{
							while (enumerator.MoveNext())
							{
		...
		private bool ChcekFriendCardDiffWidgets(COMDT_FRIEND_CARD card)
		{
			...
			num = 5;
			string arg = this.m_searchStrings[num][this.GetDropListSelectIdx(num)];
			num = 6;
			string arg2 = "-";
			if (this.m_searchStrings[num] != null)
			{
				arg2 = this.m_searchStrings[num][this.GetDropListSelectIdx(num)];
			}
			byte[] array = new byte[card.szPlace.Length];
			StringHelper.StringToUTF8Bytes(arg + '~' + arg2, ref array);
			...
			if (signInputText)
			{
				...
				StringHelper.StringToUTF8Bytes(signInputText.text, ref array);
			...
			if (x != null)
			{
				for (int k = 0; k < this.m_tags.Count; k++)
				{
					if (card.TagList[k] != (ushort)Convert.ToByte(this.m_tags[k]))
					{
						return true;
					}
				}
		...
		private void SetFriendCardByWidgets(ref COMDT_FRIEND_CARD card, bool checkNeed = false)
		{
			...
			if (!checkNeed)
			{
				...
				StringHelper.StringToUTF8Bytes(arg + '~' + arg2, ref card.szPlace);
			}
			InputField signInputText = this.GetSignInputText();
			if (signInputText && (card.szDatingDeclaration.Length == 0 || !checkNeed))
			{
				StringHelper.StringToUTF8Bytes(signInputText.text, ref card.szDatingDeclaration);
			}
			CUIFormScript form = Singleton<CUIManager>.instance.GetForm(CPlayerInfoSystem.sPlayerInfoFormPath);
			if (form == null)
			{
			...
			if (x != null && !checkNeed)
			{
				card.bTagNum = (byte)this.m_tags.Count;
				card.TagList = new ushort[(int)card.bTagNum];
				for (int i = 0; i < this.m_tags.Count; i++)
				{
					card.TagList[i] = (ushort)Convert.ToByte(this.m_tags[i]);
				}
			}
		}
		...
		public void OnEditTag(CUIEvent evt)
		{
			...
			if (widget != null)
			{
				...
				for (int i = 0; i < num; i++)
				{
					...
					if (cUIToggleListElementScript != null)
					{
						...
						if (bType != 1)
						{
							if (bType == 2)
							{
								component2.color = this.m_colorTagKawaii;
							}
						}
						else
						{
							component2.color = this.m_colorTagCool;
						}
					}
				}
				...
				for (int j = 0; j < this.m_tags.Count; j++)
				{
					component.SetMultiSelected(Convert.ToInt32(this.m_tags[j]), true);
				}
			}
		...
		private void OnTagSelectChanged(CUIEvent evt)
		{
			...
			widget2.GetComponent<Text>().text = text;
		}
		...
		private void SetHideCardButton()
		{
			...
			if (profile.IsPrivacyOpen(COM_USER_PRIVACY_MASK.COM_USER_PRIVACY_MASK_FRIEND_CARD))
			{
				component.text = Singleton<CTextManager>.GetInstance().GetText("Social_ShowCard");
			}
			else
			{
				component.text = Singleton<CTextManager>.GetInstance().GetText("Social_HideCard");
			}
		}
