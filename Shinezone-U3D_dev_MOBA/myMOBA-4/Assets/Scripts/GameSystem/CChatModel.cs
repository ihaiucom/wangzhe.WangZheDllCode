...
namespace Assets.Scripts.GameSystem
{
	public class CChatModel
	{
		...
		public void AddOfflineChatIndex(ulong ullUid, uint dwLogicWorldId, int index)
		{
			...
			{
				COfflineChatIndex cOfflineChatIndex2 = this.stOfflineChatIndexList[i];
		...
		public void ClearCOfflineChatIndex(ulong ullUid, uint dwLogicWorldId)
		{
			...
			for (int i = 0; i < this.stOfflineChatIndexList.Count; i++)
			{
				COfflineChatIndex cOfflineChatIndex = this.stOfflineChatIndexList[i];
		...
		private List<CChatModel.HeroChatTemplateInfo> GetCurGroupInfo(int groupID)
		{
			...
			{
				if (this.selectHeroTemplateList[i].GetGroupID() == groupID)
				{
		...
		public void Load_HeroSelect_ChatTemplate()
		{
			if (this.selectHeroTemplateList.Count == 0)
			{
				...
				while (enumerator.MoveNext())
				{
					...
					ResHeroSelectTextData resHeroSelectTextData = (ResHeroSelectTextData)current.Value;
					CChatModel.HeroChatTemplateInfo item = new CChatModel.HeroChatTemplateInfo(0);
					item.dwID = (int)resHeroSelectTextData.dwID;
					item.dwGroupID = (int)resHeroSelectTextData.dwGroupID;
					item.dwTag = (int)resHeroSelectTextData.dwTag;
					item.templateString = StringHelper.UTF8BytesToString(ref resHeroSelectTextData.szContent);
