...
namespace Assets.Scripts.GameSystem
{
	internal class CFakePvPHelper
	{
			...
			for (COM_PLAYERCAMP cOM_PLAYERCAMP = COM_PLAYERCAMP.COM_PLAYERCAMP_1; cOM_PLAYERCAMP < COM_PLAYERCAMP.COM_PLAYERCAMP_COUNT; cOM_PLAYERCAMP++)
			{
				...
				for (int i = 0; i < listView.Count; i++)
				{
					...
					if (memberInfo.RoomMemberType == 2u)
					{
						...
						fakePlayerConfirm.confirmWaitTime = UnityEngine.Random.Range(2, 11);
		...
		private static List<CFakePvPHelper.FakePlayerConfirm> SelectConfirmPlayer(int time)
		{
			...
			{
				if (!CFakePvPHelper.FakePlayerList[i].bConfirmed)
				{
					if (CFakePvPHelper.FakePlayerList[i].confirmWaitTime == time)
					{
		...
		private static void UpdateFakeConfirm(int seq)
		{
			...
			if (form != null)
			{
				...
				{
					CFakePvPHelper.FakePlayerConfirm fakePlayerConfirm = list[i];
		...
		public static void BeginFakeSelectHero()
		{
			...
			for (int i = 0; i < listView.Count; i++)
			{
				...
				if (memberInfo.RoomMemberType == 2u)
				{
					...
					{
						fakeHeroSelect.maxChangeHeroCount = UnityEngine.Random.Range(0, 2);
						CFakePvPHelper.DoSelectAction(ref fakeHeroSelect);
					}
					else
					{
						fakeHeroSelect.maxChangeHeroCount = UnityEngine.Random.Range(1, 4);
					}
					fakeHeroSelect.nextActionSec = UnityEngine.Random.Range(3, 6);
		...
		private static void DoSelectAction(ref CFakePvPHelper.FakeHeroSelect fakeSelect)
		{
			...
			if (fakeSelect.changeHeroCount < fakeSelect.maxChangeHeroCount)
			{
				...
				{
					int num = UnityEngine.Random.Range(0, GameDataMgr.robotRookieHeroSkinDatabin.Count());
					...
					{
						num = UnityEngine.Random.Range(0, GameDataMgr.robotRookieHeroSkinDatabin.Count());
				...
				else
				{
					...
					{
						num2 = masterRoleInfo.freeHeroList[num - num3].dwFreeHeroID;
					}
				...
				CFakePvPHelper.ChosenHeroes[fakeSelect.FakePlayer.ullUid] = num2;
			...
			else
			{
				...
				else
				{
					...
					{
						int num5 = UnityEngine.Random.Range(0, 10000);
				...
				if (dataByKey3 != null)
				{
					int num6 = UnityEngine.Random.Range(0, dataByKey3.SkillId.Length);
					while (dataByKey3.SkillId[num6] == 0u)
					{
						num6 = UnityEngine.Random.Range(0, dataByKey3.SkillId.Length);
					}
		...
		{
			CFakePvPHelper.ChosenHeroes[selfUid] = heroID;
		}
		...
		private static void SelectHeroForEnemyPlayer(ref COMDT_HEROINFO heroInfo, uint playerLevel)
		{
			...
			else
			{
				...
				{
					heroID = masterRoleInfo.freeHeroList[num - num3].dwFreeHeroID;
				}
				while (CFakePvPHelper.EnemyChosenHeroes.FindIndex((uint x) => x == heroID) != -1)
				{
					num = UnityEngine.Random.Range(0, max);
			...
			if (dataByKey != null)
			{
				int num4 = UnityEngine.Random.Range(0, dataByKey.SkillId.Length);
				while (dataByKey.SkillId[num4] == 0u)
				{
					num4 = UnityEngine.Random.Range(0, dataByKey.SkillId.Length);
				}
		...
		private static void OnSelectHeroTimeout(int seq)
		{
			...
			if (form != null)
			{
				...
				if (roomInfo != null && roomInfo.GetMasterMemberInfo() != null)
				{
					...
					DebugHelper.Assert(pvPHeroList.Count > CFakePvPHelper.ChosenHeroes.Count, "May have not enough Candidate Heroes!!!");
					int index = UnityEngine.Random.Range(0, pvPHeroList.Count);
					IHeroData heroData = pvPHeroList[index];
					while (CFakePvPHelper.ChosenHeroes.ContainsValue(heroData.cfgID))
					{
						index = UnityEngine.Random.Range(0, pvPHeroList.Count);
		...
		public static void FakeLoadProcess(float progress)
		{
			...
			while (enumerator.MoveNext())
			{
				if (enumerator.Current.Computer)
				{
					...
					GameObject gameObject = (enumerator.Current.PlayerCamp != COM_PLAYERCAMP.COM_PLAYERCAMP_1) ? form.gameObject.transform.FindChild("DownPanel").FindChild(string.Format("Down_Player{0}", num)).gameObject : form.gameObject.transform.FindChild("UpPanel").FindChild(string.Format("Up_Player{0}", num)).gameObject;
					if (gameObject != null)
					{
						...
						if (gameObject2)
						{
							...
							if (!(text == "100%"))
							{
								...
								int num2 = Math.Min(100, Convert.ToInt32(progress * 100f + (float)UnityEngine.Random.Range(0, max)));
								int num3;
								int.TryParse(text.Substring(0, 2), out num3);
								if (num2 > num3)
								{
