	...
	public class CRoleInfo
	{
						...
						if (PlayerPrefs.HasKey("GAMER_REDDOT"))
						{
							...
							if (@string.Contains(accountInfo.OpenId))
							{
								...
								if (array != null)
								{
									...
									while (i < num)
									{
										if (array[i] == accountInfo.OpenId && long.TryParse(array[i + 1], out secondsFromUtcStart))
										{
		...
		public uint BattlePower
		{
			get
			{
				...
				{
					ListView<CHeroInfo> arg_33_0 = listView;
					KeyValuePair<uint, CHeroInfo> current = enumerator.Current;
					arg_33_0.Add(current.Value);
				}
		...
		{
			return inIndex < this.NewbieAchieveBits.Count && this.NewbieAchieveBits.Get(inIndex);
		}
			...
			if (this.bNewbieAchieveChanged || resetOldPlayerGuided)
			{
				...
				for (int i = 0; i < 2; i++)
				{
					for (int j = 0; j < 64; j++)
					{
						...
						{
							cSPkg.stPkgData.stUpdateClientBitsNtf.stClientBits.BitsDetail[i] += ((!this.NewbieAchieveBits[64 * i + j]) ? 0uL : (1uL << j));
						}
					}
				}
		...
		public void SyncClientBitsToSvr()
		{
			if (this._clientBitsChanged)
			{
				...
				for (int i = 0; i < 5; i++)
				{
					for (int j = 0; j < 64; j++)
					{
						cSPkg.stPkgData.stUpdNewClientBits.stClientBits.BitsDetail[i] += ((!this._clientBits[64 * i + j]) ? 0uL : (1uL << j));
					}
				}
				Singleton<NetworkModule>.GetInstance().SendLobbyMsg(ref cSPkg, false);
		...
		{
			return this.GuidedStateBits != null && this.GuidedStateBits[105];
		}

		public bool IsOldPlayerGuided()
		{
			return this.NewbieAchieveBits != null && this.NewbieAchieveBits[17];
		}

		private void SetInBattleNewbieBit(int iLevelId, uint ageId, bool bIsLastAge)
		{
			stInBattleLevelBits value;
			if (!this.inBattleNewbieBits.TryGetValue(iLevelId, out value))
			{
				value = default(stInBattleLevelBits);
                value.bReportFinished = ((!bIsLastAge) ? (byte)0 : (byte)1);
				value.finishedDetail = new List<uint>();
				value.finishedDetail.Add(ageId);
				this.inBattleNewbieBits.Add(iLevelId, value);
			}
			else
			{
                value.bReportFinished = ((!bIsLastAge) ? (byte)0 : (byte)1);
		...
		public void InitInBattleNewbieBits(COMDT_INBATTLE_NEWBIE_BITS_DETAIL inBattleBits)
		{
			...
			for (int i = 0; i < (int)inBattleBits.bLevelNum; i++)
			{
				...
				{
					stInBattleLevelBits value = default(stInBattleLevelBits);
					value.bReportFinished = cOMDT_INBATTLE_NEWBIE_BITS_INFO.bReportFinished;
					value.finishedDetail = new List<uint>();
					for (int j = 0; j < (int)cOMDT_INBATTLE_NEWBIE_BITS_INFO.bFinishedNum; j++)
					{
						value.finishedDetail.Add(cOMDT_INBATTLE_NEWBIE_BITS_INFO.FinishedDetail[j]);
					}
					this.inBattleNewbieBits.Add(cOMDT_INBATTLE_NEWBIE_BITS_INFO.iLevelID, value);
				}
			}
		}
		...
		public void ReqSetInBattleNewbieBit(uint id, bool bIsLastAge, int time = 0)
		{
			...
			if (curLvelContext != null && curLvelContext.IsGameTypeGuide())
			{
				...
				if (this.inBattleNewbieBits.TryGetValue(curLvelContext.m_mapID, out stInBattleLevelBits))
				{
                ...
                cSPkg.stPkgData.stDyeInBattleNewbieBitReq.bIsLastAge = ((!bIsLastAge) ? (byte)0 : (byte)1);
				Singleton<NetworkModule>.GetInstance().SendLobbyMsg(ref cSPkg, false);
		...
		public int GetHaveHeroCount(bool isIncludeValidExperienceHero)
		{
			...
			while (enumerator.MoveNext())
			{
				if (isIncludeValidExperienceHero)
				{
					...
					if (this.IsValidExperienceHero(current2.Key))
					{
						goto IL_4D;
					}
					continue;
				...
				else
				{
					...
					if (this.IsOwnHero(current3.Key))
					{
						num++;
					}
				}
		...
		public int GetHaveHeroCountWithoutBanHeroID(bool isIncludeValidExperienceHero, byte mapType, uint mapID)
		{
			...
			while (enumerator.MoveNext())
			{
				if (isIncludeValidExperienceHero)
				{
					...
					continue;
					IL_A2:
					List<uint> arg_B3_0 = list;
					KeyValuePair<uint, CHeroInfo> current3 = enumerator.Current;
					if (!arg_B3_0.Contains(current3.Key))
					{
						num++;
					}
				}
				else
				{
					...
					if (this.IsOwnHero(current4.Key))
					{
						List<uint> arg_F2_0 = list;
						KeyValuePair<uint, CHeroInfo> current5 = enumerator.Current;
						if (!arg_F2_0.Contains(current5.Key))
						{
							num++;
						}
					}
		...
		public void SetSkillPoint(uint pt, bool bRefreshTime = false, bool bDispathEvent = false)
		{
			...
			if (bRefreshTime)
			{
				if ((long)(this.m_skillPtRefreshSec * 1000) > (long)(0xffffffffL))
				{
		...
		public void SetAttributes(SCPKG_CMD_GAMELOGINRSP rsp)
		{
			...
			if ((ulong)(rsp.dwSPUpdTimeSec * 1000u) > 0xffffffffL)
			{
				this.m_updateTimeMSec = 4294967295u;
			}
			else
			...
			if (Utility.ToUtcTime2Local((long)((ulong)rsp.dwServerCurTimeSec)).Date == dateTime.Date)
			{
			...
			else
			{
				...
				if ((Utility.ToUtcTime2Local((long)((ulong)rsp.stPasswdInfo.dwPswdCloseTime)) - d).TotalSeconds > 0.0)
				{
		...
		public void SetGlobalRefreshTimer(uint serverTime, bool force = false)
		{
			...
			int num2 = (num - Convert.ToInt32(dateTime.TimeOfDay.TotalSeconds) <= 0) ? (num - Convert.ToInt32(dateTime.TimeOfDay.TotalSeconds) + 86400) : (num - Convert.ToInt32(dateTime.TimeOfDay.TotalSeconds));
		...
		public bool IsCreditFreeHero(uint heroId)
		{
			for (int i = 0; i < this.freeHeroList.Count; i++)
			{
				if (this.freeHeroList[i].dwFreeHeroID == heroId)
				{
					return this.freeHeroList[i].dwCreditLevel > 0u;
				}
			}
			return false;
		}
		...
		public void OnAddHeroSkin(uint heroId, uint skinId)
		{
			...
			if (this.heroSkinDic.ContainsKey(heroId))
			{
				...
				Dictionary<uint, ulong> expr_1F = dictionary = this.heroSkinDic;
				ulong num2 = dictionary[heroId];
				expr_1F[heroId] = (num2 | num);
			}
			else
			{
		...
		public void OnGmAddAllSkin()
		{
			...
			while (enumerator.MoveNext())
			{
				...
				{
					this.heroSkinDic[key] = 18446744073709551615uL;
				}
				else
				{
		...
		public uint UInt32ChgAdjust(uint bs, int chg)
		{
			if (chg >= 0)
			{
				if ((ulong)bs + (ulong)((long)chg) >= 0xffffffffL)
				{
					return 4294967295u;
				}
				return bs + (uint)chg;
		...
		public bool IsValidExperienceSkin(uint heroId, uint skinId)
		{
			...
			return this.heroExperienceSkinDic.ContainsKey(skinCfgId) && (long)CRoleInfo.GetCurrentUTCTime() < (long)((ulong)this.heroExperienceSkinDic[skinCfgId]);
		}
		...
		public int GetExperienceSkinLeftTime(uint heroId, uint skinId)
		{
			...
			{
				return (int)(this.heroExperienceSkinDic[skinCfgId] - (uint)CRoleInfo.GetCurrentUTCTime());
			}
			return 0;
		}
		...
		public void SetCoinGetCntData(SCPKG_COINGETPATH_RSP coinData)
		{
			...
			while ((long)num < (long)((ulong)coinData.dwPathNum))
			{
				...
				{
					this.m_coinGetInfoDaily[(int)((UIntPtr)coinData.astPathCoin[num].dwPath)].GetCntDaily = coinData.astPathCoin[num].dwCoin;
				}
				num++;
			}
		}
