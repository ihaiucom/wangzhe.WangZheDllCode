...
namespace Assets.Scripts.GameSystem
{
	public class BannerImageSys : MonoSingleton<BannerImageSys>
	{
		...
		public string GetCDNUrl(uint id)
		{
			...
			{
				BannerImageSys.CDNUrl cDNUrl = this.m_CDNUrlMgr[i];
				if (cDNUrl.id == id)
				{
					return cDNUrl.url;
				}
		...
		public void LoadConfigServer()
		{
			...
			if (count > 0)
			{
				...
				while (enumerator.MoveNext())
				{
					...
					ResBannerImage value = current.Value;
		...
		private void PreloadBannerImage()
		{
			...
			for (int i = 0; i < imageListCount; i++)
			{
				...
				if (bannerImageInfo != null)
				{
					...
					{
						BannerImageSys.CDNUrl item = default(BannerImageSys.CDNUrl);
						item.id = bannerImageInfo.resImgInfo.dwID;
						item.url = bannerImageInfo.resImgInfo.szHttpUrl;
		...
		protected override void Init()
		{
			...
			{
				Debug.Log("bannerimagesys cannot create dictionary " + arg);
				this.m_GlobalBannerImagePath = CFileManager.GetCachePath();
			}
