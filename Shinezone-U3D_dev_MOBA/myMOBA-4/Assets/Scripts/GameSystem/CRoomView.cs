	...
	internal class CRoomView
	{
		public static void SetRoomData(GameObject root, RoomInfo data)
		{
			...
			for (int j = 1; j <= 5; j++)
			{
				...
				MemberInfo memberInfo = data.GetMemberInfo(COM_PLAYERCAMP.COM_PLAYERCAMP_2, j - 1);
				CRoomView.SetPlayerSlotData(gameObject, memberInfo, masterMemberInfo, COM_PLAYERCAMP.COM_PLAYERCAMP_2, j - 1, num >= j, fromType);
			}
		...
		public static void UpdateBtnStatus(GameObject root, RoomInfo data)
		{
			...
			if (isSelfRoomOwner && masterRoleInfo != null)
			{
				...
				if (memberInfo != null)
				{
					COM_PLAYERCAMP cOM_PLAYERCAMP = (memberInfo.camp != COM_PLAYERCAMP.COM_PLAYERCAMP_1) ? COM_PLAYERCAMP.COM_PLAYERCAMP_1 : COM_PLAYERCAMP.COM_PLAYERCAMP_2;
		...
		private static void SetPlayerSlotData(GameObject item, MemberInfo memberInfo, MemberInfo masterMemberInfo, COM_PLAYERCAMP camp, int pos, bool bAvailable, COM_ROOM_FROMTYPE fromType)
		{
			if (bAvailable)
			{
				...
				else
				{
					...
					string text = (!flag2) ? Singleton<CInviteSystem>.GetInstance().GetInviteGuildMemberName(memberInfo.ullUid) : Singleton<CInviteSystem>.GetInstance().GetInviteFriendName(memberInfo.ullUid, (uint)memberInfo.iLogicWorldID);
		...
		public static void SetChgEnable(bool enable)
		{
			...
			for (int i = num; i <= num2; i++)
			{
				...
				for (int j = 0; j < 5; j++)
				{
					...
					string path = string.Format("Panel_Main/{0}{1}/Occupied/BtnSwap", (cOM_PLAYERCAMP != COM_PLAYERCAMP.COM_PLAYERCAMP_1) ? "RightPlayers/Right_Player" : "LeftPlayers/Left_Player", j + 1);
		...
		public static void ShowSwapMsg(int totalSec, COM_PLAYERCAMP camp = COM_PLAYERCAMP.COM_PLAYERCAMP_1, int pos = 0)
		{
			if (totalSec > 0)
			{
				...
				vector.y += (float)((camp != COM_PLAYERCAMP.COM_PLAYERCAMP_MID) ? 28 : -20);
		...
		public static void UpdateSwapBox(COM_PLAYERCAMP camp, int pos)
		{
			...
			componetInChild.text = Singleton<CTextManager>.instance.GetText("Room_Change_Pos_Tip_3", new string[]
			{
