...
namespace Assets.Scripts.GameSystem
{
	...
	public class CHeroSkinBuyManager : Singleton<CHeroSkinBuyManager>
	{
			...
			if (cUIListScript != null)
			{
				...
				if (selectedIndex >= 0 || selectedIndex < count)
				{
					...
					else
					{
						...
						if (componetInChild != null)
						{
							msg = CUIUtility.RemoveEmoji(componetInChild.text);
						}
					}
				}
			}
		...
		private void OnPredefinedMsgTypeSelected(CUIEvent uiEvent)
		{
			...
			{
				this.m_lastCustomMsg = componetInChild.text;
			}
			if (this.m_askForTemplateList[selectedIndex].dwID == this.m_maxAskForTemplateID)
			{
				if (!string.IsNullOrEmpty(this.m_lastCustomMsg))
				{
					componetInChild.text = this.m_lastCustomMsg;
					componetInChild.placeholder.GetComponent<Text>().text = string.Empty;
				}
				else
				{
					componetInChild.text = string.Empty;
					componetInChild.placeholder.GetComponent<Text>().text = this.m_askForTemplateList[selectedIndex].szContent;
				}
				componetInChild.enabled = true;
			}
			else
			{
				componetInChild.placeholder.GetComponent<Text>().text = string.Format(this.m_askForTemplateList[selectedIndex].szContent, uiEvent.m_eventParams.tagStr);
				componetInChild.text = string.Empty;
		...
		private void InitBuyForFriendForm(CUIFormScript form, RES_ITEM_TYPE giftType, uint heroId, uint skinId = 0u, ulong friendUid = 0uL, uint worldId = 0u, enFriendGiftOpType opType = enFriendGiftOpType.Present)
		{
			...
			uint num3 = (conditionParam.Length <= 1) ? 1u : conditionParam[0];
			...
			if (giftType == RES_ITEM_TYPE.RES_ITEMTYPE_HEROSKIN)
			{
				...
				if (heroSkin != null)
				{
					...
					{
						component.text = Singleton<CTextManager>.GetInstance().GetText("Ask_For_Skin_Friend_Title");
					}
					...
					if (transform4 != null)
					{
						...
						if (component4 != null)
						{
							if (opType == enFriendGiftOpType.Present)
							{
								component4.text = string.Format(Singleton<CTextManager>.GetInstance().GetText("Buy_Skin_For_Friend_Tip"), num3);
							}
							else
							{
								component4.text = string.Format(Singleton<CTextManager>.GetInstance().GetText("Ask_For_Skin_Friend_Tip"), num3, srv2CltGlobalValue);
							}
						}
					}
				}
			...
			else
			{
				...
				if (dataByKey != null)
				{
					...
					if (opType == enFriendGiftOpType.Present)
					{
						component5.text = Singleton<CTextManager>.GetInstance().GetText("Mall_Hero_Give_Title");
					}
					else
					{
						component5.text = Singleton<CTextManager>.GetInstance().GetText("Ask_For_Hero_Friend_Title");
					}
					Text component6 = form.transform.Find("Panel/skinBgImage/skinNameText").GetComponent<Text>();
					component6.text = StringHelper.UTF8BytesToString(ref dataByKey.szName);
					...
					if (transform8 != null)
					{
						...
						if (component8 != null)
						{
							if (opType == enFriendGiftOpType.Present)
							{
								component8.text = string.Format(Singleton<CTextManager>.GetInstance().GetText("Buy_Hero_For_Friend_Tip"), num3);
							}
							else
							{
								component8.text = string.Format(Singleton<CTextManager>.GetInstance().GetText("Ask_For_Hero_Friend_Tip"), num3, srv2CltGlobalValue);
							}
						}
					}
				}
		...
		private void InitLeaveMsgForFriendForm(CUIFormScript form, int friendIndex = 0)
		{
			...
			if (this.m_giftType == RES_ITEM_TYPE.RES_ITEMTYPE_HEROSKIN)
			{
				...
				if (transform4 != null)
				{
					...
					if (component4 != null)
					{
						if (this.m_giftOpType == enFriendGiftOpType.Present)
						{
							component4.text = string.Format(Singleton<CTextManager>.GetInstance().GetText("Buy_Skin_For_Friend_Tip"), num);
						}
						else
						{
							component4.text = string.Format(Singleton<CTextManager>.GetInstance().GetText("Ask_For_Skin_Friend_Title"), num, srv2CltGlobalValue);
						}
					}
				}
			}
			else
			{
				...
				if (component8 != null)
				{
					if (this.m_giftOpType == enFriendGiftOpType.Present)
					{
						component8.text = string.Format(Singleton<CTextManager>.GetInstance().GetText("Buy_Hero_For_Friend_Tip"), num);
					}
					else
					{
						component8.text = string.Format(Singleton<CTextManager>.GetInstance().GetText("Ask_For_Hero_Friend_Title"), num, srv2CltGlobalValue);
					}
				}
			}
			...
			if (componetInChild != null)
			{
				if (this.m_giftOpType == enFriendGiftOpType.Present)
				{
					componetInChild.text = Singleton<CTextManager>.GetInstance().GetText("Buy_For_To");
				}
				else
				{
					componetInChild.text = Singleton<CTextManager>.GetInstance().GetText("Ask_For_From");
				}
			}
			...
			if (gameObject3 != null)
			{
				...
				if (this.m_giftOpType == enFriendGiftOpType.Present)
				{
					componetInChild2.text = Singleton<CTextManager>.GetInstance().GetText("Buy_For_Friend_Op");
				}
				else
				{
					componetInChild2.text = Singleton<CTextManager>.GetInstance().GetText("Ask_For_Friend_Op");
				}
			...
			if (component10 != null)
			{
				component10.onValueChange.AddListener(new UnityAction<string>(this.On_InputFiled_ValueChange));
			}
			...
			if (this.m_giftOpType == enFriendGiftOpType.AskFor)
			{
				...
				while (enumerator.MoveNext())
				{
					...
					ResAskforTemplet resAskforTemplet = current.Value as ResAskforTemplet;
				...
				if (gameObject4 != null)
				{
					...
					for (int i = 0; i < count; i++)
					{
						...
						if (component11 != null)
						{
							component11.text = Singleton<CTextManager>.GetInstance().GetText(string.Format("HeroSkinBuy_Ask_For_Template_Name_{0}", this.m_askForTemplateList[i].dwID));
						}
					}
		...
		private void On_InputFiled_ValueChange(string arg0)
		{
			...
			if (text.Length > 50)
			{
				...
				if (component != null)
				{
					...
					component.text = text.Substring(0, 50);
		...
		private void OnFriendListElementEnable(CUIEvent uiEvent)
		{
			...
			if (gameObject != null)
			{
				...
				if (this.m_giftOpType == enFriendGiftOpType.Present)
				{
					componetInChild.text = Singleton<CTextManager>.GetInstance().GetText("Buy_For_Friend_Op");
				}
				else
				{
