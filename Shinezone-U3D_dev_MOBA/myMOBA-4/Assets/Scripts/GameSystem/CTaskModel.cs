	...
	public class CTaskModel
	{
		...
		public void ParseCltCalcCompletedTasks(ref uint[] taskids)
		{
			...
			for (int j = 0; j < taskids.Length; j++)
			{
				...
				if (num != 0u)
				{
					...
					if (dataByKey != null)
					{
						...
						{
							if (num < dictionary[dataByKey.bTaskSubType])
							{
								dictionary[dataByKey.bTaskSubType] = num;
							}
						}
						else
						{
			...
			while (enumerator2.MoveNext())
			{
				...
				ResTask resTask = (ResTask)current2.Value;
				if (resTask != null)
				{
					if (resTask.dwTaskType == 0u)
					{
						if (dictionary.ContainsKey(resTask.bTaskSubType) && resTask.dwTaskID < dictionary[resTask.bTaskSubType])
						{
			...
			{
				KeyValuePair<byte, uint> current3 = enumerator3.Current;
				if (this.GetTask(current3.Value) == null)
				{
					DatabinTable<ResTask, uint> arg_25E_0 = GameDataMgr.taskDatabin;
					KeyValuePair<byte, uint> current4 = enumerator3.Current;
					ResTask dataByKey2 = arg_25E_0.GetDataByKey(current4.Value);
		...
		public void ParseLevelRewardData()
		{
			...
			while (enumerator.MoveNext())
			{
				...
				ResPvpLevelReward resPvpLevelReward = (ResPvpLevelReward)current.Value;
		...
		public void Load_Share_task()
		{
			...
			{
				KeyValuePair<long, object> current = enumerator.Current;
				ResTask resTask = (ResTask)current.Value;
