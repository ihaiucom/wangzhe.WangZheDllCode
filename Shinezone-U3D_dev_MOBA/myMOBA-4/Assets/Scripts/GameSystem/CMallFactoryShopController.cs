	...
	public class CMallFactoryShopController : Singleton<CMallFactoryShopController>
	{
		public class ShopProduct
		{
			...
			public int LimitBuyDays
			{
				get
				{
					...
					DateTime value = new DateTime(dateTime.Year, dateTime.Month, dateTime.Day, 0, 0, 0, DateTimeKind.Local);
					if (this.OnSaleTime.CompareTo(value) >= 0)
					{
						return (int)this.OffSaleTime.Subtract(this.OnSaleTime).TotalDays;
					}
					if (this.OffSaleTime.CompareTo(value) >= 0)
					{
						return (int)this.OffSaleTime.Subtract(value).TotalDays;
					}
					return -1;
				}
			}
		...
		private class ShopProductWidget
		{
			...
			public void Validate()
			{
				...
				{
					this.coinCost.text = this.data.ConvertWithRealDiscount(cUseable2.GetBuyPrice(this.data.CoinType)).ToString();
				}
				if (this.OldPriceObj != null && cUseable2 != null && this.data != null)
				{
					if (this.data.RealDiscount < 10000f)
					{
						...
						{
							this.OldPriceText.text = cUseable2.GetBuyPrice(this.data.CoinType).ToString();
						}
					}
					else
					{
				...
				{
					this.name.text = cUseable2.m_name;
				}
				if (this.desc != null && cUseable2 != null)
				{
					this.desc.text = cUseable2.m_description;
				}
					...
					if (this.limitCycle != null)
					{
						if (this.data.LimitCycle > 0u)
						{
							...
							if (num != 1u)
							{
								if (num != 7u)
								{
									this.limitCycle.text = Singleton<CTextManager>.GetInstance().GetText("Mall_Factory_Shop_Limit_Cycle_Today");
								}
								else
								{
									this.limitCycle.text = Singleton<CTextManager>.GetInstance().GetText("Mall_Factory_Shop_Limit_Cycle_Week");
								}
							}
							else
							{
						...
						{
							this.limitCycle.text = Singleton<CTextManager>.GetInstance().GetText("Mall_Factory_Shop_Limit_Cycle_Forever");
						}
					}
				}
				else
		...
		public void InitLocalData()
		{
			...
			while (enumerator.MoveNext())
			{
				...
				ResSpecSale value = current.Value;
				if (value != null)
				{
		...
		private void SortProducts()
		{
			...
			{
				KeyValuePair<uint, ListView<CMallFactoryShopController.ShopProduct>> current = enumerator.Current;
				current.Value.Sort((CMallFactoryShopController.ShopProduct l, CMallFactoryShopController.ShopProduct r) => r.RecommendIndex - l.RecommendIndex);
			}
		...
		private void InitTab(CUIFormScript form)
		{
			...
			string[] array = new string[this._availableTabs.Count];
		...
		public void BuyShopProduct(CMallFactoryShopController.ShopProduct shopProduct, uint count, bool needConfirm, CUIEvent uiEvent = null)
		{
			...
			CMallSystem.TryToPay(enPayPurpose.Buy, string.Format("{0}{1}", cUseable.m_name, (count <= 1u) ? string.Empty : ("x" + count)), payType, payValue, uiEvent.m_eventID, ref uiEvent.m_eventParams, enUIEventID.None, needConfirm, true, false);
		}
