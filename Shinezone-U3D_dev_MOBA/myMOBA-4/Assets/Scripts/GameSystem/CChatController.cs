	...
	public class CChatController : Singleton<CChatController>
	{
		...
		public void ClearAll()
		{
			...
			if (this.actionBtnsDic != null)
			{
				...
				while (enumerator.MoveNext())
				{
					...
					ListView<CChatController.ActionBtn> value = current.Value;
					if (value != null)
					{
						value.Clear();
					}
		...
		private uint GetLastRecruitInfoTime()
		{
			...
			if (this.model != null && this.model.sysData != null && this.model.sysData.m_guildRecruitInfos.Count > 0)
			{
				...
				{
					if (num < this.model.sysData.m_guildRecruitInfos[i].sendTime)
					{
						num = this.model.sysData.m_guildRecruitInfos[i].sendTime;
					}
				}
			}
			return num;
		...
		public void BuildWarmBattlePlayerLeaveRoomSystemMsg()
		{
			...
			while (enumerator.MoveNext())
			{
				...
				if (current.Value.IsComputer)
				{
					...
					this.BuildPlayerLeaveRoomSystemMsg(current2.Value.PlayerName);
				}
			}
		}
		...
		public void RefreshGuildRecruitInfo(SCPKG_GET_GUILD_RECRUIT_RSP rsp)
		{
			...
			{
				GuildRecruitInfo item = default(GuildRecruitInfo);
				...
				item.guildLogicWorldId = cOMDT_GUILD_RECRUIT_INFO.iLogicWorldID;
			...
			int num = this.model.sysData.m_guildRecruitInfos.Count - 100;
			if (num > 0)
			{
			...
			for (int i = num - 1; i >= 0; i--)
			{
				...
				if (CChatUT.Convert_ChatMsgType_Channel(cOMDT_CHAT_MSG.bType) == EChatChannel.Friend)
				{
					...
					else
					{
						...
						CChatEntity chatEnt = CChatUT.Build_4_Friend(cOMDT_CHAT_MSG.stContent.stPrivate);
				...
				else if (CChatUT.Convert_ChatMsgType_Channel(cOMDT_CHAT_MSG.bType) == EChatChannel.Select_Hero)
				{
					...
					CChatEntity chatEnt2 = CChatUT.Build_4_SelectHero(cOMDT_CHAT_MSG.stContent.stBattle);
					this.model.channelMgr.Add_ChatEntity(chatEnt2, EChatChannel.Select_Hero, 0uL, 0u);
			...
			if (this.actionBtnsDic.ContainsKey(enUIEventID.Guild_JoinPlatformGroup))
			{
				...
				for (int i = 0; i < listView.Count; i++)
				{
					...
					else
					{
						...
						float num = 0f;
						for (int j = 0; j < actionBtn.name.Length; j++)
						{
							char ch = actionBtn.name[i];
						...
						{
							componetInChild.text = actionBtn.name;
