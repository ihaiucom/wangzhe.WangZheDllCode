	...
	public class WatchController : Singleton<WatchController>
	{
		...
		public uint EndFrameNo
		{
			get
			{
				...
				{
					return (!this._isStreamEnd) ? ((this._maxFrameNo <= this._observeDelayFrames) ? 0u : (this._maxFrameNo - this._observeDelayFrames)) : this._maxFrameNo;
				}
		...
		public void UpdateFrame()
		{
			...
			else
			{
				...
				if (this._workMode == WatchController.WorkMode.Observe)
				{
					flag = (this._isStreamEnd && this._pkgQueue.Count == 0);
				}
			...
			{
				if (this.SpeedRate > 1 && Singleton<FrameSynchr>.GetInstance().CurFrameNum >= Singleton<FrameSynchr>.GetInstance().EndFrameNum && Singleton<GameReplayModule>.GetInstance().IsStreamEnd)
				{
					this.SpeedRate = 1;
				}
			}
