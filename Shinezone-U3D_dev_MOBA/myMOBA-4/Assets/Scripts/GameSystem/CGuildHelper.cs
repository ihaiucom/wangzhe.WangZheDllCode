...
namespace Assets.Scripts.GameSystem
{
	public class CGuildHelper
	{
		...
		public static string GetDonateDescription(RES_GUILD_DONATE_TYPE donateType)
		{
			...
			string text = (dwCostGold != 0u) ? dwCostGold.ToString() : dwCostCoupons.ToString();
			string text2 = Singleton<CTextManager>.GetInstance().GetText((dwCostGold != 0u) ? "Money_Type_GoldCoin" : "Money_Type_DianQuan");
		...
		public static RankpointRankInfo CreatePlayerGuildRankpointRankInfo(enGuildRankpointRankListType rankListType)
		{
			return new RankpointRankInfo
			{
				...
				rankScore = ((!CGuildHelper.IsWeekRankListType(rankListType)) ? Singleton<CGuildModel>.GetInstance().CurrentGuildInfo.RankInfo.totalRankPoint : Singleton<CGuildModel>.GetInstance().CurrentGuildInfo.RankInfo.weekRankPoint),
		...
		public static uint GetRankpointWeekAwardCoin(uint rank)
		{
			...
			return (resGuildRankReward == null) ? GameDataMgr.guildRankRewardDatabin.GetDataByKey(-1L).dwGold : resGuildRankReward.dwGold;
		}

		public static uint GetRankpointWeekAwardDiamond(uint rank)
		{
			ResGuildRankReward resGuildRankReward = GameDataMgr.guildRankRewardDatabin.FindIf((ResGuildRankReward x) => (long)x.iStartRankNo <= (long)((ulong)rank) && (ulong)rank <= (ulong)((long)x.iEndRankNo));
			return (resGuildRankReward == null) ? GameDataMgr.guildRankRewardDatabin.GetDataByKey(-1L).dwDiamond : resGuildRankReward.dwDiamond;
		}
		...
		{
			return (int)((!CGuildHelper.IsSelf(playerUid)) ? nobeLevelFromGuild : ((uint)MonoSingleton<NobeSys>.GetInstance().GetSelfNobeLevel()));
		}

		public static int GetNobeHeadIconId(ulong playerUid, uint nobeHeadIconIdFromGuild)
		{
			return (int)((!CGuildHelper.IsSelf(playerUid)) ? nobeHeadIconIdFromGuild : ((uint)MonoSingleton<NobeSys>.GetInstance().GetSelfNobeHeadIdx()));
		}
		...
		public static int GetRankpointMemberListPlayerIndex()
		{
			...
			{
				if (rankpointMemberInfoList[i].Key == Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo().playerUllUID)
				{
					return i;
				}
			}
		...
		{
			ResGuildGradeConf gradeResByRankpointScore = CGuildHelper.GetGradeResByRankpointScore(rankpointScore);
			return (uint)((gradeResByRankpointScore == null) ? (byte)0 : gradeResByRankpointScore.bIndex);
		}
		...
		public static string GetGuildJoinLimitText(int levelLimit, int gradeLimit, uint settingMask)
		{
			...
			string text3 = Singleton<CTextManager>.GetInstance().GetText((!CGuildHelper.IsGuildNeedApproval(settingMask)) ? "Guild_List_Colunm_Limit_No_Need_Apply" : "Guild_List_Colunm_Limit_Need_Apply");
