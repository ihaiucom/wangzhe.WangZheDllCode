...
namespace Assets.Scripts.GameSystem
{
	public class CMatchingView
	{
		...
		public static void InitMatchingEntry(CUIFormScript form)
		{
			...
			uint.TryParse(Singleton<CTextManager>.instance.GetText("MapID_PVP_1V1"), out array[0]);
			uint.TryParse(Singleton<CTextManager>.instance.GetText("MapID_PVP_3V3"), out array[1]);
			uint.TryParse(Singleton<CTextManager>.instance.GetText("MapID_Robot_1V1_1"), out array[2]);
			uint.TryParse(Singleton<CTextManager>.instance.GetText("MapID_Robot_1V1_1"), out array[3]);
			uint.TryParse(Singleton<CTextManager>.instance.GetText("MapID_Robot_1V1_1"), out array[4]);
			uint.TryParse(Singleton<CTextManager>.instance.GetText("MapID_Robot_3V3Team_1"), out array[5]);
			uint.TryParse(Singleton<CTextManager>.instance.GetText("MapID_Robot_3V3Team_1"), out array[6]);
			uint.TryParse(Singleton<CTextManager>.instance.GetText("MapID_Robot_3V3Team_1"), out array[7]);
			uint.TryParse(Singleton<CTextManager>.instance.GetText("MapID_Robot_5V5Team_1"), out array[8]);
			uint.TryParse(Singleton<CTextManager>.instance.GetText("MapID_Robot_5V5Team_1"), out array[9]);
			uint.TryParse(Singleton<CTextManager>.instance.GetText("MapID_Robot_5V5Team_1"), out array[10]);
			uint.TryParse(Singleton<CTextManager>.instance.GetText("MapID_Robot_Melee_Team_1"), out array[11]);
			uint.TryParse(Singleton<CTextManager>.instance.GetText("MapID_Robot_Melee_Team_1"), out array[12]);
			uint.TryParse(Singleton<CTextManager>.instance.GetText("MapID_Robot_Melee_Team_1"), out array[13]);
			uint.TryParse(Singleton<CTextManager>.instance.GetText("MapID_PVP_5V5"), out array[14]);
			uint.TryParse(Singleton<CTextManager>.instance.GetText("MapID_PVP_MELEE"), out array[15]);
			uint.TryParse(Singleton<CTextManager>.instance.GetText("MapID_PVP_Fire"), out array[16]);
			uint.TryParse(Singleton<CTextManager>.instance.GetText("MapID_PVP_5V5Clone"), out array[17]);
			uint.TryParse(Singleton<CTextManager>.instance.GetText("MapID_PVP_5V5CD"), out array[18]);
			uint.TryParse(Singleton<CTextManager>.instance.GetText("MapID_PVP_5V5Miwu"), out array[19]);
			uint.TryParse(Singleton<CTextManager>.instance.GetText("MapID_Robot_5V5TeamMiwu_1"), out array[20]);
			uint.TryParse(Singleton<CTextManager>.instance.GetText("MapID_Robot_5V5TeamMiwu_1"), out array[21]);
			uint.TryParse(Singleton<CTextManager>.instance.GetText("MapID_Robot_5V5TeamMiwu_1"), out array[22]);
			...
			gameObject3.transform.Find("Name").gameObject.GetComponent<Text>().text = masterRoleInfo.Name;
			...
			{
				gameObject3.transform.Find("DegreeNum").gameObject.GetComponent<Text>().text = Singleton<CTextManager>.GetInstance().GetText("PVP_Level_Max");
			}
			else
			{
				gameObject3.transform.Find("DegreeNum").gameObject.GetComponent<Text>().text = string.Format("{0}/{1}", masterRoleInfo.PvpExp, dataByKey.dwNeedExp);
			}
		...
		public static void SetPlayerSlotData(GameObject item, TeamMember memberInfo, bool bAvailable)
		{
			if (bAvailable)
			{
				...
				else
				{
					...
					string text = (!flag2) ? Singleton<CInviteSystem>.GetInstance().GetInviteGuildMemberName(memberInfo.uID.ullUid) : Singleton<CInviteSystem>.GetInstance().GetInviteFriendName(memberInfo.uID.ullUid, (uint)memberInfo.uID.iLogicWorldId);
					item.transform.Find("Occupied/txtPlayerName").GetComponent<Text>().text = ((!string.IsNullOrEmpty(text)) ? text : memberInfo.MemberName);
		...
		public static void InitConfirmBox(GameObject root, int PlayerNum, ref RoomInfo roomInfo, CUIFormScript form)
		{
			...
			if (component3)
			{
				component3.constraintCount = ((PlayerNum != 6) ? 5 : 3);
			}
			bool flag = roomInfo.roomAttrib.bPkAI == 2;
			flag = (!roomInfo.roomAttrib.bWarmBattle && flag);
			int num = (!flag) ? PlayerNum : (PlayerNum / 2);
			for (int i = 1; i <= 10; i++)
			{
			...
			if (transform5)
			{
				...
				if (component6)
				{
					component6.text = string.Format(Singleton<CTextManager>.GetInstance().GetText("Matching_Confirm_PlayerNum"), 0, num);
				}
			}
		...
		public static void UpdateConfirmBox(GameObject root, ulong confirmPlayerUid)
		{
			...
			if (transform)
			{
				if ((roomInfo.roomAttrib.bMapType == 3 && masterRoleInfo != null && (int)CLadderSystem.GetGradeDataByShowGrade((int)masterRoleInfo.m_rankGrade).bLogicGrade >= CMatchingView.ShowDefaultHeadImgStartLogicLadderLevel && !roomInfo.roomAttrib.bWarmBattle) || roomInfo.roomAttrib.bMapType == 5)
				{
					...
					else
					{
						...
						{
							uITimerScript.SetTotalTime((float)UnityEngine.Random.Range(1, 4));
			...
			bool flag = roomInfo.roomAttrib.bPkAI == 2;
			flag = (!roomInfo.roomAttrib.bWarmBattle && flag);
			int num2 = (!flag) ? currentMapPlayerNum : (currentMapPlayerNum / 2);
			...
			if (transform4)
			{
				...
				if (component)
				{
					component.text = string.Format(Singleton<CTextManager>.GetInstance().GetText("Matching_Confirm_PlayerNum"), confirmPlayerNum, num2);
				}
			}
