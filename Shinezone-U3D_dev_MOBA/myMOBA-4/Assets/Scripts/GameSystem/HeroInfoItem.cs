...
namespace Assets.Scripts.GameSystem
{
	public class HeroInfoItem
	{
		...
		public void ValidateKDA()
		{
			...
			{
				this._killText.text = this._heroInfo.numKill.ToString();
			}
			if (this._deadText != null)
			{
				this._deadText.text = this._heroInfo.numDead.ToString();
			}
			if (this._assiText != null)
			{
				this._assiText.text = this._heroInfo.numAssist.ToString();
			}
		}
		...
		public void ValidateMoney()
		{
			...
			{
				this._moneyText.text = string.Format("{0:N1}k", (float)this._heroInfo.TotalCoin * 0.001f);
			}
			else
			{
		...
		public void ValidateEquip()
		{
			...
			for (int i = 0; i < 6; i++)
			{
				...
				if (!flag)
				{
					image.SetSprite(string.Format((this.listCamp != COM_PLAYERCAMP.COM_PLAYERCAMP_1) ? "{0}EquipmentSpaceRed" : "{0}EquipmentSpace", CUIUtility.s_Sprite_Dynamic_Talent_Dir), Singleton<CBattleSystem>.GetInstance().WatchFormScript, true, false, false, false);
				}
			}
		}
		...
		public void ValidateReviceCD()
		{
			if (this._heroInfo != null && this._heroInfo.actorHero && this._heroInfo.actorHero.handle.ActorControl.IsDeadState)
			{
				...
				{
					this._reviveTxt.text = string.Format("{0}", Mathf.FloorToInt((float)this._heroInfo.actorHero.handle.ActorControl.ReviveCooldown * 0.001f));
				}
		...
		public void ValidateCD(SkillSlotType slot, PoolObjHandle<ActorRoot> actor)
		{
			if (this._SkillGroupGo != null && this._heroInfo != null && this._heroInfo.actorHero)
			{
				...
				if (num >= 0 && num < this._skillCDs.Length)
				{
					...
					{
						this._skillCDs[num].fillAmount = (int)skillSlot.CurSkillCD / (float)skillSlot.GetSkillCDMax();
					}
				}
			}
		}
