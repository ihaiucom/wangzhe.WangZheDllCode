...
namespace Assets.Scripts.GameSystem
{
	...
	public class CHeroInfoSystem2 : Singleton<CHeroInfoSystem2>
	{
		...
		private void OnShow2DImage(CUIEvent uiEvent)
		{
			...
			CBinaryObject x = Singleton<CResourceManager>.GetInstance().GetResource("UGUI/Sprite/Dynamic/BustHeroLarge/" + str + ".bytes", typeof(TextAsset), enResourceType.UISprite, false, false).m_content as CBinaryObject;
			if (x == null)
			{
		...
		private void RefreshBaseInfoPanel()
		{
			if (this.m_heroInfoForm != null)
			{
				...
				component.text = CHeroInfo.GetHeroJob(this.m_selectHeroID);
				Text component2 = transform.Find("jobFeatureTxt").GetComponent<Text>();
				component2.text = CHeroInfo.GetJobFeature(this.m_selectHeroID);
				...
				if (flag)
				{
					...
					{
						component3.text = heroProficiency.szTitle;
						int maxProficiency = CHeroInfo.GetMaxProficiency();
						if ((int)this.m_selectHeroData.proficiencyLV >= maxProficiency)
						{
							component4.text = this.m_selectHeroData.proficiency.ToString();
						}
						else
						{
							component4.text = string.Format("{0}/{1}", this.m_selectHeroData.proficiency, heroProficiency.dwTopPoint);
						}
				...
				component5.text = Singleton<CTextManager>.GetInstance().GetText("Hero_Viability");
				...
				component6.text = Singleton<CTextManager>.GetInstance().GetText("Hero_AtkDamage");
				...
				component7.text = Singleton<CTextManager>.GetInstance().GetText("Hero_SkillFunc");
				...
				component8.text = Singleton<CTextManager>.GetInstance().GetText("Hero_StartedDifficulty");
		...
		private void RefreshSkillPanel()
		{
			if (this.m_heroInfoForm != null)
			{
				...
				for (int i = 0; i < 4; i++)
				{
					...
					eventParams.skillTipParam.skillEnergyCost = ((i != 0 && skillCfgInfo.bEnergyCostType != 6) ? Singleton<CTextManager>.instance.GetText(EnergyCommon.GetEnergyShowText((uint)skillCfgInfo.bEnergyCostType, EnergyShowType.CostValue), new string[]
					{
					...
					if (skinFeatureCnt)
					{
						...
						if (heroSkin != null)
						{
							for (int i = 0; i < num; i++)
							{
								...
								transform3.GetComponent<Text>().text = heroSkin.astFeature[i].szDesc;
							}
						}
					}
					else
		...
		protected void SetSkinListItem(GameObject listItem, ResHeroSkin skinInfo, bool bSelect)
		{
			...
			component.text = StringHelper.UTF8BytesToString(ref skinInfo.szSkinName);
			Text component2 = transform.Find("skinInfoPanel/skinNamePanel/heroNameText").GetComponent<Text>();
			component2.text = skinInfo.szHeroName;
			...
			component5.text = Singleton<CTextManager>.GetInstance().GetText("HeroInfo_BuyAndWear");
		...
		public static string GetSkinCannotBuyStr(ResHeroSkin skinInfo)
		{
			...
			{
				return (!string.IsNullOrEmpty(resHeroSkinShop.szGetPath)) ? resHeroSkinShop.szGetPath : Singleton<CTextManager>.GetInstance().GetText("Hero_SkinState_CannotBuy");
			}
			return null;
		}
		...
		public void OnHeroSkin_ItemSelect(CUIEvent uiEvent)
		{
			...
			component2.text = CHeroInfo.GetHeroTitle(this.m_selectHeroID, skinIdByIndex);
		...
		public static void RefreshBasePropPanel(GameObject root, ref ValueDataInfo[] info, int level, uint heroId)
		{
			...
			component2.text = CHeroInfoSystem2.GetFormStr((float)info[5].basePropertyValue, (float)info[5].extraPropertyValue);
			component3.text = Singleton<CTextManager>.GetInstance().GetText("Hero_Prop_PhyAtkPt");
			component4.text = CHeroInfoSystem2.GetFormStr((float)info[1].basePropertyValue, (float)info[1].extraPropertyValue);
			component5.text = Singleton<CTextManager>.GetInstance().GetText("Hero_Prop_MgcAtkPt");
			component6.text = CHeroInfoSystem2.GetFormStr((float)info[2].basePropertyValue, (float)info[2].extraPropertyValue);
			...
			component7.text = Singleton<CTextManager>.GetInstance().GetText("Hero_Prop_PhyDefPt");
			component8.text = string.Format("{0}|{1}", CHeroInfoSystem2.GetFormStr((float)info[3].basePropertyValue, (float)info[3].extraPropertyValue), CHeroInfoSystem2.GetFormPercentStr(percent, info[3].extraPropertyValue > 0));
			...
			component9.text = Singleton<CTextManager>.GetInstance().GetText("Hero_Prop_MgcDefPt");
			component10.text = string.Format("{0}|{1}", CHeroInfoSystem2.GetFormStr((float)info[4].basePropertyValue, (float)info[4].extraPropertyValue), CHeroInfoSystem2.GetFormPercentStr(percent, info[4].extraPropertyValue > 0));
			ResHeroCfgInfo dataByKey = GameDataMgr.heroDatabin.GetDataByKey(heroId);
			uint energyType = (dataByKey != null) ? dataByKey.dwEnergyType : 0u;
			component15.text = Singleton<CTextManager>.GetInstance().GetText(EnergyCommon.GetEnergyShowText(energyType, EnergyShowType.MaxValue));
			component16.text = CHeroInfoSystem2.GetFormStr((float)info[32].basePropertyValue, (float)info[32].extraPropertyValue);
		}
		...
		public static void RefreshAtkPropPanel(GameObject root, ref ValueDataInfo[] info, int level, uint heroId)
		{
			...
			component2.text = CHeroInfoSystem2.GetFormStr((float)(info[15].basePropertyValue / 10), (float)(info[15].extraPropertyValue / 10));
			MonoSingleton<GlobalConfig>.instance.bOnExternalSpeedPicker = false;
			component3.text = Singleton<CTextManager>.GetInstance().GetText("Hero_Prop_PhyArmorHurt");
			component4.text = string.Format("{0}|{1}", CHeroInfoSystem2.GetFormStr((float)info[7].baseValue, (float)info[7].extraPropertyValue), CHeroInfoSystem2.GetFormPercentStr(info[34].totalValue, info[34].extraPropertyValue > 0));
			component5.text = Singleton<CTextManager>.GetInstance().GetText("Hero_Prop_MgcArmorHurt");
			component6.text = string.Format("{0}|{1}", CHeroInfoSystem2.GetFormStr((float)info[8].baseValue, (float)info[8].extraPropertyValue), CHeroInfoSystem2.GetFormPercentStr(info[35].totalValue, info[35].extraPropertyValue > 0));
			...
			component7.text = Singleton<CTextManager>.GetInstance().GetText("Hero_Prop_AtkSpdLvl");
			component8.text = CHeroInfoSystem2.GetFormPercentStr(percent, info[18].extraPropertyValue > 0);
			...
			component9.text = Singleton<CTextManager>.GetInstance().GetText("Hero_Prop_CritLvl");
			component10.text = CHeroInfoSystem2.GetFormPercentStr(percent, info[6].extraPropertyValue > 0);
			totalValue = info[12].totalValue;
			percent = totalValue + 10000;
			component11.text = Singleton<CTextManager>.GetInstance().GetText("Hero_Prop_CritEft");
			component12.text = CHeroInfoSystem2.GetFormPercentStr(percent, info[12].extraPropertyValue > 0);
			...
			component13.text = Singleton<CTextManager>.GetInstance().GetText("Hero_Prop_PhyVampLvl");
			component14.text = CHeroInfoSystem2.GetFormPercentStr(percent, info[9].extraPropertyValue > 0);
			...
			component15.text = Singleton<CTextManager>.GetInstance().GetText("Hero_Prop_MgcVampLvl");
			component16.text = CHeroInfoSystem2.GetFormPercentStr(percent, info[10].extraPropertyValue > 0);
			component17.text = Singleton<CTextManager>.GetInstance().GetText("Hero_Prop_CdReduce");
			percent = info[20].totalValue;
			component18.text = CHeroInfoSystem2.GetFormPercentStr(percent, info[20].extraPropertyValue > 0);
			ResHeroCfgInfo dataByKey = GameDataMgr.heroDatabin.GetDataByKey(heroId);
			if (dataByKey != null)
			{
				component19.text = Singleton<CTextManager>.GetInstance().GetText("Hero_Prop_BaseAtkRange");
				component20.text = Utility.UTF8Convert(dataByKey.szAttackRangeDesc);
			}
			else
			{
		...
		public static void RefreshDefPropPanel(GameObject root, ref ValueDataInfo[] info, int level, uint heroId)
		{
			...
			component2.text = CHeroInfoSystem2.GetFormPercentStr(percent, info[17].extraPropertyValue > 0);
			component3.text = Singleton<CTextManager>.GetInstance().GetText("Hero_Prop_HpRecover");
			...
			component4.text = CHeroInfoSystem2.GetFormStr((float)info[16].basePropertyValue, (float)info[16].extraPropertyValue);
			ResHeroCfgInfo dataByKey = GameDataMgr.heroDatabin.GetDataByKey(heroId);
			uint energyType = (dataByKey != null) ? dataByKey.dwEnergyType : 0u;
			component5.text = Singleton<CTextManager>.GetInstance().GetText(EnergyCommon.GetEnergyShowText(energyType, EnergyShowType.RecoverValue));
			...
			component6.text = CHeroInfoSystem2.GetFormStr((float)info[33].basePropertyValue, (float)info[33].extraPropertyValue);
		}
		...
		private void RefreshBuyHeroPanel()
		{
			...
			if (stPayInfoSet.m_payInfoCount <= 0)
			{
				...
				if (resHeroShop != null)
				{
					component.text = StringHelper.UTF8BytesToString(ref resHeroShop.szObtWay);
				}
				else
				{
					component.text = null;
