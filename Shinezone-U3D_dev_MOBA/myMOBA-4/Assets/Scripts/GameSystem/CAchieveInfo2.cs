	...
	public class CAchieveInfo2
	{
		...
		private void InitLocalData()
		{
			...
			while (enumerator.MoveNext())
			{
				...
				ResAchievement resAchievement = current.Value as ResAchievement;
		...
		public static void AddWorldRank(int logicWorldID, ulong playerUUID, uint rank)
		{
			...
			if (CAchieveInfo2.worldRanks.ContainsKey(key))
			{
				CAchieveInfo2.worldRanks[key] = rank;
			}
			else
			{
				CAchieveInfo2.worldRanks.Add(key, rank);
			}
		}
		...
		public uint GetWorldRank()
		{
			...
			if (CAchieveInfo2.worldRanks.ContainsKey(key))
			{
				CAchieveInfo2.worldRanks.TryGetValue(key, out result);
			}
			return result;
		}
		...
		public void OnServerAchieveInfo(ref COMDT_ACHIEVEMENT_INFO svrAchieveInfo)
		{
			...
			while ((long)num < (long)((ulong)svrAchieveInfo.dwDoneTypeNum))
			{
				...
				array[(int)((UIntPtr)dwDoneType)] = svrAchieveInfo.astDoneData[num].iDoneCnt;
				num++;
			}
			num = 0;
		...
		private void InitLocalTrophy()
		{
			...
			{
				KeyValuePair<long, object> current = enumerator.Current;
				ResTrophyLvl resTrophyLvl = current.Value as ResTrophyLvl;
				if (resTrophyLvl != null && (ulong)num < (ulong)((long)this.TrophyRewardInfoArr.Length))
				{
					this.TrophyRewardInfoArr[(int)((UIntPtr)num)] = new CTrophyRewardInfo(resTrophyLvl, TrophyState.UnFinish, (int)num, minPoint);
					minPoint = this.TrophyRewardInfoArr[(int)((UIntPtr)num)].MaxPoint;
				}
				if (!this.TrophyRewardDic.ContainsKey(this.TrophyRewardInfoArr[(int)((UIntPtr)num)].Cfg.dwTrophyLvl))
				{
					this.TrophyRewardDic.Add(this.TrophyRewardInfoArr[(int)((UIntPtr)num)].Cfg.dwTrophyLvl, this.TrophyRewardInfoArr[(int)((UIntPtr)num)]);
				}
				num += 1u;
			}
		...
		private void OnServerTrophy(ref COMDT_TROPHY_INFO TrophyRewardsInfo)
		{
			...
			while ((long)num < (long)((ulong)this.TrophyRewardInfoArrCnt))
			{
				...
				{
					this.TrophyRewardInfoArr[num].State = ((TrophyRewardsInfo.szTrophyStateArray[num2] != 1) ? TrophyState.GotRewards : TrophyState.Finished);
					num2++;
				}
