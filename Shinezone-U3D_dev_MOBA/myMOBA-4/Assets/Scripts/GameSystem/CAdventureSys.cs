...
namespace Assets.Scripts.GameSystem
{
	...
	internal class CAdventureSys : Singleton<CAdventureSys>
	{
		...
		{
			Button component = uiEvent.m_srcWidget.GetComponent<Button>();
			if (component.interactable && this.CheckChapterRewardCondition())
			{
		...
		private void OnBuyPlayTime(CUIEvent uiEvent)
		{
			...
			if (num2 == 0)
			{
				string text = (b != 2) ? Singleton<CTextManager>.GetInstance().GetText("Money_Type_GoldCoin") : Singleton<CTextManager>.GetInstance().GetText("Money_Type_DianQuan");
		...
		public static int GetLevelId(int ChapterId, int Seq, int Difficulty)
		{
			...
			return (resLevelCfgInfo == null) ? 0 : resLevelCfgInfo.iCfgID;
		}
		...
		private int CanBuyPlayTime(PVE_LEVEL_COMPLETE_INFO levelInfo, out byte CoinType, out uint Price, out string reason)
		{
			...
			if ((long)num <= (long)((ulong)CAdventureSys.CHALLENGE_BUYTIME_LIMIT))
			{
				...
				while (enumerator.MoveNext())
				{
					...
					ResShopInfo resShopInfo = (ResShopInfo)current.Value;
		...
		private bool CheckFullItem(out string itemName)
		{
			...
			for (int i = 0; i < 5; i++)
			{
				if (dataByKey.astRewardShowDetail[i].dwRewardID != 0u)
				{
					...
					{
						int useableStackCount = useableContainer.GetUseableStackCount(COM_ITEM_TYPE.COM_OBJTYPE_ITEMEQUIP, dataByKey.astRewardShowDetail[i].dwRewardID);
						...
						if (useableStackCount == dataByKey3.iOverLimit)
						{
							return false;
						}
					}
