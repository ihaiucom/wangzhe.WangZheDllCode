		...
		public static void SetObservers(GameObject root, int maxNum, ListView<MemberInfo> memberList, MemberInfo masterMember)
		{
			if (maxNum > 0)
			{
				...
				for (int i = 0; i < memberList.Count; i++)
				{
					...
					{
						array[(int)((UIntPtr)memberInfo.dwPosOfCamp)] = memberInfo;
					}
				}
				int num = 0;
				for (int j = 0; j < maxNum; j++)
				{
					...
					if (flag)
					{
						componetInChild3.text = memberInfo2.MemberName;
				...
				Utility.GetComponetInChild<Text>(root, "PersonCount").text = num + "/" + maxNum;
			}
			else
			{
