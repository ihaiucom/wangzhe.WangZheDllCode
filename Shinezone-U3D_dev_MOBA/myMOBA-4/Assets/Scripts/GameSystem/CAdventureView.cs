...
namespace Assets.Scripts.GameSystem
{
	internal class CAdventureView
	{
		...
		public static void InitChapterElement(CUIFormScript formScript, int currentChapter, int levelNo, int difficulty)
		{
			...
			formScript.transform.FindChild("ChapterElement/RecPlayerLvlText").GetComponent<Text>().text = Singleton<CTextManager>.instance.GetText("Level_Recommend_Tips_1", new string[]
			{
		...
		public static void InitLevelForm(CUIFormScript formScript, int chapterNo, int LevelNo, int difficulty)
		{
			...
			if (dataByKey != null)
			{
				...
				formScript.transform.Find("PanelLeft/RecPlayerLvlText").GetComponent<Text>().text = Singleton<CTextManager>.instance.GetText("Level_Recommend_Tips_1", new string[]
				{
					dataByKey.RecommendLevel[difficulty - 1].ToString()
				});
		...
		private static void SetStarConditionDesc(CUIFormScript formScript, GameObject descCon, uint conditionId, bool bAchieved)
		{
			...
			component2.SetSprite("UGUI/Sprite/System/" + ((!bAchieved) ? "Adventure/empty_big_star" : "Adventure/big_star"), formScript, true, false, false, false);
		}
		...
		private static void SetTeamHeroList(GameObject list, List<uint> heroIds, out int teamPower)
		{
			...
			for (int i = 0; i < heroIds.Count; i++)
			{
				...
				if (heroIds[i] > 0u)
				{
					IHeroData heroData = CHeroDataFactory.CreateHeroData(heroIds[i]);
		...
		private static void SetStartBtnEnable(GameObject Start, List<uint> heros)
		{
			...
			bool flag = heros != null && heros.Count > 0;
