	...
	public class ExchangeWgt : ActivityWidget
	{
		private class ExchangeElement
		{
			...
			public void Validate()
			{
				...
				int arg_CD_0 = (resDT_Item_Info != null) ? useableContainer.GetUseableStackCount((COM_ITEM_TYPE)resDT_Item_Info.wItemType, resDT_Item_Info.dwItemID) : 0;
				int arg_EE_0 = (resDT_Item_Info2 != null) ? useableContainer.GetUseableStackCount((COM_ITEM_TYPE)resDT_Item_Info2.wItemType, resDT_Item_Info2.dwItemID) : 0;
		...
		private void OnExchangeHeroSkinConfirm(CUIEvent uiEvent)
		{
			...
			if (commonUInt32Param >= 0 && commonUInt32Param < this._elementList.Count)
			{
				...
				{
					string arg_BE_0 = (cUseable != null) ? cUseable.m_name : string.Empty;
		...
		private void OnClickExchange(CUIEvent uiEvent)
		{
			...
			if (commonUInt32Param >= 0 && commonUInt32Param < this._elementList.Count)
			{
				...
				else
				{
					...
					par2.commonUInt16Param1 = (ushort)commonUInt32Param;
					this._elementList[commonUInt32Param].phase.SetExchangeCountOnce(1);
					Singleton<CUIManager>.GetInstance().OpenMessageBoxWithCancel(string.Format(Singleton<CTextManager>.GetInstance().GetText("confirmExchange"), maxExchangeCount, arg), enUIEventID.Activity_ExchangeConfirm, enUIEventID.None, par2, false);
				}
			}
		}
		...
		private void OnClickExchangeCountSelectReady(CUIEvent uiEvent)
		{
			if (this._elementList != null)
			{
				...
				if (commonUInt16Param >= 0 && commonUInt16Param < this._elementList.Count)
				{
					...
					Singleton<CUIManager>.GetInstance().OpenMessageBoxWithCancel(string.Format(Singleton<CTextManager>.GetInstance().GetText("confirmExchange"), commonUInt32Param, arg), enUIEventID.Activity_ExchangeConfirm, enUIEventID.None, par, false);
				}
			}
		}
	}
