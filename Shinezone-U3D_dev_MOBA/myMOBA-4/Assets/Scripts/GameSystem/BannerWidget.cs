...
namespace Assets.Scripts.GameSystem
{
	public class BannerWidget : ActivityWidget
	{
		...
		private void OnHoldStart_Item(CUIEvent uiEvent)
		{
			...
			this._lastPos = uiEvent.m_pointerEventData.position;
		...
		private void OnHoldEnd_Item(CUIEvent uiEvent)
		{
			...
			int count = this._urlaList.Count;
			if (uiEvent.m_pointerEventData.position.x <= this._lastPos.x)
			{
		...
		private void updateOverTime()
		{
			...
			if (urlAction.overTime > 0uL)
			{
				...
				text = text.Replace("{0}", timeSpan.Days.ToString());
				text = text.Replace("{1}", timeSpan.Hours.ToString());
				text = text.Replace("{2}", timeSpan.Minutes.ToString());
				text = text.Replace("{3}", timeSpan.Seconds.ToString());
