	...
	public class CSecurePwdSystem : Singleton<CSecurePwdSystem>
	{
		...
		public uint CloseTime
		{
			...
			set
			{
				...
				if (this.m_CloseTime > 0u)
				{
					...
					TimeSpan timeSpan = Utility.ToUtcTime2Local((long)((ulong)this.m_CloseTime)) - d;
					if (timeSpan.TotalSeconds > 0.0)
					{
						long num = (long)(timeSpan.TotalSeconds * 1000.0);
		...
		private void OpenCloseForm(CUIEvent uiEvent)
		{
			...
			if (this.CloseStatus == PwdCloseStatus.Open)
			{
				DateTime d = Utility.ToUtcTime2Local((long)CRoleInfo.GetCurrentUTCTime());
				TimeSpan timeSpan = Utility.ToUtcTime2Local((long)((ulong)this.m_CloseTime)) - d;
				if (timeSpan.TotalSeconds < 0.0 || timeSpan.TotalSeconds < 2.0)
				{
					return;
				}
			}
		...
		private void OpenApplyClosePwdForm(CUIEvent uiEvent)
		{
			...
			else
			{
				...
				if (timeSpan.TotalSeconds > 0.0)
				{
					component3.SetTotalTime((float)timeSpan.TotalSeconds);
