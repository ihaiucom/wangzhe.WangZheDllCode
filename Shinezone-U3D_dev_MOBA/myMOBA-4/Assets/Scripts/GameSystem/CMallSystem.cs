	...
	public class CMallSystem : Singleton<CMallSystem>
	{
		...
		private void SetTagImg(int idx, GameObject imgObj)
		{
			...
			if (currentUTCTime >= tagInfo.dwStartTime && currentUTCTime <= tagInfo.dwEndTime)
			{
				...
				{
					imgObj.GetComponentInChildren<Text>().text = Utility.UTF8Convert(tagInfo.szContent);
				}
			}
			else
			{
		...
		private void OnMallTabChange(CUIEvent uiEvent)
		{
			...
			if (num < 0 || num >= this.m_AvailableTabList.Count)
			{
				num = 0;
			}
			this.CurTab = this.m_AvailableTabList[num];
		...
		public void RefreshTabRedDot(CMallSystem.Tab tab)
		{
			...
			CUIListScript cUIListScript = (!(widget != null)) ? null : widget.GetComponent<CUIListScript>();
			if (cUIListScript != null)
			{
					...
					if (!(transform2 == null))
					{
						...
						if (component2 != null)
						{
							...
							if (curTab != CMallSystem.Tab.Hero)
							{
								if (curTab == CMallSystem.Tab.Skin)
								{
									component2.text = CMallSortHelper.CreateSkinSorter().GetSortTypeName((CMallSortHelper.SkinSortType)i);
								}
							}
							else
							{
								component2.text = CMallSortHelper.CreateHeroSorter().GetSortTypeName((CMallSortHelper.HeroSortType)i);
							}
						}
					}
				}
		...
		private void UpdateSortContainer()
		{
			...
			if (curTab != CMallSystem.Tab.Hero)
			{
				if (curTab == CMallSystem.Tab.Skin)
				{
					...
					if (component != null)
					{
						...
						component.text = mallSort.GetSortTypeName(mallSort.GetCurSortType());
					}
				}
			}
			else
			{
				...
				if (component2 != null)
				{
					...
					component2.text = mallSort2.GetSortTypeName(mallSort2.GetCurSortType());
				}
			}
		}
		...
		private void InitTab()
		{
			...
			string[] array = new string[this.m_AvailableTabList.Count];
			byte b = 0;
			while ((int)b < array.Length)
			{
				switch (this.m_AvailableTabList[(int)b])
				{
				case CMallSystem.Tab.Mystery:
			...
			if (cUIListScript != null)
			{
				...
				for (int i = 0; i < cUIListScript.m_elementAmount; i++)
				{
					...
					if (elemenet != null)
					{
						...
						if (gameObject != null)
						{
							if (this.m_AvailableTabList[i] == CMallSystem.Tab.Mystery)
							{
							...
							if (this.m_AvailableTabList[i] == CMallSystem.Tab.Roulette)
							{
								...
								if (payInfo.m_payValue < payInfo.m_oriValue || payInfo2.m_payValue < payInfo2.m_oriValue || payInfo3.m_payValue < payInfo3.m_oriValue || payInfo4.m_payValue < payInfo4.m_oriValue)
								{
									...
									{
										componetInChild3.text = Singleton<CTextManager>.GetInstance().GetText("RES_WEAL_COLORBAR_TYPE_DISCOUNT");
										gameObject.CustomSetActive(true);
									}
								}
								if (Singleton<CMallRouletteController>.GetInstance().IsProbabilityDoubled(RES_SHOPBUY_COINTYPE.RES_SHOPBUY_TYPE_NULL))
								{
									...
									{
										componetInChild4.text = Singleton<CTextManager>.GetInstance().GetText("RES_WEAL_COLORBAR_TYPE_PROBABILITY_DOUBLED");
										gameObject.CustomSetActive(true);
									}
								}
							}
							if (this.m_AvailableTabList[i] == CMallSystem.Tab.Boutique)
							{
								this.SetTagImg(4, gameObject);
							}
							else if (this.m_AvailableTabList[i] == CMallSystem.Tab.Recommend)
							{
								this.SetTagImg(5, gameObject);
							}
							else if (this.m_AvailableTabList[i] == CMallSystem.Tab.Hero)
							{
								this.SetTagImg(6, gameObject);
							}
							else if (this.m_AvailableTabList[i] == CMallSystem.Tab.Skin)
							{
								this.SetTagImg(7, gameObject);
							}
							else if (this.m_AvailableTabList[i] == CMallSystem.Tab.Factory_Shop)
							{
								this.SetTagImg(8, gameObject);
							}
						}
						if (i >= 0 && i < this.m_AvailableTabList.Count)
						{
							this.RefreshTabRedDot(this.m_AvailableTabList[i]);
						}
					}
				}
				...
				else
				{
					...
					case CMallItem.ItemType.Hero:
						if (button != null)
						{
							...
							eventParams3.heroId = item.HeroID();
							...
							cUIEventScript.SetUIEvent(enUIEventType.Click, enUIEventID.HeroView_OpenBuyHeroForFriend, eventParams3);
						}
						break;
					case CMallItem.ItemType.Skin:
						if (button != null)
						{
							...
							eventParams4.heroSkinParam.heroId = item.HeroID();
							eventParams4.heroSkinParam.skinId = item.SkinID();
							eventParams4.heroSkinParam.isCanCharge = true;
							eventParams4.commonUInt64Param1 = 0uL;
							eventParams4.commonBool = false;
							eventParams4.commonUInt32Param1 = 0u;
							button.enabled = true;
							cUIEventScript.enabled = true;
							cUIEventScript.SetUIEvent(enUIEventType.Click, enUIEventID.HeroSkin_OpenBuyHeroSkinForFriend, eventParams4);
						}
						break;
					}
				}
				...
				if (position == CMallItemWidget.NamePosition.Bottom)
				{
					...
					if (itemWidget.m_bottomNameRightText != null)
					{
						...
						else
						{
							...
							component2.text = item.SecondName();
						}
					}
				}
			}
			else
			{
				...
				if (itemWidget.m_topNameRightText != null)
				{
					...
					else
					{
						...
						component4.text = item.SecondName();
					}
				}
			}
		}
		...
		public void SetItemPriceInfo(CMallItemWidget itemWidget, CMallItem item, ref stPayInfoSet payInfoSet)
		{
			...
			for (int i = 0; i < payInfoSet.m_payInfoCount; i++)
			{
				...
				case CMallItem.OldPriceType.None:
				{
					...
					component2.text = payInfoSet.m_payInfos[i].m_payValue.ToString();
				...
				case CMallItem.OldPriceType.FirstOne:
					...
					if (i == 0)
					{
						...
						component4.text = payInfoSet.m_payInfos[i].m_oriValue.ToString();
						Text component5 = widget3.GetComponent<Text>();
						component5.text = payInfoSet.m_payInfos[i].m_payValue.ToString();
					...
					else
					{
						...
						component7.text = payInfoSet.m_payInfos[i].m_payValue.ToString();
				...
				case CMallItem.OldPriceType.SecondOne:
					...
					if (i == 1)
					{
						...
						component9.text = payInfoSet.m_payInfos[i].m_oriValue.ToString();
						Text component10 = widget3.GetComponent<Text>();
						component10.text = payInfoSet.m_payInfos[i].m_payValue.ToString();
					...
					else
					{
						...
						component12.text = payInfoSet.m_payInfos[i].m_payValue.ToString();
				...
				case CMallItem.OldPriceType.Both:
				{
					...
					component14.text = payInfoSet.m_payInfos[i].m_oriValue.ToString();
					Text component15 = widget3.GetComponent<Text>();
					component15.text = payInfoSet.m_payInfos[i].m_payValue.ToString();
		...
		private static stPayInfoSet CalLowestPayInfoSet(params stPayInfoSet[] values)
		{
			...
			for (int i = 0; i < values.Length; i++)
			{
				...
				for (int j = 0; j < stPayInfoSet.m_payInfoCount; j++)
				{
					if (stPayInfoSet.m_payInfos[j].m_payValue != 0u)
					{
						...
						if (dictionary.TryGetValue(stPayInfoSet.m_payInfos[j].m_payType, out stPayInfo))
						{
							if (stPayInfoSet.m_payInfos[j].m_payValue < stPayInfo.m_payValue)
							{
								dictionary[stPayInfo.m_payType] = stPayInfoSet.m_payInfos[j];
							}
						}
						else
						{
			...
			stPayInfoSet result = new stPayInfoSet(dictionary.Count);
			result.m_payInfoCount = dictionary.Count;
			dictionary.Values.CopyTo(result.m_payInfos, 0);
			return result;
		}
		...
		public static bool IsinRegisterSales(ResDT_RegisterSale_Info stRegisterSale)
		{
			...
			DateTime d = Utility.ToUtcTime2Local((long)((ulong)stRegisterSale.dwStartTimeGen));
			if ((Singleton<CMallSystem>.GetInstance().m_PlayerRegisterTime - d).TotalMilliseconds < 0.0)
			{
				return false;
			}
			DateTime t = Singleton<CMallSystem>.GetInstance().m_PlayerRegisterTime.AddSeconds(stRegisterSale.dwValidTime);
			DateTime t2 = Utility.ToUtcTime2Local((long)CRoleInfo.GetCurrentUTCTime());
			return t2 <= t;
		}
		...
		public static string GetRegisterSalesHeroDay(ref ResHeroPromotion heroPromotion, ResHeroShop heroShop)
		{
			...
			{
				uint num = (heroShop == null) ? 1u : heroShop.dwBuyCoin;
				uint num2 = (heroShop == null) ? 1u : heroShop.dwBuyCoupons;
				uint num3 = (heroShop == null) ? 1u : heroShop.dwBuyDiamond;
			...
			if (flag2)
			{
				DateTime d = Singleton<CMallSystem>.GetInstance().m_PlayerRegisterTime.AddSeconds(stRegisterSale.dwValidTime);
				DateTime d2 = Utility.ToUtcTime2Local((long)CRoleInfo.GetCurrentUTCTime());
				int num4 = (int)Math.Ceiling((d - d2).TotalSeconds / 86400.0);
				if (num4 > 0)
				{
		...
		public static string GetRegisterSalesSkinDay(ref ResSkinPromotion heroPromotion, ResHeroSkinShop heroShop)
		{
			...
			{
				uint num = (heroShop == null) ? 1u : heroShop.dwBuyCoupons;
				uint num2 = (heroShop == null) ? 1u : heroShop.dwBuyDiamond;
				if (flag && heroShop != null)
				{
			...
			if (flag2)
			{
				DateTime d = Singleton<CMallSystem>.GetInstance().m_PlayerRegisterTime.AddSeconds(stRegisterSale.dwValidTime);
				DateTime d2 = Utility.ToUtcTime2Local((long)CRoleInfo.GetCurrentUTCTime());
				int num3 = (int)Math.Ceiling((d - d2).TotalSeconds / 86400.0);
				if (num3 > 0)
				{
					result = Singleton<CTextManager>.GetInstance().GetText("Mall_Promotion_Tag_1", new string[]
					{
		...
		public static stPayInfoSet GetPayInfoSetOfGood(ResHeroCfgInfo resHeroCfgInfo, ResHeroPromotion resPromotion)
		{
			...
			if (resPromotion != null)
			{
				...
				return CMallSystem.GetPayInfoSetOfGood(resPromotion.bIsBuyCoin > 0, dwBuyCoin, (resHeroShop == null) ? 1u : resHeroShop.dwBuyCoin, resPromotion.bIsBuyCoupons > 0, dwBuyCoupons, (resHeroShop == null) ? 1u : resHeroShop.dwBuyCoupons, resPromotion.bIsBuyDiamond > 0, dwBuyDiamond, (resHeroShop == null) ? 1u : resHeroShop.dwBuyDiamond, 10000u);
			}
			uint num = (resHeroShop == null) ? 1u : resHeroShop.dwBuyCoin;
			uint num2 = (resHeroShop == null) ? 1u : resHeroShop.dwBuyCoupons;
			uint num3 = (resHeroShop == null) ? 1u : resHeroShop.dwBuyDiamond;
			...
			return CMallSystem.GetPayInfoSetOfGood(resHeroShop != null && resHeroShop.bIsBuyCoin > 0, num, (resHeroShop == null) ? 1u : resHeroShop.dwBuyCoin, resHeroShop != null && resHeroShop.bIsBuyCoupons > 0, num2, (resHeroShop == null) ? 1u : resHeroShop.dwBuyCoupons, resHeroShop != null && resHeroShop.bIsBuyDiamond > 0, num3, (resHeroShop == null) ? 1u : resHeroShop.dwBuyDiamond, 10000u);
		}
		...
		public static stPayInfoSet GetPayInfoSetOfGood(ResHeroSkin resHeroSkin, ResSkinPromotion resPromotion)
		{
			...
			if (resPromotion != null)
			{
				...
				return CMallSystem.GetPayInfoSetOfGood(false, 0u, 0u, resPromotion.bIsBuyCoupons > 0, dwBuyCoupons, (resHeroSkinShop == null) ? 1u : resHeroSkinShop.dwBuyCoupons, resPromotion.bIsBuyDiamond > 0, dwBuyDiamond, (resHeroSkinShop == null) ? 1u : resHeroSkinShop.dwBuyDiamond, 10000u);
			}
			uint num = (resHeroSkinShop == null) ? 1u : resHeroSkinShop.dwBuyCoupons;
			uint num2 = (resHeroSkinShop == null) ? 1u : resHeroSkinShop.dwBuyDiamond;
			...
			return CMallSystem.GetPayInfoSetOfGood(false, 0u, 0u, resHeroSkinShop != null && resHeroSkinShop.bIsBuyCoupons > 0, num, (resHeroSkinShop == null) ? 1u : resHeroSkinShop.dwBuyCoupons, resHeroSkinShop != null && resHeroSkinShop.bIsBuyDiamond > 0, num2, (resHeroSkinShop == null) ? 1u : resHeroSkinShop.dwBuyDiamond, 10000u);
		}
		...
		public static void SetPayButton(CUIFormScript formScript, RectTransform buttonTransform, enPayType payType, uint payValue, enUIEventID eventID, ref stUIEventParams eventParams)
		{
			...
			if (transform2 != null)
			{
				...
				if (component2 != null)
				{
					component2.text = ((payValue <= 0u) ? "免费" : payValue.ToString());
				}
			}
		...
		public static void SetPayButton(CUIFormScript formScript, RectTransform buttonTransform, enPayType payType, uint payValue, uint oriValue, enUIEventID eventID, ref stUIEventParams eventParams)
		{
			...
			else if (transform3 != null)
			{
				...
				if (component3 != null)
				{
					component3.text = ((oriValue <= 0u) ? "免费" : oriValue.ToString());
				}
			}
		...
		public static void TryToPay(enPayPurpose payPurpose, string goodName, enPayType payType, uint payValue, enUIEventID confirmEventID, ref stUIEventParams confirmEventParams, enUIEventID cancelEventID, bool needConfirm, bool guideToAchieveDianQuan, bool noQuestionMark = false)
		{
			...
			{
				string strContent2 = string.Format(Singleton<CTextManager>.GetInstance().GetText("CurrencyNotEnoughWithJumpToAchieve"), Singleton<CTextManager>.GetInstance().GetText((payType != enPayType.DiamondAndDianQuan) ? CMallSystem.s_payTypeNameKeys[2] : "Currency_DiamondAndDianQuan"));
			...
			{
				string strContent3 = string.Format(Singleton<CTextManager>.GetInstance().GetText("CurrencyNotEnough"), Singleton<CTextManager>.GetInstance().GetText((payType != enPayType.DiamondAndDianQuan) ? CMallSystem.s_payTypeNameKeys[(int)payType] : "Currency_DiamondAndDianQuan"));
		...
		public static void ReceiveRouletteDataNTF(CSPkg msg)
		{
			...
			if (!CMallSystem.luckyDrawDic.TryGetValue(enPayType.Diamond, out cSDT_LUCKYDRAW_INFO))
			{
			...
			if (!CMallSystem.luckyDrawDic.TryGetValue(enPayType.DianQuan, out cSDT_LUCKYDRAW_INFO2))
			{
