...
namespace Assets.Scripts.GameSystem
{
	...
	public class CPlayerPvpHistoryController : Singleton<CPlayerPvpHistoryController>
	{
		...
		{
			for (int i = 0; i < this.m_onCallList.Count; i++)
			{
				if (this.m_onCallList[i].type == call.type)
				{
		...
		private void OnHistoryItemEnable(CUIEvent uiEvt)
		{
			...
			string strContent = (!flag) ? Singleton<CTextManager>.instance.GetText("GameResult_Lose") : Singleton<CTextManager>.instance.GetText("GameResult_Win");
			...
			if (bGameType != COM_GAME_TYPE.COM_SINGLE_GAME_OF_COMBAT)
			{
				...
				{
					component.MatchTypeText.GetComponent<Text>().text = Singleton<CTextManager>.GetInstance().GetText("HistoryInfo_Tips1");
				}
			}
			...
			int days = (dateTime.Date - dateTime2.Date).Days;
			Text component3 = component.time.transform.FindChild("Today").GetComponent<Text>();
			string text = string.Empty;
			switch (days)
			{
			case 0:
				text = string.Format(Singleton<CTextManager>.GetInstance().GetText("HistoryInfo_Tips3"), dateTime2.Hour, dateTime2.Minute);
				break;
			case 1:
				text = string.Format(Singleton<CTextManager>.GetInstance().GetText("HistoryInfo_Tips4"), dateTime2.Hour, dateTime2.Minute);
				break;
			case 2:
				text = string.Format(Singleton<CTextManager>.GetInstance().GetText("HistoryInfo_Tips5"), dateTime2.Hour, dateTime2.Minute);
				break;
			default:
		...
		{
			this.initHistoryList();
			while (this.m_onCallList.Count != 0)
			{
				stOnCall stOnCall = this.m_onCallList[0];
		...
		public static bool IsPlayWithFriend(COM_PLAYERCAMP playerCmp)
		{
			...
			int count = allCampPlayers.Count;
		...
		{
			return ((ulong)achiveBits & (ulong)(1L << (int)(achiveBit & (COM_FIGHT_HISTORY_ACHIVE_BIT)31))) > 0uL;
		}
