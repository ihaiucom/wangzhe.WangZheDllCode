	...
	public class NobeSys : MonoSingleton<NobeSys>
	{
		...
		private void OnTabChange(CUIEvent uiEvent)
		{
			...
			if (this.m_Form != null)
			{
				...
				{
					if (curTab == NobeSys.Tab.Grow_Info)
					{
						if (widget2 != null && widget3 != null && !widget3.activeSelf)
						{
							widget2.CustomSetActive(false);
							widget3.CustomSetActive(true);
							component2.text = component.GetElemenet(selectedIndex).gameObject.transform.Find("Text").GetComponent<Text>().text;
							this.UpdateGrowInfo();
						}
					}
				}
		...
		public void ShowNobeTipsInDiamond()
		{
			...
			if (dwCurLevel >= num)
			{
				...
				if (transform6)
				{
					...
					transform6.GetComponent<Text>().text = text;
				}
			...
			else
			{
				...
				if (dataByKey != null)
				{
					...
					if (gameObject)
					{
						gameObject.GetComponent<Image>().fillAmount = 1f * (float)dwScore / dataByKey.dwConsumeCoupons;
					}
				}
		...
		private void UpdateNobeLevelChange()
		{
			if (this.m_Form != null)
			{
				...
				if (curTab != NobeSys.Tab.Base_Info)
				{
					if (curTab == NobeSys.Tab.Grow_Info)
					{
						if (widget != null && widget2 != null && widget2.activeSelf)
						{
							this.UpdateGrowInfo();
						}
					}
		...
		private void UpdateBaseInfo()
		{
			...
			if (dwCurLevel >= num)
			{
				...
				if (gameObject2)
				{
					gameObject2.GetComponent<Image>().fillAmount = 1f;
				}
			}
			else
			{
				...
				if (dataByKey != null)
				{
					...
					if (num3 > 0)
					{
						...
						if (gameObject2)
						{
							gameObject2.GetComponent<Image>().fillAmount = 1f * (float)dwScore / dataByKey.dwConsumeCoupons;
						}
					...
					if (dictionaryView.ContainsKey(num5))
					{
						for (int j = 0; j < dictionaryView[num5].Count; j++)
						{
							...
							for (int k = 0; k < listView3.Count; k++)
							{
								...
								string a = Utility.UTF8Convert(resNobeInfo2.szResIcon);
								string b = Utility.UTF8Convert(resNobeInfo.szResIcon);
								if (a == b)
								{
									flag = true;
									break;
			...
			for (uint num6 = 0u; num6 < 8u; num6 += 1u)
			{
				...
				if (!(elemenet == null))
				{
					if (dictionaryView2.ContainsKey(num6 + 1u))
					{
						...
						for (int l = 0; l < 4; l++)
						{
							...
							if (l < listView4.Count && gameObject3 != null)
							{
								...
								if (gameObject3)
								{
									...
									component5.text = Utility.UTF8Convert(resNobeInfo3.szResName);
								}
							}
			...
			for (int i = 1; i <= GameDataMgr.resNobeInfoDatabin.count; i++)
			{
				...
				if (dataByIndex != null && dataByIndex.bResType == 3 && (ulong)dataByIndex.dwNobeLevel == (ulong)((long)level))
				{
					...
					if (transform)
					{
						...
						transform5.gameObject.GetComponent<Text>().text = ((dataByIndex.dwJiaoBiaoNum <= 0u) ? null : dataByIndex.dwJiaoBiaoNum.ToString(CultureInfo.InvariantCulture));
							...
							if (!(transform == null))
							{
								if (transform)
								{
									...
									if (transform2 != null)
									{
										...
										if (component2 != null)
										{
											component2.text = cUseable.m_name;
										}
									}
									...
									if (transform3 != null)
									{
										...
										{
											component3.text = cUseable.m_stackCount.ToString();
										}
										else
										{
											component3.text = cUseable.m_stackCount / 10000 + "万";
										}
									}
				...
				if (dictionaryView.ContainsKey(j))
				{
					...
					for (int k = 0; k < 5; k++)
					{
						...
						if (k < listView2.Count && gameObject3 != null)
						{
							...
							component4.text = string.Format("<size=24><color=#f3CA4Dff>{0}</color></size><size=18><color=#7e88a2ff>积分</color></size>", resVIPCoupons.dwConsumeCoupons.ToString());
		...
		private void UpdateCHangeHeadFormData(int nHeadIDX)
		{
			...
			for (int i = 0; i < count; i++)
			{
				...
				string arg = Utility.UTF8Convert(resNobeInfo.szResIcon);
				string text = Utility.UTF8Convert(resNobeInfo.szResName);
				...
				string prefabPath = string.Format("{0}{1}.prefab", CUIUtility.s_Sprite_System_ShareUI_Dir, arg);
				...
				else
				{
					...
					gameObject5.GetComponent<Text>().text = text3;
				}
			}
		}
		...
		public void SetGameCenterVisible(GameObject obj, COM_PRIVILEGE_TYPE privilegeType, ApolloPlatform platform, bool platformForceShow = false, bool changeName = false, string privilegeName = "", string nonePrivilegeName = "")
		{
			...
			if (gameCenterVisible.isVisible && changeName)
			{
				...
				if (transform != null)
				{
					...
					if (component != null)
					{
						component.text = gameCenterVisible.name;
					}
				}
			}
		}
		...
		protected void ShowNobeLevelUpForm(SCPKG_GAME_VIP_NTF vipInfo)
		{
			...
			{
				cUIFormScript.m_formWidgets[0].GetComponent<Text>().text = dwCurLevel.ToString(CultureInfo.InvariantCulture);
				cUIFormScript.m_formWidgets[1].GetComponent<Text>().text = NobeSys._lastNobeLv.ToString(CultureInfo.InvariantCulture);
				cUIFormScript.m_formWidgets[2].GetComponent<Text>().text = dwCurLevel.ToString(CultureInfo.InvariantCulture);
		...
		private void ShowNobeLoseTips(SCPKG_GAME_VIP_NTF vipInfo)
		{
			...
			if (cUIFormScript != null)
			{
				...
				if (component2)
				{
					component2.text = string.Format("贵族{0}特权", dwMaxLevel);
				}
			}
		}
