...
namespace Assets.Scripts.GameSystem
{
	...
	internal class CMatchingSystem : Singleton<CMatchingSystem>
	{
		...
		public static void ReqStartSingleMatching(uint MapId, bool bPkAI, COM_BATTLE_MAP_TYPE mapType = COM_BATTLE_MAP_TYPE.COM_BATTLE_MAP_TYPE_VERSUS)
		{
			...
			cSPkg.stPkgData.stMatchReq.bIsPkAI = Convert.ToByte((!bPkAI) ? COM_GAMEPK_TYPE.COM_GAMEPK_WITHOUT_AI : COM_GAMEPK_TYPE.COM_GAMEPK_WITH_AI);
		...
		public static void ReqCreateTeam(uint MapId, bool bPkAI, COM_BATTLE_MAP_TYPE mapType, int maxTeamNum, COM_AI_LEVEL npcAILevel = COM_AI_LEVEL.COM_AI_LEVEL_MIDDLE_OF_NORMALBATTLE, bool isInviteFriendImmediately = false)
		{
			...
			if (MapId > 0u)
			{
				...
				cSPkg.stPkgData.stCreateTeamReq.stBaseInfo.bPkAI = Convert.ToByte((!bPkAI) ? COM_GAMEPK_TYPE.COM_GAMEPK_WITHOUT_AI : COM_GAMEPK_TYPE.COM_GAMEPK_WITH_AI);
		...
		public static void ReqLeaveMatching(bool bManual)
		{
			if (Singleton<CMatchingSystem>.GetInstance().bInMatching)
			{
                ...
                cSPkg.stPkgData.stQuitMultGameReq.bManualQuit = ((!bManual) ? (byte)0 : (byte)1);
				Singleton<NetworkModule>.GetInstance().SendLobbyMsg(ref cSPkg, false);
			}
		}
		...
		public static void SetTeamData(GameObject root, TeamInfo data)
		{
			...
			if (bMapType == 3)
			{
				if ((int)data.stTeamInfo.bMaxTeamNum == CLadderSystem.MultiLadderMaxTeamerNum())
				{
					...
					{
						root.transform.Find("Panel_Main/MapInfo/txtTeam").gameObject.GetComponent<Text>().text = Singleton<CTextManager>.instance.GetText("Common_Team_Player_Type_8");
					}
				}
				else if (data.stTeamInfo.bMaxTeamNum == 5)
				{
					root.transform.Find("Panel_Main/MapInfo/txtTeam").gameObject.GetComponent<Text>().text = Singleton<CTextManager>.instance.GetText("Common_Team_Player_Type_7");
				}
			}
			else
			{
				root.transform.Find("Panel_Main/MapInfo/txtTeam").gameObject.GetComponent<Text>().text = Singleton<CTextManager>.instance.GetText(string.Format("Common_Team_Player_Type_{0}", num / 2));
			}
		...
		public static uint GetCPMap3v3Id()
		{
			...
			while (enumerator.MoveNext())
			{
				...
				ResCounterPartLevelInfo resCounterPartLevelInfo = (ResCounterPartLevelInfo)current.Value;
