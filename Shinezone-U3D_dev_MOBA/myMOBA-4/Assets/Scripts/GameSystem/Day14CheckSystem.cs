...
namespace Assets.Scripts.GameSystem
{
	...
	public class Day14CheckSystem : Singleton<Day14CheckSystem>
	{
		...
		private void SetItem(CUseable usable, Transform uiNode, bool received, bool ready, uint vipLv, int elemIdx)
		{
			...
			if (transform != null)
			{
				transform.GetComponent<Text>().text = string.Format("第{0}天", Day14CheckSystem.GetDay(elemIdx));
			}
			...
			if (transform3 != null)
			{
				transform3.GetComponent<Text>().text = usable.m_name;
			}
			...
			if (transform6 != null)
			{
				...
				{
					component.text = usable.m_stackCount.ToString();
				}
				else
				{
					component.text = usable.m_stackCount / 10000 + "万";
				}
				...
				if (usable.m_type == COM_ITEM_TYPE.COM_OBJTYPE_ITEMSYMBOL)
				{
					...
					{
						component.text = usable.GetSalableCount().ToString();
					}
				}
			}
			...
			if (transform8)
			{
				if (ready)
				{
					...
					if (transform9 != null)
					{
						transform9.GetComponent<Text>().text = string.Format("第{0}天", Day14CheckSystem.GetDay(elemIdx));
					}
				}
				else
				{
		...
		private void SetLeftItem(CUseable usable, Transform uiNode, bool received, bool ready, uint vipLv, int elemIdx)
		{
			...
			if (transform5 != null)
			{
				transform5.GetComponent<Text>().text = usable.m_name;
			}
			Transform transform6 = uiNode.transform.FindChild("Num");
			if (transform6)
			{
				transform6.GetComponent<Text>().text = (elemIdx + 1).ToString();
			}
