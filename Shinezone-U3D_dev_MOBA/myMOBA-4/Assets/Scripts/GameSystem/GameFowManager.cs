	...
	public class GameFowManager : Singleton<GameFowManager>
	{
		...
		private void CopyActorInfoData(List<ACTOR_INFO> actorInfoList)
		{
			...
			for (int i = 0; i < num3; i++)
			{
				...
				this.ms_Actor_Info_Data[num4] = actorInfoList[i].location[num5].x;
				this.ms_Actor_Info_Data[num4 + 1] = actorInfoList[i].location[num5].y;
				this.ms_Actor_Info_Data[num4 + 4] = actorInfoList[i].camps[num5];
				int num6 = HorizonMarkerByFow.TranslateCampToIndex(COM_PLAYERCAMP.COM_PLAYERCAMP_2);
				this.ms_Actor_Info_Data[num4 + 2] = actorInfoList[i].location[num6].x;
				this.ms_Actor_Info_Data[num4 + 3] = actorInfoList[i].location[num6].y;
				this.ms_Actor_Info_Data[num4 + 5] = actorInfoList[i].camps[num6];
				int num7 = i * dataSizeBytes;
                this.ActorInfoDataExtra_[num7] = ((!actorInfoList[i].bDistOnly) ? (byte)0 : (byte)1);
			}
			if (IntPtr.Zero == this.m_tempCpyActorDataPtr)
			{
				int cb = num * 4;
				this.m_tempCpyActorDataPtr = Marshal.AllocHGlobal(cb);
			}
		...
		private void CopyBulletInfoData(List<BULLET_INFO> bulletInfoList)
		{
			...
			{
				int num4 = i * dataSize;
				this.ms_Bullet_Info_Data[num4] = bulletInfoList[i].location.x;
				this.ms_Bullet_Info_Data[num4 + 1] = bulletInfoList[i].location.y;
				this.ms_Bullet_Info_Data[num4 + 2] = bulletInfoList[i].radius;
				int num5 = i * dataSizeBytes;
                this.BulletInfoDataExtra_[num5] = ((!bulletInfoList[i].bDistOnly) ? (byte)0 : (byte)1);
			}
			if (IntPtr.Zero == this.m_tempCpyBulletDataPtr)
			{
				int cb = num * 4;
				this.m_tempCpyBulletDataPtr = Marshal.AllocHGlobal(cb);
			}
		...
		[DllImport("SGameFowProject", CallingConvention = CallingConvention.Cdecl)]
		private static extern void SetIsAreaPermanentLitCallBack([MarshalAs(UnmanagedType.FunctionPtr)] GameFowManager.IsAreaPermanentLitCallBack callback);
