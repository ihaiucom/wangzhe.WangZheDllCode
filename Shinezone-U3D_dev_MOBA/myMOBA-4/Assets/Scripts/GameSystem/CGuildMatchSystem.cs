...
namespace Assets.Scripts.GameSystem
{
	...
	public class CGuildMatchSystem : Singleton<CGuildMatchSystem>
	{
		...
		public void CreateGuildMatchAllTeams()
		{
			...
			for (int i = 0; i < guildMemberInfos.Count; i++)
			{
				if (guildMemberInfos[i].GuildMatchInfo.ullTeamLeaderUid > 0uL)
				{
					...
					{
						if (this.m_teamInfos[j].Key == guildMemberInfos[i].GuildMatchInfo.ullTeamLeaderUid)
						{
							if (this.IsTeamLeader(guildMemberInfos[i].stBriefInfo.uulUid, this.m_teamInfos[j].Key))
							{
								CGuildMatchSystem.TeamPlayerInfo item = this.CreateTeamPlayerInfoObj(guildMemberInfos[i]);
								this.m_teamInfos[j].Value.Insert(0, item);
							}
							else if (!this.FindAndReplaceEmptyPlayerSlot(this.m_teamInfos[j].Value, guildMemberInfos[i]))
							{
								CGuildMatchSystem.TeamPlayerInfo item2 = this.CreateTeamPlayerInfoObj(guildMemberInfos[i]);
								if (this.m_teamInfos[j].Value.Count < 5)
								{
									this.m_teamInfos[j].Value.Add(item2);
								}
							}
							flag = true;
							break;
		...
		private void CreateNewTeam(GuildMemInfo guildMemberInfo)
		{
			...
			KeyValuePair<ulong, ListView<CGuildMatchSystem.TeamPlayerInfo>> item = new KeyValuePair<ulong, ListView<CGuildMatchSystem.TeamPlayerInfo>>(guildMemberInfo.GuildMatchInfo.ullTeamLeaderUid, listView);
		...
		public bool IsSelfInAnyTeam()
		{
			...
			if (this.m_teamInfos != null)
			{
				...
				{
					for (int j = 0; j < this.m_teamInfos[i].Value.Count; j++)
					{
						if (this.m_teamInfos[i].Value[j].Uid == playerUllUID)
						{
							return true;
						}
					}
		...
		private void AddMemberToTeam(ulong memberNewTeamLeaderUid, GuildMemInfo memberInfo)
		{
			...
			for (int i = 0; i < this.m_teamInfos.Count; i++)
			{
				if (memberNewTeamLeaderUid == this.m_teamInfos[i].Key)
				{
					if (!this.FindAndReplaceEmptyPlayerSlot(this.m_teamInfos[i].Value, memberInfo))
					{
						CGuildMatchSystem.TeamPlayerInfo item = this.CreateTeamPlayerInfoObj(memberInfo);
						if (this.m_teamInfos[i].Value.Count < 5)
						{
							this.m_teamInfos[i].Value.Add(item);
						}
					}
					if (CGuildHelper.IsSelf(memberInfo.stBriefInfo.uulUid) && i != 0)
					{
						KeyValuePair<ulong, ListView<CGuildMatchSystem.TeamPlayerInfo>> item2 = this.m_teamInfos[i];
						this.m_teamInfos.RemoveAt(i);
						this.m_teamInfos.Insert(0, item2);
					}
					return;
				}
			}
		...
		private void RemoveMemberFromOldTeam(ulong memberUid)
		{
			...
			{
				if (this.IsTeamLeader(memberUid, this.m_teamInfos[i].Key))
				{
					this.m_teamInfos.RemoveAt(i);
				...
				else
				{
					for (int j = 0; j < this.m_teamInfos[i].Value.Count; j++)
					{
						if (memberUid == this.m_teamInfos[i].Value[j].Uid)
						{
							this.RemoveMemberFromTeam(this.m_teamInfos[i].Value[j]);
							this.PromptKickedFromTeamTip(memberUid);
						}
					}
				}
		...
		public void SetTeamInfo(SCPKG_CHG_GUILD_MATCH_LEADER_NTF ntf)
		{
			...
			if (ntf.ullTeamLeaderUid > 0uL)
			{
				for (int i = 0; i < this.m_teamInfos.Count; i++)
				{
					if (this.IsTeamLeader(ntf.ullTeamLeaderUid, this.m_teamInfos[i].Key))
					{
						return;
					}
				}
			...
			else
			{
				for (int j = 0; j < this.m_teamInfos.Count; j++)
				{
					if (this.IsTeamLeader(ntf.ullUid, this.m_teamInfos[j].Key))
					{
		...
		public void SetTeamMemberReadyState(SCPKG_SET_GUILD_MATCH_READY_NTF ntf)
		{
			...
			for (int i = 0; i < (int)ntf.bCnt; i++)
			{
				for (int j = 0; j < this.m_teamInfos.Count; j++)
				{
					for (int k = 0; k < this.m_teamInfos[j].Value.Count; k++)
					{
						if (this.m_teamInfos[j].Value[k].Uid == ntf.astInfo[i].ullUid)
						{
							this.m_teamInfos[j].Value[k].IsReady = Convert.ToBoolean(ntf.astInfo[i].bIsReady);
							flag = true;
							break;
						}
					}
		...
		public void SetTeamMemberReadyState(ulong teamMemberUid, bool isReady)
		{
			...
			for (int i = 0; i < this.m_teamInfos.Count; i++)
			{
				for (int j = 0; j < this.m_teamInfos[i].Value.Count; j++)
				{
					if (this.m_teamInfos[i].Value[j].Uid == teamMemberUid)
					{
						this.m_teamInfos[i].Value[j].IsReady = isReady;
						return;
					}
				}
			}
		...
		private void RefreshInvitationList()
		{
			...
			int count = this.m_invitationInfos.Count;
			...
			for (int i = 0; i < count; i++)
			{
				...
				GuildMemInfo guildMemberInfoByUid = CGuildHelper.GetGuildMemberInfoByUid(this.m_invitationInfos[i].invitationInfo.ullUid);
				if (guildMemberInfoByUid != null)
				{
					...
					component6.text = guildMemberInfoByUid.stBriefInfo.sName;
					component7.text = this.GetInvitationStateText((COM_GUILDMATCH_INVITECARD_STATE)this.m_invitationInfos[i].invitationInfo.bState, this.m_invitationInfos[i].isValid);
					this.SetInvitationViewBtnRedDot(gameObject2, (COM_GUILDMATCH_INVITECARD_STATE)this.m_invitationInfos[i].invitationInfo.bState, this.m_invitationInfos[i].isValid);
				}
			}
		}
		...
		private void SetGuildScoreListElement(GameObject listElementGo, int guildScoreListIndex)
		{
			...
			component2.text = StringHelper.UTF8BytesToString(ref cSDT_RANKING_LIST_ITEM_INFO.stExtraInfo.stDetailInfo.stGuildMatch.szGuildName);
		...
		private void SetInvitedRelatedWidgets(GameObject inviteButtonGo, Text teamStateText)
		{
			...
			teamStateText.text = "<color=#e49316>" + Singleton<CTextManager>.GetInstance().GetText("GuildMatch_SignUp_Invite_State_Wait_For_Response") + "</color>";
		}
		...
		private void OnTeamListElementEnabled(CUIEvent uiEvent)
		{
			...
			if (srcWidgetIndexInBelongedList >= 0 && srcWidgetIndexInBelongedList < this.m_teamInfos.Count)
			{
				...
				this.SetTeamListElement(teamListElement, this.m_teamInfos[srcWidgetIndexInBelongedList].Key, this.m_teamInfos[srcWidgetIndexInBelongedList].Value);
			}
		}
		...
		private void OnPlayerNameChange()
		{
			if (this.m_teamInfos != null)
			{
				for (int i = 0; i < this.m_teamInfos.Count; i++)
				{
					int count = this.m_teamInfos[i].Value.Count;
					for (int j = 0; j < count; j++)
					{
						if (CGuildHelper.IsSelf(this.m_teamInfos[i].Value[j].Uid))
						{
							this.m_teamInfos[i].Value[j].Name = Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo().Name;
							return;
						}
					}
				}
		...
		private void RefreshGuildMatchOpenTime()
		{
			...
			component.text = this.GetGuildMatchOpenTimeDesc();
		}
		...
		private void RefreshGuildMatchSignUpPanel()
		{
			...
			if (this.IsCanSignUp())
			{
				...
				{
					widget3.CustomSetActive(true);
					component.text = Singleton<CTextManager>.GetInstance().GetText("GuildMatch_SignUp_Prompt");
			...
			else
			{
				...
				{
					widget3.CustomSetActive(true);
					component.text = Singleton<CTextManager>.GetInstance().GetText("GuildMatch_SignUp_Check_Prompt");
		...
		public void RefreshTeamList()
		{
			...
			component.SetElementAmount(this.m_teamInfos.Count);
		...
		private void SetTeamListElement(CUIListElementScript teamListElement, ulong teamLeaderUid, ListView<CGuildMatchSystem.TeamPlayerInfo> teamPlayerInfos)
		{
			...
			for (int i = 0; i < 5; i++)
			{
				this.SetPlayerListElement(component7.GetElemenet(i), (i >= teamPlayerInfos.Count) ? null : teamPlayerInfos[i], teamLeaderUid);
			}
		}
		...
		public bool IsTeamFull(ulong teamLeaderUid)
		{
			...
			{
				int i = 0;
				while (i < this.m_teamInfos.Count)
				{
					if (this.m_teamInfos[i].Key == teamLeaderUid)
					{
						if (this.m_teamInfos[i].Value.Count < 5)
						{
							return false;
						...
						for (int j = 0; j < this.m_teamInfos[i].Value.Count; j++)
						{
							if (!this.IsSlotOccupied(this.m_teamInfos[i].Value[j]))
							{
								return false;
							}
						}
		...
		{
			return this.m_signUpInfos == null || this.m_signUpInfos.Count == 0;
		}
		...
		public bool IsHaveInvitationUnhandled()
		{
			if (this.m_invitationInfos != null)
			{
				for (int i = 0; i < this.m_invitationInfos.Count; i++)
				{
					if (this.m_invitationInfos[i].invitationInfo.bState == 0)
					{
						return true;
					}
				}
		...
		private void OnSignUpCardFormClosed(CUIEvent uiEvent)
		{
			...
			if (string.Equals(component.text, Singleton<CTextManager>.GetInstance().GetText("GuildMatch_SignUp_New_Card")))
			{
				this.m_createSignUpCardForm = null;
			}
			else if (string.Equals(component.text, Singleton<CTextManager>.GetInstance().GetText("GuildMatch_SignUp_Modify_Card")))
			{
		...
		private void SetSignUpCardInfo(bool isModify, CUIFormScript signUpCardForm)
		{
			...
			component.text = Singleton<CTextManager>.GetInstance().GetText((!isModify) ? "GuildMatch_SignUp_New_Card" : "GuildMatch_SignUp_Modify_Card");
			widget.CustomSetActive(!isModify);
			component2.text = playerGuildMemberInfo.stBriefInfo.sName + "   Lv." + playerGuildMemberInfo.stBriefInfo.dwLevel;
			...
			component5.text = this.m_signUpPosTexts[(int)this.m_signUpPos1];
			component6.text = this.m_signUpPosTexts[(int)this.m_signUpPos2];
			...
			{
				component8.text = this.m_signUpMemoText;
			}
			else
			{
				component8.text = string.Empty;
				component8.placeholder.GetComponent<Text>().text = Singleton<CTextManager>.GetInstance().GetText("GuildMatch_SignUp_Default_Memo");
			}
			component9.text = this.GetSignUpTimeText();
			if (isModify)
			{
		...
		private void SetSignUpRankDesc(Text txtRankGrade, Text txtRankStar, GuildMemInfo memberInfo)
		{
			...
			txtRankGrade.text = CLadderView.GetRankName(bGrade, memberInfo.stBriefInfo.dwClassOfRank);
			txtRankStar.text = "X" + dwScore;
		}
		...
		private void SetSignUpStateColumn(Transform element, ulong memberUid, bool isNeedHighlightText = false)
		{
			...
			if (this.IsSelfBelongedTeamLeader())
			{
				...
				{
					component.text = Singleton<CTextManager>.GetInstance().GetText("GuildMatch_SignUp_In_Team");
				}
				else
				{
					gameObject.CustomSetActive(false);
			...
			{
				component.text = Singleton<CTextManager>.GetInstance().GetText("GuildMatch_SignUp_In_Examine");
			}
			else
			{
				component.text = "<color=#27b56a>" + Singleton<CTextManager>.GetInstance().GetText("GuildMatch_SignUp_In_Examine") + "</color>";
			}
		}
		...
		private void InitPosDropList(GameObject posDropListGo)
		{
			...
			for (int i = 0; i < this.m_signUpPosTexts.Length; i++)
			{
				...
				if (elemenet != null)
				{
					...
					component2.text = this.m_signUpPosTexts[i];
				}
			}
		}
		...
		private void OpenSignUpListForm()
		{
			...
			component.text = Singleton<CTextManager>.GetInstance().GetText("GuildMatch_SignUp_List_Title", new string[]
			{
			...
			component2.text = Singleton<CTextManager>.GetInstance().GetText("GuildMatch_SignUp_SignUp_Time_Prefix") + "<color=#e49316>" + this.GetSignUpTimeText() + "</color>";
		...
		private void RefreshSignUpList()
		{
			...
			if (this.m_signUpInfos != null)
			{
				if (this.m_signUpInfos.Count > 0)
				{
					this.SortSignUpList();
				}
				component.SetElementAmount(this.m_signUpInfos.Count);
			}
		}
			...
			if (this.m_signUpListForm != null)
			{
				...
				for (int i = 0; i < component.GetElementAmount(); i++)
				{
					...
					if (elemenet != null)
					{
						...
						ulong num;
						if (ulong.TryParse(component2.text, out num) && num == targetUid)
						{
		...
		private void OnCreateSignUpCard(CUIEvent uiEvent)
		{
			...
			this.m_signUpMemoText = component.text;
			this.RequestGuildMatchSignUp(this.m_signUpPos1, this.m_signUpPos2, component.text);
		}
		...
		private void OnModifySignUpCard(CUIEvent uiEvent)
		{
			...
			this.m_signUpMemoText = component.text;
			this.RequestModifyGuildMatchSignUp(this.m_signUpPos1, this.m_signUpPos2, component.text);
		}
		...
		private void OnViewInvitation(CUIEvent uiEvent)
		{
			...
			int srcWidgetIndexInBelongedList = uiEvent.m_srcWidgetIndexInBelongedList;
			if (0 <= srcWidgetIndexInBelongedList && srcWidgetIndexInBelongedList < this.m_invitationInfos.Count)
			{
				this.m_curViewInvitationInviterUid = this.m_invitationInfos[srcWidgetIndexInBelongedList].invitationInfo.ullUid;
				if (!this.CheckInvitationValid())
				{
					return;
				}
				GuildMemInfo guildMemberInfoByUid = CGuildHelper.GetGuildMemberInfoByUid(this.m_invitationInfos[srcWidgetIndexInBelongedList].invitationInfo.ullUid);
				if (guildMemberInfoByUid != null)
				{
					...
					COM_GUILDMATCH_INVITECARD_STATE bState = (COM_GUILDMATCH_INVITECARD_STATE)this.m_invitationInfos[srcWidgetIndexInBelongedList].invitationInfo.bState;
		...
		{
			bool commonBool = uiEvent.m_eventParams.commonBool;
			ulong num = (!commonBool) ? this.m_curViewInvitationInviterUid : uiEvent.m_eventParams.commonUInt64Param1;
			if (num > 0uL)
			{
				GuildMemInfo playerGuildMemberInfo = CGuildHelper.GetPlayerGuildMemberInfo();
				if (playerGuildMemberInfo == null)
		...
		private bool CheckInvitationValid()
		{
			...
			if (guildMemberInfoByUid != null && guildMemberInfoByUid.GuildMatchInfo.ullTeamLeaderUid != this.m_curViewInvitationInviterUid && this.m_invitationInfos != null)
			{
				for (int i = 0; i < this.m_invitationInfos.Count; i++)
				{
					if (this.m_curViewInvitationInviterUid == this.m_invitationInfos[i].invitationInfo.ullUid)
					{
						this.m_invitationInfos[i] = new CGuildMatchSystem.stInvitationInfo
						{
							invitationInfo = this.m_invitationInfos[i].invitationInfo
						};
		...
		private void SignUpCardSelectPos(CUIEvent uiEvent, int widgetId, ref byte signUpPos)
		{
			...
			if (transform != null)
			{
				component.text = transform.GetComponent<Text>().text;
			}
		...
		private void OnSignUpListInviteBtnClick(CUIEvent uiEvent)
		{
			...
			if (parent != null)
			{
				...
				if (transform != null)
				{
					...
					component.text = "<color=#27b56a>" + Singleton<CTextManager>.GetInstance().GetText("GuildMatch_SignUp_Invite_State_Wait_For_Response") + "</color>";
				}
		...
		{
			GameObject srcWidget = uiEvent.m_srcWidget;
			if (srcWidget == null || this.m_signUpInfos == null || this.m_signUpInfos.Count == 0 || uiEvent.m_srcWidgetIndexInBelongedList < 0 || uiEvent.m_srcWidgetIndexInBelongedList >= this.m_signUpInfos.Count)
			{
				return;
			}
			Transform transform = srcWidget.transform;
			...
			CGuildMatchSystem.stSignUpInfo stSignUpInfo = this.m_signUpInfos[uiEvent.m_srcWidgetIndexInBelongedList];
			...
			component.text = stSignUpInfo.uid.ToString();
			...
			component5.text = guildMemberInfoByUid.stBriefInfo.sName;
			this.SetSignUpRankDesc(component6, component7, guildMemberInfoByUid);
			component8.text = "Lv." + guildMemberInfoByUid.stBriefInfo.dwLevel;
			...
			{
				component9.text = Singleton<CTextManager>.GetInstance().GetText("Common_None");
			}
			if (this.m_signUpPosTexts != null && this.m_signUpPosTexts.Length > 0)
			{
				component10.text = ((stSignUpInfo.pos1 != 0) ? this.m_signUpPosTexts[(int)stSignUpInfo.pos1] : "--") + " " + ((stSignUpInfo.pos2 != 0) ? this.m_signUpPosTexts[(int)stSignUpInfo.pos2] : "--");
			}
			component11.text = (string.IsNullOrEmpty(stSignUpInfo.memo) ? Singleton<CTextManager>.GetInstance().GetText("Common_None") : stSignUpInfo.memo);
		...
		private void SetHighlightedText(params Text[] txtComponents)
		{
			...
			{
				txtComponents[i].text = "<color=#e49316>" + txtComponents[i].text + "</color>";
			}
		}
		...
		private void RefreshSelfSignUpListInfo()
		{
			if (this.m_signUpInfos != null)
			{
				for (int i = 0; i < this.m_signUpInfos.Count; i++)
				{
					if (this.m_signUpInfos[i].uid == Singleton<CRoleInfoManager>.GetInstance().masterUUID)
					{
						CGuildMatchSystem.stSignUpInfo value = default(CGuildMatchSystem.stSignUpInfo);
						value.pos1 = this.m_signUpPos1;
						value.pos2 = this.m_signUpPos2;
						value.memo = ((this.m_signUpMemoText == null) ? string.Empty : this.m_signUpMemoText);
						value.uid = this.m_signUpInfos[i].uid;
						value.heroNames = this.m_signUpInfos[i].heroNames;
						value.winPercents = this.m_signUpInfos[i].winPercents;
						this.m_signUpInfos[i] = value;
					}
				}
				if (this.m_signUpListForm != null)
				{
		...
		private void RefreshInvitationStatePanel(COM_GUILDMATCH_INVITECARD_STATE state)
		{
			...
			else if (state == COM_GUILDMATCH_INVITECARD_STATE.COM_GUILDMATCH_INVITECARD_STATE_ACCEPT)
			{
				...
				widget2.CustomSetActive(true);
				widget3.CustomSetActive(true);
				widget2.GetComponent<Text>().text = "<color=#27b56a>" + Singleton<CTextManager>.GetInstance().GetText("GuildMatch_SignUp_Invitation_Accepted_Tip") + "</color>";
			}
			else if (state == COM_GUILDMATCH_INVITECARD_STATE.COM_GUILDMATCH_INVITECARD_STATE_REFUSE)
			{
				widget.CustomSetActive(false);
				widget2.CustomSetActive(true);
				widget3.CustomSetActive(false);
				widget2.GetComponent<Text>().text = Singleton<CTextManager>.GetInstance().GetText("GuildMatch_SignUp_Invitation_Rejected_Tip");
			}
		}
		...
		private void RemoveHandledInvitation(ulong inviterUid)
		{
			if (this.m_invitationInfos != null)
			{
				for (int i = 0; i < this.m_invitationInfos.Count; i++)
				{
					if (inviterUid == this.m_invitationInfos[i].invitationInfo.ullUid)
					{
		...
		public void ShowAllUnhandledOnlineInvitation()
		{
			if (this.m_unhandledOnlineInvitationInviterUids != null)
			{
				for (int i = 0; i < this.m_unhandledOnlineInvitationInviterUids.Count; i++)
				{
					this.OpenOnlineInvitationForm(this.m_unhandledOnlineInvitationInviterUids[i]);
				}
				this.m_unhandledOnlineInvitationInviterUids.Clear();
			}
		}
		...
		private void RefreshPlatformGroupInviteBtn()
		{
			...
			if (selectedIndex == CGuildMatchSystem.enRankTab.MemberInviteOrInvitation)
			{
				if (this.IsSelfBelongedTeamLeader())
				{
		...
		public static void ReceiveGuildMatchGetInvitation(CSPkg msg)
		{
			...
			{
				CGuildMatchSystem.stInvitationInfo item = new CGuildMatchSystem.stInvitationInfo
				{
				...
				instance.m_invitationInfos.Add(item);
			}
		...
		{
			if ((this.m_signUpInfos == null || this.m_signUpInfos.Count == 0) && rsp.bSingUpNum > 0)
			{
				return true;
			}
			if (this.m_signUpInfos == null)
			...
			{
				bool flag = true;
				for (int j = 0; j < this.m_signUpInfos.Count; j++)
				{
					if (rsp.astSingUpList[i].ullUid == this.m_signUpInfos[j].uid)
					{
						flag = false;
						break;
					}
		...
		public static void ReceiveGetGuildMatchSignUpList(CSPkg msg)
		{
			...
			{
				CGuildMatchSystem.stSignUpInfo item = default(CGuildMatchSystem.stSignUpInfo);
				CSDT_GUILDMATCH_SIGNUPINFO cSDT_GUILDMATCH_SIGNUPINFO = stGuildMatchSignUpListRsp.astSingUpList[i];
				item.uid = cSDT_GUILDMATCH_SIGNUPINFO.ullUid;
				item.pos1 = cSDT_GUILDMATCH_SIGNUPINFO.bPos1;
				item.pos2 = cSDT_GUILDMATCH_SIGNUPINFO.bPos2;
				item.memo = StringHelper.UTF8BytesToString(ref cSDT_GUILDMATCH_SIGNUPINFO.szBeiZhu);
				item.heroNames = new string[(int)cSDT_GUILDMATCH_SIGNUPINFO.bHeroNum];
				item.winPercents = new byte[(int)cSDT_GUILDMATCH_SIGNUPINFO.bHeroNum];
				for (int j = 0; j < (int)cSDT_GUILDMATCH_SIGNUPINFO.bHeroNum; j++)
				{
					item.heroNames[j] = CHeroInfo.GetHeroName(cSDT_GUILDMATCH_SIGNUPINFO.astHeroInfo[j].dwHeroID);
					item.winPercents[j] = (byte)(100u * cSDT_GUILDMATCH_SIGNUPINFO.astHeroInfo[j].dwGameWinNum / (cSDT_GUILDMATCH_SIGNUPINFO.astHeroInfo[j].dwGameWinNum + cSDT_GUILDMATCH_SIGNUPINFO.astHeroInfo[j].dwGameLoseNum));
				}
				instance.m_signUpInfos.Add(item);
