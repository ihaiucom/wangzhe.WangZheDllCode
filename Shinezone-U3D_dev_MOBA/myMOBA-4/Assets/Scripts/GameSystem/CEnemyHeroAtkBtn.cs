	...
	public class CEnemyHeroAtkBtn
	{
		...
		private void InitBtnInfo()
		{
			...
			for (int i = 0; i < this.m_iCurEnemyPlayerCount; i++)
			{
				...
				string heroSkinPic = CSkinInfo.GetHeroSkinPic((uint)list[i].handle.TheActorMeta.ConfigId, 0u);
				...
				if (this.m_ui3dRes != null)
				{
					...
					if (gameObject)
					{
						...
						if (gameObject2)
						{
							...
							this.m_arrBtnInfo[i] = new CEnemyHeroAtkBtn.BTN_INFO(transform, list[i], CEnemyHeroAtkBtn.ENM_ENEMY_HERO_STATE.ENM_ENEMY_HERO_STATE_TOOFAR, true, gameObject, component4);
						}
					}
				}
			}
				...
				if (btnTransform)
				{
					...
					if (objHp && component)
					{
						if ((double)num < 0.3 && this.m_arrBtnInfo[iBtnIndex].bIsUseBlueHpImage)
						{
							...
							component.color = CUIUtility.s_Color_EnemyHero_Button_PINK;
		...
		private void UpdateHeroAtkBtnWithVisible(int iBtnIndex, bool bVisible)
		{
			if (iBtnIndex >= 0 && iBtnIndex < this.m_iCurEnemyPlayerCount)
			{
				if (bVisible)
				{
					CEnemyHeroAtkBtn.BTN_INFO[] expr_25_cp_0 = this.m_arrBtnInfo;
					expr_25_cp_0[iBtnIndex].heroState = (expr_25_cp_0[iBtnIndex].heroState & -3);
				}
				else
				{
					CEnemyHeroAtkBtn.BTN_INFO[] expr_44_cp_0 = this.m_arrBtnInfo;
					expr_44_cp_0[iBtnIndex].heroState = (expr_44_cp_0[iBtnIndex].heroState | 2);
				}
		...
		private void UpdateHeroAtkBtnWithHostState(int iBtnIndex, bool bDead)
		{
			if (iBtnIndex >= 0 && iBtnIndex < this.m_iCurEnemyPlayerCount)
			{
				...
				{
					CEnemyHeroAtkBtn.BTN_INFO[] expr_25_cp_0 = this.m_arrBtnInfo;
					expr_25_cp_0[iBtnIndex].heroState = (expr_25_cp_0[iBtnIndex].heroState | 16);
				}
				else
				{
					CEnemyHeroAtkBtn.BTN_INFO[] expr_44_cp_0 = this.m_arrBtnInfo;
					expr_44_cp_0[iBtnIndex].heroState = (expr_44_cp_0[iBtnIndex].heroState & -17);
				}
				this.SetBtnStateByBtnInfo(iBtnIndex);
			}
		}
		...
		private void OnHeroDead(PoolObjHandle<ActorRoot> hero)
		{
			...
			if (hero)
			{
				if (ActorHelper.IsHostEnemyActor(ref hero))
				{
					...
					{
						CEnemyHeroAtkBtn.BTN_INFO[] expr_4A_cp_0 = this.m_arrBtnInfo;
						int expr_4A_cp_1 = btnIndexByActor;
						expr_4A_cp_0[expr_4A_cp_1].heroState = (expr_4A_cp_0[expr_4A_cp_1].heroState | 4);
		...
		public void OnActorRevive(ref DefaultGameEventParam prm)
		{
			...
			if (prm.src && prm.src.handle.TheActorMeta.ActorType == ActorTypeDef.Actor_Type_Hero)
			{
				if (prm.src.handle.TheActorMeta.ActorCamp != this.m_hostActor.handle.TheActorMeta.ActorCamp)
				{
					...
					if (btnIndexByActor >= 0 && btnIndexByActor < this.m_iCurEnemyPlayerCount)
					{
						CEnemyHeroAtkBtn.BTN_INFO[] expr_91_cp_0 = this.m_arrBtnInfo;
						int expr_91_cp_1 = btnIndexByActor;
						expr_91_cp_0[expr_91_cp_1].heroState = (expr_91_cp_0[expr_91_cp_1].heroState & -5);
		...
		private void UpdateHeroAtkBtnWithDistance(int iBtnIndex)
		{
			...
			if (iBtnIndex >= 0 && iBtnIndex < this.m_iCurEnemyPlayerCount)
			{
				...
				{
					CEnemyHeroAtkBtn.BTN_INFO[] expr_CC_cp_0 = this.m_arrBtnInfo;
					expr_CC_cp_0[iBtnIndex].heroState = (expr_CC_cp_0[iBtnIndex].heroState & -9);
				}
				else
				{
					CEnemyHeroAtkBtn.BTN_INFO[] expr_EB_cp_0 = this.m_arrBtnInfo;
					expr_EB_cp_0[iBtnIndex].heroState = (expr_EB_cp_0[iBtnIndex].heroState | 8);
