...
namespace Assets.Scripts.GameSystem
{
	...
	public class PauseControl
	{
		...
		public PauseControl(CUIFormScript rootForm)
		{
			...
			this.MaxAllowTimes = ((!curLvelContext.IsGameTypePvpRoom()) ? (byte)0 : ((!Singleton<WatchController>.GetInstance().IsLiveCast) ? curLvelContext.m_pauseTimes : (byte)255));
		...
		public void ExecPauseCommand(byte pauseCommand, byte pauseByCamp, byte[] campPauseTimes)
		{
			...
			if (this._root)
			{
				...
				if (flag)
				{
					...
					COM_PLAYERCAMP cOM_PLAYERCAMP = (!Singleton<WatchController>.GetInstance().IsWatching) ? Singleton<GamePlayerCenter>.GetInstance().hostPlayerCamp : COM_PLAYERCAMP.COM_PLAYERCAMP_MID;
					this._pauseCampTxt.text = Singleton<CTextManager>.GetInstance().GetText(string.Format("PauseTips_{0}Look{1}", (int)cOM_PLAYERCAMP, (int)pauseByCamp));
					...
					else
					{
						...
						this._waitResumeSecond = (int)((dataByKey == null) ? 60u : dataByKey.dwConfValue);
					}
					...
					if (cOM_PLAYERCAMP != COM_PLAYERCAMP.COM_PLAYERCAMP_MID)
					{
						...
						this._remainTimesTxt.text = (num - (int)campPauseTimes[(int)cOM_PLAYERCAMP]).ToString();
					}
					else
					{
