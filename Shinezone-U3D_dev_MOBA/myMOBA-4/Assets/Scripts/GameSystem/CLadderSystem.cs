...
namespace Assets.Scripts.GameSystem
{
	...
	public class CLadderSystem : Singleton<CLadderSystem>
	{
		...
		{
			CLadderSystem.REQ_HERO_NUM = ((!this.IsNoHeroLimit()) ? GameDataMgr.globalInfoDatabin.GetDataByKey(101u).dwConfValue : 1u);
		}
		...
		public static bool IsInSeason()
		{
			...
			{
				if (num >= current.Value.ullStartTime && num <= current.Value.ullEndTime)
				{
					return true;
				}
			}
		...
		{
			int logicGrade = (inLogicGrade != 0) ? inLogicGrade : 1;
		...
		public string GetLadderSeasonName(ulong time)
		{
			foreach (KeyValuePair<uint, ResRankSeasonConf> current in GameDataMgr.rankSeasonDict)
			{
				if (current.Value.ullStartTime <= time && time < current.Value.ullEndTime)
				{
					return current.Value.szSeasonName;
				}
			}
			return string.Empty;
		}
		...
		public byte GetHistorySeasonGrade(ulong time)
		{
			if (Singleton<CLadderSystem>.GetInstance().historySeasonData != null)
			{
				...
				{
					if ((ulong)Singleton<CLadderSystem>.GetInstance().historySeasonData[i].dwSeaStartTime <= time && time < (ulong)Singleton<CLadderSystem>.GetInstance().historySeasonData[i].dwSeaEndTime)
					{
			...
			if (rankSeasonDict != null)
			{
				...
				while (enumerator.MoveNext())
				{
					...
					if (current.Value.ullStartTime == Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo().m_rankCurSeasonStartTime)
					{
						...
						if (current2.Value.ullStartTime <= time)
						{
							KeyValuePair<uint, ResRankSeasonConf> current3 = enumerator.Current;
							if (time < current3.Value.ullEndTime)
							{
								return true;
							}
						}
