...
namespace Assets.Scripts.GameSystem
{
	public class KillNotify
	{
		...
		{
			byte result;
			KillNotify.knPriority.TryGetValue(type, out result);
			return result;
		}

		public static void LoadConfig()
		{
			if (!int.TryParse(Singleton<CTextManager>.instance.GetText("KN_MaxCount"), out KillNotify.max_count))
			{
				DebugHelper.Assert(false, "---killnotify 教练你配的 KN_MaxCount 好像不是整数哦， check out");
			}
			if (!int.TryParse(Singleton<CTextManager>.instance.GetText("KN_HideTime"), out KillNotify.HideTime))
			{
				DebugHelper.Assert(false, "---killnotify 教练你配的 KN_HideTime 好像不是整数哦， check out");
			}
			if (!int.TryParse(Singleton<CTextManager>.instance.GetText("KN_Play_DeltaTime"), out KillNotify.s_play_deltaTime))
			{
			...
			for (int i = 0; i < values.Length; i++)
			{
				...
				if (num != 0)
				{
					...
					if (dataByKey != null)
					{
						...
						{
							KillNotify.knPriority[(KillDetailInfoType)num] = dataByKey.bPriority;
						}
					}
				}
			}
		...
		private void _AddkillInfo(KillInfo killInfo)
		{
			...
			if (this.KillInfoList.Count < KillNotify.max_count)
			{
				this.KillInfoList.Add(killInfo);
			}
			else
			{
				...
				{
					if ((int)this.KillInfoList[i].bPriority < num)
					{
						num = (int)this.KillInfoList[i].bPriority;
						num2 = i;
					}
				}
				if ((int)killInfo.bPriority >= num && num2 >= 0 && num2 < this.KillInfoList.Count)
				{
			...
			{
				if (this.KillInfoList[j].MsgType == KillDetailInfoType.Info_Type_AllDead)
				{
					num3 = j;
				}
			}
			if (num3 >= 0 && num3 < this.KillInfoList.Count)
			{
				KillInfo value = this.KillInfoList[num3];
				this.KillInfoList[num3] = this.KillInfoList[this.KillInfoList.Count - 1];
				this.KillInfoList[this.KillInfoList.Count - 1] = value;
			}
		}
