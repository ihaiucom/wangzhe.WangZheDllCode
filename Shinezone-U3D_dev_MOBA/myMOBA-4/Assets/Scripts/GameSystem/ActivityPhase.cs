...
namespace Assets.Scripts.GameSystem
{
	public abstract class ActivityPhase
	{
		...
		public virtual uint MultipleTimes
		{
			get
			{
				return (!this.InMultipleTime || !this.Owner.InMultipleTime) ? 0u : this.Owner.MultipleTimes;
			}
		}
		...
		public virtual bool CheckTimeState()
		{
			...
			if (this._owner.timeState == Activity.TimeState.Going)
			{
				if (this.StartTime > 0 && this.CloseTime > 0)
				{
					...
					DateTime d2 = Utility.SecondsToDateTime(d.Year, d.Month, d.Day, this.StartTime);
					this._secondSpan = (int)(d - d2).TotalSeconds;
					...
					{
						DateTime d3 = Utility.SecondsToDateTime(d.Year, d.Month, d.Day, this.CloseTime);
						this._secondSpan = (int)(d - d3).TotalSeconds;
						if (this._secondSpan < 0)
						{
							this._secondSpan = -this._secondSpan;
