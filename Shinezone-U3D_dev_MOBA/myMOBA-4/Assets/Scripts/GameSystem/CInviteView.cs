	...
	internal class CInviteView
	{
		...
		public static void SetInviteFriendData(CUIFormScript form, COM_INVITE_JOIN_TYPE joinType)
		{
			...
			if (component3 != null)
			{
				...
				{
					component3.text = Singleton<CTextManager>.instance.GetText("Reserve_Condition_Tip");
				}
				else
				{
					component3.text = Singleton<CTextManager>.instance.GetText("Reserve_ServerDisable_Tip");
				}
			}
			...
			else
			{
				...
				if (flag)
				{
					...
					{
						component4.text = Singleton<CTextManager>.GetInstance().GetText("Share_Room_Info_QQ");
						gameObject.CustomSetActive(true);
						gameObject2.CustomSetActive(false);
					}
					else if (Singleton<ApolloHelper>.GetInstance().CurPlatform == ApolloPlatform.Wechat)
					{
						component4.text = Singleton<CTextManager>.GetInstance().GetText("Share_Room_Info_WX");
		...
		public static void SetLBSData(CUIFormScript form, COM_INVITE_JOIN_TYPE joinType)
		{
			...
			if (Singleton<CFriendContoller>.instance.model.EnableShareLocation)
			{
				...
				elementAmount = ((lBSList == null) ? 0 : lBSList.Count);
		...
		private static void SetFriendState(GameObject element, ref COMDT_FRIEND_INFO friend)
		{
			...
			if (friend.bIsOnline == 1)
			{
				...
				if (cOM_ACNT_GAME_STATE == COM_ACNT_GAME_STATE.COM_ACNT_GAME_STATE_IDLE)
				{
					if (flag)
					{
						component.text = Singleton<CTextManager>.instance.GetText("Common_NotDisturb");
						obj.CustomSetActive(true);
					}
					else
					{
						component.text = Singleton<CInviteSystem>.instance.GetInviteStateStr(friend.stUin.ullUid, false);
					}
					if (friendInGaming != null && friendInGaming.IsUseTGA())
					{
						component.gameObject.CustomSetActive(true);
						component.text = string.Format("<color=#ffff00>{0}</color>", Singleton<CTextManager>.instance.GetText("TGA_Friend_State"));
					}
				...
				else if (cOM_ACNT_GAME_STATE == COM_ACNT_GAME_STATE.COM_ACNT_GAME_STATE_SINGLEGAME || cOM_ACNT_GAME_STATE == COM_ACNT_GAME_STATE.COM_ACNT_GAME_STATE_MULTIGAME || cOM_ACNT_GAME_STATE == COM_ACNT_GAME_STATE.COM_ACNT_GAME_STATE_WAITMULTIGAME)
				{
					...
					if (friendInGaming.startTime > 0u)
					{
						...
						component3.text = string.Format(Singleton<CTextManager>.instance.GetText("Common_Gaming_Time"), CInviteView.GetStartMinute(friendInGaming.startTime));
							...
							else
							{
								...
								if (gameObject4 != null)
								{
									...
									if (component6 != null)
									{
										...
										{
											component6.text = model.friendReserve.Reserve_Success;
										}
										else if (ent != null && ent.result == 1)
										{
											component6.text = model.friendReserve.Reserve_Failed;
										}
										else
										{
											component6.text = model.friendReserve.Reserve_Wait4Rsp;
										}
									}
								}
							}
		...
		private static int GetStartMinute(uint startTime)
		{
			...
			int value = (int)(dateTime - dateTime2).TotalMinutes;
		...
		private static void SetLBSState(GameObject element, ref CSDT_LBS_USER_INFO LBSInfo)
		{
			...
			if (stLbsUserInfo.bIsOnline == 1)
			{
				...
				if (cOM_ACNT_GAME_STATE == COM_ACNT_GAME_STATE.COM_ACNT_GAME_STATE_IDLE)
				{
					...
					{
						component.text = Singleton<CInviteSystem>.instance.GetInviteStateStr(stLbsUserInfo.stUin.ullUid, false);
					}
		...
		public static void UpdateGuildMemberListElement(GameObject element, GuildMemInfo guildMember, bool isGuildMatchInvite)
		{
			...
			if (CGuildHelper.IsMemberOnline(guildMember))
			{
				if (guildMember.GameState == COM_ACNT_GAME_STATE.COM_ACNT_GAME_STATE_IDLE)
				{
					...
					if (flag)
					{
						component2.text = Singleton<CTextManager>.instance.GetText("Common_NotDisturb");
						obj.CustomSetActive(true);
					}
					else
					{
						component2.text = Singleton<CInviteSystem>.instance.GetInviteStateStr(guildMember.stBriefInfo.uulUid, isGuildMatchInvite);
					}
				...
				else if (guildMember.GameState == COM_ACNT_GAME_STATE.COM_ACNT_GAME_STATE_SINGLEGAME || guildMember.GameState == COM_ACNT_GAME_STATE.COM_ACNT_GAME_STATE_MULTIGAME || guildMember.GameState == COM_ACNT_GAME_STATE.COM_ACNT_GAME_STATE_WAITMULTIGAME)
				{
					if (guildMember.dwGameStartTime > 0u)
					{
						...
						if (component5 != null)
						{
							component5.text = string.Format(Singleton<CTextManager>.instance.GetText("Common_Gaming_Time"), CInviteView.GetStartMinute(guildMember.dwGameStartTime));
						}
						Singleton<CInviteSystem>.instance.CheckInviteListGameTimer();
					}
					else
					{
						...
						if (component2 != null)
						{
							component2.text = string.Format("<color=#ffff00>{0}</color>", Singleton<CTextManager>.instance.GetText("Common_Gaming_NoTime"));
						}
					}
				}
				else if (guildMember.GameState == COM_ACNT_GAME_STATE.COM_ACNT_GAME_STATE_TEAM)
				{
					component2.text = string.Format("<color=#ffff00>{0}</color>", Singleton<CTextManager>.instance.GetText("Common_Teaming"));
				}
			...
			{
				component2.text = string.Format(Singleton<CTextManager>.instance.GetText("Common_Offline"), new object[0]);
			...
			if (isGuildMatchInvite)
			{
				...
				if (transform3 != null)
				{
					...
					if (Singleton<CGuildMatchSystem>.GetInstance().IsInAnyTeam(guildMember.stBriefInfo.uulUid))
					{
						...
						if (component7 != null)
						{
							component7.text = Singleton<CTextManager>.GetInstance().GetText("GuildMatch_SignUp_In_Team");
						}
					}
					else
					{
						...
						if (component7 != null)
						{
							...
							{
								component7.text = "<color=#e49316>" + Singleton<CTextManager>.GetInstance().GetText("GuildMatch_SignUp_Invite_State_Wait_For_Response") + "</color>";
							}
							else
							{
