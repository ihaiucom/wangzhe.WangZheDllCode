...
namespace Assets.Scripts.GameSystem
{
	public class CBattleStatCompetitionSystem : Singleton<CBattleStatCompetitionSystem>
	{
		...
		public void OpenForm()
		{
			...
			this.startInput.text = "0000";
			this.endInput.text = "0000";
		...
		{
			if (this.m_heroListCamp1.Count != 0)
			{
				return;
			}
		...
		private void ResetIRView()
		{
			...
			if (gameObject)
			{
				this.imgMapBg.sprite = gameObject.GetComponent<Sprite3D>().GenerateSprite();
			}
			this.m_IRListLeft.SetElementAmount(this.m_heroListCamp1.Count);
			this.m_IRListRight.SetElementAmount(this.m_heroListCamp2.Count);
			for (int i = 0; i < this.m_heroListCamp1.Count; i++)
			{
				this.SetIRHeroInfo(this.m_IRListLeft.GetElemenet(i), this.m_heroListCamp1[i]);
			}
			for (int j = 0; j < this.m_heroListCamp2.Count; j++)
			{
				this.SetIRHeroInfo(this.m_IRListRight.GetElemenet(j), this.m_heroListCamp2[j]);
			}
		}
		...
		private void ResetTeamFightView()
		{
			...
			for (int i = 0; i < 2; i++)
			{
				...
				elemenet.transform.FindChild("Text").GetComponent<Text>().text = Singleton<CTextManager>.GetInstance().GetText(this.K_TEAMFIGHT_TITLES[i]);
			}
		...
		private void ResetTeamFightDataByData()
		{
			...
			if (flag)
			{
				int[] result = new int[this.m_heroListCamp1.Count];
				int[] result2 = new int[this.m_heroListCamp2.Count];
				int totalResult = 0;
				int totalResult2 = 0;
				...
				widget.transform.Find("LeftValue").GetComponent<Text>().text = totalResult.ToString();
				widget.transform.Find("RightValue").GetComponent<Text>().text = totalResult2.ToString();
				widget.transform.Find("Title").GetComponent<Text>().text = Singleton<CTextManager>.GetInstance().GetText(this.K_TEAMFIGHT_TITLES[teamFightSelectType]);
		...
		{
			list.SetElementAmount(kdaContaienr.Count);
			...
			for (int i = 0; i < list.GetElementAmount(); i++)
			{
				...
				this.SetHead(elemenet, kdaContaienr[num]);
				elemenet.transform.FindChild("Num").GetComponent<Text>().text = result[num].ToString();
				...
				component.fillAmount = num2;
				elemenet.transform.FindChild("Bar/FillText").GetComponent<Text>().text = (int)(num2 * 100f) + "%";
			}
		}
		...
		{
			for (int i = 0; i < kdaContaienr.Count; i++)
			{
				uint num = 0u;
				uint num2 = 0u;
				uint num3 = 4294967295u;
				foreach (uint current in dataContainer[i].Keys)
				{
				...
				else
				{
					if (showType != 0)
					{
						if (showType == 1)
						{
							resultContainer[i] = dataContainer[i][num2].m_damageBear - dataContainer[i][num].m_damageBear;
					...
					{
						resultContainer[i] = dataContainer[i][num2].m_hurtToOther - dataContainer[i][num].m_hurtToOther;
						if (resultContainer[i] < 0)
						{
							resultContainer[i] = 0;
							Debug.LogError("team fight result " + i + " is not right!");
		...
		private void SetName(CUIListElementScript elemt, HeroKDA p)
		{
			...
			component.text = CHeroInfo.GetHeroName((uint)p.HeroId);
		}
		...
		public void StoreIRViewData()
		{
			...
			int num = 0;
			foreach (PoolObjHandle<ActorRoot> current in heroActors)
			{
		...
		{
			int num = Convert.ToInt32(this.startInput.text);
			int num2 = Convert.ToInt32(this.endInput.text);
			int num3 = num / 100;
			int num4 = num % 100;
			int num5 = num2 / 100;
			int num6 = num2 % 100;
			...
			this.startTimeText.text = num3 + ":" + num4;
			this.endTimeText.text = num5 + ":" + num6;
			this.ResetTeamFightDataByData();
		}

		private void OnLeftHeroSelect(CUIEvent evt)
		{
			uint objID = this.m_heroListCamp1[this.m_IRListLeft.GetSelectedIndex()].actorHero._handleObj.ObjID;
		...
		{
			uint objID = this.m_heroListCamp2[this.m_IRListRight.GetSelectedIndex()].actorHero._handleObj.ObjID;
			int heatmapStorageIndex = this.GetHeatmapStorageIndex(objID);
			if (heatmapStorageIndex >= 0)
			{
				this.m_curIRViewIdx = heatmapStorageIndex;
		...
		private void SetFightDataSingle(HeroKDA kda, List<HeroKDA> kdaContainer, uint curTime, List<Dictionary<uint, TeamFightData>> fightDataContainer)
		{
			...
			if (kdaIdx != -1)
			{
				...
				uint num = 0u;
				foreach (uint current in fightDataContainer[kdaIdx].Keys)
				{
					if (current > num)
					{
						num = current;
				...
				if (fightDataContainer[kdaIdx].ContainsKey(num) && fightDataContainer[kdaIdx][num].m_hurtToOther == kda.hurtToHero && fightDataContainer[kdaIdx][num].m_damageBear == kda.hurtTakenByEnemy)
				{
					return;
				}
				if (!fightDataContainer[kdaIdx].TryGetValue(curTime, out teamFightData))
				{
					teamFightData = new TeamFightData();
					fightDataContainer[kdaIdx][curTime] = teamFightData;
				}
