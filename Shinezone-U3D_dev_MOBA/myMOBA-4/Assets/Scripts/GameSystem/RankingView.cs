...
namespace Assets.Scripts.GameSystem
{
	public class RankingView
	{
		...
		public static void UpdateOneGodElement(GameObject objElement, int viewIndex, CSDT_RANKING_LIST_SUCC curRankingList)
		{
			...
			RankingView.SetGameObjChildText(objElement, "PlayerLv", string.Format("Lv.{0}", Math.Max(1u, val)));
			...
			float num3 = CPlayerProfile.Divide(cSDT_RANKING_LIST_ITEM_INFO.stExtraInfo.stDetailInfo.stMasterHero.dwWinCnt * 100u, cSDT_RANKING_LIST_ITEM_INFO.stExtraInfo.stDetailInfo.stMasterHero.dwGameCnt);
			...
			{
				MonoSingleton<NobeSys>.GetInstance().SetNobeIcon(component.VipIcon.GetComponent<Image>(), (int)level, false, masterRoleInfo.playerUllUID == num, privacyBits);
			}
		...
		public static void UpdateArenaSelfInfo()
		{
			...
			RankingView.SetGameObjChildText(widget, "PlayerLv", string.Format("Lv.{0}", pvpLevel.ToString(CultureInfo.InvariantCulture)));
			...
			else if (num > 0)
			{
				...
				RankingView.SetGameObjChildText(widget, "ChangeNum", num.ToString(CultureInfo.InvariantCulture));
			}
			else if (num < 0)
			{
				gameObject2.CustomSetActive(true);
		...
		public static void UpdateRankGodTitle(ResHeroCfgInfo heroCfgInfo)
		{
			...
			Utility.GetComponetInChild<Text>(gameObject, "PlayerName").text = Singleton<CTextManager>.instance.GetText("RankGodHeroName", new string[]
			{
		...
		public static void OnRankGodDetailTab(int tabIndex, COMDT_RANKING_LIST_ITEM_EXTRA_MASTER_HERO masterHeroInfo, uint heroId)
		{
			...
			else if (tabIndex == 1)
			{
				...
				for (int m = 0; m < listView.Count; m++)
				{
					...
					componetInChild4.text = listView[m].m_name;
					Text componetInChild5 = Utility.GetComponetInChild<Text>(gameObject4, "SymbolDesc");
					componetInChild5.text = CSymbolSystem.GetSymbolAttString(listView[m], true).TrimEnd(new char[]
					{
						'\n'
					});
					Text componetInChild6 = Utility.GetComponetInChild<Text>(gameObject4, "lblIconCount");
					componetInChild6.text = string.Format("x{0}", listView[m].m_stackCount);
				...
				Utility.GetComponetInChild<Text>(gameObject2, "heroSymbolText").text = Singleton<CTextManager>.instance.GetText("RankGodHeroSymbolDesc", new string[]
				{
					text,
					szName
				});
		...
		public static void OnRankGodDetailEquipClick(CEquipInfo equipInfo, string playerName, string heroName)
		{
			...
			componetInChild2.text = ((equipInfo != null) ? equipInfo.m_equipName : string.Empty);
			componetInChild3.text = ((equipInfo != null) ? equipInfo.m_equipPropertyDesc : string.Empty);
			componetInChild.text = Singleton<CTextManager>.instance.GetText("RankGodHeroEquipDesc", new string[]
			{
				playerName,
				heroName
			});
		...
		public static void UpdateSymbolItem(CSymbolItem symbol, GameObject element, CUIFormScript form)
		{
			...
			component2.text = string.Format("x{0}", symbol.m_stackCount.ToString());
			component3.text = symbol.m_SymbolData.szName;
			component4.text = CSymbolSystem.GetSymbolAttString(symbol.m_baseID, true);
		}
