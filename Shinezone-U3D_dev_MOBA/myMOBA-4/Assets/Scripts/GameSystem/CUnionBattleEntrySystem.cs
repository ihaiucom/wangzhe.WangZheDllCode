...
namespace Assets.Scripts.GameSystem
{
	...
	public class CUnionBattleEntrySystem : Singleton<CUnionBattleEntrySystem>
	{
		...
		private void initSecondFormWidget()
		{
			...
			uint.TryParse(Singleton<CTextManager>.instance.GetText("MapID_Union_1"), out array[0]);
			uint.TryParse(Singleton<CTextManager>.instance.GetText("MapID_Union_2"), out array[1]);
			...
			widget.transform.FindChild("Desc/MapNameTxt").GetComponent<Text>().text = CUnionBattleEntrySystem.GetUnionBattleMapInfoByMapID(widget.GetComponent<CUIMiniEventScript>().m_onClickEventParams.tagUInt).stLevelCommonInfo.szName;
			widget2.transform.FindChild("Desc/MapNameTxt").GetComponent<Text>().text = CUnionBattleEntrySystem.GetUnionBattleMapInfoByMapID(widget2.GetComponent<CUIMiniEventScript>().m_onClickEventParams.tagUInt).stLevelCommonInfo.szName;
			if (CSysDynamicBlock.bLobbyEntryBlocked)
			{
				if (widget3)
				{
		...
		private void initConfirmFormWidget()
		{
			...
			component2.text = string.Format(component2.text, this.m_selectStateInfo.bMatchCnt, this.maxMatchRoundCnt);
			...
			component3.text = string.Format("x{0}", useableStackCount);
		...
		private void initThirdFormWidget()
		{
			...
			if (bLossCnt == 2 && this.m_selectStateInfo.bState != 2)
			{
				...
				component.text = Singleton<CTextManager>.GetInstance().GetText("Union_Battle_Tips20");
			}
			...
			{
				Text component2 = form.GetWidget(1).GetComponent<Text>();
				component2.text = resRewardMatchTimeInfo.szTimeTips;
			}
			CRoleInfo masterRoleInfo = Singleton<CRoleInfoManager>.instance.GetMasterRoleInfo();
			if (masterRoleInfo == null)
			{
			...
			if (this.m_selectMapRes != null)
			{
				...
				CUICommonSystem.SetItemCell(form, widget, itemUseable, true, false, false, false);
				component3.text = "x" + useableStackCount.ToString();
				...
				if (flag)
				{
					...
					{
						component4.text = Singleton<CTextManager>.GetInstance().GetText("Union_Battle_Tips18");
					}
					else
					{
						component4.text = Singleton<CTextManager>.GetInstance().GetText("Union_Battle_Tips19");
					}
				}
			}
			if (CSysDynamicBlock.bLobbyEntryBlocked)
		...
		private void refreshOneRewardElement(CUIFormScript form, GameObject element, int index)
		{
			...
			component.text = Singleton<CTextManager>.GetInstance().GetText("Union_Battle_Tips16", new string[]
			{
				key.ToString()
			});
			...
			for (int j = 0; j < count; j++)
			{
				...
				{
					component3.text = string.Format("{0}~{1}", resDT_RandomRewardInfo2.dwLowCnt, resDT_RandomRewardInfo2.dwHighCnt);
				}
				if (j + 1 >= count)
				{
			...
			if (transform2 != null)
			{
				...
				else
				{
					...
					else
					{
						...
						gameObject.GetComponent<Text>().text = string.Format(text, num3);
					}
				}
			}
		}
