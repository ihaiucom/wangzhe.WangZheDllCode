...
namespace Assets.Scripts.GameSystem
{
	internal class CSettlementView
	{
		...
		private static void SetExpInfo(GameObject root, CUIFormScript formScript)
		{
			...
			if (masterRoleInfo != null)
			{
				...
				if (dataByKey != null)
				{
					...
					component.text = string.Format("Lv.{0}", dataByKey.bLevel.ToString());
					...
					if (curLvelContext.IsGameTypeLadder())
					{
						...
						if (rankInfo != null)
						{
							...
							component8.text = StringHelper.UTF8BytesToString(ref CLadderSystem.GetGradeDataByShowGrade((int)rankInfo.bNowShowGrade).szGradeDesc);
							if ((int)CLadderSystem.GetGradeDataByShowGrade((int)rankInfo.bNowShowGrade).bLogicGrade == CLadderSystem.MAX_RANK_LEVEL)
							{
								...
								component9.text = string.Format("X{0}", rankInfo.dwNowScore);
							}
							else
							{
					...
					if (multiDetail != null)
					{
						...
						if (multiple2 > 0)
						{
							...
							string[] array2 = new string[multipleInfo2 + 2u];
							string text3 = multiple2.ToString();
							array2[0] = Singleton<CTextManager>.instance.GetText("Pvp_settle_Common_Tips_7", new string[]
							{
								text3
							});
							if (array3 != null)
							{
								...
								while ((long)num3 < (long)((ulong)multipleInfo2))
								{
									...
									byte bOperator = array3[num3].bOperator;
									if (bOperator != 0)
									{
										if (bOperator != 1)
										{
									...
									{
									case 1:
										array2[num3 + 1] = Singleton<CTextManager>.instance.GetText("Pvp_settle_Common_Tips_6", new string[]
										{
											text4
										});
										break;
									case 2:
										if (masterRoleInfo.HasVip(16))
										{
											array2[num3 + 1] = Singleton<CTextManager>.instance.GetText("Pvp_settle_Common_Tips_9", new string[]
											{
												text4
											});
										}
										...
										{
											array2[num3 + 1] = Singleton<CTextManager>.instance.GetText("Pvp_settle_Common_Tips_3", new string[]
											{
												text4
											});
										}
										break;
									case 3:
										array2[num3 + 1] = string.Format(Singleton<CTextManager>.instance.GetText("Pvp_settle_Common_Tips_10"), text4, masterRoleInfo.GetCoinWinCount(), Math.Ceiling((double)((float)masterRoleInfo.GetCoinExpireHours() / 24f)));
										break;
									case 4:
										array2[num3 + 1] = Singleton<CTextManager>.instance.GetText("Pvp_settle_Common_Tips_2", new string[]
										{
											masterRoleInfo.dailyPvpCnt.ToString(),
											text4
										});
										break;
									case 5:
										array2[num3 + 1] = Singleton<CTextManager>.instance.GetText("Pvp_settle_Common_Tips_13", new string[]
										{
											text4
										});
										break;
									case 6:
										array2[num3 + 1] = Singleton<CTextManager>.instance.GetText("Pvp_settle_Common_Tips_15", new string[]
										{
											text4
										});
										break;
									case 7:
										array2[num3 + 1] = Singleton<CTextManager>.instance.GetText("Daily_Quest_FirstVictoryName", new string[]
										{
											text4
										});
										break;
									case 8:
										array2[num3 + 1] = Singleton<CTextManager>.instance.GetText("Pvp_settle_Common_Tips_16", new string[]
										{
											text4
										});
										break;
									case 9:
										array2[num3 + 1] = Singleton<CTextManager>.instance.GetText("Pvp_settle_Common_Tips_14", new string[]
										{
											text4
										});
										break;
									case 10:
										array2[num3 + 1] = Singleton<CTextManager>.instance.GetText("Pvp_settle_Common_Tips_21", new string[]
										{
											text4
										});
										break;
									case 11:
										array2[num3 + 1] = Singleton<CTextManager>.instance.GetText("Pvp_settle_Common_Tips_17", new string[]
										{
											text4
										});
										break;
									case 12:
										array2[num3 + 1] = Singleton<CTextManager>.instance.GetText("Pvp_settle_Common_Tips_18", new string[]
										{
											text4
										});
										break;
									case 13:
										array2[num3 + 1] = Singleton<CTextManager>.instance.GetText("Pvp_settle_Common_Tips_20", new string[]
										{
											text4
										});
										break;
									case 14:
										array2[num3 + 1] = Singleton<CTextManager>.instance.GetText("Pvp_settle_Common_Tips_19", new string[]
										{
											text4
										});
										break;
							...
							gameObject5.GetComponentInChildren<Text>().text = string.Format("+{0}", text3);
					...
					component4.text = masterRoleInfo.Name;
					...
					if (acntInfo != null)
					{
						...
						float num6 = Mathf.Max(0f, ((num4 >= 0) ? acntInfo.dwPvpSettleExp : acntInfo.dwPvpExp) / dataByKey.dwNeedExp);
						root.transform.FindChild("PanelA/Award/Panel_PlayerExp/PvpExpNode/AddPvpExpTxt").GetComponent<Text>().text = ((acntInfo.dwPvpSettleExp <= 0u) ? string.Empty : string.Format("+{0}", acntInfo.dwPvpSettleExp).ToString());
						if (acntInfo.dwPvpSettleExp == 0u)
						{
						...
						component3.text = ((acntInfo.bExpDailyLimit <= 0) ? string.Empty : Singleton<CTextManager>.GetInstance().GetText("GetExp_Limit"));
						component2.text = string.Format("{0}/{1}", acntInfo.dwPvpExp.ToString(), dataByKey.dwNeedExp.ToString());
					}
				}
			}
		}
		...
		public static void DoCoinTweenEnd()
		{
			...
			{
				CSettlementView._coinTweenText.text = string.Format("+{0}", CSettlementView._coinTo.ToString("N0"));
				COMDT_ACNT_INFO acntInfo = Singleton<BattleStatistic>.GetInstance().acntInfo;
		...
		private static void SetHeroStat_Share(CUIFormScript formScript, GameObject item, HeroKDA kda, bool bSelf, bool bMvp, bool bWin)
		{
			...
			componetInChild.text = string.Format("Lv.{0}", kda.SoulLevel.ToString());
			...
			string arg3 = (kda.numAssist >= 10) ? kda.numAssist.ToString() : string.Format(" {0}", kda.numAssist.ToString());
			item.transform.Find("Txt_KDA").gameObject.GetComponent<Text>().text = string.Format("{0} / {1} / {2}", arg, arg2, arg3);
