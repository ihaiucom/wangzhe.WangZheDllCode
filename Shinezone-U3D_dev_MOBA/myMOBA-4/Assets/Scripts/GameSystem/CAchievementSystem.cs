	...
	public class CAchievementSystem : Singleton<CAchievementSystem>
	{
		...
		public void ShowTrophyDetail(CAchieveInfo2 achieveInfo, uint achievementID)
		{
			...
			if (cUIFormScript != null)
			{
				...
				if (component2 != null)
				{
					component2.text = this.m_CurAchieveItem.GetAchievementTips();
				}
		...
		{
			int srcWidgetIndexInBelongedList = uiEvent.m_srcWidgetIndexInBelongedList;
			int num = (this.m_TrophyRewardInfoWithRewardList != null) ? this.m_TrophyRewardInfoWithRewardList.Count : 0;
		...
		private void InitTypeMenu(CUIFormScript form)
		{
			...
			for (int i = 0; i < num; i++)
			{
				...
				if (widget != null)
				{
					...
					if (component2 != null)
					{
						component2.text = Singleton<CTextManager>.GetInstance().GetText(this.m_TypeMenuNameKeys[i]);
					}
				}
			}
