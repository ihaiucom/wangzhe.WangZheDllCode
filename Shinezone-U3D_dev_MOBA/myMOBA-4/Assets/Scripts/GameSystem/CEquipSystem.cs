...
namespace Assets.Scripts.GameSystem
{
	...
	public class CEquipSystem : Singleton<CEquipSystem>
	{
		...
		public static CEquipInfo GetEquipInfo(ushort equipID)
		{
			...
			if (CEquipSystem.s_equipInfoDictionary.TryGetValue(equipID, out result))
			{
				return result;
			}
			return null;
		...
		private void InitializeBackEquipListForEachEquip()
		{
			...
			while (enumerator.MoveNext())
			{
				...
				CEquipInfo value = current.Value;
				if (value.m_resEquipInBattle != null && value.m_resEquipInBattle.bInvalid == 0 && value.m_resEquipInBattle.bIsAttachEquip == 0)
				{
					for (int i = 0; i < value.m_resEquipInBattle.PreEquipID.Length; i++)
					{
						if (value.m_resEquipInBattle.PreEquipID[i] > 0)
						{
							...
							if (CEquipSystem.s_equipInfoDictionary.TryGetValue(value.m_resEquipInBattle.PreEquipID[i], out cEquipInfo) && cEquipInfo.m_resEquipInBattle.bInvalid == 0 && cEquipInfo.m_resEquipInBattle.bIsAttachEquip == 0)
							{
		...
		private void DefaultRecommendEquipsInVisitor(ResRecommendEquipInBattle resRecommendEquipInBattle)
		{
			...
			{
				ResRecommendEquipInBattle value = null;
				for (int i = 0; i < 6; i++)
				{
					CEquipInfo cEquipInfo = null;
					if (resRecommendEquipInBattle.RecommendEquipID[i] == 0 || (CEquipSystem.s_equipInfoDictionary.TryGetValue(resRecommendEquipInBattle.RecommendEquipID[i], out cEquipInfo) && cEquipInfo.m_resEquipInBattle.bInvalid <= 0 && cEquipInfo.m_resEquipInBattle.bIsAttachEquip == 0))
					{
						value = resRecommendEquipInBattle;
					}
					else
					{
				...
				this.m_defaultRecommendEquipsDictionary.Add(doubleKey, value);
			}
			else
			{
				Debug.LogError(string.Concat(new object[]
		...
		private void InitializeEditCustomRecommendEquips(uint heroID, ref bool useCustomRecommendEquips)
		{
			...
			if (masterRoleInfo != null && masterRoleInfo.m_rcmdEquipInfo != null)
			{
				...
				for (int j = 0; j < 6; j++)
				{
					...
					{
						CEquipInfo cEquipInfo = null;
						if (CEquipSystem.s_equipInfoDictionary.TryGetValue(array[j], out cEquipInfo) && cEquipInfo.m_resEquipInBattle.bInvalid <= 0 && cEquipInfo.m_resEquipInBattle.bIsAttachEquip <= 0)
						{
		...
		public ResRecommendEquipInBattle GetDefaultRecommendEquipInfo(uint heroID, uint combinationID)
		{
			...
			if (this.m_defaultRecommendEquipsDictionary.TryGetValue(doubleKey, out result))
			{
				return result;
			}
			return null;
		...
		private void OnBackEquipListSelectChanged(CUIEvent uiEvent)
		{
			...
			CEquipInfo equipInfo = CEquipSystem.GetEquipInfo(this.m_selectedEquipInfoInEquipTree.m_backEquipIDs[selectedIndex]);
			if (equipInfo == null)
			{
		...
		private void RefreshEquipCustomPanel(bool bRefreshHero, bool bRefreshEquipPlanName)
		{
			...
			if (bRefreshEquipPlanName)
			{
				...
				if (widget != null)
				{
					...
					component.text = masterRoleInfo.m_rcmdEquipInfo.GetRcmdEquipPlanName(this.m_editHeroID, rcmdEquipCurUseId);
				}
			}
		...
		private void RefreshEquipLevelPanel(Transform equipPanel, int level)
		{
			...
			{
				Transform transform = equipPanel.Find(string.Format("equipItem{0}", i));
				this.RefreshEquipItem(transform, list[i]);
		...
		private void OnBackEquipListElementEnable(CUIEvent uiEvent)
		{
			...
			if (srcWidgetIndexInBelongedList < 0 || srcWidgetIndexInBelongedList >= this.m_selectedEquipInfoInEquipTree.m_backEquipIDs.Count)
			{
				return;
			}
			if (uiEvent.m_srcWidget != null)
			{
				CEquipInfo equipInfo = CEquipSystem.GetEquipInfo(this.m_selectedEquipInfoInEquipTree.m_backEquipIDs[srcWidgetIndexInBelongedList]);
				if (equipInfo == null)
				{
		...
		private bool IsHeroCustomEquip(uint heroId)
		{
			...
			if (masterRoleInfo.m_rcmdEquipInfo.m_customRecommendEquipDictionary.TryGetValue(heroId, out stRcmdEquipListInfo))
			{
		...
		private void OnPackUpCustomEquip(CUIEvent uiEvent)
		{
			...
			if (widget != null && equipCustomPanel != null && equipExpandBtn != null)
			{
				...
				RectTransform customContentRectTransform = widget.transform as RectTransform;
				Vector2 to = new Vector2(customContentRectTransform.anchoredPosition.x, customContentRectTransform.anchoredPosition.y - this.m_uiCustomEquipContentHeight);
				LeanTween.value(widget, 
                    delegate(Vector2 pos){customContentRectTransform.anchoredPosition = pos;}, 
                    customContentRectTransform.anchoredPosition, 
                    to, 
                    this.c_moveAnimaTime).setOnComplete(() => {
		...
		private void OnGetCustomEquipRankList(SCPKG_GET_RANKING_LIST_RSP rankListRsp)
		{
			...
			CEquipSystem.stEquipRankInfo value = default(CEquipSystem.stEquipRankInfo);
			value.equipRankItemCnt = (int)stOfSucc.dwItemNum;
			value.rankDetail = stOfSucc.astItemDetail;
			if (this.m_equipRankItemDetail.ContainsKey(this.m_reqRankHeroId))
			{
				this.m_equipRankItemDetail[this.m_reqRankHeroId] = value;
			}
			else
			{
				this.m_equipRankItemDetail.Add(this.m_reqRankHeroId, value);
			}
		...
		private void RefreshGodEquipForm(uint heroId)
		{
			...
			if (this.m_equipRankItemDetail.TryGetValue(heroId, out stEquipRankInfo))
			{
			...
			if (component != null && component.GetElementAmount() == 0)
			{
				...
				for (uint num3 = 1u; num3 < 4u; num3 += 1u)
				{
					...
					if (this.m_defaultRecommendEquipsDictionary.TryGetValue(doubleKey, out array[(int)((UIntPtr)(num3 - 1u))]))
					{
						num2++;
					}
				}
		...
		{
			CEquipSystem.stEquipRankInfo stEquipRankInfo;
			if (!this.m_equipRankItemDetail.TryGetValue(this.m_reqRankHeroId, out stEquipRankInfo))
			{
				return;
			}
			int srcWidgetIndexInBelongedList = uiEvent.m_srcWidgetIndexInBelongedList;
		...
		private void OnIWantJudgeBtnClick(CUIEvent uiEvent)
		{
			...
			CEquipSystem.stEquipRankInfo stEquipRankInfo;
			if (CEquipSystem.m_judgeGodeIndex >= 0 && this.m_equipRankItemDetail.TryGetValue(this.m_reqRankHeroId, out stEquipRankInfo))
			{
		...
		private void OnJudgeSubmitBtnClick(CUIEvent uiEvent)
		{
			...
			CEquipSystem.stEquipRankInfo stEquipRankInfo;
			bool flag = this.m_equipRankItemDetail.TryGetValue(this.m_reqRankHeroId, out stEquipRankInfo);
			if (flag)
			{
				CSDT_RANKING_LIST_ITEM_INFO cSDT_RANKING_LIST_ITEM_INFO = stEquipRankInfo.rankDetail[CEquipSystem.m_judgeGodeIndex];
				uint[] equipID = cSDT_RANKING_LIST_ITEM_INFO.stExtraInfo.stDetailInfo.stCustomEquip.EquipID;
			...
			if (component != null)
			{
				...
				for (int i = 0; i < 3; i++)
				{
					...
					if (elemenet != null)
					{
						...
						component2.text = rcmdEquipList.ListItem[i].Name;
						...
						if (!rcmdEquipList.ListItem[i].bSelfDefine)
						{
							...
							if (defaultRecommendEquipInfo != null)
							{
								if (flag)
								{
									component3.GetComponent<Text>().text = string.Format("<color=#fdb900>{0} {1}</color>", defaultRecommendEquipInfo.szCombinationName, defaultRecommendEquipInfo.szCombinationDesc);
								}
								else
								{
									component3.GetComponent<Text>().text = string.Format("<color=#9fadd3>{0} {1}</color>", defaultRecommendEquipInfo.szCombinationName, defaultRecommendEquipInfo.szCombinationDesc);
								}
							}
						}
		...
		private void OnChangeEquipPlanName(stUIEventParams eventParams)
		{
			...
			if (form != null)
			{
				...
				if (transform != null && masterRoleInfo != null)
				{
					...
					transform.GetComponent<InputField>().text = rcmdEquipPlanName;
				}
			}
		}
