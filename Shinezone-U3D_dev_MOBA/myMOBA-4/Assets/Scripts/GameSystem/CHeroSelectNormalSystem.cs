...
namespace Assets.Scripts.GameSystem
{
	public class CHeroSelectNormalSystem : Singleton<CHeroSelectNormalSystem>
	{
		...
		public void HeroSelect_SelectHero(IHeroData selectHeroData)
		{
			...
			if (selectHeroData == null || Singleton<CRoleInfoManager>.instance.GetMasterRoleInfo() == null || Singleton<CHeroSelectBaseSystem>.instance.m_selectHeroIDList.Count == 0)
			{
				return;
			}
			...
			else if ((int)Singleton<CHeroSelectBaseSystem>.instance.m_selectHeroCount < Singleton<CHeroSelectBaseSystem>.instance.m_selectHeroIDList.Count)
			{
				Singleton<CHeroSelectBaseSystem>.instance.m_selectHeroIDList[(int)Singleton<CHeroSelectBaseSystem>.instance.m_selectHeroCount] = selectHeroData.cfgID;
				CHeroSelectBaseSystem expr_1DE = Singleton<CHeroSelectBaseSystem>.instance;
		...
		private void HeroSelect_Del_Hero(CUIEvent uiEvent)
		{
			...
			if (Singleton<CHeroSelectBaseSystem>.instance.m_selectHeroIDList[srcWidgetIndexInBelongedList] != 0u)
			{
				...
				CHeroSelectBaseSystem expr_5E = Singleton<CHeroSelectBaseSystem>.instance;
		...
		private void HeroSelect_ConfirmHeroSelect(CUIEvent uiEvent)
		{
			...
			else if (Singleton<CHeroSelectBaseSystem>.instance.gameType == enSelectGameType.enArenaDefTeamConfig)
			{
				...
				{
					par.tagList.Add(Singleton<CHeroSelectBaseSystem>.instance.m_selectHeroIDList[i]);
				}
		...
		public void RefreshSkinPanel(CUIEvent uiEvent = null)
		{
			...
			for (int j = 0; j < array.Length; j++)
			{
				...
				for (int k = 0; k < listView.Count; k++)
				{
					...
					ResHeroSkin resHeroSkin = listView[k];
		...
		public void RefreshRcmdEquipPlanPanel()
		{
			...
			if (Singleton<CHeroSelectBaseSystem>.instance.IsMobaMode() && GameDataMgr.heroDatabin.GetDataByKey(this.m_showHeroID) != null)
			{
				...
				component.text = rcmdEquipPlanName;
				transform.gameObject.CustomSetActive(true);
			}
			else
			{
		...
		public void RefreshAddedSkillItem(CUIFormScript form, uint addedSkillID, bool bForceRefresh = false)
		{
			...
			if (dataByKey != null)
			{
				...
				eventParams.skillTipParam.skillEnergyCost = ((dataByKey.iEnergyCost != 0 && dataByKey.bEnergyCostType != 6) ? Singleton<CTextManager>.instance.GetText(EnergyCommon.GetEnergyShowText((uint)dataByKey.bEnergyCostType, EnergyShowType.CostValue), new string[]
				{
		...
		public void RadomHeroBySelfWithSingleMode()
		{
			...
			while (!flag)
			{
				IHeroData heroData = Singleton<CHeroSelectBaseSystem>.instance.m_canUseHeroList[UnityEngine.Random.Range(0, Singleton<CHeroSelectBaseSystem>.instance.m_canUseHeroList.Count)];
				if ((Singleton<CHeroSelectBaseSystem>.instance.m_selectHeroIDList[0] != heroData.cfgID && !list.Contains(heroData.cfgID)) || Singleton<CHeroSelectBaseSystem>.instance.m_canUseHeroList.Count == 1)
				{
			...
			for (int i = 0; i < teamHeroList.Count; i++)
			{
				...
				if (Singleton<CHeroSelectBaseSystem>.instance.IsMobaMode())
				{
					...
					if (memberInfo.dwObjId == Singleton<CHeroSelectBaseSystem>.instance.roomInfo.selfObjID && memberInfo.RoomMemberType != 2u)
					{
						...
						component3.text = Singleton<CTextManager>.instance.GetText("Pvp_PlayerName", new string[]
						{
		...
		private void LeftHeroItemEnable(CUIEvent uiEvent)
		{
			...
			if (!Singleton<CHeroSelectBaseSystem>.instance.m_isSelectConfirm)
			{
				...
				if (Singleton<CHeroSelectBaseSystem>.instance.m_selectHeroCount > 0)
				{
					if (heroData.cfgID == Singleton<CHeroSelectBaseSystem>.instance.m_selectHeroIDList[(int)(Singleton<CHeroSelectBaseSystem>.instance.m_selectHeroCount - 1)])
					{
		...
		private void RefresHeroInfo_TeamTipsInfo(CUIFormScript form)
		{
			...
			int.TryParse(Singleton<CTextManager>.instance.GetText("HeroSelect_TeamTips_1"), out array2[0]);
			int.TryParse(Singleton<CTextManager>.instance.GetText("HeroSelect_TeamTips_2"), out array2[1]);
			int.TryParse(Singleton<CTextManager>.instance.GetText("HeroSelect_TeamTips_3"), out array2[2]);
			int.TryParse(Singleton<CTextManager>.instance.GetText("HeroSelect_TeamTips_4"), out array2[3]);
			int.TryParse(Singleton<CTextManager>.instance.GetText("HeroSelect_TeamTips_5"), out array2[4]);
			int.TryParse(Singleton<CTextManager>.instance.GetText("HeroSelect_TeamTips_6"), out array2[5]);
			if (array[4] >= array2[0])
			{
				list.Add(0);
			...
			if (list2.Count > 0)
			{
				for (int j = 0; j < list2.Count; j++)
				{
					if (j < 3)
					{
						CUICommonSystem.SetTextContent(array3[j].Find("Text"), list2[j]);
