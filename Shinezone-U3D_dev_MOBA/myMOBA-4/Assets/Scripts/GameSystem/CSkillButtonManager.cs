...
namespace Assets.Scripts.GameSystem
{
	public class CSkillButtonManager
	{
		...
		public void InitializeCampHeroInfo(CUIFormScript battleFormScript, bool bIsEnemyCamp = false)
		{
			...
			{
				COM_PLAYERCAMP camp = (!bIsEnemyCamp) ? player.PlayerCamp : ((player.PlayerCamp != COM_PLAYERCAMP.COM_PLAYERCAMP_1) ? COM_PLAYERCAMP.COM_PLAYERCAMP_1 : COM_PLAYERCAMP.COM_PLAYERCAMP_2);
				List<Player> allCampPlayers = Singleton<GamePlayerCenter>.GetInstance().GetAllCampPlayers(camp);
				if (allCampPlayers != null)
				{
					if (bIsEnemyCamp)
					{
						this.m_EnemyHeroInfos = new stCampHeroInfo[allCampPlayers.Count];
					}
					else
					{
					...
					{
						if (allCampPlayers[i] != player)
						{
							if (bIsEnemyCamp)
							{
								this.m_EnemyHeroInfos[num].m_headIconPath = CUIUtility.s_Sprite_Dynamic_BustCircle_Dir + CSkinInfo.GetHeroSkinPic((uint)allCampPlayers[i].Captain.handle.TheActorMeta.ConfigId, 0u);
								this.m_EnemyHeroInfos[num].m_objectID = allCampPlayers[i].Captain.handle.ObjID;
							}
							else
							{
								this.m_campHeroInfos[num].m_headIconPath = CUIUtility.s_Sprite_Dynamic_BustCircle_Dir + CSkinInfo.GetHeroSkinPic((uint)allCampPlayers[i].Captain.handle.TheActorMeta.ConfigId, 0u);
								this.m_campHeroInfos[num].m_objectID = allCampPlayers[i].Captain.handle.ObjID;
							}
							num++;
						}
					}
					...
					for (int k = 0; k < component.m_widgets.Length; k++)
					{
						...
						if (gameObject != null)
						{
							...
							if (bIsEnemyCamp)
							{
								...
								CSkillButtonManager.GetJoystickHeadAreaFan(ref this.m_EnemyTargetInfos[k].m_headAreaFan, gameObject, (k != 0) ? component.m_widgets[k - 1] : null, (k != component.m_widgets.Length - 1) ? component.m_widgets[k + 1] : null);
							}
							else
							{
		...
		public void ResetSkillTargetJoyStickHeadToCampHero()
		{
			...
			if (gameObject != null)
			{
				...
				DebugHelper.Assert((!flag) ? (CSkillButtonManager.MaxTargetNum >= this.m_campHeroInfos.Length) : (CSkillButtonManager.MaxTargetNum5 >= this.m_EnemyHeroInfos.Length), "目标数大于轮盘支持的最大英雄数!");
				if (flag)
				{
		...
		public void ResetSkillTargetJoyStickHeadToTargets(ListValueView<uint> targetIDs)
		{
			...
			{
				CUIComponent component = gameObject.GetComponent<CUIComponent>();
				int num = (!flag) ? CSkillButtonManager.MaxTargetNum : CSkillButtonManager.MaxTargetNum5;
		...
		public void SkillButtonDown(CUIFormScript formScript, SkillSlotType skillSlotType, Vector2 downScreenPosition)
		{
			...
			if (skillSlotType == SkillSlotType.SLOT_SKILL_0)
			{
				...
				Singleton<CUIParticleSystem>.GetInstance().AddParticle(CUIParticleSystem.s_particleSkillBtnEffect_Path, 0.5f, animationPresent, formScript, null);
			}
			else
			{
				if (skillSlotType != SkillSlotType.SLOT_SKILL_0)
		...
		public void CommonAtkSlide(CUIFormScript battleFormScript, Vector2 screenPosition)
		{
			if (GameSettings.TheCommonAttackType == CommonAttactType.Type2)
			{
				...
				if (this.IsSkillCursorInTargetArea(battleFormScript, ref screenPosition, widget))
				{
					...
					Singleton<CUIParticleSystem>.GetInstance().AddParticle(CUIParticleSystem.s_particleSkillBtnEffect_Path, 0.5f, widget, battleFormScript, null);
				...
				else if (this.IsSkillCursorInTargetArea(battleFormScript, ref screenPosition, widget2))
				{
					...
					Singleton<CUIParticleSystem>.GetInstance().AddParticle(CUIParticleSystem.s_particleSkillBtnEffect_Path, 0.5f, widget2, battleFormScript, null);
			...
			if (gameObject != null)
			{
				...
				if (component != null && component.m_widgets != null)
				{
					...
					while ((long)num < (long)((ulong)this.m_CurTargtNum))
					{
						...
						uint objID = (!flag) ? this.m_targetInfos[num].m_objectID : this.m_EnemyTargetInfos[num].m_objectID;
						PoolObjHandle<ActorRoot> actor = Singleton<GameObjMgr>.GetInstance().GetActor(objID);
		...
		private bool IsEnemyOrganInSearchScope()
		{
			...
			if (this.hostPlayer != null)
			{
				...
				for (int i = 0; i < count; i++)
				{
					ActorRoot handle = organActors[i].handle;
