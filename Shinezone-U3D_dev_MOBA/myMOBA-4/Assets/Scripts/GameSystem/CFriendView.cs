	...
	public class CFriendView
	{
		...
		public class AddFriendView
		{
			...
			public void On_SearchFriend(CUIEvent uiEvent)
			{
				...
				{
					FriendSysNetCore.Send_Serch_Player(this.input.text, enFriendSearchSource.FriendSystem);
				}
				this.Refresh_Friend_Recommand_List_Pos();
			}
		...
		public class Verfication
		{
			...
			public void Update()
			{
				...
				if (text != null && text.Length > CFriendView.Verfication.Verfication_ChatMaxLength)
				{
		...
		public void OpenForm(CUIEvent uiEvent)
		{
			...
			if (Singleton<ApolloHelper>.GetInstance().CurPlatform == ApolloPlatform.QQ)
			{
				...
				{
					componetInChild.text = Singleton<CTextManager>.instance.GetText("Friend_SNS_NTF_Switch_Tips_1");
				}
			}
			else if (componetInChild != null)
			{
				componetInChild.text = Singleton<CTextManager>.instance.GetText("Friend_SNS_NTF_Switch_Tips_2");
			}
		...
		public void Update()
		{
			...
			if (this.lbsRefreshBtn != null && Singleton<CFriendContoller>.instance.startCooldownTimestamp > 0uL)
			{
				...
				if (num2 > 0)
				{
					...
					component.text = string.Format(Singleton<CTextManager>.instance.GetText("LBS_Refresh_CDInfo"), (num2 / 1000).ToString());
				}
				else
				{
		...
		public void SyncGenderToggleState()
		{
			...
			{
				this.nanToggle.isOn = ((Singleton<CFriendContoller>.instance.model.fileter & 1u) != 0u);
			}
			if (this.nvToggle != null)
			{
				this.nvToggle.isOn = ((Singleton<CFriendContoller>.instance.model.fileter & 2u) != 0u);
			}
		}
		...
		public void Refresh_SnsSwitch()
		{
			...
			if (singleListForm)
			{
				...
				else
				{
					...
					widget2.transform.Find("AddToggle").GetComponent<Toggle>().isOn = isOn2;
				}
			}
			if (CSysDynamicBlock.bSocialBlocked)
			{
			...
			if (curTab == CFriendView.Tab.Friend_SNS)
			{
				...
				this.sns_share_switch.GetComponent<Toggle>().isOn = isOn3;
			}
		}
			...
			if (singleListForm)
			{
				...
				if (singleListType != enFriendSingleListType.requestList)
				{
					if (singleListType == enFriendSingleListType.blackList)
					{
						...
						{
							widget.CustomSetActive(blackList.Count == 0);
						}
					}
				}
				else
		...
		public void Refresh_List(CFriendView.Tab type)
		{
			...
			case CFriendView.Tab.Mentor:
			{
				this.ifnoText.text = Singleton<CTextManager>.instance.GetText("Mentor_NoInfoTips");
				...
				if (flag && !flag3)
				{
					...
					widget9.GetComponent<Text>().text = Singleton<CTextManager>.GetInstance().GetText("ApprenticeIntroRequire", new string[]
					{
						dwConfValue.ToString()
					});
					widget10.GetComponent<Text>().text = Singleton<CTextManager>.GetInstance().GetText("MentorIntroRequire", new string[]
					{
		...
		private void _refresh_black_list(CUIListScript listScript, List<CFriendModel.stBlackName> blackList)
		{
			...
			for (int i = 0; i < count; i++)
			{
				...
				{
					FriendShower component = elemenet.GetComponent<FriendShower>();
					CFriendModel.stBlackName stBlackName = blackList[i];
					if (component != null)
					{
