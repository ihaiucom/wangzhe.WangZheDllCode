...
namespace Assets.Scripts.GameSystem
{
	public class UT
	{
		...
		public static string GetTimeString(uint time)
		{
			...
			if (DateTime.Compare(dateTime2, dateTime) > 0)
			{
				...
				if (timeSpan.Days == 0)
				{
					if (timeSpan.Hours == 0)
					{
						result = string.Format(UT.GetText("Friend_Tips_lastTime_min"), timeSpan.Minutes);
					}
					else
					{
						result = string.Format(UT.GetText("Friend_Tips_lastTime_hour_min"), timeSpan.Hours, timeSpan.Minutes);
					}
				}
				else
				{
		...
		public static int CalcDeltaHorus(uint fromT, uint toT)
		{
			...
			if (DateTime.Compare(dateTime2, dateTime) > 0)
			{
				return Math.Max((int)(dateTime2 - dateTime).TotalHours, 1);
			}
			return 0;
		}
        ...
        public static string ErrorCode_String(uint dwResult)
        {
            switch (dwResult)
            {
                ...
                case 0xba:
                    return GetText("CS_ERR_REQUEST_LIST_NUM_EXCEED");
		...
		public static bool DebugBattleMiniMapConfigInfo(SLevelContext data)
		{
			...
			{
				str += string.Format("缩略图路径配置为空,", new object[0]);
				flag = false;
			}
			if (string.IsNullOrEmpty(data.m_bigMapPath))
			{
				str += string.Format("地图路径配置为空,", new object[0]);
				flag = false;
			}
			if (data.m_mapWidth == 0)
			{
				str += string.Format("地图宽度配置为空,", new object[0]);
				flag = false;
			}
			if (data.m_mapHeight == 0)
			{
				str += string.Format("地图高度配置为空,", new object[0]);
				flag = false;
			}
			return flag && flag;
		}
		...
		public static void SetMentorLv(GameObject mentorLvGo, int mentorLv)
		{
			...
			if (transform != null)
			{
				transform.GetComponent<Text>().text = mentorLv.ToString();
			}
		}
		...
		public static bool IsFreeze(uint lastTime)
		{
			...
			DateTime dateTime2 = Utility.ToUtcTime2Local((long)CRoleInfo.GetCurrentUTCTime());
			return DateTime.Compare(dateTime2, dateTime) > 0 && (dateTime2 - dateTime).Days >= Singleton<CFriendContoller>.instance.model.freezeDayCount;
		}
