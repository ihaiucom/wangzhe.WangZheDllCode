		...
		public void OnSkillTipsShow()
		{
			...
			for (int i = 0; i < array.Length; i++)
			{
				...
				if (skill != null)
				{
					...
					if (component2 != null && skill.cfgData.szSkillName.Length > 0)
					{
					...
					component3.text = ((i != 0) ? Singleton<CTextManager>.instance.GetText("Skill_Cool_Down_Tips", new string[]
					{
					...
					if (i < array.Length && i > 0)
					{
						...
						if (skillSlot == null)
						{
							component4.text = ((skill.cfgData.bEnergyCostType != 6) ? Singleton<CTextManager>.instance.GetText(EnergyCommon.GetEnergyShowText((uint)skill.cfgData.bEnergyCostType, EnergyShowType.CostValue), new string[]
							{
					...
					for (int j = 1; j <= 2; j++)
					{
						...
						if (j <= skillEffectType.Length && skillEffectType[j - 1] != 0)
						{
							...
							gameObject2.transform.Find("Text").GetComponent<Text>().text = CSkillData.GetEffectDesc((SkillEffectType)skillEffectType[j - 1]);
						}
						else
						{
					...
					if (component5 != null && skill.cfgData.szSkillDesc.Length > 0)
					{
						component5.text = CUICommonSystem.GetSkillDesc(skill.cfgData.szSkillDesc, actorValue, skillSlot.GetSkillLevel(), captain.handle.ValueComponent.actorSoulLevel, (uint)captain.handle.TheActorMeta.ConfigId);
					}
				}
