...
namespace Assets.Scripts.GameSystem
{
	public class CLevelCfgLogicManager
	{
		public static ResDT_LevelCommonInfo GetPvpMapCommonInfo(byte mapType, uint mapId)
		{
			...
			if (mapType == (byte)RES_BATTLE_MAP_TYPE.RES_BATTLE_MAP_TYPE_VERSUS)
			{
			...
			else if (mapType == (byte)RES_BATTLE_MAP_TYPE.RES_BATTLE_MAP_TYPE_RANK)
			{
			...
			else if (mapType == (byte)RES_BATTLE_MAP_TYPE.RES_BATTLE_MAP_TYPE_ENTERTAINMENT)
			{
			...
			else if (mapType == (byte)RES_BATTLE_MAP_TYPE.RES_BATTLE_MAP_TYPE_REWARDMATCH)
			{
			...
			else if (mapType == (byte)RES_BATTLE_MAP_TYPE.RES_BATTLE_MAP_TYPE_GUILDMATCH)
			{
			...
			else if (mapType == (byte)RES_BATTLE_MAP_TYPE.RES_BATTLE_MAP_TYPE_COUNTERPART)
			{
		...
		public static SLevelContext CreatePvpLevelContext(byte mapType, uint mapID, COM_GAME_TYPE GameType, int difficult)
		{
			...
			if (mapType == (byte)RES_BATTLE_MAP_TYPE.RES_BATTLE_MAP_TYPE_ENTERTAINMENT)
            {
		...
		public static SLevelContext CreatePveLevelContext(SCPKG_STARTSINGLEGAMERSP InMessage)
		{
			...
			if (InMessage.bGameType == (byte)COM_GAME_TYPE.COM_SINGLE_GAME_OF_GUIDE)
			{
			...
			else if (InMessage.bGameType == (byte)COM_GAME_TYPE.COM_SINGLE_GAME_OF_ADVENTURE)
			{
				sLevelContext.InitPveData(pveMapInfo, Singleton<CAdventureSys>.instance.currentDifficulty);
			}
			else if (InMessage.bGameType == (byte)COM_GAME_TYPE.COM_SINGLE_GAME_OF_BURNING)
			{
				sLevelContext.InitPveData(pveMapInfo, 1);
			}
			else if (InMessage.bGameType == (byte)COM_GAME_TYPE.COM_SINGLE_GAME_OF_ARENA)
			{
				sLevelContext.InitPveData(pveMapInfo, 1);
			}
			else if (InMessage.bGameType == (byte)COM_GAME_TYPE.COM_SINGLE_GAME_OF_COMBAT)
			{
			...
			else if (InMessage.bGameType == (byte)COM_GAME_TYPE.COM_SINGLE_GAME_OF_ACTIVITY)
			{
			}
