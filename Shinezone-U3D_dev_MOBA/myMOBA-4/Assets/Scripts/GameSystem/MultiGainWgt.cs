...
namespace Assets.Scripts.GameSystem
{
	public class MultiGainWgt : ActivityWidget
	{
		...
		public MultiGainWgt(GameObject node, ActivityView view) : base(node, view)
		{
			...
			node.GetComponent<RectTransform>().sizeDelta = new Vector2(node.GetComponent<RectTransform>().sizeDelta.x, (this._elementList.Count <= 0) ? 0f : (height * (float)this._elementList.Count + (float)(this._elementList.Count - 1) * 5f));
