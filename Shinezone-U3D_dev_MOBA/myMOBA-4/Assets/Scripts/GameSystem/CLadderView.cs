	...
	internal class CLadderView
	{
		...
		public static void InitLadderEntry(CUIFormScript form, ref COMDT_RANKDETAIL data, bool isQualified)
		{
			...
			else
			{
				...
				component2.text = string.Format("{0}/{1}", num, CLadderSystem.REQ_HERO_NUM);
				component3.text = string.Format(Singleton<CTextManager>.GetInstance().GetText("Ladder_Req_Hero_Num"), CLadderSystem.REQ_HERO_NUM.ToString());
			}
		...
		private static void ShowBraveScorePanel(CUIFormScript form, COMDT_RANKDETAIL data, bool isShow)
		{
			...
			{
				component4.text = Singleton<CTextManager>.GetInstance().GetText("Ladder_Brave_KeepGrade_Txt2");
				component.color = CUIUtility.s_Color_BraveScore_BaojiKedu_On;
			}
			else
			{
				component4.text = Singleton<CTextManager>.GetInstance().GetText("Ladder_Brave_KeepGrade_Txt1", new string[]
				{
			...
			if (data.dwContinuousWin > 0u)
			{
				...
				component5.text = data.dwContinuousWin + Singleton<CTextManager>.GetInstance().GetText("Common_Continues_Win");
			}
			else
			{
		...
		public static void SetGameInfoRecentPanel(CUIFormScript form, COMDT_RANKDETAIL rankDetail, List<COMDT_RANK_CURSEASON_FIGHT_RECORD> dataList)
		{
			...
			if (rankDetail != null && dataList != null && dataList.Count > 0 && rankDetail.dwSeasonIdx == dataList[0].dwSeasonId)
			{
				...
				COMDT_RANK_CURSEASON_FIGHT_RECORD cOMDT_RANK_CURSEASON_FIGHT_RECORD = dataList[0];
				CLadderView.SetWinLose(component, ref cOMDT_RANK_CURSEASON_FIGHT_RECORD);
				component2.text = CLadderView.GetGameTime(ref cOMDT_RANK_CURSEASON_FIGHT_RECORD);
		...
		public static void SetGameInfoSeasonPanel(CUIFormScript form, COMDT_RANKDETAIL rankdetail)
		{
			...
			component.text = rankdetail.dwTotalFightCnt.ToString();
			component2.text = rankdetail.dwTotalWinCnt.ToString();
			component3.text = rankdetail.dwMaxContinuousWinCnt.ToString();
		}
		...
		public static void SetGameInfoSeasonTimePanel(CUIFormScript form, COMDT_RANKDETAIL rankdetail)
		{
			...
			Text expr_0F = component;
			expr_0F.text = expr_0F.text + " " + CLadderView.GetSeasonNameWithBracket((ulong)rankdetail.dwSeasonStartTime);
			Text component2 = form.GetWidget(12).GetComponent<Text>();
			component2.text = CLadderView.GetSeasonText(ref rankdetail);
		}
		...
		public static void InitLadderHistory(CUIFormScript form, List<COMDT_RANK_PASTSEASON_FIGHT_RECORD> dataList)
		{
			...
			else
			{
				...
				for (int i = 0; i < dataList.Count; i++)
				{
					...
					COMDT_RANK_PASTSEASON_FIGHT_RECORD cOMDT_RANK_PASTSEASON_FIGHT_RECORD = dataList[i];
					...
					component2.text = CLadderView.GetSeasonNameWithBracket((ulong)cOMDT_RANK_PASTSEASON_FIGHT_RECORD.dwSeaStartTime) + " " + CLadderView.GetSeasonText(ref cOMDT_RANK_PASTSEASON_FIGHT_RECORD);
					component3.text = CLadderView.GetRankName(ref cOMDT_RANK_PASTSEASON_FIGHT_RECORD);
					List<COMDT_RANK_COMMON_USED_HERO> list = new List<COMDT_RANK_COMMON_USED_HERO>();
					component4.text = CLadderView.GetTopUseHeroNames(ref cOMDT_RANK_PASTSEASON_FIGHT_RECORD, out list);
					component5.text = cOMDT_RANK_PASTSEASON_FIGHT_RECORD.dwTotalFightCnt.ToString();
					component6.text = cOMDT_RANK_PASTSEASON_FIGHT_RECORD.dwTotalWinCnt.ToString();
					component7.text = ((cOMDT_RANK_PASTSEASON_FIGHT_RECORD.dwTotalFightCnt <= 0u) ? "0.00%" : string.Format("{0}%", (cOMDT_RANK_PASTSEASON_FIGHT_RECORD.dwTotalWinCnt * 100f / cOMDT_RANK_PASTSEASON_FIGHT_RECORD.dwTotalFightCnt).ToString("0.00")));
					component8.text = cOMDT_RANK_PASTSEASON_FIGHT_RECORD.dwMaxContinuousWinCnt.ToString();
					int num = (list.Count <= 4) ? list.Count : 4;
					for (int j = 0; j < num; j++)
					{
						Transform transform = elemenet.transform.Find(string.Format("Expand/Hero{0}", j + 1));
						transform.gameObject.CustomSetActive(true);
						COMDT_RANK_COMMON_USED_HERO cOMDT_RANK_COMMON_USED_HERO = list[j];
		...
		public static void InitLadderRecent(CUIFormScript form, List<COMDT_RANK_CURSEASON_FIGHT_RECORD> dataList)
		{
			...
			{
				int num = (dataList.Count >= 10) ? 10 : dataList.Count;
				component.SetElementAmount(num);
				for (int i = 0; i < num; i++)
				{
					CUIListElementScript elemenet = component.GetElemenet(i);
					COMDT_RANK_CURSEASON_FIGHT_RECORD cOMDT_RANK_CURSEASON_FIGHT_RECORD = dataList[i];
					...
					component4.text = cOMDT_RANK_CURSEASON_FIGHT_RECORD.dwAddStarScore.ToString();
					component5.text = CLadderView.GetGameTime(ref cOMDT_RANK_CURSEASON_FIGHT_RECORD);
					component6.text = string.Format("{0} / {1} / {2}", cOMDT_RANK_CURSEASON_FIGHT_RECORD.dwKillNum, cOMDT_RANK_CURSEASON_FIGHT_RECORD.dwDeadNum, cOMDT_RANK_CURSEASON_FIGHT_RECORD.dwAssistNum);
					ResHeroCfgInfo dataByKey = GameDataMgr.heroDatabin.GetDataByKey(cOMDT_RANK_CURSEASON_FIGHT_RECORD.dwHeroId);
					if (dataByKey != null)
					{
		...
		public static void OnHistoryItemChange(GameObject go, bool bExpand)
		{
			...
			if (component)
			{
				component.color = ((!bExpand) ? CLadderView.s_Bg_Color_Shrink : CLadderView.s_Bg_Color_Expand);
			}
		...
		private static void SetMostUsedHero(Transform item, ref COMDT_RANK_COMMON_USED_HERO data, CUIFormScript formScript)
		{
			...
			component2.text = data.dwWinCnt.ToString();
		...
		private static string GetTopUseHeroNames(ref COMDT_RANK_PASTSEASON_FIGHT_RECORD data, out List<COMDT_RANK_COMMON_USED_HERO> heroList)
		{
			...
			{
				ResHeroCfgInfo dataByKey = GameDataMgr.heroDatabin.GetDataByKey(heroList[i].dwHeroId);
				if (dataByKey != null)
				{
