...
namespace Assets.Scripts.GameSystem
{
	...
	public class COBSystem : Singleton<COBSystem>
	{
		...
		public bool IsInOBFriendList(ulong uid)
		{
			...
			{
				if (this.OBFriendList[i].uin.ullUid == uid)
				{
					return true;
				}
			}
		...
		private void InitTab(CUIListScript list)
		{
			...
			int num = Enum.GetValues(typeof(COBSystem.enOBTab)).Length - 1;
			if (!flag)
			{
				num--;
			}
			...
			for (int i = 0; i < num; i++)
			{
				...
				Utility.GetComponetInChild<Text>(elemenet.gameObject, "Text").text = Singleton<CTextManager>.instance.GetText(array[num2]);
			}
		...
		public void UpdateView()
		{
			...
			if (curTab == COBSystem.enOBTab.Expert)
			{
				...
				componetInChild2.SetElementAmount(this.OBExpertList.Count);
			}
			else if (curTab == COBSystem.enOBTab.Friend)
			{
				...
				component.anchoredPosition = Vector2.zero;
				component.sizeDelta = Vector2.zero;
				componetInChild2.SetElementAmount(this.OBFriendList.Count);
			}
			else if (curTab == COBSystem.enOBTab.Guild)
			{
				...
				component.anchoredPosition = Vector2.zero;
				component.sizeDelta = Vector2.zero;
				componetInChild2.SetElementAmount(this.OBGuildList.Count);
			}
			else if (curTab == COBSystem.enOBTab.Local)
			{
				...
				componetInChild.text = Singleton<CTextManager>.instance.GetText((this.curStatus != COBSystem.enStatus.Normal) ? "Common_Close" : "Common_Edit");
		...
		private void UpdateElement(GameObject element, COBSystem.stOBFriend OBFriend)
		{
			if (CFriendModel.RemarkNames != null && CFriendModel.RemarkNames.ContainsKey(OBFriend.uin.ullUid))
			{
				...
				if (CFriendModel.RemarkNames.TryGetValue(OBFriend.uin.ullUid, out empty))
				{
		...
		private void UpdateElement(GameObject element, string name, string headUrl, byte bGrade, uint subGrade, uint heroId, COBSystem.enOBTab curTab, int onlineNum, bool isExpired, COBSystem.enStatus status = COBSystem.enStatus.Normal, long localTicks = 0L, byte mapType = 0, uint mapId = 0u, string localName = "")
		{
			...
			if (curTab != COBSystem.enOBTab.Local)
			{
				...
				componetInChild9.text = Singleton<CTextManager>.instance.GetText("OB_Desc_3", new string[]
				{
					onlineNum.ToString()
				});
			...
			else
			{
				...
				componetInChild7.text = Singleton<CTextManager>.instance.GetText("OB_Desc_12", new string[]
				{
		...
		private void OnVideoEnterConfirm(CUIEvent cuiEvent)
		{
			...
			if (curTab == COBSystem.enOBTab.Expert)
			{
				...
				{
					Singleton<WatchController>.GetInstance().TargetUID = this.OBExpertList[selectedIndex].heroLabel.ullUid;
					COBSystem.SendOBServeGreat(this.OBExpertList[selectedIndex].desk);
					this.m_watchEntryData = new WatchEntryData();
					this.m_watchEntryData.headUrl = StringHelper.UTF8BytesToString(ref this.OBExpertList[selectedIndex].heroLabel.szHealUrl);
					this.m_watchEntryData.name = StringHelper.UTF8BytesToString(ref this.OBExpertList[selectedIndex].heroLabel.szRoleName);
					this.m_watchEntryData.rankClass = this.OBExpertList[selectedIndex].heroLabel.dwClass;
					this.m_watchEntryData.rankGrade = (byte)this.OBExpertList[selectedIndex].heroLabel.dwGrade;
					this.m_watchEntryData.time = (long)CRoleInfo.GetCurrentUTCTime();
					this.m_watchEntryData.usedHeroId = this.OBExpertList[selectedIndex].heroLabel.dwHeroID;
					this.m_watchEntryData.userUid = this.OBExpertList[selectedIndex].heroLabel.ullUid;
				}
				int count = this.OBExpertList.Count;
			}
			else if (curTab == COBSystem.enOBTab.Friend)
			{
				if (selectedIndex >= 0 && selectedIndex < this.OBFriendList.Count)
				{
					Singleton<WatchController>.GetInstance().TargetUID = this.OBFriendList[selectedIndex].uin.ullUid;
					COBSystem.SendOBServeFriend(this.OBFriendList[selectedIndex].uin, -1);
					this.m_watchEntryData = new WatchEntryData();
					this.m_watchEntryData.headUrl = this.OBFriendList[selectedIndex].headUrl;
					this.m_watchEntryData.name = this.OBFriendList[selectedIndex].friendName;
					this.m_watchEntryData.rankClass = this.OBFriendList[selectedIndex].gameDetail.dwClass;
					this.m_watchEntryData.rankGrade = this.OBFriendList[selectedIndex].gameDetail.bShowRankGrade;
					this.m_watchEntryData.time = (long)CRoleInfo.GetCurrentUTCTime();
					this.m_watchEntryData.usedHeroId = this.OBFriendList[selectedIndex].gameDetail.dwHeroID;
					this.m_watchEntryData.userUid = this.OBFriendList[selectedIndex].uin.ullUid;
				}
				int count = this.OBFriendList.Count;
			}
			else if (curTab == COBSystem.enOBTab.Guild)
			{
				if (selectedIndex >= 0 && selectedIndex < this.OBGuildList.Count)
				{
					...
					this.m_watchEntryData = new WatchEntryData();
					this.m_watchEntryData.headUrl = this.OBGuildList[selectedIndex].headUrl;
					this.m_watchEntryData.name = this.OBGuildList[selectedIndex].playerName;
					this.m_watchEntryData.rankClass = this.OBGuildList[selectedIndex].dwClass;
					this.m_watchEntryData.rankGrade = this.OBGuildList[selectedIndex].bGrade;
					this.m_watchEntryData.time = (long)CRoleInfo.GetCurrentUTCTime();
					this.m_watchEntryData.usedHeroId = this.OBGuildList[selectedIndex].dwHeroID;
					this.m_watchEntryData.userUid = this.OBGuildList[selectedIndex].playerUid;
				}
				int count = this.OBGuildList.Count;
			}
			else if (curTab == COBSystem.enOBTab.Local)
			{
				...
				int count = this.OBLocalList.Count;
			}
		}
		...
		public void OBFriend(COMDT_ACNT_UNIQ uin)
		{
			...
			for (int i = 0; i < this.OBFriendList.Count; i++)
			{
				if (uin.ullUid == this.OBFriendList[i].uin.ullUid)
				{
					flag = true;
					break;
				}
				...
				if (cSDT_SNS_FRIEND_INFO != null)
				{
					if (cSDT_SNS_FRIEND_INFO.bVideoState != 0)
					{
						...
						else
						{
							...
							for (int i = 0; i < this.OBFriendList.Count; i++)
							{
								if (stNtfSnsFriend.astSnsFriendList[num].stSnsFrindInfo.stUin.ullUid == this.OBFriendList[i].uin.ullUid)
								{
									COBSystem.stOBFriend stOBFriend = this.OBFriendList[i];
									stOBFriend.gameDetail = stNtfSnsFriend.astSnsFriendList[num].stGameInfo.stDetail;
									this.OBFriendList[i] = stOBFriend;
									flag = true;
									break;
								}
							}
				...
				if (cSDT_FRIEND_INFO != null)
				{
					if (cSDT_FRIEND_INFO.bVideoState != 0)
					{
						...
						else
						{
							bool flag = false;
							int num = 0;
							while (i < this.OBFriendList.Count)
							{
								if (stFriendListRsp.astFrindList[i].stFriendInfo.stUin.ullUid == this.OBFriendList[num].uin.ullUid)
								{
									COBSystem.stOBFriend stOBFriend = this.OBFriendList[num];
									stOBFriend.gameDetail = stFriendListRsp.astFrindList[i].stGameInfo.stDetail;
									this.OBFriendList[num] = stOBFriend;
									flag = true;
									break;
								}
								num++;
		...
		private void On_Friend_GAME_STATE_NTF(CSPkg msg)
		{
			...
			for (int i = 0; i < this.OBFriendList.Count; i++)
			{
				if (this.OBFriendList[i].uin.ullUid == stNtfFriendGameState.stAcntUniq.ullUid)
				{
					...
					{
						stOBFriend = this.OBFriendList[i];
		...
		private void OnGetGreatMatch(CSPkg msg)
		{
			...
			while ((long)num < (long)((ulong)msg.stPkgData.stGetGreatMatchRsp.dwCount))
			{
				...
				{
					COBSystem.stOBExpert item = default(COBSystem.stOBExpert);
					item.desk = msg.stPkgData.stGetGreatMatchRsp.astList[num].stDesk;
					item.startTime = msg.stPkgData.stGetGreatMatchRsp.astList[num].dwStartTime;
					item.observeNum = msg.stPkgData.stGetGreatMatchRsp.astList[num].dwObserveNum;
					item.heroLabel = msg.stPkgData.stGetGreatMatchRsp.astList[num].astLabel[num2];
		...
		public static void SendOBServeFriend(COMDT_ACNT_UNIQ uin, int friendType = -1)
		{
			...
			if (friendType == -1)
			{
				friendType = ((!Singleton<CFriendContoller>.GetInstance().model.IsGameFriend(uin.ullUid, uin.dwLogicWorldId)) ? 2 : 1);
			}
