	...
	public class ProgressWidget : ActivityWidget
	{
		...
		public ProgressWidget(GameObject node, ActivityView view) : base(node, view)
		{
			...
			if (phaseList.Count > 0)
			{
				...
				{
					ProgressWidget.Cursor cursor = new ProgressWidget.Cursor((GameObject)UnityEngine.Object.Instantiate(this._cursorTemplate.root));
		...
		public override void Validate()
		{
			...
			if (phaseList.Count > 0)
			{
				...
				for (int i = 0; i < phaseList.Count; i++)
				{
					...
					cursor.valTxt.text = activityPhase.Target.ToString();
