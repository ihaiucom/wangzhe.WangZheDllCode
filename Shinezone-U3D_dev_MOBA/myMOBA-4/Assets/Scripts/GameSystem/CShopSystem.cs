...
namespace Assets.Scripts.GameSystem
{
	...
	public class CShopSystem : Singleton<CShopSystem>
	{
		...
		private bool NeedAutoRefreshShop(RES_SHOP_TYPE shopType)
		{
			...
			bool flag = this.m_Shops.TryGetValue((uint)shopType, out stShopInfo);
			if (stShopInfo == null)
			{
				return false;
			}
			DateTime dateTime = new DateTime(1970, 1, 1, 0, 0, 0, DateTimeKind.Utc);
			DateTime t = dateTime.AddSeconds((double)CRoleInfo.GetCurrentUTCTime());
			DateTime t2 = dateTime.AddSeconds((double)stShopInfo.dwAutoRefreshTime);
			...
			for (int i = 0; i < refreshTime.Length; i++)
			{
				...
				DateTime dateTime3 = new DateTime(t2.Year, t2.Month, t2.Day, 0, 0, 0, DateTimeKind.Utc);
				int num = refreshTime[i] / 100;
				int num2 = refreshTime[i] % 100;
				dateTime2 = dateTime3.AddSeconds((double)(num * 3600) + (double)(num2 * 60) - 28800.0);
				while (DateTime.Compare(dateTime2, t) <= 0)
				{
					if (DateTime.Compare(t2, dateTime2) < 0 && DateTime.Compare(dateTime2, t) < 0)
					{
						return true;
					}
		...
		public void InitTab()
		{
			...
			if (component != null)
			{
				...
				for (int i = 0; i < component.m_elementAmount; i++)
				{
					...
					component2.text = list[i];
					Text component3 = elemenet.gameObject.transform.Find("txtShopTypeData").GetComponent<Text>();
					component3.text = list2[i].ToString();
					if (this.CurTab == (CShopSystem.Tab)list2[i])
					{
						index = i;
					}
				}
		...
		public void RefreshTopBar()
		{
			...
			case RES_SHOP_TYPE.RES_SHOPTYPE_BURNING:
			{
				...
				Text component = gameObject4.transform.FindChild("Text").GetComponent<Text>();
				if (component != null)
				{
			...
			case RES_SHOP_TYPE.RES_SHOPTYPE_ARENA:
			{
				...
				Text component = gameObject5.transform.FindChild("Text").GetComponent<Text>();
				if (component != null)
				{
			...
			case RES_SHOP_TYPE.RES_SHOPTYPE_GUILD_HEAD_IMAGE:
			{
				...
				Text component = gameObject6.transform.FindChild("Text").GetComponent<Text>();
				if (component != null)
				{
		...
		private void SetItemInfo(GameObject item, ref stShopItemInfo info, int slotOffset)
		{
			...
			component.text = name;
			info.sName = name;
			component3.text = info.fPrice.ToString("N0");
			switch (info.enCostType)
			{
			case RES_SHOPBUY_COINTYPE.RES_SHOPBUY_TYPE_COUPONS:
				if (masterRoleInfo.DianQuan < info.fPrice)
				{
					component3.color = new Color(1f, 0.4f, 0.4f, 1f);
				}
				else
				{
					component3.color = new Color(1f, 0.91f, 0.45f, 1f);
				}
				break;
			case RES_SHOPBUY_COINTYPE.RES_SHOPBUY_TYPE_PVPCOIN:
			...
			if (info.isSoldOut)
			{
				component3.color = new Color(0.7176f, 0.7176f, 0.7176f, 1f);
			...
			if (info.wSaleDiscount != 100)
			{
				...
				component2.text = ((float)info.wSaleDiscount / 10f).ToString("F1") + Singleton<CTextManager>.GetInstance().GetText("Shop_Discount");
			}
			else
			{
		...
		private void SetItemLockPanel(GameObject lockPanel, int slotOffset)
		{
			...
			if (this.m_CurShopType == RES_SHOP_TYPE.RES_SHOPTYPE_GUILD)
			{
				...
				component.text = Singleton<CTextManager>.GetInstance().GetText("Shop_Guild_Item_Shop_Unlock_Tip", new string[]
				{
			...
			else if (this.m_CurShopType == RES_SHOP_TYPE.RES_SHOPTYPE_GUILD_HEAD_IMAGE)
			{
				...
				component.text = Singleton<CTextManager>.GetInstance().GetText("Shop_Guild_Head_Image_Shop_Unlock_Tip", new string[]
				{
		...
		public void RefreshInfoPanel()
		{
			...
			case RES_SHOP_TYPE.RES_SHOPTYPE_GUILD_HEAD_IMAGE:
			{
				...
				DateTime dateTime = new DateTime(1970, 1, 1, 0, 0, 0, DateTimeKind.Utc);
				DateTime d = dateTime.AddSeconds((double)CRoleInfo.GetCurrentUTCTime());
				...
				{
					float totalTime = (float)((nextAutoRefreshTime - d).TotalSeconds - 28800.0);
					component2.SetTotalTime(totalTime);
					component2.StartTimer();
					component.text = Singleton<CTextManager>.GetInstance().GetText("Shop_Next_Refresh_Time_Prefix") + " <color=#f5be17>" + nextAutoRefreshTime.ToString("HH:mm") + "</color>";
					gameObject.CustomSetActive(true);
				}
				...
				component3.text = Singleton<CTextManager>.GetInstance().GetText("Shop_Today_Left_Cnt", new string[]
				{
			...
			case RES_SHOP_TYPE.RES_SHOPTYPE_MYSTERY:
			{
				...
				stShopInfo stShopInfo = new stShopInfo();
				if (this.m_Shops.TryGetValue(2u, out stShopInfo))
				{
		...
		private void ResetManualRefreshCountIfNeed()
		{
			...
			if (this.m_Shops.TryGetValue((uint)this.m_CurShopType, out stShopInfo))
			{
		...
		private void RefreshBuyPnl()
		{
			...
			case RES_SHOPBUY_COINTYPE.RES_SHOPBUY_TYPE_COUPONS:
				if (componetInChild4 != null)
				{
					...
					{
						componetInChild4.color = new Color(1f, 0.4f, 0.4f, 1f);
					}
					else
					{
						componetInChild4.color = new Color(1f, 0.91f, 0.45f, 1f);
					}
				}
			...
			{
				componetInChild2.text = num.ToString();
			}
			if (componetInChild3 != null)
			{
				componetInChild3.text = Singleton<CTextManager>.GetInstance().GetText("Shop_Item_Buy_Status_Unit");
			}
			if (componetInChild != null)
			{
				componetInChild.text = cUseable.m_name;
			}
			if (componetInChild4 != null)
			{
				componetInChild4.text = stShopItemInfo.fPrice.ToString();
			}
			if (componetInChild5 != null)
			{
				componetInChild5.text = cUseable.m_description;
			}
			if (stShopItemInfo.enItemType == COM_ITEM_TYPE.COM_OBJTYPE_ITEMPROP)
			{
				...
				{
					componetInChild5.text = CUIUtility.StringReplace(componetInChild5.text, new string[]
					{
						num.ToString()
					});
				}
			...
			{
				componetInChild6.text = stShopItemInfo.wItemCnt + Singleton<CTextManager>.GetInstance().GetText("Shop_Item_Buy_Status_Unit");
			}
			if (gameObject != null)
			{
		...
		private DateTime GetNextAutoRefreshTime(RES_SHOP_TYPE shopType)
		{
			DateTime dateTime = new DateTime(1970, 1, 1, 0, 0, 0, DateTimeKind.Utc);
			DateTime t = dateTime.AddSeconds((double)CRoleInfo.GetCurrentUTCTime());
			DateTime dateTime2 = new DateTime(t.Year, t.Month, t.Day, 0, 0, 0, DateTimeKind.Utc);
			...
			for (int i = 0; i < refreshTime.Length; i++)
			{
				...
				if (refreshTime[i] == 0)
				{
					return d.AddSeconds(115200.0);
				}
				int num = refreshTime[i] / 100;
				int num2 = refreshTime[i] % 100;
				dateTime3 = dateTime2.AddSeconds((double)(num * 3600) + (double)(num2 * 60) - 28800.0);
				if (d == DateTime.MinValue)
				{
					d = dateTime3;
				}
				if (DateTime.Compare(t, dateTime3) < 0)
				{
					return dateTime3.AddSeconds(28800.0);
				}
		...
		private void InitSaleTip()
		{
			...
			int i = 0;
			while (i < curUseableCount)
			{
				CUseable cUseable = useableContainer.GetUseableByIndex(i);
				...
				case COM_ITEM_TYPE.COM_OBJTYPE_ITEMPROP:
				{
					...
					{
						this.aSaleItems[(int)this.m_saleItemsCnt] = cUseable;
		...
		private string GetShopCharacterTip(string tipKeyPrefix)
		{
			...
			int num = new System.Random().Next(0, 10);
			return array[num];
		}
		...
		private int GetManualRefreshedCnt()
		{
			...
			if (this.m_Shops != null && this.m_Shops.TryGetValue((uint)this.m_CurShopType, out shopInfo))
			{
		...
		private void OnShop_Tab_Change(CUIEvent uiEvent)
		{
			...
			int curTab;
			if (!int.TryParse(component2.text, out curTab))
			{
		...
		private void OnShop_ConfirmManualRefresh(CUIEvent uiEvent)
		{
			...
			this.m_Shops[(uint)this.m_CurShopType] = stShopInfo;
		...
		public static void ReceiveItemBuy(CSPkg msg)
		{
			...
			int num = 0;
			stShopInfo stShopInfo = new stShopInfo();
			if (instance.m_Shops.TryGetValue((uint)instance.m_CurShopType, out stShopInfo))
			{
		...
		public static void ReceiveShopItems(CSPkg msg)
		{
			...
			{
				instance.m_Shops[(uint)wShopType] = stShopInfo2;
			}
