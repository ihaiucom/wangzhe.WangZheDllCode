...
namespace Assets.Scripts.GameSystem
{
	public class CBattleShowBuffDesc
	{
		...
		private void UpdateShowBuffList(ref PoolObjHandle<BuffSkill> stBuffSkill, bool bIsAdd)
		{
				...
				while (i < this.m_CurShowBuffCount)
				{
					...
					if (this.m_ArrShowBuffSkill[i].stBuffSkill && this.m_ArrShowBuffSkill[i].stBuffSkill.handle.cfgData.bShowBuffPriority == stBuffSkill.handle.cfgData.bShowBuffPriority)
					{
						if (this.m_ArrShowBuffSkill[i].stBuffSkill.handle.cfgData.iCfgID == stBuffSkill.handle.cfgData.iCfgID)
						{
							...
							CBattleShowBuffDesc.BuffInfo[] expr_233_cp_0 = this.m_ArrShowBuffSkill;
							int expr_233_cp_1 = i;
							expr_233_cp_0[expr_233_cp_1].iCacheCount = expr_233_cp_0[expr_233_cp_1].iCacheCount + 1;
							flag = false;
							num = -1;
							break;
						}
					...
					else if (i >= this.m_CurShowBuffCount - 1)
					{
			...
			else
			{
				for (int k = 0; k < this.m_CurShowBuffCount; k++)
				{
					...
					{
						CBattleShowBuffDesc.BuffInfo[] expr_3E8_cp_0 = this.m_ArrShowBuffSkill;
						int expr_3E8_cp_1 = k;
						expr_3E8_cp_0[expr_3E8_cp_1].iCacheCount = expr_3E8_cp_0[expr_3E8_cp_1].iCacheCount - 1;
				...
				if (i < this.m_CurShowBuffCount)
				{
					if (this.m_ArrShowBuffSkill[i].stBuffSkill && this.m_ArrShowBuffSkill[i].stBuffSkill.handle.cfgData != null && !string.IsNullOrEmpty(this.m_ArrShowBuffSkill[i].stBuffSkill.handle.cfgData.szIconPath))
					{
						...
						if (gameObject2 != null)
						{
							...
							{
								component3.text = this.m_ArrShowBuffSkill[i].stBuffSkill.handle.GetOverlayCount().ToString();
		...
		private void ShowBuffSkillDesc(uint uiBuffId, Vector3 BtnPos)
		{
			...
			if (!string.IsNullOrEmpty(dataByKey.szSkillCombineDesc))
			{
				...
				if (this.m_curActor && this.m_curActor.handle.ValueComponent != null)
				{
					...
					component.text = CUICommonSystem.GetSkillDesc(dataByKey.szSkillCombineDesc, actorValue, 1, actorSoulLevel, configId);
				}
				else
				{
				...
				this.m_textBgImage.rectTransform.sizeDelta = new Vector2(sizeDelta.x, num);
		...
		public void SwitchActor(ref PoolObjHandle<ActorRoot> actor)
		{
			...
			if (spawnedBuffList != null && spawnedBuffList.Count != 0)
			{
				...
				for (int i = 0; i < count; i++)
				{
					...
					{
						PoolObjHandle<BuffSkill> poolObjHandle = new PoolObjHandle<BuffSkill>(spawnedBuffList[i]);
		...
		public void UpdateBuffCD(int delta)
		{
			...
			{
				int num;
				if (this.m_ArrShowBuffSkill != null && this.m_ArrShowBuffSkill[i].stBuffSkill && (num = this.m_ArrShowBuffSkill[i].iBufCD - delta) > 0)
				{
					if (this.m_ArrShowBuffSkill[i].stBuffSkill.handle.cfgData != null)
					{
