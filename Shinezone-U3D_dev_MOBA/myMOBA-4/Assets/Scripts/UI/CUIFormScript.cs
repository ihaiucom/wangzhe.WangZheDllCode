	...
	public class CUIFormScript : MonoBehaviour, IComparable
	{
		...
		public void CustomLateUpdate()
		{
			if (this.m_initWidgetPositions != null)
			{
				...
				{
					CUIFormScript.stInitWidgetPosition stInitWidgetPosition = this.m_initWidgetPositions[i];
		...
		public float ChangeFormValueToScreen(float value)
		{
			if (this.m_canvasScaler.matchWidthOrHeight == 0f)
			{
				return value * (float)Screen.width / this.m_canvasScaler.referenceResolution.x;
			}
			if (this.m_canvasScaler.matchWidthOrHeight == 1f)
			{
				return value * (float)Screen.height / this.m_canvasScaler.referenceResolution.y;
			}
			return value;
		}
		...
		public void InitializeWidgetPosition(GameObject widget, Vector3 worldPosition)
		{
			...
			CUIFormScript.stInitWidgetPosition item = default(CUIFormScript.stInitWidgetPosition);
		...
		private void UpdateASyncLoadedImage()
		{
			...
			while (i < this.m_asyncLoadedImages.Count)
			{
				...
				if (image != null)
				{
					...
					if (!this.m_loadedSpriteDictionary.TryGetValue(prefabPath, out gameObject) && !flag)
					{
		...
		public float GetScreenScaleValue()
		{
			...
			if (component && this.m_canvasScaler.matchWidthOrHeight == 0f)
			{
