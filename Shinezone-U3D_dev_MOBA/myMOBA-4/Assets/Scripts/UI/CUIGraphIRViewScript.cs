...
namespace Assets.Scripts.UI
{
	public class CUIGraphIRViewScript : CUIComponent
	{
		...
		public static void StoreTextureData(List<Vector3> vertexs, stStateHeatmapStorage st, bool isUpdate = false)
		{
			...
			else if (st.myVertex.Count > vertexs.Count)
			{
				Debug.LogError("IRView should not use update way");
			}
			int count = st.myVertex.Count;
			for (int j = st.myVertex.Count; j < vertexs.Count; j++)
			{
				float value = (vertexs[j].x + (float)(num / 2)) * st.defaultCtrlRect.x / (float)num;
				float value2 = (vertexs[j].z + (float)(num2 / 2)) * st.defaultCtrlRect.y / (float)num2;
		...
		public void UpdateTexture(stStateHeatmapStorage st)
		{
			...
			if (this.m_text2D == null)
			{
				...
				this.m_image.sprite = Sprite.Create(this.m_text2D, new Rect(0f, 0f, (float)this.m_text2D.width, (float)this.m_text2D.height), new Vector2(0f, 0f));
			}
