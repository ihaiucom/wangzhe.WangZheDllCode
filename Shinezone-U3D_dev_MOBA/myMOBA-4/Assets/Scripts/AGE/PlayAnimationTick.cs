...
namespace AGE
{
	...
	public class PlayAnimationTick : TickEvent
	{
		...
		public override void Process(Action _action, Track _track)
		{
			...
			float speed = this.playSpeed * ((!this.applyActionSpeed) ? 1f : _action.playSpeed.single);
			...
			{
				AnimPlayComponent animPlayComponent = (!actorHandle) ? null : actorHandle.handle.AnimControl;
		...
		public override void ProcessBlend(Action _action, Track _track, TickEvent _prevEvent, float _blendWeight)
		{
			...
			animationState.speed = this.playSpeed * ((!this.applyActionSpeed) ? 1f : _action.playSpeed.single);
		}
