...
namespace AGE
{
	...
	public class ChangeSkillTriggerTick : TickEvent
	{
		...
		private int RandomSkillID(out int index)
		{
			...
			if (ChangeSkillTriggerTick.randomList.Count == 1)
			{
				return ChangeSkillTriggerTick.randomList[0].ID;
			}
			if (ChangeSkillTriggerTick.randomList.Count == 0)
			{
				return 0;
			}
			int num2 = (int)FrameRandom.Random((uint)num);
			for (int i = 0; i < ChangeSkillTriggerTick.randomList.Count; i++)
			{
				if (num2 < ChangeSkillTriggerTick.randomList[i].ProbabilityBase)
				{
					index = i + 1;
					return ChangeSkillTriggerTick.randomList[i].ID;
				}
			}
			return 0;
		}
