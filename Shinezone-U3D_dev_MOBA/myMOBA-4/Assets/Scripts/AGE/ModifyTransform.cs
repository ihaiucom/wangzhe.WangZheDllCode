...
namespace AGE
{
	...
	public class ModifyTransform : TickEvent
	{
		...
		public void CubicVectorBlend(Action _action, Track _track, TickEvent _prevEvent, float _blendWeight, bool isPos)
		{
			...
			Vector3 vector = (!isPos) ? modifyTransform.scaling : modifyTransform.GetTranslation(_action);
			Vector3 vector2 = (!isPos) ? this.scaling : this.GetTranslation(_action);
			Vector3 formPoint = (!isPos) ? modifyTransform2.scaling : modifyTransform2.GetTranslation(_action);
			Vector3 lattPoint = (!isPos) ? modifyTransform3.scaling : modifyTransform3.GetTranslation(_action);
			Vector3 a;
			Vector3 a2;
		...
		public Vector3 GetTranslation(Action _action)
		{
			...
			if (gameObject && gameObject2)
			{
				...
				if (this.normalizedRelative)
				{
					...
					vector += new Vector3(0f, this.translation.z * (gameObject2.transform.position.y - gameObject.transform.position.y), 0f);
				}
				else
				{
					vector = gameObject.transform.position + quaternion * this.translation;
					vector += new Vector3(0f, this.translation.z / magnitude * (gameObject2.transform.position.y - gameObject.transform.position.y), 0f);
				}
				return vector;
			}
