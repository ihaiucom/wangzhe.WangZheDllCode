...
internal class SceneInterpolation : MonoBehaviour
{
	...
	public void Play()
	{
		...
		for (int j = 0; j < this.cameraList.Count; j++)
		{
			...
			{
				SceneInterpolation.RestoreCameraClearFlags item = default(SceneInterpolation.RestoreCameraClearFlags);
				item.camera = camera3;
				item.flags = camera3.clearFlags;
				this.cameraClearFlagsList.Add(item);
	...
	public void Stop()
	{
		...
		{
			SceneInterpolation.RestoreCameraClearFlags restoreCameraClearFlags = this.cameraClearFlagsList[i];
