...
public class CAskForMailView
{
	...
	private void Draw(CMail mail)
	{
		if (this.form != null)
		{
			...
			componetInChild.text = string.Format(Singleton<CTextManager>.GetInstance().GetText("Mail_Ask_For_Myself"), masterRoleInfo.Name);
			componetInChild2.text = mail.mailContent;
			componetInChild3.text = string.Format(Singleton<CTextManager>.GetInstance().GetText("Mail_Ask_For_From", new string[]
			{
				mail.from
			}), new object[0]);
			...
			case COM_ITEM_TYPE.COM_OBJTYPE_HERO:
			{
				...
				if (dataByKey != null)
				{
					...
					component.text = Singleton<CTextManager>.GetInstance().GetText("Ask_For_Hero_Friend_Title");
			...
			case COM_ITEM_TYPE.COM_OBJTYPE_HEROSKIN:
			{
				...
				if (heroSkin != null)
				{
					...
					component4.text = Singleton<CTextManager>.GetInstance().GetText("Ask_For_Skin_Friend_Title");
					...
					component6.text = StringHelper.UTF8BytesToString(ref heroSkin.szSkinName);
