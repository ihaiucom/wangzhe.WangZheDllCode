...
public class CMailView
{
	...
	public void Open(CustomMailType mailType)
	{
		...
		this.m_CUIListScriptTab.GetElemenet(3).transform.FindChild("Text").GetComponent<Text>().text = Singleton<CTextManager>.GetInstance().GetText("Mail_Ask_For_Tab");
	...
	public void UpdateMailList(CustomMailType mailtype, ListView<CMail> mailList)
	{
		...
		else if (mailtype == CustomMailType.ASK_FOR)
		{
			...
			if (masterRoleInfo != null)
			{
				if (gameObject != null)
				{
					gameObject.GetComponent<Toggle>().isOn = masterRoleInfo.IsNoAskFor;
				}
			}
			else
			{
			...
			CUIListScript component = this.m_CUIForm.transform.FindChild("PanelAskForMail/List").GetComponent<CUIListScript>();
			component.SetElementAmount(mailList.Count);
	...
	public void UpdateListElenment(GameObject element, CMail mail)
	{
		...
		componetInChild.text = mail.subject;
		componetInChild2.text = Utility.GetTimeBeforString((long)((ulong)mail.sendTime), (long)currentUTCTime);
		...
		if (mail.mailType == CustomMailType.SYSTEM)
		{
			...
			componetInChild3.text = string.Empty;
		...
		else if (mail.mailType == CustomMailType.FRIEND)
		{
			...
			componetInChild3.text = mail.from;
		...
		else if (mail.mailType == CustomMailType.FRIEND_INVITE)
		{
			...
			componetInChild.text = string.Format("{0} {1}", mail.subject, processTypeString);
			if (text != null)
			{
				text.text = text2;
			}
			...
			if (flag2)
			{
				CUIUtility.GetComponentInChildren<Image>(componetInChild4.gameObject).color = CUIUtility.s_Color_GrayShader;
			}
			else
			{
				CUIUtility.GetComponentInChildren<Image>(componetInChild4.gameObject).color = CUIUtility.s_Color_Full;
			}
