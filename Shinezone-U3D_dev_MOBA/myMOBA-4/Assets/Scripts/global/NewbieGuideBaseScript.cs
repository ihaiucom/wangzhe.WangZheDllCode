...
public class NewbieGuideBaseScript : MonoBehaviour
{
	...
	private void TryToAddClickEvent()
	{
		...
		while (enumerator.MoveNext())
		{
			...
			if (!(current == null))
			{
				...
				if (component != null)
				{
					CUIEventScript expr_3D = component;
					expr_3D.onClick = (CUIEventScript.OnUIEventHandler)Delegate.Combine(expr_3D.onClick, new CUIEventScript.OnUIEventHandler(this.ClickHandler));
				}
				CUIMiniEventScript cUIMiniEventScript = current.GetComponent<CUIMiniEventScript>();
				if (cUIMiniEventScript != null)
				{
					CUIMiniEventScript expr_73 = cUIMiniEventScript;
					expr_73.onClick = (CUIMiniEventScript.OnUIEventHandler)Delegate.Combine(expr_73.onClick, new CUIMiniEventScript.OnUIEventHandler(this.ClickHandler));
				}
				if (component == null && cUIMiniEventScript == null)
				{
					cUIMiniEventScript = current.AddComponent<CUIMiniEventScript>();
					CUIMiniEventScript expr_B5 = cUIMiniEventScript;
					expr_B5.onClick = (CUIMiniEventScript.OnUIEventHandler)Delegate.Combine(expr_B5.onClick, new CUIMiniEventScript.OnUIEventHandler(this.ClickHandler));
				}
			}
		}
	...
	private void TryToRemoveClickEvent()
	{
		...
		while (enumerator.MoveNext())
		{
			...
			if (!(current == null))
			{
				...
				if (component != null)
				{
					CUIEventScript expr_44 = component;
					expr_44.onClick = (CUIEventScript.OnUIEventHandler)Delegate.Remove(expr_44.onClick, new CUIEventScript.OnUIEventHandler(this.ClickHandler));
					flag = true;
				}
				CUIMiniEventScript component2 = current.GetComponent<CUIMiniEventScript>();
				if (component2 != null)
				{
					CUIMiniEventScript expr_7F = component2;
					expr_7F.onClick = (CUIMiniEventScript.OnUIEventHandler)Delegate.Remove(expr_7F.onClick, new CUIMiniEventScript.OnUIEventHandler(this.ClickHandler));
					flag = true;
				}
				...
				if (component3 != null)
				{
					...
					Toggle component4 = NewbieGuideBaseScript.ms_originalGo[num].GetComponent<Toggle>();
					if (component4 != null)
					{
	...
	private void ClearHighlitObjs()
	{
		...
		int count = NewbieGuideBaseScript.ms_guideTextList.Count;
		for (int i = 0; i < count; i++)
		{
			GameObject gameObject = NewbieGuideBaseScript.ms_guideTextList[i];
			if (gameObject != null)
			{
				gameObject.CustomSetActive(false);
			}
	...
	protected virtual void Update()
	{
		...
		if (NewbieGuideBaseScript.ms_originalForm != null && formGuideMask != null)
		{
			...
			DebugHelper.Assert(count <= NewbieGuideBaseScript.ms_originalGo.Count);
			for (int i = 0; i < count; i++)
			{
				...
				if (NewbieGuideBaseScript.ms_highlighter.Count > i)
				{
					gameObject3 = NewbieGuideBaseScript.ms_highlighter[i];
				}
				if (!(gameObject == null) && !(gameObject2 == null))
				{
					...
					if (i < NewbieGuideBaseScript.ms_guideTextList.Count)
					{
						NewbieGuideSpecialTipConf specialTipConfig = Singleton<NewbieGuideDataManager>.GetInstance().GetSpecialTipConfig((uint)this.currentConf.wSpecialTip);
						if (specialTipConfig != null && specialTipConfig.bGuideTextPos > 0)
						{
							if (gameObject3 != null)
							{
								NewbieGuideBaseScript.ms_guideTextList[i].CustomSetActive(gameObject3.activeSelf);
							}
							NewbieGuideBaseScript.UpdateGuideTextPos(specialTipConfig, gameObject2, formGuideMask, NewbieGuideBaseScript.ms_originalForm, NewbieGuideBaseScript.ms_guideTextList[i]);
						}
					}
				}
			}
