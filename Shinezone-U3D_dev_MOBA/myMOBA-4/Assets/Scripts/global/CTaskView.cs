...
public class CTaskView
{
		...
		public static void ShowTaskAward(CUIFormScript formScript, CTask task, GameObject awardContainer, int awardItemcount = 2)
		{
			...
			for (int i = 0; i < awardItemcount; i++)
			{
				...
				if (resTaskRewardDetail != null && resTaskRewardDetail.iCnt > 0)
				{
					...
					gameObject.transform.FindChild("lblIconCount").GetComponent<Text>().text = string.Format("x{0}", resTaskRewardDetail.iCnt.ToString());
			...
			case 2:
				...
				if (this.tasklistScript_iwantgold != null)
				{
					...
					if (masterRoleInfo != null)
					{
						...
						component.text = Singleton<CTextManager>.GetInstance().GetText("GoldLimited_1", new string[]
						{
	...
	private void _show_day_box(GameObject box, ushort id)
	{
		...
		component.text = resHuoYueDuReward.dwHuoYueDu.ToString();
	...
	private void ShowLevelTask(LevelRewardData levelRewardData)
	{
		...
		else
		{
			...
			if (component2 != null)
			{
				component2.text = Singleton<CTextManager>.instance.GetText("Task_LevelRewardInfo_Normal", new string[]
				{
		...
		for (int i = 0; i < LevelRewardData.UNLOCK_MAX_COUNT; i++)
		{
			...
			if (resDT_LevelReward_UnlockInfo != null && !string.IsNullOrEmpty(resDT_LevelReward_UnlockInfo.szUnlockID))
			{
				...
				if (transform != null)
				{
					...
					if (component2 != null)
					{
						component2.text = resDT_LevelReward_UnlockInfo.szUnlockID;
					}
		...
		for (int i = 0; i < LevelRewardData.REWARD_MAX_COUNT; i++)
		{
			...
			if (resDT_LevelReward_Info.dwRewardNum != 0u)
			{
				...
				if (transform != null)
				{
					...
					if (component2 != null)
					{
						component2.text = "x " + resDT_LevelReward_Info.dwRewardNum;
					}
					...
					if (component3 != null)
					{
						component3.text = resDT_LevelReward_Info.szDesc;
					}
					num++;
				}
			}
