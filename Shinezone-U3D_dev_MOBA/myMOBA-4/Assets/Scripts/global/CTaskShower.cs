...
public class CTaskShower : MonoBehaviour
{
	...
	public void ShowTask(CTask task, CUIFormScript fromScript)
	{
		...
		if (flag)
		{
			...
			uint num = (this.taskID != Singleton<CTaskSys>.instance.monthCardTaskID) ? Singleton<CTaskSys>.instance.weekCardExpireTime : Singleton<CTaskSys>.instance.monthCardExpireTime;
