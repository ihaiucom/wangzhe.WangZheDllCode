...
public class NewbieGuideManager : MonoSingleton<NewbieGuideManager>
{
	...
	public void SetNewbieGuideState(uint id, bool bOpen)
	{
		...
		{
			this.mWeakCompleteCacheDic[(uint)((ulong)id - (ulong)((long)NewbieGuideManager.WEAKGUIDE_BIT_OFFSET))] = bOpen;
		}
		else
		{
	...
	{
		bool result;
		if (this.mCompleteCacheDic.TryGetValue(id, out result))
		{
			return result;
		}
		CRoleInfo masterRoleInfo = Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo();
	...
	{
		bool result;
		if (this.mWeakCompleteCacheDic.TryGetValue(lineId, out result))
		{
			return result;
		}
		CRoleInfo masterRoleInfo = Singleton<CRoleInfoManager>.GetInstance().GetMasterRoleInfo();
	...
	private void CommitIsCompleteNewbieGuide(bool IsComplete)
	{
        ...
        cSPkg.stPkgData.stAcntSetOldType.bYes = ((!IsComplete) ? (byte)0 : (byte)1);
