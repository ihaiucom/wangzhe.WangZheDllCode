...
public class Canvas3DImpl : Singleton<Canvas3DImpl>
{
	private class AutoAtlasInfo
	{
		...
		public void Rebuild()
		{
			...
			if (!flag)
			{
				...
				enumerator.MoveNext();
				Debug.LogError("Dynamic Combine Atlas Failed, maybe too many pictures of atlas tag:\"" + enumerator.Current.autoAtlasTag + "\"");
			}
		}
		...
		private bool Pack(int size)
		{
			...
			while (enumerator.MoveNext())
			{
				...
				int height = current2.Value.height;
				...
				arg_7E_0.Add(current3.Key, uVDetail);
			...
			while (enumerator.MoveNext())
			{
				...
				if (!arg_1F2_0.TryGetValue(current4.Key, out texture2D2))
				{
					texture2D2 = this.atlas;
				}
				KeyValuePair<int, AtlasInfo.UVDetail> current5 = enumerator.Current;
				AtlasInfo.UVDetail value = current5.Value;
				...
				AtlasInfo.UVDetail uVDetail2 = arg_228_0[current6.Key];
			...
			{
				enumerator2.Current.SetMaterial(this.mat);
				enumerator2.Current.SetAutoAtlas(this.atlas, this.textures[enumerator2.Current.m_textureGUID]);
			}
	...
	private void _DoRebuildAtlas()
	{
		...
		while (enumerator.MoveNext())
		{
			...
			Canvas3DImpl.AutoAtlasInfo value = current.Value;
