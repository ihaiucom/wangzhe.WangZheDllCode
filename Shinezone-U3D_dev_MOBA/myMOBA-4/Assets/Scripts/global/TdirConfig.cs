		...
		if (TdirConfig.tdirConfigData == null && File.Exists(Application.persistentDataPath + TdirConfig.tdirConfigDataPath))
		{
			try
			{
				...
				if (array != null && array.Length > 0)
				{
					...
					foreach (SecurityElement securityElement2 in securityElement.Children)
					{
						if (securityElement2.Tag == "serverType")
						{
							TdirConfig.tdirConfigData.serverType = int.Parse(securityElement2.Text);
						}
						else if (securityElement2.Tag == "versionType")
						{
							TdirConfig.tdirConfigData.versionType = int.Parse(securityElement2.Text);
						}
					}
				}
			}
