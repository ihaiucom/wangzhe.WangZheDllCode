...
internal class NewbieWeakGuideControl : Singleton<NewbieWeakGuideControl>
{
	...
	public bool TriggerWeakGuide(uint mainLineId, uint startIndex = 1u)
	{
		...
		int num;
		if (this.mWeakGuideTriggerTime.TryGetValue(newbieWeakGuideMainLineConf.dwID, out num) && CRoleInfo.GetCurrentUTCTime() - num < (int)newbieWeakGuideMainLineConf.bCDTime)
		{
			return true;
		}
		if (this.isGuiding)
	...
	public void RemoveEffectByHilight(GameObject hilighter)
	{
		if (hilighter != null)
		{
			...
			if (componentInParent != null)
			{
				...
				CUIEventScript expr_38 = componentInParent;
				expr_38.onClick = (CUIEventScript.OnUIEventHandler)Delegate.Remove(expr_38.onClick, new CUIEventScript.OnUIEventHandler(this.ClickHandler));
			}
			...
			if (componentInParent2 != null)
			{
				...
				CUIMiniEventScript expr_83 = componentInParent2;
				expr_83.onClick = (CUIMiniEventScript.OnUIEventHandler)Delegate.Remove(expr_83.onClick, new CUIMiniEventScript.OnUIEventHandler(this.ClickHandler));
			}
			this.RemoveEffect(weakGuideId);
		}
	}
	...
	private void ClickHandler(CUIEvent uiEvent)
	{
		...
		if (componentInParent != null)
		{
			CUIEventScript expr_48 = componentInParent;
			expr_48.onClick = (CUIEventScript.OnUIEventHandler)Delegate.Remove(expr_48.onClick, new CUIEventScript.OnUIEventHandler(this.ClickHandler));
		}
