...
public class CAddSkillView
{
	...
	public static void OpenForm(GameObject form)
	{
		...
		form.transform.Find("Panel_TopBg/LevelText").GetComponent<Text>().text = ((masterRoleInfo == null) ? Singleton<CTextManager>.instance.GetText("Added_Skill_Common_Tips_2", new string[]
		{
			"1"
		...
		for (int i = 0; i < num; i++)
		{
			...
			if (dataByKey != null)
			{
				...
				if (flag)
				{
					Utility.GetComponetInChild<Text>(elemenet.gameObject, "Lock/Text").text = Singleton<CTextManager>.instance.GetText("Added_Skill_Common_Tips_3", new string[]
					{
	...
	public static void OnRefresh(GameObject form, ushort addedSkillLevel)
	{
		...
		for (int i = 0; i < component.GetElementAmount(); i++)
		{
			...
			if (i == selected)
			{
				cUIListElementScript.transform.Find("SkillNameTxt").GetComponent<Text>().color = CAddSkillView.SELECTED_COLOR;
			}
			else
			{
				cUIListElementScript.transform.Find("SkillNameTxt").GetComponent<Text>().color = CAddSkillView.UN_SELECTED_COLOR;
			}
		}
		...
		if (flag)
		{
			component6.text = Singleton<CTextManager>.instance.GetText("Added_Skill_Common_Tips_3", new string[]
			{
		...
		{
			component6.text = Singleton<CTextManager>.instance.GetText("Added_Skill_Common_Tips_4");
		}
