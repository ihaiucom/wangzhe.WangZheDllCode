...
public class InBattleInputChat
{
	...
	public class InBatChatEntity
	{
		...
		public void Shade(StringBuilder builder)
		{
			...
			if ((int)this.colorEnd > this.line1.Length)
			{
			...
			if (this.type == InBattleInputChat.InBatChatEntity.EType.System)
			{
				...
				{
					this.bLine1Colored = this._shade(builder, ref this.line1, this.line1.Length, InBattleInputChat.InBatChatEntity.COLOR_SYSTEM);
				}
				if (!this.bLine2Colored && !string.IsNullOrEmpty(this.line2))
				{
					this.bLine2Colored = this._shade(builder, ref this.line2, this.line2.Length, InBattleInputChat.InBatChatEntity.COLOR_SYSTEM);
				}
			}
			else
			{
				if (!this.bLine1Colored && !string.IsNullOrEmpty(this.line1))
				{
					this.bLine1Colored = this._shade(builder, ref this.line1, (int)this.colorEnd, this.bAlice ? InBattleInputChat.InBatChatEntity.COLOR_OTHERS : InBattleInputChat.InBatChatEntity.COLOR_ENEMY);
				}
				if (!this.bLine2Colored && !string.IsNullOrEmpty(this.line2))
				{
					this.bLine2Colored = this._shade(builder, ref this.line2, this.line2.Length, InBattleInputChat.InBatChatEntity.COLOR_WHITE);
				}
			}
		}
		...
		private bool _shade(StringBuilder builder, ref string content, int endIndex, string color1)
		{
			...
			if (endIndex != content.Length)
			{
				...
				builder.Insert((int)this.colorEnd + color1.Length, "</color><color=#ffffff>");
	...
	public void Add(InBattleInputChat.InBatChatEntity ent)
	{
		...
		else
		{
			...
			{
				int characterWidth = CChatParser.GetCharacterWidth(ent.rawText[i], this.m_view.fontSize);
	...
	public InBattleInputChat.InBatChatEntity ConstructEnt(ulong ullUid, string playerName, string content, byte camp)
	{
		...
		{
			arg = playerByUid.Captain.handle.TheStaticData.TheResInfo.Name;
		...
		string text = string.Format("{0}{1}({2}):", arg2, playerName, arg);
		validChatEntity.colorEnd = (byte)text.Length;
	...
	public InBattleInputChat.InBatChatEntity ConstructEnt(string content, InBattleInputChat.InBatChatEntity.EType type = InBattleInputChat.InBatChatEntity.EType.System)
	{
		...
		validChatEntity.rawText = string.Format("{0} {1}", arg, content);
