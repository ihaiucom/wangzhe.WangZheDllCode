...
public class CTongCaiSys : MonoSingleton<CTongCaiSys>
{
    ...
    public IEnumerator CheckCanUseTongCai(ApolloAccountInfo info)
    {
        if (isOpenTongcai)
        {
            var timer = 0f;
            var failed = false;
            isTongCaiValid = false;
            var checkUrl = GetCheckUrl(info);
            var www = new WWW(checkUrl);
            while (!www.isDone)
            {
                if (timer <= timeOut)
                {
            ...
            {
                CheckDataString(www.text);
                isCanUseTongCai = true;
            }
            isChecked = true;
        }

        Debug.Log("*Login* tdir " + Time.realtimeSinceStartup);
	...
	public string GetCheckUrl(ApolloAccountInfo info)
	{
		...
		string text2 = (DateTime.Now.Ticks / 10000000L).ToString();
	...
	public void OpenTongCaiH5(ApolloAccountInfo info)
	{
		...
		string text3 = (DateTime.Now.Ticks / 10000000L).ToString();
		string input = string.Concat(new object[]
		{
			info.OpenId,
			(int)info.Platform,
