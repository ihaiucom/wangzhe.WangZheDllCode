...
public class CChatView
{
	...
	public class CChatViewEntryNode
	{
		...
		public void SetBubbleCount(int count)
		{
			...
			{
				this.entry_bubble_CountText.text = count.ToString();
			}
		}
		...
		private void Create_Content(GameObject pNode, string content, CChatParser.InfoType type, float x, float y, float width, bool bSelf, bool bVip = false, bool bUse_Entry = false, CUIFormScript form = null)
		{
			...
			if (type == CChatParser.InfoType.Text)
			{
				...
				if (bUse_Entry)
				{
					text.rectTransform.anchoredPosition = new Vector2(x, (float)(-(float)CChatParser.chat_entry_lineHeight));
				}
				else
				{
		...
		{
			GameObject gameObject = (GameObject)UnityEngine.Object.Instantiate(template);
	...
	public void Update()
	{
		...
		if (text != null && text.Length > CChatView.ChatMaxLength)
		{
			this.inputField.DeactivateInputField();
			this.inputField.text = text.Substring(0, CChatView.ChatMaxLength);
	...
	public void BuildTabList(List<uint> list, int index = 0)
	{
		...
		for (int i = 0; i < this.listScript.m_elementAmount; i++)
		{
			...
			component2.text = Singleton<CTextManager>.instance.GetText(CChatUT.GetEChatChannel_Text((EChatChannel)list[i]));
		}
	...
	public void ReBuildTabText()
	{
		...
		for (int i = 0; i < this.curChannels.Count; i++)
		{
			...
			component.text = Singleton<CTextManager>.instance.GetText(CChatUT.GetEChatChannel_Text((EChatChannel)this.curChannels[i]));
		}
	}
	...
	public void SetGuildRecruitListElement(CUIEvent uiEvent)
	{
		...
		if (uiEvent.m_srcWidgetIndexInBelongedList >= 0 && uiEvent.m_srcWidgetIndexInBelongedList < model.sysData.m_guildRecruitInfos.Count)
		{
			GuildRecruitInfo guildRecruitInfo = model.sysData.m_guildRecruitInfos[uiEvent.m_srcWidgetIndexInBelongedList];
			...
			if (transform != null)
			{
				...
				component2.text = guildRecruitInfo.senderLevel.ToString();
				component3.text = guildRecruitInfo.senderName;
				component4.text = Singleton<CTextManager>.GetInstance().GetText("Guild_Recruit_Info_Format", new string[]
				{
	...
	public void ShowDetailChatForm()
	{
		...
		for (int i = 0; i < this.curChannels.Count; i++)
		{
			if (this.curChannels[i] == (uint)this.CurTab)
			{
				index = i;
			}
		}
	...
	private void SortChannels()
	{
		for (int i = 0; i < this.curChannels.Count; i++)
		{
			EChatChannel eChatChannel = (EChatChannel)this.curChannels[i];
	...
	public void RefreshGuildRecruitList()
	{
		...
		component.SetElementAmount(model.sysData.m_guildRecruitInfos.Count);
	}

	public void RefreshGuildRecruitInfoNode()
	{
		this.info_node_obj.CustomSetActive(true);
		this.info_text.text = Singleton<CTextManager>.GetInstance().GetText("Guild_Recruit_Chat_Tip");
	}
	...
	public void On_Tab_Change(int index)
	{
		...
		if (index >= 0 && index < this.curChannels.Count)
		{
			this.CurTab = (EChatChannel)this.curChannels[index];
		}
	}
	...
	public void On_Friend_TabList_Selected(CUIEvent uiEvent)
	{
		...
		component.text = Utility.UTF8Convert(cOMDT_FRIEND_INFO.szUserName);
	}
	...
	public void Jump2FriendChat(COMDT_FRIEND_INFO info)
	{
		...
		if (num == -1)
		{
			...
			num = this.curChannels.Count - 1;
		}
		this.BuildTabList(this.curChannels, num);
		this.CurTab = EChatChannel.Friend_Chat;
		CUIListElementScript selectedElement = this.listScript.GetSelectedElement();
		Text component = selectedElement.gameObject.transform.FindChild("Text").GetComponent<Text>();
		component.text = Utility.UTF8Convert(info.szUserName);
	}
	...
	public void Refresh_ChatInputView()
	{
		...
		if (this.CurTab == EChatChannel.Lobby)
		{
			...
			{
				this.inputField.placeholder.GetComponent<Text>().text = Singleton<CTextManager>.instance.GetText("Chat_Common_Tips_1", new string[]
				{
	...
	public void ShowLoudSpeaker(bool bShow, COMDT_CHAT_MSG_HORN data = null)
	{
		...
		if (data != null)
		{
			...
			componetInChild3.text = Utility.UTF8Convert(data.szContent);
			componetInChild6.text = text3;
			...
			if (componetInChild3.preferredHeight > 60f)
			{
		...
		if (!this.bRefreshNew)
		{
			...
			if (unreadMeanbleChatEntCount > 0)
			{
				...
				if (component != null)
				{
					...
					{
						component.text = string.Format("{0}+", CChatView.max_bubble_num);
					}
					else
					{
	...
	private void calc_size(ListView<CChatEntity> data_list, List<Vector2> sizeVec)
	{
		...
		for (int i = 0; i < data_list.Count; i++)
		{
			...
			{
				float y = (cChatEntity.numLine <= 1u) ? CChatParser.element_height : (CChatParser.element_height + (float)CChatParser.lineHeight);
	...
	public void Show_FriendTabItem(GameObject node, COMDT_FRIEND_INFO info)
	{
		...
		if (last != null)
		{
			component3.text = last.text;
		}
		else
		{
			component3.text = Singleton<CTextManager>.instance.GetText("Chat_Common_Tips_3");
		}
		if (info.stGameVip != null && info.stGameVip.dwCurLevel > 0u)
		{
			component3.color = CUIUtility.s_Text_Color_Vip_Chat_Other;
		}
	}
