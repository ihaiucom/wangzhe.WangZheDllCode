...
public class FowLos
{
	...
	public struct SBlockContext
	{
		...
		public bool ContainsPoint(VInt2 point, out int index)
		{
			...
			{
				if (this.m_polylineVertices[i].IsNear(point))
				{
					index = i;
					return true;
				}
	...
	public void ExploreCellsInternal<TSetCellVisible>(ref TSetCellVisible setCellVisible, VInt2 newSurfPos, VInt3 unitLoc, int surfSightRange, GameFowManager pFowMgr, COM_PLAYERCAMP camp, FowLos.EViewExploringMode ViewExploringMode, bool bDrawDebugLines) where TSetCellVisible : ISetCellVisible
	{
		...
		if (ViewExploringMode != FowLos.EViewExploringMode.EViewExploringMode_ShadowCast)
		{
			if (ViewExploringMode == FowLos.EViewExploringMode.EViewExploringMode_DistOnly)
			{
				for (int i = -surfSightRange - 1; i <= surfSightRange + 1; i++)
				{
					for (int j = -surfSightRange - 1; j <= surfSightRange + 1; j++)
					{
						...
						if (pFowMgr.IsInsideSurface(inPos.x, inPos.y))
						{
							if (b.sqrMagnitude < surfSightRange * surfSightRange)
							{
			...
			else if (ViewExploringMode != FowLos.EViewExploringMode.EViewExploringMode_WatchTower)
			{
				if (ViewExploringMode == FowLos.EViewExploringMode.EViewExploringMode_RayCast)
				{
					...
					if (pFowMgr.m_pFieldObj.ViewBlockArrayImpl != null)
					{
						...
						{
							FowLos.SBlockWalls current = enumerator.Current;
							int areaId = (int)current.m_areaId;
							DebugHelper.Assert(areaId != 0);
							if ((int)viewBlockId != areaId || sViewBlockAttr.BlockType != 1)
							{
								FieldObj.SViewBlockAttr sViewBlockAttr2;
								if (pFowMgr.m_pFieldObj.GrassBlockView || !pFowMgr.m_pFieldObj.ViewBlockAttrMap.TryGetValue((byte)areaId, out sViewBlockAttr2) || sViewBlockAttr2.BlockType != 1)
								{
									if (this.ValidateViewBlock(current, cRaycastQuadrant.min.x, cRaycastQuadrant.max.x, cRaycastQuadrant.min.y, cRaycastQuadrant.max.y))
									{
										...
										{
											List<byte> arg_2D4_0 = cRaycastQuadrant.wallsHorizontal;
											KeyValuePair<byte, List<FowLos.SGridWall>> current2 = enumerator2.Current;
											BaseAlgorithm.AddUniqueItem<byte>(arg_2D4_0, current2.Key);
										}
										enumerator2 = current.m_wallsVertical.GetEnumerator();
										while (enumerator2.MoveNext())
										{
											List<byte> arg_310_0 = cRaycastQuadrant.wallsVertical;
											KeyValuePair<byte, List<FowLos.SGridWall>> current3 = enumerator2.Current;
											BaseAlgorithm.AddUniqueItem<byte>(arg_310_0, current3.Key);
										}
									}
								}
							}
		...
		for (int i = x; i <= x2; i++)
		{
			for (int j = y; j <= y2; j++)
			{
				...
				else if ((vInt - newSurfPos).sqrMagnitude < sightSqr)
				{
					...
					int num2 = (!flag4) ? (newSurfPos.x + 1) : newSurfPos.x;
					int num3 = (!flag5) ? (newSurfPos.y + 1) : newSurfPos.y;
					int num4 = 0;
					while (!flag && (!flag2 || !flag3))
					{
						if (!flag2)
						{
							...
							if (!flag2 && quadrant.wallsVertical.Contains((byte)num5))
							{
								...
								while (enumerator.MoveNext() && !flag)
								{
									...
									if (current.m_wallsVertical.TryGetValue((byte)num5, out list))
									{
										int count = list.Count;
										int num6 = 0;
										while (num6 < count && !flag)
										{
											FowLos.SGridWall sGridWall2 = list[num6];
											if (bDrawDebugLines)
											{
						...
						if (!flag3 && !flag)
						{
							...
							if (!flag3 && quadrant.wallsHorizontal.Contains((byte)num7))
							{
								...
								{
									FowLos.SBlockWalls current2 = enumerator2.Current;
									List<FowLos.SGridWall> list2 = null;
									if (current2.m_wallsHorizontal.TryGetValue((byte)num7, out list2))
									{
										int count2 = list2.Count;
										int num8 = 0;
										while (num8 < count2 && !flag)
										{
											FowLos.SGridWall sGridWall3 = list2[num8];
											if (bDrawDebugLines)
											{
