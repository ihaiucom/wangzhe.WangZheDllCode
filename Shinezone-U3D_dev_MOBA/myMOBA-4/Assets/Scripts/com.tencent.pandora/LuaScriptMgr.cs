...
namespace com.tencent.pandora
{
	public class LuaScriptMgr
	{
		...
		public LuaScriptMgr()
		{
			UnityEngine.Debug.Log("LuaScriptMgr ctor");
		...
		public void Start()
		{
			...
			foreach (Type current in LuaScriptMgr.checkBaseType)
			{
				Debug.LogWarning("BaseType " + current.FullName + " not register to lua");
			}
		...
		public void Destroy()
		{
			...
			LuaDLL.lua_gc(this.lua.L, LuaGCOptions.LUA_GCCOLLECT, 0);
			foreach (KeyValuePair<string, LuaBase> current in this.dict)
			{
		...
		{
			LuaBase luaBase = null;
			if (!this.dict.TryGetValue(tableName, out luaBase))
			{
				IntPtr l = this.lua.L;
				int newTop = LuaDLL.lua_gettop(l);
		...
		{
			object rawNetObject = LuaScriptMgr.GetTranslator(L).getRawNetObject(L, stackPos);
			UnityEngine.Object x = (UnityEngine.Object)rawNetObject;
			if (x == null)
			{
				LuaDLL.luaL_argerror(L, stackPos, string.Format("{0} expected, got nil", type));
				return null;
		...
		public static void PushVarObject(IntPtr L, object o)
		{
			...
			{
				UnityEngine.Object x = (UnityEngine.Object)o;
				if (x == null)
				{
					LuaDLL.lua_pushnil(L);
				}
		...
		{
			LuaScriptMgr.PushObject(L, (!(obj == null)) ? obj : null);
		}
		...
		private static void PushMetaObject(IntPtr L, ObjectTranslator translator, object o, int metaRef)
		{
			...
			int num = -1;
			bool flag = translator.objectsBackMap.TryGetValue(o, out num);
			if (flag)
			{
		...
		private static bool CheckTableType(IntPtr L, Type t, int stackPos)
		{
			...
			if (t.IsValueType)
			{
				...
				string a = LuaDLL.lua_tostring(L, -1);
				LuaDLL.lua_settop(L, newTop);
				if (a == "Vector3")
				{
		...
		public static T[] GetArrayObject<T>(IntPtr L, int stackPos)
		{
			...
			if (luaTypes == LuaTypes.LUA_TTABLE)
			{
				...
				while (true)
				{
					...
					T item = (T)((object)LuaScriptMgr.GetVarObject(L, -1));
					list.Add(item);
					LuaDLL.lua_pop(L, 1);
					num++;
				}
		...
		private static string GetErrorFunc(int skip)
		{
			...
			string arg = text.Substring(num2 + 1, num3 - num2 - 1);
			return string.Format("{0}.{1}", arg, frame.GetMethod().Name);
		}
		...
		public static T[] GetArrayNumber<T>(IntPtr L, int stackPos)
		{
			...
			if (luaTypes == LuaTypes.LUA_TTABLE)
			{
				...
				while (true)
				{
					...
					T item = (T)((object)Convert.ChangeType(LuaDLL.lua_tonumber(L, -1), typeof(T)));
					list.Add(item);
					LuaDLL.lua_pop(L, 1);
					num++;
				}
		...
		public static object GetVarObject(IntPtr L, int stackPos)
		{
			...
			case LuaTypes.LUA_TUSERDATA:
			{
				...
				{
					object result = null;
					LuaScriptMgr.GetTranslator(L).objects.TryGetValue(num, out result);
					return result;
				}
				return null;
			}
		...
		public static int IndexArray(IntPtr L)
		{
			...
			else if (luaTypes == LuaTypes.LUA_TSTRING)
			{
				...
				if (luaString == "Length")
				{
					LuaScriptMgr.Push(L, array.Length);
				}
			}
			return 1;
		}
		...
		public static int NewIndexArray(IntPtr L)
		{
			...
			object value = LuaScriptMgr.GetVarObject(L, 3);
		...
		{
			object obj = null;
			if (!LuaScriptMgr.enumMap.TryGetValue(e, out obj))
			{
				obj = e;
		...
		public static void Push(IntPtr L, Enum e)
		{
			...
			int num = -1;
			bool flag = translator.objectsBackMap.TryGetValue(enumObj, out num);
			if (flag)
			{
				if (LuaDLL.tolua_pushudata(L, weakTableRef, num))
				{
