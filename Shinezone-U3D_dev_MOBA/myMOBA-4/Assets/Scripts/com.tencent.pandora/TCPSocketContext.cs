	...
	public class TCPSocketContext
	{
		...
		private int SplitReceiveDataToPackets()
		{
			...
			while (this.receivedLength > 0)
			{
				...
				if (num2 > 0)
				{
					...
					packet.theCreateTimeMS = (long)DateTime.Now.Millisecond;
		...
		public int HandleOutputEvent(out int sentDataSize)
		{
			...
			while (this.peekPacketSentSize < packet.theContent.Length)
			{
				...
				string text = Convert.ToBase64String(packet.theContent, this.peekPacketSentSize, length);
				int num = NetLib.PandoraNet_AsyncWrite(text.Length, text, this.theUniqueSocketId);
				if (num < 0)
				{
					if (num == -100)
					{
		...
		public int HandleClose(out int discardDataSize)
		{
			Logger.DEBUG("Z12");
			discardDataSize = 0;
			foreach (Packet packet in this.pendingPackets)
			{
