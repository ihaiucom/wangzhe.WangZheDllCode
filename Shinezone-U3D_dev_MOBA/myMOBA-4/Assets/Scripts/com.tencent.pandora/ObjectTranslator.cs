...
namespace com.tencent.pandora
{
	public class ObjectTranslator
	{
		...
		public void Destroy()
		{
			...
			foreach (KeyValuePair<Type, int> current in ObjectTranslator.typeMetaMap)
			{
		...
		{
			foreach (Assembly current in this.assemblies)
			{
		...
		{
			object obj;
			bool flag = this.objects.TryGetValue(udata, out obj);
			if (flag)
			{
		...
		internal object getRawNetObject(IntPtr luaState, int index)
		{
			...
			object result = null;
			this.objects.TryGetValue(key, out result);
			return result;
		}
