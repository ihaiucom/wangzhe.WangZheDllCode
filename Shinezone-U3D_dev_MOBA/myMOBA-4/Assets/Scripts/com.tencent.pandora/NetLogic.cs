	...
	public class NetLogic
	{
		...
		public void Drive()
		{
			...
			while (this.resultQueue.Count > 0)
			{
				...
				{
					message.action(message.status, message.content);
				}
			}
			...
			while (this.logicDriveQueue.Count > 0)
			{
				...
				{
					message2.action(message2.status, message2.content);
				}
			}
		...
		private void CheckAtmSession()
		{
			if (this.atmIPAddresses.Count == 0)
			{
				...
				Action<IPHostEntry> resultAction = delegate(IPHostEntry entry)
				{
					...
					if (entry != null && entry.AddressList.Length > 0)
					{
						...
						{
							IPAddress item = addressList[i];
							this.atmIPAddresses.Add(item);
						}
					}
					else
					{
						...
						{
							string ipString = array[j];
							this.atmIPAddresses.Add(IPAddress.Parse(ipString));
						}
					}
			...
			try
			{
				Action<int, Dictionary<string, object>> statusChangedAction = delegate(int status, Dictionary<string, object> content)
				{
					...
					if (status == 0)
					{
						this.atmUniqueSocketId = (long)content["uniqueSocketId"];
					}
					else
					{
						this.atmUniqueSocketId = -1L;
				...
				long num = this.SpawnTCPSession(this.atmIPAddresses[this.lastTryAtmIPAddressIndex], NetLogic.atmReportPort, new ATMHandler(statusChangedAction));
				if (num > 0L)
				{
					Logger.DEBUG(string.Empty);
		...
		private void CheckBrokerSession()
		{
			...
			try
			{
				Action<int, Dictionary<string, object>> statusChangedAction = delegate(int status, Dictionary<string, object> content)
				{
					...
					if (status == 0)
					{
						this.brokerUniqueSocketId = (long)content["uniqueSocketId"];
					}
					else
					{
						this.brokerUniqueSocketId = -1L;
				...
				long num = this.SpawnTCPSession(this.brokerIPAddresses[this.lastTryBorkerIPAddressIndex], this.brokerPort, new BrokerHandler(statusChangedAction, packetRecvdAction));
				if (num > 0L)
				{
			...
			Action<IPHostEntry> resultAction = delegate(IPHostEntry entry)
			{
				try
				{
					...
					if (entry != null && entry.AddressList.Length > 0)
					{
						...
						{
							IPAddress item = addressList[i];
							this.brokerIPAddresses.Add(item);
						}
					}
					else
					{
			...
			try
			{
				...
				Action<IPHostEntry> resultAction = delegate(IPHostEntry entry)
				{
					...
					if (entry != null && entry.AddressList.Length > 0)
					{
						...
						long num2 = this.SpawnTCPSession(addressList[num], (ushort)uri.Port, new ConfigHandler(action, configUrl));
						if (num2 > 0L)
						{
						...
						{
							Logger.ERROR(string.Empty);
							action(-1, new Dictionary<string, object>());
						}
					}
					else
					{
				...
				this.AsyncGetHostEntry(uri.Host, resultAction);
			}
			catch (Exception ex)
			{
				action(-1, new Dictionary<string, object>());
			...
			try
			{
				...
				Action<IPHostEntry> resultAction = delegate(IPHostEntry entry)
				{
					...
					if (entry != null && entry.AddressList.Length > 0)
					{
						IPAddress[] addressList = entry.AddressList;
						int num = new Random().Next(addressList.Length);
						long num2 = this.SpawnTCPSession(addressList[num], (ushort)uri.Port, new DownloadHandler(url, size, md5, destFile, curRedirectionTimes, action));
						if (num2 > 0L)
						{
							Logger.DEBUG(addressList[num].ToString());
						}
		...
		public void AsyncGetHostEntry(string host, Action<IPHostEntry> resultAction)
		{
			...
			try
			{
				...
				if (IPAddress.TryParse(host, out iPAddress))
				{
					...
					resultAction(iPHostEntry);
				}
				else
				{
					if (this.dnsCache.ContainsKey(host))
					{
						IPHostEntry obj = this.dnsCache[host];
						resultAction(obj);
						resultAction = null;
					}
					if (!this.hasUpdateEntryHosts.Contains(host))
					{
						Action<IAsyncResult> @object = delegate(IAsyncResult ar)
						{
							try
							{
								string text = (string)ar.AsyncState;
								Logger.DEBUG(text + " end");
								IPHostEntry entry = Dns.EndGetHostEntry(ar);
								Logger.DEBUG(text + " end, entry.AddressList.Length=" + entry.AddressList.Length.ToString());
								Action<int, Dictionary<string, object>> action = delegate(int status, Dictionary<string, object> content)
								{
									string text2 = content["host"] as string;
									IPHostEntry iPHostEntry2 = content["entry"] as IPHostEntry;
									Action<IPHostEntry> action2 = content["resultAction"] as Action<IPHostEntry>;
									this.hasUpdateEntryHosts.Add(text2);
									this.dnsCache[text2] = entry;
								...
								message.status = 0;
								message.content["host"] = text;
								message.content["entry"] = entry;
								message.content["resultAction"] = resultAction;
								message.action = action;
								this.EnqueueDrive(message);
							}
							catch (Exception ex2)
							{
								Logger.ERROR(ex2.Message + ":" + ex2.StackTrace);
							}
						};
						Logger.DEBUG(host + " begin");
						Dns.BeginGetHostEntry(host, new AsyncCallback(@object.Invoke), host);
					}
				}
			}
			catch (Exception ex)
		...
		public void StreamReport(string jsonData)
		{
			...
			try
			{
				...
				dictionary["seq_id"] = pendingRequest.seqNo;
			...
			try
			{
				...
				if (num == 1L)
				{
					...
					else
					{
						...
						string text = Json.Serialize(content["body"]);
						Dictionary<string, object> dictionary = Json.Deserialize(text) as Dictionary<string, object>;
						Dictionary<string, object> headDict = dictionary["head"] as Dictionary<string, object>;
				...
				{
					int num4 = (int)((long)content["cmd_id"]);
					uint callId2 = (uint)((long)content["seq_id"]);
					int num3 = num4;
					if (num3 != 1001)
					{
						if (num3 != 1002)
						{
							...
							else
							{
								...
								int num5 = (int)((long)dictionary3["ret"]);
								if (num5 != 0)
								{
		...
		{
			UserData userData = Pandora.Instance.GetUserData();
			headDict["access_token"] = userData.sAccessToken;
			headDict["acc_type"] = userData.sAcountType;
			Dictionary<string, object> dictionary = new Dictionary<string, object>();
			dictionary["head"] = headDict;
			Dictionary<string, object> dictionary2 = new Dictionary<string, object>();
			dictionary2["ret"] = 0;
			dictionary2["err_msg"] = string.Empty;
			dictionary2["resp"] = Json.Serialize(dictionary);
