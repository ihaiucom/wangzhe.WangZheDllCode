...
namespace Photon.Hive.Tests.Disconnected
{
    ...
    public class DummyPeer : IPhotonPeer
    {
        ...
        public int GetResponseListCount()
        {
            lock (this.syncRootResponse)
            {
                return this.responseList.Count;
            }
        }
        /// <summary>
        ///   The on disconnect.
        ...
        /// </returns>
        public bool WaitForNextEvents(int eventCount, int timeout)
        {
            while (GetEventListCount() < eventCount)
            {
                if (!WaitForNextEvent(timeout))
                {
                    return false;
                }
            }
        ...
        ///   true if all events received.
        /// </returns>
        public void AssertWaitForNextEvents(int eventCount, int timeout)
        {
            if (!WaitForNextEvents(eventCount, timeout)) {
                NUnit.Framework.Assert.AreEqual(eventCount, GetEventListCount());
            }
        }
